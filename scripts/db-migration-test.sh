...
docker run -v $(pwd)/tests:/sql -it --network host --rm mysql sh -c 'exec mysql -h 127.0.0.1 -P 3306 -uroot -pmostest -e "CREATE DATABASE migrated; CREATE DATABASE latest; GRANT ALL PRIVILEGES ON migrated.* TO mmuser; GRANT ALL PRIVILEGES ON latest.* TO mmuser"'

echo "Importing mysql dump from version 4.10"
docker run -v $(pwd)/tests:/sql -it --network host --rm mysql sh -c 'exec mysql -h 127.0.0.1 -P 3306 -D migrated -uroot -pmostest < /sql/mm4.10-dump.sql'
...
docker run -v $(pwd)/tests:/sql -it --network host --rm mysql sh -c 'exec mysql -h 127.0.0.1 -P 3306 -uroot -pmostest -e "DROP DATABASE migrated; DROP DATABASE latest"'
