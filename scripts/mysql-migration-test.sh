...
cat $TMPDIR/config.json | \
    ...
    jq '.SqlSettings.DriverName = "mysql"' > $TMPDIR/config.json.tmp && mv $TMPDIR/config.json.tmp $TMPDIR/config.json
...
cat $TMPDIR/config.json | \
    jq '.SqlSettings.DataSource = "mmuser:mostest@tcp(localhost:3306)/latest?charset=utf8mb4,utf8&readTimeout=30s&writeTimeout=30s"' > $TMPDIR/config.json.tmp && mv $TMPDIR/config.json.tmp $TMPDIR/config.json
