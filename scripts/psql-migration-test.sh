...
docker exec $NETWORK $CONTAINER sh -c 'exec echo "CREATE DATABASE migrated; CREATE DATABASE latest;" | exec psql -U mmuser mattermost_test'

echo "Importing postgres dump from version 5.0"
docker exec $NETWORK  -i $CONTAINER psql -U mmuser -d migrated < $(pwd)/scripts/mattermost-postgresql-5.0.sql
...
docker exec  $NETWORK $CONTAINER pg_dump --schema-only -d migrated -U mmuser > $DUMPDIR/migrated.sql
docker exec  $NETWORK $CONTAINER pg_dump --schema-only -d latest -U mmuser > $DUMPDIR/latest.sql

echo "Removing databases created for db comparison"
docker exec  $NETWORK $CONTAINER sh -c 'exec echo "DROP DATABASE migrated; DROP DATABASE latest;" | exec psql -U mmuser mattermost_test'
