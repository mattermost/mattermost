...
func fixable() {
	mlog.Debug("message",
		mlog.String("userId", ""),             // want `mlog field key "userId" is not snake_case, use "user_id"`
		mlog.String("UserID", ""),             // want `mlog field key "UserID" is not snake_case, use "user_id"`
		mlog.Int("perPage", 0),                // want `mlog field key "perPage" is not snake_case, use "per_page"`
		mlog.String("ClusterDiscoveryID", ""), // want `mlog field key "ClusterDiscoveryID" is not snake_case, use "cluster_discovery_id"`
		mlog.String("requestURLPath", ""),     // want `mlog field key "requestURLPath" is not snake_case, use "request_url_path"`
		mlog.String("Diff", ""),               // want `mlog field key "Diff" is not snake_case, use "diff"`
		mlog.String("channel name", ""),       // want `mlog field key "channel name" is not snake_case, use "channel_name"`
		mlog.String("features.users", ""),     // want `mlog field key "features.users" is not snake_case, use "features_users"`
		mlog.String("user-id", ""),            // want `mlog field key "user-id" is not snake_case, use "user_id"`
