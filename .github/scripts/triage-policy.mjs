...
function inspect(source, name) {
    ...
    const result = {assertions: [], assertionScopes: [], tests: [], bans: [], settings: [], semantic: []};
    ...
    function visit(n, scope = '<module>') {
        ...
        if (ts.isCallExpression(n)) {
            ...
            if ((/(^|\.)(expect|assert)(\(|\.|$)/.test(c) || /\.(should|and)$/.test(c)) && !(ts.isPropertyAccessExpression(n.parent) || ts.isElementAccessExpression(n.parent))) { result.assertions.push(text); result.assertionScopes.push(scope); }
            if (/(^|\.)(it|test|describe|context)(\.(serial|parallel))?$/.test(c) && n.arguments.length) {
