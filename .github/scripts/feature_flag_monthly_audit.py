...
def find_governing_esr(ship_version: str, esrs: list[dict]) -> tuple[dict | None, bool]:
    """
    Return (governing_esr, predates_earliest).

    The governing ESR is the most recent ESR whose version is <= ship_version:
        "10.12" → ESR "10.11"
        "11.7"  → ESR "11.7"

    If the flag shipped BEFORE the earliest ESR we track (e.g. "9.8"), it is by
    definition older than every ESR in our records — the ESR that actually
    shipped it has long since been superseded and EOL'd. Rather than dropping
    the flag (which would silently hide the oldest, most overdue flags), we
    fall back to the earliest known ESR and set predates_earliest=True.
    """
    if not esrs:
        return None, False
    ...
    if governing is None:
        # Predates every tracked ESR — attribute to the earliest one.
        return esrs[0], True
...
def create_jira_ticket(entry: dict, project_key: str, auth: str, base_url: str) -> str:
    ...
    predates = entry.get("predates", False)

    if predates:
        lifecycle_text = (
            f" first shipped as enabled in v{ship_version}, which predates the earliest "
            f"ESR currently tracked (v{esr['version']}). It is therefore older than every "
            f"tracked ESR and is overdue for removal."
        )
    else:
        lifecycle_text = (
...
def main():
    ...
    if dry_run:
        ...
        if not all([jira_email, jira_token]):
            ...
            print("       (or pass --dry-run to test detection logic without Jira)")
            sys.exit(1)
    ...
    for flag in enabled_flags:
        ...
        if ship_version is None:
            print(f"  {flag}: enabled {enable_date.date()}, no release tag yet — not shipped, skipping")
            skipped.append((flag, f"enabled {enable_date.date()}, not yet in a release"))
            continue

        esr, predates = find_governing_esr(ship_version, esrs)
        if esr is None:
            print(f"  {flag}: no ESRs detected — skipping")
            skipped.append((flag, "no ESRs detected"))
            continue

        note = " [predates earliest tracked ESR]" if predates else ""

        if esr["_eol_date"] > today:
            days_left = (esr["_eol_date"] - today).days
            print(f"  {flag}: v{ship_version} → ESR v{esr['version']}, EOL {esr['eol']} "
                  f"({days_left}d away) — not yet eligible{note}")
            upcoming.append((flag, ship_version, esr, days_left))
            continue
        ...
        print(f"  {flag}: v{ship_version} → ESR v{esr['version']}, EOL {esr['eol']} — ELIGIBLE{note}")
        eligible.append({
            "flag": flag,
    ...
    print(f"\n{'='*60}")
    print(f"{len(eligible)} eligible · {len(upcoming)} upcoming · {len(skipped)} skipped")
    print(f"{'='*60}")

    if upcoming:
        print("\nUpcoming (soonest first) — no action needed yet:")
        for flag, sv, esr, days_left in sorted(upcoming, key=lambda x: x[3]):
            print(f"  {esr['eol']}  ({days_left:>4}d)  {flag}  (v{sv} → ESR v{esr['version']})")
    ...
    if not eligible:
        print("\nNo flags are eligible for removal this run.")
        return
    ...
    if dry_run:
        ...
        for entry in eligible:
            esr = entry["esr"]
            priority = "High" if entry["removal_comment"] else "Normal"
            print(f"\n  Summary:  Remove feature flag `{entry['flag']}` "
                  f"(shipped in v{entry['ship_version']}, ESR v{esr['version']} EOL)")
            print(f"  Project:  {project_key}   Type: Task   Priority: {priority}")
            print(f"  Labels:   {JIRA_LABEL}")
            print(f"  ESR EOL:  {esr['eol']}")
        print(f"\n(Dry run — duplicate detection not performed, no Jira calls made.)")
        return
