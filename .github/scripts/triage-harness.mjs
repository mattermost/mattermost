...
export function reportTests(framework, report, project) {
    const tests = [];
    if (framework === 'playwright') {
        ...
        function walk(suite, titles = []) {
            ...
            for (const spec of suite.specs || []) for (const test of spec.tests || []) if (test.projectName === project) {
                tests.push({title: [...path, spec.title].join(' > '), state: test.results?.[0]?.status, attempts: test.results?.length || 0, retry: test.results?.[0]?.retry, errors: test.results?.flatMap(r => r.errors || []) || []});
            }
        ...
        function walk(suite) {
            for (const test of suite.tests || []) tests.push({title: test.fullTitle, state: test.state, attempts: test.attempts?.length || 0, retry: 0, errors: test.err?.message ? [test.err.message] : []});
...
export function verifyClean(runs, baseline, target, requiredRuns) {
    ...
    const identities = baseline.map(t => t.title).sort();
    invariant(identities.includes(target), 'Claimed failed test missing from reproduction');
    for (const tests of runs) {
        invariant(tests.length > 0 && JSON.stringify(tests.map(t => t.title).sort()) === JSON.stringify(identities), 'Verification test identities changed or became empty');
...
export async function harness(item, cwd, output, signal) {
    ...
    return {candidate, async apply(source) { await writeFile(candidate, source); }, async execute(label) {
        ...
        try {
            ...
            else command = ['node', 'node_modules/cypress/bin/cypress', 'run', '--config-file', '.triage.config.ts', '--browser', 'electron', '--config', 'retries=0', '--reporter', 'cypress-multi-reporters', '--reporter-options', 'configFile=reporter-config.json', '--spec', spec];
            ...
            const tests = reportTests(item.framework, report, item.project);
            invariant(result.code === 0 || tests.some(t => t.state !== 'passed'), 'Runner failed without test failure (infrastructure remains red)');
            return tests;
