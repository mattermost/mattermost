...
PR publication uses one deterministic branch per queue item. Each GitHub mutation checks live lease ownership and is cancellable. Uncertain ref/PR responses are reconciled with GET rather than blindly creating another PR. Once a PR exists its receipt is persisted before the reviewer request, so a review-request failure does not requeue it. Unresolved publication uncertainty stays visible for reconciliation. Branches are based on the recorded test commit; if master's copy of that test changed meanwhile, publication stops for human rebase/revalidation.
