...
type Actions struct {
	...
	AddUserToChannel       func(*model.User, *model.Channel, bool, *model.SidebarCategoryWithChannels) (*model.ChannelMember, *model.AppError)
...
func (si *SlackImporter) addSlackUsersToChannel(members []string, users map[string]*model.User, channel *model.Channel, log *bytes.Buffer) {
	for _, member := range members {
		...
		if _, err := si.actions.AddUserToChannel(user, channel, false, nil); err != nil {
