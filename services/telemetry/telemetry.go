...
func (ds *TelemetryService) ensureTelemetryID() {
	if ds.TelemetryID != "" {
		return
	}
	...
	if len(id) == 0 {
		...
		ds.dbStore.System().Save(systemID)
	}
...
func (ds *TelemetryService) trackActivity() {
	var userCount int64
	...
	go func() {
		count, err := ds.dbStore.User().AnalyticsActiveCount(DAY_MILLISECONDS, model.UserCountOptions{IncludeBotAccounts: false, IncludeDeleted: false})
	...
	go func() {
		count, err := ds.dbStore.User().AnalyticsActiveCount(MONTH_MILLISECONDS, model.UserCountOptions{IncludeBotAccounts: false, IncludeDeleted: false})
		activeUsersMonthlyCountChan <- store.StoreResult{Data: count, Err: err}
		close(activeUsersMonthlyCountChan)
	}()

	if count, err := ds.dbStore.User().Count(model.UserCountOptions{IncludeDeleted: true}); err == nil {
		userCount = count
	}

	if count, err := ds.dbStore.User().AnalyticsGetGuestCount(); err == nil {
		guestAccountsCount = count
	}

	if count, err := ds.dbStore.User().Count(model.UserCountOptions{IncludeBotAccounts: true, ExcludeRegularUsers: true}); err == nil {
		botAccountsCount = count
	}

	if iucr, err := ds.dbStore.User().AnalyticsGetInactiveUsersCount(); err == nil {
		inactiveUserCount = iucr
	}

	teamCount, err := ds.dbStore.Team().AnalyticsTeamCount(false)
	if err != nil {
		mlog.Error(err.Error())
	}

	if ucc, err := ds.dbStore.Channel().AnalyticsTypeCount("", "O"); err == nil {
		publicChannelCount = ucc
	}

	if pcc, err := ds.dbStore.Channel().AnalyticsTypeCount("", "P"); err == nil {
		privateChannelCount = pcc
	}

	if dcc, err := ds.dbStore.Channel().AnalyticsTypeCount("", "D"); err == nil {
		directChannelCount = dcc
	}

	if duccr, err := ds.dbStore.Channel().AnalyticsDeletedTypeCount("", "O"); err == nil {
		deletedPublicChannelCount = duccr
	}

	if dpccr, err := ds.dbStore.Channel().AnalyticsDeletedTypeCount("", "P"); err == nil {
		deletedPrivateChannelCount = dpccr
	}

	postsCount, _ = ds.dbStore.Post().AnalyticsPostCount("", false, false)

	postCountsOptions := &model.AnalyticsPostCountsOptions{TeamId: "", BotsOnly: false, YesterdayOnly: true}
	postCountsYesterday, _ := ds.dbStore.Post().AnalyticsPostCountsByDay(postCountsOptions)
	...
	botPostCountsYesterday, _ := ds.dbStore.Post().AnalyticsPostCountsByDay(postCountsOptions)
	botPostsCountPreviousDay = 0
	if len(botPostCountsYesterday) > 0 {
		botPostsCountPreviousDay = int64(botPostCountsYesterday[0].Value)
	}

	slashCommandsCount, _ = ds.dbStore.Command().AnalyticsCommandCount("")

	if c, err := ds.dbStore.Webhook().AnalyticsIncomingCount(""); err == nil {
		incomingWebhooksCount = c
	}

	outgoingWebhooksCount, _ = ds.dbStore.Webhook().AnalyticsOutgoingCount("")
	...
	ds.sendTelemetry(TRACK_ACTIVITY, map[string]interface{}{
...
func (ds *TelemetryService) trackConfig() {
	cfg := ds.srv.Config()
	ds.sendTelemetry(TRACK_CONFIG_SERVICE, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_TEAM, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_CLIENT_REQ, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_SQL, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_LOG, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_AUDIT, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_NOTIFICATION_LOG, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_PASSWORD, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_FILE, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_EMAIL, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_RATE, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_PRIVACY, map[string]interface{}{
		"show_email_address": cfg.PrivacySettings.ShowEmailAddress,
		"show_full_name":     cfg.PrivacySettings.ShowFullName,
	})

	ds.sendTelemetry(TRACK_CONFIG_THEME, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_OAUTH, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_SUPPORT, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_LDAP, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_COMPLIANCE, map[string]interface{}{
		"enable":       *cfg.ComplianceSettings.Enable,
		"enable_daily": *cfg.ComplianceSettings.EnableDaily,
	})

	ds.sendTelemetry(TRACK_CONFIG_LOCALIZATION, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_SAML, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_CLUSTER, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_METRICS, map[string]interface{}{
		"enable":             *cfg.MetricsSettings.Enable,
		"block_profile_rate": *cfg.MetricsSettings.BlockProfileRate,
	})

	ds.sendTelemetry(TRACK_CONFIG_NATIVEAPP, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_EXPERIMENTAL, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_ANALYTICS, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_ANNOUNCEMENT, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_ELASTICSEARCH, map[string]interface{}{
	...
	pluginsEnvironment := ds.srv.GetPluginsEnvironment()
	...
	ds.sendTelemetry(TRACK_CONFIG_PLUGIN, pluginConfigData)

	ds.sendTelemetry(TRACK_CONFIG_DATA_RETENTION, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_MESSAGE_EXPORT, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_DISPLAY, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_GUEST_ACCOUNTS, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_IMAGE_PROXY, map[string]interface{}{
	...
	ds.sendTelemetry(TRACK_CONFIG_BLEVE, map[string]interface{}{
...
func (ds *TelemetryService) trackLicense() {
	if license := ds.srv.License(); license != nil {
		...
		ds.sendTelemetry(TRACK_LICENSE, data)
	}
}

func (ds *TelemetryService) trackPlugins() {
	pluginsEnvironment := ds.srv.GetPluginsEnvironment()
	if pluginsEnvironment == nil {
		return
	}
	...
	pluginStates := ds.srv.Config().PluginSettings.PluginStates
	...
	ds.sendTelemetry(TRACK_PLUGINS, map[string]interface{}{
...
func (ds *TelemetryService) trackServer() {
	data := map[string]interface{}{
		"edition":          model.BuildEnterpriseReady,
		"version":          model.CurrentVersion,
		"database_type":    *ds.srv.Config().SqlSettings.DriverName,
		"operating_system": runtime.GOOS,
	}

	if scr, err := ds.dbStore.User().AnalyticsGetSystemAdminCount(); err == nil {
		data["system_admins"] = scr
	}

	if scr, err := ds.dbStore.GetDbVersion(); err == nil {
		data["database_version"] = scr
	}

	ds.sendTelemetry(TRACK_SERVER, data)
}

func (ds *TelemetryService) trackPermissions() {
	phase1Complete := false
	if _, err := ds.dbStore.System().GetByName(model.ADVANCED_PERMISSIONS_MIGRATION_KEY); err == nil {
		phase1Complete = true
	}

	phase2Complete := false
	if _, err := ds.dbStore.System().GetByName(model.MIGRATION_KEY_ADVANCED_PERMISSIONS_PHASE_2); err == nil {
		phase2Complete = true
	}

	ds.sendTelemetry(TRACK_PERMISSIONS_GENERAL, map[string]interface{}{
		"phase_1_migration_complete": phase1Complete,
		"phase_2_migration_complete": phase2Complete,
	})

	systemAdminPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.SYSTEM_ADMIN_ROLE_ID); err == nil {
		systemAdminPermissions = strings.Join(role.Permissions, " ")
	}

	systemUserPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.SYSTEM_USER_ROLE_ID); err == nil {
		systemUserPermissions = strings.Join(role.Permissions, " ")
	}

	teamAdminPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.TEAM_ADMIN_ROLE_ID); err == nil {
		teamAdminPermissions = strings.Join(role.Permissions, " ")
	}

	teamUserPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.TEAM_USER_ROLE_ID); err == nil {
		teamUserPermissions = strings.Join(role.Permissions, " ")
	}

	teamGuestPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.TEAM_GUEST_ROLE_ID); err == nil {
		teamGuestPermissions = strings.Join(role.Permissions, " ")
	}

	channelAdminPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.CHANNEL_ADMIN_ROLE_ID); err == nil {
		channelAdminPermissions = strings.Join(role.Permissions, " ")
	}

	channelUserPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.CHANNEL_USER_ROLE_ID); err == nil {
		channelUserPermissions = strings.Join(role.Permissions, " ")
	}

	channelGuestPermissions := ""
	if role, err := ds.srv.GetRoleByName(model.CHANNEL_GUEST_ROLE_ID); err == nil {
		channelGuestPermissions = strings.Join(role.Permissions, " ")
	}

	ds.sendTelemetry(TRACK_PERMISSIONS_SYSTEM_SCHEME, map[string]interface{}{
	...
	if schemes, err := ds.srv.GetSchemes(model.SCHEME_SCOPE_TEAM, 0, 100); err == nil {
		for _, scheme := range schemes {
			teamAdminPermissions := ""
			if role, err := ds.srv.GetRoleByName(scheme.DefaultTeamAdminRole); err == nil {
				teamAdminPermissions = strings.Join(role.Permissions, " ")
			}

			teamUserPermissions := ""
			if role, err := ds.srv.GetRoleByName(scheme.DefaultTeamUserRole); err == nil {
				teamUserPermissions = strings.Join(role.Permissions, " ")
			}

			teamGuestPermissions := ""
			if role, err := ds.srv.GetRoleByName(scheme.DefaultTeamGuestRole); err == nil {
				teamGuestPermissions = strings.Join(role.Permissions, " ")
			}

			channelAdminPermissions := ""
			if role, err := ds.srv.GetRoleByName(scheme.DefaultChannelAdminRole); err == nil {
				channelAdminPermissions = strings.Join(role.Permissions, " ")
			}

			channelUserPermissions := ""
			if role, err := ds.srv.GetRoleByName(scheme.DefaultChannelUserRole); err == nil {
				channelUserPermissions = strings.Join(role.Permissions, " ")
			}

			channelGuestPermissions := ""
			if role, err := ds.srv.GetRoleByName(scheme.DefaultChannelGuestRole); err == nil {
				channelGuestPermissions = strings.Join(role.Permissions, " ")
			}

			count, _ := ds.dbStore.Team().AnalyticsGetTeamCountForScheme(scheme.Id)

			ds.sendTelemetry(TRACK_PERMISSIONS_TEAM_SCHEMES, map[string]interface{}{
				"scheme_id":                 scheme.Id,
				"team_admin_permissions":    teamAdminPermissions,
				"team_user_permissions":     teamUserPermissions,
				"team_guest_permissions":    teamGuestPermissions,
...
func (ds *TelemetryService) trackElasticsearch() {
	data := map[string]interface{}{}

	for _, engine := range ds.searchEngine.GetActiveEngines() {
		if engine.GetVersion() != 0 && engine.GetName() == "elasticsearch" {
			data["elasticsearch_server_version"] = engine.GetVersion()
		}
	}

	ds.sendTelemetry(TRACK_ELASTICSEARCH, data)
}

func (ds *TelemetryService) trackGroups() {
	groupCount, err := ds.dbStore.Group().GroupCount()
	if err != nil {
		mlog.Error(err.Error())
	}

	groupTeamCount, err := ds.dbStore.Group().GroupTeamCount()
	if err != nil {
		mlog.Error(err.Error())
	}

	groupChannelCount, err := ds.dbStore.Group().GroupChannelCount()
	if err != nil {
		mlog.Error(err.Error())
	}

	groupSyncedTeamCount, err := ds.dbStore.Team().GroupSyncedTeamCount()
	if err != nil {
		mlog.Error(err.Error())
	}

	groupSyncedChannelCount, err := ds.dbStore.Channel().GroupSyncedChannelCount()
	if err != nil {
		mlog.Error(err.Error())
	}

	groupMemberCount, err := ds.dbStore.Group().GroupMemberCount()
	if err != nil {
		mlog.Error(err.Error())
	}

	distinctGroupMemberCount, err := ds.dbStore.Group().DistinctGroupMemberCount()
	if err != nil {
		mlog.Error(err.Error())
	}

	groupCountWithAllowReference, err := ds.dbStore.Group().GroupCountWithAllowReference()
	if err != nil {
		mlog.Error(err.Error())
	}

	ds.sendTelemetry(TRACK_GROUPS, map[string]interface{}{
...
func (ds *TelemetryService) trackChannelModeration() {
	channelSchemeCount, err := ds.dbStore.Scheme().CountByScope(model.SCHEME_SCOPE_CHANNEL)
	if err != nil {
		mlog.Error(err.Error())
	}

	createPostUser, err := ds.dbStore.Scheme().CountWithoutPermission(model.SCHEME_SCOPE_CHANNEL, model.PERMISSION_CREATE_POST.Id, model.RoleScopeChannel, model.RoleTypeUser)
	if err != nil {
		mlog.Error(err.Error())
	}

	createPostGuest, err := ds.dbStore.Scheme().CountWithoutPermission(model.SCHEME_SCOPE_CHANNEL, model.PERMISSION_CREATE_POST.Id, model.RoleScopeChannel, model.RoleTypeGuest)
	if err != nil {
		mlog.Error(err.Error())
	}
	...
	postReactionsUser, err := ds.dbStore.Scheme().CountWithoutPermission(model.SCHEME_SCOPE_CHANNEL, model.PERMISSION_ADD_REACTION.Id, model.RoleScopeChannel, model.RoleTypeUser)
	if err != nil {
		mlog.Error(err.Error())
	}

	postReactionsGuest, err := ds.dbStore.Scheme().CountWithoutPermission(model.SCHEME_SCOPE_CHANNEL, model.PERMISSION_ADD_REACTION.Id, model.RoleScopeChannel, model.RoleTypeGuest)
	if err != nil {
		mlog.Error(err.Error())
	}
	...
	manageMembersUser, err := ds.dbStore.Scheme().CountWithoutPermission(model.SCHEME_SCOPE_CHANNEL, model.PERMISSION_MANAGE_PUBLIC_CHANNEL_MEMBERS.Id, model.RoleScopeChannel, model.RoleTypeUser)
	if err != nil {
		mlog.Error(err.Error())
	}

	useChannelMentionsUser, err := ds.dbStore.Scheme().CountWithoutPermission(model.SCHEME_SCOPE_CHANNEL, model.PERMISSION_USE_CHANNEL_MENTIONS.Id, model.RoleScopeChannel, model.RoleTypeUser)
	if err != nil {
		mlog.Error(err.Error())
	}

	useChannelMentionsGuest, err := ds.dbStore.Scheme().CountWithoutPermission(model.SCHEME_SCOPE_CHANNEL, model.PERMISSION_USE_CHANNEL_MENTIONS.Id, model.RoleScopeChannel, model.RoleTypeGuest)
	if err != nil {
		mlog.Error(err.Error())
	}

	ds.sendTelemetry(TRACK_CHANNEL_MODERATION, map[string]interface{}{
...
func (ds *TelemetryService) initRudder(endpoint string) {
	if ds.rudderClient == nil {
		...
		config.Logger = rudder.StdLogger(ds.log.StdLog(mlog.String("source", "rudder")))
		...
		ds.rudderClient = client
	}
}

func (ds *TelemetryService) doTelemetryIfNeeded(firstRun time.Time) {
	...
	if hoursSinceFirstServerRun < 1 {
		ds.doTelemetry()
	} else if hoursSinceFirstServerRun <= 12 && time.Since(ds.timestampLastTelemetrySent) >= time.Hour {
		ds.doTelemetry()
	} else if hoursSinceFirstServerRun > 12 && time.Since(ds.timestampLastTelemetrySent) >= 24*time.Hour {
		ds.doTelemetry()
	}
}

func (ds *TelemetryService) RunTelemetryJob(firstRun int64) {
	// Send on boot
	ds.doTelemetry()
	model.CreateRecurringTask("Telemetry", func() {
		ds.doTelemetryIfNeeded(utils.TimeFromMillis(firstRun))
	}, time.Minute*10)
}

func (ds *TelemetryService) doTelemetry() {
	if *ds.srv.Config().LogSettings.EnableDiagnostics {
		ds.timestampLastTelemetrySent = time.Now()
		ds.sendDailyTelemetry(false)
	}
}
