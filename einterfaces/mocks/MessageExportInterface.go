...
// RunExport provides a mock function with given fields: format, since, batchSize
func (_m *MessageExportInterface) RunExport(format string, since int64, batchSize int) (int64, *model.AppError) {
	ret := _m.Called(format, since, batchSize)

	var r0 int64
	if rf, ok := ret.Get(0).(func(string, int64, int) int64); ok {
		r0 = rf(format, since, batchSize)
	} else {
		r0 = ret.Get(0).(int64)
	}
