...
func (_m *SamlInterface) DoLogin(c einterfaces.AppContextInterface, encodedXML string, relayState map[string]string) (*model.User, *model.AppError) {
	ret := _m.Called(c, encodedXML, relayState)

	var r0 *model.User
	if rf, ok := ret.Get(0).(func(einterfaces.AppContextInterface, string, map[string]string) *model.User); ok {
	...
	var r1 *model.AppError
	if rf, ok := ret.Get(1).(func(einterfaces.AppContextInterface, string, map[string]string) *model.AppError); ok {
