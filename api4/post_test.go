...
func TestGetPostsForChannel(t *testing.T) {
	...
	posts, _, err = th.SystemAdminClient.GetPostsForChannel(th.BasicChannel.Id, 0, 60, "", false, true)
	require.NoError(t, err)
	require.Len(t, posts.Order, 12, "expected 12 posts")
	...
	// is deleted post included /posts?after={post7.Id}&includeDeleted=true
	...
	posts, _, err = th.SystemAdminClient.GetPostsBefore(th.BasicChannel.Id, post7.Id, 0, 60, "", false, false)
	require.NoError(t, err)
	require.Len(t, posts.Order, 7, "expected 7 posts")
	// is deleted post included /posts?before={post7.Id}&includeDeleted=true
