...
func (api *API) InitPost() {
	...
	api.BaseRoutes.Post.Handle("", api.APISessionRequiredWithDenyScope(deletePost)).Methods("DELETE")
	api.BaseRoutes.Posts.Handle("/ephemeral", api.APISessionRequiredWithDenyScope(createEphemeralPost)).Methods("POST")
	api.BaseRoutes.Post.Handle("/thread", api.APISessionRequiredWithDenyScope(getPostThread)).Methods("GET")
	api.BaseRoutes.Post.Handle("/files/info", api.APISessionRequiredWithDenyScope(getFileInfosForPost)).Methods("GET")
	api.BaseRoutes.PostsForChannel.Handle("", api.APISessionRequiredWithDenyScope(getPostsForChannel)).Methods("GET")
	api.BaseRoutes.PostsForUser.Handle("/flagged", api.APISessionRequiredWithDenyScope(getFlaggedPostsForUser)).Methods("GET")

	api.BaseRoutes.ChannelForUser.Handle("/posts/unread", api.APISessionRequiredWithDenyScope(getPostsForChannelAroundLastUnread)).Methods("GET")

	api.BaseRoutes.Team.Handle("/posts/search", api.APISessionRequiredDisableWhenBusy(searchPostsInTeam, model.ScopeAny(model.ScopeSearchPosts))).Methods("POST")
	api.BaseRoutes.Posts.Handle("/search", api.APISessionRequiredDisableWhenBusy(searchPostsInAllTeams, model.ScopeAny(model.ScopeSearchPosts))).Methods("POST")
	api.BaseRoutes.Post.Handle("", api.APISessionRequiredWithDenyScope(updatePost)).Methods("PUT")
	api.BaseRoutes.Post.Handle("/patch", api.APISessionRequiredWithDenyScope(patchPost)).Methods("PUT")
	api.BaseRoutes.PostForUser.Handle("/set_unread", api.APISessionRequiredWithDenyScope(setPostUnread)).Methods("POST")
	api.BaseRoutes.Post.Handle("/pin", api.APISessionRequiredWithDenyScope(pinPost)).Methods("POST")
	api.BaseRoutes.Post.Handle("/unpin", api.APISessionRequiredWithDenyScope(unpinPost)).Methods("POST")
}
