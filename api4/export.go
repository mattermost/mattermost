...
func (api *API) InitExport() {
	api.BaseRoutes.Exports.Handle("", api.APISessionRequiredWithDenyScope(listExports)).Methods("GET")
	api.BaseRoutes.Export.Handle("", api.APISessionRequiredWithDenyScope(deleteExport)).Methods("DELETE")
	api.BaseRoutes.Export.Handle("", api.APISessionRequiredWithDenyScope(downloadExport)).Methods("GET")
}
