...
func (api *API) InitUpload() {
	api.BaseRoutes.Uploads.Handle("", api.APISessionRequiredWithDenyScope(createUpload)).Methods("POST")
	api.BaseRoutes.Upload.Handle("", api.APISessionRequiredWithDenyScope(getUpload)).Methods("GET")
	api.BaseRoutes.Upload.Handle("", api.APISessionRequiredWithDenyScope(uploadData)).Methods("POST")
}
