...
func TestGetUserStatus(t *testing.T) {
	...
	t.Run("away status", func(t *testing.T) {
		th.App.SetStatusAwayIfNeeded(th.Context, th.BasicUser.Id, true)
		userStatus, _, err := client.GetUserStatus(th.BasicUser.Id, "")
		require.NoError(t, err)
	...
	t.Run("dnd status", func(t *testing.T) {
		th.App.SetStatusDoNotDisturb(th.Context, th.BasicUser.Id)
		userStatus, _, err := client.GetUserStatus(th.BasicUser.Id, "")
		require.NoError(t, err)
	...
	t.Run("dnd status timed", func(t *testing.T) {
		th.App.SetStatusDoNotDisturbTimed(th.Context, th.BasicUser.Id, time.Now().Add(10*time.Minute).Unix())
		userStatus, _, err := client.GetUserStatus(th.BasicUser.Id, "")
		require.NoError(t, err)
		assert.Equal(t, "dnd", userStatus.Status)
	})
	...
	t.Run("dnd status timed restore after time interval", func(t *testing.T) {
		task := model.CreateRecurringTaskFromNextIntervalTime("Unset DND Statuses From Test", func() { th.App.UpdateDNDStatusOfUsers(th.Context) }, 1*time.Second)
		defer task.Cancel()
		th.App.SetStatusOnline(th.Context, th.BasicUser.Id, true)
		userStatus, _, err := client.GetUserStatus(th.BasicUser.Id, "")
		require.NoError(t, err)
		assert.Equal(t, "online", userStatus.Status)
		th.App.SetStatusDoNotDisturbTimed(th.Context, th.BasicUser.Id, time.Now().Add(2*time.Second).Unix())
		userStatus, _, err = client.GetUserStatus(th.BasicUser.Id, "")
		require.NoError(t, err)
		assert.Equal(t, "dnd", userStatus.Status)
	...
	t.Run("back to offline status", func(t *testing.T) {
		th.App.SetStatusOffline(th.Context, th.BasicUser.Id, true)
		userStatus, _, err := client.GetUserStatus(th.BasicUser.Id, "")
		require.NoError(t, err)
		assert.Equal(t, "offline", userStatus.Status)
	})
...
func TestGetUsersStatusesByIds(t *testing.T) {
	...
	t.Run("away status", func(t *testing.T) {
		th.App.SetStatusAwayIfNeeded(th.Context, th.BasicUser.Id, true)
		th.App.SetStatusAwayIfNeeded(th.Context, th.BasicUser2.Id, true)
		usersStatuses, _, err := client.GetUsersStatusesByIds(usersIds)
		require.NoError(t, err)
		for _, userStatus := range usersStatuses {
			assert.Equal(t, "away", userStatus.Status)
	...
	t.Run("dnd status", func(t *testing.T) {
		th.App.SetStatusDoNotDisturb(th.Context, th.BasicUser.Id)
		th.App.SetStatusDoNotDisturb(th.Context, th.BasicUser2.Id)
		usersStatuses, _, err := client.GetUsersStatusesByIds(usersIds)
		require.NoError(t, err)
		for _, userStatus := range usersStatuses {
			assert.Equal(t, "dnd", userStatus.Status)
	...
	t.Run("dnd status", func(t *testing.T) {
		th.App.SetStatusDoNotDisturbTimed(th.Context, th.BasicUser.Id, time.Now().Add(10*time.Minute).Unix())
		th.App.SetStatusDoNotDisturbTimed(th.Context, th.BasicUser2.Id, time.Now().Add(15*time.Minute).Unix())
		usersStatuses, _, err := client.GetUsersStatusesByIds(usersIds)
		require.NoError(t, err)
		for _, userStatus := range usersStatuses {
			assert.Equal(t, "dnd", userStatus.Status)
