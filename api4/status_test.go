...
func TestGetUserStatus(t *testing.T) {
	...
	t.Run("dnd status timed restore after time interval", func(t *testing.T) {
		task := model.CreateRecurringTask("Unset DND Statuses", th.App.UpdateDNDStatusOfUsers, time.Duration(30)*time.Second)
		th.App.SetStatusOnline(th.BasicUser.Id, true)
		userStatus, resp := Client.GetUserStatus(th.BasicUser.Id, "")
		CheckNoError(t, resp)
		assert.Equal(t, "online", userStatus.Status)
		th.App.SetStatusDoNotDisturbTimed(th.BasicUser.Id, time.Now().Add(1*time.Minute).Format(time.RFC3339))
		userStatus, resp = Client.GetUserStatus(th.BasicUser.Id, "")
		CheckNoError(t, resp)
		assert.Equal(t, "dnd", userStatus.Status)
