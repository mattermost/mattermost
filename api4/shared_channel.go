...
func (api *API) InitSharedChannels() {
	api.BaseRoutes.SharedChannels.Handle("/{team_id:[A-Za-z0-9]+}", api.APISessionRequiredWithDenyScope(getSharedChannels)).Methods("GET")
	api.BaseRoutes.SharedChannels.Handle("/remote_info/{remote_id:[A-Za-z0-9]+}", api.APISessionRequiredWithDenyScope(getRemoteClusterInfo)).Methods("GET")
}
