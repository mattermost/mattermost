...
func createIncomingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), channel.TeamId, model.PermissionManageIncomingWebhooks) {
	...
	if hook.UserId != "" && hook.UserId != userId {
		if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), channel.TeamId, model.PermissionManageOthersIncomingWebhooks) {
...
func updateIncomingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if c.AppContext.Session().UserId != oldHook.UserId && !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), channel.TeamId, model.PermissionManageOthersIncomingWebhooks) {
		c.LogAudit("fail - inappropriate permissions")
		c.SetPermissionError(model.PermissionManageOthersIncomingWebhooks)
		return
	}
...
func getIncomingHooks(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if teamID != "" {
		if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), teamID, model.PermissionManageIncomingWebhooks) {
			c.SetPermissionError(model.PermissionManageIncomingWebhooks)
			return
		}

		// Remove userId as a filter if they have permission to manage others.
		if c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), teamID, model.PermissionManageOthersIncomingWebhooks) {
			userID = ""
		}

		hooks, appErr = c.App.GetIncomingWebhooksForTeamPageByUser(teamID, userID, c.Params.Page, c.Params.PerPage)
	} else {
		if !c.App.SessionHasPermissionTo(c.AppContext, *c.AppContext.Session(), model.PermissionManageIncomingWebhooks) {
			c.SetPermissionError(model.PermissionManageIncomingWebhooks)
			return
		}

		// Remove userId as a filter if they have permission to manage others.
		if c.App.SessionHasPermissionTo(c.AppContext, *c.AppContext.Session(), model.PermissionManageOthersIncomingWebhooks) {
			userID = ""
		}
...
func getIncomingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), hook.TeamId, model.PermissionManageIncomingWebhooks) ||
	...
	if c.AppContext.Session().UserId != hook.UserId && !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), hook.TeamId, model.PermissionManageOthersIncomingWebhooks) {
		c.LogAudit("fail - inappropriate permissions")
		c.SetPermissionError(model.PermissionManageOthersIncomingWebhooks)
		return
	}
...
func updateOutgoingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), updatedHook.TeamId, model.PermissionManageOutgoingWebhooks) {
		c.SetPermissionError(model.PermissionManageOutgoingWebhooks)
		return
	}

	if c.AppContext.Session().UserId != oldHook.CreatorId && !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), updatedHook.TeamId, model.PermissionManageOthersOutgoingWebhooks) {
		c.LogAudit("fail - inappropriate permissions")
...
func createOutgoingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), hook.TeamId, model.PermissionManageOutgoingWebhooks) {
		c.SetPermissionError(model.PermissionManageOutgoingWebhooks)
		return
	}
	...
	if hook.CreatorId == "" {
		...
		if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), hook.TeamId, model.PermissionManageOthersOutgoingWebhooks) {
			c.LogAudit("fail - inappropriate permissions")
			c.SetPermissionError(model.PermissionManageOthersOutgoingWebhooks)
			return
		}
...
func getOutgoingHooks(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if channelID != "" {
		...
		if !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), teamID, model.PermissionManageOutgoingWebhooks) {
			c.SetPermissionError(model.PermissionManageOutgoingWebhooks)
			return
		}

		// Remove userId as a filter if they have permission to manage others.
		if c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), teamID, model.PermissionManageOthersOutgoingWebhooks) {
			userID = ""
		}

		hooks, appErr = c.App.GetOutgoingWebhooksForTeamPageByUser(teamID, userID, c.Params.Page, c.Params.PerPage)
	} else {
		if !c.App.SessionHasPermissionTo(c.AppContext, *c.AppContext.Session(), model.PermissionManageOutgoingWebhooks) {
			c.SetPermissionError(model.PermissionManageOutgoingWebhooks)
			return
		}

		// Remove userId as a filter if they have permission to manage others.
		if c.App.SessionHasPermissionTo(c.AppContext, *c.AppContext.Session(), model.PermissionManageOthersOutgoingWebhooks) {
			userID = ""
		}
...
func getOutgoingHook(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if c.AppContext.Session().UserId != hook.CreatorId && !c.App.SessionHasPermissionToTeam(c.AppContext, *c.AppContext.Session(), hook.TeamId, model.PermissionManageOthersOutgoingWebhooks) {
		c.LogAudit("fail - inappropriate permissions")
		c.SetPermissionError(model.PermissionManageOthersOutgoingWebhooks)
		return
	}
