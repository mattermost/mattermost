...
func (api *API) InitBrand() {
	...
	api.BaseRoutes.Brand.Handle("/image", api.APISessionRequiredWithDenyScope(uploadBrandImage)).Methods("POST")
	api.BaseRoutes.Brand.Handle("/image", api.APISessionRequiredWithDenyScope(deleteBrandImage)).Methods("DELETE")
}
