...
func getDeletedChannelsForTeam(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	channels, err := c.App.GetDeletedChannels(c.AppContext, c.Params.TeamId, c.Params.Page*c.Params.PerPage, c.Params.PerPage, c.AppContext.Session().UserId, c.App.SessionHasPermissionTo(*c.AppContext.Session(), model.PermissionManageSystem))
	if err != nil {
		c.Err = err
		return
	}
