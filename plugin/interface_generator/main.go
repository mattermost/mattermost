...
var apiMetricsWrapperTemplate = `// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.
...
func (api *apiMetricsWrapper) {{.Name}}{{funcStyle .Params}} {{funcStyle .Return}} {
...
var hooksMetricsWrapperTemplate = `// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.
...
func (hooks *hooksMetricsWrapper) {{.Name}}{{funcStyle .Params}} {{funcStyle .Return}} {
	startTime := timePkg.Now()
...
func generatePluginMetricsWrapper(info *PluginInterfaceInfo) {
	templateFunctions := map[string]interface{}{
		"funcStyle":   func(fields *ast.FieldList) string { return FieldListToFuncList(fields, info.FileSet) },
		"structStyle": func(fields *ast.FieldList) string { return FieldListToStructList(fields, info.FileSet) },
		"valuesOnly":  func(fields *ast.FieldList) string { return FieldListToNames(fields, info.FileSet) },
	...
	pluginTemplates := map[string]string{
		"api_metrics_generated.go":   apiMetricsWrapperTemplate,
		"hooks_metrics_generated.go": hooksMetricsWrapperTemplate,
	}
	...
	for fileName, presetTemplate := range pluginTemplates {
		...
		if err := ioutil.WriteFile(filepath.Join(getPluginPackageDir(), fileName), importsBuffer.Bytes(), 0664); err != nil {
			panic(err)
		}
	}
}
...
func main() {
	...
	// Generate plugin metric wrappers
	log.Println("Generating plugin metrics glue")
	forMetrics, err := getPluginInfo(pluginPackageDir)
	if err != nil {
		fmt.Println("Unable to get plugin info: " + err.Error())
	}
	generatePluginMetricsWrapper(forMetrics)
}
