...
type Z_RegisterActionItemProviderArgs struct {
	A actionitem.Provider
}

type Z_RegisterActionItemProviderReturns struct {
	A error
}

func (g *apiRPCClient) RegisterActionItemProvider(provider actionitem.Provider) error {
	_args := &Z_RegisterActionItemProviderArgs{provider}
	_returns := &Z_RegisterActionItemProviderReturns{}
	if err := g.client.Call("Plugin.RegisterActionItemProvider", _args, _returns); err != nil {
		log.Printf("RPC call to RegisterActionItemProvider API failed: %s", err.Error())
	}
	return _returns.A
}

func (s *apiRPCServer) RegisterActionItemProvider(args *Z_RegisterActionItemProviderArgs, returns *Z_RegisterActionItemProviderReturns) error {
	if hook, ok := s.impl.(interface {
		RegisterActionItemProvider(provider actionitem.Provider) error
	}); ok {
		returns.A = hook.RegisterActionItemProvider(args.A)
		returns.A = encodableError(returns.A)
	} else {
		return encodableError(fmt.Errorf("API RegisterActionItemProvider called but not implemented."))
	}
	return nil
}

type Z_RegisterActionItemTypeArgs struct {
	A actionitem.Type
}

type Z_RegisterActionItemTypeReturns struct {
	A error
}

func (g *apiRPCClient) RegisterActionItemType(actionItemType actionitem.Type) error {
	_args := &Z_RegisterActionItemTypeArgs{actionItemType}
	_returns := &Z_RegisterActionItemTypeReturns{}
	if err := g.client.Call("Plugin.RegisterActionItemType", _args, _returns); err != nil {
		log.Printf("RPC call to RegisterActionItemType API failed: %s", err.Error())
	}
	return _returns.A
}

func (s *apiRPCServer) RegisterActionItemType(args *Z_RegisterActionItemTypeArgs, returns *Z_RegisterActionItemTypeReturns) error {
	if hook, ok := s.impl.(interface {
		RegisterActionItemType(actionItemType actionitem.Type) error
	}); ok {
		returns.A = hook.RegisterActionItemType(args.A)
		returns.A = encodableError(returns.A)
	} else {
		return encodableError(fmt.Errorf("API RegisterActionItemType called but not implemented."))
	}
	return nil
}
