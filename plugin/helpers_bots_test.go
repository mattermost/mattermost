...
func TestShouldProcessMessage(t *testing.T) {
	...
	t.Run("should not respond to itself", func(t *testing.T) {
		...
		api.On("KVGet", plugin.BotUserKey).Return([]byte(expectedBotID), nil)
		p.API = api
		shouldProcessMessage, _ := p.ShouldProcessMessage(&model.Post{Type: model.PostHeaderChange, UserId: expectedBotID}, plugin.AllowSystemMessages(), plugin.AllowBots())

		assert.False(t, shouldProcessMessage)
	})

	t.Run("should not process as the post is generated by system", func(t *testing.T) {
		shouldProcessMessage, _ := p.ShouldProcessMessage(&model.Post{Type: model.PostHeaderChange})
	...
	t.Run("should process the message", func(t *testing.T) {
		...
		shouldProcessMessage, _ := p.ShouldProcessMessage(&model.Post{UserId: "1", Type: model.PostHeaderChange, ChannelId: channelID},
