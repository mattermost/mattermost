...
func TestShouldProcessMessage(t *testing.T) {
	...
	t.Run("should not process as the post is created by self", func(t *testing.T) {
		assert.False(t, p.ShouldProcessMessage(&model.Post{UserId: "1"}, botUserId, ))
	})

	t.Run("should not process as the post is generated by system", func(t *testing.T) {
		assert.False(t, p.ShouldProcessMessage(&model.Post{Type: model.POST_HEADER_CHANGE}, botUserId, plugin.AllowSelf()))
	})
	...
	t.Run("should not process as the post is sent to another channel", func(t *testing.T) {
		...
		assert.False(t, p.ShouldProcessMessage(&model.Post{ChannelId: channelID}, botUserId, plugin.AllowSelf(), plugin.AllowSystemMessages()))
	})
	...
	t.Run("should not process as the post is created by bot", func(t *testing.T) {
		...
		assert.False(t, p.ShouldProcessMessage(&model.Post{UserId: userID, ChannelId: channelID},
			botUserId, plugin.AllowSelf(), plugin.AllowSystemMessages(), plugin.FilterChannelIDs([]string{channelID})))
	})

	t.Run("should process the message", func(t *testing.T) {
		channelID := "1"
		assert.True(t, p.ShouldProcessMessage(&model.Post{UserId: "1", Type: model.POST_HEADER_CHANGE, ChannelId: channelID}, botUserId,
			plugin.AllowSelf(), plugin.AllowSystemMessages(), plugin.FilterChannelIDs([]string{channelID}), plugin.AllowBots(), plugin.FilterUserIDs([]string{"1"})))
	})
}
...
func TestIsBotChannel(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.Name, func(t *testing.T) {
			assert.Equal(t, test.Expected, plugin.IsBotDMChannel(test.Channel, "botUserID"))
		})
	}
}
