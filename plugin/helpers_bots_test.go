...
func TestShouldProcessMessage(t *testing.T) {
	...
	t.Run("should not process as the post is created by self", func(t *testing.T) {
		shouldProcessMessage, _ := p.ShouldProcessMessage(&model.Post{UserId: "1"}, botUserId, )
		assert.False(t, shouldProcessMessage)
	})

	t.Run("should not process as the post is generated by system", func(t *testing.T) {
		shouldProcessMessage, _ := p.ShouldProcessMessage(&model.Post{Type: model.POST_HEADER_CHANGE}, botUserId, plugin.AllowSelf())
		assert.False(t, shouldProcessMessage)
	})
	...
	t.Run("should not process as the post is sent to another channel", func(t *testing.T) {
		...
		shouldProcessMessage, _ := p.ShouldProcessMessage(&model.Post{ChannelId: channelID}, botUserId, plugin.AllowSelf(), plugin.AllowSystemMessages())
		assert.False(t, shouldProcessMessage)
	})
