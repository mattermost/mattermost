...
func TestCheckAPIVersionComments(t *testing.T) {
	testCases := []struct {
		...
		{
			name:    "valid comments",
			pkgPath: "github.com/mattermost/mattermost-server/server/v7/plugin/checker/internal/test/valid",
			err:     "",
		},
		{
			name:    "invalid comments",
			pkgPath: "github.com/mattermost/mattermost-server/server/v7/plugin/checker/internal/test/invalid",
			expected: result{
		...
		},
		{
			...
			pkgPath: "github.com/mattermost/mattermost-server/server/v7/plugin/checker/internal/test/missing",
			err:     "could not find API interface",
		},
		{
			name:    "non-existent package path",
			pkgPath: "github.com/mattermost/mattermost-server/server/v7/plugin/checker/internal/test/does_not_exist",
			err:     "could not find API interface",
		},
	}
