...
func TestOnReceiveChannelInvite(t *testing.T) {
	...
	t.Run("when invitation prescribes a direct channel, it does create a direct channel", func(t *testing.T) {
		...
		mockApp.On("GetOrCreateDirectChannel", mock.AnythingOfType("request.CTX"), invitation.DirectParticipantIDs[0], invitation.DirectParticipantIDs[1], mock.AnythingOfType("model.ChannelOption")).Return(channel, nil)
		defer mockApp.AssertExpectations(t)
