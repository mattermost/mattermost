...
func TestStatusLogNotificationRuleInjection(t *testing.T) {
	...
	t.Run("SQL injection in event_filters field is rejected with 400", func(t *testing.T) {
		// Server should validate event_filters against known filter values and return 400
		rule := &model.StatusNotificationRule{
			Name:            "sqli-filters",
			WatchedUserID:   th.BasicUser.Id,
			RecipientUserID: th.SystemAdminUser.Id,
