...
func TestConvertBotToUser(t *testing.T) {
	...
	user, resp, err := th.Client.ConvertBotToUser(context.Background(), bot.UserId, &model.UserPatch{Password: model.NewPointer(model.NewRandomPassword())}, false)
	require.Error(t, err)
	...
	th.TestForSystemAdminAndLocal(t, func(t *testing.T, client *model.Client4) {
		...
		user, _, err = client.ConvertBotToUser(context.Background(), bot.UserId, &model.UserPatch{Password: model.NewPointer(model.NewRandomPassword())}, false)
		require.NoError(t, err)
		...
		user, _, err = client.ConvertBotToUser(context.Background(), bot.UserId, &model.UserPatch{Password: model.NewPointer(model.NewRandomPassword())}, true)
		require.NoError(t, err)
		require.NotNil(t, user)
		require.Equal(t, bot.UserId, user.Id)
