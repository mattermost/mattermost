...
func getPostsForChannel(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	etag := c.App.GetPostsEtag(channelId, userId, collapsedThreads)
	...
	if since > 0 {
		...
		list, err = c.App.GetPostsSince(c.AppContext, model.GetPostsSinceOptions{ChannelId: channelId, Time: since, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads, CollapsedThreadsExtended: collapsedThreadsExtended, UserId: userId})
	} else if afterPost != "" {
		if c.HandleEtag(etag, "Get Posts After", w, r) {
			return
		}

		list, err = c.App.GetPostsAfterPost(c.AppContext, model.GetPostsOptions{ChannelId: channelId, PostId: afterPost, Page: page, PerPage: perPage, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads, UserId: userId, IncludeDeleted: includeDeleted})
	} else if beforePost != "" {
		if c.HandleEtag(etag, "Get Posts Before", w, r) {
			return
		}

		list, err = c.App.GetPostsBeforePost(c.AppContext, model.GetPostsOptions{ChannelId: channelId, PostId: beforePost, Page: page, PerPage: perPage, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads, CollapsedThreadsExtended: collapsedThreadsExtended, UserId: userId, IncludeDeleted: includeDeleted})
	} else {
		if c.HandleEtag(etag, "Get Posts", w, r) {
			return
		}

		list, err = c.App.GetPostsPage(c.AppContext, model.GetPostsOptions{ChannelId: channelId, Page: page, PerPage: perPage, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads, CollapsedThreadsExtended: collapsedThreadsExtended, UserId: userId, IncludeDeleted: includeDeleted})
	}
	...
	clientPostList, err = c.App.SanitizePostListMetadataForUser(c.AppContext, clientPostList, userId)
	if err != nil {
		c.Err = err
		return
	}
...
func getPostsForChannelAroundLastUnread(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if len(postList.Order) == 0 {
		...
		postList, err = c.App.GetPostsPage(c.AppContext, model.GetPostsOptions{ChannelId: channelId, Page: app.PageDefault, PerPage: c.Params.LimitBefore, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads, CollapsedThreadsExtended: collapsedThreadsExtended, UserId: userId})
		if err != nil {
			c.Err = err
			return
		}
