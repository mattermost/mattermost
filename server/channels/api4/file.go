...
func uploadFileSimple(c *Context, r *http.Request, timestamp time.Time) *model.FileUploadResponse {
	...
	auditRec := c.MakeAuditRecord(audit.EventUploadFileSimple, audit.Fail)
...
NextPart:
	for {
		...
		auditRec := c.MakeAuditRecord(audit.EventUploadFileMultipart, audit.Fail)
		audit.AddEventParameter(auditRec, "channel_id", c.Params.ChannelId)
		audit.AddEventParameter(auditRec, "client_id", clientId)
...
func uploadFileMultipartLegacy(c *Context, mr *multipart.Reader,
	...
	for i, fileHeader := range fileHeaders {
		...
		auditRec := c.MakeAuditRecord(audit.EventUploadFileMultipartLegacy, audit.Fail)
		defer c.LogAuditRec(auditRec)
...
func getFile(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	auditRec := c.MakeAuditRecord(audit.EventGetFile, audit.Fail)
	defer c.LogAuditRec(auditRec)
...
func getFileLink(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	auditRec := c.MakeAuditRecord(audit.EventGetFileLink, audit.Fail)
	defer c.LogAuditRec(auditRec)
