...
func TestMoveThread(t *testing.T) {
	...
	// MoveThreadsEnabled is retired and rejected by Config.IsValid, so the
	// move-thread API remains permanently disabled until the flag is removed.
	th.App.UpdateConfig(func(cfg *model.Config) { cfg.FeatureFlags.MoveThreadsEnabled = true })
	require.False(t, th.App.Config().FeatureFlags.MoveThreadsEnabled)

	post, resp, err := th.Client.CreatePost(context.Background(), &model.Post{
		ChannelId: th.BasicChannel.Id,
		Message:   "test post",
	})
	require.NoError(t, err)
	...
	resp, err = th.Client.MoveThread(context.Background(), post.Id, &model.MoveThreadParams{
