...
func TestAddChannelMembers(t *testing.T) {
	...
	cm, resp, err := client.AddChannelMembers(context.Background(), publicChannel.Id, GenerateTestID(), []string{user.Id, user2.Id, user3.Id})
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)
	...
	cm, _, err = client.AddChannelMembers(context.Background(), privateChannel.Id, GenerateTestID(), []string{user.Id, user2.Id, user3.Id})
	require.NoError(t, err)
