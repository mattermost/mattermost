...
func TestGetUsersByNames(t *testing.T) {
	...
	t.Run("Get users by valid usernames", func(t *testing.T) {
		...
		require.Contains(t, usernames, th.BasicUser.Username)
		require.Contains(t, usernames, th.BasicUser2.Username)
	...
	t.Run("Get users by invalid usernames", func(t *testing.T) {
		users, _, err := th.Client.GetUsersByUsernames(context.Background(), []string{"invalid1", "invalid2"})
		require.NoError(t, err)
		require.Empty(t, users)
	})

	t.Run("Get users by mixed valid and invalid usernames", func(t *testing.T) {
		users, _, err := th.Client.GetUsersByUsernames(context.Background(), []string{th.BasicUser.Username, "invalid"})
		require.NoError(t, err)
		require.Len(t, users, 1)
		require.Equal(t, th.BasicUser.Username, users[0].Username)
		CheckUserSanitization(t, users[0])
	})
	...
	t.Run("Get users as system admin", func(t *testing.T) {
		users, _, err := th.SystemAdminClient.GetUsersByUsernames(context.Background(), []string{th.BasicUser.Username})
		require.NoError(t, err)
		require.Len(t, users, 1)
		require.Equal(t, th.BasicUser.Username, users[0].Username)
		CheckUserSanitization(t, users[0])
...
func TestGetFilteredUsersStats(t *testing.T) {
    ...
    t.Run("Get filtered users stats as system admin", func(t *testing.T) {
        ...
        require.True(t, stats.TotalUsersCount > 0)
    })
...
func TestGetDefaultProfileImage(t *testing.T) {
    ...
    t.Run("Get default profile image with invalid user ID", func(t *testing.T) {
		_, resp, err := th.Client.GetDefaultProfileImage(context.Background(), "invalid_user_id")
		require.Error(t, err)
		CheckNotFoundStatus(t, resp)
	})
