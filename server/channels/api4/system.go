...
func testFileStore(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	// PermissionTestS3 is kept for backwards compatibility — it was named after
	// the only supported test endpoint at the time it was introduced. The new
	// /file/test endpoint is backend-agnostic but reuses the same permission to
	// avoid a role migration.
	...
	// Restore FakeSetting placeholders from the running config. Sanitize()
	// masks these when the config is read out, so a client editing the admin
	// UI and clicking Test Connection without re-entering the secret would
	// otherwise send the placeholder back. config.Desanitize is the same
	// helper PUT /api/v4/config uses on save, keeping the two paths aligned.
