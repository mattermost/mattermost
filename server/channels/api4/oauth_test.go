...
func TestRegisterOAuthClient_PublicClient_Success(t *testing.T) {
	...
	assert.NotZero(t, response.ClientIDIssuedAt)
	assert.Equal(t, request.RedirectURIs, response.RedirectURIs)
	...
	assert.Equal(t, request.ResponseTypes, response.ResponseTypes)
	assert.Equal(t, *request.ClientName, *response.ClientName)
	assert.Equal(t, *request.ClientURI, *response.ClientURI)
	assert.Equal(t, *request.LogoURI, *response.LogoURI)
	assert.Equal(t, "user", *response.Scope) // Mattermost always sets scope to "user"
}

func TestRegisterOAuthClient_PublicClient_NoClientSecret(t *testing.T) {
	...
	// Most important: verify no client_secret field for public clients
	assert.Nil(t, response.ClientSecret)
	assert.Equal(t, model.ClientAuthMethodNone, response.TokenEndpointAuthMethod)
}

func TestRegisterOAuthClient_PublicClient_OnlyAuthCodeGrant(t *testing.T) {
	// Test that public clients can only use authorization_code grant type
	mainHelper.Parallel(t)
	th := Setup(t)
	defer th.TearDown()
	client := th.Client
	...
	// DCR request for public client with refresh_token grant (should fail)
	request := &model.ClientRegistrationRequest{
		RedirectURIs:            []string{"https://example.com/callback"},
		TokenEndpointAuthMethod: model.NewPointer(model.ClientAuthMethodNone),
	...
	_, resp, err := client.RegisterOAuthClient(context.Background(), request)
	require.Error(t, err)
	CheckBadRequestStatus(t, resp)

	// Verify error indicates public clients can't use refresh tokens
	assert.Contains(t, err.Error(), "public_client_refresh_token")
}

func TestRegisterOAuthClient_PublicClient_AuthMethodValidation(t *testing.T) {
	// Test public client auth method validation
	mainHelper.Parallel(t)
	th := Setup(t)
	defer th.TearDown()
	client := th.Client
	...
	// Should succeed with "none" auth method
	response, resp, err := client.RegisterOAuthClient(context.Background(), request)
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)
	assert.Equal(t, model.ClientAuthMethodNone, response.TokenEndpointAuthMethod)

	// Should fail with invalid auth method
	request.TokenEndpointAuthMethod = model.NewPointer("invalid_method")
	_, resp, err = client.RegisterOAuthClient(context.Background(), request)
	require.Error(t, err)
	CheckBadRequestStatus(t, resp)
}

func TestRegisterOAuthClient_PublicClient_RedirectURIValidation(t *testing.T) {
	...
	baseRequest := &model.ClientRegistrationRequest{
		TokenEndpointAuthMethod: model.NewPointer(model.ClientAuthMethodNone),
		GrantTypes:              []string{model.GrantTypeAuthorizationCode},
		ResponseTypes:           []string{model.ResponseTypeCode},
		ClientName:              model.NewPointer("Test Public Client"),
	...
	// Should succeed with valid HTTPS redirect URI
	request.RedirectURIs = []string{"https://example.com/callback"}
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)

	// Should succeed with valid HTTP localhost redirect URI
	request.RedirectURIs = []string{"http://localhost:3000/callback"}
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)

	// Should fail with invalid redirect URI
	request.RedirectURIs = []string{"invalid-uri"}
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.Error(t, err)
	CheckBadRequestStatus(t, resp)
}

func TestRegisterOAuthClient_PublicClient_MetadataValidation(t *testing.T) {
	// Test client metadata validation for public clients
	mainHelper.Parallel(t)
	th := Setup(t)
	defer th.TearDown()
	client := th.Client
	...
	// Should succeed without client name (will use default)
	request := *baseRequest
	response, resp, err := client.RegisterOAuthClient(context.Background(), &request)
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)
	assert.Equal(t, "Dynamically Registered Client", *response.ClientName)

	// Should succeed with valid client name
	request.ClientName = model.NewPointer("Valid Client Name")
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)

	// Should fail with client name too long
	longName := make([]byte, 65)
	for i := range longName {
		longName[i] = 'a'
	}
	request.ClientName = model.NewPointer(string(longName))
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.Error(t, err)
	CheckBadRequestStatus(t, resp)
	...
	// Should succeed with valid client URI
	request.ClientURI = model.NewPointer("https://example.com")
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)

	// Should fail with invalid client URI
	request.ClientURI = model.NewPointer("not-a-valid-url")
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.Error(t, err)
	CheckBadRequestStatus(t, resp)
	...
	// Should succeed with valid logo URI
	request.LogoURI = model.NewPointer("https://example.com/logo.png")
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.NoError(t, err)
	CheckCreatedStatus(t, resp)

	// Should fail with invalid logo URI
	request.LogoURI = model.NewPointer("not-a-valid-url")
	_, resp, err = client.RegisterOAuthClient(context.Background(), &request)
	require.Error(t, err)
	CheckBadRequestStatus(t, resp)
}
