...
Enforce these rules to prevent table locks and performance degradation on large
production databases (100M+ posts).
...
As noted above, `ALTER COLUMN` (type change) rewrites the entire table and blocks concurrent
...
Avoid foreign key constraints. Adding a foreign key takes a `SHARE ROW EXCLUSIVE` lock,
