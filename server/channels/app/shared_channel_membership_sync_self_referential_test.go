...
func TestSharedChannelMembershipSyncSelfReferential(t *testing.T) {
	...
	t.Run("Test 1: Automatic sync on membership changes", func(t *testing.T) {
		...
		// The sync is triggered by NotifyMembershipChanged which is called automatically by AddUserToChannel and RemoveUserFromChannel.
		...
		// Add user to channel - this triggers NotifyMembershipChanged automatically
	...
	t.Run("Test 2: Batch membership sync with user type filtering", func(t *testing.T) {
		...
		// With inclusive cursor (GtOrEq), boundary rows may be re-fetched across
		// batch boundaries, so batch count and total may exceed the minimum.
	...
	t.Run("Test 3: Cursor management", func(t *testing.T) {
		...
		// Verify incremental sync: first sync includes initial users, second
		// sync must include user3. The inclusive cursor (GtOrEq) may re-fetch
		// events at the boundary timestamp, so users from the first batch can
		// appear again. This is the expected trade-off to avoid data loss at
		// batch boundaries without requiring a composite cursor schema change.
		// The receiver is idempotent so duplicates are harmless.
	...
	t.Run("Test 5: Manual sync with cursor management", func(t *testing.T) {
		// This test verifies manual sync using NotifyMembershipChanged with complete cursor management:
	...
	t.Run("Test 6: Multiple remote clusters", func(t *testing.T) {
		...
		// Sync to all clusters via NotifyMembershipChanged
		service.NotifyMembershipChanged(channel.Id, "")

		// Wait for syncs to complete
		require.Eventually(t, func() bool {
			// Each cluster should receive at least 1 sync message (membership changes are batched)
		...
		// Verify each cluster received messages (membership changes are batched, so >= 1)
		...
		// Sync removal to all clusters via NotifyMembershipChanged
		service.NotifyMembershipChanged(channel.Id, "")
