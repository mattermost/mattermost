...
// TestGetHiddenValues covers branches of getHiddenValues that do not reach the store.
// nil App receiver is safe for all branches that don't call a.getCallerTextValues
// (AttrValue early-return, non-user-attribute path, missing field in map, source_only,
// shared_only select/multiselect, public default).
func TestGetHiddenValues(t *testing.T) {
	...
	t.Run("AttrValue condition: returns nil immediately", func(t *testing.T) {
		stored := &model.Condition{
			...
			Value:     "user.attributes.Dept",
			ValueType: model.AttrValue,
		}
		assert.Nil(t, a.getHiddenValues(rctx, "caller", stored, "", nil))
	})

	t.Run("non-user-attribute path: returns nil", func(t *testing.T) {
		stored := &model.Condition{
			Attribute: "team.attributes.Program",
			Value:     "Alpha",
			ValueType: model.LiteralValue,
		}
		assert.Nil(t, a.getHiddenValues(rctx, "caller", stored, "", map[string]*model.PropertyField{}))
	})
	...
	t.Run("field missing from prefetch map: returns nil (fail closed)", func(t *testing.T) {
		...
		// Empty map simulates a failed field lookup at prefetch time.
		result := a.getHiddenValues(rctx, "caller", stored, "", map[string]*model.PropertyField{})
		assert.Nil(t, result)
	})
	...
	t.Run("source_only: all stored values treated as hidden", func(t *testing.T) {
		...
		fields := map[string]*model.PropertyField{
			"Clearance": makeField(model.PropertyAccessModeSourceOnly, model.PropertyFieldTypeSelect),
		}
		result := a.getHiddenValues(rctx, "caller", stored, "", fields)
	...
	t.Run("public field: no values hidden", func(t *testing.T) {
		stored := &model.Condition{
			Attribute: "user.attributes.Department",
			Value:     []any{"Engineering", "Sales"},
			ValueType: model.LiteralValue,
		}
		fields := map[string]*model.PropertyField{
			"Department": makeField(model.PropertyAccessModePublic, model.PropertyFieldTypeSelect),
		}
		result := a.getHiddenValues(rctx, "caller", stored, "", fields)
		assert.Nil(t, result)
	})

	t.Run("shared_only select: values in options are visible, others are hidden", func(t *testing.T) {
		// Field options contain Alpha and Bravo. Charlie is not in options → hidden.
		stored := &model.Condition{
			Attribute: "user.attributes.Program",
		...
		fields := map[string]*model.PropertyField{
			"Program": makeField(model.PropertyAccessModeSharedOnly, model.PropertyFieldTypeSelect),
		}
		result := a.getHiddenValues(rctx, "caller", stored, "", fields)
		assert.Equal(t, []string{"Charlie"}, result)
	})

	t.Run("shared_only select: all values in options → nothing hidden", func(t *testing.T) {
		stored := &model.Condition{
			Attribute: "user.attributes.Program",
			Value:     []any{"Alpha", "Bravo"},
			ValueType: model.LiteralValue,
	...
	t.Run("shared_only multiselect: values absent from options are hidden", func(t *testing.T) {
		stored := &model.Condition{
			Attribute: "user.attributes.Programs",
		...
		fields := map[string]*model.PropertyField{
			"Programs": makeField(model.PropertyAccessModeSharedOnly, model.PropertyFieldTypeMultiselect),
		}
		// Alpha and Bravo are in field options; Charlie is not.
		result := a.getHiddenValues(rctx, "caller", stored, "", fields)
		assert.Equal(t, []string{"Charlie"}, result)
	})
}
