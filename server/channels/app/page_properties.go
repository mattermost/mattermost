...
func (a *App) SetPageStatus(rctx request.CTX, page *model.Post, status string) *model.AppError {
	...
	_, err = a.Srv().PropertyService().PropertyAccessService().UpsertPropertyValue(anonymousCallerID, propertyValue)
	if err != nil {
...
func (a *App) EnrichPagesWithProperties(rctx request.CTX, postList *model.PostList, useMaster ...bool) *model.AppError {
	...
	if wikiField != nil {
		...
		wikiValues, wikiErr := a.Srv().PropertyService().PropertyAccessService().SearchPropertyValues(anonymousCallerID, group.ID, wikiSearchOpts)
		if wikiErr != nil {
