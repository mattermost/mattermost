...
func (a *OpenTracingAppLayer) GetGroupsByIds(groupIds []string, options *store.GroupGetByIdsOpts) ([]*model.Group, *model.AppError) {
	origCtx := a.ctx
	span, newCtx := tracing.StartSpanWithParentByContext(a.ctx, "app.GetGroupsByIds")
	...
	defer span.Finish()
	resultVar0, resultVar1 := a.app.GetGroupsByIds(groupIds, options)
