...
// Categories outside this list (tutorial_step, recommended_next_steps, oauth_app)
// are excluded because clients don't use them.
...
func (a *App) loadExperienceSnapshot(rctx request.CTX, userID string, since int64, keys experienceLoadErrorKeys) (*experienceLoadSnapshot, *model.AppError) {
	...
	eg.Go(func() error {
		...
		res.me, appErr = a.GetUser(rctx, userID)
		if appErr != nil {
			return appErr
		}
		return nil
...
// Pages so a user's memberships are never silently truncated.
...
// Mirrors what the client would compute from user profiles.
func enrichDMGMDisplayNames(userID string, channels model.ChannelList, profilesByChannel map[string][]*model.User, nameFormat string) {
	...
	for _, ch := range channels {
		...
		case model.ChannelTypeDirect:
			// Self-DMs have no partner, so they fall back to the user's own profile.
...
// A DM/GM also counts as changed when a member's profile changed, since their name
// may appear in the display name.
...
// Same allowlist as filterExperiencePreferences, so clients never get tombstones for
// categories they were never sent.
...
// The view-time preferences are written by the client to persist view times across
// sessions when the server may not have updated LastViewedAt.
...
// Precedence: client hint, ExperimentalPrimaryTeam, teams_order, then locale-sorted first.
func (a *App) resolveActiveTeam(hintID string, teams []*model.Team, prefs model.Preferences, userLocale string) string {
	...
	slices.SortFunc(sortedTeams, func(a, b *model.Team) int {
		return cl.CompareString(strings.ToLower(a.DisplayName), strings.ToLower(b.DisplayName))
	})
...
// Mirrors User.Sanitize: self keeps everything, others are gated on showEmail /
// showFullName and never get NotifyProps.
func toExperienceUser(u *model.User, isSelf bool, showEmail bool, showFullName bool) *model.ExperienceUser {
	...
	if isSelf {
		...
		// Mirrors User.Sanitize: a remote user's real email can live in Props even when
		// Email is blanked. Copy first — u.Props is shared with the cached source User.
...
// A changed member whose channel is absent from the delta set still gets a slim channel
// entry, so the client can recompute unread counts.
...
// ChannelMember.MsgCount is a read counter, not an unread count — the unread
// total is the channel's TotalMsgCount minus what the member has read.
...
func filterAutoclosedDMEntries(
	...
	slices.SortStableFunc(dmCat, func(a, b dmEntry) int {
		...
		return cmp.Compare(b.lastViewed, a.lastViewed)
	})
...
func sortDMEntries(entries []dmEntry, sorting model.SidebarCategorySorting, sortOrderByID map[string]int, locale string) []dmEntry {
	...
	case model.SidebarCategorySortAlphabetical:
		...
		slices.SortStableFunc(entries, func(a, b dmEntry) int {
			...
			return col.CompareString(a.ch.DisplayName, b.ch.DisplayName)
		})
	case model.SidebarCategorySortManual:
		slices.SortStableFunc(entries, func(a, b dmEntry) int {
			return cmp.Compare(sortOrderByID[a.ch.Id], sortOrderByID[b.ch.Id])
		})
	default:
		slices.SortStableFunc(entries, func(x, y dmEntry) int {
			return cmp.Compare(max(y.ch.LastPostAt, y.ch.CreateAt), max(x.ch.LastPostAt, x.ch.CreateAt))
		})
	}
	return entries
}

// Shared by limitDMChannelsForProfiles and selectVisibleDMGMChannels so both agree on
// unread/pinned status without computing it twice.
func buildDMEntries(
...
// Bounds the profile query for very large DM/GM histories. Unread, pinned-elsewhere,
// and active channels are never dropped; the rest are capped by recency.
func limitDMChannelsForProfiles(
	...
	slices.SortStableFunc(rest, func(x, y dmEntry) int {
		return cmp.Compare(max(y.ch.LastPostAt, y.ch.CreateAt), max(x.ch.LastPostAt, x.ch.CreateAt))
	})
...
// Replicates the client-side DM/GM visibility rules server-side so every client
// agrees: filterManuallyClosedDMs, filterAutoclosedDMs, then sortChannels.
...
// ActiveChannel is stripped — it must not be visible to other users. Returns nil on
// error so a status failure doesn't fail the whole response.
