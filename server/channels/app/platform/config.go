...
func (ps *PlatformService) getSanitizedConfig(rctx request.CTX, partiallySanatizeDataSources bool) *model.Config {
	cfg := ps.Config().Clone()
	...
	if err != nil {
		...
		cfg.Sanitize(nil, partiallySanatizeDataSources)
	} else {
		cfg.Sanitize(manifests, partiallySanatizeDataSources)
	}
