...
type SuiteIFace interface {
	...
	HasPermissionToAccessChannelByID(rctx request.CTX, userID string, channelID string) bool
...
func (ps *PlatformService) GetHubForUserId(userID string) *Hub {
	...
	if err != nil {
		ps.logger.Error("Unable to write userID to hash", mlog.String("user_id", userID), mlog.Err(err))
	}
