...
func TestUnmarshalDQFullBuffer(t *testing.T) {
	...
	events2 := make([]*model.WebSocketEvent, deadQueueSize)
	for i := 0; i < deadQueueSize; i++ {
		...
		// Seq nos: 100 - 228
		events2[i] = model.NewWebSocketEvent(model.WebsocketEventPosted, "t1", "c1", "u1", nil, "").SetSequence(int64(i + 100))
	}

	// Marshal only the last entry wrapping to the first to test wraparound detection
	got2, err := ps.marshalDQ(events2, deadQueueSize-1, 0)
	require.NoError(t, err)
	...
	assert.Equal(t, events2[deadQueueSize-1], gotEvents2[0])
}
