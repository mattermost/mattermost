...
// TestUpdateSchemeRenameGuardAllowsSameNameSave pins the safe same-name path in
// checkSpaceSchemeUpdate: api4.patchScheme reads the stored scheme and applies
...
func TestUpdateSchemeGuardRejectsSeededRoleReferenceChanges(t *testing.T) {
	mainHelper.Parallel(t)
	th := Setup(t)
	require.NoError(t, th.App.SetPhase2PermissionsMigrationStatus(true))
	...
	for _, tc := range []struct {
		...
		{"admin role", func(scheme *model.Scheme) { scheme.DefaultChannelAdminRole = model.NewId() }},
		{"user role", func(scheme *model.Scheme) { scheme.DefaultChannelUserRole = model.NewId() }},
		{"guest role", func(scheme *model.Scheme) { scheme.DefaultChannelGuestRole = model.NewId() }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			candidate := *stored
			tc.mutate(&candidate)

			updated, appErr := th.App.UpdateScheme(&candidate)
			require.NotNil(t, appErr)
			assert.Nil(t, updated)
			assert.Equal(t, "app.scheme.save.space_scheme_roles.app_error", appErr.Id)
			assert.Equal(t, http.StatusBadRequest, appErr.StatusCode)
		})
	}
}
