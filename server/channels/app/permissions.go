...
func (a *App) ResetPermissionsSystem() *model.AppError {
	...
	// unlike the roles, they are not rebuilt from MakeDefaultRoles. Every channel
	// scheme reference was already cleared above, so a reseeded preset is not
	// re-attached to any space and every space falls back to the global channel
	// roles, which carry no page permissions. Access is lost rather than widened.
