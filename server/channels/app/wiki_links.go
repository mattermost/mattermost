...
func (a *App) LinkWikiToChannel(rctx request.CTX, wikiId, channelId, userId string) (*model.WikiLink, *model.AppError) {
...
func (a *App) LinkWikiToChannelWithWiki(rctx request.CTX, wiki *model.Wiki, channelId, userId string) (*model.WikiLink, *model.AppError) {
	...
	if !isValidWikiLinkSourceChannel(channel) {
		return nil, model.NewAppError("LinkWikiToChannelWithWiki", "app.wiki_link.invalid_source_channel_type", nil, "", http.StatusBadRequest)
	}
	...
	savedLink, nErr := a.Srv().Store().WikiLink().SaveAndPropagateMembers(rctx, link, channelId, false)
	if nErr != nil {
	...
	a.broadcastWikiLinkEvent(model.WebsocketEventWikiLinked, wiki.Id, channelId, savedLink.CreateAt)
...
func (a *App) UnlinkWikiFromChannel(rctx request.CTX, sourceId, destinationId string) *model.AppError {
	...
	if !isValidWikiLinkSourceChannel(sourceChannel) {
	...
	if err := a.Srv().Store().WikiLink().DeleteAndCleanupMembers(rctx, sourceId, destinationId); err != nil {
	...
	a.broadcastWikiLinkEvent(model.WebsocketEventWikiUnlinked, wiki.Id, sourceId, model.GetMillis())

	return nil
}

func (a *App) GetWikiLinksForChannel(rctx request.CTX, channelId string) ([]*model.WikiLink, *model.AppError) {
	if !model.IsValidId(channelId) {
		return nil, model.NewAppError("GetWikiLinksForChannel", "app.wiki_link.invalid_channel_id", nil, "", http.StatusBadRequest)
	}
	links, err := a.Srv().Store().WikiLink().GetBySource(channelId)
	if err != nil {
		return nil, wrapWikiLinkReadError("GetWikiLinksForChannel", "get_links", err)
	}

	wikis, wikisErr := a.Srv().Store().Wiki().GetLinkedToChannel(channelId)
	if wikisErr != nil {
		return nil, wrapWikiLinkReadError("GetWikiLinksForChannel", "get_linked", wikisErr)
	}
...
func (a *App) GetWikiLinksByDestination(rctx request.CTX, destinationId string) ([]*model.WikiLink, *model.AppError) {
	if !model.IsValidId(destinationId) {
		return nil, model.NewAppError("GetWikiLinksByDestination", "app.wiki_link.invalid_destination_id", nil, "", http.StatusBadRequest)
	}
	links, err := a.Srv().Store().WikiLink().GetByDestination(destinationId)
	if err != nil {
		return nil, wrapWikiLinkReadError("GetWikiLinksByDestination", "get_by_destination", err)
	}
	return links, nil
}
...
func (a *App) GetWikisLinkedToChannel(rctx request.CTX, channelId string) ([]*model.Wiki, *model.AppError) {
	...
	if err != nil {
		return nil, wrapWikiLinkReadError("GetWikisLinkedToChannel", "get_linked", err)
	}
	return wikis, nil
}

// wrapWikiLinkReadError maps store errors from wiki-link read methods to AppErrors.
// keyPrefix is the i18n key segment under "app.wiki_link.<prefix>.{invalid_input,app_error}".
func wrapWikiLinkReadError(caller, keyPrefix string, err error) *model.AppError {
	var errInvalid *store.ErrInvalidInput
...
// isValidWikiLinkSourceChannel rejects DM/GM and wiki-backing channels as link sources.
func isValidWikiLinkSourceChannel(ch *model.Channel) bool {
	return !ch.IsGroupOrDirect() && !ch.IsWikiBacking()
}

func (a *App) broadcastWikiLinkEvent(event model.WebsocketEventType, wikiId, sourceChannelId string, createAt int64) {
