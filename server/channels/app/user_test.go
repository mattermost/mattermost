...
func TestCreateUserWithToken(t *testing.T) {
	...
	t.Run("valid regular user request", func(t *testing.T) {
		...
		u := model.User{Email: invitationEmail, Nickname: "Darth Vader", Username: "vader" + model.NewId(), Password: model.NewRandomPassword(), AuthService: ""}
		token := model.NewToken(
			model.TokenTypeTeamInvitation,
			model.MapToJSON(map[string]string{"teamId": th.BasicTeam.Id, "email": invitationEmail}),
		)
	...
	t.Run("valid guest request", func(t *testing.T) {
		...
		require.NoError(t, th.App.Srv().Store().Token().Save(token))
		guest := model.User{Email: invitationEmail, Nickname: "Darth Vader", Username: "vader" + model.NewId(), Password: model.NewRandomPassword(), AuthService: ""}
	...
	t.Run("Guest Magic Link Invitation token adds guest to multiple channels", func(t *testing.T) {
		...
		guest := model.User{Email: invitationEmail, Nickname: "Magic Link Guest", Username: "magiclinkguest" + model.NewId(), Password: model.NewRandomPassword(), AuthService: ""}
		newGuest, err := th.App.CreateUserWithToken(th.Context, &guest, token)
	...
	t.Run("Guest Magic Link invitation token doesn't add user to channels if token is TeamInvitation", func(t *testing.T) {
		...
		regularUser := model.User{Email: invitationEmail, Nickname: "Regular User", Username: "regular" + model.NewId(), Password: model.NewRandomPassword(), AuthService: ""}
...
func TestPasswordRecovery(t *testing.T) {
	...
	t.Run("password token with same email as during creation", func(t *testing.T) {
		...
		err = th.App.ResetPasswordFromToken(th.Context, token.Token, model.NewRandomPassword())
		assert.Nil(t, err)
	})
	...
	t.Run("non-expired token", func(t *testing.T) {
		...
		err = th.App.resetPasswordFromToken(th.Context, token.Token, model.NewRandomPassword(), model.GetMillis())
		assert.Nil(t, err)
	})
	...
	t.Run("expired token", func(t *testing.T) {
		...
		err = th.App.resetPasswordFromToken(th.Context, token.Token, model.NewRandomPassword(), model.GetMillisForTime(time.Now().Add(25*time.Hour)))
		assert.NotNil(t, err)
	})
}
