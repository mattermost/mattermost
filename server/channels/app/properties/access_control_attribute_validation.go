...
// WithRequiredAttributeEnforcement wires a live-config callback so a flag flip
// takes effect without restarting the hook. A nil provider defaults to enforced.
func (h *AccessControlAttributeValidationHook) WithRequiredAttributeEnforcement(p RequiredAttributeEnforcementProvider) *AccessControlAttributeValidationHook {
	h.requiredAttributeEnforcement = p
	return h
}
