...
func (s *Server) doSetupContentFlaggingProperties() error {
	...
	existingProperties, appErr := s.propertyAccessService.SearchPropertyFields("", group.ID, model.PropertyFieldSearchOpts{PerPage: 100})
	if appErr != nil {
	...
	for _, property := range propertiesToCreate {
		if _, err := s.propertyAccessService.CreatePropertyField("", property); err != nil {
			return fmt.Errorf("failed to create content flagging property: %q, error: %w", property.Name, err)
		}
	}

	if len(propertiesToUpdate) > 0 {
		if _, err := s.propertyAccessService.UpdatePropertyFields("", group.ID, propertiesToUpdate); err != nil {
