...
// Policies are peers when they share the same Type and the same parent
// imports set (order-insensitive). Two policies with no Imports (top-
// level system policies) count as peers of one another. A policy and
// its parent are NOT peers — the parent has a smaller / different
// imports set.
