...
func (a *App) ShouldShowChannelActivityWarning(rctx request.CTX, channelID string) (bool, *model.AppError) {
...
// getLastRuleChangeTimestamp finds the timestamp to check activity against:
// - For new channels (no policy history): Returns channel creation time
// - For first rule modification: Returns current policy creation time
// - For subsequent modifications: Returns latest rule change time from history
func (a *App) getLastRuleChangeTimestamp(rctx request.CTX, channelID string) (int64, *model.AppError) {
	...
	if len(policies) == 0 {
		// No policy history means this is the first time rules are being created for this channel
		// We should check for activity since channel creation to warn about potential exposure
		// Get the current channel to use its CreateAt timestamp
		channel, err := a.Srv().Store().Channel().Get(channelID, true)
		if err != nil {
			return 0, model.NewAppError("getLastRuleChangeTimestamp", "app.access_control.get_channel.app_error", nil, err.Error(), http.StatusInternalServerError)
		}
		return channel.CreateAt, nil
	}

	if len(policies) == 1 {
		// For the first modification after creation, we need to use the timestamp
		// of when the rules were actually modified (current policy's CreateAt),
		// not when they were originally created (historical policy's CreateAt)
		currentPolicy, err := a.Srv().Store().AccessControlPolicy().Get(rctx, channelID)
		if err != nil {
			return 0, model.NewAppError("getLastRuleChangeTimestamp", "app.access_control.get_policy.app_error", nil, err.Error(), http.StatusInternalServerError)
		}
		return currentPolicy.CreateAt, nil
	}
...
func (a *App) policiesHaveDifferentRules(policy1, policy2 *model.AccessControlPolicy) bool {
	...
	// Compare each rule's expression
