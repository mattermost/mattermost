...
func (a *App) EnsureAllChannelsChildren(rctx request.CTX, parent *model.AccessControlPolicy, channels []*model.Channel) ([]*model.AccessControlPolicy, *model.AppError) {
	...
	for _, channel := range channels {
		...
		saved, appErr := a.materializeChild(rctx, acs, parent, channel.Id, child)
		if appErr != nil {
			return nil, appErr
		}
		children = append(children, saved)
	}
...
func (a *App) AssignAccessControlPolicyToChannels(rctx request.CTX, parentID string, channelIDs []string) ([]*model.AccessControlPolicy, *model.AppError) {
	...
	for _, channel := range channels {
		...
		saved, appErr := a.materializeChild(rctx, acs, policy, channel.Id, child)
		if appErr != nil {
			return nil, appErr
		}
		policies = append(policies, saved)
	}
...
func (a *App) GetAccessControlFieldsAutocomplete(rctx request.CTX, channelID string, includeResourceFields bool, after string, limit int, callerID string) ([]*model.PropertyField, *model.AppError) {
	...
	// scope, or when the caller explicitly asks for them — the parent-policy
	// editor authors resource.attributes.* against many channels and so has no
	// single channel to scope by. Each returned field carries its ObjectType,
	// which the editor uses to tag the row.
