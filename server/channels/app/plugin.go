...
func (ch *Channels) syncPlugins() *model.AppError {
	...
	for _, plugin := range pluginSignaturePathMap {
		...
		go func(plugin *pluginSignaturePath) {
			...
			if _, err := ch.installPluginLocally(reader, signature, installPluginLocallyAlways); err != nil && err.Id != "app.plugin.blocked.app_error" {
	...
	for _, psPath := range pluginSignaturePathMap {
		...
		go func(psPath *pluginSignaturePath) {
			...
			if err != nil {
				...
				if errors.As(err, &appErr) && appErr.Id == "app.plugin.blocked.app_error" {
					return
				}
