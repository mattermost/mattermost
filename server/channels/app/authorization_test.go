...
func TestSessionHasPermissionToChannelByPost(t *testing.T) {
	...
	session, err := th.App.CreateSession(&model.Session{
		UserId: th.BasicUser.Id,
		Roles:  model.SystemUserRoleId,
	})
	require.Nil(t, err)

	session2, err := th.App.CreateSession(&model.Session{
		UserId: th.BasicUser2.Id,
		Roles:  model.SystemUserRoleId,
	})
	require.Nil(t, err)
	...
	t.Run("read channel - user is admin", func(t *testing.T) {
		adminSession, err := th.App.CreateSession(&model.Session{
			UserId: th.SystemAdminUser.Id,
			Roles:  model.SystemAdminRoleId,
		})
		require.Nil(t, err)
...
func TestHasPermissionToChannelByPost(t *testing.T) {
	...
	t.Run("read channel", func(t *testing.T) {
		require.Equal(t, true, th.App.HasPermissionToChannelByPost(th.BasicUser.Id, post.Id, model.PermissionReadChannel))
		require.Equal(t, false, th.App.HasPermissionToChannelByPost(th.BasicUser2.Id, post.Id, model.PermissionReadChannel))
	})
	...
	t.Run("read archived channel - setting off", func(t *testing.T) {
		...
		require.Equal(t, false, th.App.HasPermissionToChannelByPost(th.BasicUser.Id, archivedPost.Id, model.PermissionReadChannel))
		require.Equal(t, false, th.App.HasPermissionToChannelByPost(th.BasicUser2.Id, archivedPost.Id, model.PermissionReadChannel))
	})
	...
	t.Run("read archived channel - setting on", func(t *testing.T) {
		...
		require.Equal(t, true, th.App.HasPermissionToChannelByPost(th.BasicUser.Id, archivedPost.Id, model.PermissionReadChannel))
		require.Equal(t, false, th.App.HasPermissionToChannelByPost(th.BasicUser2.Id, archivedPost.Id, model.PermissionReadChannel))
	})

	t.Run("read public channel", func(t *testing.T) {
		require.Equal(t, true, th.App.HasPermissionToChannelByPost(th.BasicUser.Id, post.Id, model.PermissionReadPublicChannel))
		require.Equal(t, true, th.App.HasPermissionToChannelByPost(th.BasicUser2.Id, post.Id, model.PermissionReadPublicChannel))
	})

	t.Run("read channel - user is admin", func(t *testing.T) {
		require.Equal(t, true, th.App.HasPermissionToChannelByPost(th.SystemAdminUser.Id, post.Id, model.PermissionReadChannel))
	})
}
