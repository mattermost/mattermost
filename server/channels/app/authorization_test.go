...
func TestSessionHasPermissionToChannel(t *testing.T) {
	...
	t.Run("does not panic if fetching channel causes an error", func(t *testing.T) {
		...
		mockChannelStore.On("GetAllChannelMembersForUser", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(th.App.Srv().Store().Channel().GetAllChannelMembersForUser(th.BasicUser.Id, false, false))
