...
// writeSidebarVersionTimestamp records a sidebar mutation for (userID, teamID); read
// back by getSidebarVersion to decide whether a client's cursor needs a fresh sidebar.
// Best-effort: failures are logged but don't abort the mutation that already succeeded.
