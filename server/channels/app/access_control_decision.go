...
type renderableActionConfig struct {
	...
	// Enabled gates the action behind a feature flag. When it returns false the
	// action is left out of discovery mode and rejected with 400 if asked for by
	// name. A nil func means the action is always available.
...
var renderableABACActions = map[string]renderableActionConfig{
	...
	model.AccessControlPolicyActionCreateBurnOnReadPost: {
		...
		Enabled: func(cfg *model.Config) bool {
			return cfg.FeatureFlags.IsBurnOnReadABACPermissionEnabled()
		},
	},
}
