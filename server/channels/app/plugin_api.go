...
func (api *PluginAPI) UpdateWikiPage(pageID, wikiID, title, content string, baseEditAt int64) (*model.Post, *model.AppError) {
	...
	wiki, err := api.app.GetWiki(api.ctx, wikiID)
	if err != nil {
		return nil, err
	}

	channel, err := api.app.GetChannel(api.ctx, wiki.ChannelId)
	if err != nil {
		return nil, err
	}

	// force=false to enforce optimistic locking (AI must respect conflicts)
	post, appErr := api.app.UpdatePageWithOptimisticLocking(api.ctx, page, title, content, "", baseEditAt, false, channel)
	if post != nil {
		post = post.ForPlugin()
	}
	return post, appErr
