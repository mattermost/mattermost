...
func TestMentionKeywords_AddUserWithRemoteClusterSupport(t *testing.T) {
	t.Run("should add @username:clustername for remote user", func(t *testing.T) {
		...
		keywords := MentionKeywords{}
		keywords.AddUserWithRemoteClusterSupport(user, channelNotifyProps, nil, false, clusterName)

		// Should have both standard mention and remote mention
		assert.Contains(t, keywords["@remoteuser"], mentionableUserID(user.Id))
		assert.Contains(t, keywords["@remoteuser:remotecluster"], mentionableUserID(user.Id))
	})

	t.Run("should not add remote mention for local user", func(t *testing.T) {
		user := &model.User{
			Id:       model.NewId(),
			Username: "localuser",
			RemoteId: nil,
		...
		// Should have only standard mention
		assert.Contains(t, keywords["@localuser"], mentionableUserID(user.Id))
		assert.NotContains(t, keywords, "@localuser:somecluster")
	})

	t.Run("should not add remote mention when cluster name is empty", func(t *testing.T) {
		remoteID := model.NewId()
		user := &model.User{
			Id:       model.NewId(),
			Username: "remoteuser",
		...
		keywords := MentionKeywords{}
		keywords.AddUserWithRemoteClusterSupport(user, channelNotifyProps, nil, false, "")

		// Should have only standard mention
		assert.Contains(t, keywords["@remoteuser"], mentionableUserID(user.Id))
		assert.NotContains(t, keywords, "@remoteuser:")
	})
}
