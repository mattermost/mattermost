...
func (h *SelfReferentialSyncHandler) HandleRequest(w http.ResponseWriter, r *http.Request) {
	...
	case "/api/v4/remotecluster/msg":
		...
		if err == nil {
			...
			// Just capture and respond - the actual processing happens on the sending side
...
func EnsureCleanState(t *testing.T, th *TestHelper, ss store.Store) {
	...
	// Note: GlobalUserSyncBatchSize setting may not be available in this version
	...
	if rcService != nil {
		// Note: SetActive method may not be available in this version
		require.Eventually(t, func() bool {
