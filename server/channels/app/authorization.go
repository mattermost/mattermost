...
func (a *App) HasPermissionToReadChannel(c request.CTX, userID string, channel *model.Channel) bool {
	...
	return a.HasPermissionToChannel(c, userID, channel.Id, model.PermissionReadChannelContent) || (channel.Type == model.ChannelTypeOpen && !*a.Config().MessageExportSettings.EnableExport && a.HasPermissionToTeam(userID, channel.TeamId, model.PermissionReadPublicChannel))
}
