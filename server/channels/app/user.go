...
func (a *App) PermanentDeleteUser(rctx request.CTX, user *model.User) *model.AppError {
	...
	if err := a.Srv().Store().PluginAccessControl().DeleteByUser(rctx, user.Id); err != nil {
		return model.NewAppError("PermanentDeleteUser", "app.plugin.access_control.permanent_delete_by_user.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
	}
