...
// runGuardedRecapWillBeProcessed dispatches RecapWillBeProcessed. Returns (skip=true, reason, nil)
// on plugin policy rejection (silent-skip preserved; reason captures the plugin's text for the
// caller's Debug log), (false, "", appErr) on infrastructure failure, (false, "", nil) on allow.
func (a *App) runGuardedRecapWillBeProcessed(rctx request.CTX, channel *model.Channel) (bool, string, *model.AppError) {
	guards, rejectErr := a.resolveGuards(rctx, channel.Id, "ProcessRecapChannel")

	// Guard plugin is unavailable — infrastructure failure, not silent skip (per the recap
	// silent-skip contract: only plugin-policy rejections are silenced, never infra issues).
	if rejectErr != nil {
		return false, "", rejectErr
	}

	// Phase A: fan out to non-guard plugins, fail-open. Any rejection → silent skip. With empty
	// guards the exclude list is empty and behavior is identical to plain RunMultiHook.
	var rejectionReason string
	pCtx := pluginContext(rctx)
	a.ch.RunMultiHookExcluding(pluginIDsOf(guards), func(hooks plugin.Hooks, _ *model.Manifest) bool {
		rejectionReason = hooks.RecapWillBeProcessed(pCtx, channel)
		return rejectionReason == ""
	}, plugin.RecapWillBeProcessedID)
	if rejectionReason != "" {
		return true, rejectionReason, nil
	}
	...
	for _, g := range guards {
		...
		if err != nil {
			// Active→inactive race: plugin deactivated between resolveGuards and now.
			return false, "", logAndErrPluginInactive(rctx, channel.Id, []string{g.PluginId}, "ProcessRecapChannel")
		}
		reason, rpcErr := hooks.RecapWillBeProcessedWithRPCErr(pCtx, channel)
		if rpcErr != nil {
			return false, "", appErrHookFailed(g.PluginId, "ProcessRecapChannel", rpcErr)
		}
		if reason != "" {
...
// runGuardedMessageWillBeRewrittenByAI dispatches MessageWillBeRewrittenByAI. Reject-only. Old
// mobile clients may omit channelID; that case proceeds unguarded because RegisterChannelGuard
// rejects empty channelIDs at write time, so resolveGuards("") returns no guards.
func (a *App) runGuardedMessageWillBeRewrittenByAI(rctx request.CTX, channelID string, syntheticPost *model.Post, action string) *model.AppError {
	guards, rejectErr := a.resolveGuards(rctx, channelID, "RewriteMessage")
	...
	pCtx := pluginContext(rctx)
	a.ch.RunMultiHookExcluding(pluginIDsOf(guards), func(hooks plugin.Hooks, _ *model.Manifest) bool {
		rejectionReason = hooks.MessageWillBeRewrittenByAI(pCtx, syntheticPost, action)
		return rejectionReason == ""
	}, plugin.MessageWillBeRewrittenByAIID)
	if rejectionReason != "" {
		return model.NewAppError("RewriteMessage", "app.post.rewrite.rejected_by_plugin",
			map[string]any{"Reason": rejectionReason}, "", http.StatusBadRequest)
	}
	...
	for _, g := range guards {
		...
		if err != nil {
			// Active→inactive race: plugin deactivated between resolveGuards and now.
			return logAndErrPluginInactive(rctx, channelID, []string{g.PluginId}, "RewriteMessage")
		}
		reason, rpcErr := hooks.MessageWillBeRewrittenByAIWithRPCErr(pCtx, syntheticPost, action)
		if rpcErr != nil {
			return appErrHookFailed(g.PluginId, "RewriteMessage", rpcErr)
		}
		if reason != "" {
			return model.NewAppError("RewriteMessage", "app.post.rewrite.rejected_by_plugin",
				map[string]any{"Reason": reason}, "", http.StatusBadRequest)
