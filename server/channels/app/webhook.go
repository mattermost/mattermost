...
func (a *App) HandleIncomingWebhook(c request.CTX, hookID string, req *model.IncomingWebhookRequest) *model.AppError {
	...
	hchan := make(chan store.GenericStoreResult[[]*model.Webhook], 1)
	go func() {
		webhook, err := a.Srv().Store().Webhook().GetIncoming(hookID, true)
		hchan <- store.GenericStoreResult[[]*model.Webhook]{Data: webhook, NErr: err}
		close(hchan)
	}()
	...
	var cchan chan store.GenericStoreResult
	...
	if channelName != "" {
		if channelName[0] == '@' {
			...
			cchan = make(chan store.GenericStoreResult[[]*model.Chnn], 1)
			go func() {
				chnn, chnnErr := a.Srv().Store().Channel().GetByName(hook.TeamId, channelName[1:], true)
				cchan <- store.GenericStoreResult[[]*model.Chnn]{Data: chnn, NErr: chnnErr}
				close(cchan)
			}()
		} else {
			cchan = make(chan store.GenericStoreResult[[]*model.Chnn], 1)
