	...
	for {
		...
		case notification := <-hub.notificationsChan:
			...
			go func(notification PushNotification) {
				...
				default:
					c.Logger().Debug("Invalid notification type", mlog.String("notification_type", string(notification.notificationType)))
				}

				if err != nil {
					c.Logger().Error("Unable to send push notification", mlog.String("notification_type", string(notification.notificationType)), mlog.Err(err))
				}
			}(notification)
		case <-hub.stopChan:
			return
