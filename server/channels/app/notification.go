...
func (a *App) SendNotifications(c request.CTX, post *model.Post, team *model.Team, channel *model.Channel, sender *model.User, parentPostList *model.PostList, setOnline bool) ([]string, error) {
	...
	if *a.Config().EmailSettings.SendEmailNotifications {
		...
		for _, id := range emailRecipients {
			...
			if *a.Config().EmailSettings.RequireEmailVerification && !profileMap[id].EmailVerified {
				a.Metrics().IncrementNotificationNotSentCounter(model.NotificationTypeEmail, model.NotificationReasonEmailNotVerified)
			...
			if a.userAllowsEmail(c, profileMap[id], channelMemberNotifyPropsMap[id], post) {
				...
				if err := a.sendNotificationEmail(c, notification, profileMap[id], team, senderProfileImage); err != nil {
					a.Metrics().IncrementNotificationErrorCounter(model.NotificationTypeEmail, model.NotificationReasonEmailSendError)
	...
	if int64(len(profileMap)) > *a.Config().TeamSettings.MaxNotificationsPerChannel {
		a.Metrics().IncrementNotificationNotSentCounter(model.NotificationTypeAll, model.NotificationReasonTooManyUsersInChannel)
	...
	if a.canSendPushNotifications() {
		...
		for _, id := range notificationsForCRT.Push {
			...
			if statusReason := DoesStatusAllowPushNotification(profileMap[id].NotifyProps, status, post.ChannelId); statusReason == "" {
				...
				a.Metrics().IncrementNotificationNotSentCounter(model.NotificationTypePush, model.NotificationReasonUserStatus)
	...
	if !published {
		...
		if jsonErr != nil {
			a.Metrics().IncrementNotificationErrorCounter(model.NotificationTypeWebsocket, model.NotificationReasonParseError)
