...
func TestDisableAutoResponder(t *testing.T) {
	...
	userUpdated1, err := th.App.GetUser(th.Context, user.Id)
	require.Nil(t, err)
	assert.Equal(t, userUpdated1.NotifyProps["auto_responder_active"], "false")

	err = th.App.DisableAutoResponder(th.Context, user.Id, true)
	require.Nil(t, err)
	userUpdated2, err := th.App.GetUser(th.Context, user.Id)
...
func TestGetUserStatusesByIdsIncludesAutoResponderMessage(t *testing.T) {
	mainHelper.Parallel(t)
	th := Setup(t)
	...
	t.Run("includes auto responder message when OOO and active", func(t *testing.T) {
		message := "I'm off today on PTO. I'll be back on Monday."

		patch := &model.UserPatch{}
		patch.NotifyProps = make(map[string]string)
		patch.NotifyProps[model.AutoResponderActiveNotifyProp] = "true"
		patch.NotifyProps[model.AutoResponderMessageNotifyProp] = message

		userUpdated, appErr := th.App.PatchUser(th.Context, user.Id, patch, true)
		require.Nil(t, appErr)

		th.App.SetAutoResponderStatus(th.Context, userUpdated, user.NotifyProps)

		statuses, appErr := th.App.GetUserStatusesByIds([]string{user.Id})
		require.Nil(t, appErr)
		require.Len(t, statuses, 1)
		assert.Equal(t, model.StatusOutOfOffice, statuses[0].Status)
		assert.Equal(t, message, statuses[0].AutoResponderMessage)
	})

	t.Run("omits auto responder message when not OOO", func(t *testing.T) {
		th.App.SetStatusOnline(user.Id, true)

		statuses, appErr := th.App.GetUserStatusesByIds([]string{user.Id})
		require.Nil(t, appErr)
		require.Len(t, statuses, 1)
		assert.Equal(t, model.StatusOnline, statuses[0].Status)
		assert.Empty(t, statuses[0].AutoResponderMessage)
	})

	t.Run("omits auto responder message when OOO but auto responder inactive", func(t *testing.T) {
		patch := &model.UserPatch{}
		patch.NotifyProps = make(map[string]string)
		patch.NotifyProps[model.AutoResponderActiveNotifyProp] = "false"
		patch.NotifyProps[model.AutoResponderMessageNotifyProp] = "Should not appear"

		_, appErr := th.App.PatchUser(th.Context, user.Id, patch, true)
		require.Nil(t, appErr)
