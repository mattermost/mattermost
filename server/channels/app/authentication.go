...
func (a *App) checkLdapUserPasswordAndAllCriteria(c request.CTX, ldapId *string, password string, mfaToken string) (*model.User, *model.AppError) {
	...
	if err != nil {
		...
		if err.Id == "ent.ldap.do_login.invalid_password.app_error" {
			c.Logger().LogM(mlog.MlvlLDAPInfo, "A user tried to sign in, which matched an LDAP account, but the password was incorrect.", mlog.String("ldap_id", *ldapId))
		}
...
func (a *App) authenticateUser(c request.CTX, user *model.User, password, mfaToken string) (*model.User, *model.AppError) {
	...
	if err := a.CheckPasswordAndAllCriteria(user, password, mfaToken); err != nil {
		if err.Id == "api.user.check_user_password.invalid.app_error" {
			c.Logger().LogM(mlog.MlvlLDAPInfo, "A user tried to sign in, which matched a Mattermost account, but the password was incorrect.", mlog.String("username", user.Username))
		}
