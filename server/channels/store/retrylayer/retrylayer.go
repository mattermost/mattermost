...
func (s *RetryLayerPropertyFieldStore) GetFromMaster(groupID string, id string) (*model.PropertyField, error) {

	tries := 0
	for {
		result, err := s.PropertyFieldStore.GetFromMaster(groupID, id)
		if err == nil {
			return result, nil
		}
		if !isRepeatableError(err) {
