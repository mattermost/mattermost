...
func (s *RetryLayerChannelStore) GetDeleted(team_id string, offset int, limit int, userID string, includePrivate bool) (model.ChannelList, error) {

	tries := 0
	for {
		result, err := s.ChannelStore.GetDeleted(team_id, offset, limit, userID, includePrivate)
		if err == nil {
			return result, nil
		}
		if !isRepeatableError(err) {
