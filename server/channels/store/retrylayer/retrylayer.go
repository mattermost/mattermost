...
func (s *RetryLayerSharedChannelStore) UpdateRemotePropertyValueCursor(id string, lastUpdateAt int64) error {

	tries := 0
	for {
		err := s.SharedChannelStore.UpdateRemotePropertyValueCursor(id, lastUpdateAt)
		if err == nil {
			return nil
		}
		if !isRepeatableError(err) {
