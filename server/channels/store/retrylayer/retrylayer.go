...
func (s *RetryLayerChannelStore) GetDeleted(teamID string, offset int, limit int, userID string, includePrivate bool) (model.ChannelList, error) {

	tries := 0
	for {
		result, err := s.ChannelStore.GetDeleted(teamID, offset, limit, userID, includePrivate)
		if err == nil {
			return result, nil
		}
		if !isRepeatableError(err) {
