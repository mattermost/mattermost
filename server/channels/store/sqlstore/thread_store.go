...
func (s *SqlThreadStore) Get(id string) (*model.Thread, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to get thread with id=%s: %w", id, err)
	}
	return &thread, nil
}
...
func (s *SqlThreadStore) GetTotalUnreadThreads(userId, teamId string, opts model.GetUserThreadsOpts) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to count unread threads for user id=%s: %w", userId, err)
	}
...
func (s *SqlThreadStore) GetTotalThreads(userId, teamId string, opts model.GetUserThreadsOpts) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to count threads for user id=%s: %w", userId, err)
	}
...
func (s *SqlThreadStore) GetTotalUnreadMentions(userId, teamId string, opts model.GetUserThreadsOpts) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to count unread mentions for user id=%s: %w", userId, err)
	}
...
func (s *SqlThreadStore) GetTotalUnreadUrgentMentions(userId, teamId string, opts model.GetUserThreadsOpts) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to count unread urgent mentions for user id=%s: %w", userId, err)
	}
...
func (s *SqlThreadStore) GetThreadsForUser(rctx request.CTX, userId, teamId string, opts model.GetUserThreadsOpts) ([]*model.ThreadResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to fetch threads for user id=%s: %w", userId, err)
	}
	...
	if opts.Extended {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get %d thread profiles for user id=%s: %w", len(participantUserIds), userId, err)
		}
...
func (s *SqlThreadStore) GetThreadFollowers(threadID string, fetchOnlyActive bool) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get thread followers for thread id=%s: %w", threadID, err)
	}
...
func (s *SqlThreadStore) GetThreadMembershipsForExport(postID string) ([]*model.ThreadMembershipForExport, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get thread members for thread id=%s: %w", postID, err)
	}
...
func (s *SqlThreadStore) GetThreadForUser(rctx request.CTX, threadMembership *model.ThreadMembership, extended, postPriorityEnabled bool) (*model.ThreadResponse, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to get thread for user id=%s, post id=%s: %w", threadMembership.UserId, threadMembership.PostId, err)
	}
	...
	if extended {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get thread for user id=%s: %w", threadMembership.UserId, err)
		}
	} else {
...
func (s *SqlThreadStore) MarkAllAsReadByChannels(userID string, channelIDs []string) error {
	...
	if _, err := s.GetMaster().ExecBuilder(query); err != nil {
		return fmt.Errorf("failed to mark all threads as read by channels for user id=%s: %w", userID, err)
	}
...
func (s *SqlThreadStore) MarkAllAsRead(userId string, threadIds []string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to mark %d threads as read for user id=%s: %w", len(threadIds), userId, err)
	}
...
func (s *SqlThreadStore) MarkAllAsReadByTeam(userId, teamId string) error {
	...
	_, err := s.GetMaster().ExecBuilder(query)
	if err != nil {
		return fmt.Errorf("failed to update thread read state for user id=%s: %w", userId, err)
	}
...
func (s *SqlThreadStore) MarkAsRead(userId, threadId string, timestamp int64) error {
	...
	_, err := s.GetMaster().ExecBuilder(query)
	if err != nil {
		return fmt.Errorf("failed to update thread read state for user id=%s thread_id=%v: %w", userId, threadId, err)
	}
	return nil
}
...
func (s *SqlThreadStore) saveMembership(ex sqlxExecutor, membership *model.ThreadMembership) (*model.ThreadMembership, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to save thread membership with postid=%s userid=%s: %w", membership.PostId, membership.UserId, err)
	}
...
func (s *SqlThreadStore) DeleteMembershipsForChannel(userID, channelID string) error {
	...
	_, err := s.GetMaster().ExecBuilder(query)
	if err != nil {
		return fmt.Errorf("failed to remove thread memberships with userid=%s channelid=%s: %w", userID, channelID, err)
	}
...
func (s *SqlThreadStore) updateMembership(ex sqlxExecutor, membership *model.ThreadMembership) (*model.ThreadMembership, error) {
	...
	_, err := ex.ExecBuilder(query)
	if err != nil {
		return nil, fmt.Errorf("failed to update thread membership with postid=%s userid=%s: %w", membership.PostId, membership.UserId, err)
	}
...
func (s *SqlThreadStore) GetMembershipsForUser(userId, teamId string) ([]*model.ThreadMembership, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get thread membership with userid=%s: %w", userId, err)
	}
...
func (s *SqlThreadStore) getMembershipForUser(ex sqlxExecutor, userId, postId string) (*model.ThreadMembership, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to get thread membership with userid=%s postid=%s: %w", userId, postId, err)
	}
...
func (s *SqlThreadStore) DeleteMembershipForUser(userId string, postId string) error {
	...
	_, err := s.GetMaster().ExecBuilder(query)
	if err != nil {
		return fmt.Errorf("failed to delete thread membership: %w", err)
	}
...
func (s *SqlThreadStore) maintainMembershipTx(trx *sqlxTxWrapper, userID, postID string, opts store.ThreadMembershipOpts) (_ *model.ThreadMembership, err error) {
	...
	if !errors.As(err, &nfErr) {
		return nil, fmt.Errorf("failed to get thread membership: %w", err)
	}
...
func (s *SqlThreadStore) GetThreadUnreadReplyCount(threadMembership *model.ThreadMembership) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to count unread reply count for post id=%s: %w", threadMembership.PostId, err)
	}
...
func (s *SqlThreadStore) SaveMultipleMemberships(memberships []*model.ThreadMembership) ([]*model.ThreadMembership, error) {
	...
	for _, chunk := range chunks {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to save thread memberships: %w", err)
		}
	}
...
func (s *SqlThreadStore) UpdateTeamIdForChannelThreads(channelId, teamId string) error {
	...
	_, err := s.GetMaster().ExecBuilder(query)
	if err != nil {
		return fmt.Errorf("failed to update threads team id for channel id=%s: %w", channelId, err)
	}
