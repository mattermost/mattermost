...
func TestPreferenceStore(t *testing.T) {
	StoreTestWithSqlStore(t, testPreferenceStore)
}
...
func testPreferenceStore(t *testing.T, rctx request.CTX, ss store.Store, s testSqlStore) {
...
func testDeleteInvalidVisibleDmsGms(t *testing.T, _ request.CTX, ss store.Store, s testSqlStore) {
	userId1 := model.NewId()
	userId2 := model.NewId()
	userId3 := model.NewId()
	userId4 := model.NewId()
