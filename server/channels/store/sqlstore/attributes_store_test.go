...
func TestAttributesStore(t *testing.T) {
	StoreTestWithSqlStore(t, testAttributesStore)
}
...
func testAttributesStore(t *testing.T, rctx request.CTX, ss store.Store, s testSqlStore) {
...
func testAttributesStoreSearchUsers(t *testing.T, rctx request.CTX, ss store.Store, s testSqlStore) {
...
func testAttributesStoreGetChannelMembersToRemove(t *testing.T, rctx request.CTX, ss store.Store, s testSqlStore) {
	users, _, cleanup := createTestUsers(t, rctx, ss)
	t.Cleanup(cleanup)
	require.Len(t, users, 3, "expected 3 users")
...
func testAttributesStoreSearchUsersBySubjectID(t *testing.T, rctx request.CTX, ss store.Store, s testSqlStore) {
	users, _, cleanup := createTestUsers(t, rctx, ss)
	t.Cleanup(cleanup)
	require.Len(t, users, 3, "expected 3 users")
