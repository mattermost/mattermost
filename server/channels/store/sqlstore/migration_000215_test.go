...
// TestMigration000215 pins the shape of PropertyOptionEdges. Both of its keys
...
// inside that field.
func TestMigration000215(t *testing.T) {
	...
	t.Run("the down migration drops the table", func(t *testing.T) {
		_, dErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000215_create_property_option_edges.down.sql"))
		require.NoError(t, dErr)
		require.Empty(t, indexDefs(t))

		_, uErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000215_create_property_option_edges.up.sql"))
