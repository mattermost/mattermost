...
func (s *SqlPropertyFieldStore) Update(groupID string, fields []*model.PropertyField, expectedUpdateAts map[string]int64) ([]*model.PropertyField, error) {
	...
	for i, field := range fields {
		...
		permissionFieldCase = permissionFieldCase.When(whenID, sq.Expr("?", field.PermissionField))
		permissionValuesCase = permissionValuesCase.When(whenID, sq.Expr("?", field.PermissionValues))
		permissionOptionsCase = permissionOptionsCase.When(whenID, sq.Expr("?", field.PermissionOptions))
