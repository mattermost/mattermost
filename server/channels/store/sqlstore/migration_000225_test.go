...
// TestMigration000225 pins how a graph-typed property value appears in the two
...
// nothing.
func TestMigration000225(t *testing.T) {
	...
	// New() applies all migrations, so 000225 is already in effect.
	...
	t.Run("the down migration restores the previous projections", func(t *testing.T) {
		_, dErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000225_add_graph_to_attribute_views.down.sql"))
		...
		_, uErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000225_add_graph_to_attribute_views.up.sql"))
