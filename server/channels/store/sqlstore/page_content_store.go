...
func (s SqlPageContentStore) Save(pageContent *model.PageContent) (*model.PageContent, error) {
	...
	query := s.getQueryBuilder().
		Insert("PageContents").
		Columns("PageId", "Content", "SearchText", "CreateAt", "UpdateAt").
		Values(pageContent.PageId, contentJSON, pageContent.SearchText, pageContent.CreateAt, pageContent.UpdateAt)
...
func (s SqlPageContentStore) Delete(pageID string) error {
	...
	result, err := s.GetMaster().Exec(queryString, args...)
	if err != nil {
		return errors.Wrapf(err, "failed to delete PageContent with pageId=%s", pageID)
	}
