...
// TestMigration000224 pins how a graph-typed property value appears in the two
...
func TestMigration000224(t *testing.T) {
	...
	t.Run("the down migration restores the previous projections", func(t *testing.T) {
		_, dErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000224_add_graph_to_attribute_views.down.sql"))
		...
		_, uErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000224_add_graph_to_attribute_views.up.sql"))
