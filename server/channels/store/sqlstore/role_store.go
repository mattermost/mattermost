...
func (s *SqlRoleStore) Save(role *model.Role) (_ *model.Role, err error) {
	...
	if role.Id == "" {
		...
		if terr != nil {
			return nil, fmt.Errorf("unable to create Role: %w", terr)
...
func (s *SqlRoleStore) GetByName(ctx context.Context, name string) (*model.Role, error) {
	...
	if err := s.DBXFromContext(ctx).GetBuilder(&dbRole, query); err != nil {
		...
		return nil, fmt.Errorf("failed to find Roles with name=%s: %w", name, err)
	}
...
func (s *SqlRoleStore) GetByNames(names []string) ([]*model.Role, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("role_tosql: %w", err)
	}
	...
	if err = rows.Err(); err != nil {
		return nil, fmt.Errorf("unable to iterate over rows: %w", err)
	}
...
func (s *SqlRoleStore) Delete(roleId string) (*model.Role, error) {
	...
	if err := s.GetReplica().GetBuilder(&role, query); err != nil {
		...
		return nil, fmt.Errorf("failed to get Role with id=%s: %w", roleId, err)
	}
...
func (s *SqlRoleStore) ChannelHigherScopedPermissions(roleNames []string) (map[string]*model.RolePermissions, error) {
	...
	if err := s.GetReplica().Select(&rolesPermissions, query); err != nil {
		return nil, fmt.Errorf("failed to find RolePermissions: %w", err)
	}
