...
// TestMigration000220 pins the shape of PropertyOptionEdges. Both of its keys
...
func TestMigration000220(t *testing.T) {
	...
	t.Run("the down migration drops the table", func(t *testing.T) {
		_, dErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000220_create_property_option_edges.down.sql"))
		require.NoError(t, dErr)
		require.Empty(t, indexDefs(t))

		_, uErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000220_create_property_option_edges.up.sql"))
		require.NoError(t, uErr)
		// The secondary index lives in 000224 so CREATE INDEX CONCURRENTLY is
		// its own non-transactional migration.
		_, iErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000224_create_property_option_edges_parent_child_index.up.sql"))
