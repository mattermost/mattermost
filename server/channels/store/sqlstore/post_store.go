...
func (s *SqlPostStore) DeleteOrphanedRowsByIds(r *model.RetentionIdsForDeletion) (int64, error) {
	...
	// 1. Delete posts by ChannelIds
	postCount, err := deleteOrphanedPostsByChannelIdsTx(txn, r.Ids)
	if err != nil {
		return 0, err
	}

	// 2. Delete threads by ChannelIds 
	threadCount, err := deleteOrphanedThreadsByChannelIdsTx(txn, r.Ids, s.SqlStore)
	if err != nil {
		return 0, err
	}

	// 3. Delete row from RetentionIdsForDeletion
	...
	totalDeleted := postCount + threadCount

	return totalDeleted, nil
}

func deleteOrphanedPostsByChannelIdsTx(txn *sqlxTxWrapper, channelIds []string) (rowsAffected int64, err error) {
	rows, err := txn.Exec("DELETE FROM Posts WHERE ChannelId IN ?", strings.Join(channelIds, "', '"))
	if err != nil {
		return 0, errors.Wrap(err, "Failed to delete from Posts")
	}
