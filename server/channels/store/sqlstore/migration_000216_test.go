...
// TestMigration000216 pins how a graph-typed property value appears in the two
...
func TestMigration000216(t *testing.T) {
	...
	t.Run("the down migration restores the previous projections", func(t *testing.T) {
		...
		_, uErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000216_add_graph_to_attribute_views.up.sql"))
