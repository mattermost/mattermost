...
// TestMigration000214 verifies that 'graph' is a usable property_field_type
...
func TestMigration000214(t *testing.T) {
	...
	t.Run("the down migration keeps the enum value", func(t *testing.T) {
		_, dErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000214_add_graph_to_property_field_type.down.sql"))
		...
		_, uErr := store.GetMaster().ExecNoTimeout(readMigrationSQL(t, "000214_add_graph_to_property_field_type.up.sql"))
