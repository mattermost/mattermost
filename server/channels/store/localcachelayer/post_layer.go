...
func (s LocalCachePostStore) ClearCaches() {
	doClearCacheCluster(s.rootStore.cluster, s.rootStore.lastPostTimeCache)
	doClearCacheCluster(s.rootStore.cluster, s.rootStore.postLastPostsCache)
	doClearCacheCluster(s.rootStore.cluster, s.rootStore.postsUsageCache)
...
func (s LocalCachePostStore) InvalidateLastPostTimeCache(channelId string) {
	doInvalidateCacheCluster(s.rootStore.cluster, s.rootStore.lastPostTimeCache, channelId)

	// Keys are "{channelid}{limit}" and caching only occurs on limits of 30 and 60
	doInvalidateCacheCluster(s.rootStore.cluster, s.rootStore.postLastPostsCache, channelId+"30")
	doInvalidateCacheCluster(s.rootStore.cluster, s.rootStore.postLastPostsCache, channelId+"60")
...
func (s LocalCachePostStore) GetEtag(channelId string, allowFromCache, collapsedThreads bool) string {
	if allowFromCache {
		var lastTime int64
		if err := doStandardReadCache(s.rootStore.metrics, s.rootStore.lastPostTimeCache, channelId, &lastTime); err == nil {
	...
	doStandardAddToCache(s.rootStore.lastPostTimeCache, channelId, lastTime)
...
func (s LocalCachePostStore) GetPostsSince(options model.GetPostsSinceOptions, allowFromCache bool, sanitizeOptions map[string]bool) (*model.PostList, error) {
	if allowFromCache {
		...
		var lastTime int64
		if err := doStandardReadCache(s.rootStore.metrics, s.rootStore.lastPostTimeCache, options.ChannelId, &lastTime); err == nil && lastTime <= options.Time {
	...
	if err == nil {
		...
		doStandardAddToCache(s.rootStore.lastPostTimeCache, options.ChannelId, latestUpdate)
	}
...
func (s LocalCachePostStore) GetPosts(options model.GetPostsOptions, allowFromCache bool, sanitizeOptions map[string]bool) (*model.PostList, error) {
	...
	if offset == 0 && (options.PerPage == 60 || options.PerPage == 30) {
		...
		if err := doStandardReadCache(s.rootStore.metrics, s.rootStore.postLastPostsCache, fmt.Sprintf("%s%v", options.ChannelId, options.PerPage), &cacheItem); err == nil {
	...
	// Caching only occurs on limits of 30 and 60, the common limits requested by MM clients
	if offset == 0 && (options.PerPage == 60 || options.PerPage == 30) {
		doStandardAddToCache(s.rootStore.postLastPostsCache, fmt.Sprintf("%s%v", options.ChannelId, options.PerPage), list)
	}
...
func (s LocalCachePostStore) AnalyticsPostCount(options *model.PostCountOptions) (int64, error) {
	...
	var count int64
	if err := doStandardReadCache(s.rootStore.metrics, s.rootStore.postsUsageCache, cacheKey, &count); err == nil {
		return count, nil
	}
	...
	doStandardAddToCache(s.rootStore.postsUsageCache, cacheKey, count)
	return count, nil
}
