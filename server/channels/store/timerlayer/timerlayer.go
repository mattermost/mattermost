...
func (s *TimerLayerTeamStore) IsUserAdminOfTeam(userEmail string) (bool, error) {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("TeamStore.IsUserAdminOfTeam", success, elapsed)
	}
	return result, err
}
