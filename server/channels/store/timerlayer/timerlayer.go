...
func (s *TimerLayerPostStore) LikeSearchPostsForUser(rctx request.CTX, paramsList []*model.SearchParams, userID string, teamID string, page int, perPage int) (*model.PostSearchResults, error) {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("PostStore.LikeSearchPostsForUser", success, elapsed)
	}
	return result, err
}
