...
func (s *TimerLayerPostStore) GetNewPinnedPosts(channelID string, userID string) ([]*model.Post, error) {
	start := time.Now()
	...
	if s.Root.Metrics != nil {
		...
		s.Root.Metrics.ObserveStoreMethodDuration("PostStore.GetNewPinnedPosts", success, elapsed)
	}
	return result, err
}
