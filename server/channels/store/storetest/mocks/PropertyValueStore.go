...
// GetForTarget provides a mock function with given fields: targetId, targetType, groupIDs
func (_m *PropertyValueStore) GetForTarget(targetId string, targetType string, groupIDs []string) ([]*model.PropertyValue, error) {
	ret := _m.Called(targetId, targetType, groupIDs)

	if len(ret) == 0 {
		panic("no return value specified for GetForTarget")
	}

	var r0 []*model.PropertyValue
	var r1 error
	if rf, ok := ret.Get(0).(func(string, string, []string) ([]*model.PropertyValue, error)); ok {
		return rf(targetId, targetType, groupIDs)
	}
	if rf, ok := ret.Get(0).(func(string, string, []string) []*model.PropertyValue); ok {
		r0 = rf(targetId, targetType, groupIDs)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*model.PropertyValue)
		}
	...
	if rf, ok := ret.Get(1).(func(string, string, []string) error); ok {
		r1 = rf(targetId, targetType, groupIDs)
	} else {
		r1 = ret.Error(1)
	}
