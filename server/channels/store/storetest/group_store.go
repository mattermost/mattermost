...
func testGroupStoreGetByUser(t *testing.T, rctx request.CTX, ss store.Store) {
	...
	groups, err := ss.Group().GetByUser(model.GroupSearchOpts{FilterHasMember: u1.Id})
	require.NoError(t, err)
	...
	groups, err = ss.Group().GetByUser(model.GroupSearchOpts{FilterHasMember: u2.Id})
	require.NoError(t, err)
	require.Equal(t, 1, len(groups))
	assert.Equal(t, g2.Id, groups[0].Id)

	groups, err = ss.Group().GetByUser(model.GroupSearchOpts{FilterHasMember: model.NewId()})
	require.NoError(t, err)
