...
func TestChannelStoreCategoriesWithAppsCategory(t *testing.T, ss store.Store, s SqlStore) {
	t.Run("CreateInitialSidebarCategoriesWithAppsCategory", func(t *testing.T) { testCreateInitialSidebarCategoriesWithAppsCategory(t, ss) })
	t.Run("CreateSidebarCategoryWithAppsCategory", func(t *testing.T) { testCreateSidebarCategoryWithAppsCategory(t, ss) })
	t.Run("GetSidebarCategoryWithAppsCategory", func(t *testing.T) { testGetSidebarCategoryWithAppsCategory(t, ss, s) })
	t.Run("GetSidebarCategoriesWithAppsCategory", func(t *testing.T) { testGetSidebarCategoriesWithAppsCategory(t, ss) })
	t.Run("UpdateSidebarCategoriesWithAppsCategory", func(t *testing.T) { testUpdateSidebarCategoriesWithAppsCategory(t, ss) })
	t.Run("ClearSidebarOnTeamLeaveWithAppsCategory", func(t *testing.T) { testClearSidebarOnTeamLeaveWithAppsCategory(t, ss, s) })
	t.Run("DeleteSidebarCategoryWithAppsCategory", func(t *testing.T) { testDeleteSidebarCategoryWithAppsCategory(t, ss, s) })
	t.Run("UpdateSidebarChannelsByPreferencesWithAppsCategory", func(t *testing.T) { testUpdateSidebarChannelsByPreferencesWithAppsCategory(t, ss) })
	t.Run("SidebarCategoryDeadlockWithAppsCategory", func(t *testing.T) { testSidebarCategoryDeadlockWithAppsCategory(t, ss) })
}

func testCreateInitialSidebarCategoriesWithAppsCategory(t *testing.T, ss store.Store) {
	t.Run("should create initial favorites/channels/DMs/apps categories", func(t *testing.T) {
		userId := model.NewId()
		...
		assert.Equal(t, model.SidebarCategoryApps, res.Categories[3].Type)

		res2, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		assert.NoError(t, err)
		assert.Equal(t, res, res2)
	})

	t.Run("should create initial favorites/channels/DMs/apps categories for multiple users", func(t *testing.T) {
		userId := model.NewId()
		userId2 := model.NewId()
		...
		res2, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId2, team.Id, opts)
		assert.NoError(t, err)
		assert.Equal(t, res, res2)
	})

	t.Run("should create initial favorites/channels/DMs/apps categories on different teams", func(t *testing.T) {
		userId := model.NewId()
		...
		res2, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team2.Id, opts)
		assert.NoError(t, err)
		assert.Equal(t, res, res2)
	})
	...
	t.Run("shouldn't create additional categories when ones already exist", func(t *testing.T) {
		...
		initialCategories, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		...
		res, err = ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		assert.NoError(t, err)
	...
	t.Run("should populate the Favorites category with regular channels", func(t *testing.T) {
		...
		require.NoError(t, nErr)
		require.Len(t, categories.Categories, 4)
		...
		categories2, nErr := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		require.NoError(t, nErr)
	...
	t.Run("should populate the Favorites category with DMs and GMs", func(t *testing.T) {
		...
		// Get and check the categories for channels
		categories2, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		require.NoError(t, err)
		require.Equal(t, categories, categories2)
	})
...
func testCreateSidebarCategoryWithAppsCategory(t *testing.T, ss store.Store) {
	...
	t.Run("should place the new category first if Favorites is not first", func(t *testing.T) {
		...
		categories, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		require.NoError(t, err)
		require.Len(t, categories.Categories, 4)
	...
	t.Run("should create the category with its channels", func(t *testing.T) {
		...
		res2, err := ss.Channel().GetSidebarCategory(created.Id, opts)
		require.NoError(t, err)
	...
	t.Run("should remove any channels from their previous categories", func(t *testing.T) {
		...
		res2, err := ss.Channel().GetSidebarCategory(favoritesCategory.Id, opts)
		require.NoError(t, err)
		assert.Equal(t, []string{}, res2.Channels)

		res2, err = ss.Channel().GetSidebarCategory(channelsCategory.Id, opts)
		require.NoError(t, err)
		assert.Equal(t, []string{}, res2.Channels)
	})
}

func testGetSidebarCategoryWithAppsCategory(t *testing.T, ss store.Store, s SqlStore) {
	...
	t.Run("should return any orphaned DM channels with the Direct Messages category", func(t *testing.T) {
		...
		res2, err := ss.Channel().GetSidebarCategory(dmsCategory.Id, opts)
		assert.NoError(t, err)
	...
	t.Run("should return any orphaned DM Bot channels with the Apps category", func(t *testing.T) {
		userId := model.NewId()
		team := setupTeam(t, ss, userId)
		...
		categories, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		require.NoError(t, err)
		require.Equal(t, model.SidebarCategoryApps, categories.Categories[3].Type)

		appsCategory := categories.Categories[3]

		// Create a Bot user
		botId := model.NewId()
		botUser, _ := makeBotWithUser(t, ss, &model.Bot{
			Username:       botId + "somebot",
			Description:    "a bot",
			OwnerId:        botId,
			LastIconUpdate: model.GetMillis(),
		})

		// Create a DM with Bot
		dmChannel, nErr := ss.Channel().SaveDirectChannel(
			&model.Channel{
				Name: model.GetDMNameFromIds(userId, botUser.UserId),
				Type: model.ChannelTypeDirect,
			},
			&model.ChannelMember{
				UserId:      userId,
		...
		// Ensure that the DM Bot is returned
		res2, err := ss.Channel().GetSidebarCategory(appsCategory.Id, opts)
		assert.NoError(t, err)
		assert.Equal(t, appsCategory.Id, res2.Id)
		assert.Equal(t, model.SidebarCategoryApps, res2.Type)
		assert.Equal(t, []string{dmChannel.Id}, res2.Channels)
	})

	t.Run("should return orphaned DM Bot channels in the Apps category which are in a custom category on another team", func(t *testing.T) {
		userId := model.NewId()
		team := setupTeam(t, ss, userId)
		...
		// Create another team and assign the DM Bot to a custom category on that team
		otherTeam := setupTeam(t, ss, userId)
		opts = &store.SidebarCategorySearchOpts{
			TeamID:              otherTeam.Id,
			ExcludeTeam:         false,
...
func testGetSidebarCategoriesWithAppsCategory(t *testing.T, ss store.Store) {
	t.Run("should return channels in the same order between different ways of getting categories", func(t *testing.T) {
		...
		gotCategory, err := ss.Channel().GetSidebarCategory(newCategory.Id, opts)
		require.NoError(t, err)
	...
	t.Run("should not return categories for teams deleted, or no longer a member", func(t *testing.T) {
		...
		for _, id := range teamIds {
			res, nErr := ss.Channel().CreateInitialSidebarCategories(userId, &store.SidebarCategorySearchOpts{TeamID: id, AppsCategoryEnabled: true})
			require.NoError(t, nErr)
			require.NotEmpty(t, res)
		}
		...
		require.NoError(t, err)
		assert.Equal(t, 4, len(res.Categories))
		...
		res, err = ss.Channel().GetSidebarCategories(userId, opts)
		require.NoError(t, err)
		assert.Equal(t, 8, len(res.Categories))
...
func testUpdateSidebarCategoriesWithAppsCategory(t *testing.T, ss store.Store) {
	t.Run("ensure the query to update SidebarCategories hasn't been polluted by UpdateSidebarCategoryOrder", func(t *testing.T) {
		...
		appsCategory := initialCategories.Categories[3]
		...
		err = ss.Channel().UpdateSidebarCategoryOrder(userId, team.Id, []string{dmsCategory.Id, favoritesCategory.Id, channelsCategory.Id, appsCategory.Id})
		require.NoError(t, err)

		// Which somehow blanks out stuff because ???
		got, err := ss.Channel().GetSidebarCategory(favoritesCategory.Id, opts)
		require.NoError(t, err)
	...
	t.Run("categories should be returned in their original order", func(t *testing.T) {
		...
		assert.Equal(t, appsCategory.Id, updatedCategories[3].Id)
	})
	...
	t.Run("should silently fail to update read only fields", func(t *testing.T) {
		...
		customCategory, err := ss.Channel().CreateSidebarCategory(userId, team.Id, &model.SidebarCategoryWithChannels{}, opts)
		require.NoError(t, err)
		...
		categoriesToUpdate := []*model.SidebarCategoryWithChannels{
			...
			// Try to change the Channels of Apps
			{
				SidebarCategory: appsCategory.SidebarCategory,
				Channels:        []string{"fakechannel"},
			},
		...
		updatedCategories, _, err := ss.Channel().UpdateSidebarCategories(userId, team.Id, categoriesToUpdate, opts)
		assert.NoError(t, err)
		...
		assert.NotEqual(t, []string{}, categoriesToUpdate[3].Channels)
		assert.Equal(t, []string{}, updatedCategories[3].Channels)
		assert.NotEqual(t, userId, categoriesToUpdate[4].UserId)
		assert.Equal(t, userId, updatedCategories[4].UserId)
	})
	...
	t.Run("should add and remove favorites preferences, even if the channel is already favorited in preferences", func(t *testing.T) {
		...
		categories2, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team2.Id, opts)
		require.NoError(t, err)
	...
	t.Run("should not affect other users' favorites preferences", func(t *testing.T) {
		...
		categories2, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId2, team.Id, opts)
		require.NoError(t, err)
	...
	t.Run("channels removed from Channels or DMs categories should be re-added", func(t *testing.T) {
		...
		// And some categories
		initialCategories, nErr := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team.Id, opts)
		require.NoError(t, nErr)
		...
		updatedCategories, _, nErr := ss.Channel().UpdateSidebarCategories(userId, team.Id, categoriesToUpdate, opts)
		assert.NoError(t, nErr)
	...
	t.Run("should be able to move DMs into and out of custom categories", func(t *testing.T) {
		...
		updatedDmsCategory, err := ss.Channel().GetSidebarCategory(dmsCategory.Id, opts)
		require.NoError(t, err)
		assert.Equal(t, []string{}, updatedDmsCategory.Channels)

		updatedCustomCategory, err := ss.Channel().GetSidebarCategory(customCategory.Id, opts)
		require.NoError(t, err)
	...
	t.Run("should successfully move channels between categories", func(t *testing.T) {
		...
		customCategory, nErr := ss.Channel().CreateSidebarCategory(userId, team.Id, &model.SidebarCategoryWithChannels{}, opts)
		require.NoError(t, nErr)
...
func setupInitialSidebarCategoriesWithAppsCategory(t *testing.T, ss store.Store) (string, string) {
	userId := model.NewId()
	team := setupTeam(t, ss, userId)
...
func testClearSidebarOnTeamLeaveWithAppsCategory(t *testing.T, ss store.Store, s SqlStore) {
	t.Run("should delete all sidebar categories and channels on the team", func(t *testing.T) {
		userId, teamId := setupInitialSidebarCategoriesWithAppsCategory(t, ss)
	...
	t.Run("MM-30314 should not delete channels on another team under specific circumstances", func(t *testing.T) {
		...
		res, err = ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team2.Id, opts)
		require.NoError(t, err)
		require.Len(t, res.Categories, 4)
		...
		res, err = ss.Channel().GetSidebarCategoriesForTeamForUser(userId, team2.Id, &store.SidebarCategorySearchOpts{
			AppsCategoryEnabled: true,
		})
		require.NoError(t, err)
		assert.Len(t, res.Categories, 5)
...
func testDeleteSidebarCategoryWithAppsCategory(t *testing.T, ss store.Store, s SqlStore) {
	t.Run("should correctly remove an empty category", func(t *testing.T) {
		...
		newCategory, err := ss.Channel().CreateSidebarCategory(userId, teamId, &model.SidebarCategoryWithChannels{}, opts)
		require.NoError(t, err)
		require.NotNil(t, newCategory)

		// Ensure that the category was created properly
		res, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, teamId, opts)
		require.NoError(t, err)
		require.Len(t, res.Categories, 5)
	...
	t.Run("should not allow you to remove non-custom categories", func(t *testing.T) {
		userId, teamId := setupInitialSidebarCategoriesWithAppsCategory(t, ss)
		defer ss.User().PermanentDelete(userId)
		res, err := ss.Channel().GetSidebarCategoriesForTeamForUser(userId, teamId, &store.SidebarCategorySearchOpts{
			AppsCategoryEnabled: true,
		})
		require.NoError(t, err)
		require.Len(t, res.Categories, 4)
		...
		require.Equal(t, model.SidebarCategoryApps, res.Categories[3].Type)
		...
		err = ss.Channel().DeleteSidebarCategory(res.Categories[3].Id)
		assert.Error(t, err)
	})
}

func testUpdateSidebarChannelsByPreferencesWithAppsCategory(t *testing.T, ss store.Store) {
...
func testSidebarCategoryDeadlockWithAppsCategory(t *testing.T, ss store.Store) {
	userID := model.NewId()
	team := setupTeam(t, ss, userID)
