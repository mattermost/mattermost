...
func TestPropertyValueStore(t *testing.T, rctx request.CTX, ss store.Store, s SqlStore) {
	...
	t.Run("DeleteValuesReferencingOptions", func(t *testing.T) { testDeleteValuesReferencingOptions(t, rctx, ss) })
}
...
func testDeleteValuesReferencingOptions(t *testing.T, _ request.CTX, ss store.Store) {
	t.Run("select type", func(t *testing.T) {
		groupID := model.NewId()
		fieldID1 := model.NewId()
		fieldID2 := model.NewId()

		// Create 3 property values with select-type JSONB string values
		val1, err := ss.PropertyValue().Create(&model.PropertyValue{
			TargetID:   model.NewId(),
			TargetType: "test_type",
			GroupID:    groupID,
			FieldID:    fieldID1,
		...
		val2, err := ss.PropertyValue().Create(&model.PropertyValue{
			TargetID:   model.NewId(),
			TargetType: "test_type",
			GroupID:    groupID,
			FieldID:    fieldID2,
		...
		val3, err := ss.PropertyValue().Create(&model.PropertyValue{
			TargetID:   model.NewId(),
			TargetType: "test_type",
			GroupID:    groupID,
			FieldID:    fieldID1,
			Value:      json.RawMessage(`"opt3"`),
		})
		require.NoError(t, err)

		// Delete values referencing opt1 and opt2
		err = ss.PropertyValue().DeleteValuesReferencingOptions(
			[]string{fieldID1, fieldID2},
			[]string{"opt1", "opt2"},
			model.PropertyFieldTypeSelect,
		)
		require.NoError(t, err)

		// Verify opt1 value is soft-deleted
		retrieved1, err := ss.PropertyValue().Get("", val1.ID)
		require.NoError(t, err)
		require.NotZero(t, retrieved1.DeleteAt, "opt1 value should be soft-deleted")

		// Verify opt2 value is soft-deleted
		retrieved2, err := ss.PropertyValue().Get("", val2.ID)
		require.NoError(t, err)
		require.NotZero(t, retrieved2.DeleteAt, "opt2 value should be soft-deleted")

		// Verify opt3 value is NOT soft-deleted
		retrieved3, err := ss.PropertyValue().Get("", val3.ID)
		require.NoError(t, err)
		require.Zero(t, retrieved3.DeleteAt, "opt3 value should not be soft-deleted")
	})
	...
	t.Run("multiselect type", func(t *testing.T) {
		...
		// Create 3 property values with multiselect-type JSON array values
		val1, err := ss.PropertyValue().Create(&model.PropertyValue{
			TargetID:   model.NewId(),
			TargetType: "test_type",
			GroupID:    groupID,
		...
		val2, err := ss.PropertyValue().Create(&model.PropertyValue{
			...
			GroupID:    groupID,
			FieldID:    fieldID2,
			Value:      json.RawMessage(`["opt2","opt3"]`),
		})
		require.NoError(t, err)
		...
		val3, err := ss.PropertyValue().Create(&model.PropertyValue{
			...
			GroupID:    groupID,
			FieldID:    fieldID1,
			Value:      json.RawMessage(`["opt3","opt4"]`),
		})
		require.NoError(t, err)

		// Remove opt1 from all matching values
		err = ss.PropertyValue().DeleteValuesReferencingOptions(
			[]string{fieldID1, fieldID2},
			[]string{"opt1"},
		...
		// Verify val1 now has opt1 removed → ["opt2"]
		retrieved1, err := ss.PropertyValue().Get("", val1.ID)
		require.NoError(t, err)
		require.JSONEq(t, `["opt2"]`, string(retrieved1.Value))
		require.Zero(t, retrieved1.DeleteAt, "val1 should not be soft-deleted yet")

		// Verify val2 is unchanged (did not contain opt1)
		retrieved2, err := ss.PropertyValue().Get("", val2.ID)
		require.NoError(t, err)
		require.JSONEq(t, `["opt2","opt3"]`, string(retrieved2.Value))
		require.Zero(t, retrieved2.DeleteAt)

		// Verify val3 is unchanged (did not contain opt1)
		retrieved3, err := ss.PropertyValue().Get("", val3.ID)
		require.NoError(t, err)
		require.JSONEq(t, `["opt3","opt4"]`, string(retrieved3.Value))
		require.Zero(t, retrieved3.DeleteAt)
		...
		// Verify val1 is now [] and soft-deleted (empty array)
		retrieved1, err = ss.PropertyValue().Get("", val1.ID)
		require.NoError(t, err)
		require.NotZero(t, retrieved1.DeleteAt, "val1 should be soft-deleted after becoming empty")

		// Verify val2 now has opt2 removed → ["opt3"]
		retrieved2, err = ss.PropertyValue().Get("", val2.ID)
		require.NoError(t, err)
		require.JSONEq(t, `["opt3"]`, string(retrieved2.Value))
		require.Zero(t, retrieved2.DeleteAt)
	})
}
