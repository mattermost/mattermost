...
func testGetUserId(t *testing.T, ss store.Store) {
	...
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("token_with_id", 1000, "server_token")
	require.NoError(t, err)

	err = ss.DesktopTokens().Insert("token_without_user_id", 2000, nil)
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("token_without_user_id", 2000, "server_token")
	require.NoError(t, err)

	err = ss.DesktopTokens().Insert("token_without_server_token", 3000, nil)
	require.NoError(t, err)
	err = ss.DesktopTokens().SetUserId("token_without_server_token", 3000, "user_id")
	require.NoError(t, err)

	t.Run("get user id", func(t *testing.T) {
		userId, err := ss.DesktopTokens().GetUserId("token_with_id", "server_token", 1000)
		assert.NoError(t, err)
		assert.Equal(t, "user_id", *userId)
	})

	t.Run("get user id - expired", func(t *testing.T) {
		userId, err := ss.DesktopTokens().GetUserId("token_with_id", "server_token", 10000)
		assert.Error(t, err)
	...
	t.Run("get user id - no id set", func(t *testing.T) {
		userId, err := ss.DesktopTokens().GetUserId("token_without_user_id", "server_token", 2000)
		assert.Error(t, err)
		assert.Nil(t, userId)
	})

	t.Run("get user id - no server token", func(t *testing.T) {
		userId, err := ss.DesktopTokens().GetUserId("token_without_server_token", "server_token", 3000)
		assert.Error(t, err)
		assert.IsType(t, &store.ErrNotFound{}, err)
		assert.Nil(t, userId)
	})
...
func testSetUserId(t *testing.T, ss store.Store) {
	...
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("new_token", 1000, "server_token")
	require.NoError(t, err)
	err = ss.DesktopTokens().Insert("new_token_2", 1000, nil)
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("new_token_2", 1000, "server_token")
	require.NoError(t, err)

	t.Run("set user id", func(t *testing.T) {
		err := ss.DesktopTokens().SetUserId("new_token", 1000, "user_id")
		assert.NoError(t, err)
		userId, err := ss.DesktopTokens().GetUserId("new_token", "server_token", 1000)
		assert.NoError(t, err)
		assert.Equal(t, "user_id", *userId)
	})
	...
	t.Run("set user id - doesn't exist", func(t *testing.T) {
		...
		assert.Error(t, err)
		_, err = ss.DesktopTokens().GetUserId("different_token", "server_token", 1000)
		assert.Error(t, err)
	})
	...
	t.Run("set user id - expired", func(t *testing.T) {
		...
		assert.Error(t, err)
		_, err = ss.DesktopTokens().GetUserId("new_token", "server_token", 2000)
		assert.Error(t, err)
	})
	...
	t.Run("set user id - user id too long", func(t *testing.T) {
		...
		assert.Error(t, err)
		userId, err := ss.DesktopTokens().GetUserId("new_token_2", "server_token", 1000)
		assert.Error(t, err)
		assert.Nil(t, userId)
	})
}
...
func testDeleteToken(t *testing.T, ss store.Store) {
	...
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("deleteable_token", 3000, "server_token")
	require.NoError(t, err)

	t.Run("delete", func(t *testing.T) {
		userId, err := ss.DesktopTokens().GetUserId("deleteable_token", "server_token", 3000)
		assert.NoError(t, err)
		assert.Equal(t, "user_id", *userId)
		...
		_, err = ss.DesktopTokens().GetUserId("deleteable_token", "server_token", 3000)
		assert.Error(t, err)
	})
}
...
func testDeleteByUserId(t *testing.T, ss store.Store) {
	...
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("deleteable_token_2", 4000, "server_token")
	require.NoError(t, err)

	t.Run("delete by user id", func(t *testing.T) {
		userId, err := ss.DesktopTokens().GetUserId("deleteable_token_2", "server_token", 3000)
		assert.NoError(t, err)
		...
		_, err = ss.DesktopTokens().GetUserId("deleteable_token_2", "server_token", 3000)
		assert.Error(t, err)
	})
}
...
func testDeleteOlderThan(t *testing.T, ss store.Store) {
	...
	require.NoError(t, err)
	err = ss.DesktopTokens().SetUserId("deleteable_token_old", 1000, "deleteable_user_id")
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("deleteable_token_old", 1000, "server_token")
	require.NoError(t, err)
	err = ss.DesktopTokens().Insert("deleteable_token_new", 5000, nil)
	require.NoError(t, err)
	err = ss.DesktopTokens().SetUserId("deleteable_token_new", 5000, "deleteable_user_id")
	require.NoError(t, err)
	err = ss.DesktopTokens().SetServerToken("deleteable_token_new", 5000, "server_token")
	require.NoError(t, err)

	t.Run("delete older than", func(t *testing.T) {
		_, err := ss.DesktopTokens().GetUserId("deleteable_token_old", "server_token", 1000)
		assert.NoError(t, err)
		_, err = ss.DesktopTokens().GetUserId("deleteable_token_new", "server_token", 5000)
		assert.NoError(t, err)
