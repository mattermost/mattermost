...
func TestChannelStore(t *testing.T, rctx request.CTX, ss store.Store, s SqlStore) {
	...
	t.Run("UpdateLastViewedAtToNow", func(t *testing.T) { testChannelStoreUpdateLastViewedAtToNow(t, rctx, ss) })
...
func testChannelStoreUpdateLastViewedAtToNow(t *testing.T, rctx request.CTX, ss store.Store) {
	o1 := model.Channel{}
	o1.TeamId = model.NewId()
	o1.DisplayName = "Channel1"
	o1.Name = NewTestId()
	...
	times, err = ss.Channel().UpdateLastViewedAtToNow([]string{m1.ChannelId}, m1.UserId, 2001)
	require.NoError(t, err, "failed to update ", err)
	require.Equal(t, int64(2001), times[o1.Id], "last viewed at time incorrect")

	rm1, err := ss.Channel().GetMember(context.Background(), m1.ChannelId, m1.UserId)
	assert.NoError(t, err)
	assert.Equal(t, int64(2001), rm1.LastViewedAt)
	assert.Equal(t, int64(2001), rm1.LastUpdateAt)
	assert.Equal(t, o1.TotalMsgCount, rm1.MsgCount)

	times, err = ss.Channel().UpdateLastViewedAtToNow([]string{m1.ChannelId, m2.ChannelId}, m1.UserId, 3003)
	require.NoError(t, err, "failed to update ", err)
	require.Equal(t, int64(3003), times[o2.Id], "last viewed at time incorrect")

	rm2, err := ss.Channel().GetMember(context.Background(), m2.ChannelId, m2.UserId)
	assert.NoError(t, err)
	assert.Equal(t, int64(3003), rm2.LastViewedAt)
	assert.Equal(t, int64(3003), rm2.LastUpdateAt)
	assert.Equal(t, o2.TotalMsgCount, rm2.MsgCount)

	_, err = ss.Channel().UpdateLastViewedAtToNow([]string{m1.ChannelId}, "missing id", 4000)
