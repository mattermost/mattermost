...
func TestCleanupPreferenceDeletions(t *testing.T) {
	...
	t.Run("full batch triggers next iteration", func(t *testing.T) {
		const limit = 5
		store := &fakeStore{batches: []int64{limit, 2}}
	...
	t.Run("max iter cap", func(t *testing.T) {
		const limit = 3
		const maxIter = 2
		store := &fakeStore{batches: []int64{limit, limit, limit}}
...
// Tolerates clock skew between test setup and the worker's own GetMillis call.
