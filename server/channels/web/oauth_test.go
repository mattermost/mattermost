...
// accessTokenFromRedirect returns the access token an authorization redirect carries, or an empty
// string when it carries none.
func accessTokenFromRedirect(t *testing.T, redirectURL string) string {
	t.Helper()
...
func TestAuthorizePageRespectsSessionOrigin(t *testing.T) {
	th := Setup(t).InitBasic(t)
	th.Login(t, apiClient, th.SystemAdminUser)
	...
	newApp := func(trusted bool) *model.OAuthApp {
		rapp, appErr := th.App.CreateOAuthApp(&model.OAuthApp{
			Name:         GenerateTestAppName(),
			Homepage:     "https://nowhere.com",
			Description:  "test",
			CallbackUrls: []string{"https://nowhere.com"},
	...
	// The token an integration holds once the user has completed the authorization flow for it.
	delegatedApp := newApp(false)
	ruri, _, err := apiClient.AuthorizeOAuthApp(context.Background(), &model.AuthorizeRequest{
		ResponseType: model.ImplicitResponseType,
		ClientId:     delegatedApp.Id,
		RedirectURI:  delegatedApp.CallbackUrls[0],
		State:        "123",
	})
	require.NoError(t, err)

	delegatedToken := accessTokenFromRedirect(t, ruri)
	...
	testCases := []struct {
		...
		{
			name:        "session obtained through the authorization flow",
			token:       delegatedToken,
			expectGrant: false,
		},
		{
			name:        "session obtained by signing in",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// A second registration the administrator marked trusted, so the authorization
			// page grants access to it without asking the user.
			trustedApp := newApp(true)

			request, err := http.NewRequest(http.MethodGet, apiClient.URL+"/oauth/authorize?response_type="+model.ImplicitResponseType+
				"&client_id="+trustedApp.Id+"&redirect_uri="+url.QueryEscape(trustedApp.CallbackUrls[0])+"&state=123", nil)
			require.NoError(t, err)
			request.Close = true
			request.Header.Set(model.HeaderAuth, model.HeaderBearer+" "+tc.token)
