...
func (w *Web) InitSaml() {
	w.MainRouter.Handle("/login/sso/saml", w.HTMLHandler(loginWithSaml)).Methods(http.MethodGet)
	w.MainRouter.Handle("/login/sso/saml", w.HTMLHandlerTrustRequester(completeSaml)).Methods(http.MethodPost)
}
