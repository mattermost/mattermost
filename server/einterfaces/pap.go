...
type PolicyAdministrationPointInterface interface {
	...
	GetBasicAutocomplete(rctx request.CTX, targetType string) (map[string]any, *model.AppError)
	...
	ExtractAttributeFields(rctx request.CTX, targetType, expression string) ([]string, *model.AppError)
