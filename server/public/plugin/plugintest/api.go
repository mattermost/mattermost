...
// MakeAuditRecordWithRequest provides a mock function with given fields: event, initialStatus, ctx, userId, apiPath
func (_m *API) MakeAuditRecordWithRequest(event string, initialStatus string, ctx plugin.Context, userId string, apiPath string) *model.AuditRecord {
	ret := _m.Called(event, initialStatus, ctx, userId, apiPath)

	if len(ret) == 0 {
		panic("no return value specified for MakeAuditRecordWithRequest")
	}

	var r0 *model.AuditRecord
	if rf, ok := ret.Get(0).(func(string, string, plugin.Context, string, string) *model.AuditRecord); ok {
		r0 = rf(event, initialStatus, ctx, userId, apiPath)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.AuditRecord)
		}
