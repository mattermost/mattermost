...
// ValidateSAMLResponse provides a mock function with given fields: encodedXML
func (_m *API) ValidateSAMLResponse(encodedXML string) (*saml2.AssertionInfo, *model.AppError) {
	ret := _m.Called(encodedXML)

	if len(ret) == 0 {
		panic("no return value specified for ValidateSAMLResponse")
	}

	var r0 *saml2.AssertionInfo
	var r1 *model.AppError
	if rf, ok := ret.Get(0).(func(string) (*saml2.AssertionInfo, *model.AppError)); ok {
		return rf(encodedXML)
	}
	if rf, ok := ret.Get(0).(func(string) *saml2.AssertionInfo); ok {
		r0 = rf(encodedXML)
	} else {
		if ret.Get(0) != nil {
