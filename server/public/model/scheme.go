...
// PluginChannelSchemeNamePrefix reserves names generated by PluginChannelSchemeName. Both suffixes
// are digests, so the whole name is fixed-length and stays well inside SchemeNameMaxLength.
...
// pluginSchemeNamePattern matches the format reserved for plugin channel schemes.
...
// given generated-role permission sets for pluginID. Equal normalized inputs produce the same
// pool key; application code validates any row already occupying that name.
...
func PluginChannelSchemeName(pluginID string, user, admin, guest []string) string {
	// Delimit the role sets so moving a permission changes the digest input.
...
// IsPluginChannelSchemeName reports whether name has the exact shape produced for plugin channel
// schemes. The full shape, rather than the plain prefix, is what application guards reserve.
