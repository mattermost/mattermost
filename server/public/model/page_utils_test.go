...
func TestParsePageUrl(t *testing.T) {
	t.Run("valid page URL", func(t *testing.T) {
		teamName, wikiId, pageId, ok := ParsePageUrl("/myteam/wiki/wiki456/page789")
		require.True(t, ok)
	...
	t.Run("external URL", func(t *testing.T) {
		_, _, _, ok := ParsePageUrl("https://example.com")
		require.False(t, ok)
	})

	t.Run("invalid internal URL", func(t *testing.T) {
		_, _, _, ok := ParsePageUrl("/myteam/channels/abc123")
		require.False(t, ok)
	})

	t.Run("empty URL", func(t *testing.T) {
		_, _, _, ok := ParsePageUrl("")
		require.False(t, ok)
	})

	t.Run("incomplete page URL", func(t *testing.T) {
		_, _, _, ok := ParsePageUrl("/myteam/wiki/wiki456")
		require.False(t, ok)
	})
}
...
func TestIsPageUrl(t *testing.T) {
	t.Run("valid page URL", func(t *testing.T) {
		require.True(t, IsPageUrl("/myteam/wiki/wiki456/page789"))
	})
...
func TestBuildPageUrl(t *testing.T) {
	t.Run("builds correct URL", func(t *testing.T) {
		url := BuildPageUrl("myteam", "wiki456", "page789")
		require.Equal(t, "/myteam/wiki/wiki456/page789", url)
	})
	...
	t.Run("roundtrip parsing", func(t *testing.T) {
		...
		url := BuildPageUrl(originalTeam, originalWiki, originalPage)
		teamName, wikiId, pageId, ok := ParsePageUrl(url)
