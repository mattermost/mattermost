...
func TestScheduledPostBaseIsValid(t *testing.T) {
	...
	t.Run("weekly repeat rejects burn-on-read posts", func(t *testing.T) {
		s := ScheduledPost{
			Draft: Draft{
				CreateAt:  GetMillis(),
				UpdateAt:  GetMillis(),
		...
		err := s.BaseIsValid()
		require.NotNil(t, err)
		assert.Equal(t, "model.scheduled_post.is_valid.repeat_burn_on_read.app_error", err.Id)
	})

	t.Run("one-shot allows burn-on-read posts", func(t *testing.T) {
		s := ScheduledPost{
			Draft: Draft{
				CreateAt:  GetMillis(),
				UpdateAt:  GetMillis(),
