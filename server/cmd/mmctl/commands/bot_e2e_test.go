...
func (s *MmctlE2ETestSuite) TestListBotCmdF() {
	...
	s.RunForSystemAdminAndLocal("List Bot", func(c client.Client) {
		...
		bot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: s.th.BasicUser.Id})
		...
		deletedBot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: s.th.BasicUser.Id})
		s.Require().Nil(appErr)
		defer func() {
	...
	s.RunForSystemAdminAndLocal("List Bot only orphaned", func(c client.Client) {
		...
		user, appErr := s.th.App.CreateUser(s.th.Context, &model.User{Email: s.th.GenerateTestEmail(), Username: "a" + model.NewId(), Password: model.NewId(), DeleteAt: 1})
		s.Require().Nil(appErr)
		defer func() {
		...
		deletedBot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: user.Id})
		s.Require().Nil(appErr)
		defer func() {
			err := s.th.App.PermanentDeleteBot(deletedBot.UserId)
			s.Require().Nil(err)
		...
		orphanBot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: user.Id})
		s.Require().Nil(appErr)
		defer func() {
	...
	s.RunForSystemAdminAndLocal("List all Bots", func(c client.Client) {
		...
		bot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: s.th.BasicUser2.Id})
		s.Require().Nil(appErr)
		defer func() {
			err := s.th.App.PermanentDeleteBot(bot.UserId)
			s.Require().Nil(err)
		...
		deletedBot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: s.th.BasicUser2.Id})
		s.Require().Nil(appErr)
		defer func() {
			err := s.th.App.PermanentDeleteBot(deletedBot.UserId)
			s.Require().Nil(err)
...
func (s *MmctlE2ETestSuite) TestBotEnableCmd() {
	...
	s.RunForSystemAdminAndLocal("enable a bot", func(c client.Client) {
		...
		newBot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: user.Id})
		s.Require().Nil(appErr)
...
func (s *MmctlE2ETestSuite) TestBotAssignCmdF() {
	...
	s.RunForSystemAdminAndLocal("Assign Bot", func(c client.Client) {
		...
		botOwner, appErr := s.th.App.CreateUser(s.th.Context, &model.User{Email: s.th.GenerateTestEmail(), Username: "a" + model.NewId(), Password: model.NewId()})
		s.Require().Nil(appErr)
		defer func() {
		...
		newBotOwner, appErr := s.th.App.CreateUser(s.th.Context, &model.User{Email: s.th.GenerateTestEmail(), Username: "a" + model.NewId(), Password: model.NewId()})
		s.Require().Nil(appErr)
		defer func() {
		...
		bot, appErr := s.th.App.CreateBot(s.th.Context, &model.Bot{Username: "a" + model.NewId(), OwnerId: botOwner.Id})
		s.Require().Nil(appErr)
