...
func listJobsCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	if len(ids) > 0 {
		...
		for _, id := range ids {
			...
			job, _, err := c.GetJob(cmdContext(cmd), id)
			if err != nil {
...
func updateJobCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	_, err = c.UpdateJobStatus(cmdContext(cmd), jobId, status, force)
	if err != nil {
		return err
	}
...
func jobListCmdF(c client.Client, command *cobra.Command, jobType string, status string) error {
	...
	for {
		jobs, _, err := c.GetJobs(cmdContext(command), jobType, status, page, perPage)
		if err != nil {
