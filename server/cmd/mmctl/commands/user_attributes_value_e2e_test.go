...
func (s *MmctlE2ETestSuite) cleanCPAValuesForUser(userID string) {
	existingValues, appErr := s.th.ListCPAValues(s.T(), userID)
	...
	if len(updates) > 0 {
		_, appErr = s.th.PatchCPAValues(s.T(), userID, updates)
		s.Require().Nil(appErr)
	}
}
...
func (s *MmctlE2ETestSuite) TestCPAValueList() {
	...
	s.Run("List CPA values with existing values", func() {
		...
		createdField, appErr := s.th.CreateCPAField(s.T(), textField)
		s.Require().Nil(appErr)
		...
		_, appErr = s.th.PatchCPAValues(s.T(), s.th.BasicUser.Id, updates)
		s.Require().Nil(appErr)
...
func (s *MmctlE2ETestSuite) TestCPAValueSet() {
	...
	s.Run("Set value for text type field", func() {
		...
		values, appErr := s.th.ListCPAValues(s.T(), s.th.BasicUser.Id)
		s.Require().Nil(appErr)
	...
	s.Run("Set value for select type field", func() {
		...
		createdField, appErr := s.th.CreateCPAField(s.T(), selectField)
		s.Require().Nil(appErr)
	...
	s.Run("Set value for multiselect type field", func() {
		...
		createdField, appErr := s.th.CreateCPAField(s.T(), multiselectField)
		s.Require().Nil(appErr)
	...
	s.Run("Set value for user type field", func() {
		...
		createdField, appErr := s.th.CreateCPAField(s.T(), userField)
		s.Require().Nil(appErr)
