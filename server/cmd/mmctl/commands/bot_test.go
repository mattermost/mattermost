...
func (s *MmctlUnitTestSuite) TestBotCreateCmd() {
	...
	s.Run("Should create a bot with an access token", func() {
		...
		s.client.
			EXPECT().
			CreateUserAccessToken(context.TODO(), mockBot.UserId, "autogenerated", int64(0)).
	...
	s.Run("Should create a bot with an expiring access token", func() {
		printer.Clean()
		...
		mockBot := model.Bot{Username: botArg}
		mockToken := model.UserAccessToken{Token: "token-id", Description: "autogenerated"}
		s.client.EXPECT().CreateBot(context.TODO(), &mockBot).Return(&mockBot, &model.Response{}, nil)
		s.client.EXPECT().GetUserByUsername(context.TODO(), botArg, "").Return(model.UserFromBot(&mockBot), &model.Response{}, nil)

		now := time.Now()
		s.client.
			EXPECT().
			CreateUserAccessToken(context.TODO(), mockBot.UserId, "autogenerated", gomock.AssignableToTypeOf(int64(0))).
			DoAndReturn(func(_ context.Context, _, _ string, expiresAt int64) (*model.UserAccessToken, *model.Response, error) {
				s.Require().InDelta(now.Add(30*24*time.Hour).UnixMilli(), expiresAt, float64(time.Minute.Milliseconds()))
				return &mockToken, &model.Response{}, nil
			})
	...
	s.Run("Should reject expires-in without with-token", func() {
		printer.Clean()

		cmd := &cobra.Command{}
		cmd.Flags().Bool("with-token", false, "")
		cmd.Flags().String("expires-in", "", "")
		s.Require().NoError(cmd.Flags().Set("expires-in", "30d"))

		err := botCreateCmdF(s.client, cmd, []string{"a-bot"})
		s.Require().EqualError(err, "--expires-in requires --with-token")
