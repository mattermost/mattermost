...
func (s *MmctlUnitTestSuite) TestBotCreateCmd() {
	s.Run("Should create a bot", func() {
		...
		s.cmd.Flags().String("display-name", "some-name", "")
		s.cmd.Flags().String("description", "some-text", "")
		mockBot := model.Bot{Username: botArg, DisplayName: "some-name", Description: "some-text"}

		s.client.
			EXPECT().
			CreateBot(s.T().Context(), &mockBot).
			Return(&mockBot, &model.Response{}, nil).
			Times(1)

		err := botCreateCmdF(s.client, s.cmd, []string{botArg})
		s.Require().Nil(err)
	...
	s.Run("Should create a bot with an access token", func() {
		...
		s.client.
			EXPECT().
			GetUserByUsername(s.T().Context(), botArg, "").
			Return(model.UserFromBot(&mockBot), &model.Response{}, nil).
			Times(1)

		s.client.
			EXPECT().
			CreateUserAccessToken(s.T().Context(), mockBot.UserId, "autogenerated", int64(0)).
...
func (s *MmctlUnitTestSuite) TestBotUpdateCmd() {
	s.Run("Should update a bot", func() {
		...
		s.cmd.Flags().String("username", "new-username", "")
		s.cmd.Flags().String("display-name", "some-name", "")
		s.cmd.Flags().String("description", "some-text", "")
		s.cmd.Flags().Lookup("username").Changed = true
		s.cmd.Flags().Lookup("display-name").Changed = true
		s.cmd.Flags().Lookup("description").Changed = true
		...
		s.client.
			EXPECT().
			PatchBot(s.T().Context(), mockUser.Id, gomock.Any()).
			Return(&mockBot, &model.Response{}, nil).
			Times(1)

		err := botUpdateCmdF(s.client, s.cmd, []string{botArg})
		s.Require().Nil(err)
		s.Require().Len(printer.GetLines(), 1)
		s.Require().Equal(&mockBot, printer.GetLines()[0])
	})
	...
	s.Run("Should error when user not found bot", func() {
		...
		botArg := "a-bot"
		s.cmd.Flags().String("username", "bot-username", "")
		s.cmd.Flags().Lookup("username").Changed = true
...
func (s *MmctlUnitTestSuite) TestBotListCmd() {
	s.Run("Should list correctly all", func() {
		...
		s.cmd.Flags().Bool("orphaned", false, "")
		...
		s.client.
			EXPECT().
			GetBotsIncludeDeleted(s.T().Context(), 0, 200, "").
			Return([]*model.Bot{&mockBot}, &model.Response{}, nil).
			Times(1)

		s.client.
			EXPECT().
			GetUsersByIds(s.T().Context(), []string{mockBot.OwnerId}).
			Return([]*model.User{&mockUser}, &model.Response{}, nil).
			Times(1)

		err := botListCmdF(s.client, s.cmd, []string{botArg})
		s.Require().Nil(err)
		s.Require().Len(printer.GetLines(), 1)
		s.Require().Equal(&mockBot, printer.GetLines()[0])
	})
	...
	s.Run("Should list correctly orphaned", func() {
		...
		s.client.
			EXPECT().
			GetBotsOrphaned(s.T().Context(), 0, 200, "").
			Return([]*model.Bot{&mockBot}, &model.Response{}, nil).
			Times(1)
...
func (s *MmctlUnitTestSuite) TestBotDisableCmd() {
	s.Run("Should disable a bot", func() {
		...
		s.client.
			EXPECT().
			DisableBot(s.T().Context(), mockUser.Id).
			Return(&mockBot, &model.Response{}, nil).
			Times(1)

		err := botDisableCmdF(s.client, s.cmd, []string{botArg})
		s.Require().Nil(err)
		s.Require().Len(printer.GetLines(), 1)
		s.Require().Equal(&mockBot, printer.GetLines()[0])
	})
...
func (s *MmctlUnitTestSuite) TestBotEnableCmd() {
	s.Run("Should enable a bot", func() {
		...
		s.client.
			EXPECT().
			EnableBot(s.T().Context(), mockUser.Id).
			Return(&mockBot, &model.Response{}, nil).
			Times(1)

		err := botEnableCmdF(s.client, s.cmd, []string{botArg})
		s.Require().Nil(err)
		s.Require().Len(printer.GetLines(), 1)
		s.Require().Equal(&mockBot, printer.GetLines()[0])
	})
...
func (s *MmctlUnitTestSuite) TestBotAssignCmd() {
	s.Run("Should assign a bot", func() {
		...
		s.client.
			EXPECT().
			AssignBot(s.T().Context(), mockBotUser.Id, mockNewOwner.Id).
			Return(&mockBot, &model.Response{}, nil).
			Times(1)

		err := botAssignCmdF(s.client, s.cmd, []string{botArg, userArg})
		s.Require().Nil(err)
		s.Require().Len(printer.GetLines(), 1)
		s.Require().Equal(&mockBot, printer.GetLines()[0])
	})
