...
func userActivateCmdF(c client.Client, command *cobra.Command, args []string) error {
	return changeUsersActiveStatus(cmdContext(command), c, args, true)
}
...
func userDeactivateCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	return changeUsersActiveStatus(cmdContext(cmd), c, args, false)
}
...
func userCreateCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	ruser, _, err := c.CreateUser(cmdContext(cmd), user)
	...
	if systemAdmin {
		if _, err := c.UpdateUserRoles(cmdContext(cmd), ruser.Id, "system_user system_admin"); err != nil {
			return errors.New("Unable to update user roles. Error: " + err.Error())
		}
	} else if guest {
		if _, err := c.DemoteUserToGuest(cmdContext(cmd), ruser.Id); err != nil {
...
func userInviteCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	teams := getTeamsFromTeamArgs(cmdContext(cmd), c, args[1:])
	for i, team := range teams {
		err := inviteUser(cmdContext(cmd), c, email, team, args[i+1])
		if err != nil {
...
func sendPasswordResetEmailCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	for _, email := range args {
		...
		if _, err := c.SendPasswordResetEmail(cmdContext(cmd), email); err != nil {
...
func changePasswordUserCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	user, err := getUserFromArg(cmdContext(cmd), c, args[0])
	if err != nil {
		return err
	}

	if hashed {
		if _, err := c.UpdateUserHashedPassword(cmdContext(cmd), user.Id, password); err != nil {
			return errors.Wrap(err, "changing user hashed password failed")
		}
	} else {
		if _, err := c.UpdateUserPassword(cmdContext(cmd), user.Id, current, password); err != nil {
...
func resetUserMfaCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	var result *multierror.Error
	users, err := getUsersFromArgs(cmdContext(cmd), c, args)
	if err != nil {
		result = multierror.Append(result, err)
	}

	for _, user := range users {
		if _, err := c.UpdateUserMfa(cmdContext(cmd), user.Id, "", false); err != nil {
...
func deleteUsersCmdF(c client.Client, cmd *cobra.Command, args []string) error {
	...
	for i, user := range users {
		...
		if res, err := c.PermanentDeleteUser(cmdContext(cmd), user.Id); err != nil {
...
func listUsersCmdF(c client.Client, command *cobra.Command, args []string) error {
	...
	if teamName != "" {
		var err error
		team, _, err = c.GetTeamByName(cmdContext(command), teamName, "")
		if err != nil {
	...
	for {
		users, _, err := c.GetUsersWithCustomQueryParameters(cmdContext(command), page, perPage, params.Encode(), "")
		if err != nil {
...
func verifyUserEmailWithoutTokenCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	var result *multierror.Error
	users, err := getUsersFromArgs(cmdContext(cmd), c, userArgs)
	if err != nil {
		result = multierror.Append(result, err)
	}

	for _, user := range users {
		if newUser, _, err := c.VerifyUserEmailWithoutToken(cmdContext(cmd), user.Id); err != nil {
...
func convertUserToBot(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	var multiErr *multierror.Error
	for _, user := range users {
		bot, _, err := c.ConvertUserToBot(cmdContext(cmd), user.Id)
		if err != nil {
			multiErr = multierror.Append(multiErr, err)
			continue
		}
...
func convertBotToUser(c client.Client, cmd *cobra.Command, userArgs []string) error {
	user, err := getUserFromArg(cmdContext(cmd), c, userArgs[0])
	if err != nil {
		return err
	}
	...
	user, _, err = c.ConvertBotToUser(cmdContext(cmd), user.Id, up, systemAdmin)
	if err != nil {
		return err
	}
...
func migrateAuthToSamlCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	resp, err := c.MigrateAuthToSaml(cmdContext(cmd), fromAuth, matches, auto)
	if err != nil {
		return err
...
func migrateAuthToLdapCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	resp, err := c.MigrateAuthToLdap(cmdContext(cmd), fromAuth, matchField, force)
	if err != nil {
		return err
	} else if resp.StatusCode == http.StatusOK {
		printer.Print("Successfully migrated accounts.")
...
func promoteGuestToUserCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	var errs *multierror.Error
	for i, user := range getUsersFromUserArgs(cmdContext(cmd), c, userArgs) {
		if user == nil {
		...
		if _, err := c.PromoteGuestToUser(cmdContext(cmd), user.Id); err != nil {
...
func demoteUserToGuestCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	for i, user := range getUsersFromUserArgs(cmdContext(cmd), c, userArgs) {
		...
		if _, err := c.DemoteUserToGuest(cmdContext(cmd), user.Id); err != nil {
...
func userEditCmdF(c client.Client, cmd *cobra.Command, args []string, fieldName string) error {
	...
	case "username":
		...
		ruser, _, err := c.UpdateUser(cmdContext(cmd), user)
		if err != nil {
	...
	case "authdata":
		...
		_, _, err := c.UpdateUserAuth(cmdContext(cmd), user.Id, &model.UserAuth{
			AuthData:    &newValue,
...
func preferencesListCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	for i, user := range getUsersFromUserArgs(cmdContext(cmd), c, userArgs) {
		...
		var err error
		if category == "" {
			preferences, _, err = c.GetPreferences(cmdContext(cmd), user.Id)
			...
			preferences, _, err = c.GetPreferencesByCategory(cmdContext(cmd), user.Id, category)
...
func preferencesGetCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	for i, user := range getUsersFromUserArgs(cmdContext(cmd), c, userArgs) {
		...
		preference, _, err := c.GetPreferenceByCategoryAndName(cmdContext(cmd), user.Id, category, preferenceName)
		if err != nil {
...
func preferencesUpdateCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	for i, user := range getUsersFromUserArgs(cmdContext(cmd), c, userArgs) {
		...
		_, err := c.UpdatePreferences(cmdContext(cmd), user.Id, preferences)
		if err != nil {
...
func preferencesDeleteCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	for i, user := range getUsersFromUserArgs(cmdContext(cmd), c, userArgs) {
		...
		_, err := c.DeletePreferences(cmdContext(cmd), user.Id, preferences)
		if err != nil {
