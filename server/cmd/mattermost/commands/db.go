...
func initDbCmdF(command *cobra.Command, _ []string) error {
	...
	if err != nil {
		return fmt.Errorf("error loading custom configuration defaults: %w", err)
	}
...
func resetCmdF(command *cobra.Command, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("could not initialize store: %w", err)
	}
	defer ss.Close()
...
func migrateCmdF(command *cobra.Command, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to migrate with the plan: %w", err)
	}
...
func dbVersionCmdF(command *cobra.Command, args []string) error {
	...
	if allFlag {
		...
		if err2 != nil {
			return fmt.Errorf("failed to get applied migrations: %w", err2)
		}
