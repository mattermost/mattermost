...
func TestCsvExport(t *testing.T) {
	...
	for _, tt := range csvExportTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			warningCount, err := CsvExport(rctx, tt.posts, shared.NewMessageExportStore(mockStore), fileBackend, exportFileName)
			assert.NoError(t, err)
...
func TestWriteExportWarnings(t *testing.T) {
	...
	exportFileName := path.Join("export", "jobName", "jobName-batch001-csv.zip")
	warningCount, err := CsvExport(rctx, posts, shared.NewMessageExportStore(mockStore), fileBackend, exportFileName)
	assert.NoError(t, err)
