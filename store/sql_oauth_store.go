...
func NewSqlOAuthStore(sqlStore *SqlStore) OAuthStore {
	...
	for _, db := range sqlStore.GetAllConns() {
		...
		tableAccess.ColMap("Token").SetMaxSize(128)
		tableAccess.ColMap("RefreshToken").SetMaxSize(128)
...
func (as SqlOAuthStore) GetAppByUser(userId string) StoreChannel {
	...
	go func() {
		...
		if _, err := as.GetReplica().Select(&apps, "SELECT * FROM OAuthApps WHERE CreatorId=?", userId); err != nil {
...
func (as SqlOAuthStore) GetAccessData(token string) StoreChannel {
	...
	go func() {
		...
		if err := as.GetReplica().SelectOne(&accessData, "SELECT * FROM OAuthAccessData WHERE Token= ? OR Token = ?", token, encryptedToken); err != nil {
...
func (as SqlOAuthStore) GetAccessDataByAuthCode(authCode string) StoreChannel {
	...
	go func() {
		...
		if err := as.GetReplica().SelectOne(&accessData, "SELECT * FROM OAuthAccessData WHERE AuthCode=?", authCode); err != nil {
...
func (as SqlOAuthStore) RemoveAccessData(token string) StoreChannel {
	...
	go func() {
		...
		if _, err := as.GetMaster().Exec("DELETE FROM OAuthAccessData WHERE Token = ? OR Token = ?", token, encryptedToken); err != nil {
