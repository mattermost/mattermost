...
type LocalCacheStore struct {
	...
	channelMembersForUserCache   *utils.Cache
...
func NewLocalCacheLayer(baseStore store.Store, metrics einterfaces.MetricsInterface, cluster einterfaces.ClusterInterface) LocalCacheStore {
	...
	localCacheStore.channelMembersForUserCache = utils.NewLruWithParams(
	...
	if cluster != nil {
		...
		cluster.RegisterClusterMessageHandler(
			model.CLUSTER_EVENT_INVALIDATE_CACHE_FOR_CHANNEL_MEMBERS_FOR_USER,
			localCacheStore.channel.handleClusterInvalidateChannelMembersForUser)
