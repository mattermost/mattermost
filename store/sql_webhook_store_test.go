...
func TestWebhookStoreCommandFunctions(t *testing.T) {
	Setup()
	...
	h1 = (<-store.Webhook().SaveCommand(h1)).Data.(*model.CommandWebhook)

	if r1 := <-store.Webhook().GetCommand(h1.Id); r1.Err != nil {
		t.Fatal(r1.Err)
	} else {
	...
	if err := (<-store.Webhook().GetCommand("123")).Err; err.StatusCode != http.StatusNotFound {
		t.Fatal("Should have set the status as not found for missing id")
	}
	...
	h2 = (<-store.Webhook().SaveCommand(h2)).Data.(*model.CommandWebhook)

	if err := (<-store.Webhook().GetCommand(h2.Id)).Err; err == nil || err.StatusCode != http.StatusNotFound {
		t.Fatal("Should have set the status as not found for expired webhook")
	}

	store.Webhook().CleanupCommand()

	if err := (<-store.Webhook().GetCommand(h1.Id)).Err; err != nil {
		t.Fatal("Should have no error getting unexpired webhook")
	}

	if err := (<-store.Webhook().GetCommand(h2.Id)).Err; err.StatusCode != http.StatusNotFound {
		t.Fatal("Should have set the status as not found for expired webhook")
	}

	if err := (<-store.Webhook().TryUseCommand(h1.Id, 1)).Err; err != nil {
		t.Fatal("Should be able to use webhook once")
	}

	if err := (<-store.Webhook().TryUseCommand(h1.Id, 1)).Err; err == nil || err.StatusCode != http.StatusBadRequest {
		t.Fatal("Should be able to use webhook once")
	}
}
