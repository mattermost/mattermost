...
func TestUserStoreGetProfilesByEmails(t *testing.T) {
	Setup()
	...
	if r1 := <-store.User().GetProfilesByEmails([]string{u1.Email, u2.Email}, teamId); r1.Err != nil {
		t.Fatal(r1.Err)
	} else {
		users := r1.Data.(map[string]*model.User)
		if len(users) != 2 {
	...
	if r1 := <-store.User().GetProfilesByEmails([]string{u1.Email}, teamId); r1.Err != nil {
		t.Fatal(r1.Err)
	} else {
		users := r1.Data.(map[string]*model.User)
		if len(users) != 1 {
