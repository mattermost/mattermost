...
func TestUserStoreSave(t *testing.T) {
	...
	if err := (<-store.User().Save(&u1, T)).Err; err != nil {
...
func TestUserStoreUpdate(t *testing.T) {
	...
	u1.TeamId = model.NewId()
	u1.Email = model.NewId()
	Must(store.User().Save(&u1, T))

	time.Sleep(100 * time.Millisecond)

	if err := (<-store.User().Update(&u1, false, T)).Err; err != nil {
		t.Fatal(err)
	}
...
func TestUserStoreUpdateLastPingAt(t *testing.T) {
	...
	if err := (<-store.User().UpdateLastPingAt(u1.Id, 1234567890, T)).Err; err != nil {
		t.Fatal(err)
	}

	if r1 := <-store.User().Get(u1.Id, T); r1.Err != nil {
		t.Fatal(r1.Err)
	} else {
...
func TestUserStoreUpdateLastActivityAt(t *testing.T) {
	...
	if err := (<-store.User().UpdateLastActivityAt(u1.Id, 1234567890, T)).Err; err != nil {
		t.Fatal(err)
	}
...
func TestUserStoreUpdateFailedPasswordAttempts(t *testing.T) {
	...
	if err := (<-store.User().UpdateFailedPasswordAttempts(u1.Id, 3, T)).Err; err != nil {
		t.Fatal(err)
	}
...
func TestUserStoreUpdateUserAndSessionActivity(t *testing.T) {
	...
	if err := (<-store.User().UpdateUserAndSessionActivity(u1.Id, s1.Id, 1234567890, T)).Err; err != nil {
		t.Fatal(err)
	}
	...
	if r2 := <-store.Session().Get(s1.Id, T); r2.Err != nil {
		t.Fatal(r2.Err)
	} else {
...
func TestUserStoreGet(t *testing.T) {
	...
	if err := (<-store.User().Get("", T)).Err; err == nil {
...
func TestUserCount(t *testing.T) {
	...
	if result := <-store.User().GetTotalUsersCount(T); result.Err != nil {
		t.Fatal(result.Err)
	} else {
...
func TestActiveUserCount(t *testing.T) {
	...
	if result := <-store.User().GetTotalActiveUsersCount(T); result.Err != nil {
		t.Fatal(result.Err)
	} else {
		count := result.Data.(int64)
		if count <= 0 {
...
func TestUserStoreGetProfiles(t *testing.T) {
	...
	Must(store.User().Save(&u2, T))

	if r1 := <-store.User().GetProfiles(u1.TeamId, T); r1.Err != nil {
		t.Fatal(r1.Err)
	} else {
	...
	if r2 := <-store.User().GetProfiles("123", T); r2.Err != nil {
		t.Fatal(r2.Err)
	} else {
...
func TestUserStoreGetSystemAdminProfiles(t *testing.T) {
	...
	if r1 := <-store.User().GetSystemAdminProfiles(T); r1.Err != nil {
		t.Fatal(r1.Err)
	} else {
		users := r1.Data.(map[string]*model.User)
		if len(users) <= 0 {
...
func TestUserStoreGetByEmail(t *testing.T) {
	...
	if err := (<-store.User().GetByEmail(u1.TeamId, u1.Email, T)).Err; err != nil {
		t.Fatal(err)
	}

	if err := (<-store.User().GetByEmail("", "", T)).Err; err == nil {
...
func TestUserStoreGetByAuthData(t *testing.T) {
	...
	if err := (<-store.User().GetByAuth(u1.TeamId, u1.AuthData, u1.AuthService, T)).Err; err != nil {
		t.Fatal(err)
	}

	if err := (<-store.User().GetByAuth("", "", "", T)).Err; err == nil {
...
func TestUserStoreGetByUsername(t *testing.T) {
	...
	if err := (<-store.User().GetByUsername(u1.TeamId, u1.Username, T)).Err; err != nil {
		t.Fatal(err)
	}

	if err := (<-store.User().GetByUsername("", "", T)).Err; err == nil {
...
func TestUserStoreUpdatePassword(t *testing.T) {
	...
	if err := (<-store.User().UpdatePassword(u1.Id, hashedPassword, T)).Err; err != nil {
		t.Fatal(err)
	}

	if r1 := <-store.User().GetByEmail(u1.TeamId, u1.Email, T); r1.Err != nil {
		t.Fatal(r1.Err)
	} else {
...
func TestUserStoreDelete(t *testing.T) {
	...
	if err := (<-store.User().PermanentDelete(u1.Id, T)).Err; err != nil {
		t.Fatal(err)
	}
}
...
func TestUserStoreUpdateAuthData(t *testing.T) {
	...
	if err := (<-store.User().UpdateAuthData(u1.Id, service, authData, T)).Err; err != nil {
		t.Fatal(err)
	}
