...
func testSearchPostsIncludingDMs(t *testing.T, th *SearchTestHelper) {
	...
	p1, err := th.createPost(th.User.Id, direct.Id, "dm test", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, direct.Id, "dm other", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "channel test", "", model.PostDefault, 0, false)
	require.NoError(t, err)
...
func testSearchPostsWithPagination(t *testing.T, th *SearchTestHelper) {
	...
	p1, err := th.createPost(th.User.Id, direct.Id, "dm test", "", model.PostDefault, 10000, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, direct.Id, "dm other", "", model.PostDefault, 20000, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "channel test", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchReturnPinnedAndUnpinned(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "channel test unpinned", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "channel test pinned", "", model.PostDefault, 0, true)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchExactPhraseInQuotes(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "channel test 1 2 3", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "channel test 123", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "channel something test 1 2 3", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "channel 1 2 3", "", model.PostDefault, 0, false)
	require.NoError(t, err)
...
func testSearchEmailAddresses(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test email test@test.com", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "test email test2@test.com", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchMarkdownUnderscores(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "_start middle end_ _another_", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchNonLatinWords(t *testing.T, th *SearchTestHelper) {
	t.Run("Should be able to search chinese words", func(t *testing.T) {
		p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "你好", "", model.PostDefault, 0, false)
		require.NoError(t, err)
		p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "你", "", model.PostDefault, 0, false)
		require.NoError(t, err)
		defer th.deleteUserPosts(th.User.Id)
	...
	t.Run("Should be able to search cyrillic words", func(t *testing.T) {
		p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "слово test", "", model.PostDefault, 0, false)
		require.NoError(t, err)
		defer th.deleteUserPosts(th.User.Id)
	...
	t.Run("Should be able to search japanese words", func(t *testing.T) {
		p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "本", "", model.PostDefault, 0, false)
		require.NoError(t, err)
		p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "本木", "", model.PostDefault, 0, false)
		require.NoError(t, err)
		defer th.deleteUserPosts(th.User.Id)
	...
	t.Run("Should be able to search korean words", func(t *testing.T) {
		p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "불", "", model.PostDefault, 0, false)
		require.NoError(t, err)
		p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "불다", "", model.PostDefault, 0, false)
		require.NoError(t, err)
		defer th.deleteUserPosts(th.User.Id)
...
func testSearchAlternativeSpellings(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "Straße test", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "Strasse test", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchAlternativeSpellingsAccents(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "café", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "café", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchOrExcludePostsBySpecificUser(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "test fromuser", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User2.Id, th.ChannelPrivate.Id, "test fromuser 2", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchOrExcludePostsInChannel(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test fromuser", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User2.Id, th.ChannelPrivate.Id, "test fromuser 2", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchOrExcludePostsInDMGM(t *testing.T, th *SearchTestHelper) {
	...
	p1, err := th.createPost(th.User.Id, direct.Id, "test fromuser", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User2.Id, group.Id, "test fromuser 2", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
	defer th.deleteUserPosts(th.User2.Id)
...
func testFilterMessagesInSpecificDate(t *testing.T, th *SearchTestHelper) {
	...
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test in specific date", "", model.PostDefault, creationDate, false)
	require.NoError(t, err)
	creationDate2 := model.GetMillisForTime(time.Date(2020, 03, 23, 0, 0, 0, 0, time.UTC))
	p2, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "test in the present", "", model.PostDefault, creationDate2, false)
	require.NoError(t, err)
	creationDate3 := model.GetMillisForTime(time.Date(2020, 03, 21, 23, 59, 59, 0, time.UTC))
	p3, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test in the present", "", model.PostDefault, creationDate3, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testFilterMessagesBeforeSpecificDate(t *testing.T, th *SearchTestHelper) {
	...
	p2, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "test in specific date 2", "", model.PostDefault, creationDate2, false)
	require.NoError(t, err)
...
func testFilterMessagesWithATerm(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "one two three", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "one four five six", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "one seven eight nine", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchUsingBooleanOperators(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "one two three message", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "two messages", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "another message", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchUsingCombinedFilters(t *testing.T, th *SearchTestHelper) {
	creationDate := model.GetMillisForTime(time.Date(2020, 03, 01, 12, 0, 0, 0, time.UTC))
	p1, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "one two three message", "", model.PostDefault, creationDate, false)
	require.NoError(t, err)
	creationDate2 := model.GetMillisForTime(time.Date(2020, 03, 10, 12, 0, 0, 0, time.UTC))
	p2, err := th.createPost(th.User2.Id, th.ChannelPrivate.Id, "two messages", "", model.PostDefault, creationDate2, false)
	require.NoError(t, err)
	creationDate3 := model.GetMillisForTime(time.Date(2020, 03, 20, 12, 0, 0, 0, time.UTC))
	p3, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "two another message", "", model.PostDefault, creationDate3, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
	defer th.deleteUserPosts(th.User2.Id)
...
func testSearchIgnoringStopWords(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "the search for a bunch of stop words", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "the objective is to avoid a bunch of stop words", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "in the a on to where you", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p4, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "where is the car?", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSupportStemming(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "search post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "another post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSupportWildcards(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "search post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "another post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testNotSupportPrecedingWildcards(t *testing.T, th *SearchTestHelper) {
	_, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "search post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "searching post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "another post", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchDiscardWildcardAlone(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "qwerty", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "qwertyjkl", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSupportTermsWithDash(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "search term-with-dash", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with dash", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSupportTermsWithUnderscore(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "search term_with_underscore", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with underscore", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchOrExcludePostsWithHashtags(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "search post with #hashtag", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with hashtag", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchHashtagWithMarkdown(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtag", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with `#hashtag`", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with **#hashtag**", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	p4, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with ~~#hashtag~~", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	p5, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with _#hashtag_", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearcWithMultipleHashtags(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtag", "#hashtwo #hashone", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching term with `#hashtag`", "#hashtwo #hashthree", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchPostsWithDotsInHashtags(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtag.dot", "#hashtag.dot", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchHashtagCaseInsensitive(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #HaShTaG", "#HaShTaG", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtag", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #HASHTAG", "#HASHTAG", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchHashtagWithDash(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtag-test", "#hashtag-test", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtagtest", "#hashtagtest", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchHashtagWithNumbers(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #h4sht4g", "#h4sht4g", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtag", "#hashtag", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchHashtagWithUnderscores(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtag_test", "#hashtag_test", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "searching hashtag #hashtagtest", "#hashtagtest", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchShouldExcludeSytemMessages(t *testing.T, th *SearchTestHelper) {
	_, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test system message one", "", model.PostJoinChannel, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "test system message two", "", model.PostLeaveChannel, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "test system message three", "", model.PostLeaveTeam, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "test system message four", "", model.PostAddToChannel, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "test system message five", "", model.PostAddToTeam, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "test system message six", "", model.PostHeaderChange, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchShouldBeAbleToMatchByMentions(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test system @testuser", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test system testuser", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "test system #testuser", "#testuser", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchInDeletedOrArchivedChannels(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelDeleted.Id, "message in deleted channel", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message in regular channel", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "message in private channel", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchTermsWithDashes(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with-dash-term", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with dash term", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchTermsWithDots(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with.dots.term", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with dots term", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchTermsWithUnderscores(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with_underscores_term", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with underscores term", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchBotAccountsPosts(t *testing.T, th *SearchTestHelper) {
	...
	require.NoError(t, err)
	p1, err := th.createPost(bot.UserId, th.ChannelBasic.Id, "bot test message", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(bot.UserId, th.ChannelPrivate.Id, "bot test message in private", "", model.PostDefault, 0, false)
	require.NoError(t, err)
...
func testSupportStemmingAndWildcards(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "approve", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "approved", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "approvedz", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSupportWildcardOutsideQuotes(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "hello world", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	p2, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "hell or heaven", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testHashtagSearchShouldSupportThreeOrMoreCharacters(t *testing.T, th *SearchTestHelper) {
	_, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "one char hashtag #1", "#1", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelPrivate.Id, "two chars hashtag #12", "#12", model.PostDefault, 0, false)
	require.NoError(t, err)
	p3, err := th.createPost(th.User.Id, th.ChannelPrivate.Id, "three chars hashtag #123", "#123", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSlashShouldNotBeCharSeparator(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "alpha/beta gamma, theta", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSearchEmailsWithoutQuotes(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message test@test.com", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	_, err = th.createPost(th.User.Id, th.ChannelBasic.Id, "message test2@test.com", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testSupportSearchTermsWithinLinks(t *testing.T, th *SearchTestHelper) {
	p1, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with link http://www.wikipedia.org/dolphins", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
...
func testShouldNotReturnLinksEmbeddedInMarkdown(t *testing.T, th *SearchTestHelper) {
	_, err := th.createPost(th.User.Id, th.ChannelBasic.Id, "message with link [here](http://www.wikipedia.org/dolphins)", "", model.PostDefault, 0, false)
	require.NoError(t, err)
	defer th.deleteUserPosts(th.User.Id)
