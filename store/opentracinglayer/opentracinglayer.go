...
func (s *OpenTracingLayerPostAcknowledgementStore) Delete(userID string, postID string) (*model.PostAcknowledgement, error) {
	origCtx := s.Root.Store.Context()
	...
	defer span.Finish()
	result, err := s.PostAcknowledgementStore.Delete(userID, postID)
	if err != nil {
		span.LogFields(spanlog.Error(err))
		ext.Error.Set(span, true)
	}
...
func (s *OpenTracingLayerPostAcknowledgementStore) Save(userID string, postID string) (*model.PostAcknowledgement, error) {
	origCtx := s.Root.Store.Context()
	...
	defer span.Finish()
	result, err := s.PostAcknowledgementStore.Save(userID, postID)
	if err != nil {
		span.LogFields(spanlog.Error(err))
		ext.Error.Set(span, true)
	}
...
func (s *OpenTracingLayerPostPriorityStore) DeletePersistentNotificationsPosts(postIds []string) error {
	origCtx := s.Root.Store.Context()
	span, newCtx := tracing.StartSpanWithParentByContext(s.Root.Store.Context(), "PostPriorityStore.DeletePersistentNotificationsPosts")
	s.Root.Store.SetContext(newCtx)
	defer func() {
		s.Root.Store.SetContext(origCtx)
	}()
...
func (s *OpenTracingLayerPostPriorityStore) GetPersistentNotificationsPosts(minCreateAt int64) ([]*model.PostPersistentNotifications, error) {
	origCtx := s.Root.Store.Context()
