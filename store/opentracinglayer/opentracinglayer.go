...
func (s *OpenTracingLayerChannelStore) GetDmAndGmToDisplayNameMap(userID string, displaySetting string) (map[string]string, error) {
	origCtx := s.Root.Store.Context()
	span, newCtx := tracing.StartSpanWithParentByContext(s.Root.Store.Context(), "ChannelStore.GetDmAndGmToDisplayNameMap")
	s.Root.Store.SetContext(newCtx)
	defer func() {
		s.Root.Store.SetContext(origCtx)
	}()
