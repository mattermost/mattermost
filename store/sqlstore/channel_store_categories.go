...
// `teamId` is replaced with _ to pass unused param lint
func (s SqlChannelStore) UpdateSidebarCategories(userId, _ string, categories []*model.SidebarCategoryWithChannels) ([]*model.SidebarCategoryWithChannels, []*model.SidebarCategoryWithChannels, error) {
	transaction, err := s.GetMaster().Begin()
	if err != nil {
...
// `newTeamId` is replaced with _ to pass unused param lint
func (s SqlChannelStore) UpdateSidebarChannelCategoryOnMove(channel *model.Channel, _ string) error {
