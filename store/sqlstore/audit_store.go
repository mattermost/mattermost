...
func (s SqlAuditStore) Save(audit *model.Audit) store.StoreChannel {
	return store.Async(func() (interface{}, *model.AppError) {
		audit.Id = model.NewId()
		audit.CreateAt = model.GetMillis()

		if err := s.GetMaster().Insert(audit); err != nil {
			return nil, model.NewAppError("SqlAuditStore.Save", "store.sql_audit.save.saving.app_error", nil, "user_id="+audit.UserId+" action="+audit.Action, http.StatusInternalServerError)
		}
		return nil, nil
	})
}
...
func (s SqlAuditStore) PermanentDeleteByUser(userId string) store.StoreChannel {
	return store.Async(func() (interface{}, *model.AppError) {
		if _, err := s.GetMaster().Exec("DELETE FROM Audits WHERE UserId = :userId",
			...
			return nil, model.NewAppError("SqlAuditStore.Delete", "store.sql_audit.permanent_delete_by_user.app_error", nil, "user_id="+userId, http.StatusInternalServerError)
		}
		return nil, nil
	})
}
...
func (s SqlAuditStore) PermanentDeleteBatch(endTime int64, limit int64) store.StoreChannel {
	return store.Async(func() (interface{}, *model.AppError) {
		...
		if err != nil {
			return nil, model.NewAppError("SqlAuditStore.PermanentDeleteBatch", "store.sql_audit.permanent_delete_batch.app_error", nil, ""+err.Error(), http.StatusInternalServerError)
		}

		rowsAffected, err1 := sqlResult.RowsAffected()
		if err1 != nil {
			return int64(0), model.NewAppError("SqlAuditStore.PermanentDeleteBatch", "store.sql_audit.permanent_delete_batch.app_error", nil, ""+err.Error(), http.StatusInternalServerError)
		}
