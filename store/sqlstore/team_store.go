...
func (s SqlTeamStore) Delete(teamId string, time int64) store.StoreChannel {
	return s.SetDeleteAt(teamId, time, time)
}

// SetDeleteAt records the given deleted and updated timestamp to the team in question.
func (s SqlTeamStore) SetDeleteAt(teamId string, deleteAt, updateAt int64) store.StoreChannel {
	return store.Do(func(result *store.StoreResult) {
		transaction, err := s.GetMaster().Begin()
		if err != nil {
			result.Err = model.NewAppError("SqlTeamStore.SetDeleteAt", "store.sql_team.set_delete_at.open_transaction.app_error", nil, err.Error(), http.StatusInternalServerError)
			return
		}

		*result = s.setDeleteAtT(transaction, teamId, deleteAt, updateAt)
		if result.Err != nil {
		...
		if err := transaction.Commit(); err != nil {
			result.Err = model.NewAppError("SqlTeamStore.SetDeleteAt", "store.sql_team.set_delete_at.commit_transaction.app_error", nil, err.Error(), http.StatusInternalServerError)
			return
		}
	})
}

func (s SqlTeamStore) setDeleteAtT(transaction *gorp.Transaction, teamId string, deleteAt, updateAt int64) store.StoreResult {
	result := store.StoreResult{}

	_, err := transaction.Exec("Update Teams SET DeleteAt = :DeleteAt, UpdateAt = :UpdateAt WHERE Id = :TeamId", map[string]interface{}{"DeleteAt": deleteAt, "UpdateAt": updateAt, "TeamId": teamId})
	if err != nil {
		result.Err = model.NewAppError("SqlTeamStore.Delete", "store.sql_team.delete.team.app_error", nil, "id="+teamId+", err="+err.Error(), http.StatusInternalServerError)
		return result
	}
	return result
}
