...
// `userId` is replaced with _ to pass unused param lint
func (s SqlTeamStore) InvalidateAllTeamIdsForUser(_ string) {}
...
// `allowFromCache` is replaced with _ to pass unused param lint
func (s SqlTeamStore) GetUserTeamIds(userId string, _ bool) ([]string, error) {
	var teamIds []string
	query, args, err := s.getQueryBuilder().
		Select("TeamId").
		From("TeamMembers").
