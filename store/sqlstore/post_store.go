...
func (s *SqlPostStore) determineMaxPostSize() int {
	...
	if s.DriverName() == model.DATABASE_DRIVER_POSTGRES {
		...
		`); err != nil {
			mlog.Error(utils.T("store.sql_post.query_max_post_size.error"), mlog.Err(err))
		}
	} else if s.DriverName() == model.DATABASE_DRIVER_MYSQL {
	...
	mlog.Info("Post.Message supports at most", mlog.Int("characters", maxPostSize), mlog.Int32("(bytes)", maxPostSizeBytes))
