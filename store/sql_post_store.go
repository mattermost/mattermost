...
func (s SqlPostStore) GetAll(channelId string) StoreChannel {
	storeChannel := make(StoreChannel)
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewAppError("SqlPostStore.GetAll", "We couldn't get the posts for the channel", "channelId="+channelId+err.Error())
		} else {
			result.Data = posts
		}
