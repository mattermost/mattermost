...
func (s SqlTeamStore) UpgradeSchemaIfNeeded(T goi18n.TranslateFunc) {
	s.RemoveColumnIfExists("Teams", "AllowValet", T)
	s.CreateColumnIfNotExists("Teams", "InviteId", "varchar(32)", "varchar(32)", "", T)
	s.CreateColumnIfNotExists("Teams", "AllowOpenInvite", "tinyint(1)", "boolean", "0", T)
	s.CreateColumnIfNotExists("Teams", "AllowTeamListing", "tinyint(1)", "boolean", "0", T)
}

func (s SqlTeamStore) CreateIndexesIfNotExists(T goi18n.TranslateFunc) {
	s.CreateIndexIfNotExists("idx_teams_name", "Teams", "Name", T)
	s.CreateIndexIfNotExists("idx_teams_invite_id", "Teams", "InviteId", T)
}
