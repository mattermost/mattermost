...
func TestUserStore(t *testing.T, ss store.Store) {
	...
	t.Run("GetUserWithServiceTerms", func(t *testing.T) { testUserStoreGetWithServiceTerms(t, ss) })
	t.Run("GetAllProfilesWithServiceTerms", func(t *testing.T) { testUserStoreGetAllProfilesWithServiceTerms(t, ss) })
	t.Run("GetAllUsingAuthServiceWithServiceTerms", func(t *testing.T) { testGetAllUsingAuthServiceWithServiceTerms(t, ss) })
}
...
func testGetAllUsingAuthServiceWithServiceTerms(t *testing.T, ss store.Store) {
	...
	u1.Email = MakeEmail()
	u1.AuthService = "someservice"
	u1.AcceptedServiceTermsId = st1.Id
	store.Must(ss.User().Save(u1))

	u2 := &model.User{}
	u2.Email = MakeEmail()
	u2.AuthService = "someservice"
	u2.AcceptedServiceTermsId = st1.Id
	store.Must(ss.User().Save(u2))
