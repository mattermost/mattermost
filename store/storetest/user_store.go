...
func testUserStoreGetProfilesWithoutTeam(t *testing.T, ss store.Store) {
	...
	t.Run("get, offset 0, limit 100", func(t *testing.T) {
		result, err := ss.User().GetProfilesWithoutTeam(0, 100, nil)
		require.Nil(t, err)
		assert.Equal(t, []*model.User{sanitized(u2), sanitized(u3)}, result)
	})

	t.Run("get, offset 1, limit 1", func(t *testing.T) {
		result, err := ss.User().GetProfilesWithoutTeam(1, 1, nil)
		require.Nil(t, err)
		assert.Equal(t, []*model.User{sanitized(u3)}, result)
	})

	t.Run("get, offset 2, limit 1", func(t *testing.T) {
		result, nil := ss.User().GetProfilesWithoutTeam(2, 1, nil)
		require.Nil(t, nil)
