...
func testPostStoreSave(t *testing.T, ss store.Store) {
	...
	t.Run("Create a post should update the channel LastPostAt and the total messages count by one", func(t *testing.T) {
		...
		channel.Name = NewTestId() + "b"
		channel.DisplayName = NewTestId() + "b"
...
func testPostStoreGetForThread(t *testing.T, ss store.Store) {
	o1 := &model.Post{ChannelId: model.NewId(), UserId: model.NewId(), Message: NewTestId() + "b"}
	o1, err := ss.Post().Save(o1)
	require.NoError(t, err)
	_, err = ss.Post().Save(&model.Post{ChannelId: o1.ChannelId, UserId: model.NewId(), Message: NewTestId() + "b", RootId: o1.Id})
	require.NoError(t, err)
...
func testPostStoreUpdate(t *testing.T, ss store.Store) {
	...
	o1.ChannelId = model.NewId()
	o1.UserId = model.NewId()
	o1.Message = NewTestId() + "AAAAAAAAAAA"
	o1, err := ss.Post().Save(o1)
	require.NoError(t, err)
	...
	o2.ChannelId = o1.ChannelId
	o2.UserId = model.NewId()
	o2.Message = NewTestId() + "CCCCCCCCC"
	o2.RootId = o1.Id
	o2, err = ss.Post().Save(o2)
	require.NoError(t, err)
	...
	o3.ChannelId = o1.ChannelId
	o3.UserId = model.NewId()
	o3.Message = NewTestId() + "QQQQQQQQQQ"
...
func testPostStoreGetPostBeforeAfter(t *testing.T, ss store.Store) {
	...
	otherChannelPost.Message = NewTestId() + "b"
...
func testPostStorePermanentDeleteBatch(t *testing.T, ss store.Store) {
	...
	o2.UserId = model.NewId()
	o2.Message = NewTestId() + "AAAAAAAAAAA"
	o2.CreateAt = 1000
	o2, err = ss.Post().Save(o2)
	require.NoError(t, err)
	...
	o3.UserId = model.NewId()
	o3.Message = NewTestId() + "AAAAAAAAAAA"
	o3.CreateAt = 100000
	o3, err = ss.Post().Save(o3)
	require.NoError(t, err)
...
func testPostStoreGetParentsForExportAfter(t *testing.T, ss store.Store) {
	...
	p1.ChannelId = c1.Id
	p1.UserId = u1.Id
	p1.Message = NewTestId() + "AAAAAAAAAAA"
	p1.CreateAt = 1000
...
func testPostStoreGetRepliesForExport(t *testing.T, ss store.Store) {
	...
	p2.ChannelId = c1.Id
	p2.UserId = u1.Id
	p2.Message = NewTestId() + "AAAAAAAAAAA"
	p2.CreateAt = 1001
	p2.RootId = p1.Id
...
func testPostStoreGetDirectPostParentsForExportAfterDeleted(t *testing.T, ss store.Store, s SqlStore) {
	...
	p1.ChannelId = o1.Id
	p1.UserId = u1.Id
	p1.Message = NewTestId() + "BBBBBBBBBBBB"
	p1.CreateAt = 1000
	p1, nErr = ss.Post().Save(p1)
	require.NoError(t, nErr)
