...
func (_m *TrueUpReviewStore) GetTrueUpReviewStatus(dueDate string) (*model.TrueUpReviewStatus, error) {
	ret := _m.Called(dueDate)

	var r0 *model.TrueUpReviewStatus
	if rf, ok := ret.Get(0).(func(string) *model.TrueUpReviewStatus); ok {
		r0 = rf(dueDate)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.TrueUpReviewStatus)
