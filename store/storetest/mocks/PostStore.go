...
// QueryHashTag provides a mock function with given fields: hash_tag_query, count
func (_m *PostStore) QueryHashTag(hash_tag_query *string, count uint64) ([]*string, error) {
	ret := _m.Called(hash_tag_query, count)

	var r0 []*string
	if rf, ok := ret.Get(0).(func(*string, uint64) []*string); ok {
		r0 = rf(hash_tag_query, count)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*string)
		}
