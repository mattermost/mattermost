...
func testGetChannelUnread(t *testing.T, ss store.Store) {
	...
	// Setup Channel 1
	c1 := &model.Channel{TeamId: m1.TeamId, Name: model.NewId(), DisplayName: "Downtown", Type: model.ChannelOpen, TotalMsgCount: 100, TotalMsgCountRoot: 99}
	...
	// Setup Channel 2
	c2 := &model.Channel{TeamId: m2.TeamId, Name: model.NewId(), DisplayName: "Cultural", Type: model.ChannelOpen, TotalMsgCount: 100, TotalMsgCountRoot: 100}
...
func testChannelStoreGetMoreChannels(t *testing.T, ss store.Store) {
	...
	t.Run("verify analytics for open channels", func(t *testing.T) {
		count, err := ss.Channel().AnalyticsTypeCount(teamId, model.ChannelOpen)
		require.NoError(t, err)
		require.EqualValues(t, 4, count)
	})

	t.Run("verify analytics for private channels", func(t *testing.T) {
		count, err := ss.Channel().AnalyticsTypeCount(teamId, model.ChannelPrivate)
		require.NoError(t, err)
