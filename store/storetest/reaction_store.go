...
func testReactionSave(t *testing.T, ss store.Store) {
	...
	postList, err := ss.Post().Get(reaction1.PostId, false, false)
	require.Nil(t, err)
	...
	postList, err = ss.Post().Get(reaction2.PostId, false, false)
	require.Nil(t, err)
...
func testReactionDelete(t *testing.T, ss store.Store) {
	...
	result, err := ss.Post().Get(reaction.PostId, false, false)
	require.Nil(t, err)
	...
	postList, err := ss.Post().Get(post.Id, false, false)
	require.Nil(t, err)
...
func testReactionDeleteAllWithEmojiName(t *testing.T, ss store.Store) {
	...
	postList, err = ss.Post().Get(post2.Id, false, false)
	require.Nil(t, err)
	assert.True(t, postList.Posts[post2.Id].HasReactions, "post should still have reactions")

	postList, err = ss.Post().Get(post3.Id, false, false)
	require.Nil(t, err)
