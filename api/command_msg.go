	...
	if profileList := <-Srv.Store.User().GetAllProfiles(); profileList.Err != nil {
		...
		for _, userProfile := range profileUsers {
			...
			if userProfile.Username == targetUser {
				...
				return &model.CommandResponse{GotoLocation: c.GetTeamURL() + "/channels/" + channelName, GotoChannel: targetChannelId, Text: c.T("api.command_msg.success"), ResponseType: model.COMMAND_RESPONSE_TYPE_EPHEMERAL}
			}
		}
	}
