...
func TestCommandWebhooks(t *testing.T) {
	...
	if _, err := Client.DoPost("/hooks/commands/123123123123", "{\"text\":\"this is a test\"}", "application/json"); err.StatusCode != http.StatusNotFound {
		t.Fatal("expected not-found for non-existent hook")
	}

	for i := 0; i < 5; i++ {
		if _, err := Client.DoPost("/hooks/commands/"+hook.Id, "{\"text\":\"this is a test\"}", "application/json"); err != nil {
			t.Fatal(err)
		}
	}

	if _, err := Client.DoPost("/hooks/commands/"+hook.Id, "{\"text\":\"this is a test\"}", "application/json"); err == nil || err.StatusCode != http.StatusBadRequest {
