...
func createTeamFromSignup(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if result := <-Srv.Store.Team().Save(&teamSignup.Team, T); result.Err != nil {
		...
		ruser, err := CreateUser(rteam, &teamSignup.User, T)
		if err != nil {
			c.Err = err
			return
		}
...
func inviteMembers(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	for i, invite := range invites.Invites {
		if result := <-Srv.Store.User().GetByEmail(c.Session.TeamId, invite["email"], T); result.Err == nil || result.Err.Message != T(store.MISSING_ACCOUNT_ERROR) {
			invNum = int64(i)
