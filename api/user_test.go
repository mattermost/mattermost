...
func TestCreateUser(t *testing.T) {
	...
	ruser, err := Client.CreateUser(&user, "", T)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := Client.CreateUser(ruser.Data.(*model.User), "", T); err == nil {
	...
	if _, err := Client.DoApiPost("/users/create", "garbage", T); err == nil {
...
func TestCreateUserAllowedDomains(t *testing.T) {
	...
	_, err := Client.CreateUser(&user, "", T)
	if err == nil {
...
func TestLogin(t *testing.T) {
	...
	if result, err := Client.LoginById(ruser.Data.(*model.User).Id, user.Password, T); err != nil {
		t.Fatal(err)
	} else {
	...
	if result, err := Client.LoginByEmail(team.Name, user.Email, user.Password, T); err != nil {
		t.Fatal(err)
	} else {
		if result.Data.(*model.User).Email != user.Email {
	...
	if _, err := Client.LoginByEmail(team.Name, user.Email, user.Password+"invalid", T); err == nil {
		t.Fatal("Invalid Password")
	}

	if _, err := Client.LoginByEmail(team.Name, "", user.Password, T); err == nil {
		t.Fatal("should have failed")
	}
	...
	if _, err := Client.GetUser(ruser.Data.(*model.User).Id, "", T); err == nil {
		t.Fatal("should have failed")
	}
	...
	rteam2 := Client.Must(Client.CreateTeam(&team2, T))
	...
	if _, err := Client.CreateUserFromSignup(&user2, "junk", "1231312", T); err == nil {
	...
	ruser2, _ := Client.CreateUserFromSignup(&user2, data, hash, T)

	if _, err := Client.LoginByEmail(team2.Name, ruser2.Data.(*model.User).Email, user2.Password, T); err != nil {
...
func TestLoginWithDeviceId(t *testing.T) {
	...
	if result, err := Client.LoginByEmailWithDevice(team.Name, user.Email, user.Password, deviceId, T); err != nil {
		t.Fatal(err)
	} else {
		ruser := result.Data.(*model.User)

		if ssresult, err := Client.GetSessions(ruser.Id, T); err != nil {
			t.Fatal(err)
		} else {
			sessions := ssresult.Data.([]*model.Session)
			if _, err := Client.LoginByEmailWithDevice(team.Name, user.Email, user.Password, deviceId, T); err != nil {
				t.Fatal(err)
			}

			if sresult := <-Srv.Store.Session().Get(sessions[0].Id, T); sresult.Err == nil {
...
func TestSessions(t *testing.T) {
	...
	deviceId := model.NewId()
	Client.LoginByEmailWithDevice(team.Name, user.Email, user.Password, deviceId, T)
	Client.LoginByEmail(team.Name, user.Email, user.Password, T)

	r1, err := Client.GetSessions(ruser.Id, T)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := Client.RevokeSession(otherSession, T); err != nil {
		t.Fatal(err)
	}

	r2, err := Client.GetSessions(ruser.Id, T)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetUser(t *testing.T) {
	...
	ruser2, _ := Client.CreateUser(&user2, "", T)
	store.Must(Srv.Store.User().VerifyEmail(ruser2.Data.(*model.User).Id, T))

	team2 := model.Team{DisplayName: "Name", Name: "z-z-" + model.NewId() + "a", Email: "test@nowhere.com", Type: model.TEAM_OPEN}
	rteam2, _ := Client.CreateTeam(&team2, T)
	...
	ruser3, _ := Client.CreateUser(&user3, "", T)
	store.Must(Srv.Store.User().VerifyEmail(ruser3.Data.(*model.User).Id, T))

	Client.LoginByEmail(team.Name, user.Email, user.Password, T)

	rId := ruser.Data.(*model.User).Id
	if result, err := Client.GetUser(rId, "", T); err != nil {
		...
		if cache_result, err := Client.GetUser(rId, result.Etag, T); err != nil {
			t.Fatal(err)
	...
	if result, err := Client.GetMe("", T); err != nil {
		t.Fatal("Failed to get user")
	} else {
		if result.Data.(*model.User).Password != "" {
			t.Fatal("User shouldn't have any password data once set")
	...
	if _, err := Client.GetUser("FORBIDDENERROR", "", T); err == nil {
		t.Fatal("shouldn't exist")
	}

	if _, err := Client.GetUser(ruser2.Data.(*model.User).Id, "", T); err == nil {
		t.Fatal("shouldn't have accss")
	}

	if userMap, err := Client.GetProfiles(rteam.Data.(*model.Team).Id, "", T); err != nil {
		t.Fatal(err)
		...
		// test etag caching
		if cache_result, err := Client.GetProfiles(rteam.Data.(*model.Team).Id, userMap.Etag, T); err != nil {
			t.Fatal(err)
	...
	if _, err := Client.GetProfiles(rteam2.Data.(*model.Team).Id, "", T); err == nil {
	...
	UpdateRoles(c, ruser.Data.(*model.User), model.ROLE_SYSTEM_ADMIN, T)
...
func TestGetAudits(t *testing.T) {
	...
	if result, err := Client.GetAudits(ruser.Data.(*model.User).Id, "", T); err != nil {
		t.Fatal(err)
	} else {
		...
		if cache_result, err := Client.GetAudits(ruser.Data.(*model.User).Id, result.Etag, T); err != nil {
			t.Fatal(err)
	...
	if _, err := Client.GetAudits("FORBIDDENERROR", "", T); err == nil {
...
func TestUserCreateImage(t *testing.T) {
	...
	Client.DoApiGet("/users/"+user.Id+"/image", "", "", T)
...
func TestUserUploadProfileImage(t *testing.T) {
	...
	if utils.Cfg.FileSettings.DriverName != "" {
		...
		if _, upErr := Client.UploadFile("/users/newimage", body.Bytes(), writer.FormDataContentType(), T); upErr == nil {
		...
		body := &bytes.Buffer{}
		writer := multipart.NewWriter(body)
		if _, upErr := Client.UploadFile("/users/newimage", body.Bytes(), writer.FormDataContentType(), T); upErr.StatusCode != http.StatusNotImplemented {
...
func TestUserUpdate(t *testing.T) {
	...
	if _, err := Client.UpdateUser(user, T); err == nil {
		t.Fatal("Should have errored")
	}
	...
	if result, err := Client.UpdateUser(user, T); err != nil {
		t.Fatal(err)
	} else {
	...
	user.TeamId = team.Id
	if _, err := Client.UpdateUser(nil, T); err == nil {
		t.Fatal("Should have errored")
	}
...
func TestUserUpdatePassword(t *testing.T) {
	...
	if _, err := Client.UpdateUserPassword(user.Id, "pwd", "newpwd", T); err == nil {
		t.Fatal("Should have errored")
	}

	Client.LoginByEmail(team.Name, user.Email, "pwd", T)

	if _, err := Client.UpdateUserPassword("123", "pwd", "newpwd", T); err == nil {
		t.Fatal("Should have errored")
	}

	if _, err := Client.UpdateUserPassword(user.Id, "", "newpwd", T); err == nil {
		t.Fatal("Should have errored")
	}

	if _, err := Client.UpdateUserPassword(user.Id, "pwd", "npwd", T); err == nil {
		t.Fatal("Should have errored")
	}

	if _, err := Client.UpdateUserPassword("12345678901234567890123456", "pwd", "newpwd", T); err == nil {
		t.Fatal("Should have errored")
	}

	if _, err := Client.UpdateUserPassword(user.Id, "badpwd", "newpwd", T); err == nil {
		t.Fatal("Should have errored")
	}
	...
	updatedUser := Client.Must(Client.GetUser(user.Id, "", T)).Data.(*model.User)
	if updatedUser.LastPasswordUpdate == user.LastPasswordUpdate {
		t.Fatal("LastPasswordUpdate should have changed")
	}

	if _, err := Client.LoginByEmail(team.Name, user.Email, "newpwd", T); err != nil {
		t.Fatal(err)
	}
...
func TestUserUpdateRoles(t *testing.T) {
	...
	if _, err := Client.UpdateUserRoles(data, T); err == nil {
	...
	if result, err := Client.UpdateUserRoles(data, T); err != nil {
...
func TestUserUpdateActive(t *testing.T) {
	...
	if _, err := Client.UpdateActive(user.Id, false, T); err == nil {
		t.Fatal("Should have errored, not logged in")
	}
	...
	if _, err := Client.UpdateActive("junk", false, T); err == nil {
		t.Fatal("Should have errored, bad id")
	}

	if _, err := Client.UpdateActive("12345678901234567890123456", false, T); err == nil {
		t.Fatal("Should have errored, bad id")
	}

	if result, err := Client.UpdateActive(user2.Id, false, T); err != nil {
		t.Fatal(err)
	} else {
	...
	if result, err := Client.UpdateActive(user2.Id, true, T); err != nil {
		t.Fatal(err)
	} else {
		if result.Data.(*model.User).DeleteAt != 0 {
...
func TestUserPermDelete(t *testing.T) {
	...
	err := PermanentDeleteUser(c, user1, T)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSendPasswordReset(t *testing.T) {
	...
	if _, err := Client.SendPasswordReset(data, T); err != nil {
		t.Fatal(err)
	}
...
func TestResetPassword(t *testing.T) {
	...
	if _, err := Client.ResetPassword(data, T); err != nil {
		t.Fatal(err)
	}
...
func TestUserUpdateNotify(t *testing.T) {
	...
	if _, err := Client.UpdateUserNotify(data, T); err == nil {
		t.Fatal("Should have errored - not logged in")
	}

	Client.LoginByEmail(team.Name, user.Email, "pwd", T)

	if result, err := Client.UpdateUserNotify(data, T); err != nil {
		t.Fatal(err)
	} else {
	...
	if _, err := Client.UpdateUserNotify(nil, T); err == nil {
		t.Fatal("Should have errored")
	}
...
func TestStatuses(t *testing.T) {
	...
	user2 := model.User{TeamId: rteam.Data.(*model.Team).Id, Email: strings.ToLower(model.NewId()) + "corey+test@test.com", Nickname: "Corey Hulen", Password: "pwd"}
	ruser2 := Client.Must(Client.CreateUser(&user2, "", T)).Data.(*model.User)
	store.Must(Srv.Store.User().VerifyEmail(ruser2.Id, T))

	Client.LoginByEmail(team.Name, user.Email, user.Password, T)

	userIds := []string{ruser2.Id}

	r1, err := Client.GetStatuses(userIds, T)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSwitchToSSO(t *testing.T) {
	...
	m := map[string]string{}
	if _, err := Client.SwitchToSSO(m, T); err == nil {
		t.Fatal("should have failed - empty data")
	}

	m["password"] = "pwd"
	_, err := Client.SwitchToSSO(m, T)
	if err == nil {
