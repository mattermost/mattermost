...
func (ds *DatabaseStore) Load() ([]byte, error) {
	...
	if len(configurationData) == 0 {
		...
		configWithDB.SqlSettings.DriverName = new(string)
		*configWithDB.SqlSettings.DriverName = ds.driverName
		configWithDB.SqlSettings.DataSource = new(string)
		*configWithDB.SqlSettings.DataSource = ds.dataSourceName
