...
3. Verify MCP client is None when no config: `python -c "from plugin import LangChainAgentPlugin; p = LangChainAgentPlugin(); print(f'mcp_client={p.mcp_client}')"` should show `mcp_client=None`
  </verify>
