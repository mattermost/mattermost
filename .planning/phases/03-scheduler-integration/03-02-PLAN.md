...
This method:
1. Resolves channel IDs based on ChannelMode (specific vs all_unreads)
2. Creates a Recap using the existing recap creation flow
3. Returns the created Recap
...
func (a *App) CreateRecapFromSchedule(rctx request.CTX, sr *model.ScheduledRecap) (*model.Recap, *model.AppError) {
    ...
    // Create the recap using the existing flow
    recap := &model.Recap{
        Title:     sr.Title,
        UserId:    sr.UserId,
    ...
    return a.CreateRecap(rctx, recap)
}
```

Note: Check if CreateRecap exists in app/recap.go. If it has a different signature, adapt accordingly. The key is to create a Recap record and trigger the existing recap job.
  </action>
  <verify>
Run `go build ./server/channels/app/` - should compile without errors.
Run `grep -n "CreateRecapFromSchedule" server/channels/app/scheduled_recap.go` - should show the method.
  ...
  <done>
CreateRecapFromSchedule method exists and creates a Recap from ScheduledRecap configuration.
  </done>
</task>
...
2. If there are compile errors, fix them:
   - Check that AppIface in worker.go matches the App method signature
...
- If CreateRecap doesn't exist with the expected signature, check app/recap.go for the actual method

If the existing recap creation uses a different pattern (e.g., job-based), adapt CreateRecapFromSchedule to match:
```go
// Alternative if recap uses job creation
func (a *App) CreateRecapFromSchedule(rctx request.CTX, sr *model.ScheduledRecap) (*model.Recap, *model.AppError) {
    // Create recap record first
    recap := &model.Recap{...}
    savedRecap, err := a.Srv().Store().Recap().CreateRecap(recap)
    if err != nil {
    ...
    _, jobErr := a.Srv().Jobs.CreateJob(rctx, model.JobTypeRecap, jobData)
    if jobErr != nil {
        return nil, jobErr
    }
