...
must_haves:
  truths:
    - "Agent recovers gracefully from transient network errors without user intervention"
    - "Agent completes without hanging or infinite loops, even on complex queries"
    - "Agent shows thoughtful reasoning on complex problems (extended thinking)"
    - "Agent provides helpful response even when tools are unavailable"
  artifacts:
...
<objective>
Enhance the LangChain agent with robust multi-step execution capabilities using LangGraph's ReAct pattern, add extended thinking for complex reasoning, and implement graceful error handling with retry logic.

Purpose: Transform the basic ReAct agent into a production-quality agentic loop. The ReAct pattern (Reason → Act → Observe → repeat) IS the multi-step execution model - the agent iterates through model→tool→observe cycles up to `recursion_limit` times. This phase hardens that loop with retry logic, iteration limits, and thoughtful error handling. Note: orchestrator-worker patterns and parallel tool execution are out of scope for this demo plugin.

Output: Enhanced `plugin.py` with recursion-limited ReAct agent (up to 10 reasoning steps), extended thinking for Anthropic, and try/except-based tool error handling with tenacity retries.
</objective>
