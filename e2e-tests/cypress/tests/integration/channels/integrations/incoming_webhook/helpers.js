...
export function enableUsernameAndIconOverrideInt(enableUsername, enableIcon) {
    // Patch only these two flags. The admin-console Save control stays disabled
    // when the radios already match leftover config, and apiUpdateConfig also
    // resets e2e defaults. Wait until both flags are visible to the API so a
