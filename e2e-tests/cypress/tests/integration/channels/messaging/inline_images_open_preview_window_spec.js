...
describe('Messaging', () => {
    ...
    it('MM-T187 Inline markdown images open preview window', () => {
        ...
        // # Post the message using the post textbox helper
        cy.uiGetPostTextBox().should('be.visible').clear().type(message + '{enter}');

        // * Scope assertions to the newly created post to avoid flakiness from global selectors
        cy.getLastPostId().then((postId) => {
            cy.get(`#post_${postId}`).within(() => {
                // * Wait for the message to appear in the post content
