...
function promptAddLink(fromChannelMenu = false) {
    if (fromChannelMenu) {
        ...
        cy.findByRole('menuitem', {name: 'Tabs Bar submenu icon'}).trigger('mouseover');
        cy.findByRole('menuitem', {name: 'Add a link not selected'}).click();
    } else {
        cy.get('#channelTabsPlusMenuButton').click();
        cy.get('#channelTabsAddLink').click();
    }
}
...
function promptAddFile(fromChannelMenu = false) {
    if (fromChannelMenu) {
        ...
        cy.get('#channelTabsPlusMenuButton').click();
        cy.get('#channelTabsAttachFile').click();
    }
}
...
function openDotMenu(name: string) {
    cy.findByTestId('channel-tabs-container').within(() => {
        ...
        cy.findByRole('link', {name}).scrollIntoView().focus().
            parent('div').findByRole('button', {name: 'Tab menu'}).click();
    });
}
...
function createFileTab({
    ...
    cy.get('#root-portal #tab-create-file-input').attachFile(file);
