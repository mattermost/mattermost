...
mme2e_server_mmctl() {
  if [ "$SERVER" = "local" ]; then
    "$MME2E_LOCAL_SERVER_ROOT/bin/mmctl" --local "$@"
  else
    ${MME2E_DC_SERVER} exec -T -- server mmctl --local "$@"
  fi
}
...
for SETTING in \
  ...
  mme2e_server_mmctl config set $(tr '=' ' ' <<<$SETTING)
done
if [ -n "${MM_LICENSE:-}" ]; then
  ...
  mme2e_server_mmctl license upload-string "$MM_LICENSE"
fi
...
if [ "$TEST" = "cypress" ]; then
  ...
  if [ "$SERVER" = "local" ]; then
    (
      ...
      tee tests/fixtures/keycloak.crt >/dev/null <../../server/build/docker/keycloak/keycloak.crt
    )
  ...
  # Download plugins using the local host or Cypress container
  for PLUGIN_URL in \
...
if [ "$TEST" = "playwright" ] && [ "$SERVER" = "local" ]; then
