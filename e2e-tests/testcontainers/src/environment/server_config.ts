...
export function buildBaseEnvOverrides(
    ...
    if (connectionInfo.libretranslate) {
        // Note: Enable and Provider are set via mmctl after startup
        envOverrides.MM_AUTOTRANSLATIONSETTINGS_LIBRETRANSLATE_URL = 'http://libretranslate:5000';
    }
...
export async function configureServerViaMmctl(
    ...
    // Enable LibreTranslate auto-translation if configured (via mmctl so it can be changed in System Console)
    if (connectionInfo.libretranslate) {
        const autoTranslationSettings: Record<string, string | boolean> = {
            'AutoTranslationSettings.Enable': true,
            'AutoTranslationSettings.Provider': 'libretranslate',
        };
        for (const [key, value] of Object.entries(autoTranslationSettings)) {
            const formattedValue = formatConfigValue(value);
            const result = await mmctl.exec(`config set ${key} ${formattedValue}`);
            if (result.exitCode !== 0) {
                log(`⚠ Failed to set ${key}: ${result.stdout || result.stderr}`);
    ...
    // Users can configure SAML/OpenID manually via System Console or server.config in mattermost-testcontainers.config.mjs.
