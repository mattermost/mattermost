...
const OUTPUT_DIR = path.join(PROJECT_ROOT, '.tc.test-ha-deps');

describe('HA with extended deps (openldap, minio, elasticsearch)', {skip: SKIP_DOCKER_HA, timeout: 900_000}, () => {
    let startStdout = '';
    let startStderr = '';
    let nginxUrl = '';
    ...
    before(() => {
        ...
        const result = runCli(['start', '--ha', '--admin', '-D', 'openldap,minio,elasticsearch', '-o', OUTPUT_DIR]);
    ...
    it('.tc.docker.json has HA + dep containers', () => {
        const dockerInfo = readDockerInfo(OUTPUT_DIR);
        const containers = dockerInfo.containers as Record<string, unknown>;
        assert.ok(containers['mattermost-leader'], 'Expected mattermost-leader container');
        assert.ok(containers['mattermost-follower'], 'Expected mattermost-follower container');
    ...
    it('API via nginx is healthy', async () => {
