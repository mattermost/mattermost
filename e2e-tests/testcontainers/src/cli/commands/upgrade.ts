...
import {getContainerName, log, pullImage} from '@/utils';
...
async function executeUpgradeCommand(options: UpgradeOptions): Promise<void> {
    ...
    try {
        ...
        for (const containerKey of mmContainerKeys) {
            ...
            const configBefore = getServerConfig(containerInfo.id);
        ...
        try {
            pullImage(newImage, 'linux/amd64');
        ...
        for (const containerKey of mmContainerKeys) {
            ...
            const result = await upgradeContainer(containerKey, containerInfo, newImage);
        ...
        for (const {key, info, newContainerId} of upgradedContainers) {
            ...
            if (isReady) {
                ...
                const configAfter = getServerConfig(newContainerId);
        ...
        for (const {key, info, newContainerId} of upgradedContainers) {
            if (!info) continue;
            const newContainerName = getContainerName(newContainerId).replace(/^\//, '');
