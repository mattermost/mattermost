...
import {waitForContainer, getServerVersion, getServerConfig, saveConfigDiff} from '../container_utils';
...
export function registerRestartCommand(program: Command): void {
    program
        ...
        .action(async (options: OutputDirOptions) => {
 ...
 */
async function executeRestartCommand(options: OutputDirOptions): Promise<void> {
    try {
        ...
        // Separate into dependencies, mattermost, and nginx containers.
        // Nginx must restart AFTER mattermost in HA mode because the upstream block
        // resolves DNS at startup — if mattermost nodes aren't running, nginx exits.
        const mmContainerKeys = allContainerKeys.filter((key) => key === 'mattermost' || key.startsWith('mattermost-'));
        const isNginxPresent = allContainerKeys.includes('nginx');
        const depContainerKeys = allContainerKeys.filter(
            (key) => key !== 'mattermost' && !key.startsWith('mattermost-') && key !== 'nginx',
        );
        ...
        for (const containerKey of mmContainerKeys) {
            ...
            try {
                if (isContainerRunning(containerInfo.id)) {
        ...
        for (const containerKey of mmContainerKeys) {
            ...
            try {
                if (isContainerRunning(containerInfo.id)) {
                    ...
                    stopContainer(containerInfo.id);
        ...
        if (depContainerKeys.length > 0) {
            ...
            for (const containerKey of depContainerKeys) {
                ...
                if (!containerExists(containerInfo.id)) {
                ...
                if (isContainerRunning(containerInfo.id)) {
                    ...
                    try {
                        restartContainer(containerInfo.id);
                    } catch (error) {
                ...
                const nowRunning = isContainerRunning(containerInfo.id);
            ...
            const postgresInfo = dockerInfo.containers.postgres as
                | {id: string; username?: string; database?: string}
                | undefined;
            if (postgresInfo?.id) {
                ...
                for (let retry = 0; retry < maxRetries && !postgresReady; retry++) {
                    for (const delay of backoffDelays) {
                        try {
                            ...
                            execInContainer(
                                postgresInfo.id,
                                ['psql', '-U', pgUser, '-d', pgDatabase, '-w', '-c', 'SELECT 1'],
                                {PGPASSWORD: 'mostest'},
                            );
                            postgresReady = true;
                            break;
        ...
        const restartedContainers: Array<{key: string; info: ContainerInfo; containerId: string}> = [];
        ...
        for (const containerKey of mmContainerKeys) {
            ...
            try {
                ...
                const newPort = getContainerPort(containerInfo.id, 8065);
                if (newPort) {
                    actualPort = newPort;
                }

                // Update info with actual port.
                // For subpath containers, directUrl has the direct URL (no subpath),
                // and url has the subpath URL via nginx. Build a health check URL
                // using the new direct port + subpath from the original url.
                const originalParsed = new URL(containerInfo.url);
                const subpath = originalParsed.pathname.replace(/\/$/, ''); // e.g., "/mattermost1" or ""
                const actualDirectUrl = `http://localhost:${actualPort}`;
                const actualHealthUrl = subpath ? `${actualDirectUrl}${subpath}` : actualDirectUrl;
                const updatedInfo = {...containerInfo, port: actualPort, url: actualHealthUrl};
        ...
        for (const {key, info, containerId} of restartedContainers) {
            ...
            if (isReady) {
                ...
                const configAfter = getServerConfig(containerId);
        ...
        // Step 4: Restart nginx AFTER mattermost containers are up.
        // In HA mode, the nginx upstream block resolves DNS at startup — if the
        // mattermost nodes aren't running, nginx fails to start. Even with mattermost
        // running, Docker's internal DNS may need a moment to propagate the network
        // aliases, so we retry with delays.
        if (isNginxPresent) {
            const nginxInfo = dockerInfo.containers.nginx as {id: string} | undefined;
            if (nginxInfo?.id && containerExists(nginxInfo.id)) {
                log('Restarting nginx...');
                const maxNginxRetries = 3;
                let nginxRunning = false;

                for (let attempt = 0; attempt < maxNginxRetries; attempt++) {
                    try {
                        // Wait for Docker DNS to propagate mattermost network aliases
                        await new Promise((resolve) => setTimeout(resolve, 2000));

                        if (isContainerRunning(nginxInfo.id)) {
                            restartContainer(nginxInfo.id);
                        } else {
                            startContainer(nginxInfo.id);
                        }

                        // Brief wait for nginx to finish startup
                        ...
                        if (attempt < maxNginxRetries - 1) {
                            log(`⚠ nginx not running, retrying (${attempt + 1}/${maxNginxRetries})...`);
                        }
                    } catch {
                        if (attempt < maxNginxRetries - 1) {
                            log(`⚠ nginx restart failed, retrying (${attempt + 1}/${maxNginxRetries})...`);
                        }
                    }
                }
                ...
                if (nginxRunning) {
                    ...
                    log('✗ nginx not running after retries');
                    hasErrors = true;
                }
            } else if (isNginxPresent) {
        ...
        const c = dockerInfo.containers as unknown as Record<string, Record<string, unknown>>;
