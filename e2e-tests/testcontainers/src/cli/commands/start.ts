...
import type {ServerMode} from '@/environment/types';
import {getContainerMetadata} from '@/environment/metadata';
import {discoverAndLoadConfig, DEFAULT_OUTPUT_DIR} from '@/config';
import {
    isContainerRunning,
    setOutputDir,
    writeDockerInfo,
...
export function registerStartCommand(program: Command): Command {
    const startCommand = program
        ...
        .option(
            '-c, --config <path>',
            'Path to config file. Accepts .mjs or .jsonc extensions. Auto-discovers mattermost-testcontainers.config.mjs or mattermost-testcontainers.config.jsonc',
        )
        ...
        .option('--esr', 'Use the current ESR (Extended Support Release) version')
        .option(
            '--admin [username]',
...
async function executeStartCommand(options: StartOptions): Promise<void> {
    try {
        ...
        if (fs.existsSync(existingDockerInfoPath)) {
            ...
            if (containers.length > 0) {
                ...
                for (const [name, info] of containers) {
                    ...
                    try {
                        const running = isContainerRunning(containerInfo.id);

                        containerStatuses.push({name, running});
                        if (running) {
                            runningCount++;
                        } else {
                            stoppedCount++;
        ...
        if (options.depsOnly) {
            ...
            console.log(
                chalk.blue(
                    `Starting Mattermost subpath mode (server1: HA 3 nodes, server2: bare, service env: ${serviceEnvDisplay})`,
                ),
            );
        } else if (config.server.subpath) {
            console.log(
                chalk.blue(
                    `Starting Mattermost subpath mode (server1: single, server2: bare, service env: ${serviceEnvDisplay})`,
                ),
            );
        ...
        if (info.subpath) {
            ...
            if (info.haCluster) {
                // Subpath + HA: show each node's direct URL
            ...
            console.log(`  Server 2:      ${chalk.yellow(info.subpath.server2Url)} ${chalk.gray('(bare)')}`);
        ...
        const containerMetadata = getContainerMetadata(env);
        ...
        if (!options.depsOnly && info.mattermost) {
            if (info.subpath) {
                // Subpath mode: write separate config files for each server
                const {MmctlClient} = await import('@/environment/mmctl');
                const subpathServers: Array<{
                    name: string;
                    container: import('testcontainers').StartedTestContainer | null;
                }> = [
                    {name: 'server1', container: env.mattermostServer1},
                    {name: 'server2', container: env.mattermostServer2},
                ];
                for (const {name, container} of subpathServers) {
                    ...
                    try {
                        ...
                        if (result.exitCode === 0) {
                            ...
                            console.log(chalk.green(`✓ ${name} configuration written to ${configPath}`));
                        } else {
                            console.log(chalk.yellow(`⚠ Could not get ${name} config via mmctl`));
                        }
                    } catch {
                        console.log(chalk.yellow(`⚠ Could not get ${name} config via mmctl`));
                    }
