...
export async function upgradeContainer(
    ...
    try {
        containerConfig = inspectContainer(containerId) as typeof containerConfig;
    } catch {
    ...
    try {
        newContainerId = dockerRun({
            platform: 'linux/amd64',
            network: selectedNetwork || undefined,
            networkAliases: networkAliases.length > 0 ? networkAliases : undefined,
            env: envVars,
            portMapping: `${originalPort}:8065`,
...
export async function waitForContainer(url: string, maxWaitMs: number = 60000): Promise<boolean> {
    ...
    const pingUrl = buildPingUrl(url);
 ...
 */
export function getServerConfig(containerId: string): Record<string, unknown> | null {
    try {
        const result = execInContainer(containerId, ['mmctl', '--local', 'config', 'show', '--json']);
