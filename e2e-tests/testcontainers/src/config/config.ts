 ...
 */
const CONFIG_FILE_NAMES = ['mattermost-testcontainers.config.mjs', 'mattermost-testcontainers.config.jsonc'] as const;
 ...
 * 1. CLI flags (when using mattermost-testcontainers CLI)
 * 2. Environment variables
 * 3. Config file
 * 4. Built-in defaults
 *
 * Supported config file formats (in priority order):
 * 1. mattermost-testcontainers.config.mjs - ES module using defineConfig
 * 2. mattermost-testcontainers.config.jsonc - JSON with comments
 *
 * @example
 * // mattermost-testcontainers.config.mjs (recommended)
...
export interface TestcontainersImages {
    ...
    promtail: string;
    /** @default 'libretranslate/libretranslate:v1.8.4' @env TC_LIBRETRANSLATE_IMAGE */
 ...
 *
 * @example
 * // mattermost-testcontainers.config.mjs
 * import {defineConfig} from '@mattermost/testcontainers';
 *
 * export default defineConfig({
 *     server: {
...
/**
 * Base image names for Mattermost editions (development/CI builds).
 * Used for tags like 'master', short commits ('0cdd106'), or dual-repo commits ('1ac14a9_57f3598').
 */
...
/**
 * Official release image names for Mattermost editions.
 * Used for version tags like '10.11.9', 'v10.11.9'.
 */
export const MATTERMOST_RELEASE_IMAGES: Record<MattermostEdition, string> = {
    enterprise: 'mattermost/mattermost-enterprise-edition',
    fips: 'mattermost/mattermost-enterprise-fips-edition',
    team: 'mattermost/mattermost-team-edition',
};

/**
 * Check if a tag is a release version available on the official mattermost/* Docker images.
 * Matches: 'release-11.2', '11.2', '11.2.2', '11.2.2-rc1'
 * Does NOT match: 'master', '0cdd106', '1ac14a9_57f3598'
 */
export function isReleaseVersionTag(tag: string): boolean {
    return /^(release-)?\d+\.\d+(\.\d+)?(-rc\d+)?$/.test(tag);
}

/**
 * Get the appropriate base image for an edition and tag.
 * Uses official mattermost/* images for release tags, mattermostdevelopment/* for dev tags.
 */
export function getEditionImage(edition: MattermostEdition, tag: string): string {
    if (isReleaseVersionTag(tag)) {
        return MATTERMOST_RELEASE_IMAGES[edition];
    }
    return MATTERMOST_EDITION_IMAGES[edition];
}
...
export const DEFAULT_CONFIG: ResolvedTestcontainersConfig = {
    ...
    images: {
        ...
        libretranslate: DEFAULT_IMAGES.libretranslate,
...
export function resolveConfig(userConfig?: TestcontainersConfig): ResolvedTestcontainersConfig {
    ...
    resolved.server.image = `${getEditionImage(resolved.server.edition, resolved.server.tag)}:${resolved.server.tag}`;
 ...
 *
 * @example
 * // Automatically discovers mattermost-testcontainers.config.mjs or mattermost-testcontainers.config.jsonc
