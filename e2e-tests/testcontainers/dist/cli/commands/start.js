...
import { writeEnvFile, writeServerConfig, writeDockerInfo, writeKeycloakCertificate, writeOpenLdapSetup, writeKeycloakSetup, } from '../../utils/print';
            ...
            if (containers.length > 0) {
                ...
                if (runningCount > 0) {
                    ...
                    if (runningContainers.has('inbucket') && c.inbucket?.host && c.inbucket?.webPort) {
                        console.log(`Inbucket:        ${chalk.yellow(`http://${c.inbucket.host}:${c.inbucket.webPort}`)}`);
                    }
