...
test(
    ...
    async ({pw, browserName, viewport}, testInfo) => {
        ...
        const publicChannel = await pw.createChannel({
            teamId: team.id,
        ...
        const privateChannel = await pw.createChannel({
            teamId: team.id,
        ...
        await pw.apiClient.deleteChannel(publicChannel.id);
        await pw.apiClient.deleteChannel(privateChannel.id);
...
test(
    ...
    async ({pw, browserName, viewport}, testInfo) => {
        ...
        // # Log in as admin
        const {page} = await pw.testBrowser.loginAsAdmin();
...
test(
    ...
    async ({pw, browserName, viewport}, testInfo) => {
        ...
        // # Log in user and visit the archived channel
        const {page, channelsPage} = await pw.testBrowser.login(user);
