 ...
 */
test('signup without email verification lands on the team-join page', {tag: '@authentication'}, async ({pw}) => {
 ...
 */
test('hides email signup when email account creation is disabled', {tag: '@authentication'}, async ({pw}) => {
 ...
 */
test('creates an account when user creation is enabled', {tag: '@authentication'}, async ({pw}) => {
    const {adminClient} = await pw.initSetup();
