 ...
 */
test('saves Generic OpenID Connect settings and shows the login button', {tag: '@openid'}, async ({pw}) => {
    ...
    try {
        ...
        await systemConsolePage.openIdConnect.discoveryEndpoint.fill(
 ...
 * A licensed server with the OpenID Connect System Console page available.
 */
test('saves Google OpenID Connect settings and shows the login button', {tag: '@openid'}, async ({pw}) => {
    await pw.ensureLicense();
    await pw.skipIfNoLicense();
