 ...
 * User-owned and plugin-owned bot account tokens are included in the policy.
 *
...
test.describe('System Console > Integrations > Revoke non-compliant tokens @system_console', () => {
    ...
    test('revokes non-compliant tokens on confirm, invalidating human and bot tokens while compliant tokens survive', async ({
        pw,
    }) => {
        const {adminUser, adminClient, user} = await pw.initSetup();
        await adminClient.patchConfig({
        ...
        // # Seed one non-compliant token, one compliant token, and one bot token, all
        ...
        // * The compliant token still authenticates, while the bot token is revoked.
