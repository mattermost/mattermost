 ...
 * hasAnyOf / hasAllOf (Phase 11 engine, authored here over the real HTTP
 * boundary). Both fields link to a shared template (equal linked_field_id) so
 * they share one option-id scale — the only shape the save-time validation
 * accepts.
 *
...
test.describe('ABAC resource.attributes - multiselect targets', {tag: ['@abac', '@abac_resource_attributes']}, () => {
    test('has any of syncs and enforces on list intersection', async ({pw}) => {
        ...
        // SQL sync lane: enforcement (remove non-matching, keep matching) runs off
        // the child existing. Auto-add is a separate, default-off gate (the child
        // materializes inactive), so the matching team-only user is NOT pulled in.
        ...
        expect(await verifyUserInChannel(adminClient, matchTeamOnly.id, channel.id)).toBe(false);

        // Runtime PDP lane agrees with the sync verdict: it admits the matching
        // team-only user (sharing >=1 value) and blocks the disjoint one.
    ...
    test('has all of syncs and enforces on channel-list subset', async ({pw}) => {
        ...
        // value is removed. Auto-add is gated off, so the exact-match team-only
        // user is not pulled in.
        expect(await verifyUserInChannel(adminClient, matchInChannel.id, channel.id)).toBe(true);
        expect(await verifyUserInChannel(adminClient, nonMatch.id, channel.id)).toBe(false);
        expect(await verifyUserInChannel(adminClient, matchTeamOnly.id, channel.id)).toBe(false);

        // Runtime PDP lane agrees: the exact-match user can be added, the
