...
// a persistent one (external), so leaving the rewrite in place would give sibling specs a set of
...
test.describe('Space permissions in the System Scheme', () => {
    ...
    // Snapshotted in the first test and put back in afterAll, so the scheme save cannot outlive the
    // spec.
     ...
     * boot, so this asserts the flag rather than switching it. That holds in either mode:
     * Testcontainers (the suite owns the server) or external (PW_BASE_URL points at an already-running
     * one).
     */
    test(
        ...
        async ({pw}) => {
            ...
            // Asserted, not switched: ensureFeatureFlag restarts the server to change a boot-time
            // flag, which ties the spec to Testcontainers and remaps the port out from under the
            // browser context. Failing loudly here says exactly which prerequisite is missing.
            ...
            // Toggle an unrelated permission and put it straight back. The net edit is nothing, but
            // the editor latches "unsaved changes" on any interaction, so Save becomes available —
            // the everyday way an admin ends up saving this page without meaning to change anything
            // on it.
