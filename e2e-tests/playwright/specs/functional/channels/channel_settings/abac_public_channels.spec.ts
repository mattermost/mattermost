...
import {ChannelsPage, components, expect, getAdminClient, test} from '@mattermost/playwright-lib';
...
test.describe('ABAC - Public channels', () => {
    ...
    test('Default channel is excluded from Team Settings policy editor channel selector', async ({pw}) => {
        ...
        const channelSelector = channelsPage.channelSelectorModal;
        await channelSelector.waitFor();
        const browseChannels = new components.BrowseChannelsModal(channelSelector);
        ...
        await expect(browseChannels.getChannelRowByName('town-square')).toHaveCount(0);
        await expect(browseChannels.getChannelRowByName('off-topic')).toHaveCount(0);
    ...
    test('Public channel with matching policy surfaces as Recommended for matching user (auto-add off)', async ({
        ...
        await browse.channelTypeFilterButton.click();
        await browse.channelsMoreDropdownRecommended.click();
        ...
        const channelRow = browse.getChannelRowByName(publicChannel.name);
