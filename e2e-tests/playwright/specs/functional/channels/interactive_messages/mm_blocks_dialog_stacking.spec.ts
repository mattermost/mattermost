...
test.describe('Interactive dialogs (stacked child modals)', () => {
    ...
    test(
        'legacy action_button stacks a Details child on the parent',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
    ...
    test(
        ...
        async ({pw, request}) => {
            ...
            await expect(appsModals(channelsPage.page).locator('#appsModalLabel')).
                toContainText('Parent Dialog with Action Button');
        },
    );

    test(
        'native blocks Details button stacks a child block_dialog',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
    ...
    test(
        'native blocks Summary child submit returns to parent',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
                scenario: 'action_parent',
            ...
            await expect(appsModals(channelsPage.page)).toHaveCount(1);
            await expect(appsModals(channelsPage.page).locator('#appsModalLabel')).
                toContainText('PW Action Buttons');
        },
    );
});
