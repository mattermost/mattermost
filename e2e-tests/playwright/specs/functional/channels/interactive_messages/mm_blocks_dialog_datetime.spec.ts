...
async function selectDayFromPicker(page: any, day: string, needsNextMonth: boolean) {
    ...
    await calendar.locator('.rdp-day:not(.rdp-day_outside), .rdp-day_button').
...
test.describe('Interactive mm_blocks (blocks dialog datetime)', () => {
    ...
    test(
        'date and datetime fields open pickers and submit',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
    ...
    test(
        'min_date today disables past days',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
            ...
            const pastDay = calendar.locator('.rdp-day:not(.rdp-day_outside), .rdp-day_button').
    ...
    test(
        'custom time_interval shows 30-minute options',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
    ...
    test(
        'relative initial_value populates date and datetime fields',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
    ...
    test(
        'date locale formatting shows Month Day, Year',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
                scenario: 'datetime_basic',
            ...
            const value = dialog.locator('.mm-blocks-date-input').filter({hasText: 'Event Date'}).locator('.date-time-input__value');
    ...
    test(
        'datetime respects 12h and 24h military time preference',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
    ...
    test(
        'manual time entry accepts formats and rejects invalid',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
