...
test.describe('Interactive mm_blocks (blocks dialog selects)', () => {
    ...
    test(
        'multiselect defaults render Engineering and Marketing',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
    ...
    test(
        'multiselect add remove and submit arrays',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
            ...
            await multi.locator('.react-select__multi-value').filter({hasText: 'Engineering'}).
                locator('.react-select__multi-value__remove').click();
    ...
    test(
        'multiselect required validation',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
                scenario: 'multiselect',
    ...
    test(
        'multiselect keyboard navigation and type-to-filter',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
                scenario: 'multiselect',
    ...
    test(
        'dynamic select search submit and required validation',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
                scenario: 'dynamic',
    ...
    test(
        'dynamic select keyboard navigation',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName} = await setupDialogOpenPost(pw, request, {
                scenario: 'dynamic',
    ...
    test(
        'users and channels selects allow picking options',
        {tag: [...dialogTags]},
        async ({pw, request}) => {
            const {channelsPage, marker, openButtonName, adminClient, team, townSquare} = await setupDialogOpenPost(pw, request, {
