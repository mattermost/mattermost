...
test(
    ...
    async ({pw}) => {
        ...
        if (process.env.PW_FAIL_ON_PAGE_ERROR === 'strict-mode') {
            expect(diagnosticsInstalled, 'StrictMode discovery requires an instrumented web app').toBe(true);
        }
        ...
        expect(violation.message).toBe(
            'React strict mode violation: Cannot update a component (`ChannelView`) while rendering a different component (`PostList`).',
        );
        ...
        // replacing it.
        expect(violation.stack).toContain('at PostList (post_list.tsx:20)');
        expect(violation.stack).toContain('at ChannelView');
        ...
        expect(recorded).toContainEqual(
            expect.objectContaining({
                componentStack: expect.stringContaining('at PostList (post_list.tsx:20)'),
            }),
        );
        ...
        if (shouldFailOnPageError()) {
            ...
            expect(formatPageErrors(collected)).toContain('at ChannelView');
        }
    },
);
