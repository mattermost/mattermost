...
test('unreads filter will show and hide unread channels',{tag: '@filters'}, async ({pw}) => {
  ...
  for (let i = 0; i < channels.length; i++) {
      addUserPromises.push(
          adminClient.addToChannel(user.id, channels[i].id)
      );
  }
 ...
 const {channelsPage} = await pw.testBrowser.login(user);
  ...
  const initialUnreadChannels = channelsPage.sidebarLeft.container.locator('.SidebarLink.unread-title');
  ...
  const unreadChannels = channelsPage.sidebarLeft.container.locator('.SidebarLink.unread-title');
