 ...
 */
test('SAML login succeeds with signature algorithm RSAwithSHA256', {tag: '@saml'}, async ({pw}) => {
 ...
 * ServiceProviderIdentifier.
 */
test('SAML login succeeds with signature algorithm RSAwithSHA512', {tag: '@saml'}, async ({pw}) => {
    await pw.ensureLicense();
    await pw.skipIfNoLicense();
    await pw.ensureKeycloak();
