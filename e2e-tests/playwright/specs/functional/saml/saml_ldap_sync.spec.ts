 ...
 * Keycloak and OpenLDAP running, with matching users in both directories.
 */
test('SAML login keeps SAML attributes when LDAP sync is disabled', {tag: '@saml'}, async ({pw}) => {
    await pw.ensureLicense();
    await pw.skipIfNoLicense();
    await pw.ensureOpenldap();
    await pw.ensureKeycloak();
 ...
 * Keycloak and OpenLDAP running, with matching users in both directories.
 */
test('SAML LDAP sync uses a custom ID Attribute mapping', {tag: '@saml'}, async ({pw}) => {
    await pw.ensureLicense();
    await pw.skipIfNoLicense();
    await pw.ensureOpenldap();
    await pw.ensureKeycloak();
