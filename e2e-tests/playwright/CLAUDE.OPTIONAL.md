...
- `PW_TESTCONTAINERS_SERVICES` - Comma-separated additional services to start, e.g. `minio,openldap` (default: none)
- `PW_TESTCONTAINERS_REUSE` - Reuse containers across repeated local runs instead of recreating them (default: false)
...
2. **Testcontainers** (recommended for testing, and what CI uses) — no separate step needed, Playwright starts and tears down the server + dependencies itself:
    ...
    # Add PW_TESTCONTAINERS_SERVICES=minio,openldap,keycloak,elasticsearch,opensearch,azurite as needed
    # Add PW_TESTCONTAINERS_REUSE=true for fast repeated local runs; tear down with `npm run testcontainers:down`
    ```
