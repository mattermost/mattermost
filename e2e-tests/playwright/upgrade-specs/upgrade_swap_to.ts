...
setup('swap to to-image', async ({pw}) => {
    ...
    try {
        ...
        // Capture to-image logs even if the swap/identity assert fails (overwritten by upgrade-to
        // afterAll on success). Guarded: a log-capture error here would otherwise replace the
        // swap failure this block exists to surface.
        try {
            await pw.saveUpgradePhaseLogs('to');
        } catch (error) {
            // eslint-disable-next-line no-console
            console.log(`upgrade-swap-to: failed to capture phase logs: ${error}`);
        }
    }
});
