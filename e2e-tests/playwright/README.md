...
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PR (automated)                  | On when the diff touches `upgrade-specs/`, `script/resolve_upgrade_matrix.mjs`, a rolling-upgrade workflow, or a server migration (`server/channels/db/migrations/`, `server/config/migrations/`, `app/migrations.go`, `app/permissions_migrations.go`, `model/version.go`) |
