...
export default class DirectChannelsModal {
    ...
    constructor(container: Locator) {
        ...
        // 11.7 rows use class more-modal__row; data-testid="direct-message-row" / "more-modal-list" were added later.
        this.results = container.locator('.filtered-user-list');
    ...
    /**
     * Locates a user/group result row in the Direct Messages modal.
     * @param username
     */
    resultRow(username: string): Locator {
        return this.container.locator('.more-modal__row').filter({hasText: `@${username}`});
    }
    ...
    async selectUser(user: UserProfile) {
        ...
        await this.resultRow(user.username).click();
    ...
    async toHaveNResults(count: number) {
        await expect(this.container.locator('.more-modal__row')).toHaveCount(count);
    }
    ...
    async toHaveUserAsNthResult(user: UserProfile, index: number) {
        const row = this.container.locator('.more-modal__row').nth(index);
