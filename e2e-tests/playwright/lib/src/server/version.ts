...
async function assertRunningContainerImage(expectedImage: string): Promise<void> {
    ...
    // Compare the full reference: matching on the tag alone would accept a same-tag image from
    // a different repository, so a mis-set image could pass the swap assertion.
    const normalizeRef = (image: string) => image.replace(/^docker\.io\/(library\/)?/, '');
    const runningImage = stdout.trim();

    if (normalizeRef(runningImage) !== normalizeRef(expectedImage)) {
        throw new Error(
