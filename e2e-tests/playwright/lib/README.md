...
| `PW_UPGRADE_FROM_SERVER_IMAGE`       | Older server image the `upgrade-from` project swaps down to — use a `release-X.Y` tag (see `script/resolve_upgrade_matrix.mjs`)                                                                      | none — required for `--project=upgrade-from`                 |
