...
| `PW_TESTCONTAINERS_SERVICES`         | Comma-separated additional services to start (`openldap`, `keycloak`, `elasticsearch`, `opensearch`, `minio`, `azurite`)                               | none (base stack only: Postgres, Inbucket, Mattermost server) |
| `PW_TESTCONTAINERS_REUSE`            | Reuse containers across repeated local runs instead of recreating them (requires `testcontainers.reuse.enable=true` in `~/.testcontainers.properties`) | `false`                                                       |
