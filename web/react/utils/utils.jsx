...
function updateTabTitle(name) {
    var paddedName = name;
    if (checkRTL(name)) {
        // Adds control characters to designate name is right to left
        paddedName = '\u202B' + name + '\u202C';
    }

    // 202D designates the entire string as left to right, except parts that are escaped as above
    document.title = '\u202D' + paddedName + ' ' + document.title.substring(document.title.lastIndexOf('-')) + '\u202C';
}
