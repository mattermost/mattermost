...
export function createChannelIntroMessage(channel, messages, locale) {
    if (channel.type === 'D') {
        ...
        return createStandardIntroMessage(channel, messages, locale);
    }
}
...
export function createStandardIntroMessage(channel, messages, locale) {
    ...
    if (creatorName === '') {
        if (locale === 'en') {
            createMessage = messages.start2 + uiName + ' ' + uiType + messages.created + Utils.displayDate(channel.create_at) + '.';
        } else if (locale === 'es') {
            createMessage = messages.start2 + uiType + ' ' + uiName + messages.created + Utils.displayDate(channel.create_at) + '.';
        }
    } else if (locale === 'en') {
        createMessage = (
            <span>
        ...
        createMessage = (
            <span>
                ...
                {uiType}
                {' '}
                <strong>{uiName}</strong>{messages.createdBy}
                <strong>{creatorName}</strong>
                {messages.on}
                <strong>{Utils.displayDate(channel.create_at)}</strong>
            </span>
