...
const messages = defineMessages({
    ...
    created: {
        ...
        defaultMessage: 'This is the start of the {uiName} {uiType}, created on {date}'
    },
    createdBy: {
        id: 'post_view_container.createdBy',
        defaultMessage: '<span>This is the start of the <strong>{uiName}</strong>{uiType}, created by <strong>{creatorName}</strong>{on}<strong>{date}</strong></span>',
    },
    on: {
...
class PostsViewContainer extends React.Component {
    ...
    render() {
        const {formatMessage, formatHTMLMessage, locale} = this.props.intl;
        ...
        const translations = {
            ...
            created: formatMessage(messages.created, {
                uiName: channel ? channel.display_name : '',
                uiType: channel ? channel.type == 'P' ? formatMessage(messages.pg) : formatMessage(messages.channel) : '',
                date: channel ? Utils.displayDate(channel.create_at, locale) : ''
            }),
            createdBy: formatHTMLMessage(messages.createdBy, {
                uiName: channel ? channel.display_name : '',
                uiType: channel ? channel.type == 'P' ? formatMessage(messages.pg) : formatMessage(messages.channel) : '',
                creatorName: '',
        ...
        for (let i = 0; i < channels.length; i++) {
            ...
            postListCtls.push(
                <PostsView
                    ...
                    introText={channel ? createChannelIntroMessage(channel, translations) : null}
