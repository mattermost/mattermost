...
export default class CodePreview extends React.Component {
    ...
    updateStateFromProps(props) {
        var usedLanguage = getlang(props.filename);
        if (!usedLanguage || props.fileInfo.size > 50000) {
            this.setState({code: '', lang: '', canLoad: false});
            return;
        }

        this.setState({code: '', lang: usedLanguage, canLoad: true});
    ...
    static formatCode(lang, data, filename) {
        var language = lang || '';
        var parsed;
        var header = '';
    ...
    static support(filename) {
        return typeof getlang(filename) !== 'undefined';
    }

    render() {
        if (!this.state.lang || !this.state.canLoad) {
            return (
                <FileInfoPreview
        ...
        return <div dangerouslySetInnerHTML={{__html: CodePreview.formatCode(this.state.lang, this.state.code, this.props.filename)}}/>;
    }
}
...
function getlang(filename) {
    var ext = re.exec(filename.trim())[1];
    if (!ext) {
        return false;
    }
