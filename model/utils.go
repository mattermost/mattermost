...
func IsValidIpOrCidr(ipOrCidr string) bool {
	return IsValidIp(ipOrCidr) || IsValidCidr(ipOrCidr)
}

func IsValidIpList(ipList StringArray) bool {
...
func IsInCidr(ip_address net.IP, cidr *net.IPNet) bool {
	return cidr.Contains(ip_address)
}

func IsInWhitelist(originIp string, whiteList StringArray) bool {
	for _, whiteIp := range whiteList {
		if IsValidIp(whiteIp) {
			if whiteIp == originIp {
				return true
			}
		} else {
			_, cidrnet, _ := net.ParseCIDR(whiteIp)
			parsedOriginIp := net.ParseIP(originIp)
			if IsInCidr(parsedOriginIp, cidrnet) {
				return true
			}
...
func GetRemoteAddress(hostPort string) string {
