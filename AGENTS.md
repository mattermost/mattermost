...
The startup update script is expected to prepare Node/npm and web dependencies before agent work begins.
...
1. **Verify Docker is available in the current session.**
   Run:
   ```bash
   docker ps
   ```
   If this fails, the environment needs Docker remediation before running Mattermost services.
