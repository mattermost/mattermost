...
type AppIface interface {
	...
	CreateCommandPost(c *Context, post *model.Post, teamID string, response *model.CommandResponse, skipSlackParsing bool) (*model.Post, *model.AppError)
	// AddChannelMember adds a user to a channel. It is a wrapper over AddUserToChannel.
	AddChannelMember(c *Context, userID string, channel *model.Channel, opts ChannelMemberOpts) (*model.ChannelMember, *model.AppError)
	...
	CreateBot(c *Context, bot *model.Bot) (*model.Bot, *model.AppError)
	...
	//
	CreateDefaultChannels(c *Context, teamID string) ([]*model.Channel, *model.AppError)
	...
	CreateDefaultMemberships(c *Context, since int64) error
	// CreateGuest creates a guest and sets several fields of the returned User struct to
	// their zero values.
	CreateGuest(c *Context, user *model.User) (*model.User, *model.AppError)
	// CreateUser creates a user and sets several fields of the returned User struct to
	// their zero values.
	CreateUser(c *Context, user *model.User) (*model.User, *model.AppError)
	...
	DeleteGroupConstrainedMemberships(c *Context) error
	...
	GetProductNotices(c *Context, userID, teamID string, client model.NoticeClientType, clientVersion string, locale string) (model.NoticeMessages, *model.AppError)
	...
	GetSuggestions(c *Context, commandArgs *model.CommandArgs, commands []*model.Command, roleID string) []model.AutocompleteSuggestion
	...
	MoveChannel(c *Context, team *model.Team, channel *model.Channel, user *model.User) *model.AppError
	// NewWebConn returns a new WebConn instance.
	NewWebConn(ws *websocket.Conn, c *Context, locale string) *WebConn
	...
	DoActionRequest(c *Context, rawURL string, body []byte) (*http.Response, *model.AppError)
	...
	// syncable.
	SyncRolesAndMembership(c *Context, syncableID string, syncableType model.GroupSyncableType)
	...
	UpdateBotActive(c *Context, botUserId string, active bool) (*model.Bot, *model.AppError)
	...
	UploadFile(c *Context, data []byte, channelID string, filename string) (*model.FileInfo, *model.AppError)
	...
	UploadFileX(c *Context, channelID, name string, input io.Reader, opts ...func(*UploadFileTask)) (*model.FileInfo, *model.AppError)
	...
	UploadFiles(c *Context, teamID string, channelID string, userID string, files []io.ReadCloser, filenames []string, clientIds []string, now time.Time) (*model.FileUploadResponse, *model.AppError)
	...
	AddTeamMember(c *Context, teamID, userID string) (*model.TeamMember, *model.AppError)
	AddTeamMemberByInviteId(c *Context, inviteId, userID string) (*model.TeamMember, *model.AppError)
	AddTeamMemberByToken(c *Context, userID, tokenID string) (*model.TeamMember, *model.AppError)
	AddTeamMembers(c *Context, teamID string, userIDs []string, userRequestorId string, graceful bool) ([]*model.TeamMemberWithError, *model.AppError)
	AddTeamsToRetentionPolicy(policyID string, teamIDs []string) *model.AppError
	AddUserToTeam(c *Context, teamID string, userID string, userRequestorId string) (*model.Team, *model.TeamMember, *model.AppError)
	AddUserToTeamByInviteId(c *Context, inviteId string, userID string) (*model.Team, *model.TeamMember, *model.AppError)
	AddUserToTeamByTeamId(c *Context, teamID string, user *model.User) *model.AppError
	AddUserToTeamByToken(c *Context, userID string, tokenID string) (*model.Team, *model.TeamMember, *model.AppError)
	...
	BulkImport(c *Context, fileReader io.Reader, dryRun bool, workers int) (*model.AppError, int)
	BulkImportWithPath(c *Context, fileReader io.Reader, dryRun bool, workers int, importPath string) (*model.AppError, int)
	...
	CheckAndSendUserLimitWarningEmails(c *Context) *model.AppError
	...
	CompleteOAuth(c *Context, service string, body io.ReadCloser, teamID string, props map[string]string, tokenUser *model.User) (*model.User, *model.AppError)
	...
	CreateChannel(c *Context, channel *model.Channel, addMember bool) (*model.Channel, *model.AppError)
	CreateChannelWithUser(c *Context, channel *model.Channel, userID string) (*model.Channel, *model.AppError)
	...
	CreateOAuthUser(c *Context, service string, userData io.Reader, teamID string, tokenUser *model.User) (*model.User, *model.AppError)
	...
	CreatePost(c *Context, post *model.Post, channel *model.Channel, triggerWebhooks, setOnline bool) (savedPost *model.Post, err *model.AppError)
	CreatePostAsUser(c *Context, post *model.Post, currentSessionId string, setOnline bool) (*model.Post, *model.AppError)
	CreatePostMissingChannel(c *Context, post *model.Post, triggerWebhooks bool) (*model.Post, *model.AppError)
	...
	CreateTeam(c *Context, team *model.Team) (*model.Team, *model.AppError)
	CreateTeamWithUser(c *Context, team *model.Team, userID string) (*model.Team, *model.AppError)
	...
	CreateUserAsAdmin(c *Context, user *model.User, redirect string) (*model.User, *model.AppError)
	CreateUserFromSignup(c *Context, user *model.User, redirect string) (*model.User, *model.AppError)
	CreateUserWithInviteId(c *Context, user *model.User, inviteId, redirect string) (*model.User, *model.AppError)
	CreateUserWithToken(c *Context, user *model.User, token *model.Token) (*model.User, *model.AppError)
	CreateWebhookPost(c *Context, userID string, channel *model.Channel, text, overrideUsername, overrideIconURL, overrideIconEmoji string, props model.StringInterface, postType string, postRootId string) (*model.Post, *model.AppError)
	DBHealthCheckDelete() error
	DBHealthCheckWrite() error
	DataRetention() einterfaces.DataRetentionInterface
	DeactivateGuests(c *Context) *model.AppError
	...
	DeleteChannel(c *Context, channel *model.Channel, userID string) *model.AppError
	...
	DeleteReactionForPost(c *Context, reaction *model.Reaction) *model.AppError
	...
	DoLocalRequest(c *Context, rawURL string, body []byte) (*http.Response, *model.AppError)
	DoLogin(c *Context, w http.ResponseWriter, r *http.Request, user *model.User, deviceID string, isMobile, isOAuthUser, isSaml bool) *model.AppError
	DoPostAction(c *Context, postID, actionId, userID, selectedOption string) (string, *model.AppError)
	DoPostActionWithCookie(c *Context, postID, actionId, userID, selectedOption string, cookie *model.PostActionCookie) (string, *model.AppError)
	DoSystemConsoleRolesCreationMigration()
	DoUploadFile(c *Context, now time.Time, rawTeamId string, rawChannelId string, rawUserId string, rawFilename string, data []byte) (*model.FileInfo, *model.AppError)
	DoUploadFileExpectModification(c *Context, now time.Time, rawTeamId string, rawChannelId string, rawUserId string, rawFilename string, data []byte) (*model.FileInfo, []byte, *model.AppError)
	...
	GetOrCreateDirectChannel(c *Context, userID, otherUserID string, channelOptions ...model.ChannelOption) (*model.Channel, *model.AppError)
	...
	GetPermalinkPost(c *Context, postID string, userID string) (*model.PostList, *model.AppError)
	...
	GetUsersByGroupChannelIds(c *Context, channelIDs []string, asAdmin bool) (map[string][]*model.User, *model.AppError)
	...
	HandleCommandResponse(c *Context, command *model.Command, args *model.CommandArgs, response *model.CommandResponse, builtIn bool) (*model.CommandResponse, *model.AppError)
	HandleCommandResponsePost(c *Context, command *model.Command, args *model.CommandArgs, response *model.CommandResponse, builtIn bool) (*model.Post, *model.AppError)
	HandleCommandWebhook(c *Context, hookID string, response *model.CommandResponse) *model.AppError
	HandleImages(previewPathList []string, thumbnailPathList []string, fileData [][]byte)
	HandleIncomingWebhook(c *Context, hookID string, req *model.IncomingWebhookRequest) *model.AppError
	...
	JoinChannel(c *Context, channel *model.Channel, userID string) *model.AppError
	JoinDefaultChannels(c *Context, teamID string, user *model.User, shouldBeAdmin bool, userRequestorId string) *model.AppError
	JoinUserToTeam(c *Context, team *model.Team, user *model.User, userRequestorId string) (*model.TeamMember, *model.AppError)
	Ldap() einterfaces.LdapInterface
	LeaveChannel(c *Context, channelID string, userID string) *model.AppError
	LeaveTeam(c *Context, team *model.Team, user *model.User, requestorId string) *model.AppError
	...
	NewPluginAPI(c *Context, manifest *model.Manifest) plugin.API
	...
	PatchChannel(c *Context, channel *model.Channel, patch *model.ChannelPatch, userID string) (*model.Channel, *model.AppError)
	PatchPost(c *Context, postID string, patch *model.PostPatch) (*model.Post, *model.AppError)
	...
	PermanentDeleteAllUsers(c *Context) *model.AppError
	...
	PostUpdateChannelDisplayNameMessage(c *Context, userID string, channel *model.Channel, oldChannelDisplayName, newChannelDisplayName string) *model.AppError
	PostUpdateChannelHeaderMessage(c *Context, userID string, channel *model.Channel, oldChannelHeader, newChannelHeader string) *model.AppError
	PostUpdateChannelPurposeMessage(c *Context, userID string, channel *model.Channel, oldChannelPurpose string, newChannelPurpose string) *model.AppError
	...
	RemoveTeamMemberFromTeam(c *Context, teamMember *model.TeamMember, requestorId string) *model.AppError
	RemoveTeamsFromRetentionPolicy(policyID string, teamIDs []string) *model.AppError
	RemoveUserFromChannel(c *Context, userIDToRemove string, removerUserId string, channel *model.Channel) *model.AppError
	RemoveUserFromTeam(c *Context, teamID string, userID string, requestorId string) *model.AppError
	RemoveUsersFromChannelNotMemberOfTeam(c *Context, remover *model.User, channel *model.Channel, team *model.Team) *model.AppError
	RequestLicenseAndAckWarnMetric(c *Context, warnMetricId string, isBot bool) *model.AppError
	...
	RestoreChannel(c *Context, channel *model.Channel, userID string) (*model.Channel, *model.AppError)
	...
	SaveReactionForPost(c *Context, reaction *model.Reaction) (*model.Reaction, *model.AppError)
	...
	SearchFilesInTeamForUser(c *Context, terms string, userId string, teamId string, isOrSearch bool, includeDeletedChannels bool, timeZoneOffset int, page, perPage int) (*model.FileInfoList, *model.AppError)
	...
	SearchPostsInTeamForUser(c *Context, terms string, userID string, teamID string, isOrSearch bool, includeDeletedChannels bool, timeZoneOffset int, page, perPage int) (*model.PostSearchResults, *model.AppError)
	...
	SendAutoResponse(c *Context, channel *model.Channel, receiver *model.User, post *model.Post) (bool, *model.AppError)
	SendAutoResponseIfNecessary(c *Context, channel *model.Channel, sender *model.User, post *model.Post) (bool, *model.AppError)
	...
	SlackImport(c *Context, fileData multipart.File, fileSize int64, teamID string) (*model.AppError, *bytes.Buffer)
	SoftDeleteTeam(teamID string) *model.AppError
	Srv() *Server
	SubmitInteractiveDialog(c *Context, request model.SubmitDialogRequest) (*model.SubmitDialogResponse, *model.AppError)
	...
	TriggerWebhook(c *Context, payload *model.OutgoingWebhookPayload, hook *model.OutgoingWebhook, post *model.Post, channel *model.Channel)
	UnregisterPluginCommand(pluginID, teamID, trigger string)
	UnregisterPluginCommands(pluginID string)
	UpdateActive(c *Context, user *model.User, active bool) (*model.User, *model.AppError)
	...
	UpdateChannelPrivacy(c *Context, oldChannel *model.Channel, user *model.User) (*model.Channel, *model.AppError)
	...
	UpdatePost(c *Context, post *model.Post, safeUpdate bool) (*model.Post, *model.AppError)
	...
	UpdateUserActive(c *Context, userID string, active bool) *model.AppError
	...
	UploadData(c *Context, us *model.UploadSession, rd io.Reader) (*model.FileInfo, *model.AppError)
	UploadEmojiImage(id string, imageData *multipart.FileHeader) *model.AppError
	UploadMultipartFiles(c *Context, teamID string, channelID string, userID string, fileHeaders []*multipart.FileHeader, clientIds []string, now time.Time) (*model.FileUploadResponse, *model.AppError)
