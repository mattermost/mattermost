...
func (a *App) parseArgument(commandArgs *model.CommandArgs, arg *model.AutocompleteArg, parsed, toBeParsed string) (found bool, alreadyParsed string, yetToBeParsed string, suggestions []model.AutocompleteSuggestion) {
	...
	if arg.Type == model.AutocompleteArgTypeText {
		...
		found, changedParsed, changedToBeParsed, dynamicListSuggestions := a.parseDynamicListArgument(commandArgs, arg, parsed, toBeParsed)
		if found {
...
func (a *App) GetDynamicListArgument(commandArgs *model.CommandArgs, fetchURL, parsed, toBeParsed string) ([]model.AutocompleteListItem, error) {
	params := url.Values{}
...
func (a *App) parseDynamicListArgument(commandArgs *model.CommandArgs, arg *model.AutocompleteArg, parsed, toBeParsed string) (found bool, alreadyParsed string, yetToBeParsed string, suggestions []model.AutocompleteSuggestion) {
	dynamicArg := arg.Data.(*model.AutocompleteDynamicListArg)
	listItems, err := a.GetDynamicListArgument(commandArgs, dynamicArg.FetchURL, parsed, toBeParsed)
	if err != nil {
		return false, parsed, toBeParsed, []model.AutocompleteSuggestion{}
	}
