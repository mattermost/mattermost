...
func TestCreateDefaultMemberships(t *testing.T) {
	...
	_, err = th.App.AddChannelMember(scientist1.Id, experimentsChannel, "", "", true)
	if err != nil {
	...
	_, err = th.App.AddChannelMember(singer1.Id, experimentsChannel, "", "", true)
	if err != nil {
		t.Errorf("unable to add user to channel: %s", err.Error())
	}
...
func TestDeleteGroupMemberships(t *testing.T) {
	...
	for _, userID := range userIDs {
		...
		_, err = th.App.AddChannelMember(userID, th.BasicChannel, "", "", true)
		require.Nil(t, err)
	}
