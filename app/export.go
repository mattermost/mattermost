...
func GetMissingAttachmentInfo(attachment *imports.AttachmentImportData) *imports.LogFormat {
	data := imports.LogFormat{
		Id:            attachment.Id,
...
func (a *App) ExportAttachments(attachment *imports.AttachmentImportData, outPath string, zipWr *zip.Writer) (*imports.LogFormat, *model.AppError) {
...
func (a *App) BulkExport(ctx request.CTX, zipWriter, logWriter io.Writer, outPath string, opts model.BulkExportOpts) *model.AppError {
	...
	var missingAttachments []*imports.LogFormat
	var logData *imports.LogFormat
	...
	if logWriter != nil && len(missingAttachments) != 0 {
		fileData, err := json.MarshalIndent(imports.MissingAttachments{MissingAttachments: missingAttachments}, "", " ")
		if err != nil {
...
func (a *App) exportCustomEmoji(writer io.Writer, outPath, exportDir string, exportFiles bool) ([]*imports.AttachmentImportData, *model.AppError) {
	var emojiDataList []*imports.AttachmentImportData
	pageNumber := 0
	for {
		...
		for _, emoji := range customEmojiList {
			...
			if exportFiles {
				...
				emojiDataList = append(emojiDataList, &imports.AttachmentImportData{
					Id:        emoji.Id,
