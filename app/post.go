...
// countMentionsInRootPostsFromPost returns the number of root posts in the post's channel that mention the user after and including the
// given post.
func (a *App) countMentionsInRootPostsFromPost(user *model.User, post *model.Post) (int, *model.AppError) {
	channel, err := a.GetChannel(post.ChannelId)
	if err != nil {
		return 0, err
	}
	...
	if channel.Type == model.CHANNEL_DIRECT {
		// In a DM channel, every post made by the other user is a mention
		_, countRoot, nErr := a.Srv().Store.Channel().CountPostsAfter(post.ChannelId, post.CreateAt-1, channel.GetOtherUserIdForDM(user.Id))
		if nErr != nil {
			return 0, model.NewAppError("countMentionsFromPost", "app.channel.count_posts_since.app_error", nil, nErr.Error(), http.StatusInternalServerError)
		}

		return countRoot, nil
	}
	...
	for {
		...
		for _, postID := range postList.Order {
			// only check root posts
			if postList.Posts[postID].RootId == "" {
				if isPostMention(user, postList.Posts[postID], keywords, otherPosts, mentionedByThread, false) {
					count += 1
				}
			}
		}
