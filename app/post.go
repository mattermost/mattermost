...
func (a *App) deduplicateCreatePost(c request.CTX, post *model.Post) (foundPost *model.Post, err *model.AppError) {
	...
	actualPost, err := a.GetSinglePost(c, postID, false)
	if err != nil {
	...
	c.Logger().Debug("Deduplicated create post", mlog.String("post_id", actualPost.Id), mlog.String("pending_post_id", post.PendingPostId))
...
func (a *App) attachFilesToPost(c request.CTX, post *model.Post) *model.AppError {
	var attachedIds []string
	for _, fileID := range post.FileIds {
		err := a.Srv().Store().FileInfo().AttachToPost(fileID, post.Id, post.UserId)
		if err != nil {
			c.Logger().Warn("Failed to attach file to post", mlog.String("file_id", fileID), mlog.String("post_id", post.Id), mlog.Err(err))
			continue
		}
...
func (a *App) DeleteEphemeralPost(c request.CTX, userID, postID string) {
	post := &model.Post{
		Id:       postID,
		UserId:   userID,
...
func (a *App) UpdatePost(c request.CTX, post *model.Post, safeUpdate bool) (*model.Post, *model.AppError) {
	post.SanitizeProps()
...
func (a *App) GetPostsPage(c request.CTX, options model.GetPostsOptions) (*model.PostList, *model.AppError) {
	...
	if appErr := a.filterInaccessiblePosts(c, postList, filterPostOptions{assumeSortedCreatedAt: true}); appErr != nil {
		return nil, appErr
	}
...
func (a *App) GetPosts(c request.CTX, channelID string, offset int, limit int) (*model.PostList, *model.AppError) {
	...
	if appErr := a.filterInaccessiblePosts(c, postList, filterPostOptions{}); appErr != nil {
		return nil, appErr
	}
...
func (a *App) GetPostsSince(c request.CTX, options model.GetPostsSinceOptions) (*model.PostList, *model.AppError) {
...
func (a *App) GetSinglePost(c request.CTX, postID string, includeDeleted bool) (*model.Post, *model.AppError) {
	...
	firstInaccessiblePostTime, appErr := a.isInaccessiblePost(c, post)
	if appErr != nil {
		return nil, appErr
	}
...
func (a *App) GetPostThread(c request.CTX, postID string, opts model.GetPostsOptions, userID string) (*model.PostList, *model.AppError) {
	...
	if appErr := a.filterInaccessiblePosts(c, posts, filterOptions); appErr != nil {
		return nil, appErr
	}
...
func (a *App) GetFlaggedPosts(c request.CTX, userID string, offset int, limit int) (*model.PostList, *model.AppError) {
...
func (a *App) GetFlaggedPostsForTeam(c request.CTX, userID, teamID string, offset int, limit int) (*model.PostList, *model.AppError) {
...
func (a *App) GetFlaggedPostsForChannel(c request.CTX, userID, channelID string, offset int, limit int) (*model.PostList, *model.AppError) {
...
func (a *App) GetPermalinkPost(c request.CTX, postID string, userID string) (*model.PostList, *model.AppError) {
	...
	if appErr := a.filterInaccessiblePosts(c, list, filterPostOptions{assumeSortedCreatedAt: true}); appErr != nil {
		return nil, appErr
	}
...
func (a *App) GetPostsBeforePost(c request.CTX, options model.GetPostsOptions) (*model.PostList, *model.AppError) {
	...
	if appErr := a.filterInaccessiblePosts(c, postList, filterOptions); appErr != nil {
		return nil, appErr
	}
...
func (a *App) GetPostsAfterPost(c request.CTX, options model.GetPostsOptions) (*model.PostList, *model.AppError) {
...
func (a *App) GetPostsAroundPost(c request.CTX, before bool, options model.GetPostsOptions) (*model.PostList, *model.AppError) {
...
func (a *App) GetNextPostIdFromPostList(c request.CTX, postList *model.PostList, collapsedThreads bool) string {
	if len(postList.Order) > 0 {
		...
		if err != nil {
			c.Logger().Warn("GetNextPostIdFromPostList: failed in getting next post", mlog.Err(err))
		}
...
func (a *App) GetPrevPostIdFromPostList(c request.CTX, postList *model.PostList, collapsedThreads bool) string {
	if len(postList.Order) > 0 {
		...
		if err != nil {
			c.Logger().Warn("GetPrevPostIdFromPostList: failed in getting previous post", mlog.Err(err))
		}
...
func (a *App) AddCursorIdsForPostList(c request.CTX, originalList *model.PostList, afterPost, beforePost string, since int64, page, perPage int, collapsedThreads bool) {
	prevPostIdSet := false
	prevPostId := ""
	nextPostIdSet := false
	nextPostId := ""
	...
	if !nextPostIdSet {
		nextPostId = a.GetNextPostIdFromPostList(c, originalList, collapsedThreads)
	}

	if !prevPostIdSet {
		prevPostId = a.GetPrevPostIdFromPostList(c, originalList, collapsedThreads)
	}
...
func (a *App) GetPostsForChannelAroundLastUnread(c request.CTX, channelID, userID string, limitBefore, limitAfter int, skipFetchThreads bool, collapsedThreads, collapsedThreadsExtended bool) (*model.PostList, *model.AppError) {
	...
	postList, err := a.GetPostThread(c, lastUnreadPostId, opts, userID)
	if err != nil {
		return nil, err
	}
	...
	if _, ok := postList.Posts[lastUnreadPostId]; ok {
		...
		if postListBefore, err := a.GetPostsBeforePost(c, model.GetPostsOptions{ChannelId: channelID, PostId: lastUnreadPostId, Page: PageDefault, PerPage: limitBefore, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads, CollapsedThreadsExtended: collapsedThreadsExtended, UserId: userID}); err != nil {
			return nil, err
	...
	if postListAfter, err := a.GetPostsAfterPost(c, model.GetPostsOptions{ChannelId: channelID, PostId: lastUnreadPostId, Page: PageDefault, PerPage: limitAfter - 1, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads, CollapsedThreadsExtended: collapsedThreadsExtended, UserId: userID}); err != nil {
		return nil, err
...
func (a *App) searchPostsInTeam(c request.CTX, teamID string, userID string, paramsList []*model.SearchParams, modifierFun func(*model.SearchParams)) (*model.PostList, *model.AppError) {
	...
	a.filterInaccessiblePosts(c, posts, filterPostOptions{assumeSortedCreatedAt: true})
...
func (a *App) ComputeLastAccessiblePostTime(c request.CTX) error {
	limit, appErr := a.getCloudMessagesHistoryLimit(c)
	if appErr != nil {
		return appErr
	}
...
func (a *App) SearchPostsInTeam(c request.CTX, teamID string, paramsList []*model.SearchParams) (*model.PostList, *model.AppError) {
	...
	return a.searchPostsInTeam(c, teamID, "", paramsList, func(params *model.SearchParams) {
		params.SearchWithoutUserId = true
	})
}

func (a *App) SearchPostsForUser(c request.CTX, terms string, userID string, teamID string, isOrSearch bool, includeDeletedChannels bool, timeZoneOffset int, page, perPage int, modifier string) (*model.PostSearchResults, *model.AppError) {
	...
	if appErr := a.filterInaccessiblePosts(c, postSearchResults.PostList, filterPostOptions{assumeSortedCreatedAt: true}); appErr != nil {
		return nil, appErr
	}
...
func (a *App) GetFileInfosForPostWithMigration(c request.CTX, postID string, includeDeleted bool) ([]*model.FileInfo, *model.AppError) {
	...
	infos, firstInaccessibleFileTime, err := a.GetFileInfosForPost(c, postID, false, includeDeleted)
	if err != nil {
		return nil, err
	}
	...
	if len(infos) == 0 && firstInaccessibleFileTime == 0 {
		...
		if len(post.Filenames) > 0 {
			...
			infos = a.MigrateFilenamesToFileInfos(c, post)
		}
	}
...
func (a *App) GetFileInfosForPost(c request.CTX, postID string, fromMaster bool, includeDeleted bool) ([]*model.FileInfo, int64, *model.AppError) {
...
func (a *App) countMentionsFromPost(c request.CTX, user *model.User, post *model.Post) (int, int, *model.AppError) {
	...
	thread, err := a.GetPostThread(c, post.Id, model.GetPostsOptions{}, user.Id)
	if err != nil {
		return 0, 0, err
	}
	...
	perPage := 200
	for {
		postList, err := a.GetPostsAfterPost(c, model.GetPostsOptions{
...
func (a *App) GetPostIfAuthorized(c request.CTX, postID string, session *model.Session, includeDeleted bool) (*model.Post, *model.AppError) {
	...
	if !a.SessionHasPermissionToChannel(c, *session, channel.Id, model.PermissionReadChannel) {
		if channel.Type == model.ChannelTypeOpen {
			if !a.SessionHasPermissionToTeam(c, *session, channel.TeamId, model.PermissionReadPublicChannel) {
...
func (a *App) GetPostsByIds(c request.CTX, postIDs []string) ([]*model.Post, int64, *model.AppError) {
	...
	posts, firstInaccessiblePostTime, appErr := a.getFilteredAccessiblePosts(c, posts, filterPostOptions{assumeSortedCreatedAt: true})
	if appErr != nil {
		return nil, 0, appErr
	}
...
func (a *App) CheckPostReminders(c request.CTX) {
	systemBot, appErr := a.GetSystemBot()
	if appErr != nil {
		c.Logger().Error("Failed to get system bot", mlog.Err(appErr))
		return
	}
	...
	if err != nil {
		c.Logger().Error("Failed to get post reminders", mlog.Err(err))
		return
	}
	...
	for userID, postIDs := range groupedReminders {
		...
		if appErr != nil {
			c.Logger().Error("Failed to get direct channel", mlog.Err(appErr))
			return
		}
		...
		for _, postID := range postIDs {
			metadata, err := a.Srv().Store().Post().GetPostReminderMetadata(postID)
			if err != nil {
				c.Logger().Error("Failed to get post reminder metadata", mlog.Err(err))
				continue
			}
			...
			if _, err := a.CreatePost(request.EmptyContext(a.Log()), dm, ch, false, true); err != nil {
				c.Logger().Error("Failed to post reminder message", mlog.Err(err))
			}
		}
	}
