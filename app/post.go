...
func (a *App) CreatePost(post *model.Post, channel *model.Channel, triggerWebhooks, setOnline bool) (savedPost *model.Post, err *model.AppError) {
	...
	if len(post.RootId) > 0 {
		...
		go func() {
			r, pErr := a.Srv().Store.Post().Get(post.RootId, false, false)
...
func (a *App) GetPostThread(postId string, skipFetchThreads, collapsedThreads bool) (*model.PostList, *model.AppError) {
	posts, err := a.Srv().Store.Post().Get(postId, skipFetchThreads, collapsedThreads)
	if err != nil {
		var nfErr *store.ErrNotFound
		var invErr *store.ErrInvalidInput
		switch {
...
func (a *App) GetPostsForChannelAroundLastUnread(channelId, userId string, limitBefore, limitAfter int, skipFetchThreads bool, collapsedThreads bool) (*model.PostList, *model.AppError) {
	...
	postList, err := a.GetPostThread(lastUnreadPostId, skipFetchThreads, collapsedThreads)
	if err != nil {
		return nil, err
	}
	...
	if postListBefore, err := a.GetPostsBeforePost(model.GetPostsOptions{ChannelId: channelId, PostId: lastUnreadPostId, Page: PAGE_DEFAULT, PerPage: limitBefore, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads}); err != nil {
		return nil, err
	} else if postListBefore != nil {
		postList.Extend(postListBefore)
	}

	if postListAfter, err := a.GetPostsAfterPost(model.GetPostsOptions{ChannelId: channelId, PostId: lastUnreadPostId, Page: PAGE_DEFAULT, PerPage: limitAfter - 1, SkipFetchThreads: skipFetchThreads, CollapsedThreads: collapsedThreads}); err != nil {
		return nil, err
...
func (a *App) countMentionsFromPost(user *model.User, post *model.Post) (int, *model.AppError) {
	...
	thread, err := a.GetPostThread(post.Id, false, false)
	if err != nil {
		return 0, err
	}
