...
func (a *App) GetFileInfosForPostWithMigration(postID string, includeDeteled bool) ([]*model.FileInfo, *model.AppError) {

	pchan := make(chan store.StoreResult, 1)
	go func() {
		post, err := a.Srv().Store.Post().GetSingle(postID, includeDeteled)
		pchan <- store.StoreResult{Data: post, NErr: err}
		close(pchan)
	}()

	infos, err := a.GetFileInfosForPost(postID, false, includeDeteled)
	if err != nil {
		return nil, err
	}
