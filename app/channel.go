...
func (a *App) UpdateChannel(c request.CTX, channel *model.Channel) (*model.Channel, *model.AppError) {
	...
	if err != nil {
		c.Logger().Warn("Failed to encode channel to JSON", mlog.Err(err))
	}
...
func (a *App) markChannelAsUnreadFromPostCRTUnsupported(c request.CTX, postID string, userID string) (*model.ChannelUnreadAt, *model.AppError) {
	...
	if *a.Config().ServiceSettings.ThreadAutoFollow {
		threadMembership, err := a.Srv().Store.Thread().GetMembershipForUser(user.Id, threadId)
		...
		if threadMembership == nil {
			...
			threadMembership, err = a.Srv().Store.Thread().MaintainMembership(user.Id, threadId, opts)
			if err != nil {
				return nil, model.NewAppError("MarkChannelAsUnreadFromPost", "app.channel.update_last_viewed_at_post.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			}
		}
		...
		threadMembership, err = a.Srv().Store.Thread().UpdateMembership(threadMembership)
		if err != nil {
			return nil, model.NewAppError("MarkChannelAsUnreadFromPost", "app.channel.update_last_viewed_at_post.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
		}
		thread, err := a.Srv().Store.Thread().GetThreadForUser(channel.TeamId, threadMembership, true)
		if err != nil {
			return nil, model.NewAppError("MarkChannelAsUnreadFromPost", "app.channel.update_last_viewed_at_post.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
		}
...
func (a *App) ClearChannelMembersCache(c request.CTX, channelID string) {
	clearSessionCache := func(channelMember model.ChannelMember) error {
		...
		memberJSON, err := json.Marshal(channelMember)
		if err != nil {
			c.Logger().Warn("Failed to encode channel member to JSON", mlog.Err(err))
		}
	...
	if err := a.forEachChannelMember(c, channelID, clearSessionCache); err != nil {
		c.Logger().Warn("error clearing cache for channel members", mlog.String("channel_id", channelID), mlog.Err(err))
	}
}
