...
func (a *App) createDirectChannel(userId string, otherUserId string) (*model.Channel, *model.AppError) {
	...
	go func() {
		user, err := a.Srv().Store.User().Get(context.Background(), otherUserId)
...
func (a *App) AddChannelMember(userId string, channel *model.Channel, userRequestorId string, postRootId string) (*model.ChannelMember, *model.AppError) {
	...
	if user, err2 = a.Srv().Store.User().Get(sqlstore.WithMaster(context.Background()), userId); err2 != nil {
		return nil, model.NewAppError("AddChannelMember", "app.channel.get_user.app_error", nil, err2.Error(), http.StatusInternalServerError)
	}
