...
func setupMultiPluginAPITest(t *testing.T, pluginCodes []string, pluginManifests []string, pluginIDs []string, asMain bool, app *App, c *request.Context) string {
	...
	env, err := plugin.NewEnvironment(newPluginAPI, NewDriverImpl(app.Srv()), pluginDir, webappPluginDir, false, app.Log(), nil, app.Srv())
	require.NoError(t, err)
...
func TestPluginAPIGetPlugins(t *testing.T) {
  ...
  `
	...
	env, err := plugin.NewEnvironment(th.NewPluginAPI, NewDriverImpl(th.Server), pluginDir, webappPluginDir, false, th.App.Log(), nil, th.Server)
	require.NoError(t, err)
...
func TestRegisterCollectionAndTopic(t *testing.T) {
  ...
  `
	...
	env, err := plugin.NewEnvironment(newPluginAPI, NewDriverImpl(th.Server), pluginDir, webappPluginDir, false, th.App.Log(), nil, th.Server)
	require.NoError(t, err)
