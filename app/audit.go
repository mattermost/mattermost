...
func (s *Server) configureAudit(c request.CTX, adt *audit.Audit, bAllowAdvancedLogging bool) error {
	...
	if bAllowAdvancedLogging && dsn != "" {
		...
		c.Logger().Debug("Loaded audit configuration", mlog.String("source", dsn))
	}
