...
func (a *App) CreateRole(role *model.Role) (*model.Role, *model.AppError) {
	...
	return a.Store().Role().Save(role)

}

func (a *App) UpdateRole(role *model.Role) (*model.Role, *model.AppError) {
	savedRole, err := a.Store().Role().Save(role)
	if err != nil {
		return nil, err
	}
