...
func (a *App) importTeam(data *TeamImportData, dryRun bool) *model.AppError {
	...
	var team *model.Team
	team, err := a.Store().Team().GetByName(*data.Name)
...
func (a *App) importChannel(data *ChannelImportData, dryRun bool) *model.AppError {
	...
	team, err := a.Store().Team().GetByName(*data.Team)
	if err != nil {
	...
	if result, err := a.Store().Channel().GetByNameIncludeDeleted(team.Id, *data.Name, true); err == nil {
		channel = result
	} else {
...
func (a *App) importUser(data *UserImportData, dryRun bool) *model.AppError {
	...
	user, err = a.Store().User().GetByUsername(*data.Username)
	if err != nil {
		user = &model.User{}
		user.MakeNonNil()
	...
	if user.Id == "" {
		...
		if len(password) > 0 {
			...
			if hasUserAuthDataChanged {
				if _, err = a.Store().User().UpdateAuthData(user.Id, authService, authData, user.Email, false); err != nil {
					return err
				}
			}
		}
...
func (a *App) importUserTeams(user *model.User, data *[]UserTeamImportData) *model.AppError {
	...
	if len(teamThemePreferences) > 0 {
		if err := a.Store().Preference().Save(&teamThemePreferences); err != nil {
...
func (a *App) importReaction(data *ReactionImportData, post *model.Post, dryRun bool) *model.AppError {
	...
	var user *model.User
	user, err = a.Store().User().GetByUsername(*data.User)
	if err != nil {
	...
	if _, err = a.Store().Reaction().Save(reaction); err != nil {
		return err
	}
...
func (a *App) importReply(data *ReplyImportData, post *model.Post, teamId string, dryRun bool) *model.AppError {
	...
	replies, err := a.Store().Post().GetPostsCreatedAt(post.ChannelId, *data.CreateAt)
	if err != nil {
		return err
	}
	...
	for _, fileID := range reply.FileIds {
		if _, ok := fileIds[fileID]; !ok {
			a.Store().FileInfo().PermanentDelete(fileID)
		}
	}
	...
	if reply.Id == "" {
		if _, err := a.Store().Post().Save(reply); err != nil {
			return err
		}
	} else {
		if _, err := a.Store().Post().Overwrite(reply); err != nil {
			return err
		}
	}
...
func (a *App) importPost(data *PostImportData, dryRun bool) *model.AppError {
	...
	channel, err := a.Store().Channel().GetByName(team.Id, *data.Channel, false)
	if err != nil {
	...
	// Check if this post already exists.
	posts, err := a.Store().Post().GetPostsCreatedAt(channel.Id, *data.CreateAt)
	if err != nil {
		return err
	}
	...
	if post.Id == "" {
		if _, err = a.Store().Post().Save(post); err != nil {
			return err
		}
	} else {
...
func (a *App) importDirectChannel(data *DirectChannelImportData, dryRun bool) *model.AppError {
	...
	if data.Header != nil {
		channel.Header = *data.Header
		if _, appErr := a.Store().Channel().Update(channel); appErr != nil {
...
func (a *App) importEmoji(data *EmojiImportData, dryRun bool) *model.AppError {
	...
	emoji, appError := a.Store().Emoji().GetByName(*data.Name, true)
	...
	if !alreadyExists {
		if _, err := a.Store().Emoji().Save(emoji); err != nil {
			return err
		}
	}
