...
func TestGetJob(t *testing.T) {
	...
	require.Nil(t, err)
	assert.Equal(t, status, received, "incorrect job status received")
}
...
func TestGetJobByType(t *testing.T) {
	...
	assert.Equal(t, statuses[2], received[0], "should've received newest job first")
	assert.Equal(t, statuses[0], received[1], "should've received second newest job second")

	received, err = th.App.GetJobsByType(jobType, 2, 2)
	require.Nil(t, err)
	require.Len(t, received, 1, "received wrong number of statuses")
	assert.Equal(t, statuses[1], received[0], "should've received oldest job last")
}
