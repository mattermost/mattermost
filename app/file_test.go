...
func TestGenerateThumbnailImage(t *testing.T) {
	t.Run("test generating thumbnail image", func(t *testing.T) {
		...
		// when
		th.App.generateThumbnailImage(th.Context, img, thumbnailName)
...
func TestGetLastAccessibleFileTime(t *testing.T) {
	...
	r, err := th.App.GetLastAccessibleFileTime(th.Context)
	require.Nil(t, err)
	...
	_, err = th.App.GetLastAccessibleFileTime(th.Context)
