...
func (a *App) GetAnalytics(name string, teamID string) (model.AnalyticsRows, *model.AppError) {
	...
	if name == "standard" {
		...
		g.Go(func() error {
			var err error
			if openChannelsCount, err = a.Srv().Store.Channel().AnalyticsTypeCount(teamID, model.ChannelOpen); err != nil {
		...
		g.Go(func() error {
			var err error
			if privateChannelsCount, err = a.Srv().Store.Channel().AnalyticsTypeCount(teamID, model.ChannelPrivate); err != nil {
				return model.NewAppError("GetAnalytics", "app.channel.analytics_type_count.app_error", nil, err.Error(), http.StatusInternalServerError)
			}
			return nil
		})
