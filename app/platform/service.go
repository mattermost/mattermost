...
func New(sc ServiceConfig, options ...Option) (*PlatformService, error) {
	...
	if ps.newStore == nil {
		ps.newStore = func() (store.Store, error) {
			var sendSQLQuery func(string, float64, ...any)
			if ps.DebugBar != nil && ps.DebugBar.IsEnabled() {
				sendSQLQuery = ps.DebugBar.SendSqlQuery
			}
			ps.sqlStore = sqlstore.New(ps.Config().SqlSettings, ps.metricsIFace, sendSQLQuery)
