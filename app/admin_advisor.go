...
func (a *App) GetWarnMetricsStatus(c request.CTX) (map[string]*model.WarnMetricStatus, *model.AppError) {
		...
		if strings.HasPrefix(key, model.WarnMetricStatusStorePrefix) {
			if warnMetric, ok := model.WarnMetricsTable[key]; ok {
				if !warnMetric.IsBotOnly && (value == model.WarnMetricStatusRunonce || value == model.WarnMetricStatusLimitReached) {
					result[key], _ = a.getWarnMetricStatusAndDisplayTextsForId(c, key, nil, isE0Edition)
				}
			}
		}
	}
...
func (a *App) getWarnMetricStatusAndDisplayTextsForId(c request.CTX, warnMetricId string, T i18n.TranslateFunc, isE0Edition bool) (*model.WarnMetricStatus, *model.WarnMetricDisplayTexts) {
	...
	if warnMetric, ok := model.WarnMetricsTable[warnMetricId]; ok {
		...
		if T == nil {
			c.Logger().Debug("No translation function")
		...
		default:
			c.Logger().Debug("Invalid metric id", mlog.String("id", warnMetricId))
			return nil, nil
		}
...
func (a *App) NotifyAndSetWarnMetricAck(c request.CTX, warnMetricId string, sender *model.User, forceAck bool, isBot bool) *model.AppError {
	if warnMetric, ok := model.WarnMetricsTable[warnMetricId]; ok {
		data, nErr := a.Srv().Store().System().GetByName(warnMetric.Id)
		if nErr == nil && data != nil && data.Value == model.WarnMetricStatusAck {
			c.Logger().Debug("This metric warning has already been acknowledged", mlog.String("id", warnMetric.Id))
			return nil
		}
		...
		if !forceAck {
			...
			if cerr != nil {
				c.Logger().Warn("Error retrieving the number of registered users", mlog.Err(cerr))
			} else {
			...
			warnMetricStatus, warnMetricDisplayTexts := a.getWarnMetricStatusAndDisplayTextsForId(c, warnMetricId, T, false)
		...
		if err := a.setWarnMetricsStatusAndNotify(c, warnMetric.Id); err != nil {
			return err
		}
	}
	return nil
...
func (a *App) setWarnMetricsStatusAndNotify(c request.CTX, warnMetricId string) *model.AppError {
	// Ack all metric warnings on the server
	if err := a.setWarnMetricsStatus(c, model.WarnMetricStatusAck); err != nil {
		return err
	}
...
func (a *App) setWarnMetricsStatus(c request.CTX, status string) *model.AppError {
	c.Logger().Debug("Set monitoring status for all warn metrics", mlog.String("status", status))
	for _, warnMetric := range model.WarnMetricsTable {
		if err := a.setWarnMetricsStatusForId(c, warnMetric.Id, status); err != nil {
			return err
		}
	}
	return nil
...
func (a *App) setWarnMetricsStatusForId(c request.CTX, warnMetricId string, status string) *model.AppError {
	c.Logger().Debug("Store status for warn metric", mlog.String("warnMetricId", warnMetricId), mlog.String("status", status))
