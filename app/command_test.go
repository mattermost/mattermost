...
func TestHandleCommandResponsePost(t *testing.T) {
	...
	assert.Nil(t, post.GetProps()["override_icon_url"])
	assert.Nil(t, post.GetProps()["override_username"])
	assert.Nil(t, post.GetProps()["from_webhook"])

	// Command is not built in, so it is a bot command.
	builtIn = false
	post, err = th.App.HandleCommandResponsePost(command, args, resp, builtIn)
	assert.Equal(t, "true", post.GetProps()["from_webhook"])
	...
	*th.App.Config().ServiceSettings.EnablePostUsernameOverride = true
	...
	post, err = th.App.HandleCommandResponsePost(command, args, resp, builtIn)
	assert.Nil(t, err)
	assert.Equal(t, command.Username, post.GetProps()["override_username"])
	assert.Equal(t, "true", post.GetProps()["from_webhook"])
	...
	post, err = th.App.HandleCommandResponsePost(command, args, resp, builtIn)
	assert.Nil(t, err)
	assert.Equal(t, resp.Username, post.GetProps()["override_username"])
	assert.Equal(t, "true", post.GetProps()["from_webhook"])
	...
	*th.App.Config().ServiceSettings.EnablePostIconOverride = true
	...
	post, err = th.App.HandleCommandResponsePost(command, args, resp, builtIn)
	assert.Nil(t, err)
	assert.Equal(t, command.IconURL, post.GetProps()["override_icon_url"])
	assert.Equal(t, "true", post.GetProps()["from_webhook"])
	...
	post, err = th.App.HandleCommandResponsePost(command, args, resp, builtIn)
	assert.Nil(t, err)
	assert.Equal(t, resp.IconURL, post.GetProps()["override_icon_url"])
	assert.Equal(t, "true", post.GetProps()["from_webhook"])
