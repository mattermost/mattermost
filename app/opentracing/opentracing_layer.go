...
func (a *OpenTracingAppLayer) AddPreviewerToChannel(c request.CTX, userID string, channel *model.Channel) (*model.ChannelMember, *model.AppError) {
	origCtx := a.ctx
	span, newCtx := tracing.StartSpanWithParentByContext(a.ctx, "app.AddPreviewerToChannel")
	...
	defer span.Finish()
	resultVar0, resultVar1 := a.app.AddPreviewerToChannel(c, userID, channel)
...
func (a *OpenTracingAppLayer) ConvertPreviewerToMember(c request.CTX, userId string, channel *model.Channel) *model.AppError {
	origCtx := a.ctx
	span, newCtx := tracing.StartSpanWithParentByContext(a.ctx, "app.ConvertPreviewerToMember")
	...
	defer span.Finish()
	resultVar0 := a.app.ConvertPreviewerToMember(c, userId, channel)
