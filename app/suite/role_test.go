...
func TestGetRolesByNames(t *testing.T) {
	testPermissionInheritance(t, func(t *testing.T, th *TestHelper, testData permissionInheritanceTestData) {
		actualRoles, err := th.Suite.GetRolesByNames([]string{testData.channelRole.Name})
		require.Nil(t, err)
...
func TestGetRoleByName(t *testing.T) {
	testPermissionInheritance(t, func(t *testing.T, th *TestHelper, testData permissionInheritanceTestData) {
		actualRole, err := th.Suite.GetRoleByName(context.Background(), testData.channelRole.Name)
		require.Nil(t, err)
		require.NotNil(t, actualRole)
		require.Equal(t, testData.channelRole.Name, actualRole.Name)
...
func TestGetRoleByID(t *testing.T) {
	testPermissionInheritance(t, func(t *testing.T, th *TestHelper, testData permissionInheritanceTestData) {
		actualRole, err := th.Suite.GetRole(testData.channelRole.Id)
		require.Nil(t, err)
		require.NotNil(t, actualRole)
...
func TestGetAllRoles(t *testing.T) {
	testPermissionInheritance(t, func(t *testing.T, th *TestHelper, testData permissionInheritanceTestData) {
		actualRoles, err := th.Suite.GetAllRoles()
		require.Nil(t, err)
...
func testPermissionInheritance(t *testing.T, testCallback func(t *testing.T, th *TestHelper, testData permissionInheritanceTestData)) {
	...
	th.Suite.SetPhase2PermissionsMigrationStatus(true)
	...
	systemSchemeRoles, err := th.Suite.GetRolesByNames([]string{
	...
	for _, systemRole := range systemSchemeRoles {
		defer th.Suite.PatchRole(systemRole, &model.RolePatch{
			Permissions: &systemRole.Permissions,
		})
	}

	// Make a channel scheme, clear its permissions
	channelScheme, err := th.Suite.CreateScheme(&model.Scheme{
		Name:        model.NewId(),
		DisplayName: model.NewId(),
		Scope:       model.SchemeScopeChannel,
	})
	require.Nil(t, err)
	defer th.Suite.DeleteScheme(channelScheme.Id)

	team := th.CreateTeam()
	defer th.Suite.PermanentDeleteTeamId(th.Context, team.Id)

	// Make a channel
	channel := th.CreateChannel(th.Context, team)
	defer th.Suite.channels.PermanentDeleteChannel(th.Context, channel)

	// Set the channel scheme
	channel.SchemeId = &channelScheme.Id
	channel, err = th.Suite.channels.UpdateChannelScheme(th.Context, channel)
	require.Nil(t, err)
	...
	test := func(higherScopedGuest, higherScopedUser, higherScopedAdmin string) {
		for _, roleNameUnderTest := range []string{higherScopedGuest, higherScopedUser, higherScopedAdmin} {
			for i, row := range records {
				...
				higherScopedRole, testErr := th.Suite.GetRoleByName(context.Background(), roleNameUnderTest)
				...
				higherScopedRole, testErr = th.Suite.PatchRole(higherScopedRole, &model.RolePatch{Permissions: &higherScopedPermissions})
				require.Nil(t, testErr)
				...
				channelRole, testErr := th.Suite.GetRoleByName(context.Background(), channelRoleName)
				require.Nil(t, testErr)
				...
				channelRole, testErr = th.Suite.PatchRole(channelRole, &model.RolePatch{Permissions: &channelSchemePermissions})
				require.Nil(t, testErr)
	...
	teamScheme, err := th.Suite.CreateScheme(&model.Scheme{
		Name:        model.NewId(),
		DisplayName: model.NewId(),
		Scope:       model.SchemeScopeTeam,
	})
	require.Nil(t, err)
	defer th.Suite.DeleteScheme(teamScheme.Id)

	// assign the scheme to the team
	team.SchemeId = &teamScheme.Id
	_, err = th.Suite.UpdateTeamScheme(team)
	require.Nil(t, err)
