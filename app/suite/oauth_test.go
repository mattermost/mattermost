...
func TestGetOAuthAccessTokenForImplicitFlow(t *testing.T) {
	...
	th.Suite.platform.UpdateConfig(func(cfg *model.Config) { *cfg.ServiceSettings.EnableOAuthServiceProvider = true })
	...
	oapp, err := th.Suite.CreateOAuthApp(oapp)
	require.Nil(t, err)
	...
	session, err := th.Suite.GetOAuthAccessTokenForImplicitFlow(th.BasicUser.Id, authRequest)
	assert.Nil(t, err)
	assert.NotNil(t, session)

	th.Suite.platform.UpdateConfig(func(cfg *model.Config) { *cfg.ServiceSettings.EnableOAuthServiceProvider = false })
	...
	session, err = th.Suite.GetOAuthAccessTokenForImplicitFlow("junk", authRequest)
...
func TestOAuthRevokeAccessToken(t *testing.T) {
	...
	th.Suite.SetSessionExpireInHours(session, 24)

	var err *model.AppError
	session, err = th.Suite.CreateSession(session)
	require.Nil(t, err)
	err = th.Suite.RevokeAccessToken(session.Token)
...
func TestOAuthDeleteApp(t *testing.T) {
	...
	*th.Suite.platform.Config().ServiceSettings.EnableOAuthServiceProvider = true
	...
	var err *model.AppError
	a1, err = th.Suite.CreateOAuthApp(a1)
	require.Nil(t, err)
	...
	th.Suite.platform.SetSessionExpireInHours(session, 24)
	...
	_, nErr := th.Suite.platform.Store.OAuth().SaveAccessData(accessData)
	require.NoError(t, nErr)

	err = th.Suite.DeleteOAuthApp(a1.Id)
	require.Nil(t, err)
...
func TestAuthorizeOAuthUser(t *testing.T) {
	...
	makeToken := func(th *TestHelper, cookie string) *model.Token {
		token, _ := th.Suite.CreateOAuthStateToken(generateOAuthStateTokenExtra("", "", cookie))
		return token
	}
	...
	t.Run("not enabled", func(t *testing.T) {
		...
		_, _, _, _, err := th.Suite.AuthorizeOAuthUser(nil, nil, model.ServiceGitlab, "", "", "")
	...
	t.Run("with an improperly encoded state", func(t *testing.T) {
		...
		_, _, _, _, err := th.Suite.AuthorizeOAuthUser(nil, nil, model.ServiceGitlab, "", state, "")
		require.NotNil(t, err)
	...
	t.Run("with email missing when changing login types", func(t *testing.T) {
		...
		token, err := th.Suite.CreateOAuthStateToken(generateOAuthStateTokenExtra(email, action, cookie))
		require.Nil(t, err)
	...
	t.Run("without an OAuth cookie", func(t *testing.T) {
		...
		_, _, _, _, err := th.Suite.AuthorizeOAuthUser(nil, request, model.ServiceGitlab, "", state, "")
		require.NotNil(t, err)
		assert.Equal(t, "api.user.authorize_oauth_user.invalid_state.app_error", err.Id)
	})
	...
	t.Run("with an invalid token", func(t *testing.T) {
		...
		token, err := th.Suite.CreateOAuthStateToken(model.NewId())
		require.Nil(t, err)
	...
	t.Run("with an incorrect token endpoint", func(t *testing.T) {
		...
		_, _, _, _, err := th.Suite.AuthorizeOAuthUser(&httptest.ResponseRecorder{}, request, model.ServiceGitlab, "", state, "")
		require.NotNil(t, err)
	...
	t.Run("with error in GetSSOSettings", func(t *testing.T) {
		...
		_, _, _, _, err := th.Suite.AuthorizeOAuthUser(nil, nil, model.ServiceOpenid, "", "", "")
		require.NotNil(t, err)
	...
	t.Run("enabled and properly configured", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.Description, func(t *testing.T) {
				...
				body, receivedTeamId, receivedStateProps, _, err := th.Suite.AuthorizeOAuthUser(&recorder, request, model.ServiceGitlab, "", state, "")
...
func TestGetAuthorizationCode(t *testing.T) {
	t.Run("not enabled", func(t *testing.T) {
		...
		_, err := th.Suite.GetAuthorizationCode(nil, nil, model.ServiceGitlab, map[string]string{}, "")
		require.NotNil(t, err)
	...
	t.Run("enabled and properly configured", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.Description, func(t *testing.T) {
				...
				recorder := httptest.ResponseRecorder{}
				url, err := th.Suite.GetAuthorizationCode(&recorder, request, model.ServiceGitlab, stateProps, "")
				require.Nil(t, err)
