...
func TestCheckForClientSideCert(t *testing.T) {
	...
	for _, tt := range tests {
		...
		_, _, actualEmail := th.Suite.CheckForClientSideCert(r)
...
func TestCWSLogin(t *testing.T) {
	...
	th.Suite.platform.SetLicense(license)
	...
	t.Run("Should authenticate user when CWS login is enabled and tokens are equal", func(t *testing.T) {
		...
		user, err := th.Suite.AuthenticateUserForLogin(th.Context, "", th.BasicUser.Username, "", "", token.Token, false)
		require.Nil(t, err)
		require.NotNil(t, user)
		require.Equal(t, th.BasicUser.Username, user.Username)
		_, apperr := th.Suite.platform.Store.Token().GetByToken(token.Token)
		require.NoError(t, apperr)
		th.Suite.DeleteToken(token)
	})
