...
func BenchmarkUploadFile(b *testing.B) {
	...
	file_benchmarks := []struct {
	...
	}{
		{
			...
			f: func(b *testing.B, n int, data []byte, ext string) {
				info1, err := th.Suite.DoUploadFile(th.Context, time.Now(), teamID, channelID,
		...
		},
		{
			...
			f: func(b *testing.B, n int, data []byte, ext string) {
				info, aerr := th.Suite.UploadFileX(th.Context, channelID,
				...
				th.Suite.platform.Store.FileInfo().PermanentDelete(info.Id)
				th.Suite.RemoveFile(info.Path)
			},
		},
		{
