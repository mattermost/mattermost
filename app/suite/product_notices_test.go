	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if ok, err := noticeMatchesConditions(
				th.Suite.platform.Config(),
				th.Suite.platform.Store.Preference(),
				"test",
				tt.args.client,
				clientVersion,
				tt.args.postCount,
...
func TestNoticeFetch(t *testing.T) {
	...
	// fetch fake notices
	appErr := th.Suite.UpdateProductNotices()
	require.Nil(t, appErr)

	// get them for specified user
	messages, appErr := th.Suite.GetProductNotices(th.Context, th.BasicUser.Id, th.BasicTeam.Id, model.NoticeClientTypeAll, "1.2.3", "en")
	require.Nil(t, appErr)
	require.Len(t, messages, 1)

	// mark notices as viewed
	appErr = th.Suite.UpdateViewedProductNotices(th.BasicUser.Id, []string{messages[0].ID})
	require.Nil(t, appErr)
	...
	views, err := th.Suite.platform.Store.ProductNotices().GetViews(th.BasicUser.Id)
	require.NoError(t, err)
