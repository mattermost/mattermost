...
func TestCustomStatus(t *testing.T) {
	...
	err := th.Suite.SetCustomStatus(th.Context, user.Id, cs)
	require.Nil(t, err, "failed to set custom status %v", err)

	csSaved, err := th.Suite.GetCustomStatus(user.Id)
	require.Nil(t, err, "failed to get custom status after save %v", err)
	require.Equal(t, cs, csSaved)

	err = th.Suite.RemoveCustomStatus(th.Context, user.Id)
...
func TestCustomStatusErrors(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			case "set":
				appErr = th.Suite.SetCustomStatus(th.Context, fakeUserID, cs)
			case "remove":
				appErr = th.Suite.RemoveCustomStatus(th.Context, fakeUserID)
			}
