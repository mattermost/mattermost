...
func TestCreateBot(t *testing.T) {
	t.Run("invalid bot", func(t *testing.T) {
		t.Run("relative to user", func(t *testing.T) {
			...
			_, err := th.Suite.CreateBot(th.Context, &model.Bot{
	...
	t.Run("create bot", func(t *testing.T) {
		...
		user, err := th.Suite.GetUser(bot.UserId)
		require.Nil(t, err)

		// Check that a post was created to add bot to team and channels
		channel, err := th.Suite.channels.GetOrCreateDirectChannelWithUser(th.Context, user, th.BasicUser)
		require.Nil(t, err)
		posts, err := th.Suite.channels.GetPosts(channel.Id, 0, 1)
		require.Nil(t, err)
...
func TestPatchBot(t *testing.T) {
	t.Run("invalid patch for user", func(t *testing.T) {
		...
		_, err = th.Suite.PatchBot(bot.UserId, botPatch)
		require.NotNil(t, err)
	...
	t.Run("patch bot", func(t *testing.T) {
		...
		createdBot, err := th.Suite.CreateBot(th.Context, bot)
		require.Nil(t, err)
		defer th.Suite.PermanentDeleteBot(createdBot.UserId)
		...
		patchedBot, err := th.Suite.PatchBot(createdBot.UserId, botPatch)
		require.Nil(t, err)
...
func TestGetBot(t *testing.T) {
	...
	bot1, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username",
		Description: "a bot",
		OwnerId:     th.BasicUser.Id,
	})
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(bot1.UserId)

	bot2, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username2",
		Description: "a second bot",
		OwnerId:     th.BasicUser.Id,
	})
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(bot2.UserId)

	deletedBot, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username3",
		Description: "a deleted bot",
		OwnerId:     th.BasicUser.Id,
	})
	require.Nil(t, err)
	deletedBot, err = th.Suite.UpdateBotActive(th.Context, deletedBot.UserId, false)
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(deletedBot.UserId)

	t.Run("get unknown bot", func(t *testing.T) {
		_, err := th.Suite.GetBot(model.NewId(), false)
		require.NotNil(t, err)
		require.Equal(t, "store.sql_bot.get.missing.app_error", err.Id)
	})

	t.Run("get bot1", func(t *testing.T) {
		bot, err := th.Suite.GetBot(bot1.UserId, false)
		require.Nil(t, err)
		assert.Equal(t, bot1, bot)
	})

	t.Run("get bot2", func(t *testing.T) {
		bot, err := th.Suite.GetBot(bot2.UserId, false)
		require.Nil(t, err)
		assert.Equal(t, bot2, bot)
	})

	t.Run("get deleted bot", func(t *testing.T) {
		_, err := th.Suite.GetBot(deletedBot.UserId, false)
		require.NotNil(t, err)
		require.Equal(t, "store.sql_bot.get.missing.app_error", err.Id)
	})

	t.Run("get deleted bot, include deleted", func(t *testing.T) {
		bot, err := th.Suite.GetBot(deletedBot.UserId, true)
		require.Nil(t, err)
...
func TestGetBots(t *testing.T) {
	...
	deletedBot1, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username4",
		Description: "a deleted bot",
		OwnerId:     OwnerId1,
	})
	require.Nil(t, err)
	deletedBot1, err = th.Suite.UpdateBotActive(th.Context, deletedBot1.UserId, false)
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(deletedBot1.UserId)
	...
	bot3, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username3",
		Description: "a third bot",
		OwnerId:     OwnerId1,
	})
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(bot3.UserId)

	bot4, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username5",
		Description: "a fourth bot",
		OwnerId:     OwnerId2,
	})
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(bot4.UserId)

	deletedBot2, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username6",
		Description: "a deleted bot",
		OwnerId:     OwnerId2,
	})
	require.Nil(t, err)
	deletedBot2, err = th.Suite.UpdateBotActive(th.Context, deletedBot2.UserId, false)
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(deletedBot2.UserId)

	t.Run("get bots, page=0, perPage=10", func(t *testing.T) {
		bots, err := th.Suite.GetBots(&model.BotGetOptions{
			Page:           0,
			PerPage:        10,
			OwnerId:        "",
...
func TestUpdateBotActive(t *testing.T) {
	t.Run("unknown bot", func(t *testing.T) {
		...
		_, err := th.Suite.UpdateBotActive(th.Context, model.NewId(), false)
		require.NotNil(t, err)
	...
	t.Run("disable/enable bot", func(t *testing.T) {
		...
		disabledBot, err := th.Suite.UpdateBotActive(th.Context, bot.UserId, false)
		require.Nil(t, err)
		require.NotEqual(t, 0, disabledBot.DeleteAt)

		// Disabling should be idempotent
		disabledBotAgain, err := th.Suite.UpdateBotActive(th.Context, bot.UserId, false)
		require.Nil(t, err)
		require.Equal(t, disabledBot.DeleteAt, disabledBotAgain.DeleteAt)

		reenabledBot, err := th.Suite.UpdateBotActive(th.Context, bot.UserId, true)
		require.Nil(t, err)
		require.EqualValues(t, 0, reenabledBot.DeleteAt)

		// Re-enabling should be idempotent
		reenabledBotAgain, err := th.Suite.UpdateBotActive(th.Context, bot.UserId, true)
		require.Nil(t, err)
...
func TestPermanentDeleteBot(t *testing.T) {
	...
	require.Nil(t, th.Suite.PermanentDeleteBot(bot.UserId))

	_, err = th.Suite.GetBot(bot.UserId, false)
	require.NotNil(t, err)
	require.Equal(t, "store.sql_bot.get.missing.app_error", err.Id)
}
...
func TestDisableUserBots(t *testing.T) {
	...
	defer func() {
		for _, bot := range bots {
			th.Suite.PermanentDeleteBot(bot.UserId)
		}
	}()
	...
	u2bot1, err := th.Suite.CreateBot(th.Context, &model.Bot{
		Username:    "username_nodisable",
		Description: "a bot",
		OwnerId:     ownerId2,
	})
	require.Nil(t, err)
	defer th.Suite.PermanentDeleteBot(u2bot1.UserId)

	err = th.Suite.disableUserBots(th.Context, ownerId1)
	require.Nil(t, err)

	// Check all bots and corresponding users are disabled for creator 1
	for _, bot := range bots {
		retbot, err2 := th.Suite.GetBot(bot.UserId, true)
		require.Nil(t, err2)
		require.NotZero(t, retbot.DeleteAt, bot.Username)
	}

	// Check bots and corresponding user not disabled for creator 2
	bot, err := th.Suite.GetBot(u2bot1.UserId, true)
	require.Nil(t, err)
	require.Zero(t, bot.DeleteAt)

	user, err := th.Suite.GetUser(u2bot1.UserId)
	require.Nil(t, err)
	require.Zero(t, user.DeleteAt)

	// Bad id doesn't do anything or break horribly
	err = th.Suite.disableUserBots(th.Context, model.NewId())
	require.Nil(t, err)
}
...
func TestNotifySysadminsBotOwnerDisabled(t *testing.T) {
	...
	_, err := th.Suite.CreateUser(th.Context, &sysadmin1)
	require.Nil(t, err, "failed to create user")
	th.Suite.UpdateUserRoles(th.Context, sysadmin1.Id, model.SystemUserRoleId+" "+model.SystemAdminRoleId, false)
	...
	sysadmin2 := model.User{
		...
		Roles:    model.SystemAdminRoleId + " " + model.SystemUserRoleId}
	_, err = th.Suite.CreateUser(th.Context, &sysadmin2)
	require.Nil(t, err, "failed to create user")
	th.Suite.UpdateUserRoles(th.Context, sysadmin2.Id, model.SystemUserRoleId+" "+model.SystemAdminRoleId, false)

	// create user to be disabled
	user1, err := th.Suite.CreateUser(th.Context, &model.User{
	...
	user2, err := th.Suite.CreateUser(th.Context, &model.User{
	...
	channelSys1, appErr := th.Suite.channels.GetOrCreateDirectChannel(th.Context, sysadmin1.Id, sysadmin1.Id)
	require.Nil(t, appErr)
	channelSys2, appErr := th.Suite.channels.GetOrCreateDirectChannel(th.Context, sysadmin2.Id, sysadmin2.Id)
	require.Nil(t, appErr)

	// send notification for user without bots
	err = th.Suite.notifySysadminsBotOwnerDeactivated(th.Context, user2.Id)
	require.Nil(t, err)

	// get posts from sysadmin1 and sysadmin2 DM channels
	posts1, err := th.Suite.channels.GetPosts(channelSys1.Id, 0, 5)
	require.Nil(t, err)
	assert.Empty(t, posts1.Order)

	posts2, err := th.Suite.channels.GetPosts(channelSys2.Id, 0, 5)
	require.Nil(t, err)
	assert.Empty(t, posts2.Order)

	// send notification for user with bots
	err = th.Suite.notifySysadminsBotOwnerDeactivated(th.Context, user1.Id)
	require.Nil(t, err)
	...
	// print all bots
	th.Suite.platform.UpdateConfig(func(cfg *model.Config) { *cfg.ServiceSettings.DisableBotsWhenOwnerIsDeactivated = true })
	message := th.Suite.getDisableBotSysadminMessage(user1, userBots)
	...
	// print all bots
	th.Suite.platform.UpdateConfig(func(cfg *model.Config) { *cfg.ServiceSettings.DisableBotsWhenOwnerIsDeactivated = false })
	message = th.Suite.getDisableBotSysadminMessage(user1, userBots)
...
func TestConvertUserToBot(t *testing.T) {
	t.Run("invalid user", func(t *testing.T) {
		t.Run("invalid user id", func(t *testing.T) {
			...
			_, err := th.Suite.ConvertUserToBot(&model.User{
				Username: "username",
				Id:       "",
			})
			require.NotNil(t, err)
	...
	t.Run("valid user", func(t *testing.T) {
		...
		bot, err := th.Suite.ConvertUserToBot(&model.User{
			Username: "username",
			Id:       th.BasicUser.Id,
		})
		require.Nil(t, err)
...
func TestGetSystemBot(t *testing.T) {
	t.Run("An error should be returned if there are no sysadmins in the instance", func(t *testing.T) {
		...
		require.Nil(t, th.Suite.PermanentDeleteAllUsers(th.Context))

		_, err := th.Suite.GetSystemBot()
		require.NotNil(t, err)
	...
	t.Run("The bot should be created the first time it's retrieved", func(t *testing.T) {
		...
		_, err := th.Suite.GetUserByUsername(model.BotSystemBotUsername)
		require.NotNil(t, err)

		bot, err := th.Suite.GetSystemBot()
		require.Nil(t, err)
	...
	t.Run("The bot should be correctly retrieved if it exists already", func(t *testing.T) {
		...
		botUser, err := th.Suite.GetUserByUsername(model.BotSystemBotUsername)
		require.Nil(t, err)
