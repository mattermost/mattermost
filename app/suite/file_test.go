...
func TestDoUploadFile(t *testing.T) {
	...
	info1, err := th.Suite.DoUploadFile(th.Context, time.Date(2007, 2, 4, 1, 2, 3, 4, time.Local), teamID, channelID, userID, filename, data)
	require.Nil(t, err, "DoUploadFile should succeed with valid data")
	defer func() {
		th.Suite.platform.Store.FileInfo().PermanentDelete(info1.Id)
		th.Suite.RemoveFile(info1.Path)
	}()
	...
	info2, err := th.Suite.DoUploadFile(th.Context, time.Date(2007, 2, 4, 1, 2, 3, 4, time.Local), teamID, channelID, userID, filename, data)
	require.Nil(t, err, "DoUploadFile should succeed with valid data")
	defer func() {
		th.Suite.platform.Store.FileInfo().PermanentDelete(info2.Id)
		th.Suite.RemoveFile(info2.Path)
	}()
	...
	info3, err := th.Suite.DoUploadFile(th.Context, time.Date(2008, 3, 5, 1, 2, 3, 4, time.Local), teamID, channelID, userID, filename, data)
	require.Nil(t, err, "DoUploadFile should succeed with valid data")
	defer func() {
		th.Suite.platform.Store.FileInfo().PermanentDelete(info3.Id)
		th.Suite.RemoveFile(info3.Path)
	}()
	...
	info4, err := th.Suite.DoUploadFile(th.Context, time.Date(2009, 3, 5, 1, 2, 3, 4, time.Local), "../../"+teamID, "../../"+channelID, "../../"+userID, "../../"+filename, data)
	require.Nil(t, err, "DoUploadFile should succeed with valid data")
	defer func() {
		th.Suite.platform.Store.FileInfo().PermanentDelete(info4.Id)
		th.Suite.RemoveFile(info4.Path)
	}()
...
func TestUploadFile(t *testing.T) {
	...
	info1, err := th.Suite.UploadFile(th.Context, data, "wrong", filename)
	require.NotNil(t, err, "Wrong Channel ID.")
	require.Nil(t, info1, "Channel ID does not exist.")

	info1, err = th.Suite.UploadFile(th.Context, data, "", filename)
	require.Nil(t, err, "empty channel IDs should be valid")
	require.NotNil(t, info1)

	info1, err = th.Suite.UploadFile(th.Context, data, channelID, filename)
...
func TestGetInfoForFilename(t *testing.T) {
	...
	info := th.Suite.getInfoForFilename(post, teamID, post.ChannelId, post.UserId, "someid", "somefile.png")
...
func TestFindTeamIdForFilename(t *testing.T) {
	...
	teamID := th.Suite.findTeamIdForFilename(th.BasicPost, "someid", "somefile.png")
	assert.Equal(t, th.BasicTeam.Id, teamID)

	_, err := th.Suite.CreateTeamWithUser(th.Context, &model.Team{Email: th.BasicUser.Email, Name: "zz" + model.NewId(), DisplayName: "Joram's Test Team", Type: model.TeamOpen}, th.BasicUser.Id)
	require.Nil(t, err)
...
func TestMigrateFilenamesToFileInfos(t *testing.T) {
	...
	post := th.BasicPost
	infos := th.Suite.MigrateFilenamesToFileInfos(post)
	...
	_, err := th.Suite.WriteFile(file, fpath)
	require.Nil(t, err)
	rpost, err := th.Suite.channels.CreatePost(th.Context, &model.Post{UserId: th.BasicUser.Id, ChannelId: th.BasicChannel.Id, Filenames: []string{fmt.Sprintf("/%v/%v/%v/test.png", th.BasicChannel.Id, th.BasicUser.Id, fileID)}}, th.BasicChannel, false, true)
	require.Nil(t, err)

	infos = th.Suite.MigrateFilenamesToFileInfos(rpost)
...
func TestCreateZipFileAndAddFiles(t *testing.T) {
	...
	err := th.Suite.CreateZipFileAndAddFiles(&mockBackend, []model.FileData{}, "zip-file-name-to-heaven.zip", "directory-to-heaven")
...
func TestCopyFileInfos(t *testing.T) {
	...
	infoIds, err := th.Suite.CopyFileInfos(userID, []string{info1.Id})
	require.Nil(t, err)

	info2, err := th.Suite.GetFileInfo(infoIds[0])
	require.Nil(t, err)
	defer func() {
		th.Suite.platform.Store.FileInfo().PermanentDelete(info2.Id)
		th.Suite.RemoveFile(info2.Path)
...
func TestGenerateThumbnailImage(t *testing.T) {
	t.Run("test generating thumbnail image", func(t *testing.T) {
		...
		// when
		th.Suite.GenerateThumbnailImage(img, "jpg", thumbnailName)
...
func TestSearchFilesInTeamForUser(t *testing.T) {
	...
	setup := func(t *testing.T, enableElasticsearch bool) (*TestHelper, []*model.FileInfo) {
		...
		for i := 0; i < cap(fileInfos); i++ {
			fileInfo, err := th.Suite.platform.Store.FileInfo().Save(&model.FileInfo{
		...
		if enableElasticsearch {
			th.Suite.platform.SetLicense(model.NewTestLicense("elastic_search"))
	...
	t.Run("should return everything as first page of fileInfos from database", func(t *testing.T) {
		...
		results, err := th.Suite.SearchFilesInTeamForUser(th.Context, searchTerm, th.BasicUser.Id, th.BasicTeam.Id, false, false, 0, page, perPage, model.ModifierFiles)
	...
	t.Run("should return first page of fileInfos from ElasticSearch", func(t *testing.T) {
		...
		th.Suite.platform.SearchEngine.ElasticsearchEngine = es
		defer func() {
			th.Suite.platform.SearchEngine.ElasticsearchEngine = nil
		}()
...
func TestExtractContentFromFileInfo(t *testing.T) {
	...
	require.NoError(t, ss.ExtractContentFromFileInfo(fi))
}
...
func TestGetLastAccessibleFileTime(t *testing.T) {
	...
	r, err := th.Suite.GetLastAccessibleFileTime()
	require.Nil(t, err)
	assert.Equal(t, int64(0), r)

	th.Suite.platform.SetLicense(model.NewTestLicense("cloud"))

	mockStore := th.Suite.platform.Store.(*storemocks.Store)
	...
	_, err = th.Suite.GetLastAccessibleFileTime()
...
func TestComputeLastAccessibleFileTime(t *testing.T) {
	...
	err := th.Suite.ComputeLastAccessibleFileTime()
	require.NoError(t, err)
