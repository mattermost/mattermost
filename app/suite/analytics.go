...
func (s *SuiteService) GetAnalytics(name string, teamID string) (model.AnalyticsRows, *model.AppError) {
	skipIntensiveQueries := false
	var systemUserCount int64
	systemUserCount, err := s.platform.Store.User().Count(model.UserCountOptions{})
	if err != nil {
	...
	if systemUserCount > int64(*s.platform.Config().AnalyticsSettings.MaxUsersForStatistics) {
		mlog.Debug("More than limit users are on the system, intensive queries skipped", mlog.Int("limit", *s.platform.Config().AnalyticsSettings.MaxUsersForStatistics))
	...
	if name == "standard" {
		...
		g.Go(func() error {
			var err error
			if openChannelsCount, err = s.platform.Store.Channel().AnalyticsTypeCount(teamID, model.ChannelTypeOpen); err != nil {
		...
		g.Go(func() error {
			var err error
			if privateChannelsCount, err = s.platform.Store.Channel().AnalyticsTypeCount(teamID, model.ChannelTypePrivate); err != nil {
				return model.NewAppError("GetAnalytics", "app.channel.analytics_type_count.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			}
			return nil
		})
		...
		if teamID == "" {
			g.Go(func() error {
				var err error
				if inactiveUsersCount, err = s.platform.Store.User().AnalyticsGetInactiveUsersCount(); err != nil {
			...
			g.Go(func() error {
				var err error
				if usersCount, err = s.platform.Store.User().Count(model.UserCountOptions{TeamId: teamID}); err != nil {
		...
		if !skipIntensiveQueries {
			g.Go(func() error {
				var err error
				if postsCount, err = s.platform.Store.Post().AnalyticsPostCount(&model.PostCountOptions{TeamId: teamID}); err != nil {
		...
		g.Go(func() error {
			var err error
			if teamsCount, err = s.platform.Store.Team().AnalyticsTeamCount(nil); err != nil {
		...
		g.Go(func() error {
			var err error
			if dailyActiveUsersCount, err = s.platform.Store.User().AnalyticsActiveCount(DayMilliseconds, model.UserCountOptions{IncludeBotAccounts: false, IncludeDeleted: false}); err != nil {
		...
		g.Go(func() error {
			var err error
			if monthlyActiveUsersCount, err = s.platform.Store.User().AnalyticsActiveCount(MonthMilliseconds, model.UserCountOptions{IncludeBotAccounts: false, IncludeDeleted: false}); err != nil {
				return model.NewAppError("GetAnalytics", "app.user.analytics_daily_active_users.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
			}
			return nil
		})
		...
		if s.platform.Cluster() != nil && *s.platform.Config().ClusterSettings.Enable {
			stats, err2 := s.platform.Cluster().GetClusterStats()
			if err2 != nil {
				return nil, err2
			}

			totalSockets := s.platform.TotalWebsocketConnections()
			totalMasterDb := s.platform.Store.TotalMasterDbConnections()
			totalReadDb := s.platform.Store.TotalReadDbConnections()
			...
			rows[5].Value = float64(s.platform.TotalWebsocketConnections())
			rows[6].Value = float64(s.platform.Store.TotalMasterDbConnections())
			rows[7].Value = float64(s.platform.Store.TotalReadDbConnections())
		}
		...
		analyticsRows, nErr := s.platform.Store.Post().AnalyticsPostCountsByDay(&model.AnalyticsPostCountsOptions{
			TeamId:        teamID,
			BotsOnly:      true,
			YesterdayOnly: false,
		})
		...
		analyticsRows, nErr := s.platform.Store.Post().AnalyticsUserCountsWithPostsByDay(teamID)
		if nErr != nil {
		...
		g2.Go(func() error {
			var err error
			if incomingWebhookCount, err = s.platform.Store.Webhook().AnalyticsIncomingCount(teamID); err != nil {
		...
		g2.Go(func() error {
			var err error
			if outgoingWebhookCount, err = s.platform.Store.Webhook().AnalyticsOutgoingCount(teamID); err != nil {
		...
		g2.Go(func() error {
			var err error
			if commandsCount, err = s.platform.Store.Command().AnalyticsCommandCount(teamID); err != nil {
		...
		g2.Go(func() error {
			var err error
			if sessionsCount, err = s.platform.Store.Session().AnalyticsSessionCount(); err != nil {
		...
		if !skipIntensiveQueries {
			g2.Go(func() error {
				var err error
				if filesCount, err = s.platform.Store.Post().AnalyticsPostCount(&model.PostCountOptions{TeamId: teamID, MustHaveFile: true}); err != nil {
					return model.NewAppError("GetAnalytics", "app.post.analytics_posts_count.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
				}
				return nil
			})
			...
			g2.Go(func() error {
				var err error
				if hashtagsCount, err = s.platform.Store.Post().AnalyticsPostCount(&model.PostCountOptions{TeamId: teamID, MustHaveHashtag: true}); err != nil {
					return model.NewAppError("GetAnalytics", "app.post.analytics_posts_count.app_error", nil, "", http.StatusInternalServerError).Wrap(err)
				}
				return nil
			})
...
func (s *SuiteService) GetRecentlyActiveUsersForTeam(teamID string) (map[string]*model.User, *model.AppError) {
	users, err := s.platform.Store.User().GetRecentlyActiveUsersForTeam(teamID, 0, 100, nil)
	if err != nil {
...
func (s *SuiteService) GetRecentlyActiveUsersForTeamPage(teamID string, page, perPage int, asAdmin bool, viewRestrictions *model.ViewUsersRestrictions) ([]*model.User, *model.AppError) {
	users, err := s.platform.Store.User().GetRecentlyActiveUsersForTeam(teamID, page*perPage, perPage, viewRestrictions)
	if err != nil {
...
func (s *SuiteService) GetNewUsersForTeamPage(teamID string, page, perPage int, asAdmin bool, viewRestrictions *model.ViewUsersRestrictions) ([]*model.User, *model.AppError) {
	users, err := s.platform.Store.User().GetNewUsersForTeam(teamID, page*perPage, perPage, viewRestrictions)
	if err != nil {
