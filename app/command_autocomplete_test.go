...
func TestParseFixedListArgument(t *testing.T) {
	fixedArgs := model.NewFixedListArgument()
	fixedArgs.AddArgument("on", "help")

	argument := &model.Argument{
		Name:     "", //positional
		HelpText: "some_help",
		Type:     model.FixedListArgumentType,
		Data:     fixedArgs,
	}
	found, _, suggestions := parseFixedListArgument(argument, "")
	assert.True(t, found)
	assert.Equal(t, []model.Suggestion{{Hint: "on", Description: "help"}}, suggestions)

	found, _, suggestions = parseFixedListArgument(argument, "o")
	assert.True(t, found)
	assert.Equal(t, []model.Suggestion{{Hint: "on", Description: "help"}}, suggestions)

	found, changedInput, _ := parseFixedListArgument(argument, "on ")
	assert.False(t, found)
	assert.Equal(t, "", changedInput)

	found, changedInput, _ = parseFixedListArgument(argument, "on some")
	assert.False(t, found)
	...
	found, _, suggestions = parseFixedListArgument(argument, "of")
	assert.True(t, found)
	assert.Equal(t, []model.Suggestion{{Hint: "off", Description: "help"}}, suggestions)

	found, _, suggestions = parseFixedListArgument(argument, "o some")
	assert.True(t, found)
	assert.Len(t, suggestions, 0)

	found, changedInput, _ = parseFixedListArgument(argument, "off some")
	assert.False(t, found)
	assert.Equal(t, "some", changedInput)

	fixedArgs.AddArgument("onon", "help")

	found, _, suggestions = parseFixedListArgument(argument, "on")
	assert.True(t, found)
	assert.Equal(t, []model.Suggestion{{Hint: "on", Description: "help"}, {Hint: "onon", Description: "help"}}, suggestions)

	found, _, suggestions = parseFixedListArgument(argument, "ono")
	assert.True(t, found)
	...
	found, changedInput, _ = parseFixedListArgument(argument, "onon some")
	assert.False(t, found)
	assert.Equal(t, "some", changedInput)
}
