...
func TestSuggestions(t *testing.T) {
	...
	emptyCmdArgs.Command = "bla /jir"
	commands := []*model.Command{{
		Trigger:          "jira",
		AutocompleteData: jira,
	}}
	suggestions = th.App.GetSuggestions(emptyCmdArgs, commands, model.SYSTEM_ADMIN_ROLE_ID)
	assert.Len(t, suggestions, 0)

	jira.CanAutocompleteInTheMiddle = true
	suggestions = th.App.GetSuggestions(emptyCmdArgs, commands, model.SYSTEM_ADMIN_ROLE_ID)
	assert.Len(t, suggestions, 1)
	assert.Equal(t, "bla /jira", suggestions[0].Complete)

	jira.Replace = "changed text"
	suggestions = th.App.GetSuggestions(emptyCmdArgs, commands, model.SYSTEM_ADMIN_ROLE_ID)
	assert.Len(t, suggestions, 1)
	assert.Equal(t, "bla changed text", suggestions[0].Complete)
}
