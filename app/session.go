...
func (a *App) UpdateSessionsIsGuest(userId string, isGuest bool) {
	...
	if err != nil {
		mlog.Error("Unable to get user sessions: userId=%s err=%s", mlog.String("user_id", userId), mlog.Err(err))
	}
	...
	for _, session := range sessions {
		...
		if err != nil {
			mlog.Error("Unable to update isGuest session: %s", mlog.Err(err))
			continue
		}
		a.AddSessionToCache(session)
	}
...
func (a *App) RevokeSessionsForDeviceId(userId string, deviceId string, currentSessionId string) *model.AppError {
	...
	for _, session := range sessions {
		if session.DeviceId == deviceId && session.Id != currentSessionId {
			mlog.Debug("Revoking sessionId=%v for userId=%v re-login with same device Id", mlog.String("session_id", session.Id), mlog.String("user_id", userId))
...
func (a *App) UpdateLastActivityAtIfNeeded(session model.Session) {
	...
	if err := a.Srv.Store.Session().UpdateLastActivityAt(session.Id, now); err != nil {
		mlog.Error("Failed to update LastActivityAt for user_id=%v and session_id=%v, err=%v", mlog.String("user_id", session.UserId), mlog.String("session_id", session.Id), mlog.Err(err))
	}
