...
func (w *licenseWrapper) RequestTrialLicense(requesterID string, users int, termsAccepted bool, receiveEmailsAccepted bool) *model.AppError {
	if *w.srv.platformService.Config().ExperimentalSettings.RestrictSystemAdmin {
	...
	if *w.srv.platformService.Config().ServiceSettings.SiteURL == "" {
	...
	trialLicenseRequest := &model.TrialLicenseRequest{
		...
		SiteName:              *w.srv.platformService.Config().TeamSettings.SiteName,
		SiteURL:               *w.srv.platformService.Config().ServiceSettings.SiteURL,
		Users:                 users,
...
func (s *Server) LoadLicense() {
	...
	if !model.IsValidId(licenseId) {
		...
		license, licenseBytes := utils.GetAndValidateLicenseFileFromDisk(*s.platformService.Config().ServiceSettings.LicenseFileLocation)
...
func (s *Server) SetLicense(license *model.License) bool {
	...
	if license != nil {
		...
		if s.platformService != nil {
			s.platformService.SetLicense(license)
		}
