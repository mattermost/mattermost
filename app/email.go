...
func (es *EmailService) SendAtUserLimitWarningEmail(email string, locale string) (bool, *model.AppError) {
	return true, nil
}

func (es *EmailService) SendOverUserLimitWarningEmail(email string, locale string) (bool, *model.AppError) {
	T := utils.GetUserTranslations(locale)

	subject := T("api.templates.over_limit_subject")

	bodyPage := es.newEmailTemplate("reached_user_limit_body", locale)
	bodyPage.Props["SiteURL"] = "http://localhost:9005"
