...
func (a *App) NotifyAndSetWarnMetricAck(warnMetricId string, sender *model.User, forceAck bool) *model.AppError {
	...
	mlog.Debug("Disable the monitoring of warn metric", mlog.String("metric", warnMetricId))
...
func (s *Server) sendMail(to, subject, htmlBody string, ccMail string) *model.AppError {
	license := s.License()
	return mailservice.SendMailUsingConfig(to, subject, htmlBody, s.Config(), license != nil && *license.Features.Compliance, ccMail)
}
