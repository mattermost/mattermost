	...
	if *a.Config().ServiceSettings.ThreadAutoFollow && post.RootId != "" {
		...
		for id := range threadParticipants {
			...
			go func(userID string) {
				...
				_, err := a.Srv().Store.Thread().MaintainMembership(userID, post.RootId, true, incrementMentions, *a.Config().ServiceSettings.ThreadAutoFollow, userID == post.UserId, true)
				if err != nil {
