...
func TestGetTopChannelsForTeamSince(t *testing.T) {
	...
	t.Run("get-top-channels-for-team-since", func(t *testing.T) {
		topChannels, err := th.App.GetTopChannelsForTeamSince(th.BasicChannel.TeamId, th.BasicUser.Id, &model.InsightsOpts{TimeRange: timeRange, Page: 0, PerPage: 5})
		require.Nil(t, err)

		for i, channel := range topChannels {
			assert.Equal(t, expectedTopChannels[i].ID, channel.ID)
			assert.Equal(t, expectedTopChannels[i].Score, channel.Score)
		}
	})
}
