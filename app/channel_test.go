...
func TestAddChannelMemberNoUserRequestor(t *testing.T) {
	...
	userRequestorId := ""
	postRootId := ""
	if _, err := th.App.AddChannelMember(user.Id, channel, userRequestorId, postRootId, "", true); err != nil {
		t.Fatal("Failed to add user to channel. Error: " + err.Message)
	}
...
func TestGetChannelMembersTimezones(t *testing.T) {
	...
	userRequestorId := ""
	postRootId := ""
	if _, err := th.App.AddChannelMember(th.BasicUser2.Id, th.BasicChannel, userRequestorId, postRootId, "", true); err != nil {
		t.Fatal("Failed to add user to channel. Error: " + err.Message)
	}
