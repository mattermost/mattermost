...
func (a *App) installPluginLocally(pluginFile io.ReadSeeker, signatures []io.ReadSeeker, replace bool) (*model.Manifest, *model.AppError) {
	...
	// verify signature
	if signatures != nil {
		if appErr := a.VerifyPluginWithSignatures(pluginFile, signatures); appErr != nil {
			return nil, appErr
		}
	}
...
func extractPlugin(pluginFile io.ReadSeeker, extractDir string) (*model.Manifest, string, error) {
	pluginFile.Seek(0, 0)
	if err := utils.ExtractTarGz(pluginFile, extractDir); err != nil {
		return nil, extractDir, errors.New("Encountered error extracting plugin")
	}

	dir, err := ioutil.ReadDir(extractDir)
	if err != nil {
		return nil, extractDir, errors.Errorf("Can't read directory %s", extractDir)
	}
	...
	if err != nil {
		return nil, extractDir, errors.Wrapf(err, "Can't find manifest in directory %s", extractDir)
	}

	if !plugin.IsValidId(manifest.Id) {
		return nil, extractDir, errors.Errorf("Plugin Id must be at least %d characters, at most %d characters and match %d.", plugin.MinIdLength, plugin.MaxIdLength, http.StatusBadRequest)
	}
	return manifest, extractDir, nil
}

func (a *App) installExtractedPlugin(manifest *model.Manifest, fromPluginDir string, replace bool) (*model.Manifest, *model.AppError) {
	pluginsEnvironment := a.GetPluginsEnvironment()
	if pluginsEnvironment == nil {
