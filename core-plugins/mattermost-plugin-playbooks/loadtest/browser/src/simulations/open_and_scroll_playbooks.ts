...
export async function openAndScrollPlaybooks(
  page: Page,
  log: Logger,
  {scrollDistance = 500}: ExtraArgs,
): Promise<void> {
  log.info("run--openAndScrollPlaybooks");

  try {
    // # Click on view all playbooks left sidebar button
    const viewAllPlaybooksLHSButton = page.getByTestId("playbooksLHSButton");
    await viewAllPlaybooksLHSButton.click();

    // # Wait for search input to be visible in the all playbooks page
    const searchInput = page.getByPlaceholder("Search for a playbook");
    await searchInput.waitFor({state: "visible"});

    // # Scroll gradually until reaching the bottom
    const scrollContainer = page.getByTestId("playbook-list-scroll-container");
    ...
    log.info("pass--openAndScrollPlaybooks");
  } catch (error) {
    throw {error, testId: "openAndScrollPlaybooks"};
  }
}
