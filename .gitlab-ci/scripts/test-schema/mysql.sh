...
docker exec ${COMPOSE_PROJECT_NAME}_mysql_1 mysql -uroot -pmostest -e "CREATE DATABASE migrated; CREATE DATABASE latest; GRANT ALL PRIVILEGES ON migrated.* TO mmuser; GRANT ALL PRIVILEGES ON latest.* TO mmuser; "
echo "Importing mysql dump from version 6.0"
docker exec ${COMPOSE_PROJECT_NAME}_mysql_1 mysql -D migrated -uroot -pmostest < "$CI_PROJECT_DIR"/scripts/mattermost-mysql-6.0.sql
...
docker exec ${COMPOSE_PROJECT_NAME}_mysql_1 mysqldump --skip-opt --no-data --compact -u root -pmostest migrated > migrated.sql
docker exec ${COMPOSE_PROJECT_NAME}_mysql_1 mysqldump --skip-opt --no-data --compact -u root -pmostest latest > latest.sql
echo "Removing databases created for db comparison"
docker exec ${COMPOSE_PROJECT_NAME}_mysql_1 mysql -uroot -pmostest -e 'DROP DATABASE migrated; DROP DATABASE latest'
