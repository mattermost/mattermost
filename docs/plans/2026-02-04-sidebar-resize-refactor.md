# Sidebar Resize Refactor Implementation Plan
...
**Tech Stack:** React hooks, Redux (useGlobalState for persistence), TypeScript, SCSS
...
```
┌─────────────────────────────────────────────────────────────┐
│  useResizable(name, defaultWidth, minWidth, maxWidth)       │
│  - Returns: { width, containerProps, dividerProps, reset }  │
│  - Persists to Redux via useGlobalState                     │
│  - Sets inline width directly on container                  │
│  - No CSS variables, no global styles                       │
└─────────────────────────────────────────────────────────────┘
           │
           ▼
┌─────────────────────────────────────────────────────────────┐
│  ResizableSidebar component                                  │
│  - Wraps any sidebar (LHS or RHS)                           │
│  - Handles resize divider rendering                         │
│  - Applies width via inline style                           │
└─────────────────────────────────────────────────────────────┘
```
...
```typescript
// webapp/channels/src/components/resizable_sidebar/use_resizable.ts
...
## Task 2: Create ResizeDivider Styled Component
...
```typescript
// webapp/channels/src/components/resizable_sidebar/resize_divider.tsx
...
```typescript
// webapp/channels/src/components/resizable_sidebar/resizable_sidebar.tsx
import React from 'react';
import styled from 'styled-components';
...
**Step 1: Add LHS min/max constants**

```typescript
// webapp/channels/src/components/resizable_sidebar/constants.ts
...
export const MAX_LHS_WIDTH = 500;
...
git commit -m "feat: update sidebar width constraints for free resizing"
```
...
```typescript
// webapp/channels/src/components/resizable_sidebar/resizable_lhs/index.tsx
...
```typescript
// webapp/channels/src/components/resizable_sidebar/resizable_rhs/index.tsx
import type {HTMLAttributes} from 'react';
import React from 'react';
import {useSelector} from 'react-redux';
...
function ResizableRhs({
    ...
    // When expanded, don't apply custom width
...
#SidebarContainer {
    ...
    // Desktop: resizable (inline styles from JS take precedence)
...
Replace lines 17-46 with:

```scss
.sidebar--right {
    // Base width controlled by inline styles from JS
    ...
    &:not(.expanded) {
        ...
        // No max-width constraints - JS controls this
    }
...
Also remove the width-holder CSS variable usage (lines 5-15):
...
**Files:**
- Delete: `webapp/channels/src/components/resizable_sidebar/resizable_divider.tsx`
- Delete: `webapp/channels/src/components/resizable_sidebar/utils.ts`

**Step 1: Remove old divider component**

The old `resizable_divider.tsx` is no longer needed - we have `resize_divider.tsx` and `use_resizable.ts`.

```bash
git rm webapp/channels/src/components/resizable_sidebar/resizable_divider.tsx
```

**Step 2: Remove old utils (snap logic no longer used)**
...
**Files:**
- Create or modify: `webapp/channels/src/components/resizable_sidebar/index.ts`

**Step 1: Create clean exports**

```typescript
// webapp/channels/src/components/resizable_sidebar/index.ts
...
|------|-------------|---------------|
| 1 | Create useResizable hook | +1 new file |
| 2 | Create ResizeDivider component | +1 new file |
| 3 | Create ResizableSidebar wrapper | +1 new file |
| 4 | Update constants | 1 modified |
| 5 | Update ResizableLhs | 1 modified |
| 6 | Update ResizableRhs | 1 modified |
| 7 | Remove LHS CSS constraints | 1 modified |
| 8 | Remove RHS CSS constraints | 1 modified |
| 9 | Delete old files | -2 files |
| 10 | Update exports | +1 new file |
| 11 | Test locally | verification |

**Total: +4 new files, 4 modified, -2 deleted**
