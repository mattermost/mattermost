...
// Ejected from @docusaurus/theme-search-algolia@3.10.1 (`SearchPage`).
//
// Upstream never forwards `themeConfig.algolia.searchParameters` to this
// component — it destructures only {appId, apiKey, indexName,
// contextualSearch} and builds the query helper from a hardcoded parameter
// set. The DocSearch modal does honour `searchParameters`, so the two search
// surfaces returned differently ordered results for the same query. There is
// no config option to fix this, and Algolia Rules aren't available on our
// plan, so swizzling is the only route.
//
// The ONLY delta from upstream is the three edits marked "Mattermost delta"
// below: `searchParameters` added to the `useAlgoliaThemeConfig()`
// destructure, `...searchParameters` spread last into the
// `algoliaSearchHelper` options, and the `PlainSearchParameters` type import
// the spread's cast needs. Everything else is verbatim upstream, including
// the swizzle-stripped MIT license header. Filter values and scores are
// deliberately not duplicated here — docusaurus.config.ts stays the single
// source of truth so the modal and this page cannot drift apart again.
//
// On a Docusaurus upgrade, re-eject from the new version and re-apply those
// three edits rather than diffing this whole file:
//   npm run swizzle @docusaurus/theme-search-algolia SearchPage -- \
//     --eject --danger --typescript
// (the theme is only registered when ALGOLIA_APP_ID and
// ALGOLIA_SEARCH_API_KEY are set, so export placeholders first). Drop this
// file entirely once upstream passes `searchParameters` through.
...
function SearchPageContent(): ReactNode {
  ...
  const {
    // Mattermost delta: `searchParameters` added (upstream omits it).
    algolia: {appId, apiKey, indexName, contextualSearch, searchParameters},
  } = useAlgoliaThemeConfig();
  ...
  const algoliaHelper = algoliaSearchHelper(algoliaClient, indexName, {
    ...
    advancedSyntax: true,
    disjunctiveFacets,
    // Mattermost delta: spread last so theme config wins on conflicts. The
    // cast bridges DocSearch's `SearchOptions` (where `optionalFilters` also
    // accepts a bare string) and the helper's narrower array-only type.
    ...(searchParameters as PlainSearchParameters),
  });
