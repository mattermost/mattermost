...
Not started. Written as a handoff plan for an implementing agent — no code changes have been made yet as part of this plan.
...
- [ ] All three pages render full generated content locally (`npm run start` and `npm run build`
      both succeed and produce the expected sections) with no leftover placeholder `<Note>`.
- [ ] Server reference: API/Hooks/Helpers sections present, grouped correctly by category/tag,
      with working in-page anchor links from the TOC to each method's full docs.
- [ ] Web app reference: full method list from `registry.ts` with signatures and JSDoc comments
      rendered, in both light and dark mode without contrast issues (reuse the dark-mode
      code-block fixes already in `docs/site/src/css/custom.css` — `.theme-code-block` /
      `.prism-code` styling, and check `!important` is needed for any inline Prism token colors).
- [ ] Manifest reference: nested schema renders correctly (arrays/dicts/objects), TOC links work.
- [ ] `hello-world.md`'s inline example renders the actual Go source instead of the current raw
      escaped TODO comment.
- [ ] New generator scripts run correctly in CI (check `.github/workflows/docs-preview-fork.yml`
      and any other docs build/deploy workflow for where `npm run build`/`prebuild` is invoked, and
      confirm the Go toolchain is available in that job if Go-based generators are added).
- [ ] No regression to existing prebuild steps (`stage:agents-docs`, sidebars, OpenAPI docs).
