...
Search by path, method, or name — `post channel`, `delete reaction`, `/users/{user_id}/image`. The same box sits at the top of the sidebar on every API page (press <kbd>/</kbd> to jump to it).
