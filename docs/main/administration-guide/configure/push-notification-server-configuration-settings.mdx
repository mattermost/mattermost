...
When a self-hosted server's license includes HPNS, Mattermost automatically switches the **Push Notification Server** from the test endpoint (`https://push-test.mattermost.com`) to the hosted endpoint (`https://global.push.mattermost.com`), and switches it back to the test endpoint when the license no longer includes HPNS. Only these two exact values are ever changed automatically; custom or regional endpoints, and values managed through the `MM_EMAILSETTINGS_PUSHNOTIFICATIONSERVER` environment variable, are never modified.
