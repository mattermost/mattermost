...
Post identity and display-override props (`from_webhook`, `from_bot`, `override_username`, `override_icon_url`, and related keys) are always stripped on create/update for non-integration authors and re-applied only under verified integration authority. That behavior is independent of hardened mode.
