...
Post identity and display-override props (`from_webhook`, `from_bot`, `override_username`, `override_icon_url`, and related keys) are always stripped from caller-supplied create/update payloads for non-integration authors and re-applied only under verified integration authority. On update, stripping removes values the caller sent; existing identity props already stored on the post are preserved so a normal edit cannot wipe or replace a bot or webhook identity. That behavior is independent of hardened mode.
