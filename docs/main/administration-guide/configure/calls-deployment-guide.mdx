...
- [ ] You can provision at least one dedicated Linux server or VM if you plan to use the RTCD service.
  ...
  _See [Infrastructure Decisions](#12-infrastructure-decisions) (Step 1.2) if you're unsure if you need RTCD._
...
- [ ] You are prepared to deploy a TURN server if your users cannot reliably reach the media service on UDP or TCP `8443`.
  ...
  _See [Networking Decisions](#13-networking-decisions) (Step 1.3) if you're unsure if you need a TURN server._
...
- [ ] You know how end users will connect to Calls (From private networks, VPN, or from the public internet)
  ...
  _This impacts your [networking decisions](#13-networking-decisions) (Step 1.3)_
...
- [ ] You know how to open a [request](https://support.mattermost.com) with Mattermost support if you encounter issues.
  ...
  _Please include the exact **step number** (e.g. 2A.2.1) that failed, along with your [Mattermost support packet](/../../administration-guide/manage/admin/generating-support-packet) and [Calls logs](calls-logging.mdx)._
...
You will need to provision a new server for RTCD. Use the [performance baselines](calls-metrics-monitoring.mdx#performance-baselines) for benchmark examples of hardware sizing. The RTCD service supports [horizontal scaling](calls-rtcd-setup.mdx#horizontal-scaling), but we recommend starting with one server and then scaling out if your expected workload requires it.
...
<Tabs>
<TabItem value="path-a-integrated" label="Path A: Integrated">
...
<TabItem value="path-b-install-and-configure-rtcd" label="Path B: Install and Configure RTCD">
...
Follow the [RTCD Setup and Configuration](calls-rtcd-setup.mdx) guide for the actual installation. The guide covers:
...
If you are deploying on Kubernetes, also use the [Calls Deployment on Kubernetes](calls-kubernetes.mdx) guide for cluster-specific installation and Helm-based configuration.
...
Calls monitoring uses Prometheus (a tool that collects metrics from your servers) and Grafana (a dashboard that visualizes those metrics). If you don't have these set up yet, see [Mattermost monitoring setup](../../administration-guide/scale/deploy-prometheus-grafana-for-performance-monitoring.mdx) before continuing. If you already have Prometheus and Grafana running, add the following scrape targets and import the dashboard:
...
See [Calls Metrics and Monitoring](calls-metrics-monitoring.mdx) for full configuration details.
...
Follow the [Calls Offloader Setup and Configuration](calls-offloader-setup.mdx) guide for installation. The guide covers:
