...
Mattermost provides a reliable migration path from Slack, enabling you to bring your organization’s collaboration history into a secure, self-hosted Mattermost environment. The migration process supports full workspaces, including users, channels and message history, direct messages, and threads so your teams can continue working without losing valuable context. See [What migrates / What doesn’t](#what-migrates--what-doesnt) for a coverage reference before you begin.
...
## What migrates / What doesn’t \{#what-migrates--what-doesnt}

The Slack migration path focuses on core collaboration data: posts, threads, reactions, file attachments, users, and channels. Bot users and bot messages also migrate when you configure `--bot-owner`. Slash commands, workflows, webhooks, and other integrations do not migrate and must be rebuilt in Mattermost. Because Slack and Mattermost differ architecturally, some content types are best-effort or unsupported.
...
| --- | --- | --- |
| Posts and threads | Yes | Most message types and thread replies migrate; some unsupported Slack subtypes are skipped, and some thread relationships may not preserve given differences in how Mattermost and Slack threading works |
| File attachments | Yes | Included in the Slack export when not skipped with `--skip-attachments`. Posts longer than Mattermost’s message limit are split into thread replies; more than five attachments on a post overflow into additional replies |
| Reactions | Yes | Imported by emoji name. Custom emoji images are not imported — recreate custom emojis in Mattermost with matching names so reactions resolve |
| Users | Yes | Created automatically. By default (`--guest-handling=guest`, or omit the flag), Slack guests map to Mattermost guests; this requires Guest Accounts licensed and enabled. Guests with no public or private channel membership are skipped, including their memberships, authored posts, and reactions — use `--guest-handling=user` to import those guests as regular users with full permissions instead |
...
| Public channels | Yes | Archived Slack channels import as archived when timestamps are available |
| Private channels | Yes | Archived Slack channels import as archived when timestamps are available |
| Multi-party DMs | Yes (converted) | Become Group Messages. Oversized MPIMs (above Mattermost’s group-channel member limit) are converted to private channels |
| 1:1 DMs | Yes | |
| Bots | Yes | Bot users and bot messages migrate when you pass `--bot-owner <MATTERMOST_USERNAME>` to `mmetl transform slack` |
| Channel mentions (embedded) | Best effort | Slack `<#CHANNEL_ID>` mentions are converted to Mattermost `~channel` mentions when the channel is in the export |
| Links to specific posts | Preserved as-is | Original Slack URLs are preserved; they still work if Slack remains accessible |
| External links | Preserved as-is | |
| Slash commands | No | Must be reconfigured in Mattermost |
| Workflow builders and other integrations | No | Must be rebuilt using [Mattermost integrations](/integrations-guide/integrations-guide-index) |
| Slack Connect | No | No Connect-specific handling; external org membership is not preserved. Prefer retaining Connect channels in Slack |
| Custom emoji images | No | Recreate in Mattermost; see Reactions above |
| Emoji skin tone modifiers | No | Stripped during transform (for example `+1::skin-tone-3` becomes `+1`) |
| Starred conversations | No | |
| User groups | No | Recreate using [Custom Groups](/end-user-guide/collaborate/organize-using-custom-user-groups) |
| Canvases | No | |
| Presence, profile pictures, custom profile fields | No | Name, email, and title/position migrate; profile images and presence do not. Users update their profiles in Mattermost |
...
See [What migrates / What doesn’t](#what-migrates--what-doesnt) for the full coverage reference. In short, core collaboration data (messages, files, channels, users, and bots with `--bot-owner <MATTERMOST_USERNAME>`) can be migrated, subject to the coverage reference above, while slash commands, workflows, webhooks, and other integrations do not migrate and must be rebuilt using the [integration and automation](/integrations-guide/integrations-guide-index) options in Mattermost.
