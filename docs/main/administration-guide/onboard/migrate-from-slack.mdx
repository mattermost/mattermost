...
The Slack migration path focuses on core collaboration data: posts, threads, reactions, file attachments, users, and channels. Slash commands, workflows, webhooks, and similar integrations do not migrate and must be rebuilt in Mattermost. Because Slack and Mattermost differ architecturally, some content types are best-effort or unsupported.
...
| Users | Yes | Created automatically; Slack guests map to Mattermost guests by default (`mmetl transform slack --guest-handling`). Correct guest behavior requires Guest Accounts licensed and enabled |
...
| 1:1 DMs | Yes | |
| Bots | Yes | Bot users and bot messages migrate when you pass `--bot-owner` to `mmetl transform slack` |
...
See [What migrates / What doesn’t](#what-migrates--what-doesnt) for the full coverage reference. In short, core collaboration data (messages, files, channels, users, and bots with `--bot-owner`) is preserved, while slash commands, workflows, webhooks, and similar integrations do not migrate and must be rebuilt using the [integration and automation](/integrations-guide/integrations-guide-index) options in Mattermost.
