...
If a user has logged into Mattermost through AD/LDAP, they're managed by your directory server. AD/LDAP users can't be deactivated manually from **System Console \> User Management \> Users** - the **Deactivate** action is rejected for AD/LDAP-managed accounts. Instead, deactivate them in one of two ways:
