...
The TCP socket targets can be configured with an IP address or domain name, port, and optional TLS certificate. TCP socket target support requires Mattermost Enterprise. You can [download a sample JSON file](https://github.com/mattermost/docs/blob/master/source/samples/sample-logger-config.json) of the configuration to use as a starting point.
