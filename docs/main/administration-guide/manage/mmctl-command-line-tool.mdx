...
<Tabs>
<TabItem value="release-package" label="Release package">
...
<TabItem value="local-git-clone" label="Local git clone">
...
Only available in Mattermost Cloud, and requires `FileSettings` > `EnableCloudExportDirectDownload` (environment variable `MM_FILESETTINGS_ENABLECLOUDEXPORTDIRECTDOWNLOAD`) and `FileSettings` > `DedicatedExportStore` to be set to `true`, with export storage that supports presigned links (Amazon S3 or Azure Blob Storage).
