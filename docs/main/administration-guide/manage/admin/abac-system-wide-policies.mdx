...
Permission policies are managed in **System Console \> System Attributes \> Permission Policies**, and can be used to restrict the following actions based on user attributes:
...
- **Channel read access**: From Mattermost v12.1, prevent users who don't match the defined attribute rules from reading a channel and its content. See [Channel read and write access actions](/administration-guide/manage/admin/abac-channel-read-write-access).
- **Channel write access**: From Mattermost v12.1, prevent users who don't match the defined attribute rules from posting in a channel or changing its content. See [Channel read and write access actions](/administration-guide/manage/admin/abac-channel-read-write-access).
...
Unlike membership policies, which are applied when channel membership is synchronized, permission policies are evaluated on every request. Rules can also reference session attributes such as IP address and user agent, so the same user can be allowed an action from one session and denied it from another.
