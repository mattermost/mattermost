...
- **Permission policies** (managed by System Admins): Attribute-based restrictions on user actions such as file upload, file download, channel read access, and channel write access. Unlike membership policies, these are evaluated on every request and can reference session attributes. See [Permission policies](/administration-guide/manage/admin/abac-system-wide-policies#permission-policies) and [Channel read and write access actions](/administration-guide/manage/admin/abac-channel-read-write-access).
...
- From Mattermost v12.1, restrict [channel read access and channel write access](/administration-guide/manage/admin/abac-channel-read-write-access) with permission policies.
...
- Scope [channel read and write access rules](/administration-guide/manage/admin/abac-channel-read-write-access#channel-settings-channel-scoped-permission-rules) to a single channel, per channel role, from the **Permissions Policy** tab in Channel Settings.
