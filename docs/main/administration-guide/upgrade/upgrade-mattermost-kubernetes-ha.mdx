...
No manual action is required. The Mattermost Operator runs a pre-flight job with the new image to apply any pending database schema migrations before rolling out the upgrade; if the migration fails, the rollout is blocked and no changes are applied. The Mattermost server binary also applies any pending migrations automatically on startup, so the schema stays current even if the pre-flight job is disabled (`spec.updateJob.disabled: true`).
