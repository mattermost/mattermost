...
From Mattermost v12.0, the Legal Hold plugin is pre-packaged with the Mattermost Server, so there's no need to download the plugin binary. Because the plugin ships with the server, it's also available from the Marketplace on servers without internet access.
...
3.  Search for or scroll to **Legal Hold Plugin**, and select **Install**.
4.  Go to **System Console \> Plugins \> Plugin Management**, and in the **Installed Plugins** section, scroll to **Legal Hold Plugin** and select **Enable**.

<Note>

If your deployment is on a release prior to v12.0, download the latest version of the [plugin binary release](https://github.com/mattermost/mattermost-plugin-legal-hold/releases/), compatible with Mattermost v8.0.1 and later, and upload it via **System Console \> Plugins \> Plugin Management \> Upload Plugin**. If you are using a version of Mattermost earlier than v8.0.1, [follow our documentation](/administration-guide/upgrade/upgrading-mattermost-server) to upgrade first.
