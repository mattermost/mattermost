...
- **Maintain control over mission-critical data**: Enable safe delivery of notifications via [ID-only push notifications](/administration-guide/configure/environment-configuration-settings#id-only-push-notifications) that prevent exposure of sensitive content to third-party systems like Apple or Google.
...
- **Deliver rapid alerts with low bandwidth impact** using [ID-only push notifications](/administration-guide/configure/environment-configuration-settings#id-only-push-notifications), ideal for DDIL (disconnected, intermittent, low-bandwidth) conditions.
