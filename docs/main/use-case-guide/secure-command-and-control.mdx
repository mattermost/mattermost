...
- **Maintain command resilience** using [high availability cluster-based deployment](/deployment-guide/server/high-availability-cluster-based-deployment) and [horizontal scalability](/deployment-guide/scale/scaling-for-enterprise) to support operational continuity at scale.
...
- **Unify mission stakeholders on a common-use platform** that supports [hybrid deployments](/deployment-guide/server/deploy-server#deployment-options) across private cloud, edge environments, and [air-gapped infrastructure](/deployment-guide/deployment-scenarios/air-gapped-deployment).
