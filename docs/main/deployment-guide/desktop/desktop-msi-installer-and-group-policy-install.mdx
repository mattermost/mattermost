...
3.  Navigate to the release page for [version v6.3.1](https://github.com/mattermost/desktop/releases/tag/v6.3.1) and download the appropriate installer for your version of Windows (32-bit vs. 64-bit).

4.  Download the [source.zip](https://github.com/mattermost/desktop/archive/v6.3.1.zip) file as well to extract group policy files.
...
Users upgrading from v5.9 to v6.0.4 with per-user installations must manually uninstall the old version before installing v6.1.0 to avoid duplicate installs. See [Desktop troubleshooting](./desktop-troubleshooting#upgrade-to-v610-fails-or-installs-duplicate-version-windows-msi) for detailed upgrade instructions.
...
\- Replace `<version>` with the actual version number (e.g., `v6.3.1`). - From v6.1.0, the MSI installs per-machine by default, requiring administrator privileges.
