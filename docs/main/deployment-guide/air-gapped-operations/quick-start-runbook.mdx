...
11. **Configure FIPS at install time** if required for the enclave (DoD IL4 / IL5, FedRAMP Moderate). FIPS cannot be enabled post-deploy. See [Configure FIPS at Install Time](/deployment-guide/server/configure-fips-at-install-time).
