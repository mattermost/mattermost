...
- **Plugin-initiated outbound calls**. Each plugin has its own outbound behavior. Audit each plugin you install. The Mattermost Calls plugin in particular requires careful egress configuration; see [Calls Deployment](../) (top-level Deployment Guide → Calls Deployment section, expanded in Phase 2).
