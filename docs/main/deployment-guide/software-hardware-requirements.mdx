...
<tr>
<td>iOS</td>
<td>iOS 16.0+ with Safari 26.2+ or Chrome 150+</td>
</tr>
<tr>
<td>Android</td>
<td>Android 7+ with Chrome 150+</td>
</tr>
</tbody>
</table>
...
In MySQL 8.0.4, the default authentication plugin was changed from `mysql_native_password` to `caching_sha2_password`. Mattermost's MySQL driver supports `caching_sha2_password` natively, so no configuration change is required to run the Mattermost server.

<Note>

If you're migrating to PostgreSQL using `pgloader`, its MySQL driver doesn't support `caching_sha2_password`. See [Unsupported authentication for MySQL](/administration-guide/manage/admin/postgres-migration#unsupported-authentication-for-mysql) for the workaround, including the MySQL 8.4+ path via `authentication_policy`.
...
- Elasticsearch to support highly efficient database searches in a cluster environment. Elasticsearch v8.x and v9.x are supported, and Elasticsearch v9.x or AWS OpenSearch is recommended. [Learn more](/deployment-guide/scale/enterprise-search).
