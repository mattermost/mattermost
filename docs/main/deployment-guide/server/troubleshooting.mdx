...
---
This document summarizes common deployment troubleshooting issues and resolutions, and describes the diagnostic data to gather before opening a support ticket. Some of these suggestions can be done directly, and others may need consultation from your network administrator.

If you already know which part of your deployment is failing, start with the guide for that layer:

- [PostgreSQL troubleshooting](/deployment-guide/server/prepare-database#troubleshooting), on the database setup page under **Prepare**.
- [Docker deployment troubleshooting](/deployment-guide/server/docker-troubleshooting), alongside the container deployment steps under **Choose a deployment method**.
- [Desktop app troubleshooting](/deployment-guide/desktop/desktop-troubleshooting), at the end of **Desktop app deployment**.
- [Mobile deployment troubleshooting](/deployment-guide/mobile/mobile-troubleshooting), at the end of **Mobile app deployment**.
...
1.  If log files aren't appearing in **System Console \> Reporting \> Server Logs** or support packets, check error messages in server console to see if the log file path is outside the allowed directory: `"Blocked attempt to read log file outside allowed root"` Log file paths outside the directory specified by the `MM_LOG_PATH` environment variable generate errors and are excluded from support packet downloads. If `MM_LOG_PATH` is not set, the default `logs` directory is used.
    ...
    - `ExperimentalAuditSettings.FileName` - audit log file
...
#### Server fails to start with a log path error

If the `EnforceLogPathRoot` feature flag is enabled (`MM_FEATUREFLAGS_ENFORCELOGPATHROOT=true`), a log or audit file path resolving outside the logging root stops the server from starting instead of only logging an error. Startup fails with a message like:

```
failed to initialize logging: log file paths outside logging root: LogSettings.AdvancedLoggingJSON target "custom": path /var/log/custom.log is outside logging root /opt/mattermost/logs
```

With the flag enabled, configuration changes that introduce such a path are also rejected, and **System Console \> Environment \> Logging** returns an error instead of saving.

To resolve, either set the `MM_LOG_PATH` environment variable to a parent directory that contains every configured log path, or move the log files into the existing logging root - then restart the server. The error message names the configuration setting and the offending path. Setting `MM_FEATUREFLAGS_ENFORCELOGPATHROOT=false` restores the warn-only behavior, but leave enforcement on where you can: it will become the default in a future release.
