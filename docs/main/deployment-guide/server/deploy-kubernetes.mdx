...
Mattermost no longer publishes deployment guidance for Azure Kubernetes Service (AKS). The retired AKS guide isn't being maintained, so it has been withdrawn rather than left to go stale. AKS is still a standard Kubernetes distribution — deploy with the [Mattermost Operator](/deployment-guide/server/kubernetes/deploy-k8s) as you would on any other conformant cluster.
