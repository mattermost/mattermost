...
### Bug Fixes
 - Fixed an issue where channel links ending in a channel ID failed to open.
 - Fixed an issue where urgent mention badges used normal mention colors in the channel list.
 - Fixed an issue where pasting a full server URL containing a path, such as a channel link, into **Add Server** showed a confusing error instead of connecting.
 - Fixed an intermittent issue where answering an incoming call from the iOS lock screen immediately disconnected.
