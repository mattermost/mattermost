...
## 2.45.0 Release \{#release-v2-45-0}
...
### Compatibility
 - **Upgrade to Android 11 or later, or iOS 18 or later, is required.** The minimum supported operating system versions increased in this release. Devices running earlier operating system versions must remain on v2.44.
 ...
 - Android 11+ is required.
 - iOS 18.0+ is required.

### Improvements
 - Improved the Direct Message call experience to behave more like a native phone call.
 - Added tappable ``tel:`` links for phone numbers in posts, which open the device calling app.
 - Added support for the automatic cache cleanup setting provided by the server, performing rolling cleanups of cached content to prevent it from accumulating.
 - Added collapse and expand all checklists, and task filtering by state and assignee, to playbook runs.
 - Updated onboarding and server connection welcome messaging.

### Bug Fixes
 - Fixed an issue where channel links ending in a channel ID failed to open (reported on iOS).
 - Fixed an issue where urgent mention badges used normal mention colors in the channel list (reported on iOS).
 - Fixed an issue where pasting a full server URL containing a path, such as a channel link, into **Add Server** showed a confusing error instead of connecting (reported on iOS and Android).
 - Fixed an intermittent issue where answering an incoming call from the iOS lock screen immediately disconnected (reported on iOS).
 - Fixed an issue where the tool approval user interface briefly offered **Accept**/**Reject** for AI tool calls configured to run automatically, which could race with server-side auto-execution. Interrupted auto-run tool rounds now show a **Run tools** option instead of approval buttons.
 - Fixed an issue with deeplink routing (reported on Android).
...
## 2.44.0 Release \{#release-v2-44-0}
...
### Improvements
 ...
 - Improved Android cold start by decrypting credentials only for known servers instead of listing the entire Keystore.
...
### Bug Fixes
 ...
 - Fixed the biometric prompt from not appearing and failing the authentication when the app was brought to the foreground.
 - Fixed a crash on the channel and **Drafts** screens when a draft held an attachment that was persisted without its file metadata.
 - Fixed iOS crash when attaching photos (SIGABRT).

### Open Source Components
 - Removed ``react-native-incall-manager``.

### Known Issues
 - Users are unable to adjust the font size via the OS font size setting.
 - Some Google Pixel phones on Android 12+ might not continue past the login screen. This is a known issue with the OS, and the current workaround is to restart the device.

## 2.43.1 Release \{#release-v2-43-1}
...
## 2.43.0 Release \{#release-v2-43-0}
...
### Improvements
 - Added [classification markings](https://docs.mattermost.com/end-user-guide/collaborate/display-channel-banners.html) for global and per-channel banners. Requires server v11.9.0+.
...
## 2.42.3 Release \{#release-v2-42-3}
