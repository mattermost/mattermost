...
## Release v11.11 - [Feature Release](https://docs.mattermost.com/product-overview/release-policy.html#release-types) \{#release-v11-11-feature-release}

- **11.11.1, released 2026-09-24**
  - Mattermost v11.11.1 contains Medium severity level security fixes. [Upgrading](https://docs.mattermost.com/upgrade/upgrading-mattermost-server.html) to this release is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  - Pre-packaged Zoom plugin version [v1.13.1](https://github.com/mattermost/mattermost-plugin-zoom/releases/tag/v1.13.1).
  - Pre-packaged Jira plugin version [v4.8.1](https://github.com/mattermost/mattermost-plugin-jira/releases/tag/v4.8.1).
  - Pre-packaged Playbooks plugin version [v2.11.2](https://github.com/mattermost/mattermost-plugin-playbooks/releases/tag/v2.11.2).
  ...
  - Mattermost v11.11.1 contains no database or functional changes.
- **11.11.0, released 2026-09-16**
  - Original 11.11.0 release.
...
#### User Interface
 ...
 - Pre-packaged Agents plugin version [v2.6.1](https://github.com/mattermost/mattermost-plugin-agents/releases/tag/v2.6.1).
...
#### Administration
 ...
 - OAuth Dynamic Client Registration (DCR) redirect URI validation now accepts custom (non-HTTP) URI schemes such as cursor:// in addition to http:// and https://, for both the DCR Redirect URI Allowlist config setting and redirect URIs supplied at client registration, unblocking desktop OAuth clients.
 - Truncated PDF text extraction to a specific limit.
 - Editing an outgoing OAuth connection now requires re-entering the client secret before validating or saving.
 - Updated how error details are formatted for session and OAuth authorization flows.
 - Updated AD/LDAP test connection handling to require re-entering the bind password when connection settings change.
 - Mention keywords are now limited to 500 keys and 50,000 bytes total. Users or imports that exceed these limits will receive a descriptive error.
 - Updated how user presence status responses are serialized.
 - Updated ``golang.org/x/image`` to v0.45.0 and ``msgpack`` fork to cap byte-string allocations.
...
#### Plugins/Integrations
 ...
 - Skip cluster now sends after closing the gossip socket, avoiding unnecessary ERROR logs on shutdown.
...
#### Performance
 - Benchmarking test results showed no significant difference: a -0.09% delta in the number of supported users for the new release, which lies within the ``[-5%, +5%]`` prediction interval. View the full raw data and methodology in our [Performance Reports repository](https://github.com/mattermost/performance-reports/tree/main/performance-comparisons/v11.11).
...
### Bug Fixes
 ...
 - Fixed an issue with public channel permalinks so that clicking them joins the channel when compliance is enabled, while still hiding permalink previews for non-members.
 - Fixed an issue where a message could be sent to the previously viewed channel after navigating with the ``/msg`` slash command or the **Ctrl/Cmd+K** channel switcher (reported on webapp, Chrome and MacOS).
 - Fixed an issue where OAuth requests did not check the server's outbound connection restrictions.
 - Fixed Elasticsearch and OpenSearch startup on AWS managed domains that omit plugin data from the Nodes Info API.
 ...
 - Fixed an issue where a plugin request body wasn't properly validated.
...
### API Changes
 ...
 - Added ``DELETE /scheduled_recaps/{scheduled_recap_id} (deleteScheduledRecap)`` API endpoint.
 - Added ``GET /recaps/limit_status (getRecapLimitStatus)`` API endpoint.
 - Added ``GET /scheduled_recaps/{scheduled_recap_id} (getScheduledRecap)`` API endpoint.
 - Added ``POST /scheduled_recaps (createScheduledRecap)`` API endpoint.
 - Added ``POST /scheduled_recaps/{scheduled_recap_id}/pause (pauseScheduledRecap)`` API endpoint.
 - Added ``POST /scheduled_recaps/{scheduled_recap_id}/resume (resumeScheduledRecap)`` API endpoint.
 - Added ``PUT /scheduled_recaps/{scheduled_recap_id} (updateScheduledRecap)`` API endpoint.
 - Added a new API file ``scheduled_recap.go``.
 - Removed the unneeded ``POST /api/v4/posts/ids/reactions`` endpoint.
...
- **11.10.2, released 2026-09-15**
  - Mattermost v11.10.2 contains Medium severity level security fixes. [Upgrading](https://docs.mattermost.com/upgrade/upgrading-mattermost-server.html) to this release is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  - Pre-packaged Agents plugin version [v2.5.3](https://github.com/mattermost/mattermost-plugin-agents/releases/tag/v2.5.3).
  - Updated how error details are formatted for session and OAuth authorization flows.
  - Updated AD/LDAP test connection handling to require re-entering the bind password when connection settings change.
  - Updated how user presence status responses are serialized.
  - Updated the msgpack fork to cap byte-string allocations.
  - Fixed an issue where a plugin request body wasn't properly validated.
  - Fixed an issue where team admins could accidentally update roles incorrectly.
  - Fixed an issue where OAuth requests did not check the server's outbound connection restrictions.
  - Editing an outgoing OAuth connection now requires re-entering the client secret before validating or saving.
  - Truncated PDF text extraction to a specific limit.
  - Removed the unneeded ``POST /api/v4/posts/ids/reactions`` endpoint.
  - Mention keywords are now limited to 500 keys and 50,000 bytes in total. Users or imports that exceed these limits receive a descriptive error.
  - Fixed the cloud preview introduction modal so it is vertically centered instead of sitting at the bottom of the page.
  - Mattermost v11.10.2 contains no database or functional changes.
...
#### Database Schema Changes
 - The following schema changes are included in the v11.10 release. No database downtime is expected for this upgrade. See the [Important Upgrade Notes](https://docs.mattermost.com/upgrade/important-upgrade-notes.html) for more details.
   ...
   - Adds composite index ``idx_propertyvalues_groupid_updateat_id`` on ``PropertyValues(GroupID, UpdateAt, ID)`` to improve Custom Attributes query performance. The index is created concurrently, so its creation does not block DML.
...
- **11.9.2, released 2026-09-15**
  - Mattermost v11.9.2 contains Medium to High severity level security fixes. [Upgrading](https://docs.mattermost.com/upgrade/upgrading-mattermost-server.html) to this release is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  - Pre-packaged Agents plugin version [v2.4.5](https://github.com/mattermost/mattermost-plugin-agents/releases/tag/v2.4.5).
  - Updated how error details are formatted for session and OAuth authorization flows.
  - Updated AD/LDAP test connection handling to require re-entering the bind password when connection settings change.
  - Updated how user presence status responses are serialized.
  - Updated the msgpack fork to cap byte-string allocations.
  ...
  - Enhanced team search to only display teams relevant and accessible to the requesting user.
  - Updated account type switch handling.
  - Mattermost v11.9.2 contains no database or functional changes.
...
#### Administration
  ...
  - Added a ``mattermost db ping`` [subcommand](https://docs.mattermost.com/deployment-guide/deployment-scenarios/air-gapped-deployment.html) that waits for the database to become reachable, with configurable ``--timeout`` and ``--retry-interval`` flags.
...
- **11.7.11, released 2026-09-15**
  - Mattermost v11.7.11 contains Low to High severity level security fixes. [Upgrading](https://docs.mattermost.com/upgrade/upgrading-mattermost-server.html) to this release is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  - Pre-packaged Agents plugin version [v2.0.9](https://github.com/mattermost/mattermost-plugin-agents/releases/tag/v2.0.9).
  - Pre-packaged Boards plugin version [v9.2.8](https://github.com/mattermost/mattermost-plugin-boards/releases/tag/v9.2.8).
  - Pre-packaged Jira plugin version [v4.7.2](https://github.com/mattermost/mattermost-plugin-jira/releases/tag/v4.7.2).
  - Updated how error details are formatted for session and OAuth authorization flows.
  - Updated AD/LDAP test connection handling to require re-entering the bind password when connection settings change.
  - Updated how user presence status responses are serialized.
  - Updated the msgpack fork to cap byte-string allocations.
  ...
  - Improved performance of the bulk-indexing job for Elasticsearch and OpenSearch on very large ``FileInfo`` and ``Channels`` tables.
  - Mattermost v11.7.11 contains the following database changes:
