...
New setting options were added to ``config.json``. Below is a list of the additions and their default values on install. The settings can be modified in ``config.json``, or the System Console when available.
  - **Changes to Enterprise Advanced plan:**
    - Under ``AccessControlSettings`` in ``config.json``, added ``EnableAccessControlAuditLogging`` configuration setting to enable audit logging for attribute-based access control policy decisions. Defaults to off and is only available when server audit logging is active.
...
#### User Interface
  ...
  - Pre-packaged Mattermost Agents plugin at version 2.6.1.
  ...
  - Updated the invite modal to return an error and not send an invitation email when the invited email address belongs to a deactivated user account.
...
#### Administration
  - Added a new Post Exposure Report feature to the Data Spillage Handling workflow for content reviewers.
  - Added a new opt-in System Console setting, ``AccessControlSettings.EnableAccessControlAuditLogging`` (Attribute-Based Access Control), to enable audit logging for ABAC policy decisions. The setting defaults to off and is only available when server audit logging is active.
  ...
  - Prevented a post belonging to a Direct Message or Group Message from being quarantined.
...
#### Performance
  - Fixed a memory leak caused by Elasticsearch starting bulk indexers without stopping them, which could lead to an out-of-memory error. Added logging to alert system administrators when the required ``analysis-icu`` Elasticsearch/OpenSearch plugin is missing.
  - Skipped cluster sends after closing the gossip socket, avoiding unnecessary ERROR logs on shutdown.
...
### Bug Fixes
  ...
  - Fixed an issue where a confidential OAuth application kept rotated tokens in memory during the session cache window (configured by ``SessionCacheInMinutes``).
  ...
  - Fixed the policy simulation to show "No recent session" when session attributes are unavailable, and restricted detailed evaluation traces in simulation results to system administrators.
  ...
  - Fixed public channel permalinks so that clicking them joins the channel when compliance is enabled, while still hiding permalink previews for non-members.
  ...
  - Fixed an error ("Unable to save access control policy.") that occurred when deleting the only remaining rule in a channel's Permissions Policy tab.
  ...
  - Fixed a bug where permanently deleting a bot account left the bot's personal access tokens and their sessions orphaned in the database.
  ...
  - Fixed an issue where a plugin request body was not properly validated.
  ...
  - Fixed an issue where a message could be sent to the previously viewed channel after navigating with the ``/msg`` slash command or the Ctrl/Cmd+K channel switcher.
  - Fixed an issue where a bug in the Attribute-Based Access Control policy editor prevented switching from Advanced back to Simple Mode when an attribute value contained a quote character (such as an apostrophe).
  - Fixed an issue in which policies containing Rank attributes with 'shared-only' access mode were unable to be created.
...
### API Changes
  ...
  - Removed the ``POST /api/v4/posts/ids/reactions`` (``getBulkReactions``) API endpoint.
...
### Open Source Components
  - Updated dependencies in https://github.com/mattermost/mattermost/.
