...
## Release v6.3 \{#release-v6-3}

- **v6.3.1, released TBD**

  - Mattermost Desktop App v6.3.1 contains medium severity level security fixes. Upgrading is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  - Fixed an issue where a malformed ``SiteURL`` in a configured server's client configuration could persistently crash the Desktop App on launch.
  - Updated file download finalization handling for saved downloads.
  - Fixed an issue with some local network access checks.
  - Fixed an issue where the Calls widget could crash the app when resized to a very large size.

- **v6.3.0, released 2026-08-14**

  - Original v6.3.0 release

**Download Binaries:** [Mattermost Desktop on GitHub](https://github.com/mattermost/desktop/releases/v6.3.1)
...
- Users seeing an endless "Loading..." screen when attempting to log in to the app may need to manually clear cached data. The Mattermost data directory is `/Users/<username>/Library/Containers/Mattermost/Data/Library/Application Support/Mattermost` on macOS, `Users/<username>/AppData/Roaming/Mattermost` on Windows, and `~/.config/Mattermost` on Linux. Delete only the `Cache`, `Code Cache`, and `GPUCache` subdirectories. Deleting the full Mattermost data directory also resets desktop configuration and session data.
...
## Release v6.2 (Extended Support Release) \{#release-v6-2}

- **v6.2.4, released TBD**

  - Mattermost Desktop App v6.2.4 contains medium severity level security fixes. Upgrading is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  - Fixed an issue where a malformed ``SiteURL`` in a configured server's client configuration could persistently crash the Desktop App on launch.
  - Updated file download finalization handling for saved downloads.
  - Fixed an issue with some local network access checks.
  - Fixed an issue where the Calls widget could crash the app when resized to a very large size.
...
**Download Binaries:** [Mattermost Desktop on GitHub](https://github.com/mattermost/desktop/releases/v6.2.4)
