...
|---|---|
| FIPS 140-3 validated cryptographic modules in all data paths | See [Configure FIPS at Install Time](../../deployment-guide/server/configure-fips-at-install-time). Enable FIPS mode at install. |
...
1. **FIPS mode enabled at install time** — see [Configure FIPS at Install Time](../../deployment-guide/server/configure-fips-at-install-time).
