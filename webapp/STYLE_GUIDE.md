...
- **Scope & Naming**:
    ...
    - **Modifiers**: When applying a class to modify a component, that should be done as a separate CSS class applied to styled element. For example, a compact version of `MyComponent` would be styled by putting a `&.compact` rule inside of `.MyComponent`.
...
- **Text For Images**:
    ...
    - Don't include the words "image" or "icon" in an image's alt text or `aria-label`. Those are already read out by accessibility tools.
