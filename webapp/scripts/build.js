...
async function buildAll() {
    ...
    try {
        const { result } = concurrently(getPlatformCommands("build"), {
            killOthers: "failure",
        });
        ...
        console.error(
            chalk.inverse.bold.red("Failed to build subpackages"),
            closeEvents,
        );
    ...
    try {
        const { result } = concurrently([
            {
                command: "npm:build --workspace=channels",
                name: "webapp",
                prefixColor: "cyan",
            },
        ]);
        ...
        console.error(
            chalk.inverse.bold.red("Failed to build web app"),
            closeEvents,
        );
        return getExitCode(closeEvents);
    }
