    ...
    async initialize(registry: PluginRegistry, mmStore: Store<GlobalState, Action<Record<string, unknown>>>): Promise<void> {
        ...
        if (this.registry.registerProduct) {
            ...
            if (registry.registerChannelIntroButtonAction) {
                this.channelHeaderButtonId = registry.registerChannelIntroButtonAction(<FocalboardIcon/>, goToFocalboardTemplate, <FormattedMessage id='ChannelIntro.CreateBoard' defaultMessage='Create a board'/>)
            }

            if (this.registry.registerAppBarComponent) {
                this.registry.registerAppBarComponent(appBarIcon, () => mmStore.dispatch(toggleRHSPlugin), <FormattedMessage id='AppBar.Tooltip' defaultMessage='Toggle linked boards'/>)
            }
            ...
            if (registry.registerSiteStatisticsHandler) {
                registry.registerSiteStatisticsHandler(async () => {
                    ...
                    if(siteStats){
                        return {
                            boards_count: {
                                name: <FormattedMessage id='SiteStats.total_boards' defaultMessage='Total boards'/>,
                                id: 'total_boards',
                                icon: 'icon-product-boards',
                                value: siteStats.board_count,
                            },
                            cards_count: {
                                name: <FormattedMessage id='SiteStats.total_cards' defaultMessage='Total cards'/>,
                                id: 'total_cards',
                                icon: 'icon-products',
