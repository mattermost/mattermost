...
module.exports = {
    meta: {
        docs: {
            description:
                'Enforce all anchors with target="_blank" to use ExternalLink component',
        },
    },
    create: (context) => {
        ...
        return {
            JSXOpeningElement: (node) => {
                ...
                const values = propsToValidate.map((prop) =>
                    astUtils.getPropValue(
                        astUtils.getProp(node.attributes, prop),
                    ),
                );
                // Checks if the target attribute is set to _blank (ie, is an external link)
                const hasBlankTarget = values.some(
                    (value) => value != null && value === "_blank",
                );
                ...
                context.report({
                    node,
                    message:
                        "Use ExternalLink component (components/external_link) for _blank target link-outs",
                });
                return;
            },
        };
