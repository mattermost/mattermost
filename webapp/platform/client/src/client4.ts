...
// Header names are case-insensitive on the wire, so match existing keys case-insensitively to avoid duplicates.
function findHeaderKey(headers: {[x: string]: string}, name: string): string | undefined {
...
export default class Client4 {
    ...
    getOptions(options: Options) {
        ...
        if (options.body && !findHeaderKey(headers, HEADER_CONTENT_TYPE)) {
        ...
        if (newOptions.headers) {
            for (const [name, value] of Object.entries(newOptions.headers)) {
