...
Main Mattermost web client workspace containing all UI components, Redux state management, and application logic.
...
- `npm run build --workspace=channels` – production bundle
...
- `webpack.config.js` – module federation + alias map
...
The app uses webpack module federation for plugin architecture:
...
- `@mattermost/types`, `@mattermost/client`, and `platform/components` are first-party packages; import via full package names
...
- Environment-specific overrides live in `config/` on the server side; do not hard-code URLs or feature flags
