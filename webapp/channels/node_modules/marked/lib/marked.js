...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    // underline (extra formatting - must be before strong to intercept __ when enabled)
    if (this.options.extraFormatting && (cap = this.rules.underline.exec(src))) {
      src = src.substring(cap[0].length);
      tokens.push({
        type: 'underline',
    ...
    // spoiler
    if (this.options.spoiler && (cap = this.rules.spoiler.exec(src))) {
      src = src.substring(cap[0].length);
      // Recursively lex the inner content so markdown inside spoilers works
      var innerTokens = this.output(cap[1]);
      tokens.push({
        type: 'spoiler',
        text: innerTokens
      });
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    switch(token.type) {
      ...
      case 'del':
        ...
        break;
      case 'spoiler':
        out += this.renderer.spoiler(token.text);
        break;
      case 'underline':
        out += this.renderer.underline(token.text);
        break;
      case 'inlinelatex':
