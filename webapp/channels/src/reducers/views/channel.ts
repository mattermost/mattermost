...
function loadingPosts(
    state: { [channelId: string]: boolean } = {},
    action: GenericAction,
) {
    switch (action.type) {
...
function lastUnreadChannel(
    state: { channelId: string; hadMentions: boolean } | null = null,
    action: GenericAction,
) {
    switch (action.type) {
        ...
        case ActionTypes.SET_LAST_UNREAD_CHANNEL: {
            const { channelId, hadMentions, hadUnreads } = action;
...
function channelPrefetchStatus(
    state: { [channelId: string]: string } = {},
    action: GenericAction,
) {
    switch (action.type) {
