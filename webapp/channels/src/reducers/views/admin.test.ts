...
describe("views/admin reducers", () => {
    describe("needsLoggedInLimitReachedCheck", () => {
        it("defaults to false", () => {
            const actual = needsLoggedInLimitReachedCheck(undefined, {
                type: "asdf",
            });
        ...
        it("is set by NEEDS_LOGGED_IN_LIMIT_REACHED_CHECK", () => {
            const falseValue = needsLoggedInLimitReachedCheck(undefined, {
                type: ActionTypes.NEEDS_LOGGED_IN_LIMIT_REACHED_CHECK,
                data: false,
            });
            expect(falseValue).toBe(false);

            const trueValue = needsLoggedInLimitReachedCheck(false, {
                type: ActionTypes.NEEDS_LOGGED_IN_LIMIT_REACHED_CHECK,
                data: true,
            });
