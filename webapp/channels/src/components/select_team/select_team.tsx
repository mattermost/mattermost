...
export default class SelectTeam extends React.PureComponent<Props, State> {
    ...
    static getDerivedStateFromProps(props: Props, state: State) {
        if (props.listableTeams.length !== state.currentListableTeams.length) {
            return {
                currentListableTeams: props.listableTeams.slice(
                    0,
                    TEAMS_PER_PAGE * state.currentPage,
                ),
            };
        }
        return null;
    ...
    fetchMoreTeams = async () => {
        ...
        const response = await actions.getTeams(
            currentPage,
            TEAMS_PER_PAGE,
            true,
        );
            ...
            <SystemPermissionGate permissions={[Permissions.CREATE_TEAM]}>
                <div className="margin--extra" style={{ marginTop: "0.5em" }}>
                    <Link
                        ...
                        to="/create_team"
                        onClick={() =>
                            trackEvent("select_team", "click_create_team")
                        }
        ...
        return (
            <div>
                ...
                <div className="col-sm-12">
                    <div
                        className={
                            "select-team__container signup-team__container"
                        }
                    >
                        <img
