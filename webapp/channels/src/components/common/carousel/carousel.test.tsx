...
describe("/components/common/Carousel", () => {
    ...
    const baseProps = {
        ...
        infiniteSlide: true,
        dataSlides: [
            <p className="slide" key={1}>
                {texts[0]}
            </p>,
            <p className="slide" key={2}>
                {texts[1]}
            </p>,
            <p className="slide" key={3}>
                {texts[2]}
            </p>,
        ],
    };
    ...
    test("test carousel shows next and previous chevrons when this option is sent", () => {
        const wrapper = mountWithIntl(
            <Carousel {...baseProps} btnsStyle={BtnStyle.CHEVRON} />,
        );
        const nextButton = wrapper
            .find(Carousel)
            .find("CarouselButton div.chevron-right");
        const prevButton = wrapper
            .find(Carousel)
            .find("CarouselButton div.chevron-left");
