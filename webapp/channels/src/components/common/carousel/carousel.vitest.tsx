...
describe('/components/common/Carousel', () => {
    ...
    test('test carousel shows first slide as active', () => {
        const {container} = renderWithContext(<Carousel {...baseProps}/>);
        const activeSlide = container.querySelector('.active-anim');
    ...
    test('test carousel moves slides when clicking buttons', async () => {
        ...
        await waitFor(() => {
            const newActiveSlide = container.querySelector('.active-anim');
            expect(newActiveSlide?.textContent).toContain('Second Slide');
        });
    });

    test('test carousel executes custom next and prev btn callback functions', () => {
        const onPrevSlideClick = vi.fn();
        const onNextSlideClick = vi.fn();
        const props = {
            ...baseProps,
            onPrevSlideClick,
            onNextSlideClick,
