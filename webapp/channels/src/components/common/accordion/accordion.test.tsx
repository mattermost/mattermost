...
describe("/components/common/Accordion", () => {
        ...
        accordionItemsData: [
            {
                ...
                items: [
                    <p className="accordion-item-content-el" key={1}>
                        {texts[0]}
                    </p>,
                    <p className="accordion-item-content-el" key={2}>
                        {texts[1]}
                    </p>,
                ],
            },
    ...
    test("test accordion opens first accordion item when clicked", () => {
        ...
        const firstChildItem = firstAccordionCard.find(
            "div.accordion-card-container p.accordion-item-content-el:first-child",
        );
    ...
    test("test accordion opens ONLY one accordion item at a time if NO openMultiple prop is set or set to FALSE", () => {
        ...
        const firstAccordionCardAfterEvent = wrapper.find(
            "ul AccordionCard:first-child",
        );
        const secondAccordionCardAfterEvent = wrapper.find(
            "ul AccordionCard:last-child",
        );

        // clicking first list element should only apply the active class to the first one and not to the last
        expect(
            firstAccordionCardAfterEvent
                .find("li.accordion-card")
                .hasClass("active"),
        ).toEqual(true);
        expect(
            secondAccordionCardAfterEvent
                .find("li.accordion-card")
                .hasClass("active"),
        ).toEqual(false);
        ...
        // refind the element after making changes so those gets reflected
        const firstAccordionCardAfterEvent1 = wrapper.find(
            "ul AccordionCard:first-child",
        );
        const secondAccordionCardAfterEvent1 = wrapper.find(
            "ul AccordionCard:last-child",
        );

        // clicking last list element should only apply the active class to the last one and not to the first
        expect(
            firstAccordionCardAfterEvent1
                .find("li.accordion-card")
                .hasClass("active"),
        ).toEqual(false);
        expect(
            secondAccordionCardAfterEvent1
                .find("li.accordion-card")
                .hasClass("active"),
        ).toEqual(true);
    });
    ...
    test("test accordion opens MORE THAN one accordion item at a time if openMultiple prop IS set to TRUE", () => {
        const wrapper = mountWithIntl(
            <Accordion {...baseProps} expandMultiple={true} />,
        );
        const firstAccordionCard = wrapper.find("ul AccordionCard:first-child");
        const secondAccordionCard = wrapper.find("ul AccordionCard:last-child");
