...
export const withBackupValue = (
    maybeLimit: number | undefined,
    limitsLoaded: boolean,
) => (limitsLoaded ? maybeLimit ?? Number.MAX_VALUE : Number.MAX_VALUE);
        ...
        return {
            files: {
                totalStorage:
                    ...
                    withBackupValue(limits.files?.total_storage, limitsLoaded),
                totalStorageLoaded: usage.files.totalStorageLoaded,
            },
            messages: {
                history:
                    usage.messages.history -
                    withBackupValue(limits.messages?.history, limitsLoaded),
                historyLoaded: usage.messages.historyLoaded,
            },
            teams: {
                active:
                    usage.teams.active -
                    withBackupValue(limits.teams?.active, limitsLoaded),
