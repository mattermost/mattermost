...
type RadioGroupProps = {
    id: string;
    values: Array<{
        key: React.ReactNode | React.ReactNodeArray;
        value: string;
        testId?: string;
    }>;
    value: string;
    badge?:
        | { matchVal: string; badgeContent: ReactNode; extraClass?: string }
        | undefined
        | null;
...
const RadioButtonGroup = ({
            ...
            <div className="radio" key={val}>
                <label className={val === value ? "selected" : ""}>
                    ...
                    {sideLegend && val === sideLegend?.matchVal && (
                        <span className="side-legend">{sideLegend.text}</span>
                    )}
                </label>
    ...
    return (
        <div className="radio-list" data-testid={testId || ""}>
            {options}
        </div>
    );
};
