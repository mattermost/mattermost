...
function PlanLabel(props: PlanLabelProps) {
    ...
    if (
        props.renderLastDaysOnTrial &&
        ((isCloud && subscription?.is_free_trial === "true") ||
            isSelfHostedEnterpriseTrial)
    ) {
        const daysLeftOnTrial = Math.min(
            getRemainingDaysFromFutureTimestamp(
                isSelfHostedEnterpriseTrial
        ...
        text = formatMessage(
            {
                id: "pricing_modal.plan_label_trialDays",
                defaultMessage: "{days} DAYS LEFT ON TRIAL",
            },
            { days: daysLeftOnTrial },
        );
    }
