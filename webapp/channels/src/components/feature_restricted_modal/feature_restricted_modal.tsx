...
const FeatureRestrictedModal = ({
    ...
    const prevTrialLicense = useSelector(
        (state: GlobalState) => state.entities.admin.prevTrialLicense,
    );
    ...
    const show = useSelector((state: GlobalState) =>
        isModalOpen(state, ModalIdentifiers.FEATURE_RESTRICTED_MODAL),
    );
    ...
    const getTitle = () => {
        if (isSystemAdmin) {
            return hasPriorTrial || cloudFreeDeprecated
                ? titleAdminPostTrial
                : titleAdminPreTrial;
        }
    ...
    const showStartTrial =
        isSystemAdmin && !hasPriorTrial && !cloudFreeDeprecated;

    // define what is the secondary button text and action, by default will be the View Plan button
    let secondaryBtnMsg = formatMessage({
        id: "feature_restricted_modal.button.plans",
        ...
        trialBtn = (
            <CloudStartTrialButton
                ...
                message={formatMessage({
                    id: "trial_btn.free.tryFreeFor30Days",
    ...
    return (
        ...
        >
            <div className="FeatureRestrictedModal__body">
                ...
                <div
                    className={classNames("FeatureRestrictedModal__buttons", {
                        single: !showStartTrial,
                    })}
                >
                    <button
                        id="button-plans"
                        ...
                        disabled={
                            notifyRequestStatus === NotifyStatus.AlreadyComplete
                        }
                    >
                        {secondaryBtnMsg}
                    </button>
                    {showStartTrial && trialBtn}
                </div>
            </div>
        </GenericModal>
    );
