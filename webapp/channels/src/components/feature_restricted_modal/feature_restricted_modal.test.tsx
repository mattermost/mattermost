...
describe("components/global/product_switcher_menu", () => {
    ...
    test("should show with end user pre trial", () => {
        ...
        expect(
            wrapper.find(".FeatureRestrictedModal__description").text(),
        ).toEqual(defaultProps.messageEndUser);
        expect(wrapper.find(".FeatureRestrictedModal__terms").length).toEqual(
            0,
        );
        expect(
            wrapper.find(".FeatureRestrictedModal__buttons").hasClass("single"),
        ).toEqual(true);
    ...
    test("should show with system admin pre trial for self hosted", () => {
        ...
        expect(
            wrapper.find(".FeatureRestrictedModal__description").text(),
        ).toEqual(defaultProps.messageAdminPreTrial);
        expect(wrapper.find(".FeatureRestrictedModal__terms").length).toEqual(
            1,
        );
        expect(
    ...
    test("should match snapshot with system admin post trial", () => {
        ...
        expect(
            wrapper.find(".FeatureRestrictedModal__description").text(),
        ).toEqual(defaultProps.messageAdminPostTrial);
        expect(wrapper.find(".FeatureRestrictedModal__terms").length).toEqual(
            0,
        );
        expect(wrapper.find(".button-plans").length).toEqual(1);
