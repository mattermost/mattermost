...
function PricingModal(props: Props) {
    ...
    const isCloudPurchaseModalOpen = useSelector((state: GlobalState) =>
        isModalOpen(state, ModalIdentifiers.CLOUD_PURCHASE),
    );
    ...
    const content = isCloud ? (
        <Content onHide={onHide} callerCTA={props.callerCTA} />
    ) : (
        <SelfHostedContent onHide={onHide} />
    );
