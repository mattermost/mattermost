...
function SelfHostedContent(props: ContentProps) {
    ...
    useEffect(() => {
        async function fetchSelfHostedProducts() {
            try {
                ...
                const professionalProduct = products.find(
                    (prod) =>
                        prod.sku === LicenseSkus.Professional &&
                        prod.recurring_interval === RecurringIntervals.YEAR,
                );
                const price = professionalProduct
                    ? professionalProduct.price_per_seat.toString()
                    : FALL_BACK_PROFESSIONAL_PRICE;
    ...
    const prevSelfHostedTrialLicense = useSelector(
    ...
    const isPostSelfHostedEnterpriseTrial =
        prevSelfHostedTrialLicense.IsLicensed === "true";
    ...
    const starterBriefing = [
        formatMessage({
            id: "pricing_modal.briefing.unlimitedWorkspaceTeams",
            defaultMessage: "Unlimited workspace teams",
        }),
        formatMessage({
            id: "pricing_modal.briefing.unlimitedPlaybookRuns",
            defaultMessage: "Unlimited playbooks and runs",
        }),
        formatMessage({
            id: "pricing_modal.extra_briefing.free.calls",
            defaultMessage: "Voice calls and screen share",
        }),
        formatMessage({
            id: "pricing_modal.briefing.fullMessageAndHistory",
            defaultMessage: "Full message and file history",
        }),
        formatMessage({
            id: "pricing_modal.briefing.ssoWithGitLab",
            defaultMessage: "SSO with Gitlab",
        }),
    ];

    const professionalBriefing = [
        formatMessage({
            id: "pricing_modal.briefing.customUserGroups",
        ...
        formatMessage({
            ...
            defaultMessage: "SSO support with AD/LDAP, Google, O365, OpenID",
        }),
        formatMessage({
            id: "pricing_modal.extra_briefing.professional.guestAccess",
            defaultMessage: "Guest access with MFA enforcement",
        }),
    ];
    ...
    const enterpriseBriefing = [
        ...
        formatMessage({
            ...
            defaultMessage: "Advanced compliance management",
        }),
        formatMessage({
            id: "pricing_modal.extra_briefing.enterprise.playBookAnalytics",
            defaultMessage: "Playbook analytics dashboard",
        }),
    ];
    ...
    const renderAlert = () => {
        return (
            <div className="alert-option">
                <span>
                    {formatMessage({
                        id: "pricing_modal.lookingForCloudOption",
                        defaultMessage: "Looking for a cloud option?",
                    })}
                </span>
                <ExternalLink
                    onClick={() => {
                ...
                >
                    {formatMessage({
                        ...
                        defaultMessage: "Review deployment options",
                    })}
                </ExternalLink>
            </div>
        );
            ...
            <Modal.Body>
                ...
                <div className="PricingModal__body">
                        ...
                        buttonDetails={{
                            action: () => {
                                ...
                                if (!signupAvailable.ok) {
                                    if (
                                        signupAvailable.cwsContacted &&
                                        !signupAvailable.cwsServiceOn
                                    ) {
                                        window.open(
                                            CloudLinks.SELF_HOSTED_SIGNUP,
                                            "_blank",
                                ...
                                const professionalProduct =
                                    findSelfHostedProductBySku(
                                        products,
                                        SelfHostedProducts.PROFESSIONAL,
                                    );
                            ...
                            customClass: isPostSelfHostedEnterpriseTrial
                                ? ButtonCustomiserClasses.special
                    ...
                    <Card
                        ...
                        buttonDetails={
                            isPostSelfHostedEnterpriseTrial || !isAdmin
                                ? {
                                      action: () => {
                                          trackEvent(
                                              "self_hosted_pricing",
                        ...
                        contactSalesCTA={
                            isPostSelfHostedEnterpriseTrial ||
                            !isAdmin ? undefined : (
                                <ContactSalesCTA />
                            )
                        }
