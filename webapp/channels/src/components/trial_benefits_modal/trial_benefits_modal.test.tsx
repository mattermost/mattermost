...
describe("components/trial_benefits_modal/trial_benefits_modal", () => {
    ...
    test("should match snapshot when trial has already started", () => {
        const wrapper = shallow(
            <Provider store={store}>
                <TrialBenefitsModal {...props} trialJustStarted={true} />
            </Provider>,
        );
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test("should show the benefits modal", () => {
        ...
        expect(
            wrapper.find("TrialBenefitsModal").find("Carousel"),
        ).toHaveLength(1);
    });
    ...
    test("should call on exited", () => {
        ...
        const wrapper = mountWithIntl(
            <Provider store={store}>
                <TrialBenefitsModal {...props} onExited={mockOnExited} />
            </Provider>,
        );
    ...
    test("should present the just started trial modal content", () => {
        ...
        const title = wrapper
            .find("#trialBenefitsModalStarted-trialStart div.title")
            .text();
    ...
    test("should have a shorter title and not include the cta button when in cloud env", () => {
        ...
        const ctaBtn = wrapper.find(
            "#trialBenefitsModalStarted-trialStart button.btn-primary",
        );
