...
import type {
    ProductNotices,
    ProductNotice,
} from "@mattermost/types/product_notices";
...
export default class ProductNoticesModal extends React.PureComponent<
    Props,
    State
> {
    ...
    public componentDidUpdate(prevProps: Props) {
        ...
        if (
            presentSocketState.connected &&
            !prevSocketState.connected &&
            prevSocketState.lastConnectAt
        ) {
            const presentTime = Date.now();
            const previousSocketConnectDate = new Date(
                prevSocketState.lastConnectAt,
            ).getDate();
            const presentDate = new Date(presentTime).getDate();
            if (
                presentDate !== previousSocketConnectDate &&
                presentTime > prevSocketState.lastConnectAt
            ) {
                this.fetchNoticesData();
            }
        }
    ...
    private async fetchNoticesData() {
        ...
        const { data } = await this.props.actions.getInProductNotices(
            currentTeamId,
            client,
            clientVersion,
        );
        if (data) {
            ...
            if (data.length) {
                const presentNoticeInfo =
                    this.state.noticesData[this.state.presentNoticeIndex];
                this.props.actions.updateNoticesAsViewed([
                    presentNoticeInfo.id,
                ]);
            }
        }
    }
    ...
    private renderAdminOnlyText() {
        if (
            this.state.noticesData[this.state.presentNoticeIndex].sysAdminOnly
        ) {
            return (
                <>
                    <AdminEyeIcon />
    ...
    private handleNextButton = () => {
        ...
        if (noOfNotices === 1 && presentNoticeInfo.actionText) {
            ...
            const nextNoticeInfo =
                this.state.noticesData[this.state.presentNoticeIndex + 1];
    ...
    render() {
        ...
        const handlePreviousButton =
            this.state.presentNoticeIndex === 0
                ? undefined
                : this.handlePreviousButton;
        const autoCloseOnConfirmButton =
            this.state.presentNoticeIndex === this.state.noticesData.length - 1;
