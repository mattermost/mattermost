...
const ViewUserGroupListItem = (props: Props) => {
    const { user, group, groupId } = props;
    ...
    const removeUserFromGroup = useCallback(async () => {
        ...
        user.id,
        groupId,
        props.decrementMemberCount,
        props.actions.removeUsersFromGroup,
    ]);
    ...
    return (
        <div key={user.id} className="group-member-row">
            ...
            <div className="group-member-name">{Utils.getFullName(user)}</div>
            <div className="group-member-username">{`@${user.username}`}</div>
            {group.source.toLowerCase() !== GroupSource.Ldap &&
                props.permissionToLeaveGroup && (
                    <button
                        type="button"
