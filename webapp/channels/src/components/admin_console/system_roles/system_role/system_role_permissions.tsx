...
const sectionsList: SystemSection[] = [
    {
        name: "about",
        hasDescription: true,
        subsections: [{ name: "about_edition_and_license" }],
    },
    {
        name: "billing",
        hasDescription: true,
...
const getSectionsListForRole = memoizeResult(
    (
        sections: SystemSection[],
        roleName: string,
        sectionsByRole: Record<string, Record<string, boolean>>,
    ) => {
        return sections.filter(
            (section) =>
                !sectionsByRole[roleName] ||
                sectionsByRole[roleName][section.name],
        );
    },
);

export default class SystemRolePermissions extends React.PureComponent<
    Props,
    State
> {
    ...
    getRows = (
        ...
        if (
            ...
            Constants.PERMISSIONS_SYSTEM_CUSTOM_GROUP_ADMIN
        ) {
            return (
        ...
        if (
            this.props.role.name === Constants.PERMISSIONS_SYSTEM_USER_MANAGER
        ) {
            ...
            Object.keys(permissionsMap).forEach((permission) => {
                if (permission.startsWith("sysconsole_")) {
                    const permissionShortName = permission.replace(
                        /sysconsole_(read|write)_/,
                        "",
                    );
                    permissionsToShow = {
                        ...permissionsToShow,
        ...
        return getSectionsListForRole(
            sectionsList,
            this.props.role.name,
            editedSectionsByRole,
        ).map((section: SystemSection) => {
            return (
