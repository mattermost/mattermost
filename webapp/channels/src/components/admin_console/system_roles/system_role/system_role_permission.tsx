...
export default class SystemRolePermission extends React.PureComponent<Props> {
    ...
    renderSubsectionToggle = (
        ...
        const chevron = isSectionVisible ? (
    ...
    renderSectionRow = (
        ...
        return (
            <div key={section.name} className="PermissionSection">
                ...
                <div className="PermissionSectionDropdown">
                    <SystemRolePermissionDropdown
                        ...
                        access={this.getAccessForSection(
                            section,
                            permissionsMap,
                            permissionsToUpdate,
                        )}
                        isDisabled={
                            this.props.readOnly || Boolean(section.disabled)
                        }
                    />
                </div>
            </div>
    ...
    getAccessForSection = (
        ...
        permissionsToUpdate: Record<string, PermissionAccess>,
    ) => {
        ...
        if (section.subsections && section.subsections.length > 0) {
            ...
            if (
                [canRead, canWrite, hasNoAccess].filter((val) => val).length > 1
            ) {
                return mixedAccess;
            } else if (canRead) {
                return readAccess;
        ...
        return this.getAccessForSectionByName(
            section.name,
            permissions,
            permissionsToUpdate,
        );
    ...
    render() {
        ...
        const isSectionVisible = this.isSectionVisible(
            section,
            visibleSections,
        );
        return (
