    ...
    for (const permission of permissions) {
        if (typeof permission === "string") {
            ...
            result = result.concat(
                getRecursivePermissions(permission.permissions),
            );
        }
    }
    return result;
...
export default class PermissionGroup extends React.PureComponent<Props, State> {
    ...
    static getDerivedStateFromProps(props: Props, state: State) {
        if (props.selected !== state.selected) {
            if (
                getRecursivePermissions(props.permissions).indexOf(
        ...
        for (const permission of permissions) {
            if (typeof permission === "string") {
                ...
                anyUnchecked =
                    anyUnchecked ||
                    (!this.fromParent(permission) &&
                        this.props.role?.permissions?.indexOf(permission) ===
                            -1);
            } else {
    ...
    hasPermissionsOnScope = () => {
        return getRecursivePermissions(this.props.permissions).some(
    ...
    render = () => {
        ...
        const additionalValuesProp = additionalValues?.[id]
            ? additionalValues[id]
            : undefined;
