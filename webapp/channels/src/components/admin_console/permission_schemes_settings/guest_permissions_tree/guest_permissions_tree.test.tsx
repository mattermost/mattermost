...
describe("components/admin_console/permission_schemes_settings/permission_tree", () => {
    ...
    test("should match snapshot on default data", () => {
        const wrapper = shallow(<GuestPermissionsTree {...defaultProps} />);
        expect(wrapper).toMatchSnapshot();
    });

    test("should match snapshot on read only", () => {
        const wrapper = shallow(
            <GuestPermissionsTree {...defaultProps} readOnly={true} />,
        );
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test("should match snapshot on team scope", () => {
        const wrapper = shallow(
            <GuestPermissionsTree {...defaultProps} scope={"team_scope"} />,
        );
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test("should match snapshot on system scope", () => {
        const wrapper = shallow(
            <GuestPermissionsTree {...defaultProps} scope={"system_scope"} />,
        );
        expect(wrapper).toMatchSnapshot();
    });

    test("should match snapshot with parentRole with permissions", () => {
        const defaultParentRole = TestHelper.getRoleMock({
    ...
    test("should ask to toggle on row toggle", () => {
        ...
        const wrapper = shallow(
            <GuestPermissionsTree {...defaultProps} onToggle={onToggle} />,
        );
        wrapper.find(PermissionGroup).first().prop("onChange")([
            "test_permission",
            "test_permission2",
        ]);
    ...
    test("should match snapshot on license without LDAPGroups", () => {
        const wrapper = shallow(
            <GuestPermissionsTree {...defaultProps} license={{}} />,
        );
        expect(wrapper).toMatchSnapshot();
    });
});
