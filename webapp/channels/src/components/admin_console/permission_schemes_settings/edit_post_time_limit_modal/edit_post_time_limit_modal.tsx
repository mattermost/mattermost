...
type Props = {
    ...
    actions: {
        updateConfig: (
            config: AdminConfig,
        ) => ActionFunc & { error?: ClientErrorPlaceholder };
    };
};
...
export default function EditPostTimeLimitModal(props: Props) {
    ...
    const [postEditTimeLimit, setPostEditTimeLimit] = useState<number>(
        ServiceSettings?.PostEditTimeLimit ||
            Constants.UNSET_POST_EDIT_TIME_LIMIT,
    );
    const [alwaysAllowPostEditing, setAlwaysAllowPostEditing] = useState(
        postEditTimeLimit < 0,
    );
    ...
    const save = async () => {
        ...
        if (
            isNaN(postEditTimeLimit) ||
            postEditTimeLimit < 0 ||
            postEditTimeLimit > INT32_MAX
        ) {
            setErrorMessage(
                localizeMessage(
        ...
        newConfig.ServiceSettings.PostEditTimeLimit = alwaysAllowPostEditing
            ? Constants.UNSET_POST_EDIT_TIME_LIMIT
            : postEditTimeLimit;
    ...
    const handleOptionChange = ({
        ...
        setAlwaysAllowPostEditing(
            currentTarget.value === Constants.ALLOW_EDIT_POST_ALWAYS,
        );
    };
    ...
    const handleSecondsChange = ({
        ...
        setPostEditTimeLimit(parseInt(currentTarget.value, 10));
                ...
                <div className="pl-3 pb-3 pt-3">
                    ...
                    <div className="pt-2">
                        ...
                        <input
                            ...
                            value={
                                alwaysAllowPostEditing ? "" : postEditTimeLimit
                            }
                        />
                        <label htmlFor="timelimit">
                            <FormattedMessage
                    ...
                    <FormattedMessage
                        id={
                            saving
                                ...
                                : t("edit_post.time_limit_modal.save_button")
                        }
