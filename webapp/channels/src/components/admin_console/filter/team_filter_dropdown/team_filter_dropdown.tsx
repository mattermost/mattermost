...
const getFilteredTeams = createSelector(
    ...
    (term: string, teams: Team[]) => {
        return teams.filter(
            (team: Team) => team?.display_name?.toLowerCase().includes(term),
        );
    },
);
...
class TeamFilterDropdown extends React.PureComponent<Props, State> {
    ...
    togglePopover = (
        ...
        const selectedTeamIds = this.props.option.values.team_ids
            .value as string[];
        const selectedTeams = getSelectedTeams(
            selectedTeamIds,
            this.props.teams,
        );
        const savedSelectedTeams = selectedTeams.sort((a, b) =>
            a.display_name.localeCompare(b.display_name),
        );
        this.setState(
            { show: true, savedSelectedTeams, searchTerm: "" },
            () => {
    ...
    searchTeams = async (term: string, page: number) => {
        ...
        if (response?.data) {
            ...
            searchResults =
                page > 0
                    ? this.state.searchResults.concat(data.teams)
                    : data.teams;
        ...
        this.searchRetryId = window.setTimeout(
            this.searchTeams.bind(null, term, page),
            this.searchRetryInterval,
        );
    };

    searchTeamsDebounced = debounce(
        (page, term) => this.searchTeams(term, page),
        INITIAL_SEARCH_RETRY_TIMEOUT,
        false,
        () => {},
    );
    ...
    resetTeams = () => {
        ...
        this.props.updateValues(
            { team_ids: { name: "Teams", value: [] } },
            "teams",
        );
    };

    toggleTeam = (checked: boolean, teamId: string) => {
        const prevSelectedTeamIds = this.props.option.values.team_ids
            .value as string[];
        let selectedTeamIds;
        if (checked) {
        ...
        this.props.updateValues(
            { team_ids: { name: "Teams", value: selectedTeamIds } },
            "teams",
        );
    };
        ...
        selectedTeams.forEach((team, index) => {
            buttonOverflowed =
                ...
                !(
                    ...
                    buttonText.length + team.display_name.length
                );
            if (index === 0) {
    ...
    render() {
        ...
        if (this.state.searchTerm.length === 0) {
            visibleTeams = this.props.teams
                .slice(0, (this.state.page + 1) * TEAMS_PER_PAGE)
                .filter(
                    (team) =>
                        !this.state.savedSelectedTeams.some(
                            (selectedTeam) => selectedTeam.id === team.id,
                        ),
                );
            selectedTeams =
                this.state.savedSelectedTeams.map(createFilterCheckbox);
        } else {
            visibleTeams = getFilteredTeams(
                this.state.searchTerm,
                visibleTeams,
            );
        }
        const teamsToDisplay = visibleTeams.map(createFilterCheckbox);
        const chevron = this.state.show ? (
        ...
        return (
            <div className="FilterList FilterList__full">
                <div className="FilterList_name">{this.props.option.name}</div>

                <div className="TeamFilterDropdown" ref={this.ref}>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        {selectedTeamIds.length > 0 && (
                            <i
                                className={
                                    "TeamFilterDropdownButton_clear fa fa-times-circle"
                                }
                    ...
                    <div
                        className={
                            this.state.show
                                ? "TeamFilterDropdownOptions TeamFilterDropdownOptions__active"
                    ...
                    >
                        ...
                        {selectedTeamIds.length === 0 && (
                            <div className="TeamFilterDropdown_allTeams">
                                <FormattedMessage
                        ...
                        >
                            ...
                            {selectedTeams.length > 0 && (
                                <div className="TeamFilterDropdown_divider" />
                            )}
