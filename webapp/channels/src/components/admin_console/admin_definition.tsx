...
import BillingHistory, {
    searchableStrings as billingHistorySearchableStrings,
} from "./billing/billing_history";
import BillingSubscriptions, {
    searchableStrings as billingSubscriptionSearchableStrings,
} from "./billing/billing_subscriptions";
import CompanyInfo, {
    searchableStrings as billingCompanyInfoSearchableStrings,
} from "./billing/company_info";
import CompanyInfoEdit from "./billing/company_info_edit";
import BleveSettings, {
    searchableStrings as bleveSearchableStrings,
} from "./bleve_settings";
...
import ClusterSettings, {
    searchableStrings as clusterSearchableStrings,
} from "./cluster_settings";
import CustomEnableDisableGuestAccountsSetting from "./custom_enable_disable_guest_accounts_setting";
import CustomTermsOfServiceSettings from "./custom_terms_of_service_settings";
import {
    messages as customTermsOfServiceMessages,
    searchableStrings as customTermsOfServiceSearchableStrings,
} from "./custom_terms_of_service_settings/custom_terms_of_service_settings";
...
import DatabaseSettings, {
    searchableStrings as databaseSearchableStrings,
} from "./database_settings";
import ElasticSearchSettings, {
    searchableStrings as elasticSearchSearchableStrings,
...
import FeatureFlags, {
    messages as featureFlagsMessages,
} from "./feature_flags";
...
import MessageExportSettings, {
    searchableStrings as messageExportSearchableStrings,
} from "./message_export_settings";
import OpenIdConvert from "./openid_convert";
import PasswordSettings, {
    searchableStrings as passwordSearchableStrings,
...
import PushNotificationsSettings, {
    searchableStrings as pushSearchableStrings,
} from "./push_settings";
import SecureConnections, {
    searchableStrings as secureConnectionsSearchableStrings,
} from "./secure_connections";
...
import SessionLengthSettings, {
    searchableStrings as sessionLengthSearchableStrings,
} from "./session_length_settings";
import SystemProperties, {
    searchableStrings as systemPropertiesSearchableStrings,
...
import type {
    Check,
    AdminDefinition as AdminDefinitionType,
    ConsoleAccess,
} from "./types";
...
export const it = {
    ...
    stateMatches:
        ...
        (config: Partial<AdminConfig>, state: any) =>
            state[key].match(regex),
    stateEquals:
        (key: string, value: any) =>
        (config: Partial<AdminConfig>, state: any) =>
            state[key] === value,
    stateIsTrue: (key: string) => (config: Partial<AdminConfig>, state: any) =>
        Boolean(state[key]),
    stateIsFalse: (key: string) => (config: Partial<AdminConfig>, state: any) =>
        !state[key],
    configIsTrue:
        (group: keyof Partial<AdminConfig>, setting: string) =>
        (config: Partial<AdminConfig>) =>
            Boolean((config[group] as any)?.[setting]),
    configIsFalse:
        (group: keyof Partial<AdminConfig>, setting: string) =>
        (config: Partial<AdminConfig>) =>
            !(config[group] as any)?.[setting],
    configContains:
        (group: keyof Partial<AdminConfig>, setting: string, word: string) =>
        (config: Partial<AdminConfig>) =>
            Boolean((config[group] as any)?.[setting]?.includes(word)),
    enterpriseReady: (
        config: Partial<AdminConfig>,
        state: any,
        license?: ClientLicense,
        enterpriseReady?: boolean
    ) => Boolean(enterpriseReady),
    licensed: (
        config: Partial<AdminConfig>,
        state: any,
        license?: ClientLicense
    ...
    cloudLicensed: (
        ...
        state: any,
        license?: ClientLicense
    ) => Boolean(license?.IsLicensed && isCloudLicense(license)),
    licensedForFeature:
        (feature: string) =>
        (config: Partial<AdminConfig>, state: any, license?: ClientLicense) =>
    ...
    userHasReadPermissionOnResource:
        ...
        (
            ...
            (consoleAccess?.read as any)?.[key],
    userHasReadPermissionOnSomeResources: (
        key: string | { [key: string]: string }
    ) =>
        Object.values(key).some((resource) =>
            it.userHasReadPermissionOnResource(resource)
        ),
    userHasWritePermissionOnResource:
        ...
        (
            ...
            (consoleAccess?.write as any)?.[key],
    isSystemAdmin: (
        config: Partial<AdminConfig>,
        state: any,
        license?: ClientLicense,
...
export const validators = {
    isRequired: (text: MessageDescriptor | string) => (value: string) =>
        new ValidationResult(Boolean(value), text),
    minValue:
        (min: number, text: MessageDescriptor | string) => (value: number) =>
            new ValidationResult(value >= min, text),
    maxValue:
        (max: number, text: MessageDescriptor | string) => (value: number) =>
            new ValidationResult(value <= max, text),
};
...
const getRestrictedIndicator = (
    ...
    value: (cloud: CloudState) => (
        <RestrictedIndicator
            ...
            blocked={
                displayBlocked ||
                !(cloud?.subscription?.is_free_trial === "true")
            }
    ...
    shouldDisplay: (
        license: ClientLicense,
        subscriptionProduct: Product | undefined
    ) =>
        displayBlocked ||
        (isCloudLicense(license) &&
...
const adminDefinitionMessages = defineMessages({
    ...
    ip_filtering_title: {
        id: "admin.sidebar.ip_filtering",
        defaultMessage: "IP Filtering",
    },
    ...
    cache_type_title: {
        id: "admin.cacheSettings.cacheTypeTitle",
        defaultMessage: "Cache Type",
    },
    cache_type_desc: {
        id: "admin.cacheSettings.cacheTypeDesc",
        defaultMessage: 'The type of the cache backend. E.g.: "redis" or "lru"',
    },

    redis_address_title: {
        id: "admin.cacheSettings.redisAddress",
        defaultMessage: "Redis Address",
    },
    redis_address_desc: {
        id: "admin.cacheSettings.redisAddressDesc",
        defaultMessage:
            'The hostname:port of the Redis server. E.g.: "localhost:6379"',
    },
    redis_address_placeholder: {
        id: "admin.cacheSettings.redisAddressPlaceholder",
        defaultMessage: "localhost:6379",
    },

    redis_password_title: {
        id: "admin.cacheSettings.redisPassword",
        defaultMessage: "Redis Password",
    },
    redis_password_desc: {
        id: "admin.cacheSettings.redisPasswordDesc",
        defaultMessage: "The password of the Redis server.",
    },

    redis_db_title: {
        id: "admin.cacheSettings.redisDB",
        defaultMessage: "Redis DB",
    },
    redis_db_desc: {
        id: "admin.cacheSettings.redisDBDesc",
        defaultMessage: 'The database of the Redis server. E.g.: "0"',
    },
    redis_db_placeholder: {
        id: "admin.cacheSettings.redisDBPlaceholder",
        defaultMessage: "0",
    },

    redis_clientcache_title: {
        id: "admin.cacheSettings.redisClientCache",
        defaultMessage: "Disable Client Cache",
    },
    redis_clientcache_desc: {
        id: "admin.cacheSettings.redisClientCacheDesc",
        defaultMessage: "When true, client-side caching is disabled.",
    },
});
const AdminDefinition: AdminDefinitionType = {
    about: {
        icon: <InformationOutlineIcon size={16} color={"currentColor"} />,
        ...
        subsections: {
            license: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.ABOUT.EDITION_AND_LICENSE
                    )
                ),
                isDisabled: it.not(
    ...
    billing: {
        icon: <CreditCardOutlineIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
        ...
        subsections: {
            subscription: {
                ...
                isHidden: it.not(it.licensedForFeature("Cloud")),
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource("billing")
                ),
            },
            billing_history: {
                url: "billing/billing_history",
                title: defineMessage({
                    id: "admin.sidebar.billing_history",
    ...
    reporting: {
        icon: <ChartBarIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
            id: "admin.sidebar.reporting",
            defaultMessage: "Reporting",
        }),
        isHidden: it.not(
            it.userHasReadPermissionOnSomeResources(RESOURCE_KEYS.REPORTING)
        ),
        subsections: {
            workspace_optimization: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.REPORTING.SITE_STATISTICS
                    )
                ),
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource(
            ...
            team_statistics: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.REPORTING.TEAM_STATISTICS
                    )
                ),
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource(
            ...
            server_logs: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.REPORTING.SERVER_LOGS
                        )
                    )
                ),
                isDisabled: it.not(
    ...
    user_management: {
        icon: <AccountMultipleOutlineIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
            id: "admin.sidebar.userManagement",
            defaultMessage: "User Management",
        }),
        isHidden: it.not(
            it.userHasReadPermissionOnSomeResources(
                RESOURCE_KEYS.USER_MANAGEMENT
            )
        ),
        subsections: {
            system_users: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.USER_MANAGEMENT.USERS
                    )
                ),
                schema: {
                    id: "SystemUsers",
            ...
            group_detail: {
                ...
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource(
                        RESOURCE_KEYS.USER_MANAGEMENT.GROUPS
                    )
                ),
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
            ...
            team_detail: {
                ...
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource(
                        RESOURCE_KEYS.USER_MANAGEMENT.TEAMS
                    )
                ),
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
            ...
            systemScheme: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.USER_MANAGEMENT.PERMISSIONS
                    )
                ),
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource(
            ...
            system_role: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.USER_MANAGEMENT.SYSTEM_ROLES
                        )
                    )
                ),
                isDisabled: it.not(
    ...
    environment: {
        icon: <ServerVariantIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
            id: "admin.sidebar.environment",
            defaultMessage: "Environment",
        }),
        isHidden: it.not(
            it.userHasReadPermissionOnSomeResources(RESOURCE_KEYS.ENVIRONMENT)
        ),
        subsections: {
            web_server: {
                ...
                schema: {
                    id: "ServiceSettings",
                    name: defineMessage({
                        id: "admin.environment.webServer",
                        defaultMessage: "Web Server",
                    }),
                    settings: [
                        {
                        ...
                        },
                        {
                            ...
                            action: testSiteURL,
                            label: defineMessage({
                                id: "admin.service.testSiteURL",
                                defaultMessage: "Test Live URL",
                            }),
                            loading: defineMessage({
                                id: "admin.service.testSiteURLTesting",
                                defaultMessage: "Testing...",
                            }),
                            error_message: defineMessage({
                                id: "admin.service.testSiteURLFail",
                                defaultMessage: "Test unsuccessful: {error}",
                            }),
                            success_message: defineMessage({
                                id: "admin.service.testSiteURLSuccess",
                                defaultMessage:
                                    "Test successful. This is a valid URL.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.ENVIRONMENT.WEB_SERVER
                        ...
                        },
                        {
                            ...
                            disabled_help_text: defineMessage({
                                id: "admin.service.forward80To443Description.disabled",
                                defaultMessage:
                                    "Forwards all insecure traffic from port 80 to secure port 443. Not recommended when using a proxy server. This setting cannot be enabled until your server is [listening](#ServiceSettings.ListenAddress) on port 443.",
                            }),
                        ...
                        },
                        {
                            ...
                            help_text:
                                DefinitionConstants.CONNECTION_SECURITY_HELP_TEXT_WEBSERVER,
                            options: [
                                {
                                    value: "",
                                    display_name: defineMessage({
                        ...
                        },
                        {
                            ...
                            disabled_help_text: defineMessage({
                                id: "admin.service.useLetsEncryptDescription.disabled",
                                defaultMessage:
                                    "Enable the automatic retrieval of certificates from Let's Encrypt. The certificate will be retrieved when a client attempts to connect from a new domain. This will work with multiple domains. This setting cannot be enabled unless the [Forward port 80 to 443](#ServiceSettings.Forward80To443) setting is set to true.",
                            }),
                            disabled_help_text_markdown: true,
                            isDisabled: it.any(
                                it.not(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.maximumPayloadSize",
                                defaultMessage: "Maximum Payload Size (Bytes):",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.maximumPayloadSizeDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            help_text:
                                DefinitionConstants.WEBSERVER_MODE_HELP_TEXT,
                            options: [
                                {
                                    value: "gzip",
                                    display_name: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.managedResourcePaths",
                                defaultMessage: "Managed Resource Paths:",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.managedResourcePathsDescription",
                                defaultMessage:
                            ...
                            help_text_values: {
                                link: (msg: string) => (
                                    <ExternalLink
                                        ...
                                        href={
                                            DocLinks.DESKTOP_MANAGED_RESOURCES
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
            ...
            elasticsearch: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.ELASTICSEARCH
                        )
                    )
                ),
            ...
            storage: {
                ...
                schema: {
                    id: "FileSettings",
                    name: defineMessage({
                        id: "admin.environment.fileStorage",
                        defaultMessage: "File Storage",
                    }),
                    settings: [
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    'Storage system where files and image attachments are saved. Selecting "Amazon S3" enables fields to enter your Amazon credentials and bucket details. Selecting "Local File System" enables the field to specify a local file directory.',
                            }),
                            help_text_markdown: true,
                            options: [
                                {
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "Directory to which files and images are written. If blank, defaults to ./data/.",
                            }),
                            placeholder: defineMessage({
                                id: "admin.image.localExample",
                                defaultMessage: 'E.g.: "./data/"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.image.maxFileSizeExample",
                                defaultMessage: "50",
                            }),
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.image.extractContentTitle",
                                defaultMessage:
                                    "Enable document search by content:",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.extractContentDescription",
                                defaultMessage:
                            ...
                            help_text_values: {
                                link: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DocLinks.CONFIGURE_DOCUMENT_CONTENT_SEARCH
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.image.archiveRecursionTitle",
                                defaultMessage:
                                    "Enable searching content of documents within ZIP files:",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.archiveRecursionDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                ...
                                defaultMessage: 'E.g.: "mattermost-media"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.image.amazonS3PathPrefixTitle",
                                defaultMessage: "Amazon S3 Path Prefix:",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.amazonS3PathPrefixDescription",
                                defaultMessage:
                                    "Prefix you selected for your S3 bucket in AWS.",
                            }),
                            placeholder: defineMessage({
                                id: "admin.image.amazonS3PathPrefixExample",
                                defaultMessage:
                                    'E.g.: "subdir1/" or you can leave it .',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                ...
                                defaultMessage: 'E.g.: "AKIADTOVBGERKLCBV"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.image.amazonS3EndpointExample",
                                defaultMessage: 'E.g.: "s3.amazonaws.com"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                ...
                                defaultMessage:
                                    'E.g.: "jcuS8PuvcpGhpgHhlcpT1Mx42pnqMxQY"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            help_text_markdown: false,
                            isHidden: it.not(
                                it.licensedForFeature("Compliance")
                            ),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.image.amazonS3StorageClassTitle",
                                defaultMessage: "Amazon S3 Storage Class:",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.amazonS3StorageClassDescription",
                                defaultMessage:
                                    "Storage class for your S3 Compatible Storage provider. Defaults to empty.",
                            }),
                            placeholder: defineMessage({
                                id: "admin.image.amazonS3StorageClassExample",
                                defaultMessage:
                                    'E.g.: "STANDARD" or "STANDARD_IA"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
            ...
            export_storage: {
                ...
                title: defineMessage({
                    id: "admin.sidebar.exportStorage",
                    defaultMessage: "Export Storage",
                }),
                isHidden: it.any(
                    ...
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.exportStorage.dedicatedExportStore",
                                defaultMessage:
                                    "Enable Dedicated Export Store:",
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.exportStorage.exportDriverName",
                                defaultMessage: "Export Storage Driver:",
                            }),
                            isDisabled: true,
                            isHidden: it.stateEquals(
                                "FileSettings.DedicatedExportStore",
                                false
                            ),
                            options: [
                                {
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.exportStorage.exportDirectory",
                                defaultMessage: "Export Directory",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.exportDirectoryDescription",
                                defaultMessage:
                                    "Directory to which files are written. If blank, defaults to ./data/.",
                            }),
                            placeholder: defineMessage({
                                id: "admin.image.localExample",
                                defaultMessage: 'E.g.: "./data/"',
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.image.amazonS3BucketExample",
                                defaultMessage: 'E.g.: "mattermost-export"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.image.amazonS3SignV2",
                                defaultMessage: "Enable Sign V2",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.amazonS3SignV2Description",
                                defaultMessage:
                                    "When true, use Sign V2 for Amazon S3 connections",
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
            ...
            image_proxy: {
                ...
                schema: {
                    id: "ImageProxy",
                    name: defineMessage({
                        id: "admin.environment.imageProxy",
                        defaultMessage: "Image Proxy",
                    }),
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.image.enableProxy",
                                defaultMessage: "Enable Image Proxy:",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.enableProxyDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                ...
                                defaultMessage: "Remote Image Proxy URL:",
                            }),
                            help_text: defineMessage({
                                id: "admin.image.proxyURLDescription",
                                defaultMessage:
                                    "URL of your remote image proxy server.",
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                                ...
                                it.stateIsFalse("ImageProxySettings.Enable"),
                                it.stateEquals(
                                    "ImageProxySettings.ImageProxyType",
                                    "local"
                                )
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                ...
                                defaultMessage: "Remote Image Proxy Options:",
                            }),
                            help_text: defineMessage({
            ...
            smtp: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.SMTP
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.smtpServer.title",
                                defaultMessage: "SMTP Server:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.smtp.smtpServer.placeholder",
                                defaultMessage:
                                    'Ex: "smtp.yourcompany.com", "email-smtp.us-east-1.amazonaws.com"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.smtp.smtpServer.description",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.smtpPort.title",
                                defaultMessage: "SMTP Server Port:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.smtp.smtpPort.placeholder",
                                defaultMessage: 'Ex: "25", "465", "587"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.smtp.smtpPort.description",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.smtpAuth.title",
                                defaultMessage: "Enable SMTP Authentication:",
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.smtp.smtpAuth.description",
                                defaultMessage:
                                    "When true, SMTP Authentication is enabled.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.ENVIRONMENT.SMTP
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.smtpUsername.title",
                                defaultMessage: "SMTP Server Username:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.smtp.smtpUsername.placeholder",
                                defaultMessage:
                                    'Ex: "admin@yourcompany.com", "AKIADTOVBGERKLCBV"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.smtp.smtpUsername.description",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.smtpPassword.title",
                                defaultMessage: "SMTP Server Password:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.smtp.smtpPassword.placeholder",
                                defaultMessage:
                                    'Ex: "yourpassword", "jcuS8PuvcpGhpgHhlcpT1Mx42pnqMxQY"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.smtp.smtpPassword.description",
                                defaultMessage:
                                    "Obtain this credential from administrator setting up your email server.",
                            }),
                            isDisabled: it.any(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.connectionSecurity.title",
                                defaultMessage: "Connection Security:",
                            }),
                            help_text:
                                DefinitionConstants.CONNECTION_SECURITY_HELP_TEXT_EMAIL,
                            options: [
                                {
                                    value: "",
                                    display_name: defineMessage({
                                        id: "admin.environment.smtp.connectionSecurity.option.none",
                                        defaultMessage: "None",
                                    }),
                                },
                                {
                                    value: "TLS",
                                    display_name: defineMessage({
                                        id: "admin.environment.smtp.connectionSecurity.option.tls",
                                        defaultMessage: "TLS (Recommended)",
                                    }),
                                },
                                {
                                    value: "STARTTLS",
                                    display_name: defineMessage({
                                        id: "admin.environment.smtp.connectionSecurity.option.starttls",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.connectionSmtpTest",
                                defaultMessage: "Test Connection",
                            }),
                            loading: defineMessage({
                                id: "admin.environment.smtp.testing",
                                defaultMessage: "Testing...",
                            }),
                            error_message: defineMessage({
                                id: "admin.environment.smtp.smtpFail",
                                defaultMessage:
                                    "Connection unsuccessful: {error}",
                            }),
                            success_message: defineMessage({
                                id: "admin.environment.smtp.smtpSuccess",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.skipServerCertificateVerification.title",
                                defaultMessage:
                                    "Skip Server Certificate Verification:",
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.smtp.skipServerCertificateVerification.description",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.smtp.enableSecurityFixAlert.title",
                                defaultMessage: "Enable Security Alerts:",
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.smtp.enableSecurityFixAlert.description",
                                defaultMessage:
            ...
            push_notification_server: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.PUSH_NOTIFICATION_SERVER
                        )
                    )
                ),
                isDisabled: it.not(
            ...
            high_availability: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.HIGH_AVAILABILITY
                        )
                    )
                ),
                    ...
                    settings: [
                            ...
                            options: [
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.cacheSettings.cacheType.lru",
                                        defaultMessage: "LRU",
                                    }),
                                },
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.cacheSettings.cacheType.redis",
                        ...
                        },
                        {
                            ...
                            help_text:
                                adminDefinitionMessages.redis_address_desc,
                            placeholder:
                                adminDefinitionMessages.redis_address_placeholder,
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                                        RESOURCE_KEYS.ENVIRONMENT
                                            .HIGH_AVAILABILITY
                                    )
                                ),
                                it.not(
                        ...
                        },
                        {
                            ...
                            placeholder:
                                adminDefinitionMessages.redis_db_placeholder,
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                                        RESOURCE_KEYS.ENVIRONMENT
            ...
            rate_limiting: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.RATE_LIMITING
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.rate.noteDescription",
                                defaultMessage:
                                    "Changing properties other than Site URL in this section will require a server restart before taking effect.",
                            }),
                            banner_type: "info",
                        },
                        {
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                ...
                                defaultMessage: 'E.g.: "10000"',
                            }),
                            help_text: defineMessage({
                                id: "admin.rate.memoryDescription",
                                defaultMessage:
                                    'Maximum number of users sessions connected to the system as determined by "Vary rate limit by remote address" and "Vary rate limit by HTTP header".',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "When true, rate limit API access by user athentication token.",
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            isDisabled: it.any(
                                ...
                                it.stateEquals(
                                    "RateLimitSettings.VaryByRemoteAddr",
                                    true
                                )
                            ),
                        },
            ...
            logging: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.LOGGING
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.log.consoleJsonTitle",
                                defaultMessage: "Output console logs as JSON:",
                            }),
                            help_text: defineMessage({
                                id: "admin.log.jsonDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.log.fileJsonTitle",
                                defaultMessage: "Output file logs as JSON:",
                            }),
                            help_text: defineMessage({
                                id: "admin.log.jsonDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.log.AdvancedLoggingJSONTitle",
                                defaultMessage: "Advanced Logging:",
                            }),
                            help_text: defineMessage({
                                id: "admin.log.AdvancedLoggingJSONDescription",
                                defaultMessage:
                            ...
                            placeholder: defineMessage({
                                id: "admin.log.AdvancedLoggingJSONPlaceholder",
                                defaultMessage: "Enter your JSON configuration",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.ENVIRONMENT.LOGGING
                            ...
                            onConfigLoad: (configVal) =>
                                JSON.stringify(configVal, null, "  "),
            ...
            session_lengths: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.SESSION_LENGTHS
                        )
                    )
                ),
            ...
            metrics: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.PERFORMANCE_MONITORING
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                            ...
                            help_text_values: {
                                link: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DocLinks.SETUP_PERFORMANCE_MONITORING
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.metrics.enableClientMetricsTitle",
                                defaultMessage:
                                    "Enable Client Performance Monitoring:",
                            }),
                            help_text: defineMessage({
                                id: "admin.metrics.enableClientMetricsDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.metrics.listenAddressEx",
                                defaultMessage: 'E.g.: ":8067"',
                            }),
                            help_text: defineMessage({
                                id: "admin.metrics.listenAddressDesc",
                                defaultMessage:
                                    "The address the server will listen on to expose performance metrics.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.ENVIRONMENT
            ...
            developer: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.ENVIRONMENT.DEVELOPER
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "When true, /test slash command is enabled to load test accounts, data and text formatting. Changing this requires a server restart before taking effect.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.ENVIRONMENT.DEVELOPER
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.performanceDebuggingTitle",
                                defaultMessage:
                                    "Enable Client Performance Debugging: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.performanceDebuggingDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.service.internalConnectionsEx",
                                defaultMessage:
    ...
    site: {
        icon: <CogOutlineIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
            id: "admin.sidebar.site",
            defaultMessage: "Site Configuration",
        }),
        isHidden: it.not(
            it.userHasReadPermissionOnSomeResources(RESOURCE_KEYS.SITE)
        ),
        subsections: {
            customization: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.SITE.CUSTOMIZATION
                    )
                ),
                schema: {
                    id: "Customization",
                    name: defineMessage({
                        id: "admin.site.customization",
                        defaultMessage: "Customization",
                    }),
                    settings: [
                        {
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.support.enableAskCommunityTitle",
                                defaultMessage: "Enable Ask Community Link:",
                            }),
                            help_text: defineMessage({
                                id: "admin.support.enableAskCommunityDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                ...
                                defaultMessage: "Terms of Use Link:",
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.support.forgotPasswordTitle",
                                defaultMessage: "Forgot Password Custom Link:",
                            }),
                            help_text: defineMessage({
                                id: "admin.support.forgotPasswordDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "The URL for the Report a Problem link in the Help Menu. If this field is empty, the link is removed from the Help Menu.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.SITE.CUSTOMIZATION
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.enableDesktopLandingPageTitle",
                                defaultMessage:
                                    "Enable Desktop App Landing Page:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.enableDesktopLandingPageDesc",
                                defaultMessage:
            ...
            localization: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.SITE.LOCALIZATION
                    )
                ),
                schema: {
                    id: "LocalizationSettings",
                    name: defineMessage({
                        id: "admin.site.localization",
                        defaultMessage: "Localization",
                    }),
                    settings: [
                        {
                ...
                schema: {
                    ...
                    settings: [
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.team.maxChannelsExample",
                                defaultMessage: 'E.g.: "100"',
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.team.enableJoinLeaveMessageTitle",
                                defaultMessage:
                                    "Enable join/leave messages by default:",
                            }),
                            help_text: defineMessage({
                                id: "admin.team.enableJoinLeaveMessageDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "Set how to display users' names in posts and the Direct Messages list.",
                            }),
                            options: [
                                {
                                    value: Constants.TEAMMATE_NAME_DISPLAY
                                        .SHOW_USERNAME,
                                    display_name: defineMessage({
                                        id: "admin.team.showUsername",
                                        defaultMessage:
                                            "Show username (default)",
                                    }),
                                },
                                {
                                    value: Constants.TEAMMATE_NAME_DISPLAY
                                        .SHOW_NICKNAME_FULLNAME,
                                    display_name: defineMessage({
                                        id: "admin.team.showNickname",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.lockTeammateNameDisplay",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "When true, allows users to view, share and search for content of channels that have been archived. Users can only view the content in channels of which they were a member before the channel was archived.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.SITE.USERS_AND_TEAMS
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.team.customUserStatusesTitle",
                                defaultMessage: "Enable Custom Statuses: ",
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.team.lastActiveTimeTitle",
                                defaultMessage: "Enable last active time: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.team.lastActiveTimeDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.team.customUserGroupsTitle",
                                defaultMessage: "Enable Custom User Groups: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.team.customUserGroupsDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.team.refreshPostStatsRunTimeTitle",
                                defaultMessage: "User Statistics Update Time:",
                            }),
                            help_text: defineMessage({
                                id: "admin.team.refreshPostStatsRunTimeDescription",
                                defaultMessage:
                            ...
                            placeholder: defineMessage({
                                id: "admin.team.refreshPostStatsRunTimeExample",
                                defaultMessage: 'E.g.: "00:00"',
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.SITE.USERS_AND_TEAMS
            ...
            notifications: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.SITE.NOTIFICATIONS
                    )
                ),
                schema: {
                    id: "notifications",
                    name: defineMessage({
                        id: "admin.environment.notifications",
                        defaultMessage: "Notifications",
                    }),
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.enableConfirmNotificationsToChannel.label",
                                defaultMessage:
                                    "Show @channel, @all, @here and group mention confirmation dialog:",
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.notifications.enableConfirmNotificationsToChannel.help",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                id: "admin.environment.notifications.enable.help",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.enablePreviewModeBanner.label",
                                defaultMessage: "Enable Preview Mode Banner:",
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.notifications.enablePreviewModeBanner.help",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.enableEmailBatching.label",
                                defaultMessage: "Enable Email Batching:",
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.notifications.enableEmailBatching.help",
                                defaultMessage:
                            ...
                            options: [
                                {
                                    value: "full",
                                    display_name: defineMessage({
                                        id: "admin.environment.notifications.contents.full",
                                        defaultMessage:
                                            "Send full message contents",
                                    }),
                                },
                                {
                                    value: "generic",
                                    display_name: defineMessage({
                                        id: "admin.environment.notifications.contents.generic",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.notificationDisplay.label",
                                defaultMessage: "Notification Display Name:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.notifications.notificationDisplay.placeholder",
                                defaultMessage:
                                    'Ex: "Mattermost Notification", "System", "No-Reply"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.notifications.notificationDisplay.help",
                                defaultMessage:
                            ...
                            validate: validators.isRequired(
                                defineMessage({
                                    id: "admin.environment.notifications.notificationDisplay.required",
                                    defaultMessage:
                                        '"Notification Display Name" is required',
                                })
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.feedbackEmail.label",
                                defaultMessage: "Notification From Address:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.notifications.feedbackEmail.placeholder",
                                defaultMessage:
                                    'Ex: "mattermost@yourcompany.com", "admin@yourcompany.com"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.notifications.feedbackEmail.help",
                                defaultMessage:
                            ...
                            validate: validators.isRequired(
                                defineMessage({
                                    id: "admin.environment.notifications.feedbackEmail.required",
                                    defaultMessage:
                                        '"Notification From Address" is required',
                                })
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.supportEmail.label",
                                defaultMessage: "Support Email Address:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.notifications.supportAddress.placeholder",
                                defaultMessage:
                            ...
                            validate: validators.isRequired(
                                defineMessage({
                                    id: "admin.environment.notifications.supportEmail.required",
                                    defaultMessage:
                                        '"Support Email Address" is required',
                                })
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.replyToAddress.label",
                                defaultMessage:
                                    "Notification Reply-To Address:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.notifications.replyToAddress.placeholder",
                                defaultMessage:
                                    'Ex: "mattermost@yourcompany.com", "admin@yourcompany.com"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.notifications.replyToAddress.help",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.feedbackOrganization.label",
                                defaultMessage:
                                    "Notification Footer Mailing Address:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.environment.notifications.feedbackOrganization.placeholder",
                                defaultMessage:
                                    'Ex: "© ABC Corporation, 565 Knight Way, Palo Alto, California, 94305, USA"',
                            }),
                            help_text: defineMessage({
                                id: "admin.environment.notifications.feedbackOrganization.help",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.environment.notifications.pushContents.label",
                            ...
                            options: [
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.environment.notifications.pushContents.genericNoChannel",
                                        defaultMessage:
                                ...
                                },
                                {
                                    value: "generic",
                                    display_name: defineMessage({
                                        id: "admin.environment.notifications.pushContents.generic",
                                        defaultMessage:
                                ...
                                },
                                {
                                    value: "full",
                                    display_name: defineMessage({
                                        id: "admin.environment.notifications.pushContents.full",
                                        defaultMessage:
                                            "Full message content sent in the notification payload",
                                    }),
                                },
                            ],
                            isHidden: it.licensedForFeature(
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                id: "admin.environment.notifications.pushContents.withIdLoaded.help",
                                defaultMessage:
                            ...
                            options: [
                                ...
                                },
                                {
                                    value: "id_loaded",
                                    display_name: defineMessage({
                                        id: "admin.environment.notifications.pushContents.idLoaded",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.metrics.enableNotificationMetricsTitle",
                                defaultMessage:
            ...
            announcement_banner: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.SITE.ANNOUNCEMENT_BANNER
                        )
                    )
                ),
                schema: {
                    ...
                    name: defineMessage({
                        id: "admin.site.announcementBanner",
                        defaultMessage: "System-wide Notifications",
                    }),
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.announcement.enableBannerTitle",
                                defaultMessage:
                                    "Enable System-wide Notifications:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.announcement.enableBannerDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.announcement.bannerTextTitle",
                                defaultMessage: "Banner Text:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.announcement.bannerTextDesc",
                                defaultMessage:
                                    "Text that will appear in the announcement banner.",
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                                ...
                                it.stateIsFalse(
                                    "AnnouncementSettings.EnableBanner"
                                )
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.announcement.bannerColorTitle",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.announcement.bannerTextColorTitle",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.announcement.allowBannerDismissalTitle",
            ...
            emoji: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(RESOURCE_KEYS.SITE.EMOJI)
                ),
                schema: {
                    id: "EmojiSettings",
                    name: defineMessage({
            ...
            posts: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(RESOURCE_KEYS.SITE.POSTS)
                ),
                schema: {
                    id: "PostSettings",
                    name: defineMessage({
                    ...
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.threadAutoFollow.title",
                                defaultMessage: "Automatically Follow Threads",
                            }),
                            help_text: defineMessage({
                            ...
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
                                )
                            ),
                            isHidden: it.licensedForFeature("Cloud"),
                        },
                            ...
                            options: [
                                {
                                    value: "disabled",
                                    display_name: defineMessage({
                                        id: "admin.experimental.collapsedThreads.off",
                                        defaultMessage: "Disabled",
                                    }),
                                },
                                {
                                    value: "default_off",
                                    display_name: defineMessage({
                                        id: "admin.experimental.collapsedThreads.default_off",
                                        defaultMessage: "Enabled (Default Off)",
                                    }),
                                },
                                {
                                    value: "default_on",
                                    display_name: defineMessage({
                                        id: "admin.experimental.collapsedThreads.default_on",
                                        defaultMessage: "Enabled (Default On)",
                                    }),
                                },
                                {
                                    value: "always_on",
                                    display_name: defineMessage({
                                        id: "admin.experimental.collapsedThreads.always_on",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.posts.postPriority.title",
                                defaultMessage: "Message Priority",
                            }),
                            help_text: defineMessage({
                                id: "admin.posts.postPriority.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.posts.persistentNotifications.title",
                                defaultMessage: "Persistent Notifications",
                            }),
                            help_text: defineMessage({
                                id: "admin.posts.persistentNotifications.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.posts.scheduledPosts.title",
                                defaultMessage: "Scheduled Posts",
                            }),
                            help_text: defineMessage({
                                id: "admin.posts.scheduledPosts.description",
                                defaultMessage:
                                    "When enabled, users can schedule and send messages in the future.",
                            }),
                            help_text_markdown: false,
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.posts.persistentNotificationsMaxRecipients.title",
                                defaultMessage:
                                    "Maximum number of recipients for persistent notifications",
                            }),
                            help_text: defineMessage({
                                id: "admin.posts.persistentNotificationsMaxRecipients.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.posts.persistentNotificationsInterval.title",
                                defaultMessage:
                                    "Frequency of persistent notifications",
                            }),
                            help_text: defineMessage({
                                id: "admin.posts.persistentNotificationsInterval.desc",
                                defaultMessage:
                            ...
                            validate: validators.minValue(
                                2,
                                defineMessage({
                                    id: "admin.posts.persistentNotificationsInterval.minValue",
                                    defaultMessage:
                                        "Frequency cannot not be set to less than 2 minutes",
                                })
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.posts.persistentNotificationsMaxCount.title",
                                defaultMessage:
                                    "Total number of persistent notification per post",
                            }),
                            help_text: defineMessage({
                                id: "admin.posts.persistentNotificationsMaxCount.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.posts.persistentNotificationsGuests.title",
                                defaultMessage:
                                    "Allow guests to send persistent notifications",
                            }),
                            help_text: defineMessage({
                                id: "admin.posts.persistentNotificationsGuests.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                ...
                                defaultMessage: "Enable website link previews:",
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.restrictLinkPreviewsTitle",
                                defaultMessage:
                                    "Disable website link previews from these domains:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.restrictLinkPreviewsDesc",
                                defaultMessage:
                                    "Link previews and image link previews will not be shown for the above list of comma-separated domains.",
                            }),
                            placeholder: defineMessage({
                                id: "admin.customization.restrictLinkPreviewsExample",
                                defaultMessage:
                                    'E.g.: "internal.mycompany.com, images.example.com"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.enablePermalinkPreviewsTitle",
                                defaultMessage: "Enable message link previews:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.enablePermalinkPreviewsDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.enableSVGsTitle",
                                defaultMessage: "Enable SVGs:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.enableSVGsDesc",
                                defaultMessage:
                                    "Enable previews for SVG file attachments and allow them to appear in messages. Enabling SVGs is not recommended in environments where not all users are trusted.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.SITE.POSTS
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.enableLatexTitle",
                                defaultMessage: "Enable Latex Rendering:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.enableLatexDesc",
                                defaultMessage:
                                    "Enable rendering of Latex in code blocks. If false, Latex code will be highlighted only. Enabling Latex is not recommended in environments where not all users are trusted.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.SITE.POSTS
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.enableInlineLatexTitle",
                                defaultMessage:
                                    "Enable Inline Latex Rendering:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.enableInlineLatexDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.maxMarkdownNodesTitle",
                                defaultMessage: "Max Markdown Nodes:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.maxMarkdownNodesDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                ...
                                defaultMessage:
                                    'E.g.: "7rAh6iwQCkV4cA1Gsg3fgGOXJAQ43QV"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.allowSyncedDrafts",
                                defaultMessage:
                                    "Enable server syncing of message drafts:",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.allowSyncedDraftsDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.customization.uniqueEmojiReactionLimitPerPost",
                                defaultMessage: "Unique Emoji Reaction Limit:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.customization.uniqueEmojiReactionLimitPerPostPlaceholder",
                                defaultMessage: "E.g.: 25",
                            }),
                            help_text: defineMessage({
                                id: "admin.customization.uniqueEmojiReactionLimitPerPostDesc",
                                defaultMessage:
                            ...
                            validate: (value) => {
                                const maxResult = validators.maxValue(
                                    500,
                                    defineMessage({
                                        id: "admin.customization.uniqueEmojiReactionLimitPerPost.maxValue",
                                        defaultMessage:
                                ...
                                const minResult = validators.minValue(
                                    0,
                                    defineMessage({
                                        id: "admin.customization.uniqueEmojiReactionLimitPerPost.minValue",
                                        defaultMessage:
            ...
            wrangler: {
                ...
                title: defineMessage({
                    id: "admin.sidebar.move_thread",
                    defaultMessage: "Move Thread (Beta)",
                }),
                isHidden: it.any(
                    it.not(
                        it.userHasReadPermissionOnResource(
                    ...
                    it.configIsFalse("FeatureFlags", "MoveThreadsEnabled"),
                    it.not(it.licensed)
                ),
                schema: {
                    id: "WranglerSettings",
                    name: defineMessage({
                        id: "admin.site.move_thread",
                        defaultMessage: "Move Thread (Beta)",
                    }),
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.PermittedMoveThreadRoles.title",
                                defaultMessage: "Permitted Roles",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.PermittedMoveThreadRoles.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            multiple: true,
                            label: defineMessage({
                                id: "admin.experimental.allowedEmailDomain.title",
                                defaultMessage: "Allowed Email Domain",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.allowedEmailDomain.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.moveThreadMaxCount.title",
                                defaultMessage: "Max Thread Count Move Size",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.moveThreadMaxCount.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.moveThreadToAnotherTeamEnable.title",
                                defaultMessage:
                                    "Enable Moving Threads To Different Teams",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.moveThreadToAnotherTeamEnable.desc",
                                defaultMessage:
                                    "Control whether Wrangler is permitted to move message threads from one team to another or not.",
                            }),
                            help_text_markdown: false,
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.moveThreadFromPrivateChannelEnable.title",
                                defaultMessage:
                                    "Enable Moving Threads From Private Channels",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.moveThreadFromPrivateChannelEnable.desc",
                                defaultMessage:
                                    "Control whether Wrangler is permitted to move message threads from private channels or not.",
                            }),
                            help_text_markdown: false,
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.moveThreadFromDirectMessageChannelEnable.title",
                                defaultMessage:
                                    "Enable Moving Threads From Direct Message Channels",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.moveThreadFromDirectMessageChannelEnable.desc",
                                defaultMessage:
                                    "Control whether Wrangler is permitted to move message threads from direct message channels or not.",
                            }),
                            help_text_markdown: false,
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.moveThreadFromGroupMessageChannelEnable.title",
                                defaultMessage:
                                    "Enable Moving Threads From Group Message Channels",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.moveThreadFromGroupMessageChannelEnable.desc",
                                defaultMessage:
                                    "Control whether Wrangler is permitted to move message threads from group message channels or not.",
                            }),
                            help_text_markdown: false,
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
            ...
            file_sharing_downloads: {
                ...
                isHidden: it.any(
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.SITE.FILE_SHARING_AND_DOWNLOADS
                        )
                    )
                ),
                schema: {
                    ...
                    name: defineMessage({
                        id: "admin.site.fileSharingDownloads",
                        defaultMessage: "File Sharing and Downloads",
                    }),
                    settings: [
                        {
            ...
            public_links: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.SITE.PUBLIC_LINKS
                        )
                    )
                ),
                schema: {
            ...
            notices: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.SITE.NOTICES
                    )
                ),
                schema: {
                    id: "NoticesSettings",
                    name: defineMessage({
                        id: "admin.site.notices",
                        defaultMessage: "Notices",
                    }),
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.notices.enableAdminNoticesTitle",
                                defaultMessage: "Enable Admin Notices: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.notices.enableAdminNoticesDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.notices.enableEndUserNoticesTitle",
                                defaultMessage: "Enable End User Notices: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.notices.enableEndUserNoticesDescription",
                                defaultMessage:
            ...
            ip_filtering: {
                ...
                isHidden: it.not(
                    it.all(
                        it.configIsTrue("FeatureFlags", "CloudIPFiltering"),
                        it.licensedForSku("enterprise")
                    )
                ),
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource(
                        RESOURCE_KEYS.SITE.IP_FILTERING
                    )
                ),
            ...
            secure_connections: {
                ...
                title: defineMessage({
                    ...
                    defaultMessage: "Connected Workspaces (Beta)",
                }),
    ...
    authentication: {
        icon: <ShieldOutlineIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
        ...
        subsections: {
            signup: {
                ...
                schema: {
                    id: "SignupSettings",
                    name: defineMessage({
                        id: "admin.authentication.signup",
                        defaultMessage: "Signup",
                    }),
                    settings: [
                        {
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                ...
                                defaultMessage:
                                    "Restrict new system and team members to specified email domains:",
                            }),
                            help_text: defineMessage({
                                id: "admin.team.restrictDescription",
                                defaultMessage:
                                    'New user accounts are restricted to the above specified email domain (e.g. "mattermost.com") or list of comma-separated domains (e.g. "corp.mattermost.com, mattermost.com"). New teams can only be created by users from the above domain(s). This setting only affects email login for users.',
                            }),
                            placeholder: defineMessage({
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                id: "admin.team.restrictGuestDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.team.emailInvitationsTitle",
                                defaultMessage: "Enable Email Invitations: ",
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.team.invalidateEmailInvitesTitle",
                                defaultMessage:
                                    "Invalidate pending email invites",
                            }),
                            help_text: defineMessage({
                                id: "admin.team.invalidateEmailInvitesDescription",
                                defaultMessage:
                            ...
                            error_message: defineMessage({
                                id: "admin.team.invalidateEmailInvitesFail",
                                defaultMessage:
                                    "Unable to invalidate pending email invites: {error}",
                            }),
                            success_message: defineMessage({
                                id: "admin.team.invalidateEmailInvitesSuccess",
                                defaultMessage:
                                    "Pending email invitations invalidated successfully",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.AUTHENTICATION.SIGNUP
            ...
            email: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.AUTHENTICATION.EMAIL
                    )
                ),
                schema: {
                    id: "EmailSettings",
            ...
            password: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.AUTHENTICATION.PASSWORD
                    )
                ),
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource(
            ...
            mfa: {
                ...
                isHidden: it.not(
                    it.userHasReadPermissionOnResource(
                        RESOURCE_KEYS.AUTHENTICATION.MFA
                    )
                ),
                schema: {
                    id: "ServiceSettings",
                    name: defineMessage({
                        id: "admin.authentication.mfa",
                    ...
                    settings: [
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "When true, users with AD/LDAP or email login can add multi-factor authentication to their account using Google Authenticator.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.AUTHENTICATION.MFA
            ...
            ldap: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.AUTHENTICATION.LDAP
                        )
                    )
                ),
                schema: {
                    ...
                    sections: [
                        {
                            ...
                            title: "Connection",
                            subtitle:
                                "Connection and security level to your AD/LDAP server.",
                            settings: [
                                {
                                    type: "bool",
                                    ...
                                    help_text: defineMessage({
                                        ...
                                        defaultMessage:
                                            "When true, Mattermost allows login using AD/LDAP",
                                    }),
                                    isDisabled: it.not(
                                        it.userHasWritePermissionOnResource(
                                            RESOURCE_KEYS.AUTHENTICATION.LDAP
                                ...
                                },
                                {
                                    ...
                                    placeholder: defineMessage({
                                        id: "admin.ldap.loginNameEx",
                                        defaultMessage:
                                            'E.g.: "AD/LDAP Username"',
                                    }),
                                    help_text: defineMessage({
                                ...
                                },
                                {
                                    ...
                                    help_text: defineMessage({
                                        ...
                                        defaultMessage:
                                            "The domain or IP address of AD/LDAP server.",
                                    }),
                                    placeholder: defineMessage({
                                        id: "admin.ldap.serverEx",
                                        defaultMessage: 'E.g.: "10.0.0.23"',
                                    }),
                                    isDisabled: it.any(
                                        it.not(
                                            it.userHasWritePermissionOnResource(
                                ...
                                },
                                {
                                    ...
                                    help_text:
                                        DefinitionConstants.CONNECTION_SECURITY_HELP_TEXT_LDAP,
                                    options: [
                                        {
                                            value: "",
                                            display_name: defineMessage({
                                ...
                                },
                                {
                                    ...
                                    help_text: defineMessage({
                                        ...
                                        defaultMessage:
                                            "Skips the certificate verification step for TLS or STARTTLS connections. Skipping certificate verification is not recommended for production environments where TLS is required.",
                                    }),
                                    isDisabled: it.any(
                                        it.not(
                                            it.userHasWritePermissionOnResource(
                                        ...
                                        it.stateIsFalse(
                                            "LdapSettings.ConnectionSecurity"
                                        )
                                    ),
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.privateKeyFileTitle",
                                        defaultMessage: "Private Key:",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.privateKeyFileFileDesc",
                                        defaultMessage:
                                            "The private key file for TLS Certificate. If using TLS client certificates as primary authentication mechanism. This will be provided by your LDAP Authentication Provider.",
                                    }),
                                    remove_help_text: defineMessage({
                                        id: "admin.ldap.privateKeyFileFileRemoveDesc",
                                        defaultMessage:
                                            "Remove the private key file for TLS Certificate.",
                                    }),
                                    remove_button_text: defineMessage({
                                        id: "admin.ldap.remove.privKey",
                                        defaultMessage:
                                            "Remove TLS Certificate Private Key",
                                    }),
                                    removing_text: defineMessage({
                                        id: "admin.ldap.removing.privKey",
                                        defaultMessage:
                                            "Removing Private Key...",
                                    }),
                                    uploading_text: defineMessage({
                                        id: "admin.ldap.uploading.privateKey",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.publicCertificateFileTitle",
                                        defaultMessage: "Public Certificate:",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.publicCertificateFileDesc",
                                        defaultMessage:
                                            "The public certificate file for TLS Certificate. If using TLS client certificates as primary authentication mechanism. This will be provided by your LDAP Authentication Provider.",
                                    }),
                                    remove_help_text: defineMessage({
                                        id: "admin.ldap.publicCertificateFileRemoveDesc",
                                        defaultMessage:
                                            "Remove the public certificate file for TLS Certificate.",
                                    }),
                                    remove_button_text: defineMessage({
                                        id: "admin.ldap.remove.sp_certificate",
                                        defaultMessage:
                                            "Remove Service Provider Certificate",
                                    }),
                                    removing_text: defineMessage({
                                        id: "admin.ldap.removing.certificate",
                                        defaultMessage:
                                            "Removing Certificate...",
                                    }),
                                    uploading_text: defineMessage({
                                        id: "admin.ldap.uploading.certificate",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    help_text: defineMessage({
                                        ...
                                        defaultMessage:
                                            "The username used to perform the AD/LDAP search. This should typically be an account created specifically for use with Mattermost. It should have access limited to read the portion of the AD/LDAP tree specified in the Base DN field.",
                                    }),
                                    isDisabled: it.any(
                                        it.not(
                                            it.userHasWritePermissionOnResource(
                            ...
                            settings: [
                                {
                                    ...
                                    placeholder: defineMessage({
                                        ...
                                        defaultMessage:
                                            'E.g.: "ou=Unit Name,dc=corp,dc=example,dc=com"',
                                    }),
                                    isDisabled: it.any(
                                        it.not(
                                            it.userHasWritePermissionOnResource(
                                ...
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.groupFilterTitle",
                                        defaultMessage: "Group Filter:",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.groupFilterFilterDesc",
                                        defaultMessage:
                                            "(Optional) Enter an AD/LDAP filter to use when searching for group objects. Only the groups selected by the query will be available to Mattermost. From [User Management > Groups]({siteURL}/admin_console/user_management/groups), select which AD/LDAP groups should be linked and configured.",
                                    }),
                                    help_text_markdown: true,
                                    help_text_values: { siteURL: getSiteURL() },
                                    placeholder: defineMessage({
                                        id: "admin.ldap.groupFilterEx",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.enableAdminFilterTitle",
                                        defaultMessage: "Enable Admin Filter:",
                                    }),
                                    isDisabled: it.any(
                                ...
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.adminFilterTitle",
                                        defaultMessage: "Admin Filter:",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.adminFilterFilterDesc",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            settings: [
                                ...
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.loginAttrTitle",
                                        defaultMessage: "Login ID Attribute: ",
                                    }),
                                    placeholder: defineMessage({
                                        id: "admin.ldap.loginIdAttrEx",
                                        defaultMessage:
                                            'E.g.: "sAMAccountName"',
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.loginAttrDesc",
                                        defaultMessage:
                                            'The attribute in the AD/LDAP server used to log in to Mattermost. Normally this attribute is the same as the "Username Attribute" field above. If your team typically uses domain/username to log in to other services with AD/LDAP, you may enter domain/username in this field to maintain consistency between sites.',
                                    }),
                                    help_text_markdown: false,
                                    isDisabled: it.any(
                                        it.not(
                                ...
                                },
                                {
                                    ...
                                    help_text: defineMessage({
                                        id: "admin.ldap.usernameAttrDesc",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    placeholder: defineMessage({
                                        id: "admin.ldap.positionAttrEx",
                                        defaultMessage: 'E.g.: "title"',
                                    }),
                                    help_text: defineMessage({
                                ...
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.pictureAttrTitle",
                                        defaultMessage:
                                            "Profile Picture Attribute:",
                                    }),
                                    placeholder: defineMessage({
                                        id: "admin.ldap.pictureAttrEx",
                                        defaultMessage:
                                            'E.g.: "thumbnailPhoto" or "jpegPhoto"',
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.pictureAttrDesc",
                                        defaultMessage:
                            ...
                            settings: [
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.groupDisplayNameAttributeTitle",
                                        defaultMessage:
                                            "Group Display Name Attribute:",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.groupDisplayNameAttributeDesc",
                                        defaultMessage:
                                            "The attribute in the AD/LDAP server used to populate the group display names.",
                                    }),
                                    placeholder: defineMessage({
                                        id: "admin.ldap.groupDisplayNameAttributeEx",
                                        defaultMessage: 'E.g.: "cn"',
                                    }),
                                    isHidden: it.not(
                                        it.licensedForFeature("LDAPGroups")
                                    ),
                                ...
                                },
                                {
                                    ...
                                    label: defineMessage({
                                        id: "admin.ldap.groupIdAttributeTitle",
                                        defaultMessage: "Group ID Attribute:",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.ldap.groupIdAttributeDesc",
                                        defaultMessage:
                                            "The attribute in the AD/LDAP server used as a unique identifier for Groups. This should be a AD/LDAP attribute with a value that does not change such as `entryUUID` for LDAP or `objectGUID` for Active Directory.",
                                    }),
                                    help_text_markdown: true,
                                    placeholder: defineMessage({
                                        id: "admin.ldap.groupIdAttributeEx",
                                        defaultMessage:
                                            'E.g.: "objectGUID" or "entryUUID"',
                                    }),
                                    isHidden: it.not(
                                        it.licensedForFeature("LDAPGroups")
                                    ),
                        ...
                        },
                        {
                            ...
                            settings: [
                                ...
                                },
                                {
                                    ...
                                    placeholder: defineMessage({
                                        id: "admin.ldap.maxPageSizeEx",
                                        defaultMessage: 'E.g.: "2000"',
                                    }),
                                    help_text: defineMessage({
                                    ...
                                    help_text_values: {
                                        link: (msg: string) => (
                                            <ExternalLink
                                                location="admin_console"
                                                href={
                                                    DocLinks.CONFIGURE_AD_LDAP_QUERY_TIMEOUT
                                                }
                                            >
                                                {msg}
                                            </ExternalLink>
                            ...
                            settings: [
                                {
                                    ...
                                    render_job: (job: Job) => {
                                        ...
                                        if (job && job.data) {
                                            ...
                                            if (
                                                job.data.delete_count &&
                                                job.data.delete_count.length > 0
                                            ) {
                                                deleteCount =
                                                    job.data.delete_count;
                                            }
                                            ...
                                            if (
                                                job.data.update_count &&
                                                job.data.update_count.length > 0
                                            ) {
                                                updateCount =
                                                    job.data.update_count;
                                            }
                                            ...
                                            if (job.data.ldap_groups_count) {
                                                linkedLdapGroupsCount =
                                                    job.data.ldap_groups_count;
                                            }

                                            // Groups count representing the total number of LDAP groups available based on
                                            // the configured based DN and groups filter.
                                            if (
                                                job.data.total_ldap_groups_count
                                            ) {
                                                totalLdapGroupsCount =
                                                    job.data
                                                        .total_ldap_groups_count;
                                            }

                                            if (job.data.group_delete_count) {
                                                groupDeleteCount =
                                                    job.data.group_delete_count;
                                            }

                                            if (
                                                job.data
                                                    .group_member_delete_count
                                            ) {
                                            ...
                                            if (
                                                job.data.group_member_add_count
                                            ) {
                                                groupMemberAddCount =
                                                    job.data
                                                        .group_member_add_count;
                                            }
                                        }
                                                ...
                                                <FormattedMessage
                                                    id={
                                                        linkedLdapGroupsCount
                                                            ...
                                                            : "admin.ldap.jobExtraInfoTotal"
                                                    }
                                                    defaultMessage={
                                                        linkedLdapGroupsCount
                                                            ? "Scanned {ldapUsers, number} LDAP users and {ldapGroups, number} linked groups."
                                                            : "Scanned {ldapUsers, number} LDAP users and {ldapGroups, number} groups."
                                                    }
                                                    values={{
                                                        ldapUsers,
                                                        ldapGroups:
                                                            linkedLdapGroupsCount ||
                                                            totalLdapGroupsCount, // Show the old count for jobs records containing the old JSON key.
                                                    }}
                                                />
                                                <ul>
                                                    {updateCount > 0 && (
            ...
            saml: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.AUTHENTICATION.SAML
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.ignoreGuestsLdapSyncTitle",
                                defaultMessage:
                            ...
                            help_text_values: {
                                link: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DocLinks.CONFIGURE_OVERRIDE_SAML_BIND_DATA_WITH_LDAP
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.idpMetadataUrlTitle",
                                defaultMessage:
                                    "Identity Provider Metadata URL:",
                            }),
                            help_text: defineMessage({
                                id: "admin.saml.idpMetadataUrlDesc",
                                defaultMessage:
                                    "The Metadata URL for the Identity Provider you use for SAML requests",
                            }),
                            placeholder: defineMessage({
                                id: "admin.saml.idpMetadataUrlEx",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.getSamlMetadataFromIDPUrl",
                                defaultMessage: "Get SAML Metadata from IdP",
                            }),
                            loading: defineMessage({
                                id: "admin.saml.getSamlMetadataFromIDPFetching",
                                defaultMessage: "Fetching...",
                            }),
                            error_message: defineMessage({
                                id: "admin.saml.getSamlMetadataFromIDPFail",
                                defaultMessage:
                                    "SAML Metadata URL did not connect and pull data successfully",
                            }),
                            success_message: defineMessage({
                                id: "admin.saml.getSamlMetadataFromIDPSuccess",
                                defaultMessage:
                                    "SAML Metadata retrieved successfully. Two fields below have been updated",
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.idpDescriptorUrlEx",
                                defaultMessage:
                                    'E.g.: "https://idp.example.org/SAML2/issuer"',
                            }),
                        ...
                        },
                        {
                            ...
                            remove_button_text: defineMessage({
                                id: "admin.saml.remove.idp_certificate",
                                defaultMessage:
                                    "Remove Identity Provider Certificate",
                            }),
                            removing_text: defineMessage({
                                id: "admin.saml.removing.certificate",
                                defaultMessage: "Removing Certificate...",
                            }),
                            uploading_text: defineMessage({
                                id: "admin.saml.uploading.certificate",
                                defaultMessage: "Uploading Certificate...",
                            }),
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.assertionConsumerServiceURLEx",
                                defaultMessage:
                                    'E.g.: "<urlChunk>your-mattermost-url</urlChunk>"',
                            }),
                            placeholder_values: {
                                urlChunk: (chunk: string) =>
                                    `https://'<${chunk}>'/login/sso/saml`,
                            },
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.serviceProviderIdentifierTitle",
                                defaultMessage: "Service Provider Identifier:",
                            }),
                            help_text: defineMessage({
                                id: "admin.saml.serviceProviderIdentifierDesc",
                                defaultMessage:
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.serviceProviderIdentifierEx",
                                defaultMessage:
                                    'E.g.: "<urlChunk>your-mattermost-url</urlChunk>"',
                            }),
                            placeholder_values: {
                        ...
                        },
                        {
                            ...
                            remove_button_text: defineMessage({
                                id: "admin.saml.remove.privKey",
                                defaultMessage:
                                    "Remove Service Provider Private Key",
                            }),
                            removing_text: defineMessage({
                                id: "admin.saml.removing.privKey",
                                defaultMessage: "Removing Private Key...",
                            }),
                            uploading_text: defineMessage({
                                id: "admin.saml.uploading.privateKey",
                                defaultMessage: "Uploading Private Key...",
                            }),
                            fileType: ".key",
                        ...
                        },
                        {
                            ...
                            remove_button_text: defineMessage({
                                id: "admin.saml.remove.sp_certificate",
                                defaultMessage:
                                    "Remove Service Provider Certificate",
                            }),
                            removing_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.signRequestTitle",
                                defaultMessage: "Sign Request:",
                            }),
                            help_text: defineMessage({
                                id: "admin.saml.signRequestDescription",
                                defaultMessage:
                            ...
                            isDisabled: it.any(
                                ...
                                it.stateIsFalse(
                                    "SamlSettings.PublicCertificateFile"
                                )
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.signatureAlgorithmTitle",
                            ...
                            options: [
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.saml.signatureAlgorithmDisplay.sha1",
                                        defaultMessage: "RSAwithSHA1",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.saml.signatureAlgorithmDescription.sha1",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.saml.signatureAlgorithmDisplay.sha256",
                                        defaultMessage: "RSAwithSHA256",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.saml.signatureAlgorithmDescription.sha256",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.saml.signatureAlgorithmDisplay.sha512",
                                        defaultMessage: "RSAwithSHA512",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.saml.signatureAlgorithmDescription.sha512",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.canonicalAlgorithmTitle",
                                defaultMessage: "Canonicalization Algorithm",
                            }),
                            options: [
                                {
                                    value: SAML_SETTINGS_CANONICAL_ALGORITHM_C14N,
                                    display_name: defineMessage({
                                        id: "admin.saml.canonicalAlgorithmDisplay.n10",
                                        defaultMessage:
                                            "Exclusive XML Canonicalization 1.0 (omit comments)",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.saml.canonicalAlgorithmDescription.exc",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.saml.canonicalAlgorithmDisplay.n11",
                                        defaultMessage:
                                            "Canonical XML 1.1 (omit comments)",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.saml.canonicalAlgorithmDescription.c14",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.emailAttrEx",
                                defaultMessage:
                                    'E.g.: "Email" or "PrimaryEmail"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.usernameAttrEx",
                                defaultMessage: 'E.g.: "Username"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.idAttrEx",
                                defaultMessage: 'E.g.: "Id"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.guestAttrTitle",
                                defaultMessage: "Guest Attribute:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.saml.guestAttrEx",
                                defaultMessage:
                                    'E.g.: "usertype=Guest" or "isGuest=true"',
                            }),
                            help_text: defineMessage({
                                id: "admin.saml.guestAttrDesc",
                                defaultMessage:
                                    "(Optional) Requires Guest Access to be enabled before being applied. The attribute in the SAML Assertion that will be used to apply a guest role to users in Mattermost. Guests are prevented from accessing teams or channels upon logging in until they are assigned a team and at least one channel. Note: If this attribute is removed/changed from your guest user in SAML and the user is still active, they will not be promoted to a member and will retain their Guest role. Guests can be promoted in **System Console > User Management**. Existing members that are identified by this attribute as a guest will be demoted from a member to a guest when they are asked to login next. The next login is based upon Session lengths set in **System Console > Session Lengths**. It is highly recommend to manually demote users to guests in **System Console > User Management ** to ensure access is restricted immediately.",
                            }),
                            help_text_markdown: true,
                            isDisabled: it.any(
                                it.not(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.enableAdminAttrTitle",
                                defaultMessage: "Enable Admin Attribute:",
                            }),
                            isDisabled: it.any(
                                it.not(it.isSystemAdmin),
                                it.stateIsFalse("SamlSettings.Enable")
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.saml.adminAttrTitle",
                                defaultMessage: "Admin Attribute:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.saml.adminAttrEx",
                                defaultMessage:
                                    'E.g.: "usertype=Admin" or "isAdmin=true"',
                            }),
                            help_text: defineMessage({
                                id: "admin.saml.adminAttrDesc",
                                defaultMessage:
                                    "(Optional) The attribute in the SAML Assertion for designating System Admins. The users selected by the query will have access to your Mattermost server as System Admins. By default, System Admins have complete access to the Mattermost System Console. Existing members that are identified by this attribute will be promoted from member to System Admin upon next login. The next login is based upon Session lengths set in **System Console > Session Lengths.** It is highly recommend to manually demote users to members in **System Console > User Management** to ensure access is restricted immediately. Note: If this filter is removed/changed, System Admins that were promoted via this filter will be demoted to members and will not retain access to the System Console. When this filter is not in use, System Admins can be manually promoted/demoted in **System Console > User Management**.",
                            }),
                            help_text_markdown: true,
                            isDisabled: it.any(
                                it.not(it.isSystemAdmin),
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.firstnameAttrEx",
                                defaultMessage: 'E.g.: "FirstName"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.lastnameAttrEx",
                                defaultMessage: 'E.g.: "LastName"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.positionAttrEx",
                                defaultMessage: 'E.g.: "Role"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.localeAttrEx",
                                defaultMessage:
                                    'E.g.: "Locale" or "PrimaryLanguage"',
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.saml.loginButtonTextEx",
                                defaultMessage: 'E.g.: "OKTA"',
                            }),
                            help_text: defineMessage({
            ...
            gitlab: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.AUTHENTICATION.OPENID
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.gitlab.siteUrlExample",
                                defaultMessage: "E.g.: https://",
                            }),
                            isDisabled: it.any(
                                it.not(
                                    it.userHasWritePermissionOnResource(
            ...
            oauth: {
                ...
                isHidden: it.any(
                    it.any(it.not(it.licensed), it.licensedForSku("starter")),
                    it.all(
                ...
                schema: {
                    ...
                    settings: [
                        ...
                        },
                        {
                            ...
                            options: [
                                ...
                                },
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.oauth.gitlab",
                                        defaultMessage: "GitLab",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.gitlab.EnableMarkdownDesc",
                                        defaultMessage:
                                ...
                                },
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.oauth.google",
                                        defaultMessage: "Google Apps",
                                    }),
                                    isHidden: it.all(
                                        it.not(
                                            it.licensedForFeature("GoogleOAuth")
                                        ),
                                        it.not(it.cloudLicensed)
                                    ),
                                    help_text: defineMessage({
                                        id: "admin.google.EnableMarkdownDesc",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            isHidden: it.not(
                                it.stateEquals("oauthType", "gitlab")
                            ),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.AUTHENTICATION.OPENID
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                ...
                                defaultMessage:
                                    'E.g.: "7602141235235-url0fhs1mayfasbmop5qlfns8dh4.apps.googleusercontent.com"',
                            }),
                            isHidden: it.not(
                                it.stateEquals("oauthType", "google")
                            ),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.AUTHENTICATION.OPENID
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.office365.clientIdExample",
                                defaultMessage:
                                    'E.g.: "adf3sfa2-ag3f-sn4n-ids0-sh1hdax192qq"',
                            }),
                            isHidden: it.not(
                                it.stateEquals("oauthType", "office365")
                            ),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.AUTHENTICATION.OPENID
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.office365.clientSecretExample",
                                defaultMessage:
                                    'E.g.: "shAieM47sNBfgl20f8ci294"',
                            }),
                            isHidden: it.not(
                                it.stateEquals("oauthType", "office365")
                            ),
                            ...
                            dynamic_value: (value, config, state) => {
                                if (state["Office365Settings.DirectoryId"]) {
                                    return (
                                        ...
                                        state["Office365Settings.DirectoryId"] +
            ...
            openid: {
                ...
                schema: {
                    id: "OpenIdSettings",
                    name: defineMessage({
                        id: "admin.authentication.openid",
                        defaultMessage: "OpenID Connect",
                    }),
                    onConfigLoad: (config) => {
                        const newState: {
                    ...
                    onConfigSave: (config) => {
                        ...
                        if (config.openidType === Constants.OFFICE365_SERVICE) {
                            ...
                            config.openidType === Constants.GOOGLE_SERVICE
                        ) {
                            configSetting = "GoogleSettings";
                        } else if (
                            config.openidType === Constants.GITLAB_SERVICE
                        ) {
                            configSetting = "GitLabSettings";
                        } else if (
                            config.openidType === Constants.OPENID_SERVICE
                        ) {
                    ...
                    settings: [
                        {
                            ...
                            component: OpenIdConvert,
                            key: "OpenIdConvert",
                            isHidden: it.any(it.not(usesLegacyOauth)),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.AUTHENTICATION.OPENID
                                )
                        ...
                        },
                        {
                            ...
                            options: [
                                ...
                                },
                                {
                                    ...
                                    display_name: defineMessage({
                                        id: "admin.oauth.openid",
                                        defaultMessage:
                                            "OpenID Connect (Other)",
                                    }),
                                    help_text: defineMessage({
                                        id: "admin.openid.EnableMarkdownDesc",
                                        defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.openid.discoveryEndpointTitle",
                                defaultMessage: "Discovery Endpoint:",
                            }),
                            help_text: defineMessage({
                                id: "admin.gitlab.discoveryEndpointDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            key: "GitLabSettings.Id",
                            label: defineMessage({
                                id: "admin.openid.clientIdTitle",
                                defaultMessage: "Client ID:",
                            }),
                            help_text: defineMessage({
                                id: "admin.openid.clientIdDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            key: "GitLabSettings.Secret",
                            label: defineMessage({
                                id: "admin.openid.clientSecretTitle",
                                defaultMessage: "Client Secret:",
                            }),
                            help_text: defineMessage({
                                id: "admin.openid.clientSecretDescription",
                                defaultMessage:
                            ...
                            placeholder: defineMessage({
                                id: "admin.gitlab.clientSecretExample",
                                defaultMessage:
                                    'E.g.: "jcuS8PuvcpGhpgHhlcpT1Mx442pnqMxQY"',
                            }),
                            isHidden: it.not(
                                it.stateEquals(
                                    "openidType",
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                id: "admin.google.discoveryEndpointDesc",
                                defaultMessage:
                                    "The URL of the discovery document for OpenID Connect with Google.",
                            }),
                            help_text_markdown: false,
                            dynamic_value: () =>
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                id: "admin.office365.discoveryEndpointDesc",
                                defaultMessage:
                                    "The URL of the discovery document for OpenID Connect with Entra ID.",
                            }),
                            help_text_markdown: false,
                            dynamic_value: (value, config, state) => {
                                if (state["Office365Settings.DirectoryId"]) {
...

                        {
                            ...
                            label: defineMessage({
                                id: "admin.openid.buttonTextTitle",
                                defaultMessage: "Button Name:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.openid.buttonTextEx",
                                defaultMessage: "Custom Button Name",
                            }),
                            help_text: defineMessage({
                                id: "admin.openid.buttonTextDesc",
                                defaultMessage:
                                    "The text that will show on the login button.",
                            }),
                            isHidden: it.any(
                                it.not(
                                    it.stateEquals(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.openid.buttonColorTitle",
                                defaultMessage: "Button Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.openid.buttonColorDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.openid.discovery.placeholder",
                                defaultMessage:
                                    "https://id.mydomain.com/.well-known/openid-configuration",
                            }),
                            help_text: defineMessage({
                                id: "admin.openid.discoveryEndpointDesc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.openid.clientIdExample",
                                defaultMessage:
                                    'E.g.: "adf3sfa2-ag3f-sn4n-ids0-sh1hdax192qq"',
                            }),
                            isHidden: it.any(
                        ...
                        },
                        {
                            ...
                            placeholder: defineMessage({
                                id: "admin.openid.clientSecretExample",
                                defaultMessage:
                                    'E.g.: "H8sz0Az-dDs2p15-7QzD231"',
                            }),
                            isHidden: it.any(
            ...
            guest_access: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.AUTHENTICATION.GUEST_ACCESS
                        )
                    )
                ),
                schema: {
                    ...
                    name: defineMessage({
                        id: "admin.authentication.guest_access",
                        defaultMessage: "Guest Access",
                    }),
                    settings: [
                        {
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.guest_access.hideTags",
                                defaultMessage: "Hide guest tag",
                            }),
                            help_text: defineMessage({
                                id: "admin.guest_access.hideTagsDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.guest_access.whitelistedDomainsTitle",
                                defaultMessage: "Whitelisted Guest Domains:",
                            }),
                            help_text: defineMessage({
                            ...
                            help_text_markdown: true,
                            placeholder: defineMessage({
                                id: "admin.guest_access.whitelistedDomainsExample",
                                defaultMessage:
                                    'E.g.: "company.com, othercorp.org"',
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.AUTHENTICATION.GUEST_ACCESS
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.guest_access.mfaTitle",
                                defaultMessage:
                                    "Enforce Multi-factor Authentication: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.guest_access.mfaDescriptionMFANotEnabled",
                                defaultMessage:
                                    "[Multi-factor authentication](./mfa) is currently not enabled.",
                            }),
                            help_text_markdown: true,
                            isHidden: it.configIsTrue(
                                "ServiceSettings",
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                id: "admin.guest_access.mfaDescription",
                                defaultMessage:
    ...
    plugins: {
        icon: <PowerPlugOutlineIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
        ...
        subsections: {
            plugin_management: {
                ...
                isDisabled: it.not(
                    it.userHasWritePermissionOnResource("plugins")
                ),
                schema: {
    ...
    integrations: {
        icon: <SitemapIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
        ...
        id: "integrations",
        isHidden: it.not(
            it.userHasReadPermissionOnSomeResources(RESOURCE_KEYS.INTEGRATIONS)
        ),
        subsections: {
            integration_management: {
                ...
                isHidden: it.all(
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.INTEGRATIONS.INTEGRATION_MANAGEMENT
                        )
                    )
                ),
                schema: {
                    ...
                    name: defineMessage({
                        id: "admin.integrations.integrationManagement.title",
                        defaultMessage: "Integration Management",
                    }),
                    settings: [
                        {
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.outgoingOAuthConnectionsTitle",
                                defaultMessage:
                                    "Enable Outgoing OAuth Connections: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.outgoingOAuthConnectionsDesc",
                                defaultMessage:
                            ...
                            help_text_values: {
                                link: (text: string) => (
                                    <a href="https://mattermost.com/pl/outgoing-oauth-connections">
                                        {text}
                                    </a>
                                ),
                            },
                            ...
                            help_text_values: {
                                link: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DeveloperLinks.SETUP_CUSTOM_SLASH_COMMANDS
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.integrationRequestTitle",
                                defaultMessage: "Integration request timeout: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.integrationRequestDesc",
                                defaultMessage:
                            ...
                            help_text_values: {
                                ...
                                interactiveMessages: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DeveloperLinks.INTERACTIVE_MESSAGES
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
                                ...
                                interactiveDialogs: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DeveloperLinks.INTERACTIVE_DIALOGS
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
                            ...
                            help_text_values: {
                                link: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DeveloperLinks.PERSONAL_ACCESS_TOKENS
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
            ...
            bot_accounts: {
                ...
                isHidden: it.all(
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.INTEGRATIONS.BOT_ACCOUNTS
                        )
                    )
                ),
                schema: {
                    ...
                    name: defineMessage({
                        id: "admin.integrations.botAccounts.title",
                        defaultMessage: "Bot Accounts",
                    }),
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.enableBotTitle",
                                defaultMessage: "Enable Bot Account Creation: ",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.enableBotAccountCreation",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.disableBotOwnerDeactivatedTitle",
                                defaultMessage:
            ...
            gif: {
                ...
                isHidden: it.all(
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.INTEGRATIONS.GIF
                        )
                    )
                ),
                schema: {
            ...
            cors: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.INTEGRATIONS.CORS
                        )
                    )
                ),
                schema: {
                    ...
                    settings: [
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.corsExposedHeadersTitle",
                                defaultMessage: "CORS Exposed Headers:",
                            }),
                            placeholder: defineMessage({
                                id: "admin.service.corsHeadersEx",
                                defaultMessage: "X-My-Header",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.corsExposedHeadersDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.corsAllowCredentialsLabel",
                                defaultMessage: "CORS Allow Credentials:",
                            }),
                            help_text: defineMessage({
                                id: "admin.service.corsAllowCredentialsDescription",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.service.CorsDebugLabel",
                                defaultMessage: "CORS Debug:",
                            }),
                            help_text: defineMessage({
    ...
    compliance: {
        icon: <FormatListBulletedIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
            id: "admin.sidebar.compliance",
            defaultMessage: "Compliance",
        }),
        isHidden: it.not(
            it.userHasReadPermissionOnSomeResources(RESOURCE_KEYS.COMPLIANCE)
        ),
        subsections: {
            custom_policy_form_edit: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnSomeResources(
                            RESOURCE_KEYS.COMPLIANCE.DATA_RETENTION_POLICY
                        )
                    )
                ),
                isDisabled: it.not(
            ...
            message_export: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.COMPLIANCE.COMPLIANCE_EXPORT
                        )
                    )
                ),
                isDisabled: it.not(
            ...
            audits: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.COMPLIANCE.COMPLIANCE_MONITORING
                        )
                    )
                ),
                isDisabled: it.not(
                ...
                schema: {
                    id: "Audits",
                    name: defineMessage({
                        id: "admin.compliance.complianceMonitoring",
                        defaultMessage: "Compliance Monitoring",
                    }),
                    component: Audits,
                    ...
                    settings: [
                        {
                            type: "banner",
                            label: defineMessage({
                                id: "admin.compliance.newComplianceExportBanner",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "When true, Mattermost will generate a daily compliance report.",
                            }),
                            isHidden: it.not(
                                it.licensedForFeature("Compliance")
                            ),
            ...
            custom_terms_of_service: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.COMPLIANCE.CUSTOM_TERMS_OF_SERVICE
                        )
                    )
                ),
                isDisabled: it.not(
    ...
    experimental: {
        icon: <FlaskOutlineIcon size={16} color={"currentColor"} />,
        sectionTitle: defineMessage({
            id: "admin.sidebar.experimental",
            defaultMessage: "Experimental",
        }),
        isHidden: it.not(
            it.userHasReadPermissionOnSomeResources(RESOURCE_KEYS.EXPERIMENTAL)
        ),
        subsections: {
            experimental_features: {
                ...
                schema: {
                    ...
                    name: defineMessage({
                        id: "admin.experimental.experimentalFeatures",
                    ...
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.ldapSettingsLoginButtonColor.title",
                                defaultMessage: "AD/LDAP Login Button Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.ldapSettingsLoginButtonColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.ldapSettingsLoginButtonBorderColor.title",
                                defaultMessage:
                                    "AD/LDAP Login Button Border Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.ldapSettingsLoginButtonBorderColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.ldapSettingsLoginButtonTextColor.title",
                                defaultMessage:
                                    "AD/LDAP Login Button Text Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.ldapSettingsLoginButtonTextColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.experimentalEnableAuthenticationTransfer.title",
                                defaultMessage:
                                    "Allow Authentication Transfer:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.experimentalEnableAuthenticationTransfer.desc",
                                defaultMessage:
                                    "When true, users can change their sign-in method to any that is enabled on the server, any via Account Settings or the APIs. When false, Users cannot change their sign-in method, regardless of which authentication options are enabled.",
                            }),
                            help_text_markdown: false,
                            isHidden: it.any(
                                // documented as E20 and higher, but only E10 in the code
                                it.not(it.licensed),
                                it.licensedForSku("starter")
                            ),
                            isDisabled: it.not(
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.linkMetadataTimeoutMilliseconds.title",
                                defaultMessage: "Link Metadata Timeout:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.linkMetadataTimeoutMilliseconds.desc",
                                defaultMessage:
                                    "The number of milliseconds to wait for metadata from a third-party link. Used with Post Metadata.",
                            }),
                            help_text_markdown: false,
                            placeholder: defineMessage({
                                id: "admin.experimental.linkMetadataTimeoutMilliseconds.example",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.emailBatchingBufferSize.title",
                                defaultMessage: "Email Batching Buffer Size:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.emailBatchingBufferSize.desc",
                                defaultMessage:
                                    "Specify the maximum number of notifications batched into a single email.",
                            }),
                            help_text_markdown: false,
                            placeholder: defineMessage({
                                id: "admin.experimental.emailBatchingBufferSize.example",
                                defaultMessage: 'E.g.: "256"',
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.emailBatchingInterval.title",
                                defaultMessage: "Email Batching Interval:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.emailBatchingInterval.desc",
                                defaultMessage:
                            ...
                            help_text_markdown: false,
                            placeholder: defineMessage({
                                id: "admin.experimental.emailBatchingInterval.example",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.emailSettingsLoginButtonColor.title",
                                defaultMessage: "Email Login Button Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.emailSettingsLoginButtonColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.emailSettingsLoginButtonBorderColor.title",
                                defaultMessage:
                                    "Email Login Button Border Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.emailSettingsLoginButtonBorderColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.emailSettingsLoginButtonTextColor.title",
                                defaultMessage:
                                    "Email Login Button Text Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.emailSettingsLoginButtonTextColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.enableUserDeactivation.title",
                                defaultMessage: "Enable Account Deactivation:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.enableUserDeactivation.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.experimentalEnableAutomaticReplies.title",
                                defaultMessage: "Enable Automatic Replies:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.experimentalEnableAutomaticReplies.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.enableChannelViewedMessages.title",
                                defaultMessage:
                                    "Enable Channel Viewed WebSocket Messages:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.enableChannelViewedMessages.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.clientSideCertEnable.title",
                                defaultMessage:
                                    "Enable Client-Side Certification:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.clientSideCertEnable.desc",
                                defaultMessage:
                            ...
                            help_text_values: {
                                link: (msg: string) => (
                                    <ExternalLink
                                        location="admin_console"
                                        href={
                                            DocLinks.ENABLE_CLIENT_SIDE_CERTIFICATION
                                        }
                                    >
                                        {msg}
                                    </ExternalLink>
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.clientSideCertCheck.title",
                                defaultMessage:
                            ...
                            help_text: defineMessage({
                                ...
                                defaultMessage:
                                    "When **primary**, after the client side certificate is verified, user’s email is retrieved from the certificate and is used to log in without a password. When **secondary**, after the client side certificate is verified, user’s email is retrieved from the certificate and matched against the one supplied by the user. If they match, the user logs in with regular email/password credentials.",
                            }),
                            help_text_markdown: true,
                            options: [
                                {
                                    value: "primary",
                                    display_name: defineMessage({
                                        id: "admin.experimental.clientSideCertCheck.options.primary",
                                ...
                                },
                                {
                                    value: "secondary",
                                    display_name: defineMessage({
                                        id: "admin.experimental.clientSideCertCheck.options.secondary",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.experimentalEnableDefaultChannelLeaveJoinMessages.title",
                                defaultMessage:
                                    "Enable Default Channel Leave/Join System Messages:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.experimentalEnableDefaultChannelLeaveJoinMessages.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.experimentalEnableHardenedMode.title",
                                defaultMessage: "Enable Hardened Mode:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.experimentalEnableHardenedMode.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.enableThemeSelection.title",
                                defaultMessage: "Enable Theme Selection:",
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.allowCustomThemes.title",
                                defaultMessage: "Allow Custom Themes:",
                            }),
                            help_text: defineMessage({
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.defaultTheme.title",
                                defaultMessage: "Default Theme:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.defaultTheme.desc",
                                defaultMessage:
                            ...
                            options: [
                                {
                                    value: "denim",
                                    display_name: defineMessage({
                                        id: "admin.experimental.defaultTheme.options.denim",
                                ...
                                },
                                {
                                    value: "sapphire",
                                    display_name: defineMessage({
                                        id: "admin.experimental.defaultTheme.options.sapphire",
                                ...
                                },
                                {
                                    value: "quartz",
                                    display_name: defineMessage({
                                        id: "admin.experimental.defaultTheme.options.quartz",
                                ...
                                },
                                {
                                    value: "indigo",
                                    display_name: defineMessage({
                                        id: "admin.experimental.defaultTheme.options.indigo",
                                ...
                                },
                                {
                                    value: "onyx",
                                    display_name: defineMessage({
                                        id: "admin.experimental.defaultTheme.options.onyx",
                                        defaultMessage: "Onyx",
                                    }),
                                },
                            ],
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.enableTutorial.title",
                                defaultMessage: "Enable Tutorial:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.enableTutorial.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.enableOnboardingFlow.title",
                                defaultMessage: "Enable Onboarding:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.enableOnboardingFlow.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.enableUserTypingMessages.title",
                                defaultMessage: "Enable User Typing Messages:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.enableUserTypingMessages.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.timeBetweenUserTypingUpdatesMilliseconds.title",
                                defaultMessage: "User Typing Timeout:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.timeBetweenUserTypingUpdatesMilliseconds.desc",
                                defaultMessage:
                                    "The number of milliseconds to wait between emitting user typing websocket events.",
                            }),
                            help_text_markdown: false,
                            placeholder: defineMessage({
                                id: "admin.experimental.timeBetweenUserTypingUpdatesMilliseconds.example",
                                defaultMessage: 'E.g.: "5000"',
                            }),
                            isDisabled: it.any(
                                it.not(
                                ...
                                it.stateIsFalse(
                                    "ServiceSettings.EnableUserTypingMessages"
                                )
                            ),
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.UsersStatusAndProfileFetchingPollIntervalMilliseconds.title",
                                defaultMessage:
                                    "User's Status and Profile Fetching Poll Interval:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.UsersStatusAndProfileFetchingPollIntervalMilliseconds.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.experimentalPrimaryTeam.title",
                                defaultMessage: "Primary Team:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.experimentalPrimaryTeam.desc",
                                defaultMessage:
                            ...
                            help_text_markdown: true,
                            placeholder: defineMessage({
                                id: "admin.experimental.experimentalPrimaryTeam.example",
                                defaultMessage: 'E.g.: "teamname"',
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.samlSettingsLoginButtonColor.title",
                                defaultMessage: "SAML Login Button Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.samlSettingsLoginButtonColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.samlSettingsLoginButtonBorderColor.title",
                                defaultMessage:
                                    "SAML Login Button Border Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.samlSettingsLoginButtonBorderColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.samlSettingsLoginButtonTextColor.title",
                                defaultMessage: "SAML Login Button Text Color:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.samlSettingsLoginButtonTextColor.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.useChannelInEmailNotifications.title",
                                defaultMessage:
                                    "Use Channel Name in Email Notifications:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.useChannelInEmailNotifications.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.userStatusAwayTimeout.title",
                                defaultMessage: "User Status Away Timeout:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.userStatusAwayTimeout.desc",
                                defaultMessage:
                            ...
                            help_text_markdown: false,
                            placeholder: defineMessage({
                                id: "admin.experimental.userStatusAwayTimeout.example",
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.disableAppBar.title",
                                defaultMessage: "Disable Apps Bar:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.disableAppBar.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.disableRefetchingOnBrowserFocus.title",
                                defaultMessage:
                                    "Disable data refetching on browser refocus:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.disableRefetchingOnBrowserFocus.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.disableWakeUpReconnectHandler.title",
                                defaultMessage:
                                    "Disable Wake Up Reconnect Handler:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.disableWakeUpReconnectHandler.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.delayChannelAutocomplete.title",
                                defaultMessage: "Delay Channel Autocomplete:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.delayChannelAutocomplete.desc",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.experimental.youtubeReferrerPolicy.title",
                                defaultMessage: "YouTube Referrer Policy:",
                            }),
                            help_text: defineMessage({
                                id: "admin.experimental.youtubeReferrerPolicy.desc",
                                defaultMessage:
            ...
            feature_flags: {
                ...
                isHidden: it.any(
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.EXPERIMENTAL.FEATURE_FLAGS
                        )
                    )
                ),
                isDisabled: true,
                searchableStrings: [featureFlagsMessages.title],
                schema: {
                    id: "Feature Flags",
            ...
            bleve: {
                ...
                isHidden: it.any(
                    ...
                    it.not(
                        it.userHasReadPermissionOnResource(
                            RESOURCE_KEYS.EXPERIMENTAL.BLEVE
                        )
                    )
                ),
                isDisabled: it.not(
            ...
            audit_logging: {
                ...
                isHidden: it.any(
                    ...
                    it.not(it.licensedForSku("enterprise"))
                ),
                schema: {
                    ...
                    settings: [
                        {
                            ...
                            label: defineMessage({
                                id: "admin.audit_logging_experimental.file_enabled.title",
                                defaultMessage: "File Enabled",
                            }),
                            help_text: defineMessage({
                                id: "admin.audit_logging_experimental.file_enabled.help_text",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.audit_logging_experimental.file_name.title",
                                defaultMessage: "File Name",
                            }),
                            help_text: defineMessage({
                                id: "admin.audit_logging_experimental.file_name.help_text",
                                defaultMessage:
                                    "The name of the file to write to. NOTE: If ExperimentalAuditSettings.FileEnabled is set to TRUE, this field is required.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_size.title",
                                defaultMessage: "Max File Size (MB)",
                            }),
                            help_text: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_size.help_text",
                                defaultMessage:
                                    "The maximum size of a single exported file, in MB.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_age.title",
                                defaultMessage: "Max File Age (Days)",
                            }),
                            help_text: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_age.help_text",
                                defaultMessage:
                                    "The maximum age of an exported file, in days.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_backups.title",
                                defaultMessage: "Maximum File Backups",
                            }),
                            help_text: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_backups.help_text",
                                defaultMessage:
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.audit_logging_experimental.file_compress.title",
                                defaultMessage: "File Compression",
                            }),
                            help_text: defineMessage({
                                id: "admin.audit_logging_experimental.file_compress.help_text",
                                defaultMessage:
                                    "Choose whether enable or disable file compression.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
                        ...
                        },
                        {
                            ...
                            label: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_queue_size.title",
                                defaultMessage: "Maximum File Queue",
                            }),
                            help_text: defineMessage({
                                id: "admin.audit_logging_experimental.file_max_queue_size.help_text",
                                defaultMessage:
                                    "The maximum number of files to be retained in the queue.",
                            }),
                            isDisabled: it.not(
                                it.userHasWritePermissionOnResource(
                                    RESOURCE_KEYS.EXPERIMENTAL.FEATURES
