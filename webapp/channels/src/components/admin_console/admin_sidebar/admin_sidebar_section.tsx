...
const AdminSidebarSection = ({
    ...
    const className = classNames("sidebar-section", {
    ...
    const indicatorElem = restrictedIndicator && (
        <span className={`${className}-indicator`}>{restrictedIndicator}</span>
    );
    const sidebarItemSafeId = createSafeId(name);
    const sidebarItemClickHandler = () =>
        trackEvent("admin", sidebarItemSafeId);
    let sidebarItem = (
        <BlockableLink
    ...
    return (
        <li className={className} data-testid={definitionKey}>
            {sidebarItem}
            {clonedChildren}
        </li>
    );
