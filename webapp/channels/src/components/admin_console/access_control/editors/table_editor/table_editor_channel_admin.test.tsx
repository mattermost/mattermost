...
describe('TableEditor - injected searchUsers', () => {
    ...
    test('should thread the editor channel into the injected searchUsers', async () => {
        // Regression: a resource.attributes.* rule can only be tested against a
        // concrete channel. The channel (editor scope here, or one picked in the
        // modal) must reach the searchUsers override — dropping it made the server
        // fail to resolve the resource side and the test showed no users.
        const mockSearch = jest.fn().mockResolvedValue({data: {users: [], total: 0}});
