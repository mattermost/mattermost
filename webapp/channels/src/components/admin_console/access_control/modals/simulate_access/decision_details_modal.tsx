        ...
        >
            <div className='SimulateAccessModal__detailsHeader'>
                ...
                <div className='SimulateAccessModal__detailsIdentity'>
                    <span className='SimulateAccessModal__detailsDisplayName'>
                        {displayUsername(user, 'full_name') || user.username}
                    </span>
                    <span className='SimulateAccessModal__detailsUsername'>{`@${user.username}`}</span>
                </div>
            </div>
        ...
        >
            ...
            <dl className='SimulateAccessModal__detailsAttributeList'>
                {entries.map(([key, value]) => (
                    ...
                    >
                        <dt className='SimulateAccessModal__detailsAttributeKey'>{key}</dt>
                        <dd className='SimulateAccessModal__detailsAttributeValue'>{value}</dd>
                    </div>
                ))}
            </dl>
        </div>
 ...
 * monospace plus an actual / expected value pair when the server
        ...
        >
            <div className='SimulateAccessModal__traceHeader'>
                ...
                {isCompound ? (
                    ...
                    <code className='SimulateAccessModal__traceExpression'>
                        {node.expression}
                    </code>
                )}
            </div>
            {/* For leaves, surface the user's actual value next to the
              * literal we expected — the cleanest "why did this fail"
              * read. We only render when both pieces are present; an
              * attribute-vs-attribute leaf has no `expected_value` so
              * we render only the actual side. Errors render their
              * message in place of the values. */}
            {!isCompound && node.outcome === POLICY_SIMULATION_EVALUATION_OUTCOME.ERROR && node.error ? (
                <div className='SimulateAccessModal__traceError'>{node.error}</div>
            ) : null}
            {!isCompound && (node.actual_value || node.expected_value) ? (
                <div className='SimulateAccessModal__traceValues'>
                    {node.attribute ? (
                        <span className='SimulateAccessModal__traceAttribute'>{node.attribute}</span>
                    ) : null}
                    {node.actual_value ? (
                        <span className='SimulateAccessModal__traceActual'>
                            <FormattedMessage
                                id='admin.access_control.simulate_access.details.trace.actual'
                                defaultMessage='your value: {value}'
                                values={{value: node.actual_value}}
                            />
                        </span>
                    ) : null}
                    {node.expected_value ? (
                        <span className='SimulateAccessModal__traceExpected'>
                            <FormattedMessage
                                id='admin.access_control.simulate_access.details.trace.expected'
                                defaultMessage='expected: {value}'
                                values={{value: node.expected_value}}
                            />
                        </span>
                    ) : null}
                </div>
