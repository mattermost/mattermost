...
describe("components/admin_console/license_settings/modals/upload_license_modal", () => {
    ...
    test("should display upload btn Disabled on initial load and no file selected", () => {
        ...
        const uploadButton = wrapper
            .find("UploadLicenseModal")
            .find("button#upload-button");
        expect(uploadButton.prop("disabled")).toBe(true);
    });

    test("should display upload btn Enabled when file is loaded", () => {
        const realUseState = React.useState;
        const initialStateForFileObj = {
            name: "testing.mattermost-license",
            size: 10240000,
        } as File;

        jest.spyOn(React, "useState").mockImplementationOnce(() =>
            realUseState(initialStateForFileObj as any),
        );
        const wrapper = mountWithIntl(
            <reactRedux.Provider store={store}>
                <UploadLicenseModal {...props} />
    ...
    test("should display no file selected text when no file is loaded", () => {
        ...
        const fileText = wrapper
            .find("UploadLicenseModal")
            .find(".file-name-section span");
    ...
    test("should display the file name when is selected", () => {
        ...
        const fileTextName = wrapper
            .find("UploadLicenseModal")
            .find(".file-name-section span.file-name");
        const fileTextSize = wrapper
            .find("UploadLicenseModal")
            .find(".file-name-section span.file-size");
    ...
    test("should show success image when open and there is a license (successful license upload)", async () => {
        ...
        useDispatchMock.mockReturnValue(
            dummyDispatch.mockImplementation(
                () =>
                    new Promise((resolve) => {
                        resolve("");
                    }),
        ...
        await act(async () => {
            wrapper
                .find("UploadLicenseModal")
                .find("#upload-button")
                .simulate("click"); // simulate successful upload of license
        });

        wrapper.update();
        expect(
            wrapper.find("UploadLicenseModal").find(".hands-svg"),
        ).toHaveLength(1);
        expect(
            wrapper.find("UploadLicenseModal").find("#done-button"),
        ).toHaveLength(1);
    ...
    test("should hide the upload modal", () => {
        ...
        expect(
            wrapper.find("UploadLicenseModal").find("content-body"),
        ).toHaveLength(0);
    });
});
