...
const EnterpriseEditionLeftPanel = ({
    ...
    const selfHostedExpansionModal = useControlSelfHostedExpansionModal({
        trackingLocation: "license_settings_add_seats",
    });
    ...
    useEffect(() => {
        if (
            actionQueryParam === "show_expansion_modal" &&
            canExpand &&
            isSelfHostedPurchaseEnabled
        ) {
    ...
    const skuName = getSkuDisplayName(
        unsanitizedLicense.SkuShortName,
        unsanitizedLicense.IsGovSku === "true",
    );
    const expirationDays = getRemainingDaysFromFutureTimestamp(
        parseInt(unsanitizedLicense.ExpiresAt, 10),
    );
    ...
    const viewPlansButton = (
        <button
            ...
            onClick={() =>
                openPricingModal({
                    trackingLocation: "license_settings_view_plans",
                })
            }
...
const renderLicenseValues =
    (activeUsers: number, seatsPurchased: number, expirationDays: number) =>
    (
        {
            legend,
            value,
        }: { legend: LegendValues; value: string | JSX.Element | null },
        index: number,
    ): React.ReactNode => {
        if (legend === "ACTIVE USERS:") {
            ...
            } = calculateOverageUserActivated({ activeUsers, seatsPurchased });
            return (
                <div className="item-element" key={value + index.toString()}>
                    <span
                    ...
                    <span
                        className={classNames({
                            value: true,
                            "value--warning-over-seats-purchased":
                                isBetween5PercerntAnd10PercentPurchasedSeats,
                            "value--over-seats-purchased":
                                isOver10PercerntPurchasedSeats,
                        })}
                    >
                        {value}
            ...
            return (
                <div className="item-element" key={value + index.toString()}>
                    ...
                    {expirationDays <=
                        DAYS_UNTIL_EXPIRY_WARNING_DISPLAY_THRESHOLD && (
                        <span
                            className={classNames("expiration-days", {
                                ...
                                "expiration-days-danger":
                                    expirationDays <=
                                    DAYS_UNTIL_EXPIRY_DANGER_DISPLAY_THRESHOLD,
                            })}
                        >
                            {`Expires in ${expirationDays} day${
                                expirationDays > 1 ? "s" : ""
                            }`}
                        </span>
                    )}
...
const renderLicenseContent = (
    ...
    const sku = license.SkuShortName ? (
        <>{`Mattermost ${toTitleCase(skuName)}${
            isTrialLicense ? " License Trial" : ""
        }`}</>
    ) : null;
    ...
    const startsAt = (
        <FormattedDate value={new Date(parseInt(license.StartsAt, 10))} />
    );
    const expiresAt = (
        <FormattedDate value={new Date(parseInt(license.ExpiresAt, 10))} />
    );
