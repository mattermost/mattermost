...
describe("components/admin_console/license_settings/enterprise_edition/enterprise_edition_left_panel", () => {
    ...
    test("should not add any class if active users is lower than the minimal", async () => {
        ...
        expect(
            screen.getByText(
                Intl.NumberFormat("en").format(props.statsActiveUsers),
            ),
        ).toHaveClass("value");
        expect(
            screen.getByText(
        ...
        ).not.toHaveClass("value--warning-over-seats-purchased");
        expect(
            screen.getByText(
                Intl.NumberFormat("en").format(props.statsActiveUsers),
            ),
        ).not.toHaveClass("value--over-seats-purchased");
        expect(screen.getByText("ACTIVE USERS:")).toHaveClass("legend");
        expect(screen.getByText("ACTIVE USERS:")).not.toHaveClass(
            "legend--warning-over-seats-purchased",
        );
        expect(screen.getByText("ACTIVE USERS:")).not.toHaveClass(
            "legend--over-seats-purchased",
        );
    });

    test("should add warning class to active users", async () => {
        const minWarning =
            Math.ceil(parseInt(license.Users, 10) * OverActiveUserLimits.MIN) +
            parseInt(license.Users, 10);
        const store = await mockStore(initialState);
        ...
        expect(
            screen.getByText(Intl.NumberFormat("en").format(minWarning)),
        ).toHaveClass("value");
        expect(
            screen.getByText(Intl.NumberFormat("en").format(minWarning)),
        ).toHaveClass("value--warning-over-seats-purchased");
        expect(
            screen.getByText(Intl.NumberFormat("en").format(minWarning)),
        ).not.toHaveClass("value--over-seats-purchased");
        expect(screen.getByText("ACTIVE USERS:")).toHaveClass("legend");
        expect(screen.getByText("ACTIVE USERS:")).toHaveClass(
            "legend--warning-over-seats-purchased",
        );
        expect(screen.getByText("ACTIVE USERS:")).not.toHaveClass(
            "legend--over-seats-purchased",
    ...
    test("should add over-seats-purchased class to active users", async () => {
        const exceedHighLimitExtraUsersError =
            Math.ceil(parseInt(license.Users, 10) * OverActiveUserLimits.MAX) +
            parseInt(license.Users, 10);
        ...
        expect(
            screen.getByText(
                Intl.NumberFormat("en").format(exceedHighLimitExtraUsersError),
            ),
        ).toHaveClass("value");
        expect(
            screen.getByText(
        ...
        ).toHaveClass("value--over-seats-purchased");
        expect(
            screen.getByText(
                Intl.NumberFormat("en").format(exceedHighLimitExtraUsersError),
            ),
    ...
    test("should add warning class to days expired indicator when there are more than 5 days until expiry", async () => {
        ...
        expect(screen.getByText("Expires in 6 days")).toHaveClass(
    ...
    test("should add danger class to days expired indicator when there are at least 5 days until expiry", async () => {
        ...
        expect(screen.getByText("Expires in 5 days")).toHaveClass(
    ...
    test("should display add seats button when there are more than 60 days until expiry and self hosted expansion is available", async () => {
        ...
        const store = await mockStore(initialState);
        jest.spyOn(useCanSelfHostedExpand, "default").mockImplementation(
            () => true,
        );
        renderWithIntl(
            <Provider store={store}>
