...
describe("components/feature_discovery", () => {
    describe("FeatureDiscovery", () => {
        test("should match the default state of the component when is not cloud environment", async () => {
            ...
            //cloud option
            expect(
                screen.queryByRole("button", { name: "Try free for 30 days" }),
            ).not.toBeInTheDocument();

            const featureLink = screen.getByTestId(
                "featureDiscovery_secondaryCallToAction",
            );

            expect(featureLink).toBeInTheDocument();
            expect(featureLink).toHaveAttribute(
                "href",
                "https://test.mattermost.com/secondary/?utm_source=mattermost&utm_medium=in-product&utm_content=feature_discovery&uid=&sid=",
            );
            ...
            expect(
                ...
                "href",
                "https://mattermost.com/pl/software-and-services-license-agreement?utm_source=mattermost&utm_medium=in-product&utm_content=feature_discovery&uid=&sid=",
            );
            expect(screen.getByText("Privacy Policy")).toHaveAttribute(
                "href",
                AboutLinks.PRIVACY_POLICY +
                    "?utm_source=mattermost&utm_medium=in-product&utm_content=feature_discovery&uid=&sid=",
            );
        ...
        test("should match component state when is cloud environment", async () => {
            ...
            expect(
                screen.getByRole("button", { name: "Try free for 30 days" }),
            ).toBeInTheDocument();
            ...
            expect(screen.getByText("Privacy Policy")).toHaveAttribute(
                "href",
                "https://mattermost.com/pl/privacy-policy/?utm_source=mattermost&utm_medium=in-product&utm_content=feature_discovery&uid=&sid=",
            );
        ...
        test("should match component state when is cloud environment and subscription is not loaded yet in redux store", () => {
            ...
            expect(
                screen.queryByTestId("featureDiscovery_secondaryCallToAction"),
            ).not.toBeInTheDocument();
