...
type Props = BaseProps & {
    ...
    actions: {
        updateConfig: (
            config: AdminConfig,
        ) => ActionFunc & Partial<{ error?: ClientErrorPlaceholder }>;
    };
};
type State = {
    serverError?: string;
...
export default class OpenIdConvert extends React.PureComponent<Props, State> {
    ...
    upgradeConfig = async (e: React.MouseEvent) => {
        ...
        if (newConfig.Office365Settings.DirectoryId) {
            newConfig.Office365Settings.DiscoveryEndpoint =
                "https://login.microsoftonline.com/" +
                newConfig.Office365Settings.DirectoryId +
                "/v2.0/.well-known/openid-configuration";
        }
        newConfig.GoogleSettings.DiscoveryEndpoint =
            "https://accounts.google.com/.well-known/openid-configuration";

        if (newConfig.GitLabSettings.UserAPIEndpoint) {
            const url = newConfig.GitLabSettings.UserAPIEndpoint.replace(
                "/api/v4/user",
                "",
            );
            newConfig.GitLabSettings.DiscoveryEndpoint =
                url + "/.well-known/openid-configuration";
        }

        ["Office365Settings", "GoogleSettings", "GitLabSettings"].forEach(
            (setting) => {
