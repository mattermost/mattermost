...
export type Props = {
    ...
    actions: {
        ...
        addUserToTeam: (
            teamId: string,
            userId?: string,
        ) => Promise<{ data: TeamMembership; error?: any }>;
    };
};
...
> {
    ...
    handleShowDeactivateMemberModal = (
        e: React.MouseEvent<HTMLButtonElement, MouseEvent>,
    ): void => {
        e.preventDefault();
            ...
            adminResetEmail(
                ...
                () => {
                    this.props.history.push(
                        "/admin_console/user_management/users",
                    );
                },
                (err: Error) => {
                    const serverError = (
    ...
    getAuthenticationText(): string {
        ...
        if (user.auth_service) {
            ...
            authLine = Utils.localizeMessage(
                "admin.userManagement.userDetail.email",
                "Email",
            );
        }
        if (mfaEnabled) {
            if (user.mfa_active) {
                authLine += ", ";
                authLine += Utils.localizeMessage(
                    "admin.userManagement.userDetail.mfa",
                    "MFA",
                );
            }
        }
        ...
        return (
            <div className="SystemUserDetail wrapper--fixed">
                ...
                <div className="admin-console__wrapper">
                    <div className="admin-console__content">
                        <AdminUserCard
                            ...
                            body={
                                <React.Fragment>
                                    <span className="SystemUserDetail__position">
                                        {user.position}
                                    </span>
                                    <span className="SystemUserDetail__field-label">
                                        {Utils.localizeMessage(
                                            "admin.userManagement.userDetail.email",
                                            "Email",
                                        )}
                                    ...
                                    <span className="SystemUserDetail__field-label">
                                        {Utils.localizeMessage(
                                            "admin.userManagement.userDetail.username",
                                            "Username",
                                        )}
                                    </span>
                                    <div>
                                        <AtIcon className="SystemUserDetail__field-icon" />
                                        <span className="SystemUserDetail__field-text">
                                            {user.username}
                                        </span>
                                    </div>
                                    <span className="SystemUserDetail__field-label">
                        ...
                        <AdminPanel
                            subtitleId={t(
                                "admin.userManagement.userDetail.teamsSubtitle",
                            )}
                            subtitleDefault={"Teams to which this user belongs"}
                            titleId={t(
                                "admin.userManagement.userDetail.teamsTitle",
                            )}
                ...
                <div className="admin-console-save">
                    ...
                    />
                    <div className="error-message" ref={this.errorMessageRef}>
