...
export default class TeamList extends React.PureComponent<Props, State> {
    ...
    private getTeamsAndMemberships = async (
    ...
    // check this out
    private mergeTeamsWithMemberships = (
        data: [{ data: Team[] }, { data: TeamMembership[] }],
    ): TeamWithMembership[] => {
        const teams = data[0].data;
        const memberships = data[1].data;
        let teamsWithMemberships = teams.map((object: Team) => {
            const results = memberships.filter(
                (team: TeamMembership) => team.team_id === object.id,
            );
            const team = { ...object, ...results[0] };
            return team;
        });
        teamsWithMemberships = filterAndSortTeamsByDisplayName(
            teamsWithMemberships,
            this.props.locale,
        );
    ...
    private doRemoveUserFromTeam = async (teamId: string): Promise<void> => {
        const { error } = await this.props.actions.removeUserFromTeam(
            teamId,
            this.props.userId,
        );
        if (error) {
            ...
            <React.Fragment>
                ...
                <AbstractList
                    ...
                    emptyListTextDefaultMessage={
                        this.props.emptyListTextDefaultMessage
                    }
                    userId={this.props.userId}
                />
            </React.Fragment>
