...
import {act, renderHookWithContext} from 'tests/vitest_react_testing_utils';
...
describe('useOperation', () => {
    ...
    it('should run operation on command with response value and loading phases: false -> true -> false', async () => {
        const testResolvingAsyncAction = vi.fn().mockImplementation((responseValue) => new Promise((r) => setTimeout(() => r(responseValue), 1000)));
    ...
    it('should run operation on command with error and loading phases: false -> true -> false', async () => {
        const testRejectingAsyncAction = vi.fn().mockImplementation(() => new Promise((resolve, reject) => setTimeout(() => reject(new Error('error somewhere')), 1000)));
