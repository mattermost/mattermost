...
import {act, renderHookWithContext, waitFor} from 'tests/vitest_react_testing_utils';
...
describe('useUserPropertyFields', () => {
    let getFields: ReturnType<typeof vi.spyOn>;
    let patchField: ReturnType<typeof vi.spyOn>;
    let deleteField: ReturnType<typeof vi.spyOn>;
    let createField: ReturnType<typeof vi.spyOn>;
    ...
    beforeEach(() => {
        ...
        getFields = vi.spyOn(Client4, 'getCustomProfileAttributeFields');
        patchField = vi.spyOn(Client4, 'patchCustomProfileAttributeField');
        deleteField = vi.spyOn(Client4, 'deleteCustomProfileAttributeField');
        createField = vi.spyOn(Client4, 'createCustomProfileAttributeField');
    ...
    it('should successfully handle creates', async () => {
        createField.mockImplementation((patch: UserPropertyFieldPatch) => Promise.resolve({...baseField, ...patch, id: generateId()} as UserPropertyField));
