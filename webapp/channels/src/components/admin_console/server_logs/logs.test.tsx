...
describe('components/admin_console/server_logs/Logs', () => {
    ...
    test.each(['caller', 'msg', 'worker', 'whatever'])('should search input be performed on %s attribute',
    ...
    test.each(['job_id', 'level', 'timestamp'])('should search input not be performed on %s attribute',
        async (searchString: string) => {
            const searchInput = screen.getByTestId('searchInput');
            userEvent.type(searchInput, searchString);
