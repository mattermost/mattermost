...
describe('components/admin_console/brand_image_setting', () => {
    ...
    test('should register and unregister save handler when mounted and unmounted respectively', async () => {
        const {unmount} = await renderWithContext(<BrandImageSetting {...baseProps}/>);
    ...
    test('should show delete button if brand image exists', async () => {
        await renderWithContext(<BrandImageSetting {...baseProps}/>);
    ...
    test('should hide delete button if the setting is disabled', async () => {
        ...
        await renderWithContext(<BrandImageSetting {...props}/>);
