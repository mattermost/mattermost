    ...
    line_items: [
        {
            ...
            quantity: 1,
            price_per_unit: 1000,
            description: "1 × Cloud Professional (at $10.00 / month)",
            type: "onpremise",
            metadata: {},
        },
    ],
    ...
    line_items: [
        {
            ...
            quantity: 1,
            price_per_unit: 1000,
            description: "Trial period for Cloud Professional",
            type: "onpremise",
            metadata: {},
        },
    ],
...
describe("components/admin_console/billing/billing_history", () => {
    ...
    test("should match the default state of the component with given props", () => {
        ...
        expect(
            screen.queryByText("All of your invoices will be shown here"),
        ).toBeInTheDocument();
        expect(screen.getByTestId(invoiceA.number)).toHaveTextContent(
            (invoiceA.total / 100.0).toString(),
        );
        expect(screen.getByTestId(invoiceB.number)).toHaveTextContent(
            (invoiceB.total / 100.0).toString(),
        );
    ...
    test("Billing history section shows template when no invoices have been emitted yet", () => {
        ...
        expect(screen.getByRole("link")).toHaveAttribute(
            ...
            CloudLinks.BILLING_DOCS +
                "?utm_source=mattermost&utm_medium=in-product-cloud&utm_content=billing_history&uid=current_user_id&sid=",
        );
        expect(screen.getByRole("link")).toHaveTextContent(
            "See how billing works",
        );
        expect(screen.getByTestId("no-invoices")).toHaveTextContent(
            NO_INVOICES_LEGEND,
        );
    });
    ...
    test("Billing history section download button has the target property set as _self so it works well in desktop app", () => {
        ...
        expect(
            screen.getByTestId(`billingHistoryLink-${invoiceA.id}`),
        ).toHaveAttribute("target", "_self");
        expect(
            screen.getByTestId(`billingHistoryLink-${invoiceB.id}`),
        ).toHaveAttribute("target", "_self");
        expect(
            screen.getByTestId(`billingHistoryLink-${invoiceA.id}`),
        ).toHaveAttribute(
            "href",
            "/api/v4/cloud/subscription/invoices/in_1KNb3DI67GP2qpb4ueaJYBt8/pdf",
        );
        expect(
            screen.getByTestId(`billingHistoryLink-${invoiceB.id}`),
        ).toHaveAttribute(
            "href",
            "/api/v4/cloud/subscription/invoices/in_1KIWNTI67GP2qpb4KjGj1KAy/pdf",
        );
    });
});
    ...
    test("Billing history section shows template when no invoices have been emitted yet", () => {
        const noBillingHistoryState = {
            ...
            entities: {
                ...state.entities,
                hostedCustomer: {
                    invoices: { invoices: {}, invoicesLoaded: true },
                    errors: {},
                },
            },
        };
        ...
        expect(screen.getByRole("link")).toHaveAttribute(
            ...
            HostedCustomerLinks.SELF_HOSTED_BILLING +
                "?utm_source=mattermost&utm_medium=in-product&utm_content=billing_history&uid=current_user_id&sid=",
        );
        expect(screen.getByRole("link")).toHaveTextContent(
            "See how billing works",
        );
...
describe("NoBillingHistorySection", () => {
    const state = {
        entities: { users: {}, general: { config: {}, license: {} } },
    } as any;
