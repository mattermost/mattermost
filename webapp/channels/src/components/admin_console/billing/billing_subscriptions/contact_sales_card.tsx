...
type Props = {
    ...
    onUpgradeMattermostCloud: (
        telemetryProps?: TelemetryProps | undefined,
    ) => void;
};

const ContactSalesCard = (props: Props) => {
    const [openSalesLink, contactSalesLink] = useOpenSalesLink();
    const { isFreeTrial, subscriptionPlan, onUpgradeMattermostCloud } = props;
    let title;
    let description;
    ...
    return (
        <div className="PrivateCloudCard">
            <div className="PrivateCloudCard__text">
                <div className="PrivateCloudCard__text-title">{title}</div>
                ...
                {!isFreeTrial &&
                    subscriptionPlan !== CloudProducts.ENTERPRISE &&
                    subscriptionPlan !== CloudProducts.LEGACY && (
                        <button
                            type="button"
                            onClick={() => {
                                if (
                                    ...
                                    onUpgradeMattermostCloud({
                                        trackingLocation:
                                            "admin_console_subscription_card_upgrade_now_button",
                                    });
                                } else {
                                    openSalesLink();
                                }
            ...
            <div className="PrivateCloudCard__image">
                {isFreeTrial ? (
                    <CloudTrialSvg width={170} height={123} />
                ) : (
                    <PrivateCloudSvg width={170} height={123} />
                )}
            </div>
        </div>
    );
