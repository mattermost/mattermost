...
export const ToPaidPlanBannerDismissable = () => {
    ...
    const snoozePreferenceVal = useSelector((state: GlobalState) =>
        getPreference(
            state,
            Preferences.TO_PAID_PLAN_NUDGE,
            CloudBanners.NUDGE_TO_PAID_PLAN_SNOOZED,
            '{"range": 0, "show": true}',
        ),
    );
    const snoozeInfo = JSON.parse(
        snoozePreferenceVal,
    ) as ToPaidPlanDismissPreference;
    ...
    let message = {
        ...
        defaultMessage:
            "Cloud Free will be deprecated on {date}. To keep your workspace, upgrade to a paid plan",
        values: {
            date: moment(cloudFreeCloseMoment, "YYYYMMDD").format(
                "MMMM DD, YYYY",
            ),
        },
    };
    ...
    if (daysToCloudFreeEnd < 0) {
        message = {
            ...
            defaultMessage:
                "Cloud Free was deprecated. To keep your workspace, upgrade to a paid plan",
        } as any;
    }
...
export const ToPaidNudgeBanner = () => {
    ...
    const viewPlansAction = (
        <button
            onClick={() =>
                openPurchaseModal({
                    trackingLocation: "to_paid_plan_nudge_banner",
                })
            }
            className="btn ToPaidNudgeBanner__primary"
        >
            {formatMessage({
                id: "cloud_billing.nudge_to_paid.learn_more",
    ...
    const contactSalesAction = (
        ...
        >
            {formatMessage({
                id: "cloud_billing.nudge_to_paid.contact_sales",
