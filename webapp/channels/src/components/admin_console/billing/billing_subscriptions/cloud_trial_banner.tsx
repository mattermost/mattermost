...
const CloudTrialBanner = ({ trialEndDate }: Props): JSX.Element | null => {
    ...
    const storedDismissedEndDate = useSelector((state: GlobalState) =>
        getPreference(
            state,
            Preferences.CLOUD_TRIAL_BANNER,
            CloudBanners.UPGRADE_FROM_TRIAL,
        ),
    );
    ...
    const onDismissBanner = () => {
        setShowBanner(false);
        const addTenDaysToToday = moment(new Date())
            .add(DISMISSED_DAYS, "days")
            .format("x");
        dispatch(
            ...
            actionButtonLeft={
                <UpgradeLink
                    buttonText={formatMessage({
                        id: "admin.subscription.cloudTrialCard.upgrade",
