...
const Limits = (): JSX.Element | null => {
    ...
    if (subscriptionProduct.sku === CloudProducts.STARTER) {
        title = (
            <FormattedMessage
                ...
                values={{
                    planName:
                        products?.[subscription?.product_id || ""]?.name ||
                        "Unknown product",
                }}
            />
        );
        description = intl.formatMessage(
            {
                ...
                defaultMessage:
                    "{planName} is restricted to {messagesLimit} message history and {storageLimit} file storage. You can delete items to free up space or upgrade to a paid plan.",
            },
            {
                planName: subscriptionProduct.name,
                messagesLimit: intl.formatNumber(
                    cloudLimits?.messages?.history ||
                        fallbackStarterLimits.messages.history,
                ),
                storageLimit: asGBString(
                    ...
                    <LimitCard
                        ...
                        status={
                            <FormattedMessage
                                ...
                                values={{
                                    actual: `${Math.floor(
                                        usage.messages.history / 1000,
                                    )}K`,
                                    limit: `${Math.floor(
                                        cloudLimits.messages.history / 1000,
                                    )}K`,
                                    percent: Math.floor(
                                        (usage.messages.history /
                                            cloudLimits.messages.history) *
                                            100,
                                    ),
                                }}
    ...
    return (
        <div className={panelClassname}>
            ...
            {description && (
                <div className="ProductLimitsPanel__description">
                    {description}
                </div>
            )}
            {currentUsage}
                    ...
                    <>
                        <button
                            onClick={() =>
                                openPricingModal({
                                    trackingLocation:
                                        "billing_subscriptions_limits_dashboard",
                                })
                            }
