...
const ToYearlyNudgeBannerDismissable = () => {
    ...
    const snoozePreferenceVal = useSelector((state: GlobalState) =>
        getPreference(
            state,
            Preferences.TO_CLOUD_YEARLY_PLAN_NUDGE,
            CloudBanners.NUDGE_TO_CLOUD_YEARLY_PLAN_SNOOZED,
            '{"range": 0, "show": true}',
        ),
    );
    const snoozeInfo = JSON.parse(
        snoozePreferenceVal,
    ) as ToYearlyPlanDismissPreference;
    ...
    const currentProductIsMonthly =
        product?.recurring_interval === RecurringIntervals.MONTH;
    const currentProductProMonthly =
        currentProductProfessional && currentProductIsMonthly;
    ...
    useEffect(() => {
        if (!snoozeInfo.show) {
            if (
                daysToProMonthlyEnd >= 90 &&
                !snoozedForRange(DismissShowRange.GreaterThanEqual90)
            ) {
                showBanner(true);
            }
            ...
            if (
                daysToProMonthlyEnd < 90 &&
                daysToProMonthlyEnd > 60 &&
                !snoozedForRange(DismissShowRange.BetweenNinetyAnd60)
            ) {
                showBanner(true);
            }
            ...
            if (
                daysToProMonthlyEnd <= 60 &&
                daysToProMonthlyEnd > 30 &&
                !snoozedForRange(DismissShowRange.SixtyTo31)
            ) {
                showBanner(true);
            }
            ...
            if (
                daysToProMonthlyEnd <= 30 &&
                daysToProMonthlyEnd > 10 &&
                !snoozedForRange(DismissShowRange.ThirtyTo11)
            ) {
                showBanner(true);
            }
    ...
    if (
        subscription?.billing_type === CloudBillingTypes.INTERNAL ||
        subscription?.billing_type === CloudBillingTypes.LICENSED
    ) {
        return null;
    }
    ...
    const announcementType =
        daysToProMonthlyEnd <= 10
            ...
            : AnnouncementBarTypes.ANNOUNCEMENT;
    ...
    return (
        <AnnouncementBar
            ...
            onButtonClick={() =>
                openPurchaseModal({
                    trackingLocation: "to_yearly_nudge_annoucement_bar",
                })
            }
...
const ToYearlyNudgeBanner = () => {
    ...
    const description = (
        <FormattedMessage
            ...
            values={{
                date: moment(cloudProMonthlyCloseMoment, "YYYYMMDD").format(
                    "MMMM DD, YYYY",
                ),
            }}
        />
    ...
    const viewPlansAction = (
        <button
            onClick={() =>
                openPurchaseModal({
                    trackingLocation: "to_yearly_nudge_banner",
                })
            }
            className="btn ToYearlyNudgeBanner__primary"
        >
            {formatMessage({
                id: "cloud_billing.nudge_to_yearly.learn_more",
    ...
    const contactSalesAction = (
        ...
        >
            {formatMessage({
                id: "cloud_billing.nudge_to_yearly.contact_sales",
...
export { ToYearlyNudgeBanner, ToYearlyNudgeBannerDismissable };
