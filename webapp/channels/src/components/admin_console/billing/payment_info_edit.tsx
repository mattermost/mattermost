...
import {
    STRIPE_CSS_SRC,
    getStripePublicKey,
} from "components/payment_form/stripe";
...
const PaymentInfoEdit: React.FC = () => {
    ...
    const stripePublicKey = useSelector((state: GlobalState) =>
        getStripePublicKey(state),
    );
    ...
    const handleSubmit = async () => {
        setIsSaving(true);
        const setPaymentMethod = completeStripeAddPaymentMethod(
            (await stripePromise)!,
            billingDetails!,
            cwsMockMode,
        );
