...
export default class UserGridRoleDropdown extends React.PureComponent<Props> {
    private getDropDownOptions = () => {
        if (this.props.scope === "team") {
            return {
                makeAdmin: Utils.localizeMessage(
                    "team_members_dropdown.makeAdmin",
                    "Make Team Admin",
                ),
                makeMember: Utils.localizeMessage(
        ...
        return {
            makeAdmin: Utils.localizeMessage(
                "channel_members_dropdown.make_channel_admin",
                "Make Channel Admin",
            ),
            makeMember: Utils.localizeMessage(
                "channel_members_dropdown.make_channel_member",
                "Make Channel Member",
            ),
        };
    };
        ...
        switch (role) {
            case "system_admin":
                return Utils.localizeMessage(
                    "admin.user_grid.system_admin",
                    "System Admin",
                );
            case "team_admin":
                return Utils.localizeMessage(
                    "admin.user_grid.team_admin",
                    "Team Admin",
                );
            case "channel_admin":
                return Utils.localizeMessage(
                    "admin.user_grid.channel_admin",
                    "Channel Admin",
                );
            case "shared_member":
                return Utils.localizeMessage(
                    "admin.user_grid.shared_member",
                    "Shared Member",
                );
            case "team_user":
            case "channel_user":
                return Utils.localizeMessage(
                    "admin.group_teams_and_channels_row.member",
                    "Member",
                );
            default:
    ...
    public render = (): React.ReactNode => {
        ...
        const dropdownEnabled = !["system_admin", "guest"].includes(
            currentRole,
        );
        const showMakeAdmin = ["channel_user", "team_user"].includes(
            currentRole,
        );
        const showMakeMember = ["channel_admin", "team_admin"].includes(
            currentRole,
        );
