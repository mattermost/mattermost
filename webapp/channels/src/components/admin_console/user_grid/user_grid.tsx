...
type Props = {
    ...
    scope: "team" | "channel";
    memberships: {
        [userId: string]: BaseMembership | TeamMembership | ChannelMembership;
    };
...
export default class UserGrid extends React.PureComponent<Props, State> {
    ...
    private getRows = (): Row[] => {
        ...
        if (
            term === "" &&
            usersToDisplay.length < USERS_PER_PAGE &&
            users.length < totalCount
        ) {
            const numberOfUsersRemoved = Object.keys(excludeUsers).length;
            const pagesOfUsersRemoved = Math.floor(
                numberOfUsersRemoved / USERS_PER_PAGE,
            );
        ...
        return usersToDisplay.map((user) => {
            const membership =
                membershipsToUpdate[user.id] ||
                memberships[user.id] ||
                this.newMembership(user);
            return {
                cells: {
                    id: user.id,
    ...
    private getColumns = (): Column[] => {
        ...
        const role: JSX.Element = (
            <FormattedMessage id="admin.user_grid.role" defaultMessage="Role" />
        );
