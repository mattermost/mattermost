...
export default class PushSettings extends AdminSettings<Props, State> {
    ...
    handleDropdownChange = (id: string, value: string) => {
            ...
            if (value === PUSH_NOTIFICATIONS_MHPNS) {
                this.setState({
                    pushNotificationServer:
                        PUSH_NOTIFICATIONS_SERVER_DIC[
                            this.state.pushNotificationServerLocation
                        ],
                });
        ...
        if (id === DROPDOWN_ID_SERVER_LOCATION) {
            this.setState({
                pushNotificationServer:
                    PUSH_NOTIFICATIONS_SERVER_DIC[
                        value as EmailSettings["PushNotificationServerLocation"]
                    ],
    ...
    getConfigFromState = (config: Props["config"]) => {
        ...
        config.EmailSettings.PushNotificationServer =
            this.state.pushNotificationServer.trim();
    ...
    getStateFromConfig(config: Props["config"]) {
        let pushNotificationServerType: EmailSettings["PushNotificationServerType"] =
            PUSH_NOTIFICATIONS_CUSTOM;
        let agree = false;
        let pushNotificationServerLocation: EmailSettings["PushNotificationServerLocation"] =
            PUSH_NOTIFICATIONS_LOCATION_US;
        ...
        if (pushNotificationServerType === PUSH_NOTIFICATIONS_MTPNS) {
            ...
            pushNotificationServer =
                PUSH_NOTIFICATIONS_SERVER_DIC[pushNotificationServerLocation];
        }
    ...
    renderSettings = () => {
        ...
        if (
            ...
            const pushNotificationServerLocations = [
                {
                    value: PUSH_NOTIFICATIONS_LOCATION_US,
                    text: Utils.localizeMessage(
                        "admin.email.pushServerLocationUS",
                        "US",
                    ),
                },
                {
                    value: PUSH_NOTIFICATIONS_LOCATION_DE,
                    text: Utils.localizeMessage(
                        "admin.email.pushServerLocationDE",
                        "Germany",
                    ),
                },
            ];
            ...
            <SettingsGroup>
                ...
                <TextSetting
                    ...
                    placeholder={Utils.localizeMessage(
                        ...
                        'E.g.: "https://push-test.mattermost.com"',
                    )}
