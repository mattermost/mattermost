...
export default function getEnablePluginSetting(
    plugin: PluginRedux,
): Partial<AdminDefinitionSetting & PluginSetting> {
    const escapedPluginId = SchemaAdminSettings.escapePathPart(plugin.id);
    const pluginEnabledConfigKey =
        "PluginSettings.PluginStates." + escapedPluginId + ".Enable";
