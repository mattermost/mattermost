...
function makeGetPluginSchema() {
    return createSelector(
        "makeGetPluginSchema",
        (state: GlobalState, pluginId: string) =>
            state.entities.admin.plugins?.[pluginId],
        (state: GlobalState, pluginId: string) =>
            getAdminConsoleCustomComponents(state, pluginId),
        (state) => appsFeatureFlagEnabled(state),
        isCurrentLicenseCloud,
        (
            plugin: PluginRedux | undefined,
            customComponents: Record<string, AdminConsolePluginComponent>,
            ...
            if (plugin.settings_schema && plugin.settings_schema.settings) {
                settings = plugin.settings_schema.settings.map((setting) => {
                    ...
                    let isDisabled = it.any(
                        it.stateIsFalse(pluginEnabledConfigKey),
                    ...
                    if (customComponents[key]) {
                        ...
                        setting.type === Constants.SettingsTypes.TYPE_CUSTOM
                    ) {
                        ...
                        bannerType = "warning";
                        isDisabled = it.any(
                            it.stateIsTrue(pluginEnabledConfigKey),
                            it.not(
                    ...
                    const isHidden = () => {
                        return (
                            (isCloudLicense && setting.hosting === "on-prem") ||
                    ...
                    return {
                        ...
                        component,
                        showTitle: customComponents[key]
                            ? customComponents[key].options.showTitle
                            : false,
                    };
            ...
            if (plugin.id !== appsPluginID || appsFeatureFlagIsEnabled) {
                ...
                if (pluginEnableSetting.isDisabled) {
                    pluginEnableSetting.isDisabled = it.any(
                        pluginEnableSetting.isDisabled,
                        it.not(it.userHasWritePermissionOnResource("plugins")),
                    );
                } else {
                    pluginEnableSetting.isDisabled = it.not(
                        it.userHasWritePermissionOnResource("plugins"),
                    );
                }
