...
export type Props = {
    ...
    actions: {
        ...
        revokeAllSessionsForUser: (
            id: string,
        ) => Promise<{ error: ServerError; data: any }>;
...
> {
    ...
    handleMakeActive = (
        ...
        disableActivationToggle: boolean,
    ) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    handleShowDeactivateMemberModal = async (
        e: React.MouseEvent<HTMLButtonElement>,
        disableActivationToggle: boolean,
    ) => {
        e.preventDefault();
        ...
        if (this.shouldDisableBotsWhenOwnerIsDeactivated()) {
            for (const bot of Object.values(this.props.bots)) {
                if (
                    ...
                    this.state.showDeactivateMemberModal &&
                    bot.delete_at === 0
                ) {
    ...
    handleShowCreateGroupSyncableMembershipsModal = (e: {
        preventDefault: () => void;
    }) => {
        e.preventDefault();
    ...
    handleCreateGroupSyncableMemberships = async () => {
        const { error } = await this.props.actions.createGroupTeamsAndChannels(
            this.props.user.id,
        );
        if (error) {
    ...
    handleRevokeSessions = async () => {
        ...
        const { data, error } =
            await this.props.actions.revokeAllSessionsForUser(
                this.props.user.id,
            );
    ...
    handlePromoteToUserConfirm = async () => {
        const { error } = await this.props.actions.promoteGuestToUser(
            this.props.user.id,
        );
        if (error) {
            this.props.onError(error);
    ...
    handleDemoteToGuestConfirm = async () => {
        const { error } = await this.props.actions.demoteUserToGuest(
            this.props.user.id,
        );
        if (error) {
            this.props.onError(error);
    ...
    render() {
        ...
        if (!isDisabled) {
            ...
            isDisabled =
                UserUtils.isSystemAdmin(user.roles) &&
                !UserUtils.isSystemAdmin(currentUser.roles);
        }
        ...
        const createGroupSyncablesMembershipsModal =
            this.renderCreateGroupSyncablesMembershipsModal();

        const getExtraText = (disableActivationToggle: boolean) => {
            return disableActivationToggle
                ? {
                      extraText: Utils.localizeMessage(
                          "admin.user_item.managedByLdap",
                          "Managed by LDAP",
                      ),
                  }
                : {};
                ...
                <MenuWrapper isDisabled={isDisabled}>
                    <div className="text-right">
                        <a>
                            ...
                            {!isDisabled && <span className="caret" />}
                        </a>
                    ...
                    <Menu
                        openLeft={true}
                        openUp={
                            totalUsers > TOTAL_USERS_TO_OPEN_UP &&
                    ...
                    >
                        ...
                        <Menu.ItemAction
                            ...
                            onClick={(e: React.MouseEvent<HTMLButtonElement>) =>
                                this.handleShowDeactivateMemberModal(
                                    e,
                                    disableActivationToggle,
                                )
                            }
                        ...
                        <Menu.ItemAction
                            show={
                                ...
                                isLicensed &&
                                config.GuestAccountsSettings?.Enable
                            }
                            onClick={this.handleDemoteToGuest}
                            text={Utils.localizeMessage(
                                "admin.user_item.demoteToGuest",
                                "Demote to Guest",
                            )}
                        />
                        <SystemPermissionGate
                            permissions={[Permissions.REVOKE_USER_ACCESS_TOKEN]}
                        >
                            <Menu.ItemAction
                        ...
                        <SystemPermissionGate
                            permissions={[
                                Permissions.SYSCONSOLE_WRITE_USERMANAGEMENT_GROUPS,
                            ]}
                        >
                            <Menu.ItemAction
                                show={
                                ...
                                onClick={
                                    this
                                        .handleShowCreateGroupSyncableMembershipsModal
                                }
                                text={Utils.localizeMessage(
                                    "admin.user_item.createGroupTeamChannelMemberships",
                                    "Re-sync user via LDAP groups",
                                )}
                            />
                        </SystemPermissionGate>
                    </Menu>
