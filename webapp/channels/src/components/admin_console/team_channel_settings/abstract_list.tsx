...
export default class AbstractList extends React.PureComponent<Props, State> {
    ...
    performSearch = (page: number): void => {
        ...
        this.props.actions
            .getData(page, PAGE_SIZE, "", false, true)
            .then((response) => {
                if (this.props.onPageChangedCallback) {
                    this.props.onPageChangedCallback(
                        this.getPaging(),
                        response,
                    );
                }
    ...
    render = () => {
        ...
        return (
            <div
                className={classNames("groups-list", "groups-list-no-padding", {
                    "groups-list-less-padding": noPadding,
                })}
            >
                {this.renderHeader()}
                <div id="groups-list--body" className="groups-list--body">
                    {this.renderRows()}
                </div>
                {total > 0 && (
