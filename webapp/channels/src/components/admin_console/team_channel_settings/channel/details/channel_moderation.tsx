...
const MEMBERS_CAN_MANAGE_CHANNEL_MEMBERS_PERMISSION =
    "manage_{public_or_private}_channel_members";
const GUESTS_CAN_MANAGE_CHANNEL_MEMBERS_PERMISSION =
    "guest_manage_{public_or_private}_channel_members";
...
function getChannelModerationPermissionNames(permission: string) {
    ...
    if (
        permission === Permissions.CHANNEL_MODERATED_PERMISSIONS.MANAGE_MEMBERS
    ) {
        return {
...
function getChannelModerationRowsMessages(
    permission: string,
): Record<string, MessageDescriptor> | null {
...
const channelModerationHeaderMessages = defineMessages({
    ...
    subtitleMessageForMembersAndGuests: {
        ...
        defaultMessage:
            "Manage the actions available to channel members and guests.",
    },
...
export const ChannelModerationTableRow = (props: ChannelModerationTableRow) => {
    const channelModerationPermissionMessages =
        getChannelModerationRowsMessages(props.name);
    let descriptionId = channelModerationPermissionMessages?.description.id;
    let descriptionDefaultMessage =
        channelModerationPermissionMessages?.description.defaultMessage;
    if (
        !props.guestAccountsEnabled &&
        channelModerationPermissionMessages?.descriptionMembers
    ) {
        descriptionId =
            channelModerationPermissionMessages.descriptionMembers?.id ?? "";
    ...
    return (
        <tr>
            <td>
                <label
                    data-testid={channelModerationPermissionMessages?.title?.id?.replace(
                        PERIOD_TO_SLASH_REGEX,
                        "-",
                    )}
                >
                    <FormattedMessage
                        ...
                        defaultMessage={
                            channelModerationPermissionMessages?.title
                                ?.defaultMessage
                        }
                    />
                </label>
                <div
                    data-testid={channelModerationPermissionMessages?.description?.id?.replace(
                        PERIOD_TO_SLASH_REGEX,
                        "-",
                    )}
                >
                    ...
                    {!isNil(props.guests) && (
                        <button
                            ...
                            onClick={() =>
                                props.onClick(
                                    props.name,
                                    Roles.GUESTS as ChannelModerationRoles,
                                )
                            }
                            disabled={props.guestsDisabled || props.readOnly}
                        >
                            {props.guests && !props.guestsDisabled && (
                                <CheckboxCheckedIcon />
                            )}
                        </button>
                    )}
                ...
                {!isNil(props.members) && (
                    <button
                        ...
                        onClick={() =>
                            props.onClick(
                                props.name,
                                Roles.MEMBERS as ChannelModerationRoles,
                            )
                        }
                        disabled={props.membersDisabled || props.readOnly}
                    >
                        {props.members && !props.membersDisabled && (
                            <CheckboxCheckedIcon />
                        )}
                    </button>
                )}
...
export default class ChannelModeration extends React.PureComponent<Props> {
    private errorMessagesToDisplay = (
        entry: ChannelPermissions,
    ): JSX.Element[] => {
        const channelModerationPermissionMessages =
            getChannelModerationRowsMessages(entry.name);

        const errorMessages: JSX.Element[] = [];
        const isGuestsDisabled =
            !isNil(entry.roles.guests?.enabled) &&
            !entry.roles.guests?.enabled &&
            this.props.guestAccountsEnabled;
        ...
        if (
            ...
            if (
                !createPostsObject!.roles.guests!.value &&
                this.props.guestAccountsEnabled &&
                !createPostsObject!.roles.members!.value
            ) {
                errorMessages.push(
                    <div
                        data-testid={channelModerationPermissionMessages?.disabledBothDueToCreatePosts?.id?.replace(
                            PERIOD_TO_SLASH_REGEX,
                            "-",
                        )}
                        key={
                            channelModerationPermissionMessages
                                ?.disabledBothDueToCreatePosts?.id
                        }
                    >
                        <FormattedMessage
                            id={
                ...
                errorMessages.push(
                    <div
                        data-testid={channelModerationPermissionMessages?.disabledGuestsDueToCreatePosts?.id?.replace(
                            PERIOD_TO_SLASH_REGEX,
                            "-",
                        )}
                        key={
                            channelModerationPermissionMessages
                                ?.disabledGuestsDueToCreatePosts?.id
                        }
                    >
                        <FormattedMessage
                            id={
                ...
                errorMessages.push(
                    <div
                        data-testid={channelModerationPermissionMessages?.disabledMembersDueToCreatePosts?.id?.replace(
                            PERIOD_TO_SLASH_REGEX,
                            "-",
                        )}
                        key={
                            channelModerationPermissionMessages
                                ?.disabledMembersDueToCreatePosts?.id
                        }
                    >
                        <FormattedMessage
                            id={
        ...
        if (
            ...
            if (permissionName?.disabledBoth) {
                ...
                if (schemeLink.includes("{public_or_private}")) {
                    const publicOrPrivate = this.props.isPublic
                        ? "public"
                        : "private";
                    schemeLink = schemeLink.replace(
                        "{public_or_private}",
                        publicOrPrivate,
                    );
                }
            ...
            disabledKeyId =
                channelModerationPermissionMessages?.disabledBoth?.id;
            disabledKeyMessage =
                channelModerationPermissionMessages?.disabledBoth
                    ?.defaultMessage;
        } else if (
            isGuestsDisabled &&
            createPostsKey !== "disabledGuestsDueToCreatePosts"
            ...
            disabledKeyId =
                channelModerationPermissionMessages?.disabledGuests?.id;
            disabledKeyMessage =
                channelModerationPermissionMessages?.disabledGuests
                    ?.defaultMessage;
        } else if (
            isMembersDisabled &&
            createPostsKey !== "disabledMembersDueToCreatePosts"
            ...
            disabledKeyId =
                channelModerationPermissionMessages?.disabledMembers?.id;
            disabledKeyMessage =
                channelModerationPermissionMessages?.disabledMembers
                    ?.defaultMessage;
        }
        ...
        if (disabledKey) {
            errorMessages.push(
                <div
                    data-testid={disabledKeyId?.replace(
                        PERIOD_TO_SLASH_REGEX,
                        "-",
                    )}
                    key={disabledKeyId}
    ...
    render = (): JSX.Element => {
        ...
        return (
            <AdminPanel
                ...
                titleDefault={
                    channelModerationHeaderMessages.titleMessage.defaultMessage
                }
                subtitleId={
                    guestAccountsEnabled
                        ? channelModerationHeaderMessages
                              .subtitleMessageForMembersAndGuests.id
                        : channelModerationHeaderMessages
                              .subtitleMessageForMembers.id
                }
                subtitleDefault={
                    guestAccountsEnabled
                        ? channelModerationHeaderMessages
                              .subtitleMessageForMembersAndGuests.defaultMessage
                        : channelModerationHeaderMessages
                              .subtitleMessageForMembers.defaultMessage
                }
            >
                                ...
                                {channelPermissions?.map((entry) => {
                                    return (
                                        <ChannelModerationTableRow
                                            ...
                                            membersDisabled={
                                                !entry.roles.members.enabled
                                            }
                                            onClick={
                                                onChannelPermissionsChanged
                                            }
                                            errorMessages={this.errorMessagesToDisplay(
                                                entry,
                                            )}
                                            guestAccountsEnabled={
                                                guestAccountsEnabled
