...
interface ChannelDetailsState {
    ...
    originalAccessControlPolicies: AccessControlPolicy[]; // Track original state for diff
...
export default class ChannelDetails extends React.PureComponent<ChannelDetailsProps, ChannelDetailsState> {
    constructor(props: ChannelDetailsProps) {
        ...
        this.state = {
            ...
            originalAccessControlPolicies: [], // Initialize original state tracking
    ...
    private handleSubmit = async () => {
        ...
        if (policyToggled) {
            ...
            // Calculate the diff: which policies to add and which to remove
            const {originalAccessControlPolicies} = this.state;
            const currentPolicyIds = accessControlPolicies.map((p) => p.id);
            const originalPolicyIds = originalAccessControlPolicies.map((p) => p.id);

            // Policies to add: in current but not in original
            const policiesToAdd = accessControlPolicies.filter(
                (policy) => !originalPolicyIds.includes(policy.id),
            );

            // Policies to remove: in original but not in current OR explicitly marked for removal
            const policyIdsToRemove = [
                ...originalPolicyIds.filter((id) => !currentPolicyIds.includes(id)),
                ...accessControlPoliciesToRemove,
            ];

            // Only assign policies that are newly added
            ...
            // Unassign policies that were removed
            if (policyIdsToRemove.length > 0) {
                try {
                    await Promise.all(
                        policyIdsToRemove.map((policyId) =>
        ...
        if (policyToggled && this.state.channelRulesHaveChanges) {
            try {
                ...
                // Check if we're entering empty rules state (SAME LOGIC as Channel Settings Modal)
                ...
                if (isEmptyRulesState) {
                    ...
                    // Create or update channel-level access policy
                        const channelPolicy: AccessControlPolicy = {
                            ...
                            // Include parent policies as imports (SAME LOGIC as Channel Settings Modal)
                        ...
                        // No parent policies, delete policy entirely (SAME LOGIC as Channel Settings Modal)
                        if (this.state.accessControlPolicies.length === 0) {
                            await actions.deleteAccessControlPolicy(channelID).catch((error) => {
                                if (error.message && !error.message.includes('not found')) {
                                    serverError = <FormError error={error.message}/>;
        ...
        this.setState({
            ...
            originalAccessControlPolicies: !serverError && !saveNeeded ? [...this.state.accessControlPolicies] : this.state.originalAccessControlPolicies,
            accessControlPoliciesToRemove: !serverError && !saveNeeded ? [] : this.state.accessControlPoliciesToRemove, // Clear removal list on successful save
        }, () => {
            actions.setNavigationBlocked(saveNeeded);
    ...
    private combineParentAndChannelExpressions = (channelExpression: string): string => {
        // Get expressions from parent policies (same logic as Channel Settings Modal)
                ...
                if (currentAccessControlPolicy.imports && currentAccessControlPolicy.imports.length > 0) {
                    ...
                    Promise.all(promises).then(() => {
                        this.setState({
                            ...
                            originalAccessControlPolicies: [...policies], // Track original state
                        });
                    });
                } else {
                    ...
                    this.setState({
                        accessControlPolicies: [],
                        originalAccessControlPolicies: [], // Track original state
                    });
                }
            }
        });
