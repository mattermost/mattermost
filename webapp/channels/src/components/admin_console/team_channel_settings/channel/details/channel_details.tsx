...
export type ChannelDetailsActions = {
    ...
    ) => { data: Channel; error: ServerError };
...
export default class ChannelDetails extends React.PureComponent<
    ChannelDetailsProps,
    ChannelDetailsState
> {
    ...
    componentDidUpdate(prevProps: ChannelDetailsProps) {
        ...
        if (
            !prevProps.team.id &&
            !prevProps.channel.team_id &&
            channel.team_id
        ) {
            actions
                .getTeam(channel.team_id)
                .then(async (data: any) => {
                ...
                .then(() =>
                    this.setState({ teamScheme: this.props.teamScheme }),
                );
        }
    }
        ...
        if (channelID) {
            if (this.props.channelModerationEnabled) {
                ...
                actions
                    ...
                    .then(() => this.restrictChannelMentions());
            }
    ...
    private restrictChannelMentions() {
        ...
        const currentCreatePostRoles: any = channelPermissions.find(
            (element) =>
                element.name ===
        ...
        if (currentCreatePostRoles) {
            for (const channelRole of Object.keys(currentCreatePostRoles)) {
                channelPermissions = channelPermissions.map((permission) => {
                    if (
                        permission.name ===
                            Permissions.CHANNEL_MODERATED_PERMISSIONS
                                .USE_CHANNEL_MENTIONS &&
                        (!currentCreatePostRoles[channelRole].value ||
                            !currentCreatePostRoles[channelRole].enabled)
                    ) {
                        return {
    ...
    async processGroupsChange(groups: Group[]) {
        ...
        if (this.state.isSynced) {
            try {
                if (groups.length === 0) {
                    serverError = (
                        <NeedGroupsError warning={true} isChannel={true} />
                    );
                } else {
                    if (!channelID) {
                        return;
    ...
    private channelPermissionsChanged = (
        name: string,
        channelRole: ChannelModerationRoles,
    ) => {
        const currentValueIndex = this.state.channelPermissions.findIndex(
            (element) => element.name === name,
        );
        const currentValue =
            this.state.channelPermissions[currentValueIndex].roles[channelRole]!
                .value;
        const newValue = !currentValue;
        let channelPermissions = [...this.state.channelPermissions];

        if (name === Permissions.CHANNEL_MODERATED_PERMISSIONS.CREATE_POST) {
            const originalObj = this.props.channelPermissions.find(
                (element) =>
                    element.name ===
                    Permissions.CHANNEL_MODERATED_PERMISSIONS
                        .USE_CHANNEL_MENTIONS,
    ...
    private handleGroupChange = (groupIDs: string[]) => {
        const groups = [
            ...this.state.groups,
            ...groupIDs.map((gid: string) => this.props.allGroups[gid]),
        ];
        this.setState({
    ...
    private handleSubmit = async () => {
        ...
        if (isPrivacyChanging) {
            const convert = actions.updateChannelPrivacy(
                channel.id,
                isPublic ? Constants.OPEN_CHANNEL : Constants.PRIVATE_CHANNEL,
            );
            promises.push(
        ...
        const link = groups
            ...
            .map((g) =>
                actions.linkGroupSyncable(
                    g.id,
                    channelID,
                    SyncableType.Channel,
                    { auto_add: true, scheme_admin: g.scheme_admin },
                ),
            );

        const groupActions = [
            ...promises,
            ...patchChannelSyncable,
            ...unlink,
            ...link,
        ];
            ...
            if (resultWithError && "error" in resultWithError) {
                ...
                if (unlink.length > 0) {
                    trackEvent(
                        ...
                        "groups_removed_from_channel",
                        { count: unlink.length, channel_id: channelID },
                    );
                }
                if (link.length > 0) {
                    trackEvent(
                        "admin_channel_config_page",
                        "groups_added_to_channel",
                        { count: link.length, channel_id: channelID },
                    );
                }
                    ...
                    if (this.props.channelModerationEnabled) {
                        actionsToAwait.push(
                            actions
                                .getChannelModerations(channelID)
                                .then(() => {
                                    const manageMembersIndex =
                                        channelPermissions.findIndex(
                                            (element) =>
                                                element.name ===
                                                Permissions
                                    ...
                                    if (channelPermissions) {
                                        const updatedManageMembers =
                                            this.props.channelPermissions.find(
                                                (element) =>
                                                    element.name ===
                                                    Permissions
                                                        .CHANNEL_MODERATED_PERMISSIONS
                                        ...
                                        channelPermissions[manageMembersIndex] =
                                            updatedManageMembers ||
                                            channelPermissions[
                                                manageMembersIndex
                                            ];
                                    }
            ...
            const patchChannelPermissionsArray: ChannelModerationPatch[] =
                channelPermissions.map((p) => {
                    return {
                        ...
                        roles: {
                            ...(p.roles.members &&
                                p.roles.members.enabled && {
                                    members: p.roles.members!.value,
                                }),
                            ...(p.roles.guests &&
                                p.roles.guests.enabled && {
                                    guests: p.roles.guests!.value,
                                }),
                        },
                    };
                });

            const patchChannelModerationsResult =
                await actions.patchChannelModerations(
                    channelID,
                    patchChannelPermissionsArray,
                );
            if (patchChannelModerationsResult.error) {
                serverError = (
                    <FormError
                        error={patchChannelModerationsResult.error.message}
                    />
                );
            }
        ...
        if (usersToUpdate && !isSynced) {
            ...
            if (addUserActions.length > 0) {
                ...
                if (count > 0) {
                    trackEvent(
                        "admin_channel_config_page",
                        "members_added_to_channel",
                        { count, channel_id: channelID },
                    );
                }
            }
            ...
            if (rolesToPromote.length > 0) {
                ...
                if (count > 0) {
                    trackEvent(
                        "admin_channel_config_page",
                        "members_elevated_to_channel_admin",
                        { count, channel_id: channelID },
                    );
                }
            }
            ...
            if (rolesToDemote.length > 0) {
                ...
                if (count > 0) {
                    trackEvent(
                        "admin_channel_config_page",
                        "admins_demoted_to_channel_member",
                        { count, channel_id: channelID },
                    );
                }
            }
    ...
    public render = (): JSX.Element => {
        ...
        const missingGroup = (og: { id: string }) =>
            !groups.find((g: Group) => g.id === og.id);
        ...
        const nonArchivedContent = (
            <>
                ...
                {this.props.channelModerationEnabled && (
                    <ChannelModeration
                        channelPermissions={channelPermissions}
                        onChannelPermissionsChanged={
                            this.channelPermissionsChanged
                        }
                        ...
                        isPublic={
                            this.props.channel.type === Constants.OPEN_CHANNEL
                        }
