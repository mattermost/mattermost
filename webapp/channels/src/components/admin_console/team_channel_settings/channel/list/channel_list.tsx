...
> {
    ...
    searchChannels = async (page = 0, term = "", filters = {}) => {
        ...
        const response = await this.props.actions.searchAllChannels(term, {
            ...
            include_search_by_id: true,
        });
        if (response?.data) {
            channels =
                page > 0
                    ? this.state.channels.concat(response.data.channels)
    ...
    getRows = (): Row[] => {
        ...
        let channelsToDisplay = this.isSearching(term, filters)
            ? channels
            : data;
    ...
    onFilter = (filterOptions: FilterOptions) => {
        ...
        if (
            ...
            if (filters.public) {
                trackEvent(
                    "admin_channels_page",
                    "public_filter_applied_to_channel_list",
                );
            }
            ...
            if (filters.private) {
                trackEvent(
                    "admin_channels_page",
                    "private_filter_applied_to_channel_list",
                );
            }
            ...
            if (filters.deleted) {
                trackEvent(
                    "admin_channels_page",
                    "archived_filter_applied_to_channel_list",
                );
            }
            ...
            if (!(groupConstrained.value && excludeGroupConstrained.value)) {
                ...
                if (filters.group_constrained) {
                    trackEvent(
                        "admin_channels_page",
                        "group_sync_filter_applied_to_channel_list",
                    );
                }
                filters.exclude_group_constrained =
                    excludeGroupConstrained.value as boolean;
                if (filters.exclude_group_constrained) {
                    trackEvent(
                        "admin_channels_page",
                        "manual_invites_filter_applied_to_channel_list",
                    );
                }
            }
            ...
            if (filters.team_ids.length > 0) {
                trackEvent(
                    "admin_channels_page",
                    "team_id_filter_applied_to_channel_list",
                );
            }
        }
