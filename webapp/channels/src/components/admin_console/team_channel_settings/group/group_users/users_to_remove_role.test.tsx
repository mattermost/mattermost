...
describe("components/admin_console/team_channel_settings/group/UsersToRemoveRole", () => {
    const groups = [
        TestHelper.getGroupMock({ id: "group1", display_name: "group1" }),
    ];
    ...
    const teamMembership = TestHelper.getTeamMembershipMock({
        scheme_admin: false,
    });
    const adminTeamMembership = TestHelper.getTeamMembershipMock({
        scheme_admin: true,
    });
    const channelMembership = TestHelper.getChannelMembershipMock(
        { scheme_admin: false },
        {},
    );
    const adminChannelMembership = TestHelper.getChannelMembershipMock(
        { scheme_admin: true },
        {},
    );
    const guestMembership = TestHelper.getTeamMembershipMock({
        scheme_admin: false,
        scheme_user: false,
    });
