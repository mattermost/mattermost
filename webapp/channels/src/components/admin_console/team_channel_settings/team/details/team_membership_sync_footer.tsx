...
import './team_membership_sync_footer.scss';

const JOB_TYPE_TEAM_SYNC = 'access_control_team_sync' as JobType;
...
export default function TeamMembershipSyncFooter({teamId, hasAbacPolicy}: Props) {
    ...
    const fetchSyncStatus = useCallback(async () => {
        try {
            const result = await dispatch(getJobsByType(JOB_TYPE_TEAM_SYNC, 0, JOBS_PER_PAGE, undefined, teamId));
            if (result.data) {
            ...
            // API may fail if the server hasn't been updated yet; show footer anyway
        }
        setLoaded(true);
    }, [dispatch, teamId]);
    ...
    }, [hasAbacPolicy, fetchSyncStatus]);

    const handleSyncNow = useCallback(async () => {
        setSyncing(true);
        try {
            const result = await dispatch(createAccessControlTeamSyncJob({policy_id: teamId}));
            if (result.error) {
                setSyncing(false);
            }
        } catch {
