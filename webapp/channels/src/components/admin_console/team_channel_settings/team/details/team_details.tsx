...
export default class TeamDetails extends React.PureComponent<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this.state = {
            ...
            allowedDomainsChecked: Boolean(
                team.allowed_domains && team.allowed_domains !== "",
            ),
    ...
    componentDidUpdate(prevProps: Props) {
        ...
        if (
            prevProps.team.id !== team.id ||
            totalGroups !== prevProps.totalGroups
        ) {
            this.setState({
                totalGroups,
                syncChecked: Boolean(team.group_constrained),
    ...
    handleSubmit = async () => {
        ...
        if (allowedDomainsChecked && allowedDomains.trim().length === 0) {
            ...
            const patchTeamSyncable = groups
                .filter((g) => {
                    return origGroups.some(
                        (group) =>
                            group.id === g.id &&
                            group.scheme_admin !== g.scheme_admin,
                    );
                })
                .map((g) =>
                    actions.patchGroupSyncable(
                        g.id,
                        teamID,
                        SyncableType.Team,
                        { scheme_admin: g.scheme_admin },
                    ),
                );
            ...
            const link = groups
                ...
                .map((g) =>
                    actions.linkGroupSyncable(g.id, teamID, SyncableType.Team, {
                        auto_add: true,
                        scheme_admin: g.scheme_admin,
                    }),
                );
            ...
            if (resultWithError) {
                ...
                if (unlink.length > 0) {
                    trackEvent(
                        ...
                        "groups_removed_from_team",
                        { count: unlink.length, team_id: teamID },
                    );
                }
                if (link.length > 0) {
                    trackEvent(
                        "admin_team_config_page",
                        "groups_added_to_team",
                        { count: link.length, team_id: teamID },
                    );
                }
        ...
        const usersToUpdate =
            ...
            usersToRemoveList.length > 0 ||
            userRolesToUpdate.length > 0;
        if (usersToUpdate && !syncChecked) {
            ...
            if (addUserActions.length > 0) {
                ...
                if (count > 0) {
                    trackEvent(
                        "admin_team_config_page",
                        "members_added_to_team",
                        { count, team_id: teamID },
                    );
                }
            }
            ...
            if (rolesToPromote.length > 0) {
                ...
                if (count > 0) {
                    trackEvent(
                        "admin_team_config_page",
                        "members_elevated_to_team_admin",
                        { count, team_id: teamID },
                    );
                }
            }
        ...
        if (this.state.syncChecked) {
            try {
                if (groups.length === 0) {
                    ...
                    const result = await actions.membersMinusGroupMembers(
                        teamID,
                        groups.map((g) => g.id),
                    );
                    usersToRemoveCount = result.data
    ...
    handleGroupChange = (groupIDs: string[]) => {
        const groups = [
            ...this.state.groups,
            ...groupIDs.map((gid) => this.props.allGroups[gid]),
        ];
        this.setState({
    ...
    onToggleArchive = () => {
        const { isLocalArchived, serverError, previousServerError } =
            this.state;
    ...
    render = () => {
        ...
        const missingGroup = (og: { id: string }) =>
            !groups.find((g) => g.id === og.id);
