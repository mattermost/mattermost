...
const getConfigurationData = async (
    ...
    icon: (
        <div className="icon">
            <TuneIcon size={20} color={"var(--sys-center-channel-color)"} />
        </div>
    ),
...
const useMetricsData = (config: Partial<AdminConfig>) => {
    ...
    const installedVersion = useSelector((state: GlobalState) =>
        getServerVersion(state),
    );
    const analytics = useSelector(
        (state: GlobalState) => state.entities.admin.analytics,
    ) as unknown as Options["analytics"];
    ...
    const isLicensed =
        license?.IsLicensed === "true" && daysUntilExpiration >= 0;
    ...
    const trialOrEnterpriseCtaConfig = useMemo(
        () => ({
            ...
            configText: canStartTrial
                ? formatMessage({
                      id: "admin.reporting.workspace_optimization.cta.startTrial",
                      defaultMessage: "Start trial",
                  })
                : formatMessage({
                      id: "admin.reporting.workspace_optimization.cta.upgradeLicense",
    ...
    useEffect(() => {
        ...
        const refreshData = async () => {
            const data = {
                ...
                configuration: await getConfigurationData(
                    config,
                    formatMessage,
                    options,
                ),
                ...
                dataPrivacy: await getDataPrivacyData(
                    config,
                    formatMessage,
                    options,
                ),
                easyManagement: await getEaseOfManagementData(
                    config,
                    formatMessage,
                    options,
                ),
