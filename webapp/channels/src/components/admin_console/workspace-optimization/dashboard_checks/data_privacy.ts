...
const dataRetentionCheck = async (
    ...
    const testDataRetention = async (
        ...
        const policyCount: { total_count: number } = await fetch(
            `${Client4.getBaseRoute()}/data_retention/policies_count`,
        ...
        ...(options.isLicensed && options.isEnterpriseLicense
            ? {
                  ...
                  configText: formatMessage({
                      id: "admin.reporting.workspace_optimization.data_privacy.retention.cta",
                      defaultMessage: "Try data retention",
                  }),
              }
...
export const runDataPrivacyChecks = async (
    ...
    const checks = [dataRetentionCheck];
