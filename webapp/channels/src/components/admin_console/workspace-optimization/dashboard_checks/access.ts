...
const siteURLCheck = async (
    ...
    return {
        ...
        configText: formatMessage({
            id: "admin.reporting.workspace_optimization.access.site_url.cta",
            defaultMessage: "Configure web server",
        }),
...
export const runAccessChecks = async (
    ...
    const results = await Promise.all(
        checks.map((check) => check(config, formatMessage)),
    );
    return results;
};
