...
const testServerVersion = async (
    ...
    const fetchVersion = async (
        ...
        const result = await fetch(
            `${Client4.getBaseRoute()}/latest_version`,
        ).then((result) => result.json());

        if (result.tag_name) {
            const sanitizedVersion = result.tag_name.startsWith("v")
                ? result.tag_name.slice(1)
                : result.tag_name;
            const newVersionParts = sanitizedVersion.split(".");
            const installedVersionParts = installedVersion
                .split(".")
                .slice(0, 3);
    ...
    return {
        ...
        configText: formatMessage({
            id: "admin.reporting.workspace_optimization.updates.server_version.cta",
...
export const runUpdateChecks = async (
    ...
    const checks = [testServerVersion];
