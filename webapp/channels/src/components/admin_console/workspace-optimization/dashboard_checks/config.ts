...
const ssl = (
    ...
    return {
        ...
        infoText: formatMessage({
            id: "admin.reporting.workspace_optimization.cta.learnMore",
...
const sessionLength = (
    ...
    const status =
        config.ServiceSettings?.SessionLengthMobileInHours === 720
            ? ItemStatus.INFO
            : ItemStatus.OK;
    return {
        id: "session-length",
        ...
        configText: formatMessage({
            id: "admin.reporting.workspace_optimization.configuration.session_length.cta",
            defaultMessage: "Configure session length",
        }),
...
const fileStorage = async (
    ...
    const testFileStorage = async () => {
        ...
        return pingResponse.filestore_status === "OK"
            ? ItemStatus.OK
            : ItemStatus.ERROR;
    };
    ...
    return {
        ...
        configText: formatMessage({
            id: "admin.reporting.workspace_optimization.configuration.file_storage.cta",
            defaultMessage: "Config file storage",
        }),
...
export const runConfigChecks = async (
    ...
    const checks = [ssl, sessionLength, fileStorage];
    const results = await Promise.all(
        checks.map((check) => check(config, formatMessage, options)),
    );
    return results;
};
