...
const search = async (
    ...
    const testElasticsearch = async (
        ...
        if (
            !options.isLicensed ||
            !options.isEnterpriseLicense ||
            !(
                config.ElasticsearchSettings?.EnableIndexing &&
    ...
    const status =
        totalPosts < 2_000_000 && totalUsers < 500
            ? ItemStatus.OK
            : await testElasticsearch(config, options);
    return {
        id: "search",
        title: formatMessage({
        ...
        ...(options.isLicensed && options.isEnterpriseLicense
            ? {
                  ...
                  configText: formatMessage({
                      id: "admin.reporting.workspace_optimization.search.cta",
                      defaultMessage: "Try Elasticsearch",
                  }),
              }
