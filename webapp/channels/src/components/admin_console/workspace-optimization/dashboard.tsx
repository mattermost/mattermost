...
const successIcon = (
    <div className="success">
        <CheckIcon size={20} color={"var(--sys-online-indicator)"} />
    </div>
);
...
const WorkspaceOptimizationDashboard = (props: Props) => {
    ...
    const accordionItemsData: AccordionItemType[] | undefined =
        data &&
        Object.entries(data)
            .filter(([_, y]) => !y.hide)
            .map(([accordionKey, accordionData]) => {
                ...
                accordionData.items.forEach((item) => {
                    ...
                    overallScore.current +=
                        item.scoreImpact * item.impactModifier;
                ...
                const { title, description, descriptionOk, icon } =
                    accordionData;
                return {
                    title,
                    description:
                        items.length === 0 ? descriptionOk : description,
    ...
    return loading || !accordionItemsData ? (
        <LoadingScreen />
    ) : (
        <div className="WorkspaceOptimizationDashboard wrapper--fixed">
            ...
            <div className="admin-console__wrapper">
                <OverallScore
                    ...
                    chartValue={Math.floor(
                        (overallScore.current / overallScore.max) * 100,
                    )}
                />
                <Accordion
