...
const msg = defineMessages({
    pageTitle: {
        id: "admin.sidebar.classificationMarkings",
    ...
    errorGlobalBannerNoLevel: {
        id: "admin.classification_markings.error.global_banner_no_level",
        defaultMessage:
            "A global classification level must be selected when the global banner is enabled.",
    },
    errorGlobalBannerLevelMissing: {
        id: "admin.classification_markings.error.global_banner_level_missing",
        defaultMessage:
    ...
    errorDeleteHasDependents: {
        id: "admin.classification_markings.error.delete_has_dependents",
        defaultMessage:
...
export default function ClassificationMarkings({ disabled }: Props) {
    ...
    const [existingField, setExistingField] = useState<PropertyField | null>(
        null
    );
    const [existingLinkedField, setExistingLinkedField] =
    ...
    const [globalBanner, setGlobalBanner] = useState<GlobalBannerConfig>({
        ...DEFAULT_GLOBAL_BANNER,
    });
    ...
    const [initialGlobalBanner, setInitialGlobalBanner] =
        useState<GlobalBannerConfig>({ ...DEFAULT_GLOBAL_BANNER });

    const [confirmPresetSwitch, setConfirmPresetSwitch] = useState<
        string | null
    >(null);
    ...
    const hasChanges = useMemo(() => {
        ...
        return levels.some((level, i) => {
            ...
            return (
                ...
                level.color !== initial.color ||
                level.id !== initial.id ||
                level.rank !== initial.rank
            );
        });
    }, [
        enabled,
        ...
        (async () => {
            try {
                ...
                if (field) {
                    ...
                    let banner: GlobalBannerConfig = {
                        ...DEFAULT_GLOBAL_BANNER,
                    };
                    if (linkedField) {
                        const actions =
                            (linkedField.attrs?.actions as string[]) ?? [];
                        let levelName = "";
                        if (actions.includes(DISPLAY_BANNER_TOP)) {
                            const optionId =
                                await fetchSystemClassificationValue(
                                    linkedField.id
                                );
                            if (cancelled) {
                                return;
                            ...
                            if (optionId) {
                                const options =
                                    ...
                                    [];
                                levelName =
                                    findOptionById(options, optionId)?.name ??
                                    "";
                            }
                        }
                ...
                if (!isNotFound) {
                    const message =
                        err instanceof Error
                            ? err.message
                            : "Failed to load classification markings";
    ...
    const handleClassificationEnabledChange = useCallback(
    ...
    const presetDropdownValue = useMemo(() => {
        return (
            presetDropdownOptions.find((o) => o.value === presetId) ??
            presetDropdownOptions[presetDropdownOptions.length - 1]!
        );
    }, [presetDropdownOptions, presetId]);

    const handlePresetDropdownChange = useCallback(
    ...
    const updateLevel = useCallback(
        (id: string, updates: Partial<ClassificationLevel>) => {
            setLevels((prev) =>
                prev.map((level) =>
    ...
    const handleGlobalBannerChange = useCallback(
        (updates: Partial<GlobalBannerConfig>) => {
        ...
        if (enabled) {
            if (levels.length === 0) {
                return formatMessage({
                    id: "admin.classification_markings.error.no_levels",
                    defaultMessage:
            ...
            if (emptyName) {
                return formatMessage({
                    id: "admin.classification_markings.error.empty_name",
                    defaultMessage:
                        "All classification levels must have a name.",
                });
            }
            ...
            if (duplicateName) {
                return formatMessage(
                    {
                        id: "admin.classification_markings.error.duplicate_name",
                        defaultMessage:
            ...
            if (globalBanner.enabled) {
                ...
                if (
                    !levels.some(
                        (l) => l.name.trim() === globalBanner.level_name
                    )
                ) {
    ...
    const persistLevels = useCallback(async (): Promise<void> => {
        const effectiveBanner: GlobalBannerConfig = enabled
            ? globalBanner
            : { ...DEFAULT_GLOBAL_BANNER };
        ...
        if (enabled) {
            ...
            const disabledBanner: GlobalBannerConfig = {
                ...DEFAULT_GLOBAL_BANNER,
            };
            let savedLinked: PropertyField;
            if (linkedField) {
                savedLinked = await savePatchLinkedField(
                    linkedField.id,
                    disabledBanner
                );
            } else {
                savedLinked = await saveCreateLinkedField(
                    savedTemplate.id,
                    disabledBanner
                );
            }
            ...
            if (effectiveBanner.enabled && effectiveBanner.level_name) {
                const options =
                    (savedTemplate.attrs?.options as PropertyFieldOption[]) ??
                    [];
                const optionId = findOptionIdByName(
                    options,
                    effectiveBanner.level_name
                );
                if (optionId) {
                    const savedValues = await saveUpsertSystemValue(
                        savedLinked.id,
                        optionId
                    );
                    dispatch({
                        type: PropertyTypes.RECEIVED_PROPERTY_VALUES,
                        data: { values: savedValues },
                    });
                }
            }
            ...
            dispatch({
                ...
                data: { fields: [savedTemplate, savedLinked] },
            });
            ...
            if (linkedField) {
                ...
                dispatch({
                    ...
                    data: { fieldId: linkedField.id },
                });
            }
            await saveDeleteField(templateField.id);
            dispatch({
                type: PropertyTypes.PROPERTY_FIELD_DELETED,
                data: { fieldId: templateField.id },
            });
        ...
        <div className="wrapper--fixed">
            ...
            <SaveChangesPanel
                ...
                savingMessage={formatMessage({
                    id: "admin.classification_markings.saving",
            ...
            <ConfirmModal
                ...
                title={formatMessage({
                    id: "admin.classification_markings.preset_switch.title",
                    defaultMessage: "Change classification preset?",
                })}
                message={formatMessage({
                    id: "admin.classification_markings.preset_switch.message",
                    defaultMessage:
                ...
                confirmButtonText={formatMessage({
                    id: "admin.classification_markings.preset_switch.confirm",
                    defaultMessage: "Change preset",
                })}
