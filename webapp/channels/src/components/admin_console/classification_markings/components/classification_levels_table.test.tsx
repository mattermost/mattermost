...
describe("ClassificationLevelsTable", () => {
    ...
    test("hides delete buttons when disabled", () => {
        renderWithContext(
            <ClassificationLevelsTable {...makeProps({ disabled: true })} />
        );

        expect(
            screen.queryByRole("button", { name: "Delete level" })
    ...
    test("calls deleteLevel with the correct id when delete is clicked", () => {
        ...
        renderWithContext(
            <ClassificationLevelsTable {...makeProps({ deleteLevel })} />
        );
    ...
    test("displays rows sorted by rank regardless of input order", () => {
        ...
        renderWithContext(
            <ClassificationLevelsTable
                {...makeProps({ levels: unsortedLevels })}
            />
        );

        const inputs = screen.getAllByRole<HTMLInputElement>("textbox", {
            name: "Classification level name",
        });
    ...
    test("renders empty state with no rows when levels array is empty", () => {
        renderWithContext(
            <ClassificationLevelsTable {...makeProps({ levels: [] })} />
        );

        expect(
            screen.queryByRole("textbox", { name: "Classification level name" })
        ).not.toBeInTheDocument();
    });
});
