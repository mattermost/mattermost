...
const ENABLED_BANNER: GlobalBannerConfig = {
    enabled: true,
    placement: "top",
...
describe("GlobalClassificationIndicators", () => {
    test("renders section title and description", () => {
        ...
        expect(
            screen.getByText("Global Classification Indicators")
        ).toBeInTheDocument();
        expect(
            screen.getByText("Configure the global classification banner")
        ).toBeInTheDocument();
    });
    ...
    test("renders the enable toggle", () => {
        ...
        expect(
            screen.getByText("Global Classification Banner")
        ).toBeInTheDocument();
        expect(
    ...
    test("does not render placement and level controls when banner is disabled", () => {
        ...
        expect(
            screen.queryByText("Global classification level")
    ...
    test("renders placement and level controls when banner is enabled", () => {
        renderWithContext(
            <GlobalClassificationIndicators
                {...makeProps({ globalBanner: ENABLED_BANNER })}
            />
        );
        ...
        expect(
            screen.getByText("Global classification level")
        ).toBeInTheDocument();
    });
    ...
    test("calls onChange with enabled: true when True radio is clicked", () => {
        ...
        renderWithContext(
            <GlobalClassificationIndicators {...makeProps({ onChange })} />
        );
    ...
    test("calls onChange with enabled: false when False radio is clicked", () => {
        ...
        renderWithContext(
            <GlobalClassificationIndicators
                {...makeProps({ globalBanner: ENABLED_BANNER, onChange })}
            />
        );
    ...
    test("calls onChange with placement top when Top only is clicked", () => {
        const onChange = jest.fn();
        const banner: GlobalBannerConfig = {
            ...ENABLED_BANNER,
            placement: "top_and_bottom",
        };
        renderWithContext(
            <GlobalClassificationIndicators
                {...makeProps({ globalBanner: banner, onChange })}
            />
        );
    ...
    test("all controls are disabled when disabled prop is true", () => {
        renderWithContext(
            <GlobalClassificationIndicators
                {...makeProps({ globalBanner: ENABLED_BANNER, disabled: true })}
            />
        );
    ...
    test("renders empty state gracefully with no levels", () => {
        renderWithContext(
            <GlobalClassificationIndicators
                {...makeProps({ levels: [], globalBanner: ENABLED_BANNER })}
            />
        );
    ...
    test("shows inline error when banner is enabled and the selected level no longer exists", () => {
        const banner: GlobalBannerConfig = {
            enabled: true,
            placement: "top",
            level_name: "RENAMED OR DELETED",
        };
        renderWithContext(
            <GlobalClassificationIndicators
                {...makeProps({ globalBanner: banner })}
            />
        );

        expect(
            screen.getByText(
                /The previously selected level no longer exists\. Select a level from the current classification levels\./
            )
        ).toBeInTheDocument();
    });
