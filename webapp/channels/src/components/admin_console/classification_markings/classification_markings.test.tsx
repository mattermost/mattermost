...
describe("detectPreset", () => {
    ...
    test("should match colors case-insensitively", () => {
        const usPreset = presets.find((p) => p.id === "us")!;
        const lowered = usPreset.levels.map((l) => ({
            ...l,
...
describe("optionsToLevels", () => {
    ...
    test("should default color to #000000 when missing", () => {
        const options: PropertyFieldOption[] = [{ id: "a", name: "NoColor" }];
...
describe("fetchClassificationField", () => {
    ...
    test("should paginate when field not found on first page", async () => {
        ...
        const secondCallArgs = (Client4.getPropertyFields as jest.Mock).mock
            .calls[1];
        expect(secondCallArgs[4]).toEqual({
            cursorId: "p2",
            cursorCreateAt: 200,
        });
    });
...
describe("ClassificationMarkings component", () => {
    ...
    test("should show loading screen initially", () => {
        jest.spyOn(Client4, "getPropertyFields").mockReturnValue(
            new Promise(() => {})
        );
    ...
    test("should show informational notice when loaded", async () => {
        ...
        expect(
            screen.getByRole("heading", {
                name: "Classification markings are informational only",
            })
        ).toBeInTheDocument();
        expect(
            screen.getByText(
    ...
    test("should render disabled state when no existing field", async () => {
        ...
        expect(
            screen.queryByText("Classification preset")
    ...
    test("should render enabled state with levels when field exists", async () => {
        ...
        const classificationTrueRadio = screen.getByTestId(
    ...
    test("should validate empty levels when saving while enabled", async () => {
        ...
        await screen.findByText(
            /At least one classification level is required/
        );
    });
    ...
    test("should pass disabled prop to disable controls", async () => {
        ...
        renderWithContext(
            <ClassificationMarkings disabled={true} />,
            BASE_STATE
        );
...
describe("GlobalClassificationIndicators section", () => {
    ...
    test("should not show Global Classification Indicators when classification is disabled", async () => {
        ...
        expect(
            screen.queryByText("Global Classification Indicators")
        ).not.toBeInTheDocument();
    });
    ...
    test("should read initial banner state from linked field actions and system property value", async () => {
        ...
        jest.spyOn(Client4, "getSystemPropertyValues").mockResolvedValueOnce([
            sysValue,
        ]); // system value
    ...
    test("should validate that a level is selected when global banner is enabled", async () => {
        ...
        await screen.findByText(
            /A global classification level must be selected/
        );
    });
    ...
    test("should validate that the referenced level still exists when it was renamed", async () => {
        ...
        const nameInput = screen.getByRole("textbox", {
            name: /Classification level name/i,
        });
        ...
        expect(
            await screen.findByText(
                /The previously selected level no longer exists/
            )
        ).toBeInTheDocument();
        ...
        await screen.findByText(
            /The global classification banner is configured with a level that no longer exists/
        );
    });
    ...
    test("should patch linked field with actions and upsert system value when banner changes", async () => {
        ...
        const saveUpsertSpy = jest
            .spyOn(Utils, "saveUpsertSystemValue")
            .mockResolvedValue([]);
    ...
    test("should patch template and linked field when only levels change", async () => {
        ...
        const patchedTemplate = makePropertyField({
            attrs: {
                options: [
                    { id: "lvl1", name: "MODIFIED", color: "#007A33", rank: 1 },
                ],
            },
        });
    ...
    test("should delete linked field before template when classification is disabled", async () => {
        ...
        try {
            ...
            await act(async () => {
                await user.click(
                    screen.getByTestId("classificationEnabledfalse")
                );
            });
