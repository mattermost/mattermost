 ...
 */
export function actionsToGlobalBanner(
    actions: string[],
    levelName: string
): GlobalBannerConfig {
...
export function findOptionIdByName(
...
export function detectPreset(levels: ClassificationLevel[]): string {
    for (const preset of presets) {
        ...
        const matches = preset.levels.every((presetLevel, i) => {
            ...
            return (
                presetLevel.name === level.name &&
                presetLevel.color.toUpperCase() === level.color.toUpperCase() &&
                presetLevel.rank === level.rank
            );
        });
        if (matches) {
            return preset.id;
...
export function optionsToLevels(
    options: PropertyFieldOption[]
): ClassificationLevel[] {
    return options
        .map((opt, i) => ({
...
export function levelsToOptions(
    levels: ClassificationLevel[]
): Array<{ id: string; name: string; color: string; rank: number }> {
...
export function processClassificationField(field: PropertyField): {
    levels: ClassificationLevel[];
    presetId: string;
} {
...
export async function fetchClassificationField(): Promise<
...
> {
    ...
    while (fetched < maxItems) {
        const fields = await Client4.getPropertyFields(
            GROUP_NAME,
            OBJECT_TYPE,
            TARGET_TYPE,
            TARGET_ID,
        ...
        const found = fields.find(
            (f: PropertyField) => f.name === FIELD_NAME && f.delete_at === 0
        );
...
export async function saveCreateField(
    levels: ClassificationLevel[]
...
export async function savePatchField(
    fieldId: string,
    levels: ClassificationLevel[]
): Promise<PropertyField> {
    const options = levelsToOptions(levels);
...
export async function fetchLinkedClassificationField(): Promise<
    PropertyField | undefined
> {
    const maxItems = 500;
    let fetched = 0;
...
export async function saveCreateLinkedField(
...
export async function savePatchLinkedField(
    linkedFieldId: string,
    config: GlobalBannerConfig
): Promise<PropertyField> {
    return Client4.patchPropertyField(
        GROUP_NAME,
        LINKED_OBJECT_TYPE,
        linkedFieldId,
        {
 ...
 */
export async function fetchSystemClassificationValue(
    linkedFieldId: string
): Promise<string | undefined> {
    const values = await Client4.getSystemPropertyValues<string>(GROUP_NAME);
    const match = ((values as Array<PropertyValue<string>>) ?? []).find(
        (v) => v.field_id === linkedFieldId
    );
    return match?.value;
}
 ...
 */
export async function saveUpsertSystemValue(
    linkedFieldId: string,
    optionId: string
