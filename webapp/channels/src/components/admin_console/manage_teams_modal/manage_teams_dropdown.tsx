...
const ManageTeamsDropdown = (props: Props) => {
    const makeTeamAdmin = async () => {
        const { error } = await props.updateTeamMemberSchemeRoles(
            props.teamMember.team_id,
            props.user.id,
            true,
            true,
        );
    ...
    const removeFromTeam = () =>
        props.handleRemoveUserFromTeam(props.teamMember.team_id);

    const isTeamAdmin =
        isAdmin(props.teamMember.roles) || props.teamMember.scheme_admin;
