...
describe("ManageTeamsModal", () => {
    ...
    test("should save data in state from api calls", async () => {
        ...
        const getTeamMembersForUser = jest
            .fn()
            .mockReturnValue(
                Promise.resolve({ data: [{ team_id: "123test" }] }),
            );
        ...
        expect(wrapper!.find(".manage-teams__team-name").text()).toEqual(
            mockTeamData.display_name,
        );
        expect(wrapper!.find(ManageTeamsDropdown).props().teamMember).toEqual({
            team_id: "123test",
        });
    });
});
