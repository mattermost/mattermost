...
type Props = {
    ...
    actions: {
        ...
        ) => Promise<{ data?: AdminConfig; error?: ServerError }>;
...
export default class GlobalPolicyForm extends React.PureComponent<
    Props,
    State
> {
    constructor(props: Props) {
        ...
        this.state = {
            ...
            serverError: null,
            formErrorText: "",
            messageRetentionDropdownValue: this.getDefaultDropdownValue(
                DataRetentionSettings?.EnableMessageDeletion,
                DataRetentionSettings?.MessageRetentionDays,
            ),
            messageRetentionInputValue: this.getDefaultInputValue(
                DataRetentionSettings?.EnableMessageDeletion,
                DataRetentionSettings?.MessageRetentionDays,
            ),
            fileRetentionDropdownValue: this.getDefaultDropdownValue(
                DataRetentionSettings?.EnableFileDeletion,
                DataRetentionSettings?.FileRetentionDays,
            ),
            fileRetentionInputValue: this.getDefaultInputValue(
                DataRetentionSettings?.EnableFileDeletion,
                DataRetentionSettings?.FileRetentionDays,
            ),
        };
    ...
    handleSubmit = async () => {
        const {
            ...
            fileRetentionInputValue,
        } = this.state;
        const newConfig: AdminConfig = JSON.parse(
        ...
        if (
            (messageRetentionDropdownValue.value !== FOREVER &&
                parseInt(messageRetentionInputValue, 10) < 1) ||
            (fileRetentionDropdownValue.value !== FOREVER &&
                parseInt(fileRetentionInputValue, 10) < 1)
        ) {
            this.setState({
                formErrorText: Utils.localizeMessage(
                    "admin.data_retention.global_policy.form.numberError",
                    "You must add a number greater than or equal to 1.",
                ),
                saving: false,
            });
            return;
        ...
        newConfig.DataRetentionSettings.EnableMessageDeletion =
            this.setDeletionEnabled(messageRetentionDropdownValue.value);

        const messageDays = this.setRetentionDays(
            messageRetentionDropdownValue.value,
            messageRetentionInputValue,
        );
        if (messageDays >= 1) {
        ...
        newConfig.DataRetentionSettings.EnableFileDeletion =
            this.setDeletionEnabled(fileRetentionDropdownValue.value);

        const fileDays = this.setRetentionDays(
            fileRetentionDropdownValue.value,
            fileRetentionInputValue,
        );
        if (fileDays >= 1) {
                                ...
                                <div className="global_policy">
                                    ...
                                    <div id="global_direct_message_dropdown">
                                        <DropdownInputHybrid
                                            ...
                                            dropdownClassNamePrefix={
                                                "channel_message_retention_dropdown"
                                            }
                                            inputId={
                                                "channel_message_retention_input"
                                            }
                                        />
                                    </div>
                                    <div id="global_file_dropdown">
                                        <DropdownInputHybrid
                                            ...
                                            legend={Utils.localizeMessage(
                                                "admin.data_retention.form.fileRetention",
                                                "File retention",
                                            )}
                                            placeholder={Utils.localizeMessage(
                                                "admin.data_retention.form.fileRetention",
