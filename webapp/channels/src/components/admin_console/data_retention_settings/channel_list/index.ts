...
import {
    ...
    searchDataRetentionCustomPolicyChannels as searchChannels,
...
function searchChannelsToAdd(
    channels: Record<string, Channel>,
    term: string,
    filters: ChannelSearchOpts,
): Record<string, Channel> {
    let filteredTeams = filterChannelsMatchingTerm(
    ...
    return (state: GlobalState, ownProps: OwnProps) => {
        ...
        if (searchTerm || (filters && Object.keys(filters).length !== 0)) {
            channels = policyId
                ...
                : [];
            channelsToAdd = searchChannelsToAdd(
                channelsToAdd,
                searchTerm,
                filters,
            ) as Record<string, ChannelWithTeamData>;
            totalCount = channels.length;
        } else {
            channels = policyId
                ? (getPolicyChannels(state, {
                      policyId,
                  }) as ChannelWithTeamData[])
                : [];
