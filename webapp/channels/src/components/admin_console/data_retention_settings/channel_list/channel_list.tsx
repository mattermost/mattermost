...
export default class ChannelList extends React.PureComponent<Props, State> {
    ...
    private loadPage = async (page: number, pageSize = PAGE_SIZE) => {
        if (this.props.policyId) {
            ...
            await this.props.actions.getDataRetentionCustomPolicyChannels(
                this.props.policyId,
                page,
                pageSize,
            );
    ...
    getRows = () => {
        const { page } = this.state;
        const { channels, channelsToRemove, channelsToAdd, totalCount } =
            this.props; // term was here
        ...
        if (
            channelsToDisplay.length < PAGE_SIZE &&
        ...
        if (searchTermModified || filtersModified) {
            ...
            if (searchTerm === "") {
                if (filtersModified && policyId) {
                    await prevProps.actions.searchChannels(
                        policyId,
                        searchTerm,
                        filters,
                    );
