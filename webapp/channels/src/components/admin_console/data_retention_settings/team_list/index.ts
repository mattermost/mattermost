...
import {
    ...
    searchDataRetentionCustomPolicyTeams as searchTeams,
...
function searchTeamsToAdd(
    teams: Record<string, Team>,
    term: string,
): Record<string, Team> {
    const filteredTeams = filterTeamsStartingWithTerm(
        Object.keys(teams).map((key) => teams[key]),
        term,
    );
...
function mapStateToProps() {
    ...
    return (state: GlobalState, ownProps: OwnProps) => {
        ...
        const policy = policyId
            ? getDataRetentionCustomPolicy(state, policyId) ||
              ({} as DataRetentionCustomPolicy)
            : ({} as DataRetentionCustomPolicy);
        let totalCount = 0;
