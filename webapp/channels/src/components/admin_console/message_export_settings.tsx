...
> {
    getConfigFromState = (config: AdminConfig) => {
        config.MessageExportSettings.EnableExport =
            this.state.enableComplianceExport;
        config.MessageExportSettings.ExportFormat = this.state.exportFormat;
        config.MessageExportSettings.DailyRunTime =
    ...
    getJobDetails = (job: Job) => {
        if (job.data) {
            ...
            if (job.data.warning_count > 0) {
                if (
                    job.data.export_type ===
                    exportFormats.EXPORT_FORMAT_GLOBALRELAY
                ) {
                    message.push(
                        <div>
                            <FormattedMessage
            ...
            const globalRelayEmail = (
                <TextSetting
                    ...
                    placeholder={Utils.localizeMessage(
                        ...
                        'E.g.: "globalrelay@mattermost.com"',
                    )}
                    helpText={
                        <FormattedMessage
