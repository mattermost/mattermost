...
const ChannelMoveToSubMenu = (props: Props) => {
    ...
    function createSubmenuItemsForCategoryArray(
        ...
        const allCategories = categories.map((category: ChannelCategory) => {
            ...
            if (
                currentCategory &&
                currentCategory.display_name === category.display_name
            ) {
                selectedCategory = (
                    <CheckIcon color="var(--button-bg)" size={18} />
                );
            }
            ...
            return (
                <Menu.Item
                    ...
                    onClick={(event) =>
                        handleMoveToCategory(event, category.id)
                    }
                />
            );
        });
    ...
    function getMoveToCategorySubmenuItems(
        ...
        if (props.inHeaderDropdown || !isSubmenuOneOfSelectedChannels) {
            ...
            return createSubmenuItemsForCategoryArray(
                filteredCategories,
                currentCategory,
            );
        }
