...
describe("component/ConvertGmToChannelModal", () => {
    ...
    test("members part of multiple common teams", async () => {
        ...
        expect(
            screen.queryByText("URL: http://localhost:8065/team_1/channels/"),
        ).toBeInTheDocument();
    });
    ...
    test("multiple common teams - trying conversion", async () => {
        ...
        fireEvent.change(channelNameInput!, {
            target: { value: "Channel name set by me" },
        });
    ...
    test("duplicate channel names should npt be allowed", async () => {
        ...
        expect(
            screen.queryByText("A channel with that URL already exists"),
        ).toBeInTheDocument();
    });
});
