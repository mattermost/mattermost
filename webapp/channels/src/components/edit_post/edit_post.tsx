...
const EditPost = ({
    ...
    const [editText, setEditText] = useState<string>(
        draft.message ||
            editingPost?.post?.message_source ||
            editingPost?.post?.message ||
            "",
    );
    const [selectionRange, setSelectionRange] = useState<
        State["selectionRange"]
    >({ start: editText.length, end: editText.length });
        ...
        (e: ClipboardEvent) => {
            ...
            if (table && isGitHubCodeBlock(table.className)) {
                const { formattedMessage, formattedCodeBlock } =
                    formatGithubCodePaste({
                        selectionStart: (target as any).selectionStart,
                        selectionEnd: (target as any).selectionEnd,
                        message,
                        clipboardData,
                    });
                message = formattedMessage;
                newCaretPosition =
                    selectionRange.start + formattedCodeBlock.length;
            } else {
                message = formatMarkdownMessage(
                    clipboardData,
                    editText.trim(),
                    newCaretPosition,
                ).formattedMessage;
                newCaretPosition =
                    message.length - (editText.length - newCaretPosition);
            }
    ...
    const handleEdit = async () => {
        ...
        const hookResult = await actions.runMessageWillBeUpdatedHooks(
            updatedPost,
            editingPost.post,
        );
    ...
    const handleKeyDown = (e: React.KeyboardEvent<TextboxElement>) => {
        ...
        if (Utils.isUnhandledLineBreakKeyCombo(e)) {
            ...
            Keyboard.isKeyPressed(e, KeyCodes.ESCAPE) &&
            !showEmojiPicker
        ) {
            handleAutomatedRefocusAndExit();
    ...
    const handleEmojiClick = (emoji?: Emoji) => {
        const emojiAlias =
            emoji &&
            (((emoji as SystemEmoji).short_names &&
    ...
    const getEmojiContainerRef = useCallback(
        () => textboxRef.current,
        [textboxRef],
    );
    const getEmojiTargetRef = useCallback(
    ...
    if (config.EnableEmojiPicker === "true") {
        emojiPicker = (
            <>
                ...
                >
                    <EmoticonPlusOutlineIcon size={18} color="currentColor" />
                </button>
            </>
        );
    }
    ...
    return (
        ...
        >
            ...
            {postError && (
                <div
                    className={classNames("edit-post-footer", {
                        "has-error": postError,
                    })}
                >
