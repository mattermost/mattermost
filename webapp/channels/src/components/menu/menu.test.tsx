...
describe("menu click handlers", () => {
    test("should be able to open a React Bootstrap modal with the mouse", async () => {
        renderWithFullContext(<MenuWithModal />);
    ...
    for (const enterOrSpace of ["enter", "space"]) {
        test(`should be able to open a React Bootstrap modal with the keyboard using the ${enterOrSpace} key`, async () => {
            ...
            expect(
                screen.getByLabelText("menu with modal button"),
            ).toHaveFocus();
    ...
    test("should be able to open a React Bootstrap modal from a submenu with the mouse", async () => {
        renderWithFullContext(<MenuWithSubMenuModal />);
        ...
        expect(
            screen.queryByText("Open model from submenu"),
        ...
        await waitForElementToBeRemoved(() =>
            screen.queryByText("Open modal from submenu"),
        );
        await waitForElementToBeRemoved(() =>
            screen.queryByText("Open submenu"),
        );
    ...
    for (const enterOrSpace of ["enter", "space"]) {
        test(`should be able to open a React Bootstrap modal with the keyboard using the ${enterOrSpace} key`, async () => {
            ...
            expect(
                screen.getByText("Open submenu").closest("li"),
            ).toHaveFocus();
            ...
            expect(
                screen.getByText("Open modal from submenu"),
            ).toBeInTheDocument();
            ...
            expect(
                screen.getByText("Open modal from submenu").closest("li"),
            ).toHaveFocus();
