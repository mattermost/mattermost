...
export function MenuItem(props: Props) {
    ...
    const hasSecondaryLabel =
        ...
        Children.count(labels.props.children) === 2;
    ...
    return (
        ...
        >
            {leadingElement && (
                <div className="leading-element">{leadingElement}</div>
            )}
            <div className="label-elements">{labels}</div>
            {trailingElements && (
                <div className="trailing-elements">{trailingElements}</div>
            )}
            {children}
        </MenuItemStyled>
    );
...
const MenuItemStyled = styled(MuiMenuItem, {
    shouldForwardProp: (prop) =>
        prop !== "isDestructive" &&
        prop !== "hasSecondaryLabel" &&
        prop !== "isLabelsRowLayout",
})<MenuItemStyledProps>(({
    isDestructive = false,
    hasSecondaryLabel = false,
    isLabelsRowLayout = false,
}) => {
    ...
    return {
        "&.MuiMenuItem-root": {
            ...
            alignItems:
                hasOnlyPrimaryLabel || isLabelsRowLayout
                    ? "center"
                    : "flex-start",
            minHeight: "36px",
            maxHeight: "56px",
            ...
            "&.Mui-focusVisible": {
                boxShadow: isRegular
                    ? "0 0 0 2px var(--sidebar-text-active-border) inset"
                    : "0 0 0 2px rgba(var(--button-color-rgb), 0.16) inset",
                backgroundColor: isRegular
 ...
 **/
function isCorrectKeyPressedOnMenuItem(
    event: MouseEvent<HTMLLIElement> | KeyboardEvent<HTMLLIElement>,
) {
    if (event.type === EventTypes.KEY_DOWN) {
        const keyboardEvent = event as KeyboardEvent<HTMLLIElement>;
        if (
            isKeyPressed(keyboardEvent, Constants.KeyCodes.ENTER) ||
            isKeyPressed(keyboardEvent, Constants.KeyCodes.SPACE)
        ) {
            return true;
        }
