...
import Menu, {
    ...
    MenuItem,
    ToggleItem,
} from "./post_priority_picker_item";
...
function PostPriorityPicker({ onApply, onClose, settings }: Props) {
    const { formatMessage } = useIntl();
    const [priority, setPriority] = useState<PostPriority | "">(
        settings?.priority || "",
    );
    const [requestedAck, setRequestedAck] = useState<boolean>(
        settings?.requested_ack || false,
    );
    const [persistentNotifications, setPersistentNotifications] =
        useState<boolean>(settings?.persistent_notifications || false);

    const postAcknowledgementsEnabled = useSelector(
        isPostAcknowledgementsEnabled,
    );
    const persistentNotificationsEnabled =
        useSelector(isPersistentNotificationsEnabled) &&
        postAcknowledgementsEnabled;
    const interval = useSelector(getPersistentNotificationIntervalMinutes);

    const makeOnSelectPriority = useCallback(
        (type?: PostPriority) => (e: React.MouseEvent<HTMLButtonElement>) => {
            e.stopPropagation();
            e.preventDefault();
    ...
    const feedbackLink = postAcknowledgementsEnabled
        ? "https://forms.gle/noA8Azg7RdaBZtMB6"
        : "https://forms.gle/mMcRFQzyKAo9Sv49A";
                            ...
                            {postAcknowledgementsEnabled && (
                                <ToggleItem
                                    ...
                                    description={formatMessage({
                                        ...
                                        defaultMessage:
                                            "An acknowledgement button will appear with your message",
                                    })}
                                />
                            )}
