...
describe("components/new_channel_modal", () => {
        ...
        mockState = {
            entities: {
                ...
                channels: {
                    ...
                    roles: {
                        current_channel_id: ["channel_user", "channel_admin"],
                    },
                },
                teams: {
    ...
    test("should match component state with given props", () => {
        ...
        expect(channelNameHeading).toHaveClass(
        ...
        expect(channelNameInput).toHaveAttribute(
            "name",
            "new-channel-modal-name",
        );
        expect(channelNameInput).toHaveAttribute(
            "id",
            "input_new-channel-modal-name",
        );
        expect(channelNameInput).toHaveClass(
            "Input form-control medium new-channel-modal-name-input Input__focus",
        );
        ...
        const publicChannelSvg = screen.getByLabelText(
            "Globe Circle Solid Icon",
        );
        ...
        expect(publicChannelHeading.nextSibling).toHaveTextContent(
            "Anyone can join",
        );

        const privateChannelSvg = screen.getByLabelText(
            "Lock Circle Solid Icon",
        );
        ...
        expect(privateChannelHeading.nextSibling).toHaveTextContent(
            "Only invited members",
        );

        const purposeTextArea = screen.getByPlaceholderText(
            "Enter a purpose for this channel (optional)",
        );
        expect(purposeTextArea).toBeInTheDocument();
        expect(purposeTextArea).toHaveClass(
            "new-channel-modal-purpose-textarea",
        );

        const purposeDesc = screen.getByText(
    ...
    test("should handle type changes", () => {
        ...
        expect(
            privateChannel.parentElement?.nextSibling?.firstChild,
        ).toHaveAttribute("aria-label", "Check Circle Icon");
        ...
        expect(
            publicChannel.parentElement?.nextSibling?.firstChild,
        ).toHaveAttribute("aria-label", "Check Circle Icon");
    });
    ...
    test("should handle purpose changes", () => {
        ...
        // Change purpose
        const ChannelPurposeTextArea = screen.getByPlaceholderText(
            "Enter a purpose for this channel (optional)",
        );
    ...
    test("should disable confirm button when server error", async () => {
        ...
        const serverError = screen.getByText(
    ...
    test("should request team creation on submit", async () => {
        ...
        renderWithIntl(<NewChannelModal />);
