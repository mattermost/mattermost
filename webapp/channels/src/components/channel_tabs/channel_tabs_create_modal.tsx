...
function ChannelTabCreateModal({
    ...
    const [linkError, {loading: checkingLink, suppressed: linkErrorBypass}] = useTabLinkValidation(link === prevLink.current ? '' : link, (validatedLink, forced) => {
        if (!forced) {
    ...
    const handleEditFileClick = (e: MouseEvent<HTMLDivElement>) => {
        const innerClick = document.querySelector(`
            .channel-tabs-create-modal .post-image__download a,
            .channel-tabs-create-modal a.file-preview__remove
        `);
        if (
            innerClick === e.target ||
    ...
    const fileInput = (
        <input
            type='file'
            id='tab-create-file-input-in-modal'
            className='tab-create-file-input'
            ref={fileInputRef}
...
export default ChannelTabCreateModal;
...
const msg = defineMessages({
    heading: {id: 'channel_bookmarks.create.title', defaultMessage: 'Add a tab'},
    editHeading: {id: 'channel_bookmarks.create.edit.title', defaultMessage: 'Edit tab'},
    ...
    addTabText: {id: 'channel_bookmarks.create.confirm_add.button', defaultMessage: 'Add tab'},
    saveText: {id: 'channel_bookmarks.create.confirm_save.button', defaultMessage: 'Save tab'},
    ...
    saveError: {id: 'channel_bookmarks.create.error.generic_save', defaultMessage: 'There was an error trying to save the tab.'},
});

const TYPING_DELAY_MS = 250;
const REQUEST_TIMEOUT = 10000;
export const useTabLinkValidation = (link: string, onValidated: (validatedLink: string, forced?: boolean) => void) => {
    const {formatMessage} = useIntl();
