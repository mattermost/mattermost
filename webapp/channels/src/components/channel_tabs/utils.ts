...
import {fetchChannelTabs, reorderTab} from 'actions/channel_tabs';
...
export const MAX_TABS_PER_CHANNEL = 50;
...
export const useChannelTabPermission = (channelId: string, action: TAction) => {
    return useSelector((state: GlobalState) => getHaveIChannelTabPermission(state, channelId, action));
};

export const getHaveIChannelTabPermission = (state: GlobalState, channelId: string, action: TAction) => {
    ...
    const permission = TAB_PERMISSION[key(action, type)];
...
export const getIsChannelTabsEnabled = (state: GlobalState) => {
    const isEnabled = getFeatureFlagValue(state, 'ChannelTabs') === 'true' || getFeatureFlagValue(state, 'ChannelBookmarks') === 'true';
...
export const useChannelTabs = (channelId: string) => {
    ...
    useEffect(() => {
        if (channelId) {
            dispatch(fetchChannelTabs(channelId));
        }
    }, [channelId]);
    ...
    const reorder = async (id: string, prevOrder: number, nextOrder: number) => {
        ...
        const {error} = await dispatch(reorderTab(channelId, id, nextOrder));
