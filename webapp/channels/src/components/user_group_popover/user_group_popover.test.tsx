...
describe("component/user_group_popover", () => {
    ...
    test("should open modal", async () => {
        ...
        expect(
            wrapper.find("button.user-group-popover_header-button").exists(),
        ).toBe(true);
        wrapper
            .find("button.user-group-popover_header-button")
            .simulate("click");
    ...
    test("should not show search bar", async () => {
        ...
        const wrapper = mountWithIntl(
            <Provider store={store}>
                <BrowserRouter>
                    <UserGroupPopover {...baseProps} group={group2} />
                </BrowserRouter>
            </Provider>,
        );
        await actImmediate(wrapper);

        expect(wrapper.find(".user-group-popover_search-bar").exists()).toBe(
            false,
        );
    });
    ...
    test("should show and set search term", async () => {
        ...
        expect(
            wrapper.find(".user-group-popover_search-bar input").exists(),
        ).toBe(true);
        wrapper
            .find(".user-group-popover_search-bar input")
            .simulate("change", { target: { value: "a" } });
        expect(baseProps.actions.setPopoverSearchTerm).toHaveBeenCalledWith(
            "a",
        );
    });
    ...
    test("should show users", async () => {
        ...
        await actImmediate(wrapper);
        expect(wrapper.find(".group-member-list_item").length).toBeGreaterThan(
            0,
        );
    });
});
