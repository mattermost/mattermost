...
const reportValidationError = debounce(
    (error: string) => {
        trackEvent(
            "first_admin_setup",
            "admin_onboarding_organization_submit_fail",
            { error },
        );
    },
    700,
...
const Organization = (props: Props) => {
    ...
    const updateTeamNameFromOrgName = async () => {
        ...
        if (currentTeam && name && name !== currentTeam.display_name) {
            const { error } = await props.updateTeam({
                ...currentTeam,
                display_name: name,
            });
            if (error !== null) {
    ...
    const onNext = (e?: React.KeyboardEvent | React.MouseEvent) => {
        if (e && (e as React.KeyboardEvent).key) {
            if (
                (e as React.KeyboardEvent).key !== Constants.KeyCodes.ENTER[0]
            ) {
                return;
            }
        }
        ...
        if (validation.error || teamApiError.current) {
            reportValidationError(
                validation.error
                    ? validation.error
                    : (teamApiError.current! as string),
            );
            return;
        }
        props.next?.();
                    ...
                    <div className="Organization-form-wrapper">
                        ...
                        <div className="Organization__content">
                            ...
                            <Description>
                                <FormattedMessage
                                    id={
                                        "onboarding_wizard.organization.description"
                                    }
