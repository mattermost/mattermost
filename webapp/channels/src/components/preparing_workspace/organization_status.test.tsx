...
describe("components/preparing-workspace/organization_status", () => {
    ...
    it("should render no error message when error prop is null", () => {
        ...
        expect(
            container.getElementsByClassName("Organization__status").length,
        ).toBe(1);
    ...
    it("should render an error message for an empty organization name", () => {
        const component = withIntl(
            <OrganizationStatus error={BadUrlReasons.Empty} />,
        );
        const { getByText } = render(component);
        expect(
            getByText(/You must enter an organization name/i),
        ).toBeInTheDocument();
    });
    ...
    it("should render an error message for a team API error", () => {
        ...
        const { getByText } = render(component);
        expect(
            getByText(/There was an error, please try again/i),
        ).toBeInTheDocument();
    });

    it("should render an error message for an organization name with invalid length", () => {
        const component = withIntl(
            <OrganizationStatus error={BadUrlReasons.Length} />,
        );
        const { getByText } = render(component);
        expect(
            getByText(/Organization name must be between 2 and 64 characters/i),
        ).toBeInTheDocument();
    });
});
