...
const GroupOption = (props: Props) => {
    ...
    const profiles = useSelector(
        (state: GlobalState) =>
            getProfilesByIdsAndUsernames(state, {
                allUserIds: group.member_ids || [],
                allUsernames: [],
            }) as UserProfileValue[],
    );
    const overflowNames = useSelector((state: GlobalState) => {
        if (group?.member_ids) {
            return group?.member_ids
                .map((userId) =>
                    displayNameGetter(state, true)(getUser(state, userId)),
                )
                .join(", ");
        }
        return "";
    ...
    return (
        ...
        >
            <span className="more-modal__group-image">
                <AccountMultipleOutlineIcon
                    size={16}
            ...
            <div className="more-modal__details">
                <div className="more-modal__name">
                    ...
                    <span className="ml-2 light group-name">
                        {"@"}
                        {group.name}
                    </span>
                    <SimpleTooltip
