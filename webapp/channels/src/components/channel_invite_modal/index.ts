...
function makeMapStateToProps(
    ...
    return (state: GlobalState, props: OwnProps) => {
        ...
        if (props.channelId && props.teamId) {
            profilesNotInCurrentChannel = doGetProfilesNotInChannel(
                state,
                props.channelId,
            );
            profilesInCurrentChannel = doGetProfilesInChannel(
                state,
                props.channelId,
            );
            profilesNotInCurrentTeam = getProfilesNotInTeam(
                state,
                props.teamId,
            );
        ...
        const currentTeam = props.teamId
            ? getTeam(state, props.teamId)
            : getCurrentTeam(state);

        const guestAccountsEnabled = config.EnableGuestAccounts === "true";
        const emailInvitationsEnabled =
            config.EnableEmailInvitations === "true";
