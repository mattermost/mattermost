...
const TeamWarningBanner = (props: Props) => {
    const { teamId, users, guests } = props;

    const { formatMessage } = useIntl();

    const team = useSelector((state: GlobalState) => getTeam(state, teamId));

    const getCommaSeparatedUsernames = useCallback(
        (users: Array<UserProfileValue | UserProfile>) => {
            return users
                .map((user) => {
    ...
    const getGuestMessage = useCallback(() => {
        ...
        if (guests.length > 10) {
            return formatMessage(
                ...
                {
                    firstUser: (
                        <AtMention key={firstName} mentionName={firstName} />
                    ),
                    others: (
                        <SimpleTooltip
                            id={"usernames-overflow"}
                            content={commaSeparatedUsernames.replace(
                                `@${firstName}, `,
                                "",
                            )}
                        >
                            <span className="add-others-link">
                                <FormattedMessage
        ...
        const guestsList = guests.map((user) => {
            return (
                <AtMention key={user.username} mentionName={user.username} />
            );
        });
        ...
        <>
            {(users.length > 0 || guests.length > 0) && (
                <AlertBanner
                    ...
                    message={users.length > 0 && getMessage()}
                    footerMessage={guests.length > 0 && getGuestMessage()}
                />
            )}
        </>
    );
