...
> {
    ...
    isUser = (
        option: UserProfileValue | GroupValue,
    ): option is UserProfileValue => {
    ...
    private addValue = (value: UserProfileValue | GroupValue): void => {
        if (this.isUser(value)) {
            ...
            if (
                ...
                !this.props.membersInTeam[profile.id]
            ) {
            ...
            if (this.state.selectedUsers.indexOf(profile) === -1) {
                this.setState((prevState) => {
                    return {
                        selectedUsers: [...prevState.selectedUsers, profile],
                    };
                });
            }
        }
    ...
    private removeUsersFromValuesNotInTeam = (
    ...
    public componentDidMount(): void {
        ...
        this.props.actions.loadStatusesForProfilesList(
            this.props.profilesInCurrentChannel,
        );
    }
    ...
    public async componentDidUpdate(prevProps: Props, prevState: State) {
        if (prevState.term !== this.state.term) {
            ...
            if (!isEqual(values, this.state.groupAndUserOptions)) {
                if (userIds.length > 0) {
                    this.props.actions.getTeamMembersByIds(
                        this.props.channel.team_id,
                        userIds,
                    );
                }
    ...
    getExcludedUsers = (): Set<string> => {
        if (this.props.excludeUsers) {
            return new Set(
                ...this.props.profilesNotInCurrentTeam.map((user) => user.id),
                Object.values(this.props.excludeUsers).map((user) => user.id),
            );
        }
        return new Set(
            this.props.profilesNotInCurrentTeam.map((user) => user.id),
    ...
    public getOptions = () => {
        ...
        const filteredDmUsers = filterProfilesStartingWithTerm(
            this.props.profilesFromRecentDMs,
            this.state.term,
        );
        ...
        const filteredUsers: UserProfile[] = filterProfilesStartingWithTerm(
            this.props.profilesNotInCurrentChannel.concat(
                this.props.profilesInCurrentChannel,
            ),
            this.state.term,
        );
        users = this.filterOutDeletedAndExcludedAndNotInTeamUsers(
            filteredUsers,
            excludedAndNotInTeamUserIds,
        );
        ...
        const optionValues = [...dmUsers, ...groupsAndUsers].slice(
            0,
            MAX_USERS,
        );
    ...
    private handleDelete = (
        values: Array<UserProfileValue | GroupValue>,
    ): void => {
    ...
    public search = (searchTerm: string): void => {
        ...
        this.searchTimeoutId = window.setTimeout(async () => {
            ...
            const promises = [this.props.actions.searchProfiles(term, options)];
    ...
    private filterOutDeletedAndExcludedAndNotInTeamUsers = (
        users: UserProfile[],
    ...
    renderOption = (
        option: UserProfileValue | GroupValue,
        isSelected: boolean,
        onAdd: (option: UserProfileValue | GroupValue) => void,
        onMouseMove: (option: UserProfileValue | GroupValue) => void,
    ) => {
        let rowSelected = "";
        if (isSelected) {
        ...
        if (this.isUser(option)) {
            const ProfilesInGroup = this.props.profilesInCurrentChannel.map(
                (user) => user.id,
            );
                        ...
                        <div className="more-modal__name">
                            <span>
                                ...
                                {displayName === option.username ? null : (
                                    <UsernameSpan className="ml-2 light">
                                        {"@"}
                                        {option.username}
                                    </UsernameSpan>
                                )}
                                <UserMappingSpan className="light">
    ...
    public render = (): JSX.Element => {
        ...
        const InviteModalLink = (props: {
            ...
            return (
                <ToggleModalButton
                    id="inviteGuest"
                    className={`${
                        props.inviteAsGuest ? "invite-as-guest" : ""
                    } btn btn-link`}
                ...
                <FormattedMessage
                    ...
                    values={{
                        InvitationModalLink: (chunks: string) => (
                            <InviteModalLink>{chunks}</InviteModalLink>
                        ),
                    }}
                />
            </div>
            ...
            <MultiSelect
                ...
                placeholderText={
                    this.props.isGroupsEnabled
                        ? localizeMessage(
                              "multiselect.placeholder.peopleOrGroups",
        ...
        return (
            ...
            >
                <Modal.Header id="channelInviteModalLabel" closeButton={true} />
                <Modal.Body role="application" className="overflow--visible">
                    <div className="channel-invite__header">{header}</div>
                    {inviteError}
