...
import type { GroupChannel, OptionValue } from "../types";
...
const ListItem = React.forwardRef(
    (props: Props, ref?: React.Ref<HTMLDivElement>) => {
        const { option, isMobileView, isSelected, add, select } = props;
            ...
            >
                ...
                {isMobileView && Boolean(lastPostAt) && (
                    <div className="more-modal__lastPostAt">
                        <Timestamp {...TIME_SPEC} value={lastPostAt} />
                    </div>
                )}
...
function GMDetails(props: { option: GroupChannel }) {
    ...
    return (
        <>
            <div className="more-modal__gm-icon">{option.profiles.length}</div>
