...
export function makeGetOptions(): (
    ...
    const getUsersWithDMs = createSelector(
        ...
        (currentUserId, allChannels, users) => {
            const directChannels = Object.values(allChannels).filter(
                (channel) => channel.type === Constants.DM_CHANNEL,
            );
        ...
        (channelsWithProfiles, searchTerm, values) => {
            return channelsWithProfiles.filter((channel) => {
                if (searchTerm) {
                    ...
                    const matches = filterProfilesStartingWithTerm(
                        channel.profiles,
                        searchTerm,
                    );
                    if (matches.length === 0) {
                ...
                if (values) {
                    ...
                    const valuesInProfiles = values.every((value) =>
                        channel.profiles.find((user) => user.id === value.id),
                    );
                    if (!valuesInProfiles) {
                        return false;
                    }
    ...
    return createSelector(
        "makeGetOptions",
        getUsersWithDMs,
        (state: GlobalState, users: UserProfile[], values: OptionValue[]) =>
            getFilteredGroupChannels(state, values),
        (state: GlobalState, users: UserProfile[]) => users,
        ...
        (usersWithDMs, filteredGroupChannels, users, isSearch) => {
            ...
            const recents = [...usersWithDMs, ...filteredGroupChannels].sort(
            ...
            const usersWithoutDMs = users
                .filter(
                    (user) =>
                        user.delete_at === 0 &&
                        !usersWithDMs.some((other) => other.id === user.id),
                )
            ...
            return [...recents, ...usersWithoutDMs];
        },
    );
}
