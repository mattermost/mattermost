...
function simpleSet(
    keys: Array<Extract<keyof State, string>>,
    state: State,
    action: Action,
): [State, boolean] {
...
function reducer(state: State, action: Action): State {
    ...
    switch (action.type) {
        case "update_progress_bar_fake": {
            ...
            if (
                state.progressBar >= convertProgressToBar(firstLongStep) &&
                state.progressBar <= maxFakeProgress - maxFakeProgressIncrement
            ) {
                return {
                    ...state,
                    progressBar: state.progressBar + maxFakeProgressIncrement,
                };
            }
            return state;
        }
        ...
        default:
            ...
            console.error(
                `Exhaustiveness failure for self hosted purchase modal. action: ${JSON.stringify(
                    action,
                )}`,
            );
            return state;
...
export function canSubmit(
    ...
    const validCard = Boolean(state.cardName && state.cardFilled);
    ...
    switch (progress) {
        ...
        case SelfHostedSignupProgress.CONFIRMED_INTENT: {
            return Boolean(validAddress && validSeats && state.agreedTerms);
        }
        ...
        case SelfHostedSignupProgress.CREATED_INTENT:
            return Boolean(
                validCard && validAddress && validSeats && state.agreedTerms,
            );
        default: {
            // eslint-disable-next-line no-console
...
export default function SelfHostedPurchaseModal(props: Props) {
    ...
    useNoEscape();
    const controlModal = useControlSelfHostedPurchaseModal({
    ...
    useEffect(() => {
        if (
            ...
            (currentUsers > parseInt(state.seats.quantity, 10) ||
                !parseInt(state.seats.quantity, 10))
        ) {
            dispatch({
                type: "set_seats",
                data: {
    ...
    async function submit() {
        ...
        try {
            ...
            signupCustomerResult = await Client4.createCustomerSelfHostedSignup(
                {
                    ...
                    shipping_address: state.shippingSame
                        ? billingAddress
                        : {
            ...
            dispatch({
                type: "set_error",
                data: "Failed to submit payment information",
            });
            return;
        }
        ...
        try {
            ...
            const finished = await reduxDispatch(
            ...
            if (finished.data) {
                trackEvent(
                    ...
                    "purchase_success",
                    {
                        seats: parseInt(finished.data?.Users, 10) || 0,
                        users: currentUsers,
                    },
                );
                    ...
                    <div className="SelfHostedPurchaseModal">
                        ...
                        {state.error && (
                            <ErrorPage
                                ...
                                canRetry={canRetry}
                                errorType={
                                    canRetry ? "generic" : "failed_export"
                                }
                            />
                        )}
