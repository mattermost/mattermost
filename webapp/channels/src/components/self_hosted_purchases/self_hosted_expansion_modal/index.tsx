...
export function canSubmit(
    ...
    const validCard = Boolean(formState.cardName && formState.cardFilled);
...
export default function SelfHostedExpansionModal() {
    ...
    const activeUsers =
        useSelector(getFilteredUsersStats)?.total_users_count || 0;
    const [minimumSeats] = useState(
        activeUsers <= licensedSeats ? 1 : activeUsers - licensedSeats,
    );
    ...
    const submit = async () => {
        ...
        let signupCustomerResult: SelfHostedSignupCustomerResponse | null =
            null;
        ...
        if (
            ...
            progress === SelfHostedSignupProgress.CREATED_CUSTOMER
        ) {
            dispatch({
    ...
    const resetToken = () => {
        try {
            Client4.bootstrapSelfHostedSignup(true).then((data) => {
                dispatch({
                    type: HostedCustomerTypes.RECEIVED_SELF_HOSTED_SIGNUP_PROGRESS,
                    data: data.progress,
                });
    ...
    return (
        <StripeProvider stripeRef={stripeRef}>
            <RootPortal>
                <FullScreenModal
                    show={show}
                ...
                >
                    <div className="SelfHostedExpansionModal">
                        <div
                            className={classNames("form-view", {
                                "form-view--hide": !showForm,
                            })}
                        >
                            <div className="lhs">
                                <h2 className="title">{title}</h2>
                                <UpgradeSvg width={267} height={227} />
                        ...
                        {(formState.succeeded ||
                            progress ===
                                SelfHostedSignupProgress.CREATED_LICENSE) &&
                            !formState.error &&
                            !formState.submitting && (
                                <SuccessPage
                                    onClose={() => {
                        ...
                        {formState.submitting && (
                            <Submitting currentPlan={currentPlan} />
                        )}
                        {formState.error && (
                            <ErrorPage
                                canRetry={canRetry}
