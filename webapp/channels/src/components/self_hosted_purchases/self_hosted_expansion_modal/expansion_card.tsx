...
export default function SelfHostedExpansionCard(props: Props) {
    ...
    const startsAt = moment(parseInt(license.StartsAt, 10)).format(
        "MMM. D, YYYY",
    );
    const endsAt = moment(parseInt(license.ExpiresAt, 10)).format(
        "MMM. D, YYYY",
    );
    ...
    const invalidAdditionalSeats =
        additionalSeats === 0 ||
        isNaN(additionalSeats) ||
        additionalSeats < props.minimumSeats;
    const [products] = useGetSelfHostedProducts();
    const currentProduct = findSelfHostedProductBySku(
        products,
    ...
    const getMonthsUntilExpiry = () => {
        const now = new Date();
        return (
            licenseExpiry.getMonth() -
            now.getMonth() +
            MONTHS_IN_YEAR * (licenseExpiry.getFullYear() - now.getFullYear())
        );
    };
    ...
    const getMaximumAdditionalSeats = () => {
        ...
        const remainingTransactionLimit =
            MAX_TRANSACTION_VALUE - currentPaymentPrice;
        const remainingSeats = Math.floor(
            remainingTransactionLimit / (costPerMonth * 12),
        );
        return Math.max(0, remainingSeats);
    };
    const maxAdditionalSeats = getMaximumAdditionalSeats();

    const handleNewSeatsInputChange = (
        ...
        const finalSeatCount = overMaxAdditionalSeats
            ? maxAdditionalSeats
            : requestedSeats;
                            ...
                            <FormattedMessage
                                ...
                                values={{
                                    warningIcon: (
                                        <WarningIcon
                                            additionalClassName={
                                                "SelfHostedExpansionRHSCard__warning"
                                            }
                                        />
                                    ),
                                }}
