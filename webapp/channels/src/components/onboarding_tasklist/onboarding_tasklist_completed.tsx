...
const Completed = (props: Props): JSX.Element => {
    ...
    const subscription = useSelector(
        (state: GlobalState) => state.entities.cloud.subscription,
    );
    const isCloud = license?.Cloud === "true";
    const isFreeTrial = subscription?.is_free_trial === "true";
    const hadPrevCloudTrial =
    ...
    const selfHostedTrialCondition =
        isCurrentLicensed === "false" &&
        isPrevLicensed === "false" &&
        (props.isCurrentUserSystemAdmin || props.isFirstAdmin);

    // if Cloud, show if not in trial and had never been on trial
    const cloudTrialCondition =
        isCloud && !isFreeTrial && !hadPrevCloudTrial && !isPaidSubscription;
    ...
    return (
        <>
            <CSSTransition in={true} timeout={150} classNames="fade">
                <CompletedWrapper>
                    <img src={completedImg} alt={"completed tasks image"} />
                    <h2>
                        <FormattedMessage
                        ...
                        <>
                            ...
                            {isCloud ? (
                                <CloudStartTrialButton
                                    ...
                                    telemetryId={
                                        "start_cloud_trial_after_completing_steps"
                                    }
                                ...
                                <StartTrialBtn
                                    message={formatMessage({
                                        id: "start_trial.modal_btn.start_free_trial",
                                        defaultMessage:
