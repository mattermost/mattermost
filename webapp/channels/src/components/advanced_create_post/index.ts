...
function makeMapStateToProps() {
    const getMessageInHistoryItem = makeGetMessageInHistoryItem(
        Posts.MESSAGE_TYPES.POST as any,
    );
    ...
    return (state: GlobalState) => {
        ...
        const currentChannelTeammateUsername = getUser(
            state,
        ...
        const isRemoteDraft =
            state.views.drafts.remotes[
                `${StoragePrefixes.DRAFT}${currentChannel.id}`
            ] || false;
        ...
        const canPost = haveICurrentChannelPermission(
            state,
            Permissions.CREATE_POST,
        );
        const useChannelMentions = haveICurrentChannelPermission(
            state,
        ...
        const isFormattingBarHidden = getBool(
            state,
            Preferences.ADVANCED_TEXT_EDITOR,
            AdvancedTextEditor.POST,
        );
