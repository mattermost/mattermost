...
import FileUpload, {
    type FileUpload as FileUploadClass,
...
describe("components/FileUpload", () => {
    ...
    test("should match snapshot", () => {
        const wrapper = shallowWithIntl(<FileUpload {...baseProps} />);
    ...
    test("should prevent event default and progogation on call of onTouchEnd on fileInput", () => {
        ...
        instance.fileInput = {
            current: {
                click: () =>
                    instance.handleLocalFileUploaded(
    ...
    test("should match state and call handleMaxUploadReached or props.onClick on handleLocalFileUploaded", () => {
        const wrapper = shallowWithIntl(
            <FileUpload {...baseProps} fileCount={9} />,
        );
    ...
    test("should props.onFileUpload when fileUploadSuccess is called", () => {
        ...
        expect(baseProps.onFileUpload).toHaveBeenCalledWith(
            data.file_infos,
            data.client_ids,
            "channel_id",
            "root_id",
    ...
    test("should upload file on paste", () => {
        ...
        const getAsFile = jest
            .fn()
            .mockReturnValue(new File(["test"], "test.png"));
        ...
        expect(spy).toHaveBeenCalledWith([
            expect.objectContaining({ name: expectedFileName }),
        ]);
    ...
    test("should not prevent paste event default if no file in clipboard", () => {
        ...
        (event as any).clipboardData = {
            items: [{ getAsString, kind: "string", type: "text/plain" }],
            types: ["text/plain"],
            getData: () => {
                return "";
            },
        ...
        const instance = wrapper.instance() as FileUploadClass;
        const spy = jest
            .spyOn(instance, "containsEventTarget")
    ...
    test("should error max upload files", () => {
        ...
        const wrapper = shallowWithIntl(<FileUpload {...props} />);
    ...
    test("should functions when handleDrop is called", () => {
        ...
        const e = {
            dataTransfer: { files: [{ name: "file1.pdf" }] },
        } as unknown as DragEvent<HTMLInputElement>;
        const instance = wrapper.instance() as FileUploadClass;
        instance.uploadFiles = jest.fn();
    ...
    test("FilesWillUploadHook - should reject all files", () => {
        ...
        const props = {
            ...baseProps,
            pluginFilesWillUploadHooks: [
                { hook: pluginHook } as unknown as FilesWillUploadHook,
            ],
        };
        const files = [
            { name: "file1.pdf" } as File,
    ...
    test("FilesWillUploadHook - should reject one file and allow one file", () => {
        ...
        expect(baseProps.onUploadStart).toHaveBeenCalledTimes(1);
        expect(baseProps.onUploadStart).toHaveBeenCalledWith(
            [expect.stringMatching(generatedIdRegex)],
            props.channelId,
        );
