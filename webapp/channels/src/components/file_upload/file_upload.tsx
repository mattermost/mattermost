...
export class FileUpload extends PureComponent<Props, State> {
    ...
    handleDrop = (e: DragEvent<HTMLInputElement>) => {
        if (!this.props.canUploadFiles) {
            this.props.onUploadError(intl.formatMessage({id: 'file_upload.disabled', defaultMessage: 'File attachments are disabled.'}));
            return;
        }
        ...
        if (files.length === 0) {
            this.props.onUploadError(intl.formatMessage({id: 'file_upload.drag_folder', defaultMessage: 'This attachment cannot be uploaded.'}));
            return;
        }
