...
export type Actions = {
    getGroupsNotAssociatedToTeam: (
        ...
        page?: number,
        perPage?: number,
    ) => Promise<{ data: Group[] } | { error: Error }>;
    ...
    ) => Promise<{ data: GroupsWithCount } | { error: Error }>;
};
...
> {
    ...
    public addValue = (value: GroupValue): void => {
        const values = Object.assign<GroupValue[], GroupValue[]>(
            [],
            this.state.values,
        );
    ...
    public renderOption = (
        option: GroupValue,
        isSelected: boolean,
        onAdd: (value: GroupValue) => void,
        onMouseMove: (value: GroupValue) => void,
    ): JSX.Element => {
            ...
            >
                ...
                <div className="more-modal__details">
                    <div className="more-modal__name">
                        ...
                        {"-"}
                        <Nbsp />
                        <span className="more-modal__name_sub">
                            <FormattedMessage
                                id="numMembers"
    ...
    public render(): JSX.Element {
        ...
        if (this.props.excludeGroups) {
            const hasGroup = (og: Group) =>
                !this.props.excludeGroups?.find((g) => g.id === og.id);
            groupsToShow = groupsToShow.filter(hasGroup);
        }
        if (this.props.includeGroups) {
            const hasGroup = (og: Group) =>
                this.props.includeGroups?.find((g) => g.id === og.id);
            groupsToShow = [
                ...groupsToShow,
                ...this.props.includeGroups.filter(hasGroup),
            ];
        }
