...
const ChannelMoveToSubMenuOld = (props: Props) => {
    ...
    function handleMoveToNewCategory() {
        dispatch(
            openModal({
                ...
                dialogProps: {
                    channelIdsToAdd:
                        multiSelectedChannelIds.indexOf(props.channel.id) === -1
                            ? [props.channel.id]
    ...
    function createSubmenuItemsForCategoryArray(
    ...
    function filterCategoriesBasedOnChannelType(
        categories: ChannelCategory[],
        isDmOrGm = false,
    ) {
        if (isDmOrGm) {
            return categories.filter(
                (category) => category.type !== CategoryTypes.CHANNELS,
            );
        }
    ...
    function getMoveToCategorySubmenuItems(categories: ChannelCategory[]) {
        const isSubmenuOneOfSelectedChannels = multiSelectedChannelIds.includes(
            props.channel.id,
        );
        ...
        if (props.inHeaderDropdown || !isSubmenuOneOfSelectedChannels) {
            ...
            const filteredCategories = filterCategoriesBasedOnChannelType(
                categories,
                isDmOrGm,
            );
        ...
        const areAllSelectedChannelsDMorGM = multiSelectedChannelIds.every(
            (channelId) =>
                allChannels[channelId].type === Constants.DM_CHANNEL ||
                allChannels[channelId].type === Constants.GM_CHANNEL,
        );
        ...
        const areAllSelectedChannelsAreNotDMorGM =
            multiSelectedChannelIds.every(
                (channelId) =>
                    allChannels[channelId].type !== Constants.DM_CHANNEL &&
                    allChannels[channelId].type !== Constants.GM_CHANNEL,
            );
    ...
    return (
        <Menu.Group>
            <Menu.ItemSubMenu
                ...
                direction={"right"}
                icon={
                    props.inHeaderDropdown ? null : (
                        <FolderMoveOutlineIcon size={16} />
                    )
                }
                openUp={props.openUp}
