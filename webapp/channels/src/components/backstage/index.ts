...
function mapStateToProps(state: GlobalState) {
    ...
    if (!canCreateOrDeleteCustomEmoji) {
        for (const t of getMyTeams(state)) {
            if (
                haveITeamPermission(state, t.id, Permissions.CREATE_EMOJIS) ||
                haveITeamPermission(state, t.id, Permissions.DELETE_EMOJIS)
            ) {
    ...
    const canManageTeamIntegrations =
        haveITeamPermission(state, "", Permissions.MANAGE_SLASH_COMMANDS) ||
        haveITeamPermission(state, "", Permissions.MANAGE_OAUTH) ||
        haveITeamPermission(state, "", Permissions.MANAGE_INCOMING_WEBHOOKS) ||
        haveITeamPermission(state, "", Permissions.MANAGE_OUTGOING_WEBHOOKS);
