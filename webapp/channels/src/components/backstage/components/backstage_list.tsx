...
const BackstageList = ({
    searchPlaceholder = localizeMessage("backstage_list.search", "Search"),
    ...remainingProps
}: Props) => {
        ...
        if (children.length === 0 || !hasChildren) {
            if (!filterLowered) {
                ...
                children = (
                    ...
                    >
                        {React.cloneElement(remainingProps.emptyTextSearch, {
                            values: { searchTerm: filterLowered },
                        })}
                    </div>
                );
            }
    ...
    if (remainingProps.addLink && remainingProps.addText) {
        addLink = (
            <Link className="add-link" to={remainingProps.addLink}>
                <button
                    type="button"
                    className="btn btn-primary"
                    id={remainingProps.addButtonId}
                >
                    <span>{remainingProps.addText}</span>
                </button>
            </Link>
        );
    }
