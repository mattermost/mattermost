...
describe("components/emoji/components/AddEmoji", () => {
    const baseProps: AddEmojiProps = {
        emojiMap: new EmojiMap(
            new Map([
                [
                    "mycustomemoji",
                    TestHelper.getCustomEmojiMock({ name: "mycustomemoji" }),
                ],
            ]),
        ),
        team: {
        ...
        actions: {
            createCustomEmoji: jest
                .fn()
                .mockImplementation((emoji: CustomEmoji) => ({
                    data: { name: emoji.name },
                })),
        },
    };

    const assertErrorIsElementWithMatchingId = (
    ...
    test("should submit the new added emoji", () => {
        const wrapper = shallow<AddEmoji>(<AddEmoji {...baseProps} />, {
            context,
        });
    ...
    test("should show error if emoji name unset", () => {
        ...
        assertErrorIsElementWithMatchingId(
            wrapper.state().error,
            "add_emoji.nameRequired",
        );
    });
    ...
    test.each([
        ...
        "emoji name '%s' should be corrected as '%s'",
        (emojiName, expectedName) => {
            const wrapper = shallow<AddEmoji>(<AddEmoji {...baseProps} />, {
                context,
            });
    ...
    test("should show error when emoji name is taken by an existing custom emoji", () => {
        ...
        nameInput.simulate("change", {
            target: { name: "name", value: "mycustomemoji" },
        });
        form.simulate("submit", { preventDefault: jest.fn() });
    ...
    test("should show error when image is too large", () => {
        ...
        nameInput.simulate("change", {
            target: { name: "name", value: "newcustomemoji" },
        });
        form.simulate("submit", { preventDefault: jest.fn() });
    ...
    test("should show generic error when action response cannot be parsed", async () => {
        ...
        nameInput.simulate("change", {
            target: { name: "name", value: "newemoji" },
        });
        form.simulate("submit", { preventDefault: jest.fn() });
