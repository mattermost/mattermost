...
function mapStateToProps(state: GlobalState) {
    ...
    const isInvitingPeople =
        isModalOpen(state, ModalIdentifiers.CHANNEL_INVITE) ||
        isModalOpen(state, ModalIdentifiers.CREATE_DM_CHANNEL);
    ...
    if (channel.type === Constants.DM_CHANNEL) {
        const user = getUser(
            state,
            getUserIdFromChannelId(channel.name, currentUser.id),
        );
        props.dmUser = {
            user,
