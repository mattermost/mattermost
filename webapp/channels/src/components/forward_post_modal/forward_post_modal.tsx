...
const ForwardPostModal = ({ onExited, post, actions }: Props) => {
    ...
    const channel = useSelector((state: GlobalState) =>
        getChannel(state, { id: post.channel_id }),
    );
    const currentTeam = useSelector(getCurrentTeam);

    const relativePermaLink = useSelector((state: GlobalState) =>
        getPermalinkURL(state, currentTeam.id, post.id),
    );
    ...
    const canPostInSelectedChannel = useSelector((state: GlobalState) => {
        const channelId = isPrivateConversation
            ? channel.id
            : selectedChannelId;
        const isDMChannel =
            selectedChannel?.details?.type === Constants.DM_CHANNEL;
        const teamId = isPrivateConversation
            ? currentTeam.id
    ...
    const canForwardPost =
        (isPrivateConversation || canPostInSelectedChannel) && !postError;
    ...
    const handleSubmit = () => {
        ...
        const channelToForward = isPrivateConversation
            ? makeSelectedChannelOption(channel)
            : selectedChannel;
    ...
    return (
        <GenericModal
            ...
            onExited={onHide}
        >
            <div className={"forward-post__body"} ref={measuredRef}>
