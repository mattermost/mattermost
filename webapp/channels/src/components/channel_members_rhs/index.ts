...
const searchProfiles = createSelector(
    "searchProfiles",
    (state: GlobalState, search: string) =>
        searchActiveProfilesInCurrentChannel(state, search, false),
    getUserStatuses,
    getTeammateNameDisplaySetting,
    getMembersInCurrentChannel,
    buildProfileList,
...
function mapStateToProps(state: GlobalState) {
    ...
    const { member_count: membersCount } = getCurrentChannelStats(state) || {
        member_count: 0,
    };
    ...
    const hasInfoPrevState =
        prevRhsState === RHSStates.CHANNEL_INFO ||
