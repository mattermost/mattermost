...
const CodeBlock: React.FC<Props> = ({
    ...
    const codeBlockActions = useSelector(
        (state: GlobalState) => state.plugins.components.CodeBlockAction,
    );
    const pluginItems = codeBlockActions?.map((item) => {
        if (!item.component) {
            return null;
        }

        const Component = item.component as any;
        return <Component key={item.id} code={code} />;
    });
