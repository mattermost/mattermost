...
describe("components/PostPreHeader", () => {
    ...
    test("should not render anything if the post is neither flagged nor pinned", () => {
        ...
        const wrapper = shallowWithIntl(<PostPreHeader {...props} />);
    ...
    test("should properly handle flagged posts (and not pinned)", () => {
        ...
        expect(wrapper.find(FormattedMessage).prop("defaultMessage")).toEqual(
            "Saved",
        );
        expect(wrapper).toMatchSnapshot();
    ...
    describe("should properly handle posts that are flagged and pinned", () => {
        test("both skipFlagged and skipPinned are not true", () => {
            ...
            expect(
                wrapper.find(FormattedMessage).first().prop("defaultMessage"),
            ).toEqual("Pinned");
            expect(
                wrapper.find(FormattedMessage).last().prop("defaultMessage"),
            ).toEqual("Saved");
            expect(wrapper).toMatchSnapshot();
        });
    ...
    test("should properly handle link clicks", () => {
        ...
        expect(baseProps.actions.showPinnedPosts).toHaveBeenNthCalledWith(
            1,
            baseProps.channelId,
        );
