...
class PostPreHeader extends React.PureComponent<Props> {
    ...
    getMessageInfo(
        postStatus: PostPinnedOrFlagged,
        skipFlagged?: boolean,
        skipPinned?: boolean,
    ): MessageInfoKey | false {
    ...
    render() {
        const { isFlagged, isPinned, skipPinned, skipFlagged, channelId } =
            this.props;

        const messageKey = this.getMessageInfo(
            this.getPostStatus(isFlagged, isPinned),
            skipFlagged,
            skipPinned,
        );
        ...
        return (
            <div className="post-pre-header">
                <div className="post-pre-header__icons-container">
                    {isPinned && !skipPinned && (
                        <span className="icon-pin icon icon--post-pre-header" />
                    )}
                    {isFlagged && !skipFlagged && (
                        <FlagIconFilled className="icon icon--post-pre-header" />
                    )}
                </div>
                    ...
                    {messageKey &&
                        messageKey === MessageInfoKey.PinnedAndFlagged && (
                            <span>
                                ...
                                <span className="post-pre-header__link-separator">
                                    {"\u2B24"}
                                </span>
                                <a
                                    onClick={() =>
