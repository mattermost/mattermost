...
describe("components/post_view/post_view", () => {
    ...
    test("should match snapshot for channel loading", () => {
        const wrapper = shallow(
            <PostView {...{ ...baseProps, channelLoading: true }} />,
        );
    ...
    test("unreadChunkTimeStamp should be set for first load of channel", () => {
        const wrapper = shallow(
            <PostView {...{ ...baseProps, isFirstLoad: true }} />,
        );
        expect(wrapper.state("unreadChunkTimeStamp")).toEqual(
            baseProps.lastViewedAt,
        );
    });
    ...
    test("changeUnreadChunkTimeStamp", () => {
        ...
        wrapper
            .find(PostList)
            .prop<(lastViewedAt?: string) => void>(
                "changeUnreadChunkTimeStamp",
            )("1234678");
