...
describe("components/post_view/MessageAttachment", () => {
    ...
    test("should match value on renderPostActions", () => {
        let wrapper = shallow<MessageAttachment>(
            <MessageAttachment {...baseProps} />,
        );
    ...
    test("should call actions.doPostActionWithCookie on handleAction", () => {
        ...
        const props = {
            ...baseProps,
            actions: { doPostActionWithCookie, openModal },
    ...
    test("should use ExternalImage for images", () => {
        ...
        expect(
            wrapper
                .find(ExternalImage)
                .find({ src: props.attachment.author_icon })
                .exists(),
        ).toBe(true);
        expect(
            wrapper
                .find(ExternalImage)
                .find({ src: props.attachment.image_url })
                .exists(),
        ).toBe(true);
        expect(
            wrapper
                .find(ExternalImage)
                .find({ src: props.attachment.footer_icon })
                .exists(),
        ).toBe(true);
        expect(
            wrapper
                .find(ExternalImage)
                .find({ src: props.attachment.thumb_url })
                .exists(),
        ).toBe(true);
    });
