...
function GMConversionMessage(props: Props): JSX.Element {
    ...
    const gmMembersDuringConversionIDs = props.post.props
        .gmMembersDuringConversionIDs as string[];
    ...
    const getProfilesByIdsAndUsernames = useRef(
        makeGetProfilesByIdsAndUsernames(),
    );
    const userProfiles = useSelector((state: GlobalState) =>
        getProfilesByIdsAndUsernames.current(state, {
            allUserIds: gmMembersDuringConversionIDs,
            allUsernames: [],
        }),
    );

    const convertedByUserUsername = userProfiles.find(
        (user) => user.id === convertedByUserId,
    )!.username;
    const gmMembersUsernames = userProfiles.map((user) =>
    ...
    if (
        ...
        gmMembersDuringConversionIDs.length === 0
    ) {
        return <span>{props.post.message}</span>;
    }
