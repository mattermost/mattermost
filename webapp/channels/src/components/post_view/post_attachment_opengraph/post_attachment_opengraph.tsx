...
export function getBestImage(
    ...
    return getNearestPoint<ImageMetadata>(
...
const PostAttachmentOpenGraph = ({
    ...
    const { current: bestImageData } = useRef<ImageMetadata>(
        getBestImage(openGraphData, post.metadata.images),
    );
    const isPreviewRemoved =
        post?.props?.[PostTypes.REMOVE_LINK_PREVIEW] === "true";
    ...
    const removeButtonTooltip = (
        <Tooltip id={`removeLinkPreview-${post.id}`}>
            {formatMessage({
                id: "link_preview.remove_link_preview",
                defaultMessage: "Remove link preview",
            })}
        </Tooltip>
    );
...
export const PostAttachmentOpenGraphBody = memo(
    ({ title, isInPermalink, sitename = "", description = "" }: BodyProps) => {
        return title ? (
            <div
                className={classNames("PostAttachmentOpenGraph__body", {
                    isInPermalink,
                })}
            >
                {!isInPermalink && sitename && (
...
export const PostAttachmentOpenGraphImage = memo(
    ({
        imageMetadata,
        isInPermalink,
        ...
        const collapsedLabel = formatMessage({
            id: "link_preview.image_preview",
            defaultMessage: "Show image preview",
        });
        ...
        const imageCollapseButton = (
            ...
            >
                {isEmbedVisible ? (
                    <MenuDownIcon size={18} color="currentColor" />
                ) : (
                    <>
                        <MenuRightIcon size={18} color="currentColor" />
                        {collapsedLabel}
                    </>
                )}
            </button>
        ...
        return (
            <div
                className={classNames("PostAttachmentOpenGraph__image", {
                    large,
                    collapsed: !isEmbedVisible,
                })}
            >
                {large ? (
                    <AutoHeightSwitcher
