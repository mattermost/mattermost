...
const preferenceKeys = {
    COLLAPSE_DISPLAY: getPreferenceKey(
        Preferences.CATEGORY_DISPLAY_SETTINGS,
        Preferences.COLLAPSE_DISPLAY,
    ),
    LINK_PREVIEW_DISPLAY: getPreferenceKey(
        Preferences.CATEGORY_DISPLAY_SETTINGS,
    ...
    images: [
        {
            height: 1256,
            secure_url:
                "http://localhost:8065/api/v4/image?url=http%3A%2F%2Fmattermo…t.com%2Fwp-content%2Fuploads%2F2021%2F09%2FHomepage%402x.png",
            type: "image/png",
            url: "",
            width: 2400,
        },
...
const initialState = {
    entities: {
        ...
        preferences: {
            myPreferences: {
                [preferenceKeys.COLLAPSE_DISPLAY]: {
                    value: Preferences.COLLAPSE_DISPLAY_DEFAULT,
                },
                [preferenceKeys.LINK_PREVIEW_DISPLAY]: {
                    value: Preferences.LINK_PREVIEW_DISPLAY_DEFAULT,
                },
            },
        },
        teams: {
...
describe("PostAttachmentOpenGraph", () => {
    ...
    test("should render nothing when link previews are disabled by the user", () => {
        ...
        set(
            state,
            `entities.preferences.EnableLinkPreviews["${preferenceKeys.LINK_PREVIEW_DISPLAY}"]`,
            "false",
        );
...
describe("PostAttachmentOpenGraphImage", () => {
    ...
    test("should render a large image with toggle", () => {
        ...
        expect(wrapper.find(".PostAttachmentOpenGraph__image").exists()).toBe(
            true,
        );
        expect(
            wrapper.find(".PostAttachmentOpenGraph__image.large").exists(),
        ).toBe(true);
        expect(
            wrapper
                .find(".PostAttachmentOpenGraph__image .preview-toggle")
                .exists(),
        ).toBe(true);
    });
