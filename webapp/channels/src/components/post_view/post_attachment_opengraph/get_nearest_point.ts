...
export function getDistanceBW2Points(point1: Point, point2: Point) {
    return Math.sqrt(
        Math.pow(point1.width - point2.width, 2) +
 ...
 */
export function getNearestPoint<T extends PartialPoint>(
    pivot: Point,
    points: T[],
): T {
    return points.reduce((nearest, point) => {
        if (
            ...
            getDistanceBW2Points(point, pivot) >=
                getDistanceBW2Points(nearest, pivot)
        ) {
            return nearest;
        }
        return point;
