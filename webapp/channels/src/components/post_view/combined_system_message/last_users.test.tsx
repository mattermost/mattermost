...
describe("components/post_view/combined_system_message/LastUsers", () => {
    ...
    test("should match component state", () => {
        renderWithIntlAndStore(<LastUsers {...baseProps} />, initialState);

        expect(screen.getByText(getMentionKeyAt(0))).toBeInTheDocument();
        expect(screen.getByText(getMentionKeyAt(0))).toHaveAttribute(
            "data-mention",
            "username2",
        );
        ...
        userEvent.click(
            screen.getByText(`${formatOptions.mentionKeys.length - 1} others`),
        );

        expect(screen.getByText("added to the channel")).toBeInTheDocument();
        expect(
            screen.getByText(`by ${baseProps.actor}`, { exact: false }),
        ).toBeInTheDocument();
    });
