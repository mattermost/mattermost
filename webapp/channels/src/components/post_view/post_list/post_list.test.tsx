...
const actionsProp = {
    loadPostsAround: jest
        .fn()
        .mockImplementation(() =>
            Promise.resolve({ atLatestMessage: true, atOldestmessage: true }),
        ),
    loadUnreads: jest
        .fn()
        .mockImplementation(() =>
    ...
    loadPosts: jest
        .fn()
        .mockImplementation(() => Promise.resolve({ moreToLoad: false })),
    syncPostsInChannel: jest.fn().mockResolvedValue({}),
    loadLatestPosts: jest
        .fn()
        .mockImplementation(() =>
            Promise.resolve({ atLatestMessage: true, atOldestmessage: true }),
        ),
...
describe("components/post_view/post_list", () => {
    ...
    it("Should call postsOnLoad", async () => {
        ...
        expect(actionsProp.loadUnreads).toHaveBeenCalledWith(
            baseProps.channelId,
        );
    ...
    it("Should not call loadUnreads if isPrefetchingInProcess is true", async () => {
        ...
        shallow(
            <PostList
                {...{
                    ...baseProps,
                    postListIds: emptyPostList,
                    isPrefetchingInProcess: true,
                }}
            />,
        );

        expect(actionsProp.loadUnreads).not.toHaveBeenCalledWith(
            baseProps.channelId,
        );
    });
    ...
    it("Should call for before and afterPosts", async () => {
        ...
        await wrapper
            .instance()
            .callLoadPosts("undefined", "undefined", undefined);
        expect(wrapper.state("loadingOlderPosts")).toBe(false);
    ...
    it("Should call for permalink posts", async () => {
        ...
        expect(actionsProp.loadPostsAround).toHaveBeenCalledWith(
            baseProps.channelId,
            focusedPostId,
        );
    ...
    it("Should call for loadLatestPosts", async () => {
        ...
        expect(actionsProp.loadLatestPosts).toHaveBeenCalledWith(
            baseProps.channelId,
        );
    ...
    describe("getPostsSince", () => {
        test("should call getPostsSince on channel switch", () => {
            ...
            expect(actionsProp.syncPostsInChannel).toHaveBeenCalledWith(
                baseProps.channelId,
                1234,
                false,
            );
    ...
    describe("canLoadMorePosts", () => {
        ...
        test("Should call getPostsAfter canLoadMorePosts is requested with AFTER_ID", async () => {
            ...
            wrapper
                .find(VirtPostList)
                .prop("actions")
                .canLoadMorePosts(PostRequestTypes.AFTER_ID);
            expect(actionsProp.loadPosts).toHaveBeenCalledWith({
                channelId: baseProps.channelId,
                postId: postIds[0],
                type: PostRequestTypes.AFTER_ID,
    ...
    describe("markChannelAsReadAndViewed", () => {
        test("Should call markChannelAsReadAndViewed on postsOnLoad", async () => {
            ...
            await wrapper.instance().postsOnLoad("undefined");
            expect(actionsProp.markChannelAsRead).toHaveBeenCalledWith(
                baseProps.channelId,
            );
        });
