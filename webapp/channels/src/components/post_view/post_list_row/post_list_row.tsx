...
export default class PostListRow extends React.PureComponent<PostListRowProps> {
    ...
    componentDidUpdate(prevProps: PostListRowProps) {
        const { listId, isLastPost, shortcutReactToLastPostEmittedFrom } =
            this.props;

        const shortcutReactToLastPostEmittedFromCenter =
            prevProps.shortcutReactToLastPostEmittedFrom !==
    ...
    render() {
        const { listId, previousListId, loadingOlderPosts, loadingNewerPosts } =
            this.props;
        const {
            ...
            return (
                <CenterMessageLock
                    ...
                    firstInaccessiblePostTime={
                        this.props.firstInaccessiblePostTime
                    }
                />
            );
        }
        ...
        if (
            listId === LOAD_OLDER_MESSAGES_TRIGGER ||
            listId === LOAD_NEWER_MESSAGES_TRIGGER
        ) {
            return (
                <button
                    ...
                    onClick={
                        listId === LOAD_OLDER_MESSAGES_TRIGGER
                            ? this.props.loadOlderPosts
                            : this.props.loadNewerPosts
                    }
                >
                    <FormattedMessage
        ...
        if (isOlderMessagesLoader || isNewerMessagesLoader) {
            const shouldHideAnimation =
                !loadingOlderPosts && !loadingNewerPosts;

            return (
                <div className="loading-screen">
                    <div
                        className={classNames("loading__content", {
                            hideAnimation: shouldHideAnimation,
                        })}
                    >
