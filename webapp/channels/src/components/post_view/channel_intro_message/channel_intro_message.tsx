...
export default class ChannelIntroMessage extends React.PureComponent<Props> {
    ...
    render() {
        ...
        if (channel.type === Constants.DM_CHANNEL) {
            ...
            return createOffTopicIntroMessage(
                channel,
                centeredIntro,
                stats,
                usersLimit,
            ...
            return createStandardIntroMessage(
                channel,
                centeredIntro,
                stats,
                usersLimit,
...
const getGMIntroMessageSpecificPart = (
    ...
    const channelNotifyProp =
        membership?.notify_props?.desktop || NotificationLevel.DEFAULT;
    const userNotifyProp =
        userProfile?.notify_props?.desktop || NotificationLevel.MENTION;
    let notifyLevelToUse = channelNotifyProp;
    if (notifyLevelToUse === NotificationLevel.DEFAULT) {
        notifyLevelToUse = userNotifyProp;
    }
    if (
        channelNotifyProp === NotificationLevel.DEFAULT &&
            ...
            >
                ...
                <p className="channel-intro-text">
                    <FormattedMessage
                        ...
                        defaultMessage={
                            "This is the start of your group message history with {names}.{br}"
                        }
                        values={{
                ...
                <div style={{ display: "flex" }}>
                    {createNotificationPreferencesButton(
                        channel,
                        currentUserProfile,
                    )}
...
function createDMIntroMessage(
    ...
    if (teammate) {
        const src = teammate
            ? Utils.imageURLForUser(teammate.id, teammate.last_picture_update)
            : "";
...
function createNotificationPreferencesButton(
    channel: Channel,
