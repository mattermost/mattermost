...
describe("components/post_view/ChannelIntroMessages", () => {
    ...
    describe("test Open Channel", () => {
        test("should match component state, without boards", () => {
            ...
            const beginningHeading = screen.getByText(
                "Beginning of test channel",
            );
            ...
            expect(
                screen.getByText(
                    `This is the start of the test channel channel, created by ${baseProps.creatorName} on October 17, 2017.`,
                ),
            );
            expect(
                screen.getByText("Any member can join and read this channel."),
            ).toBeInTheDocument();
        });
    });
    ...
    describe("test Group Channel", () => {
        ...
        test("should match component state, no profiles", () => {
            renderWithIntlAndStore(
                <ChannelIntroMessage {...props} />,
                initialState,
            );

            expect(
                screen.queryByText("Beginning of test channel"),
            ).not.toBeInTheDocument();
            expect(
                screen.queryByText(
                    "Any member can join and read this channel.",
                ),
            ).not.toBeInTheDocument();
            ...
            expect(
                screen.getByText(
                    "This is the start of your group message history with these teammates. Messages and files shared here are not shown to people outside this area.",
                ),
            ).toBeInTheDocument();

            expect(
                screen.getByText("This is the start of your", { exact: false }),
            ).toHaveClass("channel-intro-text");
        });

        test("should match component state, with profiles", () => {
            renderWithIntlAndStore(
                <ChannelIntroMessage {...props} channelProfiles={users} />,
                initialState,
            );

            expect(
                screen.getByText(
                    "This is the start of your group message history with test channel",
                    { exact: false },
                ),
            ).toBeInTheDocument();
            ...
            expect(headerDialog).toHaveClass(
                "style--none intro-links color--link channelIntroButton",
            );
            ...
            const notificationPreferencesButton = screen.getByText(
    ...
    describe("test DIRECT Channel", () => {
        ...
        test("should match component state, without teammate", () => {
            ...
            const message = screen.getByText(
                "This is the start of your direct message history with this teammate",
                { exact: false },
            );
        ...
        test("should match component state, with teammate", () => {
            ...
            expect(
                screen.getByText(
                    "This is the start of your direct message history with my teammate",
                    { exact: false },
                ),
            ).toBeInTheDocument();
    ...
    describe("test DEFAULT Channel", () => {
        ...
        test("should match component state, readonly", () => {
            renderWithIntlAndStore(
                <ChannelIntroMessage {...props} isReadOnly={true} />,
                initialState,
            );
            ...
            expect(
                screen.getByText("Welcome to test channel!"),
            ).toBeInTheDocument();
            expect(
                screen.getByText(
        ...
        test("should match component state without any permission", () => {
            ...
            expect(
                screen.queryByText("Add other groups to this team"),
            ).not.toBeInTheDocument();
    ...
    describe("test OFF TOPIC Channel", () => {
        ...
        test("should match component state", () => {
            ...
            expect(
                screen.getByText("Beginning of off-topic"),
            ).toBeInTheDocument();
            screen.getByText(
                "This is the start of off-topic, a channel for non-work-related conversations.",
            );
            expect(
                screen.getByText(
                    "This is the start of off-topic, a channel for non-work-related conversations.",
                ),
            ).toHaveClass("channel-intro__content");
