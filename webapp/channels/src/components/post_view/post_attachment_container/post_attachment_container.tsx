...
const getTeamAndPostIdFromLink = (link: string) => {
    const match = matchPath<LinkParams>(link, {
        path: "/:teamName/pl/:postId",
    });
    return match?.params;
};
...
const PostAttachmentContainer = (props: Props) => {
    ...
    const shouldFocusPostWithoutRedirect = useSelector((state: GlobalState) =>
        isTeamSameWithCurrentTeam(state, params?.teamName ?? ""),
    );
    const post = useSelector((state: GlobalState) =>
        getPost(state, params?.postId ?? ""),
    );
    ...
    const handleOnClick = useCallback(
        (e) => {
            ...
            if (
                ...
                if (
                    params &&
                    shouldFocusPostWithoutRedirect &&
                    crtEnabled &&
                    post &&
                    post.root_id
                ) {
                    dispatch(
                        focusPost(params.postId, link, currentUserId, {
                            skipRedirectReplyPermalink: true,
                        }),
                    );
                    return;
                }
