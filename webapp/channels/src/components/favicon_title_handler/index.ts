...
): ComponentProps<typeof FaviconTitleHandler> {
    const config = getConfig(state);
    const currentChannel = getCurrentChannel(state);
    const currentTeammate =
        currentChannel && currentChannel.teammate_id ? currentChannel : null;
...
export default withRouter(
    connect(mapStateToProps, mapDispatchToProps)(FaviconTitleHandler),
);
