        ...
        switch (postId) {
            case "postid1":
                return {
                    ...
                    data: {
                        posts: { replypostid1: replyPost1, postid1: post },
                        order: [post.id, replyPost1.id],
                    },
                };
            case "postid2":
                return {
...
jest.mock("mattermost-redux/actions/users", () => ({
    getMissingProfilesByIds: (userIds: string[]) => ({
        type: "MOCK_GET_MISSING_PROFILES",
        userIds,
    }),
}));
...
jest.mock("mattermost-redux/actions/channels", () => ({
    ...
    getChannelStats: (...args: any) => ({
        type: "MOCK_GET_CHANNEL_STATS",
        args,
    }),
...
describe("components/PermalinkView", () => {
    ...
    test("should call baseProps.actions.focusPost on doPermalinkEvent", async () => {
        await act(async () => {
            mountWithIntl(<PermalinkView {...baseProps} />);
        });
    ...
    test("should call baseProps.actions.focusPost when postid changes", async () => {
        ...
        await act(async () => {
            wrapper = mountWithIntl(<PermalinkView {...baseProps} />);
        });
        const newPostid = `${baseProps.match.params.postid}_new`;
        await wrapper!.setProps({
            ...baseProps,
            match: { params: { postid: newPostid } },
        } as any);
    ...
    test("should match snapshot with archived channel", async () => {
        ...
        await act(async () => {
            wrapper = mountWithIntl(<PermalinkView {...props} />);
        });
            ...
            entities: {
                ...
                channels: {
                    channels: {
                        ...
                        dmchannelid: {
                            id: "dmchannelid",
                            name: "dmchannel__current_user_id",
                            type: "D",
                            team_id: "",
                        },
                        gmchannelid: {
        ...
        describe("focusPost", () => {
            test("should redirect to error page for DM channel not a member of", async () => {
                ...
                await testStore.dispatch(
                    focusPost("dmpostid1", undefined, baseProps.currentUserId),
                );
            ...
            test("should redirect to error page for GM channel not a member of", async () => {
                const testStore = await mockStore(initialState);
                await testStore.dispatch(
                    focusPost("gmpostid1", undefined, baseProps.currentUserId),
                );
            ...
            test("should redirect to DM link with postId for permalink", async () => {
                ...
                expect(getHistory().replace).toHaveBeenCalledWith(
                    "/currentteam/messages/@otherUser/dmpostid1",
                );
            ...
            test("should redirect to GM link with postId for permalink", async () => {
                ...
                expect(getHistory().replace).toHaveBeenCalledWith(
                    "/currentteam/messages/gmchannel/gmpostid1",
                );
            });
            ...
            test("should redirect to channel link with postId for permalink", async () => {
                const testStore = await mockStore(initialState);
                await testStore.dispatch(
                    focusPost("postid1", undefined, baseProps.currentUserId),
                );
                ...
                expect(getHistory().replace).toHaveBeenCalledWith(
                    "/currentteam/channels/channel1/postid1",
                );
            });
            ...
            test("should not redirect to channel link with postId for a reply permalink when collapsedThreads enabled and option is set true", async () => {
                ...
                jest.spyOn<typeof Channels, keyof typeof Channels>(
                    Channels,
                    "getCurrentChannel",
                ).mockReturnValue({
                    id: "channelid1",
                ...
                expect(testStore.getActions()).toEqual([
                    ...
                    {
                        type: "MOCK_SELECT_POST_AND_HIGHLIGHT",
                        args: [
                            {
                                id: "replypostid1",
                                message: "some message",
