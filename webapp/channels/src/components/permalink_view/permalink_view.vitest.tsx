...
vi.mock('actions/channel_actions', () => ({
    loadChannelsForCurrentUser: vi.fn(() => {
        return {type: 'MOCK_LOAD_CHANNELS_FOR_CURRENT_USER'};
    }),
}));

vi.mock('actions/views/rhs.ts', () => ({
    selectPostAndHighlight: vi.fn((post) => {
        return {type: 'MOCK_SELECT_POST_AND_HIGHLIGHT', args: [post]};
    }),
}));

vi.mock('mattermost-redux/actions/posts', () => ({
    getPostThread: vi.fn((postId) => {
...
describe('components/PermalinkView', () => {
    ...
    beforeEach(() => {
        focusPostMock.mockClear();
        vi.mocked(getPostThread).mockClear();
        vi.mocked(joinPrivateChannelPrompt).mockClear();
        vi.mocked(getHistory().replace).mockClear();
    });
    ...
    test('should call baseProps.actions.focusPost on doPermalinkEvent', async () => {
        ...
        await waitFor(() => {
            expect(focusPostMock).toHaveBeenCalledTimes(1);
            expect(focusPostMock).toHaveBeenCalledWith(baseProps.match.params.postid, baseProps.returnTo, baseProps.currentUserId);
        });
    });
    ...
    test('should call baseProps.actions.focusPost when postid changes', async () => {
        ...
        await waitFor(() => {
            expect(focusPostMock).toHaveBeenCalledTimes(2);
            expect(focusPostMock).toHaveBeenCalledWith(newPostid, baseProps.returnTo, baseProps.currentUserId);
        });
    });
    ...
    describe('actions', () => {
        ...
        describe('focusPost', () => {
            ...
            test('should not redirect to channel link with postId for a reply permalink when collapsedThreads enabled and option is set true', async () => {
                ...
                vi.spyOn(Channels, 'getCurrentChannel').mockReturnValue({id: 'channelid1', name: 'channel1', type: 'O', team_id: 'current_team_id'} as any);
            ...
            describe('focusPost - with prompt', () => {
                ...
                test('should allow redirect to private channel link if prompt response true', async () => {
                    ...
                    vi.mocked(joinPrivateChannelPrompt).mockReturnValueOnce(async () => ({data: {join: true}}));
