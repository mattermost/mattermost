...
function focusReplyPost(
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (data.first_inaccessible_post_time) {
            getHistory().replace(
                `/error?type=${ErrorPageTypes.CLOUD_ARCHIVED}&returnTo=${returnTo}`,
            );
            return { data: false };
        }
...
export function focusPost(
    postId: string,
    returnTo = "",
    currentUserId: string,
    option: Option = { skipRedirectReplyPermalink: false },
) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (!myMember) {
            ...
            const membership = await dispatch(
                getChannelMember(channel.id, currentUserId),
            );
            ...
            if (!myMember) {
                ...
                if (
                    ...
                    const joinPromptResult = await dispatch(
                        joinPrivateChannelPrompt(currentTeam, channel),
                    );
                    privateChannelJoinPromptVisible = false;
                    if (
                        "data" in joinPromptResult &&
                        !joinPromptResult.data.join
                    ) {
                        return;
                    }
                }
                await dispatch(
                    joinChannel(currentUserId, "", channelId, channel.name),
                );
            }
        }
        ...
        if (channel && channel.type === Constants.DM_CHANNEL) {
            const userId = getUserIdFromChannelName(
                currentUserId,
                channel.name,
            );
        ...
        if (isCollapsed && isComment(post)) {
            const { data } = await dispatch(
                focusReplyPost(post, channel, teamId, returnTo, option),
            );
            if (!data) {
                return;
            }
