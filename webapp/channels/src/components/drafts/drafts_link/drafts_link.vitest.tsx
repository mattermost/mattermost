...
import * as scheduledPostsActions from 'mattermost-redux/actions/scheduled_posts';
...
vi.mock('actions/views/drafts', () => ({
    getDrafts: vi.fn(() => ({type: 'MOCK_GET_DRAFTS'})),
}));

vi.mock('mattermost-redux/actions/scheduled_posts', () => ({
    fetchTeamScheduledPosts: vi.fn(() => ({type: 'MOCK_FETCH_SCHEDULED_POSTS'})),
}));
...
describe('components/drafts/drafts_link', () => {
    ...
    it('should fetch scheduled posts when component mounts', async () => {
        const fetchTeamScheduledPosts = scheduledPostsActions.fetchTeamScheduledPosts as ReturnType<typeof vi.fn>;
