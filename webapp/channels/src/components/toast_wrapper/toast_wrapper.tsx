...
export type Props = WrappedComponentProps &
    RouteComponentProps<{ team: string }> & {
...
export class ToastWrapperClass extends React.PureComponent<Props, State> {
    ...
    static countNewMessages = (
        ...
        let newMessages = postListIds
            .slice(0, mark)
            .filter((id) => !isIdNotPost(id));
        if (isCollapsedThreadsEnabled) {
            // in collapsed mode we only count root posts
    ...
    static getDerivedStateFromProps(props: Props, prevState: State) {
        ...
        if (props.atLatestPost) {
            if (
                props.unreadScrollPosition ===
                    Preferences.UNREAD_SCROLL_POSITION_START_FROM_NEWEST &&
                prevState.unreadCountInChannel
            ) {
                unreadCount =
                    prevState.unreadCountInChannel +
                    props.newRecentMessagesCount;
            } else {
                unreadCount = ToastWrapperClass.countNewMessages(
                    props.rootPosts,
            ...
            if (
                ...
                unreadCount =
                    props.unreadCountInChannel + props.newRecentMessagesCount;
            } else {
                unreadCount = prevState.unreadCountInChannel;
            }
        } else {
            unreadCount =
                prevState.unreadCountInChannel + props.newRecentMessagesCount;
        }
        ...
        if (
            typeof showMessageHistoryToast === "undefined" &&
            props.focusedPostId !== "" &&
            props.atBottom !== null
        ) {
            showMessageHistoryToast =
                props.initScrollOffsetFromBottom > 1000 || !props.atLatestPost;
        }

        // show unread toast when a channel is marked as unread
        if (
            props.channelMarkedAsUnread &&
            props.atBottom === false &&
        ...
        if (
            ...
            unreadCount > 0 &&
            props.atBottom === false &&
            props.latestPostTimeStamp &&
            props.lastViewedBottom < props.latestPostTimeStamp
        ) {
    ...
    componentDidUpdate(prevProps: Props, prevState: State) {
        ...
        const postsAddedAtBottom =
            presentPostsCount !== prevPostsCount &&
            postListIds[0] !== prevProps.postListIds[0];
        const notBottomWithLatestPosts =
            atBottom === false && atLatestPost && presentPostsCount > 0;
        ...
        const toastStateChanged =
            ...
            prevState.showUnreadWithBottomStartToast !==
                showUnreadWithBottomStartToast;
    ...
    newMessagesToastText = (count: number | undefined, since: number) => {
        if (
            this.props.width > TOAST_TEXT_COLLAPSE_WIDTH &&
        ...
        return (
            <FormattedMessage
                ...
                defaultMessage={
                    "{count, number} new {count, plural, one {message} other {messages}}"
                }
                values={{ count }}
            />
        );
    ...
    scrollToLatestMessages = () => {
        const { focusedPostId, atLatestPost, scrollToLatestMessages } =
            this.props;
    ...
    getToastToRender() {
        ...
        const unreadToastProps = {
            ...
            onClickMessage: localizeMessage(
        ...
        if (showNewMessagesToast) {
            const showNewMessagesToastOverrides = {
                ...
                onClickMessage: localizeMessage(
                    "postlist.toast.scrollToLatest",
                    "Jump to new messages",
                ),
            };

            return (
                <Toast {...unreadToastProps} {...showNewMessagesToastOverrides}>
                    {this.newMessagesToastText(unreadCount, lastViewedAt)}
                </Toast>
            );
        }
        ...
        if (showSearchHintToast) {
            return (
                <HintToast onDismiss={this.hideSearchHintToast}>
    ...
    render() {
        ...
        return <React.Fragment>{toastToRender}</React.Fragment>;
    }
}
