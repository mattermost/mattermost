...
export function makeCountUnreadsBelow() {
    return createSelector(
        ...
        (state: GlobalState, postIds, lastViewedBottom: number) =>
            lastViewedBottom,
        isCollapsedThreadsEnabled,
        (allPosts, currentUserId, postIds, lastViewedBottom, isCollapsed) => {
            ...
            return postIds
                ...
                .filter((post) => {
                    ...
                    ); // in collapsed threads mode, only count root posts
                }).length;
        },
    );
}
...
function makeMapStateToProps() {
    ...
    return function mapStateToProps(state: GlobalState, ownProps: OwnProps) {
        ...
        const channelMarkedAsUnread = isManuallyUnread(
            state,
            ownProps.channelId,
        );
        const lastViewedAt =
        ...
        if (!ownProps.atLatestPost) {
            ...
            newRecentMessagesCount = countUnreadsBelow(
                state,
                postIds,
                lastViewedAt,
            );
...
export default withRouter(
    connect(makeMapStateToProps, mapDispatchToProps)(ToastWrapper),
);
