    ...
    return {
        ...
        [OnboardingTasksName.VISIT_SYSTEM_CONSOLE]: {
            ...
            message: formatMessage({
                ...
                defaultMessage:
                    "Visit the System Console to configure your workspace.",
            }),
        },
...
export const useTasksList = () => {
    ...
    if (
        isUserFirstAdmin ||
        (!pluginsList.playbooks && !pluginsList.focalboard)
    ) {
...
export const useTasksListWithStatus = () => {
    const dataInDb = useSelector((state: GlobalState) =>
        getCategory(state, OnboardingTaskCategory),
    );
    ...
    return useMemo(
        () =>
            tasksList.map((task) => {
                const status = dataInDb.find((pref) => pref.name === task)
                    ?.value;
                return {
                    name: task,
...
export const useHandleOnBoardingTaskData = () => {
    ...
    return useCallback(
        (
            ...
            if (trackEvent) {
                const eventSuffix = trackEventSuffix
                    ? `${step}--${trackEventSuffix}`
                    : step.toString();
...
export const useHandleOnBoardingTaskTrigger = () => {
    ...
    const inChannels =
        matchPath(pathname, { path: "/:team/channels/:chanelId" }) != null;
    ...
    return (taskName: string) => {
        switch (taskName) {
            case OnboardingTasksName.CHANNELS_TOUR: {
                handleSaveData(
                    taskName,
                    TaskNameMapToSteps[taskName].STARTED,
                    true,
                );
                ...
                const preferences = [
                    {
                        ...
                        value: isGuestUser
                            ? OnboardingTourStepsForGuestUsers.SEND_MESSAGE.toString()
                            : OnboardingTourSteps.CHANNELS_AND_DIRECT_MESSAGES.toString(),
                    },
                    {
                        user_id: currentUserId,
                        category: tourCategory,
            ...
            case OnboardingTasksName.PLAYBOOKS_TOUR: {
                ...
                handleSaveData(
                    taskName,
                    TaskNameMapToSteps[taskName].FINISHED,
                    true,
                );
                break;
            }
