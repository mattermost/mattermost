...
describe("onboarding tasks manager", () => {
    ...
    it("Places all the elements (6 ignoring plugins) when user is first admin or admin", () => {
        ...
        expect(
            wrapper.findWhere((node) => node.key() === "visit_system_console"),
        ).toHaveLength(1);
        expect(
            wrapper.findWhere((node) => node.key() === "start_trial"),
        ).toHaveLength(1);
    });
    ...
    it("Removes start_trial and visit_system_console when user is end user", () => {
        const endUserState = {
            ...
            entities: {
                ...
                users: { ...initialState.entities.users, currentUserId: user2 },
            },
        };
    ...
    it("Removes invite people task item when user is GUEST user", () => {
        const endUserState = {
            ...
            entities: {
                ...initialState.entities,
                users: { ...initialState.entities.users, currentUserId: user3 },
            },
        };
        const store = configureStore(endUserState);
        ...
        // verify visit_system_console and start_trial were removed
        expect(
            wrapper.findWhere((node) => node.key() === "invite_people"),
        ).toHaveLength(0);
    });
});
