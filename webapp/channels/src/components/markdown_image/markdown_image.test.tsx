...
describe("components/MarkdownImage", () => {
    ...
    test("should render a link if the source is unsafe", () => {
        ...
        expect(wrapper.find("img").props().className).toBe(
            props.className + " broken-image",
        );
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test("should call openModal when showModal is called", () => {
        ...
        (wrapper.instance() as MarkdownImage).showModal(
            mockEvent as unknown as React.MouseEvent<HTMLImageElement>,
    ...
    test("should render image with title, height, width", () => {
        ...
        expect(childrenWrapper.find(SizeAwareImage).prop("width")).toEqual(
            "50",
        );
        expect(childrenWrapper.find(SizeAwareImage).prop("height")).toEqual(
            "76",
        );

        expect(childrenWrapper.find(SizeAwareImage).prop("title")).toEqual(
            "test title",
        );
    });
