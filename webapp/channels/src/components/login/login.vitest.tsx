...
vi.mock('utils/desktop_api', () => ({
    default: {
        ...
        hasGPOEnabledSSO: vi.fn().mockReturnValue(false),
    },
    dispatchNotification: vi.fn(),
    setSessionExpired: vi.fn(),
}));
...
describe('components/login/Login', () => {
    ...
    it('should match snapshot', () => {
        ...
        // Snapshot test - verify the component renders
    ...
    it('should handle session expired', async () => {
        ...
        (showNotification as ReturnType<typeof vi.fn>).mockReturnValue(() => Promise.resolve({status: 'success', callback: () => {}}));
    ...
    it('should call the correct function to show the session expired notification on Desktop App', () => {
        (isDesktopApp as ReturnType<typeof vi.fn>).mockReturnValue(true);
        LocalStorageStore.setWasLoggedIn(true);
        const state = mergeObjects(baseState, {
            entities: {
                general: {
    ...
    it('should redirect on login', async () => {
        ...
        history.push = vi.fn().mockImplementation(history.push);
    ...
    describe('EnableGuestMagicLink', () => {
        ...
        it('should show GuestMagicLinkCard when user login type is guest_magic_link', async () => {
            ...
            const mockGetUserLoginType = vi.fn().mockReturnValue(async () => ({
                data: {
                    auth_service: 'guest_magic_link',
                    is_deactivated: false,
                },
            }));
            vi.spyOn(loginActions, 'getUserLoginType').mockImplementation(mockGetUserLoginType);
        ...
        it('should submit with password when password field is shown', async () => {
            ...
            const mockLogin = vi.fn().mockReturnValue(async () => ({
                data: true,
            }));
            vi.spyOn(loginActions, 'login').mockImplementation(mockLogin);
