...
describe('components/sidebar/sidebar_channel/sidebar_channel_link', () => {
    ...
    test('should match snapshot for desktop', async () => {
    ...
    test('should match snapshot when tooltip is enabled', async () => {
        const props = {
            label: 'a'.repeat(200),
        };
        ...
        const {container} = await renderLink(props);
    ...
    test('should match snapshot with aria label prefix and unread mentions', async () => {
        const props = {
            isUnread: true,
            unreadMentions: 2,
    ...
    test('should enable tooltip when needed', async () => {
        Object.defineProperty(HTMLElement.prototype, 'offsetWidth', {configurable: true, value: 50});
    ...
    test('should not fetch shared channels for non-shared channels', async () => {
        const props = {
            isSharedChannel: false,
        };
    ...
    test('should fetch shared channels data when channel is shared', async () => {
        const props = {
    ...
    test('should not fetch shared channels data when data already exists', async () => {
        const props = {
            isSharedChannel: true,
    ...
    test('should include urgent mention in link accessible name when hasUrgent', async () => {
        await renderLink({
            unreadMentions: 2,
            hasUrgent: true,
        });
    ...
    test('should not include urgent mention in link accessible name when not hasUrgent', async () => {
        await renderLink({
            unreadMentions: 2,
            hasUrgent: false,
        });
    ...
    test('should refetch when channel changes', async () => {
        const props = {
            isSharedChannel: true,
            remoteNames: [],
        };

        const {rerender} = await renderLink(props);
