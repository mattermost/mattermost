...
function mapStateToProps(state: GlobalState) {
    ...
    if (currentTeam) {
        canCreatePublicChannel = haveICurrentChannelPermission(
            state,
            Permissions.CREATE_PUBLIC_CHANNEL,
        );
        canCreatePrivateChannel = haveICurrentChannelPermission(
            state,
            Permissions.CREATE_PRIVATE_CHANNEL,
        );
        canJoinPublicChannel = haveICurrentChannelPermission(
            state,
            Permissions.JOIN_PUBLIC_CHANNELS,
        );
    }

    const canCreateCustomGroups =
    ...
    return {
        ...
        isKeyBoardShortcutModalOpen: isModalOpen(
            state,
