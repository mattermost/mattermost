...
describe("/components/create_team/components/display_name", () => {
    ...
    test("should successfully submit", async () => {
        ...
        await (wrapper.instance() as unknown as TeamUrl).submitNext({
            preventDefault: jest.fn(),
        } as unknown as React.MouseEvent<Button, MouseEvent>);
    ...
    test("should display charLength error", () => {
        ...
        (
            wrapper
                .find(".form-control")
                .instance() as unknown as HTMLInputElement
        ).value = "a".repeat(Constants.MAX_TEAMNAME_LENGTH + 1);
        wrapper.find(".form-control").simulate("change");
        wrapper
            .find("button")
            .simulate("click", { preventDefault: () => jest.fn() });
    ...
    test("should display teamUrl taken error", () => {
        ...
        wrapper
            ...
            .simulate("click", { preventDefault: () => jest.fn() });
        expect(
            (wrapper as any).find(TeamUrl).state("nameError").props.id,
        ).toEqual("create_team.team_url.taken");
    });
});
