...
const UpdateUserGroupModal = (props: Props) => {
    ...
    const patchGroup = useCallback(async () => {
        ...
        if (newMention.length < 1) {
            setMentionInputErrorText(
                Utils.localizeMessage(
                    "user_groups_modal.mentionIsEmpty",
                    "Mention is a required field.",
                ),
            );
            setSaving(false);
            return;
        ...
        if (Constants.SPECIAL_MENTIONS.includes(newMention.toLowerCase())) {
            setMentionInputErrorText(
                Utils.localizeMessage(
                    "user_groups_modal.mentionReservedWord",
                    "Mention contains a reserved word.",
                ),
            );
            setSaving(false);
            return;
        ...
        if (!mentionRegEx.test(newMention)) {
            setMentionInputErrorText(
                Utils.localizeMessage(
                    "user_groups_modal.mentionInvalidError",
                    "Invalid character in mention.",
                ),
            );
            setSaving(false);
            return;
        ...
        if (data?.error) {
            if (
                data.error?.server_error_id === "app.custom_group.unique_name"
            ) {
                setMentionInputErrorText(
                    Utils.localizeMessage(
                        "user_groups_modal.mentionNotUnique",
                        "Mention needs to be unique.",
                    ),
                );
                setSaving(false);
            } else if (
                data.error?.server_error_id === "app.group.username_conflict"
            ) {
                setMentionInputErrorText(
                    Utils.localizeMessage(
                        "user_groups_modal.mentionUsernameConflict",
    ...
    return (
        ...
        >
            <Modal.Header closeButton={true}>
                ...
                <Modal.Title componentClass="h1" id="updateGroupsModalTitle">
                    <FormattedMessage
                ...
                <div className="user-groups-modal__content">
                    ...
                    <div className="update-buttons-wrapper">
                        ...
                        <SaveButton
                            ...
                            defaultMessage={Utils.localizeMessage(
                                "multiselect.saveDetailsButton",
                                "Save Details",
                            )}
                            savingMessage={Utils.localizeMessage(
                                "multiselect.savingDetailsButton",
                                "Saving...",
                            )}
                        />
                    </div>
