...
class ChannelHeader extends React.PureComponent<Props, State> {
    ...
    getHeaderMarkdownOptions: (
        channelNamesMap: Record<string, any>,
    ) => Record<string, any>;
    getPopoverMarkdownOptions: (
        channelNamesMap: Record<string, any>,
    ) => Record<string, any>;
    ...
    constructor(props: Props) {
        ...
        this.getHeaderMarkdownOptions = memoizeResult(
            (channelNamesMap: Record<string, any>) => ({
                ...headerMarkdownOptions,
                channelNamesMap,
            }),
        );
        this.getPopoverMarkdownOptions = memoizeResult(
            (channelNamesMap: Record<string, any>) => ({
    ...
    showChannelHeaderPopover = (headerText: string) => {
        ...
        if (headerPopoverTextMeasurerRect && headerDescriptionRect) {
            if (
                headerPopoverTextMeasurerRect.width >
                    headerDescriptionRect.width ||
                headerText.match(/\n{2,}/g)
            ) {
                this.setState({
                    showChannelHeaderPopover: true,
                    leftOffset:
                        this.headerDescriptionRef.current?.offsetLeft || 0,
                });
            }
        }
        ...
        const topOffset =
            announcementBarSize * this.props.announcementBarCount + 40;
    ...
    handleFormattedTextClick = (e: MouseEvent<HTMLSpanElement>) =>
    ...
    render() {
        ...
        const ariaLabelChannelHeader = localizeMessage(
        ...
        const archivedIcon = channelIsArchived ? (
        ...
        if (isDirect && !dmUser?.delete_at && !dmUser?.is_bot) {
            ...
            if (
                this.props.isLastActiveEnabled &&
                this.props.lastActivityTimestamp &&
                this.props.timestampUnits
            ) {
                dmHeaderTextStatus = (
                    <span className="header-status__text">
        ...
        const channelFilesIconClass = classNames(
        ...
        const pinnedIconClass = classNames(
            "channel-header__icon channel-header__icon--wide channel-header__icon--left",
            {
        ...
        const pinnedIcon = this.props.pinnedPostsCount ? (
            <>
                ...
                />
                <span id="channelPinnedPostCountText" className="icon__text">
        ...
        if (!isDirect) {
            const membersIconClass = classNames(
                "member-rhs__trigger channel-header__icon channel-header__icon--left channel-header__icon--wide",
                {
                    "channel-header__icon--active":
            ...
            const membersIcon = this.props.memberCount ? (
                <>
                    ...
                    />
                    <span id="channelMemberCountText" className="icon__text">
        ...
        const headerText =
            isDirect && dmUser?.is_bot
                ? dmUser.bot_description
                : channel.header;
        if (headerText) {
            const imageProps = {
                hideUtilities: true,
                ...
                >
                    ...
                    >
                        <Overlay
                            ...
                            rootClose={true}
                            target={
                                this.headerDescriptionRef
                                    .current as React.ReactInstance
                            }
