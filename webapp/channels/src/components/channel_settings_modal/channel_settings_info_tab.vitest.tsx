...
vi.mock('components/admin_console/team_channel_settings/convert_confirm_modal', () => {
    return {
        __esModule: true,
        default: vi.fn().mockImplementation(({show, onCancel, onConfirm, displayName}) => {
            if (!show) {
                return null;
            }
            return (
...
vi.mock('mattermost-redux/selectors/entities/roles', () => ({
    ...
    haveIChannelPermission: vi.fn().mockImplementation((state, teamId, channelId, permission: string) => {
...
vi.mock('selectors/views/textbox', () => ({
    showPreviewOnChannelSettingsHeaderModal: vi.fn().mockReturnValue(false),
    showPreviewOnChannelSettingsPurposeModal: vi.fn().mockReturnValue(false),
}));

vi.mock('actions/views/textbox', () => ({
    setShowPreviewOnChannelSettingsHeaderModal: vi.fn(),
    setShowPreviewOnChannelSettingsPurposeModal: vi.fn(),
}));

// Mock the isChannelAdmin function
vi.mock('mattermost-redux/utils/user_utils', async () => {
    const original = await vi.importActual('mattermost-redux/utils/user_utils');
    return {
        ...original,
        isChannelAdmin: vi.fn().mockReturnValue(false),
    };
});
...
vi.mock('mattermost-redux/selectors/entities/channels', async () => {
    const actual = await vi.importActual('mattermost-redux/selectors/entities/channels') as typeof import('mattermost-redux/selectors/entities/channels');
    return {
        ...actual,
        getChannelMember: vi.fn(() => mockChannelMember),
    };
});

vi.mock('mattermost-redux/selectors/entities/common', async () => {
    const actual = await vi.importActual('mattermost-redux/selectors/entities/common') as typeof import('mattermost-redux/selectors/entities/users');
    return {
        ...actual,
...
describe('ChannelSettingsInfoTab', () => {
    ...
    it('should call patchChannel with updated values when Save is clicked (non-privacy changes)', async () => {
        vi.mocked(channelActions.patchChannel).mockReturnValue({type: 'MOCK_ACTION', data: {}} as any);
        ...
        expect(channelActions.patchChannel).toHaveBeenCalledWith('channel1', {
            ...mockChannel,
    ...
    it('should show error state when save fails', async () => {
        vi.mocked(channelActions.patchChannel).mockReturnValue({type: 'MOCK_ACTION', error: {message: 'Error saving channel'}} as any);
    ...
    it('should convert channel when confirming in ConvertConfirmModal', async () => {
        ...
        vi.mocked(channelActions.updateChannelPrivacy).mockReturnValue({type: 'MOCK_ACTION', data: {}} as any);
        ...
        expect(channelActions.updateChannelPrivacy).toHaveBeenCalledWith('channel1', 'P');
    });
    ...
    it('should not convert channel when canceling in ConvertConfirmModal', async () => {
        ...
        expect(channelActions.updateChannelPrivacy).not.toHaveBeenCalled();
    });

    it('should handle errors when converting channel privacy', async () => {
        mockConvertToPrivatePermission = true;

        vi.mocked(channelActions.updateChannelPrivacy).mockReturnValue({
            type: 'MOCK_ACTION',
