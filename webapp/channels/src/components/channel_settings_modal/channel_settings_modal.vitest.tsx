...
vi.mock('mattermost-redux/selectors/entities/channel_banner', () => ({
    selectChannelBannerEnabled: vi.fn().mockImplementation((state) => {
...
vi.mock('mattermost-redux/selectors/entities/roles', () => ({
    haveIChannelPermission: vi.fn().mockImplementation((state, teamId, channelId, permission) => {
...
vi.mock('selectors/general', () => ({
    isChannelAccessControlEnabled: vi.fn().mockReturnValue(true),
    getBasePath: vi.fn().mockReturnValue(''),
}));

// Mock child components to provide controlled testing interfaces
vi.mock('./channel_settings_info_tab', () => {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const React = require('react');

    return {
        __esModule: true,
        default: function MockChannelSettingsInfoTab({
...
vi.mock('./channel_settings_configuration_tab', () => {
    return {
        __esModule: true,
        default: function MockConfigTab(): JSX.Element {
            return <div data-testid='config-tab'>{'Configuration Tab Content'}</div>;
        },
    };
});

vi.mock('./channel_settings_archive_tab', () => {
    return {
        __esModule: true,
        default: function MockArchiveTab(): JSX.Element {
            return <div data-testid='archive-tab'>{'Archive Tab Content'}</div>;
        },
    };
});

vi.mock('./channel_settings_access_rules_tab', () => {
    return {
        __esModule: true,
        default: function MockAccessRulesTab(): JSX.Element {
...
vi.mock('components/settings_sidebar', () => {
    return {
        __esModule: true,
        default: function MockSettingsSidebar({tabs, activeTab, updateTab}: {tabs: TabType[]; activeTab: string; updateTab: (tab: string) => void}): JSX.Element {
            return (
