...
vi.mock('actions/views/channel', () => ({
    deleteChannel: vi.fn().mockReturnValue({type: 'MOCK_DELETE_ACTION'}),
}));
...
vi.mock('mattermost-redux/selectors/entities/roles', () => ({
    haveITeamPermission: vi.fn().mockReturnValue(true),
    haveIChannelPermission: vi.fn().mockReturnValue(true),
...
describe('ChannelSettingsArchiveTab', () => {
    beforeEach(() => {
        ...
        vi.spyOn(teams, 'getCurrentTeam').mockReturnValue({
            id: 'team1',
            name: 'team-name',
        } as Team);

        const historyPush = vi.fn();
        vi.mocked(browserHistory.getHistory).mockReturnValue({push: historyPush} as any);
    });
    ...
    it('should show confirmation modal when archive button is clicked', async () => {
        ...
        expect(screen.getByRole('heading', {name: /Archive channel\?/i})).toBeInTheDocument();
    });
    ...
    it('should call deleteChannel which handles redirection when archived channel cannot be viewed', async () => {
        ...
        vi.mocked(channelActions.deleteChannel).mockImplementationOnce(() => {
            return () => {
