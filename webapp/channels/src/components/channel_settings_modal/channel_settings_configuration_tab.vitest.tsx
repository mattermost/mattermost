...
describe('ChannelSettingsConfigurationTab', () => {
    ...
    it('should save valid colors in hex format', async () => {
        ...
        expect(channelActions.patchChannel).toHaveBeenCalledWith('channel1', expect.objectContaining({
    ...
    it('only valid colors will make the save changes panel visible', async () => {
        vi.mocked(channelActions.patchChannel).mockReturnValue({type: 'MOCK_ACTION', data: {}} as any);
        vi.mocked(channelActions.patchChannel).mockClear(); // Clear any previous calls
