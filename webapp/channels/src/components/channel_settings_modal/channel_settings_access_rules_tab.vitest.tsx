...
import {act, renderWithContext, screen, waitFor, userEvent} from 'tests/vitest_react_testing_utils';
...
vi.mock('components/admin_console/access_control/editors/table_editor/table_editor', () => {
    return {
        default: vi.fn(() => <div data-testid='table-editor'>{'TableEditor'}</div>),
    };
});

const mockUseChannelAccessControlActions = useChannelAccessControlActions as ReturnType<typeof vi.fn>;
const mockUseChannelSystemPolicies = useChannelSystemPolicies as ReturnType<typeof vi.fn>;
const MockedTableEditor = TableEditor as ReturnType<typeof vi.fn>;
...
describe('components/channel_settings_modal/ChannelSettingsAccessRulesTab', () => {
    const mockActions = {
        ...
        createJob: vi.fn(),
        createAccessControlSyncJob: vi.fn(),
        updateAccessControlPoliciesActive: vi.fn(),
        validateExpressionAgainstRequester: vi.fn(),
    };
    ...
    beforeEach(() => {
        ...
        // Console methods are suppressed in these tests
    ...
    test('should call useChannelAccessControlActions hook', async () => {
        ...
        await waitFor(() => {
            expect(mockUseChannelAccessControlActions).toHaveBeenCalled();
        });
    });
    ...
    test('should call setAreThereUnsavedChanges when expression changes', async () => {
        const setAreThereUnsavedChanges = vi.fn();
