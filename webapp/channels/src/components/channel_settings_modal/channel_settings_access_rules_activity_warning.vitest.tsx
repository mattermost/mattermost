...
vi.mock('hooks/useChannelSystemPolicies');

// Mock TableEditor - mimic the real component behavior
vi.mock('components/admin_console/access_control/editors/table_editor/table_editor', () => {
    const React = require('react');
    return {
        __esModule: true,
        default: vi.fn(() => React.createElement('div', {'data-testid': 'table-editor'}, 'TableEditor')),
    };
});

// Mock SaveChangesPanel - mimic the real component behavior
vi.mock('components/widgets/modals/components/save_changes_panel', () => {
    const React = require('react');
    return {
        __esModule: true,
        default: vi.fn((props) => {
...
vi.mock('./channel_activity_warning_modal', () => {
    return {
        __esModule: true,
        default: function MockChannelActivityWarningModal({
            isOpen,
            onClose,
            onConfirm,
        }: {
...
vi.mock('./channel_access_rules_confirm_modal', () => {
    return {
        __esModule: true,
        default: function MockChannelAccessRulesConfirmModal({
            show,
            onHide,
            onConfirm,
            channelName,
...
const mockUseChannelAccessControlActions = useChannelAccessControlActions as Mock;
const mockUseChannelSystemPolicies = useChannelSystemPolicies as Mock;
const MockedTableEditor = TableEditor as Mock;
...
describe('ChannelSettingsAccessRulesTab - Activity Warning Integration', () => {
    ...
    const mockActions = {
        getAccessControlFields: vi.fn().mockResolvedValue({data: mockUserAttributes}),
        getVisualAST: vi.fn().mockResolvedValue({data: {}}),
        searchUsers: vi.fn().mockResolvedValue({data: {users: [], total: 0}}),
        getChannelPolicy: vi.fn().mockResolvedValue({data: null}),
        saveChannelPolicy: vi.fn().mockResolvedValue({data: {}}),
        deleteChannelPolicy: vi.fn().mockResolvedValue({data: {}}),
        getChannelMembers: vi.fn().mockResolvedValue({data: []}),
        createJob: vi.fn().mockResolvedValue({data: {}}),
        createAccessControlSyncJob: vi.fn().mockResolvedValue({data: {}}),
        updateAccessControlPolicyActive: vi.fn().mockResolvedValue({data: {}}),
        validateExpressionAgainstRequester: vi.fn().mockResolvedValue({data: {requester_matches: true}}),
        savePreferences: vi.fn().mockResolvedValue({data: {}}),
        updateAccessControlPoliciesActive: vi.fn().mockResolvedValue({data: {}}),
    };

    const defaultProps = {
        channel: mockChannel,
        setAreThereUnsavedChanges: vi.fn(),
