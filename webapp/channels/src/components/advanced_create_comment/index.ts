...
function makeMapStateToProps() {
    ...
    return (state: GlobalState, ownProps: OwnProps) => {
        ...
        const isRemoteDraft =
            state.views.drafts.remotes[
                `${StoragePrefixes.COMMENT_DRAFT}${ownProps.rootId}`
            ] || false;
        ...
        const channelMemberCountsByGroup = selectChannelMemberCountsByGroup(
            state,
            ownProps.channelId,
        );
        const groupsWithAllowReference =
            useLDAPGroupMentions || useCustomGroupMentions
                ...
                : null;
        const isFormattingBarHidden = getBool(
            state,
            Constants.Preferences.ADVANCED_TEXT_EDITOR,
            AdvancedTextEditor.COMMENT,
        );
...
function makeOnUpdateCommentDraft(rootId: string, channelId: string) {
    return (draft?: PostDraft, save = false) =>
        updateCommentDraft(
            rootId,
            draft ? { ...draft, channelId } : draft,
            save,
        );
}

function makeUpdateCommentDraftWithRootId(channelId: string) {
    return (rootId: string, draft?: PostDraft, save = false) =>
        updateCommentDraft(
            rootId,
            draft ? { ...draft, channelId } : draft,
            save,
...
function makeMapDispatchToProps() {
    ...
    let updateCommentDraftWithRootId: (
        rootID: string,
        draft: PostDraft,
        save?: boolean,
    ) => void;
    ...
    return (dispatch: Dispatch, ownProps: OwnProps) => {
        if (rootId !== ownProps.rootId) {
            onUpdateCommentDraft = makeOnUpdateCommentDraft(
                ownProps.rootId,
                ownProps.channelId,
            );
        }
        ...
        if (channelId !== ownProps.channelId) {
            updateCommentDraftWithRootId = makeUpdateCommentDraftWithRootId(
                ownProps.channelId,
            );
        }
...
export default connect(makeMapStateToProps, makeMapDispatchToProps, null, {
    forwardRef: true,
