...
describe('components/user_settings/general/UserSettingsGeneral', () => {
    ...
    const requiredProps = {
        ...
        actions: {
            ...
            saveCustomProfileAttribute: vi.fn(),
            getCustomProfileAttributeValues: vi.fn(),
        },
        maxFileSize: 1024,
    ...
    test('should not show position input field when LDAP or SAML position attribute is set', () => {
        ...
        const propsLdap = {
            ...
            ldapPositionAttributeSet: true,
            samlPositionAttributeSet: false,
        };

        const {container: containerLdap, unmount: unmountLdap} = renderWithContext(<UserSettingsGeneral {...propsLdap}/>);
        expect(containerLdap.querySelector('#position')).not.toBeInTheDocument();
        unmountLdap();
        ...
        const {container: containerSaml} = renderWithContext(<UserSettingsGeneral {...propsSaml}/>);
        expect(containerSaml.querySelector('#position')).not.toBeInTheDocument();
    });
    ...
    test('submitAttribute() should have called saveCustomProfileAttribute', async () => {
        const saveCustomProfileAttribute = vi.fn().mockResolvedValue({field1: 'Updated Value'});
        const props = {
            ...requiredProps,
            enableCustomProfileAttributes: true,
    ...
    test('submitAttribute() should handle server error', async () => {
        const saveCustomProfileAttribute = vi.fn().mockResolvedValue({error: {message: 'Server Error'}});
        const props = {
            ...requiredProps,
            enableCustomProfileAttributes: true,
            actions: {...requiredProps.actions, saveCustomProfileAttribute},
    ...
    test('updateSelectAttribute() should handle single select changes', async () => {
        const saveCustomProfileAttribute = vi.fn().mockResolvedValue({});
        const selectAttribute: UserPropertyField = {
            ...customProfileAttribute,
            type: 'select',
            attrs: {
    ...
    test('should validate email custom attribute field value', async () => {
        ...
        // Type an invalid email value
        await userEvent.type(input, 'invalid-email');

        // Try to save - should be prevented by validation in submitAttribute
        await userEvent.click(saveButton);

        // Save should not have been called due to invalid email
        expect(saveCustomProfileAttribute).not.toHaveBeenCalled();

        // Now clear and enter a valid email
        await userEvent.clear(input);
        await userEvent.type(input, 'test@example.com');
        await userEvent.click(saveButton);

        // Now save should be called with valid email
    ...
    test('submitUser() should have called updateMe', async () => {
        const updateMe = vi.fn().mockResolvedValue({data: true});
        const props = {
            ...requiredProps,
    ...
    test('submitPicture() should not have called uploadProfileImage', async () => {
        const uploadProfileImage = vi.fn().mockResolvedValue({});
        const props = {
            ...requiredProps,
            actions: {...requiredProps.actions, uploadProfileImage},
            activeSection: 'picture',
        };

        renderWithContext(<UserSettingsGeneral {...props}/>);

        // Without selecting a file, upload should not be called on render
        expect(uploadProfileImage).not.toHaveBeenCalled();
    });

    test('submitPicture() should have called uploadProfileImage', async () => {
        const uploadProfileImage = vi.fn().mockResolvedValue({data: true});
        const props = {
            ...requiredProps,
            actions: {...requiredProps.actions, uploadProfileImage},
            activeSection: 'picture',
        ...
        await userEvent.upload(fileInput as HTMLInputElement, mockFile);
        ...
        await waitFor(() => {
            expect(uploadProfileImage).toHaveBeenCalledWith(user.id, mockFile);
        });
    });
    ...
    test('it should display an error about a username conflicting with a group name', async () => {
        const updateMe = vi.fn().mockResolvedValue({
            data: false,
            error: {server_error_id: 'app.user.group_name_conflict', message: ''},
        });
        const props = {
            ...requiredProps,
            actions: {...requiredProps.actions, updateMe},
        ...
        await waitFor(() => {
            expect(screen.getByText('This username conflicts with an existing group name.')).toBeInTheDocument();
        });
    });
});
