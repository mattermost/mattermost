...
describe("components/user_settings/general/UserSettingsGeneral", () => {
    ...
    test("submitUser() should have called updateMe", () => {
        ...
        (wrapper.instance() as UserSettingsGeneralTab).submitUser(
            requiredProps.user,
            false,
        );
    ...
    test("submitPicture() should have called uploadProfileImage", async () => {
        ...
        const mockFile = {
            type: "image/jpeg",
            size: requiredProps.maxFileSize,
        };
        ...
        expect(
            (wrapper.instance() as UserSettingsGeneralTab).submitActive,
        ).toBe(true);
    ...
    test("it should display an error about a username conflicting with a group name", async () => {
        const updateMe = () =>
            Promise.resolve({
                data: false,
                error: {
                    server_error_id: "app.user.group_name_conflict",
                    message: "",
                },
            });
        const props = {
        ...
        const wrapper = shallowWithIntl(<UserSettingsGeneral {...props} />);
        await (wrapper.instance() as UserSettingsGeneralTab).submitUser(
            requiredProps.user,
            false,
        );
        expect(wrapper.state("serverError")).toBe(
            "This username conflicts with an existing group name.",
        );
    });
});
