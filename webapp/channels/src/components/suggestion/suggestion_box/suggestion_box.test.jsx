...
jest.mock("mattermost-redux/client", () => {
    ...
    return {
        ...
        Client4: {
            ...actual.Client4,
            getCommandAutocompleteSuggestionsList: jest
                .fn()
                .mockResolvedValue([]),
        },
    };
...
describe("components/SuggestionBox", () => {
    ...
    test("should reset selection after provider.handlePretextChanged is handled", () => {
        ...
        ).mockImplementation(() => [userid1, userid2, userid3]);

        const props = {
            ...baseProps,
            providers: [provider],
        };
        const wrapper = shallow(<SuggestionBox {...props} />);
        const instance = wrapper.instance();
    ...
    test("Test for suggestionBoxAlgn when slash command at beginning and when slash command in middle of text", () => {
        ...
        Utils.getSuggestionBoxAlgn = jest
            .fn()
            .mockReturnValue({ pixelsToMoveX: 0, pixelsToMoveY: 35 });
        ...
        instance.nonDebouncedPretextChanged(
            "I should still have a empty suggestionBoxAlgn /",
        );
        expect(wrapper.state("suggestionBoxAlgn")).toEqual({});
    });
