...
function itemToTerm(
    isAtSearch: boolean,
    item: { type: string; display_name: string; name: string },
) {
    ...
    if (
        item.type === Constants.OPEN_CHANNEL ||
        item.type === Constants.PRIVATE_CHANNEL
    ) {
        return item.name;
    }
    return item.name;
...
export default class SearchChannelProvider extends Provider {
    ...
    handlePretextChanged(
        pretext: string,
        resultsCallback: ResultsCallback<Channel>,
    ) {
            ...
            this.autocompleteChannelsForSearch(
                ...
                (data: Channel[]) => {
                    ...
                    channels = channels.sort(
                        sortChannelsByTypeListAndDisplayName.bind(
                            null,
                            locale,
                            [
                    ...
                    const channelNames = channels.map(
                        itemToTerm.bind(null, isAtSearch),
                    );
