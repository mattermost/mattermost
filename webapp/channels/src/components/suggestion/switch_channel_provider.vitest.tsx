...
vi.mock('mattermost-redux/client', async () => {
    const original = await vi.importActual('mattermost-redux/client');

    return {
        ...original,
        Client4: {
            ...(original as any).Client4,
            autocompleteUsers: vi.fn().mockResolvedValue([]),
        },
    };
});

vi.mock('mattermost-redux/actions/channels', async () => {
    const original = await vi.importActual('mattermost-redux/actions/channels');
    return {
        ...original,
        searchAllChannels: () => vi.fn().mockResolvedValue(Promise.resolve({
            data: [{
