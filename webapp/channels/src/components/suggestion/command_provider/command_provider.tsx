...
const EXECUTE_CURRENT_COMMAND_ITEM_ID =
    Constants.Integrations.EXECUTE_CURRENT_COMMAND_ITEM_ID;
const OPEN_COMMAND_IN_MODAL_ITEM_ID =
    Constants.Integrations.OPEN_COMMAND_IN_MODAL_ITEM_ID;
const COMMAND_SUGGESTION_ERROR =
    Constants.Integrations.COMMAND_SUGGESTION_ERROR;

const CommandSuggestion = React.forwardRef<
    HTMLDivElement,
    SuggestionProps<AutocompleteSuggestion>
>((props, ref) => {
    const { item } = props;
    ...
    return (
        <SuggestionContainer ref={ref} {...props}>
            ...
            <div className="slash-command__info">
                ...
                <div className="slash-command__desc">{item.Description}</div>
            </div>
        </SuggestionContainer>
    );
});
...
export default class CommandProvider extends Provider {
    ...
    setProps(props: Props) {
        this.props = props;
        this.appCommandParser.setChannelContext(
            props.channelId,
            props.teamId,
            props.rootId,
        );
    ...
    handlePretextChanged(
        pretext: string,
        resultCallback: ResultsCallback<AutocompleteSuggestion>,
    ) {
        ...
        if (
            appsEnabled(this.store.getState()) &&
            this.appCommandParser.isAppCommand(pretext)
        ) {
            this.appCommandParser
                .getSuggestions(pretext)
                .then((suggestions) => {
                    ...
                    const terms = matches.map(
                        (suggestion) => suggestion.Complete,
                    );
                    resultCallback({
    ...
    handleMobile(
        ...
        Client4.getCommandsList(teamId).then((data) => {
            let matches: AutocompleteSuggestion[] = [];
            if (appsEnabled(this.store.getState())) {
                const appCommandSuggestions =
                    this.appCommandParser.getSuggestionsBase(pretext);
            ...
            data.forEach((cmd) => {
                ...
                if (
                    (this.triggerCharacter + cmd.trigger).indexOf(command) === 0
                ) {
    ...
    handleWebapp(
        ...
        Client4.getCommandAutocompleteSuggestionsList(
            command,
            teamId,
            args,
            ...
            if (appsEnabled(this.store.getState())) {
                const appCommandSuggestions = this.appCommandParser
            ...
            data.forEach((s) => {
                if (
                    !this.contains(matches, this.triggerCharacter + s.Complete)
                ) {
                    matches.push({
            ...
            if (!pretext.includes(" ")) {
                matches.sort((a, b) => {
                    if (
                        a.Suggestion.toLowerCase() > b.Suggestion.toLowerCase()
                    ) {
                        return 1;
                    } else if (
                        a.Suggestion.toLowerCase() < b.Suggestion.toLowerCase()
