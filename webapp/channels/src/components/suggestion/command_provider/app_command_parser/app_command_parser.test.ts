...
import { AppCallResponseTypes } from "./app_command_parser_dependencies";
import type { AutocompleteSuggestion } from "./app_command_parser_dependencies";
import {
    reduxTestState,
    testBindings,
...
import type { AppBinding } from "./tests/app_command_parser_test_dependencies";
...
describe("AppCommandParser", () => {
        ...
        const table: TC[] = [
            {
                title: "full command",
                command:
                    '/jira issue create --project P  --summary = "SUM MA RY" --verbose --epic=epic2',
                submit: {
                    verify: (parsed: ParsedCommand): void => {
        ...
        const table: TC[] = [
            {
                ...
                command:
                    '/jira issue create --project `P 1`  --summary "SUM MA RY" --verbose --epic=epic2',
                autocomplete: {
                    verify: (parsed: ParsedCommand): void => {
                        expect(parsed.state).toBe(ParseState.EndValue);
                        expect(parsed.binding?.label).toBe("create");
                        expect(parsed.resolvedForm?.submit?.path).toBe(
                            "/create-issue",
                        );
            ...
            },
            {
                title: "partial epic",
                command:
                    '/jira issue create --project KT --summary "great feature" --epic M',
                autocomplete: {
                    verify: (parsed: ParsedCommand): void => {
                        expect(parsed.state).toBe(ParseState.EndValue);
                        expect(parsed.binding?.label).toBe("create");
            ...
            },
            {
                ...
                submit: {
                    expectError: "Command does not accept flag `{flagName}`.",
                },
            },
            {
            ...
            },
            {
                ...
                command:
                    '/jira issue rest --summary "hello world" --verbose true',
                autocomplete: {
                    verify: (parsed: ParsedCommand): void => {
                        expect(parsed.state).toBe(ParseState.EndValue);
                        expect(parsed.binding?.label).toBe("rest");
            ...
            },
            {
                ...
                command:
                    '/jira issue rest --summary "hello world" --verbose true hello world',
                submit: { expectError: "Unable to identify argument." },
            },
        ];
    ...
    describe("getSuggestions", () => {
        ...
        test("view just after subcommand (positional)", async () => {
            const suggestions =
                await parser.getSuggestions("/jira issue view ");
            expect(suggestions).toEqual([
                {
                    Complete: "jira issue view",
        ...
        test("create flags just after subcommand", async () => {
            const suggestions = await parser.getSuggestions(
                "/jira issue create ",
            );
                ...
                executeCommand = [
                    {
                        ...
                        Description:
                            "Select this option or use Ctrl+Enter to execute the current command.",
                        Hint: "",
        ...
        test("used flags do not appear", async () => {
            ...
            if (checkForExecuteSuggestion) {
                executeCommand = [
                    {
                        Complete:
                            "jira issue create --project KT _execute_current_command",
                        Description:
                            "Select this option or use Ctrl+Enter to execute the current command.",
                        Hint: "",
                        IconData: "_execute_current_command",
        ...
        test("create flags mid-flag", async () => {
            const mid = await parser.getSuggestions(
                "/jira issue create --project KT --summ",
            );
            expect(mid).toEqual([
                {
                    Complete: "jira issue create --project KT --summary",
            ...
            const full = await parser.getSuggestions(
                "/jira issue create --project KT --summary",
            );
            expect(full).toEqual([
                {
                    Complete: "jira issue create --project KT --summary",
        ...
        test("partial text value suggestion", async () => {
            const suggestions = await parser.getSuggestions(
                "/jira issue create --project KT --summary Sum",
            );
            expect(suggestions).toEqual([
                {
        ...
        test("create flag epic static select value", async () => {
            let suggestions = await parser.getSuggestions(
                '/jira issue create --project KT --summary "great feature" --epic ',
            );
            expect(suggestions).toEqual([
                {
                    Complete:
                        'jira issue create --project KT --summary "great feature" --epic epic1',
                ...
                {
                    Complete:
                        'jira issue create --project KT --summary "great feature" --epic epic2',
            ...
            suggestions = await parser.getSuggestions(
                '/jira issue create --project KT --summary "great feature" --epic M',
            );
            expect(suggestions).toEqual([
                {
            ...
            suggestions = await parser.getSuggestions(
                '/jira issue create --project KT --summary "great feature" --epic Nope',
            );
            expect(suggestions).toEqual([
                {
                    Complete:
        ...
        test("filled out form shows execute", async () => {
            const suggestions = await parser.getSuggestions(
                '/jira issue create --project KT --summary "great feature" --epic epicvalue --verbose true ',
            );
            ...
            expect(suggestions).toEqual([
                {
                    Complete:
                        'jira issue create --project KT --summary "great feature" --epic epicvalue --verbose true _execute_current_command',
                    Suggestion: "Execute Current Command",
                    Description:
                        "Select this option or use Ctrl+Enter to execute the current command.",
                    IconData: "_execute_current_command",
    ...
    describe("composeCommandSubmitCall", () => {
        ...
        test("full form", async () => {
            const cmd =
                '/jira issue create --summary "Here it is" --epic epic1 --verbose true --project';
            const values = {
                summary: "Here it is",
                epic: {
                    label: "Dylan Epic",
        ...
        test("dynamic lookup test", async () => {
            ...
            const suggestions = await parser.getSuggestions(
                '/jira issue create --summary "The summary" --epic epic1 --project special',
            );
            Client4.executeAppCall = f;
            ...
            expect(suggestions).toEqual([
                {
                    Complete:
                        'jira issue create --summary "The summary" --epic epic1 --project special-value',
                    Suggestion: "special-value",
                    Description: "special-label",
                    Hint: "",
                    IconData: "Create icon",
