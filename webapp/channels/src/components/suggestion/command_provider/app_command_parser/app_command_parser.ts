...
interface FormsCache {
    getSubmittableForm: (
        location: string,
        binding: AppBinding,
    ) => Promise<{ form?: AppForm; error?: string } | undefined>;
}
...
const getRHSCommandBindings = makeRHSAppBindingSelector(
...
export class ParsedCommand {
    ...
    public matchBinding = async (
                ...
                this.intl.formatMessage(
                    {
                        ...
                        defaultMessage:
                            "`{command}`: No matching command found in this workspace.",
                    },
                    {
        ...
        if (!this.binding.bindings?.length) {
            ...
            if (this.binding.submit && !this.binding.form) {
                ...
                if (!form.submit) {
                    const fetched = await this.formsCache.getSubmittableForm(
                        this.location,
                        this.binding,
                    );
                    if (fetched?.error) {
    ...
    public parseForm = (autocompleteMode = false): ParsedCommand => {
        ...
        fields = fields.filter(
            (f) => f.type !== AppFieldTypes.MARKDOWN && !f.readonly,
        );
        ...
        while (true) {
            ...
            switch (this.state) {
                    ...
                    switch (c) {
                        ...
                        default: {
                            ...
                            let field = fields.find(
                                (f: AppField) => f.position === this.position,
                            );
                            if (!field) {
                                field = fields.find(
                                    (f) =>
                            ...
                            if (!autocompleteMode) {
                                return this.asError(
                                    this.intl.formatMessage({
                                        ...
                                        defaultMessage:
                                            "Matching double quote expected before end of input.",
                                    }),
                                );
                            }
                            return this;
                            ...
                            if (!autocompleteMode) {
                                return this.asError(
                                    this.intl.formatMessage({
                                        ...
                                        defaultMessage:
                                            "Matching tick quote expected before end of input.",
                                    }),
                                );
                            }
                            return this;
                ...
                case ParseState.EndValue: {
                    ...
                    // --next-flag...')
                    if (
                        this.field.type === AppFieldTypes.BOOL &&
                        ((autocompleteMode &&
                            !"true".startsWith(this.incomplete) &&
                            !"false".startsWith(this.incomplete)) ||
                            (!autocompleteMode &&
                ...
                case ParseState.MultiselectEndValue: {
                    ...
                    (this.values![this.field.name] as string[]).push(
                        this.incomplete,
                    );
                    this.incomplete = "";
                    this.incompleteStart = this.i;
...
export class AppCommandParser {
    ...
    public composeCommandSubmitCall = async (
        command: string,
    ): Promise<{ creq: AppCallRequest | null; errorMessage?: string }> => {
        ...
        const { creq, errorMessage } = await this.composeCallRequest(
            parsed,
            parsed.resolvedForm?.submit,
        );
        if (errorMessage) {
    ...
    public composeFormFromCommand = async (
        command: string,
    ): Promise<{
        form: AppForm | null;
    ...
    private async addDefaultAndReadOnlyValues(parsed: ParsedCommand) {
        ...
        await Promise.all(
            parsed.resolvedForm?.fields.map(async (f) => {
                if (!f.value) {
                    return;
                }
                ...
                if (f.readonly || !(f.name in parsed.values)) {
                    switch (f.type) {
                        ...
                        case AppFieldTypes.CHANNEL: {
                            const channelID = (f.value as AppSelectOption)
                                .label;
                        ...
                        case AppFieldTypes.DYNAMIC_SELECT:
                            parsed.values[f.name] = (
                                f.value as AppSelectOption
                            ).value;
                            break;
    ...
    public getSuggestions = async (
        pretext: string,
    ): Promise<ExtendedAutocompleteSuggestion[]> => {
        ...
        const call =
            parsed.resolvedForm?.submit || parsed.binding?.form?.submit;
        const hasRequired = this.getMissingFields(parsed).length === 0;
        const hasValue =
            parsed.state !== ParseState.EndValue ||
            (parsed.field && parsed.values[parsed.field.name] !== undefined);

        if (
            executableStates.includes(parsed.state) &&
            call &&
            hasRequired &&
            hasValue
        ) {
            ...
            parsed.field?.type !== AppFieldTypes.USER &&
            parsed.field?.type !== AppFieldTypes.CHANNEL
        ) {
            suggestions = this.getNoMatchingSuggestion();
        }

        if (
            modalStates.includes(parsed.state) &&
            call &&
            parsed.resolvedForm?.fields?.length
        ) {
        ...
        return suggestions.map((suggestion) =>
            this.decorateSuggestionComplete(parsed, suggestion),
        );
    };
    ...
    private composeCallRequest = async (
        parsed: ParsedCommand,
        call: AppCall | undefined,
    ): Promise<{ creq: AppCallRequest | null; errorMessage?: string }> => {
        if (!parsed.binding) {
            return {
                creq: null,
        ...
        const context = this.getAppContext(parsed.binding);
        return {
            creq: createCallRequest(call, context, {}, values, parsed.command),
        };
    };

    private expandOptions = async (
        parsed: ParsedCommand,
                ...
                switch (f.type) {
                                    ...
                                    errors[f.name] = this.intl.formatMessage(
                                        {
                                            ...
                                            defaultMessage:
                                                "Option repeated for field `{fieldName}`: `{option}`.",
                                        },
                                        {
                                            fieldName: f.name,
                                            option: value,
                    ...
                    case AppFieldTypes.USER: {
                        const getUser = async (userName: string) => {
                            ...
                            if (!user) {
                                const dispatchResult =
                                    await this.store.dispatch(
                                        getUserByUsername(userName) as any,
                                    );
                                if ("error" in dispatchResult) {
                                    return null;
                                }
                                    ...
                                    errors[f.name] = this.intl.formatMessage(
                                        {
                                            ...
                                            defaultMessage:
                                                "User repeated for field `{fieldName}`: `{option}`.",
                                        },
                                        {
                                            fieldName: f.name,
                                            option: userName,
                    ...
                    case AppFieldTypes.CHANNEL: {
                        const getChannel = async (channelName: string) => {
                            let channel = selectChannelByName(
                                this.store.getState(),
                                channelName,
                            );
                            if (!channel) {
                        ...
                        if (f.multiselect && Array.isArray(values[f.name])) {
                            ...
                            for (const value of commandValues) {
                                ...
                                if (
                                    options.find((o) => o.value === channel?.id)
                                ) {
                                    errors[f.name] = this.intl.formatMessage(
                                        {
                                            id: "apps.error.command.same_channel",
                                            defaultMessage:
                                                "Channel repeated for field `{fieldName}`: `{option}`.",
                                        },
                                        {
                                            fieldName: f.name,
                                            option: channelName,
    ...
    private decorateSuggestionComplete = (
        parsed: ParsedCommand,
        choice: AutocompleteSuggestion,
    ): AutocompleteSuggestion => {
        if (
            choice.Complete &&
        ...
        let complete = parsed.command.substring(
            0,
            parsed.incompleteStart - goBackSpace,
        );
        complete +=
            choice.Complete === undefined ? choice.Suggestion : choice.Complete;
    ...
    public getSubmittableForm = async (
        location: string,
        binding: AppBinding,
    ): Promise<{ form?: AppForm; error?: string } | undefined> => {
        const rootID = this.rootPostID || "";
        const key = `${this.channelID}-${rootID}-${location}`;
        const submittableForm = this.rootPostID
            ? getAppRHSCommandForm(this.store.getState(), key)
            : getAppCommandForm(this.store.getState(), key);
        if (submittableForm) {
        ...
        const fetched = await this.fetchSubmittableForm(
            binding.form.source,
            context,
        );
        if (fetched?.form) {
    ...
    private getCommandSuggestions = (
        ...
        bindings.forEach((b) => {
            if (
                b.label
                    .toLowerCase()
                    .startsWith(parsed.incomplete.toLowerCase())
            ) {
                result.push({
                    Complete: b.label,
                    Suggestion: b.label,
    ...
    private getParameterSuggestions = async (
        parsed: ParsedCommand,
    ): Promise<ExtendedAutocompleteSuggestion[]> => {
        switch (parsed.state) {
            case ParseState.StartParameter: {
                // see if there's a matching positional field
                const positional = parsed.resolvedForm?.fields?.find(
                    (f: AppField) => f.position === parsed.position + 1,
                );
                if (positional) {
    ...
    private getMultiselectValueSeparatorSuggestion =
        (): AutocompleteSuggestion[] => {
            return [
                {
                    Complete: ",",
    ...
    private getFlagNameSuggestions = (
        parsed: ParsedCommand,
    ): AutocompleteSuggestion[] => {
        ...
        const applicable = parsed.resolvedForm.fields.filter(
            (field) =>
                field.label &&
                field.label
                    .toLowerCase()
    ...
    private getValueSuggestions = async (
        parsed: ParsedCommand,
        delimiter?: string,
    ): Promise<ExtendedAutocompleteSuggestion[]> => {
        ...
        const mentionSuggestions = await inTextMentionSuggestions(
            parsed.incomplete,
            this.store,
            this.channelID,
            this.teamID,
        ...
        const fieldName =
            parsed.field.modal_label || parsed.field.label || parsed.field.name;
        return [
            {
                Complete: complete,
                Suggestion: `${fieldName}: ${delimiter || '"'}${
                    parsed.incomplete
                }${delimiter || '"'}`,
                Description: f.description || "",
                Hint: "",
    ...
    private getStaticSelectSuggestions = (
        parsed: ParsedCommand,
        delimiter?: string,
    ): AutocompleteSuggestion[] => {
        const f = parsed.field as AutocompleteStaticSelect;
        const opts = f.options?.filter((opt) =>
            opt.label.toLowerCase().startsWith(parsed.incomplete.toLowerCase()),
        );
        if (!opts?.length) {
            return [
                {
    ...
    private getDynamicSelectSuggestions = async (
        parsed: ParsedCommand,
        delimiter?: string,
        ...
        if (!f) {
            // Should never happen
            return this.makeDynamicSelectSuggestionError(
                this.intl.formatMessage({
                ...
                this.intl.formatMessage(
                    {
                        ...
                        defaultMessage:
                            "Error preparing lookup request: {errorMessage}",
                    },
                    {
                        errorMessage,
                    },
        ...
        const res = (await this.store.dispatch(
            doAppLookup(creq, this.intl),
        )) as DoAppCallResult<AppLookupResponse>;
    ...
    private makeDynamicSelectSuggestionError = (
        message: string,
    ): AutocompleteSuggestion[] => {
    ...
    private getUserFieldSuggestions = async (
        parsed: ParsedCommand,
    ): Promise<AutocompleteSuggestion[]> => {
        ...
        const { data } = await this.store.dispatch(
            autocompleteUsersInChannel(input, this.channelID),
        );
        return getUserSuggestions(data);
    };

    private getChannelFieldSuggestions = async (
        parsed: ParsedCommand,
    ): Promise<AutocompleteSuggestion[]> => {
        let input = parsed.incomplete.trim();
        if (input[0] === "~") {
        ...
        const { data } = await this.store.dispatch(
            autocompleteChannels(this.teamID, input),
        );
