...
describe('CommandSuggestion', () => {
    ...
    test('should match snapshot', () => {
        ...
        expect(screen.getByText('invite @[username] ~[channel]')).toBeInTheDocument();
    });
});
...
test('should forward pretext to handleWebapp unaltered (case-preserving)', () => {
    const uaSpy = vi.spyOn(UserAgent, 'isMobile').mockReturnValue(false);
    ...
    const webappSpy = vi.spyOn(provider as any, 'handleWebapp').mockImplementation(() => true);
...
test('handleWebapp keeps matchedPretext as original (UI case-preserving)', async () => {
    const original = Client4.getCommandAutocompleteSuggestionsList;
    Client4.getCommandAutocompleteSuggestionsList = vi.fn().mockResolvedValue([]);
