...
export default class AppCommandProvider extends Provider {
    ...
    handlePretextChanged(
        pretext: string,
        resultCallback: ResultsCallback<Item>,
    ) {
    ...
    public async openAppsModalFromCommand(pretext: string) {
        const { form, context } =
            await this.appCommandParser.composeFormFromCommand(pretext);
        if (!form || !context) {
            return;
        }
