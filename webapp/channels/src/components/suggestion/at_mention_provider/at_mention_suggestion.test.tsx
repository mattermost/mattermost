...
describe("at mention suggestion", () => {
    ...
    it("Should not display nick name of the signed in user", () => {
        const wrapper = mountWithIntl(
            <AtMentionSuggestion {...baseProps} item={userid1} />,
        );

        expect(wrapper).toMatchSnapshot();

        expect(wrapper.find(".suggestion-list__ellipsis").text()).toContain(
            "a b",
        );
        expect(wrapper.find(".suggestion-list__ellipsis").text()).not.toContain(
            "a b (c)",
        );
    });
    ...
    it("Should display nick name of non signed in user", () => {
        const wrapper = mountWithIntl(
            <AtMentionSuggestion {...baseProps} item={userid2} />,
        );
