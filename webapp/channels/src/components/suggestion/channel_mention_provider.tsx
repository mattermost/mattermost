...
export const ChannelMentionSuggestion = React.forwardRef<
    HTMLDivElement,
    SuggestionProps<WrappedChannel>
>((props, ref) => {
    const { item } = props;
    const channelIsArchived =
        item.channel && item.channel.delete_at && item.channel.delete_at !== 0;
    ...
    if (channelIsArchived) {
        ...
        channelIcon = (
            <span className="suggestion-list__icon suggestion-list__icon--large">
                <i
                    className={`icon icon--no-spacing icon-${
                        item.channel?.type === Constants.OPEN_CHANNEL
                            ? "globe"
                            : "lock-outline"
                    }`}
                />
    ...
    return (
        <SuggestionContainer ref={ref} {...props}>
            ...
            <div className="suggestion-list__ellipsis">
                <span className="suggestion-list__main">{channelName}</span>
                {description}
            </div>
        </SuggestionContainer>
    );
...
export default class ChannelMentionProvider extends Provider {
    ...
    handlePretextChanged(
        pretext: string,
        resultCallback: ResultsCallback<WrappedChannel>,
    ) {
        this.resetRequest();
        ...
        if (
            this.delayChannelAutocomplete &&
            prefix.length < MIN_CHANNEL_LINK_LENGTH
        ) {
            return false;
        }
        ...
        const handleChannels = (channels: Channel[], withError: boolean) => {
            if (
                prefix !== this.latestPrefix ||
                this.shouldCancelDispatch(prefix)
            ) {
                return;
            }
            ...
            channels.forEach((item) => {
                ...
                if (!myMembers[item.id] && wrappedChannelIds[item.id]) {
                    ...
                    const idx = wrappedChannels
                        .map((el) => el.channel?.id)
                        .indexOf(item.id);
                    if (idx >= 0) {
