...
): { icon: React.ReactElement; name: string; description: string } | null {
    if (item.type === Constants.DM_CHANNEL) {
        const profilePicture = (
            <Avatar
                url={imageURLForUser(
                    getUserIdFromChannelName(currentUserId, item.name),
                )}
                size="sm"
            />
        );
...
const SearchChannelSuggestion = React.forwardRef<HTMLDivElement, Props>(
    (props, ref) => {
        ...
        const tag =
            item.type === Constants.DM_CHANNEL && teammateIsBot ? (
                <BotTag />
            ) : null;
