...
type ChannelSearchFunction = (
    teamId: string,
    channelPrefix: string,
) => Promise<ActionResult>;

const SearchChannelWithPermissionsSuggestion = React.forwardRef<
    HTMLDivElement,
...
>((props, ref) => {
    ...
    if (channelIsArchived) {
        icon = <i className="icon icon--no-spacing icon-archive-outline" />;
    } else if (channel.type === Constants.OPEN_CHANNEL) {
        icon = <i className="icon icon--no-spacing icon-globe" />;
    } else if (channel.type === Constants.PRIVATE_CHANNEL) {
        icon = <i className="icon icon--no-spacing icon-lock-outline" />;
    }
...
SearchChannelWithPermissionsSuggestion.displayName =
    "SearchChannelWithPermissionsSuggestion";

let prefix = "";

function channelSearchSorter(
...
export default class SearchChannelWithPermissionsProvider extends Provider {
    ...
    async fetchChannels(
        ...
        const channelsAsync = this.autocompleteChannelsForSearch(
            teamId,
            channelPrefix,
        );
        ...
        const channels =
            getChannelsInCurrentTeam(state).concat(channelsFromServer);
        this.formatChannelsAndDispatch(
            channelPrefix,
            resultsCallback,
            channels,
