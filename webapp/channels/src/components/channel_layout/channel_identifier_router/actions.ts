...
const USER_ID_PAIR_REGEXP = new RegExp(
    `^[a-zA-Z0-9]{${LENGTH_OF_ID}}__[a-zA-Z0-9]{${LENGTH_OF_ID}}$`,
);

export function onChannelByIdentifierEnter({
    match,
    history,
}: MatchAndHistory): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        const channelPath = await getPathFromIdentifier(
            state,
            path,
            identifier,
        );
...
export async function getPathFromIdentifier(
    state: GlobalState,
    path: string,
    identifier: string,
) {
    if (path === "channels") {
        ...
        if (identifier.length === LENGTH_OF_ID) {
            ...
            (moreChannelsByName &&
                moreChannelsByName.type === Constants.GM_CHANNEL)
        ) {
...
export function goToChannelByChannelId(
    match: Match,
    history: History,
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
...
export function goToChannelByChannelName(
    match: Match,
    history: History,
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (!channel) {
            const getChannelDispatchResult = await dispatch(
        ...
        if (channel) {
            ...
            if (!member) {
                const membership = await dispatch(
                    getChannelMember(channel.id, getCurrentUserId(state)),
                );
        ...
        if (!channel || !member) {
            ...
            if ("error" in joinChannelDispatchResult) {
                if (!channel) {
                    ...
                    if (
                        "error" in getChannelDispatchResult ||
                        getChannelDispatchResult.data.delete_at === 0
                    ) {
                        await dispatch(fetchChannelsAndMembers(teamObj!.id));
                        handleChannelJoinError(
                            match,
...
function goToDirectChannelByUsername(
    match: Match,
    history: History,
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
...
export function goToDirectChannelByUserId(
    match: Match,
    history: History,
    userId: string,
): ActionFunc {
...
export function goToDirectChannelByUserIds(
    match: Match,
    history: History,
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
...
export function goToDirectChannelByEmail(
    match: Match,
    history: History,
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
...
function goToGroupChannelByGroupId(match: Match, history: History): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (!channel) {
            const dispatchResult = await dispatch(
                joinChannel(getCurrentUserId(state), teamObj!.id, "", groupId),
            );
            if ("error" in dispatchResult) {
                await dispatch(fetchChannelsAndMembers(teamObj!.id));
                handleError(
...
function isDirectChannelIdentifier(identifier: string) {
    return (
        ...
        USER_ID_PAIR_REGEXP.test(identifier)
    );
}

async function handleChannelJoinError(
    match: Match,
    history: History,
