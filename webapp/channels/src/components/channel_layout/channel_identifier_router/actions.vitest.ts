...
vi.mock('actions/global_actions', () => ({
    emitChannelClickEvent: vi.fn(),
}));

vi.mock('mattermost-redux/actions/channels', () => ({
    joinChannel: vi.fn(() => ({type: '', data: {channel: {id: 'channel_id3', name: 'achannel3', team_id: 'team_id1', type: 'O'}}})),
    getChannelByNameAndTeamName: vi.fn(() => ({type: '', data: {id: 'channel_id3', name: 'achannel3', team_id: 'team_id1', type: 'O'}})),
    getChannelMember: vi.fn(() => ({type: '', error: {}})),
}));

vi.mock('mattermost-redux/actions/users', () => ({
    getUserByEmail: vi.fn(() => ({type: '', data: {id: 'user_id3', email: 'user3@bladekick.com', username: 'user3'}})),
    getUser: vi.fn(() => ({type: '', data: {id: 'user_id3', email: 'user3@bladekick.com', username: 'user3'}})),
}));

vi.mock('utils/channel_utils', () => ({
    joinPrivateChannelPrompt: vi.fn(() => {
        return async () => {
...
describe('Actions', () => {
    ...
    describe('goToChannelByChannelName', () => {
        ...
        test('switch to public channel we don\'t have locally and need to join', async () => {
            ...
            (joinChannel as Mock).mockReturnValueOnce({type: '', data: {channel}});
            (getChannelByNameAndTeamName as Mock).mockReturnValueOnce({type: '', data: channel});
