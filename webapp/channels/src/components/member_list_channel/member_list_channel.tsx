...
export type Props = {
    ...
    actions: {
        ...
        getChannelMembers: (
            channelId: string,
        ) => Promise<{ data: ChannelMembership[] }>;
...
> {
    ...
    async componentDidMount() {
        ...
        await Promise.all([
            actions.loadProfilesAndTeamMembersAndChannelMembers(
                0,
    ...
    componentDidUpdate(prevProps: Props) {
        if (prevProps.searchTerm !== this.props.searchTerm) {
            ...
            const searchTimeoutId = window.setTimeout(async () => {
                const { data } = await prevProps.actions.searchProfiles(
                    searchTerm,
                    {
                        team_id: this.props.currentTeamId,
                        in_channel_id: this.props.currentChannelId,
                    },
                );
