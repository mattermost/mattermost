...
const EmailToLDAP = ({ email, siteName, ldapLoginFieldName }: Props) => {
    ...
    const preSubmit = (e: React.FormEvent) => {
        ...
        if (!password) {
            setPasswordError(
                formatMessage({
                    id: "claim.email_to_ldap.pwdError",
        ...
        if (!ldapId) {
            setLdapError(
                formatMessage({
                    id: "claim.email_to_ldap.ldapIdError",
                    defaultMessage: "Please enter your AD/LDAP ID.",
                }),
            );
            setPasswordError("");
            setLdapPasswordError("");
        ...
        if (!ldapPassword) {
            setLdapPasswordError(
                formatMessage({
                    id: "claim.email_to_ldap.ldapPasswordError",
                    defaultMessage: "Please enter your AD/LDAP password.",
                }),
            );
            setLdapError("");
            setPasswordError("");
        ...
        emailToLdap(
            ...
            (err: { server_error_id: string; id: string; message: string }) => {
                if (
                    !showMfa &&
                    err.server_error_id ===
                        ClaimErrors.MFA_VALIDATE_TOKEN_AUTHENTICATE
                ) {
                    setShowMfa(true);
                } else {
                    switch (err.id) {
    ...
    const loginPlaceholder =
        ldapLoginFieldName ||
        formatMessage({
            id: "claim.email_to_ldap.ldapId",
            defaultMessage: "AD/LDAP ID",
        });
            ...
            >
                ...
                >
                    <input
                        ...
                        autoComplete="off"
                        placeholder={formatMessage({
                            id: "claim.email_to_ldap.ldapPwd",
