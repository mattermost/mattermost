...
> {
    ...
    onContactUsClick = async (e: any) => {
        ...
        if (e && e.target && e.target.dataset && e.target.dataset.forceack) {
            forceAck = true;
            trackEvent("admin", "click_warn_metric_mailto", {
                metric: this.props.warnMetricStatus.id,
            });
        } else {
            trackEvent("admin", "click_warn_metric_contact_us", {
                metric: this.props.warnMetricStatus.id,
            });
        }

        const { error } = await this.props.actions.sendWarnMetricAck(
            this.props.warnMetricStatus.id,
            forceAck,
        );
        if (error) {
    ...
    renderContactUsError = () => {
        ...
        mailBody +=
            "If you have any additional inquiries, please contact support@mattermost.com";

        const mailToLinkText =
            "mailto:" +
            mailRecipient +
            "?cc=" +
                        ...
                        values={{
                            link: (
                                <WarnMetricAckErrorLink
                                    url={mailToLinkText}
                                    messageId={t(
                                        "warn_metric_ack_modal.mailto.link",
                                    )}
                                    forceAck={true}
    ...
    render() {
        ...
        let descriptionText;
        const learnMoreLink =
            "https://mattermost.com/pl/default-admin-advisory";
        ...
        const subText = (
            <div style={containerStyles} className="help__format-text">
                <FormattedMessage
                    ...
                    values={{
                        link: (
                            <ErrorLink
                                url={learnMoreLink}
                                messageId={t(
                                    "warn_metric_ack_modal.learn_more.link",
                                )}
                ...
                >
                    <LoadingWrapper
                        ...
                        text={Utils.localizeMessage(
                            "admin.warn_metric.sending-email",
                            "Sending email",
                        )}
                    >
                        <FormattedMessage
...
const WarnMetricAckErrorLink: React.FC<ErrorLinkProps> = ({
    defaultMessage,
    messageId,
    onClickHandler,
    url,
