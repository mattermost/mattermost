...
} from "components/widgets/simple_tooltip";
...
function countMeta<T>(
    ...
    const overflowUnnamedCount = Math.max(
        total - displayItems.length - overflowItems.length,
        0,
    );
    ...
    return [
        displayItems,
        overflowItems,
        { overflowUnnamedCount, nonDisplayCount },
    ];
}
...
function UserAvatar({
    ...
    const name = useSelector((state: GlobalState) =>
        displayNameGetter(state, true)(user),
    );
...
function Avatars({
    ...
    const [
        ...
        overflowUserIds,
        { overflowUnnamedCount, nonDisplayCount },
    ] = countMeta(userIds, totalUsers);
    const overflowNames = useSelector((state: GlobalState) => {
        return overflowUserIds
            .map((userId) =>
                displayNameGetter(state, true)(selectUser(state, userId)),
            )
            .join(", ");
    });
    ...
    const avatarStyle: CSSProperties = useMemo(
        () => ({
            background: tinycolor
                .mix(centerChannelBg, centerChannelColor, 8)
                .toRgbString(),
        }),
                            ...
                            : formatMessage(
                                  {
                                      ...
                                      defaultMessage:
                                          "{overflowUnnamedCount, plural, =1 {one other} other {# others}}",
                                  },
                                  { overflowUnnamedCount },
                              )
                    }
