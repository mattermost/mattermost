...
describe("components/widgets/users/Avatars", () => {
    ...
    test("should support userIds", () => {
        ...
        expect(
            wrapper
                .find(Avatar)
                .find({ url: "/api/v4/users/1/image?_=1620680333191" })
                .exists(),
        ).toBe(true);
        expect(
            wrapper
                .find(Avatar)
                .find({ url: "/api/v4/users/2/image?_=1620680333191" })
                .exists(),
        ).toBe(true);
        expect(
            wrapper
                .find(Avatar)
                .find({ url: "/api/v4/users/3/image?_=1620680333191" })
                .exists(),
        ).toBe(true);
    ...
    test("should properly count overflow", () => {
        ...
        const wrapper = mount(
            <Avatars size="xl" userIds={["1", "2", "3", "4", "5"]} />,
            mountOptions,
        );
        ...
        expect(
            wrapper
                .find(Avatar)
                .find({ url: "/api/v4/users/4/image?_=1620680333191" })
                .exists(),
        ).toBe(false);
        expect(
            wrapper
                .find(Avatar)
                .find({ url: "/api/v4/users/5/image?_=1620680333191" })
                .exists(),
        ).toBe(false);
    ...
    test("should not duplicate displayed users in overflow tooltip", () => {
        ...
        expect(
            wrapper
                .find(SimpleTooltip)
                .find({ id: "names-overflow" })
                .prop("content"),
        ).toBe("first.last4, first.last5");
    });
    ...
    test("should fetch missing users", () => {
        ...
        const wrapper = mount(
            <Avatars size="xl" userIds={["1", "6", "7", "2", "8", "9"]} />,
            mountOptions,
        );
        ...
        expect(
            wrapper
                .find(Avatar)
                .find({ url: "/api/v4/users/6/image?_=0" })
                .exists(),
        ).toBe(true);
        expect(
            wrapper
                .find(Avatar)
                .find({ url: "/api/v4/users/7/image?_=0" })
                .exists(),
        ).toBe(true);
        expect(
            wrapper
                ...
                .find({ id: "names-overflow" })
                .prop("content"),
        ).toBe("first.last2, Someone, Someone");
    });
});
