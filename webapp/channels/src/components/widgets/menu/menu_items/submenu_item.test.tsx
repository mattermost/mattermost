...
describe("components/widgets/menu/menu_items/submenu_item", () => {
    ...
    test("should show/hide submenu based on keyboard commands", () => {
        ...
        wrapper
            .instance()
            .handleKeyDown({ keyCode: Constants.KeyCodes.ENTER[1] } as any);
        expect(wrapper.instance().show).toHaveBeenCalled();

        wrapper
            .instance()
            .handleKeyDown({ keyCode: Constants.KeyCodes.LEFT[1] } as any);
        expect(wrapper.instance().hide).toHaveBeenCalled();

        wrapper
            .instance()
            .handleKeyDown({ keyCode: Constants.KeyCodes.RIGHT[1] } as any);
        expect(wrapper.instance().show).toHaveBeenCalled();
    });
});
