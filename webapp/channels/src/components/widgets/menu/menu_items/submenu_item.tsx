...
export default class SubMenuItem extends React.PureComponent<Props, State> {
    ...
    public render() {
                ...
                <React.Fragment>
                    <span
                        className={classNames([
                            "icon",
                            { "sorting-menu-icon": styleSelectableItem },
                        ])}
                    >
                        {icon}
                    </span>
        ...
        const subMenuStyle: CSSProperties = {
            visibility: (this.state.show && hasSubmenu && !isMobile
                ? "visible"
                : "hidden") as "visible" | "hidden",
            top:
                this.node && this.node.current
                    ? String(this.node.current.offsetTop) + "px"
                    : "unset",
        };
        ...
        if (!isMobile) {
            subMenuContent = (
                <ul
                    className={classNames([
                        "a11y__popup Menu dropdown-menu SubMenu",
                        { styleSelectableItem },
                    ])}
                    style={subMenuStyle}
                >
                    {hasSubmenu
                        ? subMenu!.map((s) => {
                              const hasDivider =
                                  s.id === "ChannelMenu-moveToDivider";
                              let aria = ariaLabel;
                              if (s.action) {
                                  aria =
        ...
        return (
            ...
            >
                <div
                    className={classNames([
                        { styleSelectableItemDiv: styleSelectableItem },
                    ])}
                    id={id}
                    ...
                    <div className={icon ? "grid" : "flex"}>
                        ...
                        {id !== "ChannelMenu-moveToDivider" && (
                            <span
                                ...
                                className={classNames([
                                    `fa fa-angle-right SubMenu__icon-right${
                                        hasSubmenu ? "" : "-empty"
                                    }`,
                                    { mobile: isMobile },
                                ])}
                                aria-label={Utils.localizeMessage(
                                    "post_info.submenu.icon",
                                    "submenu icon",
                                ).toLowerCase()}
                            />
                        )}
