...
const MenuItemCloudLimit = ({ id }: Props) => {
    ...
    if (
        (highestLimit.usage / highestLimit.limit) * 100 >=
        limitThresholds.danger
    ) {
    ...
    return (
        <li className={itemClass} role="menuitem" id={id}>
            ...
            <div className="MenuItemCloudLimit__usage">
                ...
                />
                <span className="MenuItemCloudLimit__usage-label">
                    {words.status}
                </span>
            </div>
        </li>
