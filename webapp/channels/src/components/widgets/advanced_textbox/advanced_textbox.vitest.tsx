...
import {render, screen, fireEvent, userEvent} from 'tests/vitest_react_testing_utils';
...
vi.mock('components/advanced_text_editor/show_formatting/show_formatting', () => ({
    __esModule: true,
    default: vi.fn().mockImplementation((props) => (
        <button
...
describe('AdvancedTextbox', () => {
    ...
    test('forces preview mode when readOnly is true regardless of preview prop', async () => {
        // Get reference to the mocked Textbox component
        const TextboxMock = (await import('components/textbox')).default;

        // Clear previous calls to the mock
        vi.mocked(TextboxMock).mockClear();
