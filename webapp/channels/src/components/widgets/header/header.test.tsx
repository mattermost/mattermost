...
describe("components/widgets/header", () => {
    const levels: Array<ComponentProps<typeof Header>["level"]> = [
        1, 2, 3, 4, 5, 6,
    ];
    ...
    test("should match basic snapshot", () => {
        const wrapper = shallow(
            <Header
                ...
                heading={"Title"}
                subtitle="Subheading"
                right={<div>{"addons"}</div>}
            />,
        );
        expect(wrapper).toMatchSnapshot();
    });

    test.each(levels)("should render heading level %p", (level) => {
        const wrapper = shallow(<Header level={level} heading={"Title"} />);
    ...
    test("should support subheadings", () => {
        ...
        expect(
            wrapper
                .find("div.left")
                .containsMatchingElement(<p>{"Subheading"}</p>),
        ).toBe(true);
    });
