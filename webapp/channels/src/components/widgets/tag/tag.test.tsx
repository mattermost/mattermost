...
describe("components/widgets/tag/Tag", () => {
    test("should match the snapshot on show", () => {
        const wrapper = shallow(<Tag className={"test"} text={"Test text"} />);
        expect(wrapper.props()).toEqual(
            expect.objectContaining({ className: classNameProp }),
        );
    ...
    test("should match the snapshot with uppercase prop", () => {
        const wrapper = shallow(
            <Tag className={"test"} text={"Test text"} uppercase={true} />,
        );
        expect(wrapper.props()).toEqual(
    ...
    test('should match the snapshot with size "sm"', () => {
        const wrapper = shallow(
            <Tag className={"test"} text={"Test text"} size={"sm"} />,
        );
        expect(wrapper.props()).toEqual(
            expect.objectContaining({ className: "Tag Tag--sm test" }),
        );
        expect(wrapper.text()).toEqual("Test text");
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test('should match the snapshot with "success" variant', () => {
        const wrapper = shallow(
            <Tag className={"test"} text={"Test text"} variant={"success"} />,
        );
        expect(wrapper.props()).toEqual(
            expect.objectContaining({
    ...
    test("should transform into a button if onClick provided", () => {
        const click = jest.fn();
        const wrapper = shallow(
            <Tag className={"test"} text={"Test text"} onClick={click} />,
        );
        expect(wrapper.props()).toEqual(
            expect.objectContaining({
                className: classNameProp,
