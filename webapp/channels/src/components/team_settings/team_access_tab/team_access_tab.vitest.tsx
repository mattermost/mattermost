...
import {act, renderWithContext, screen, userEvent} from 'tests/vitest_react_testing_utils';
import {TestHelper} from 'utils/test_helper';

import AccessTab from './team_access_tab';

describe('components/TeamSettings', () => {
    const getTeam = vi.fn().mockResolvedValue({data: true});
    const patchTeam = vi.fn().mockReturnValue({data: true});
    const regenerateTeamInviteId = vi.fn().mockReturnValue({data: true});
    const removeTeamIcon = vi.fn().mockReturnValue({data: true});
    const setTeamIcon = vi.fn().mockReturnValue({data: true});
    const baseActions = {
        getTeam,
        patchTeam,
