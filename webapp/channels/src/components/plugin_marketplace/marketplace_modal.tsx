...
import MarketplaceList, {
    ITEMS_PER_PAGE,
} from "./marketplace_list/marketplace_list";
...
const linkConsole = (msg: string): ReactNode => (
    <Link to="/admin_console/plugins/plugin_management">{msg}</Link>
);

export type OpenedFromType =
    | "actions_menu"
    | "app_bar"
    | "channel_header"
    | "command"
    | "open_plugin_install_post"
    | "product_menu";
...
const MarketplaceModal = ({ openedFrom }: MarketplaceModalProps) => {
    const dispatch = useDispatch();
    const { formatMessage } = useIntl();
    const listRef = useRef<HTMLDivElement>(null);

    const show = useSelector((state: GlobalState) =>
        isModalOpen(state, ModalIdentifiers.PLUGIN_MARKETPLACE),
    );
    const listing = useSelector(getListing);
    const installedListing = useSelector(getInstalledListing);
    const pluginStatuses = useSelector(
        (state: GlobalState) => state.entities.admin.pluginStatuses,
    );
    const hasFirstAdminVisitedMarketplace = useSelector(
        getFirstAdminVisitMarketplaceStatus,
    );
    const isStreamlinedMarketplaceEnabled = useSelector(
        streamlinedMarketplaceEnabled,
    );
    ...
    const handleChangeTab: SelectCallback = useCallback(
        (tabKey) => {
            setTabKey(tabKey);
            setPage(0);
            scrollListToTop();
                    ...
                    ? formatMessage(
                          {
                              ...
                              defaultMessage:
                                  "Error connecting to the marketplace server. Please check your settings in the <linkConsole>System Console</linkConsole>.",
                          },
                          { linkConsole },
                      )
                    : undefined
            ...
            {isStreamlinedMarketplaceEnabled ? (
                ...
                >
                    <Tab
                        ...
                        title={formatMessage({
                            id: "marketplace_modal.tabs.all_listing",
                            defaultMessage: "All",
                        })}
                    >
                        {loading ? (
                    ...
                    <Tab
                        ...
                        title={formatMessage(
                            {
                                id: "marketplace_modal.tabs.installed_listing",
                                defaultMessage: "Installed ({count})",
                            },
