...
const CustomStatusModal: React.FC<Props> = (props: Props) => {
    ...
    const customStatusExpired = useSelector((state: GlobalState) =>
        isCustomStatusExpired(state, currentCustomStatus),
    );
    ...
    const isCurrentCustomStatusSet =
        !customStatusExpired &&
        (currentCustomStatus?.text || currentCustomStatus?.emoji);
    const [showEmojiPicker, setShowEmojiPicker] = useState<boolean>(false);
    const [text, setText] = useState<string>(
        isCurrentCustomStatusSet ? currentCustomStatus?.text : "",
    );
    const [emoji, setEmoji] = useState<string>(
        isCurrentCustomStatusSet ? currentCustomStatus?.emoji : "",
    );
    const initialDuration = isCurrentCustomStatusSet
        ? currentCustomStatus?.duration
        : defaultDuration;
    const [duration, setDuration] = useState<CustomStatusDuration>(
        initialDuration === undefined ? defaultDuration : initialDuration,
    );
    ...
    if (
        isCurrentCustomStatusSet &&
        currentCustomStatus?.duration === DATE_AND_TIME &&
        currentCustomStatus?.expires_at
    ) {
    ...
    const loadCustomEmojisForRecentStatuses = () => {
        ...
        recentCustomStatuses.forEach((customStatus: UserCustomStatus) =>
    ...
    const handleClearStatus = isCurrentCustomStatusSet
        ? () => dispatch(unsetCustomStatus())
        : undefined;
    ...
    const handleRecentCustomStatusClear = (status: UserCustomStatus) =>
    ...
    const renderCustomStatusSuggestions = () => {
        const recentCustomStatusTexts = recentCustomStatuses.map(
            (status: UserCustomStatus) => status.text,
        );
        const customStatusSuggestions = defaultCustomStatusSuggestions
            ...
            .filter(
                (status: UserCustomStatus) =>
                    !recentCustomStatusTexts.includes(status.text),
            )
    ...
    const areEmojiAndTextSame =
        currentCustomStatus?.emoji === emoji &&
        currentCustomStatus?.text === text;
    const areSelectedAndSetStatusSame =
        areEmojiAndTextSame && duration === currentCustomStatus?.duration;

    const showSuggestions = !isStatusSet || areSelectedAndSetStatusSame;

    const disableSetStatus =
        !isStatusSet || text.length > CUSTOM_STATUS_TEXT_CHARACTER_LIMIT;

    const showDateAndTimeField =
        !showSuggestions &&
        (duration === CUSTOM_DATE_TIME || duration === DATE_AND_TIME);
                    ...
                    >
                        ...
                        <button
                            ...
                            className={classNames(
                                ...
                                "StatusModal__emoji-button",
                                {
                                    "StatusModal__emoji-button--active":
                                        showEmojiPicker,
                                },
                            )}
                        >
