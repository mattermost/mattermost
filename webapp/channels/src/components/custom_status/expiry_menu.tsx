...
const ExpiryMenu: React.FC<Props> = (props: Props) => {
    ...
    const expiryMenuItems: { [key in CustomStatusDuration]?: ExpiryMenuItem } =
        {
            ...
            [CUSTOM_DATE_TIME]: {
                text: formatMessage({
                    id: "custom_status.expiry_dropdown.choose_date_and_time",
    ...
    useEffect(() => {
        const menuItemArray = Object.keys(expiryMenuItems).map(
            (item, index) => (
                <Menu.ItemAction
                    key={item.toString()}
                    onClick={(
                    ...
                    ariaLabel={expiryMenuItems[
                        item as CustomStatusDuration
                    ]?.text.toLowerCase()}
    ...
    return (
        <div className="statusExpiry">
            <div className="statusExpiry__content">
                <MenuWrapper className={"statusExpiry__menu"}>
