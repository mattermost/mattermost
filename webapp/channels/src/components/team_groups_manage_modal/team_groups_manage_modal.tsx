...
class TeamGroupsManageModal extends React.PureComponent<Props, State> {
    ...
    loadItems = async (pageNumber: number, searchTerm: string) => {
        const { data } = await this.props.actions.getGroupsAssociatedToTeam(
            this.props.team.id,
            searchTerm,
            pageNumber,
    ...
    handleDeleteConfirmed = () => {
        ...
        this.props.actions
            .unlinkGroupSyncable(item.id, this.props.team.id, SyncableType.Team)
            .then(async () => {
                if (listModal) {
                    listModal.setState({ loading: true });
                    const { items, totalCount } =
                        await listModal.props.loadItems(
                            listModal.state.page,
    ...
    onClickConfirmRemoveGroup = (item: Group, listModal: ListModal) =>
        this.props.actions
            .unlinkGroupSyncable(item.id, this.props.team.id, SyncableType.Team)
            .then(async () => {
                listModal.setState({ loading: true });
                const { items, totalCount } = await listModal.props.loadItems(
                    listModal.state.page,
                    listModal.state.searchTerm,
                );
                listModal.setState({ loading: false, items, totalCount });
                        ...
                        >
                            <Menu.ItemAction
                                ...
                                text={Utils.localizeMessage(
                                    "team_members_dropdown.makeTeamAdmins",
                                    "Make Team Admins",
                                )}
                            />
                            <Menu.ItemAction
                                ...
                                text={Utils.localizeMessage(
                                    "team_members_dropdown.makeTeamMembers",
                                    "Make Team Members",
                                )}
                            />
                            <Menu.ItemAction
            ...
            <>
                <ListModal
                    ...
                    titleText={formatMessage(
                        { id: "team_groups", defaultMessage: "{team} Groups" },
                    ...
                    title={formatMessage(
                        {
                            ...
                            defaultMessage:
                                "Remove Group and {memberCount, number} {memberCount, plural, one {Member} other {Members}}",
                        },
                        { memberCount },
                    )}
                    message={formatMessage(
                        {
                            ...
                            defaultMessage:
                                "{memberCount, number} {memberCount, plural, one {member} other {members}} associated to this group will be removed from the team. Are you sure you wish to remove this group and {memberCount} {memberCount, plural, one {member} other {members}}?",
                        },
                        { memberCount },
                    )}
                    confirmButtonText={formatMessage(
                        {
                            ...
                            defaultMessage:
                                "Yes, Remove Group and {memberCount, plural, one {Member} other {Members}}",
                        },
                        { memberCount },
                    )}
