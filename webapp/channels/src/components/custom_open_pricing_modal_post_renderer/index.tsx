...
export default function OpenPricingModalPost(props: { post: Post }) {
    ...
    ): string[] => requests.map((request: FeatureRequest) => request.user_id);
    ...
    const isDowngradeNotification = (featureId: string) =>
        featureId === MattermostFeatures.UPGRADE_DOWNGRADED_WORKSPACE;
                    ...
                    <span>
                        <Markdown
                            message={formatMessage(
                                {
                                    id: "postypes.custom_open_pricing_modal_post_renderer.availableOn",
                                    defaultMessage:
                                        " - available on the {feature}",
                                },
                                { feature: mapFeatureToPlan(featureId) },
                            )}
                            options={{
                                ...markDownOptions,
                ...
                <ul id={`${featureId}-subtitle`.replaceAll(".", "_")}>
                    <li>
                        <Markdown
                            ...
                            message={formatMessage(
                                {
                                    id: "postypes.custom_open_pricing_modal_post_renderer.userRequests",
                                    defaultMessage:
                                        "{userRequests} requested access to this feature",
                                },
                                {
                                    userRequests:
                                        renderUsersThatRequestedFeature(
                                            requestFeatures[featureId],
                                        ),
                                },
                            )}
                    ...
                    <ul id={`${featureId}-subtitle`.replaceAll(".", "_")}>
                        <li>
                            <Markdown
                                ...
                                message={formatMessage(
                                    {
                                        id: "postypes.custom_open_pricing_modal_post_renderer.downgradeNotfication",
                                        defaultMessage:
                                            "{userRequests} requested to revert the workspace to a paid plan",
                                    },
                                    {
                                        userRequests:
                                            renderUsersThatRequestedFeature(
                ...
                <>
                    <button
                        ...
                        onClick={() => {
                            trackEvent(
                                "cloud_admin",
                                "click_learn_more_trial_modal",
                                {
                                    callerInfo:
                                        "notify_admin_learn_more_about_trial",
                                },
                            );
                    ...
                    >
                        {formatMessage({
                            id: "postypes.custom_open_pricing_modal_post_renderer.learn_trial",
                            defaultMessage: "Learn more about trial",
                        })}
                    </button>
                    <button
                        onClick={() =>
                            openPricingModal({
                                trackingLocation:
                                    "notify_admin_message_view_upgrade_options",
                            })
                        }
                        style={{
                            ...btnStyle,
                            color: "var(--button-bg)",
                            background:
                                "rgba(var(--denim-button-bg-rgb), 0.08)",
                        }}
                    >
                        {formatMessage({
                            id: "postypes.custom_open_pricing_modal_post_renderer.view_options",
            ...
            return (
                <>
                    <button
                        ...
                        onClick={() =>
                            openPurchaseModal({
                                trackingLocation: "notify_admin_message_view",
                            })
                        }
                        style={btnStyle}
                    >
                        {formatMessage({
                            id: "postypes.custom_open_pricing_modal_post_renderer.upgrade_professional",
                            defaultMessage: "Upgrade to Professional",
                        })}
                    </button>
                    <button
    ...
    return (
        <div>
            ...
            <div style={{ display: "flex" }}>
                <div style={style}>{renderButtons()}</div>
            </div>
        </div>
    );
}
