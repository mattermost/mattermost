...
describe("OverlayTrigger", () => {
    ...
    const baseProps = {
        overlay: (
            <FormattedMessage id={testId} defaultMessage="Default value" />
        ),
    };
    ...
    test("style and className should correctly be passed to overlay", () => {
        ...
        const overlay = mount(
            (wrapper.find(BaseOverlayTrigger).instance() as any)._overlay,
        ).find("span"); // eslint-disable-line no-underscore-dangle
    ...
    test("disabled and style should both be supported", () => {
        const props = {
            ...baseProps,
            overlay: (
                <span style={{ backgroundColor: "red" }}>{"test-overlay"}</span>
            ),
            defaultOverlayShown: true, // Make sure the overlay is visible
            disabled: true,
        };
        ...
        // Confirm that our props are included
        expect(overlay.prop("style")).toMatchObject({
            backgroundColor: "red",
            visibility: "hidden",
        });
    });
