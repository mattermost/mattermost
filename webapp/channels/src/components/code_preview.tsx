...
export default class CodePreview extends React.PureComponent<Props, State> {
    ...
    static getDerivedStateFromProps(props: Props, state: State) {
        if (props.fileUrl !== state.prevFileUrl) {
            const usedLanguage =
                SyntaxHighlighting.getLanguageFromFileExtension(
    ...
    getCode = async () => {
        if (
            !this.state.lang ||
            this.props.fileInfo.size > Constants.CODE_PREVIEW_MAX_FILE_SIZE
        ) {
            return;
        }
        try {
    ...
    handleReceivedCode = async (data: string | Node) => {
        ...
        this.setState({
            code,
            highlighted: await SyntaxHighlighting.highlight(
                this.state.lang,
                code,
            ),
            loading: false,
