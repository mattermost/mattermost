...
describe("components/MaxLengthInput", () => {
    ...
    test.each([[undefined], ["less than 20"], ["Where is Jessica Hyde?"]])(
    ...
    test.each([
        ...
        "defaultValue: %s .has-error: %s, .MaxLengthInput__validation: %s",
        (defaultValue, hasError, maxLengthExists) => {
            const wrapper = shallow(
                <MaxLengthInput
                    {...requiredProps}
                    defaultValue={defaultValue}
            ...
            expect(wrapper.exists(".MaxLengthInput__validation")).toBe(
                maxLengthExists,
            );
        },
    );
    ...
    test("should display the number of times value length exceeds maxLength", () => {
        ...
        const wrapper = shallow(<MaxLengthInput {...props} />);
