...
export function makeGetPostOptionBinding(): (
    state: GlobalState,
    location?: string,
) => AppBinding[] | null {
    const centerBindingsSelector = makeAppBindingsSelector(
        AppBindingLocations.POST_MENU_ITEM,
    );
    const rhsBindingsSelector = makeRHSAppBindingSelector(
        AppBindingLocations.POST_MENU_ITEM,
    );
    return createSelector(
        ...
        (
            centerBindings: AppBinding[],
            rhsBindings: AppBinding[],
            location?: string,
        ) => {
            switch (location) {
