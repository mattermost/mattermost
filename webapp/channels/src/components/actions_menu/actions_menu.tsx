...
export class ActionMenuClass extends React.PureComponent<Props, State> {
    ...
    tooltip = (
        <Tooltip id="actions-menu-icon-tooltip" className="hidden-xs">
            <FormattedMessage
    ...
    fetchBindings = () => {
        if (this.props.appsEnabled && !this.state.appBindings) {
            this.props.actions
                .fetchBindings(this.props.post.channel_id, this.props.teamId)
                .then(({ data }) => {
    ...
    render(): React.ReactNode {
        ...
        if (this.props.appsEnabled && this.state.appBindings) {
            appBindings = this.state.appBindings.map((item) => {
                ...
                if (item.icon) {
                    icon = (
                        <img key={item.app_id + "app_icon"} src={item.icon} />
                    );
                }
        ...
        const hasPluggables = Boolean(
            this.props.components[PLUGGABLE_COMPONENT]?.length,
        );
        ...
        if (hasPluginMenuItems) {
            ...
            menuItems = [pluginItems, appBindings, pluggable, marketPlace];
        } else {
                ...
                >
                    <button
                        ...
                        aria-label={Utils.localizeMessage(
                            "post_info.actions.tooltip.actions",
                            "Actions",
                        ).toLowerCase()}
