...
type Props = {
    ...
    actions: {
        ...
        unlinkGroupSyncable: (
            itemId: string,
            channelId: string,
            groupsSyncableTypeChannel: string,
        ) => any;
        patchGroupSyncable: (
            itemId: string,
            channelId: string,
            groupsSyncableTypeChannel: string,
            params: { scheme_admin: boolean },
        ) => any;
...
class ChannelGroupsManageModal extends React.PureComponent<Props> {
    ...
    public onClickRemoveGroup = (item: Group, listModal: any) =>
        this.props.actions
            .unlinkGroupSyncable(
                item.id,
                this.props.channel.id,
    ...
    public setChannelMemberStatus = async (
        item: Group,
        listModal: any,
        ...
        this.props.actions
            ...
            .then(async () => {
                ...
                await this.props.actions.getMyChannelMember(
                    this.props.channel.id,
                );
    ...
    public renderRow = (item: Group, listModal: any) => {
        ...
        if (item.scheme_admin) {
            title = Utils.localizeMessage(
                "channel_members_dropdown.channel_admins",
                "Channel Admins",
            );
        } else {
            title = Utils.localizeMessage(
                "channel_members_dropdown.channel_members",
                "Channel Members",
            );
        }
                        ...
                        >
                            <Menu.ItemAction
                                ...
                                text={Utils.localizeMessage(
                                    "channel_members_dropdown.make_channel_admins",
                                    "Make Channel Admins",
                                )}
                            />
                            <Menu.ItemAction
                                ...
                                text={Utils.localizeMessage(
                                    "channel_members_dropdown.make_channel_members",
                                    "Make Channel Members",
                                )}
                            />
                            <Menu.ItemAction
                                onClick={() =>
