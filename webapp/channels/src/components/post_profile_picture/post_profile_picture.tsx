...
export default class PostProfilePicture extends React.PureComponent<Props> {
    ...
    render() {
        ...
        const src = this.getPostIconURL(
            profileSrc,
            fromAutoResponder,
            fromWebhook,
        );

        const overrideIconEmoji = post.props
            ? post.props.override_icon_emoji
            : "";
        const overwriteName = post.props ? post.props.override_username : "";
        const isEmoji =
            typeof overrideIconEmoji == "string" && overrideIconEmoji !== "";
