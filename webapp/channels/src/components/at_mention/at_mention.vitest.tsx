...
describe('components/AtMention', () => {
    const getMissingMentionedUsers = vi.fn();
    const baseProps = {
        currentUserId: 'abc1',
    ...
    beforeEach(() => {
        getMissingMentionedUsers.mockClear();
    });
    ...
    describe('fetchMissingUsers', () => {
        test('when fetchMissingUsers is true, should fetch an unloaded user on mount', () => {
            ...
            expect(getMissingMentionedUsers).toHaveBeenCalledWith('someuser');
        });
        ...
        test('when fetchMissingUsers is false, should not fetch an unloaded user on mount', () => {
            ...
            expect(getMissingMentionedUsers).not.toHaveBeenCalled();
        });
    });
});
