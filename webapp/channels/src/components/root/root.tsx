...
const LazyPasswordResetSendLink = React.lazy(
    () => import("components/password_reset_send_link"),
);
const LazyPasswordResetForm = React.lazy(
    () => import("components/password_reset_form"),
);
...
const LazyShouldVerifyEmail = React.lazy(
    () => import("components/should_verify_email/should_verify_email"),
);
const LazyDoVerifyEmail = React.lazy(
    () => import("components/do_verify_email/do_verify_email"),
);
const LazyClaimController = React.lazy(() => import("components/claim"));
const LazyLinkingLandingPage = React.lazy(
    () => import("components/linking_landing_page"),
);
...
const LazyPreparingWorkspace = React.lazy(
    () => import("components/preparing_workspace"),
);
const LazyTeamController = React.lazy(
    () => import("components/team_controller"),
);
const LazyDelinquencyModalController = React.lazy(
    () => import("components/delinquency_modal"),
);
const LazyOnBoardingTaskList = React.lazy(
    () => import("components/onboarding_tasklist"),
);
...
const PreparingWorkspace = makeAsyncComponent(
    "PreparingWorkspace",
    LazyPreparingWorkspace,
);
const TeamController = makeAsyncComponent("TeamController", LazyTeamController);
const DelinquencyModalController = makeAsyncComponent(
    "DelinquencyModalController",
    LazyDelinquencyModalController,
);
const OnBoardingTaskList = makeAsyncComponent(
    "OnboardingTaskList",
    LazyOnBoardingTaskList,
);
...
export default class Root extends React.PureComponent<Props, State> {
    ...
    onConfigLoaded = () => {
        ...
        const androidDownloadLink = getConfig(
            store.getState(),
        ).AndroidAppDownloadLink;
        const desktopAppDownloadLink = getConfig(
            store.getState(),
        ).AppDownloadLink;
        ...
        if (
            ...
            !window.location.hostname?.endsWith(".test.mattermost.com") &&
            !UserAgent.isDesktopApp() &&
            !UserAgent.isChromebook()
        ) {
    ...
    async redirectToOnboardingOrDefaultTeam() {
        ...
        const firstAdminSetupComplete =
            await this.props.actions.getFirstAdminSetupComplete();
        if (firstAdminSetupComplete?.data) {
            GlobalActions.redirectUserToDefaultTeam();
            return;
        }

        const profilesResult = await this.props.actions.getProfiles(
            0,
    ...
    componentDidMount() {
        ...
        this.props.actions.registerCustomPostRenderer(
            "custom_up_notification",
            OpenPricingModalPost,
            "upgrade_post_message_renderer",
        );
        this.props.actions.registerCustomPostRenderer(
    ...
    handleLogoutLoginSignal = (e: StorageEvent) => {
        ...
        const isNewLocalStorageEvent = (event: StorageEvent) =>
            event.storageArea === localStorage && event.newValue;
