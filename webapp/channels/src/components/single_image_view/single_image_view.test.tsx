...
describe("components/SingleImageView", () => {
    ...
    test("should correctly pass prop down to surround small images with a container", () => {
        ...
        expect(
            wrapper.find(SizeAwareImage).prop("handleSmallImageContainer"),
        ).toEqual(true);
    });

    test("should not show filename when image is displayed", () => {
        const wrapper = shallow(
            <SingleImageView {...baseProps} isEmbedVisible={true} />,
        );
    ...
    test("should show filename when image is collapsed", () => {
        const wrapper = shallow(
            <SingleImageView {...baseProps} isEmbedVisible={false} />,
        );

        expect(wrapper.find(".image-header").text()).toEqual(
            baseProps.fileInfo.name,
        );
    });
