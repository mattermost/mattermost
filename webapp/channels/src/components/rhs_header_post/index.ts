...
function makeMapStateToProps() {
    ...
    return function mapStateToProps(
        ...
        const tipStep = getInt(
            state,
            Preferences.CRT_THREAD_PANE_STEP,
            currentUserId,
        );
        ...
        if (root && collapsedThreads) {
            ...
            if (isFollowingThread === null && thread.reply_count === 0) {
                ...
                isFollowingThread = matchUserMentionTriggersWithMessageMentions(
        ...
        const showThreadsTutorialTip =
            tipStep === CrtThreadPaneSteps.THREADS_PANE_POPOVER &&
            isCollapsedThreadsEnabled(state) &&
            onboardingTourTipsEnabled(state);
