...
import type {AppCallRequest} from '@mattermost/types/apps';
...
// Type for the adapter functions used by InteractiveDialogAdapter
// These differ from DoAppSubmit/DoAppLookup which expect (call, intl) - the adapters only take the call
// Uses Partial because tests only need to pass values, not a full AppCallRequest
type SubmitAdapter = (call: Partial<AppCallRequest>) => Promise<DoAppCallResult<unknown>>;
type LookupAdapter = (call: Partial<AppCallRequest>) => Promise<DoAppCallResult<unknown>>;

// Mock AppsFormContainer to avoid dynamic import complexity in tests
vi.mock('components/apps_form/apps_form_container', () => {
    return {
        __esModule: true,
...
// Get the mocked AppsFormContainer
const MockAppsFormContainer = vi.mocked(AppsFormContainer);
...
describe('components/interactive_dialog/InteractiveDialogAdapter', () => {
    const baseProps = {
        ...
        actions: {
            ...
            lookupInteractiveDialog: vi.fn().mockResolvedValue({data: {items: []}}),
        },
    ...
    describe('Submit and Cancel Functionality', () => {
        ...
        test('should handle submit adapter with server validation errors', async () => {
            ...
            const submitAdapter = mockCall.actions.doAppSubmit as SubmitAdapter;
            ...
            expect((result.error!.data as any).errors).toEqual({'test-field': 'Field is required'});
        });

        test('should handle submit adapter with network error', async () => {
            const mockSubmitWithNetworkError = vi.fn().mockResolvedValue({
                error: {
        ...
        test('should handle submit adapter exception', async () => {
            const mockSubmitThrows = vi.fn().mockRejectedValue(new Error('Submit failed'));
        ...
        test('should handle cancel adapter with notifyOnCancel enabled', async () => {
            const mockSubmit = vi.fn().mockResolvedValue({data: {}});
        ...
        test('should handle cancel adapter errors gracefully', async () => {
            const mockSubmitThrows = vi.fn().mockRejectedValue(new Error('Cancel failed'));
    ...
    describe('Dynamic Select Integration', () => {
        ...
        test('should handle lookup calls for dynamic select', async () => {
            ...
            const mockLookupDialog = vi.fn().mockResolvedValue(mockLookupResponse);
            ...
            const mockCall = MockAppsFormContainer.mock.calls[0][0];
            const lookupHandler = mockCall.actions.doAppLookup as LookupAdapter;
        ...
        test('should handle lookup call exceptions', async () => {
            const mockLookupException = vi.fn().mockRejectedValue(new Error('Network error'));
        ...
        test('should handle empty lookup responses gracefully', async () => {
            const mockLookupEmpty = vi.fn().mockResolvedValue({
                data: {items: []},
            });
