...
describe('AppsFormDateTimeField', () => {
    ...
    it('should render datetime input component', async () => {
        await renderComponent();
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should render datetime input regardless of field requirements', async () => {
        const requiredField = {...defaultField, is_required: true};
        await renderComponent({field: requiredField});
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should render datetime input regardless of field description', async () => {
    ...
    it('should render DateTimeInput when value exists', async () => {
        await renderComponent({value: '2025-01-15T14:30:00Z'});

        // DateTimeInput renders current time by default in the mock
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should render DateTimeInput when no value', async () => {
        await renderComponent();
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should render DateTimeInput with custom hint', async () => {
        const fieldWithHint = {...defaultField, hint: 'Choose datetime'};
        await renderComponent({field: fieldWithHint});
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should render DateTimeInput for interaction', async () => {
    ...
    it('should render fields with default time', async () => {
        const fieldWithDefaultTime = {...defaultField, default_time: '09:00'};
        await renderComponent({field: fieldWithDefaultTime});
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should use custom time_interval', async () => {
        const fieldWithInterval = {...defaultField, time_interval: 30};
        await renderComponent({field: fieldWithInterval, value: '2025-01-15T14:30:00Z'});

        // The time_interval is passed to DateTimeInput component
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should handle datetime change', async () => {
        const mockOnChange = jest.fn();
        await renderComponent({onChange: mockOnChange, value: '2025-01-15T14:30:00Z'});
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });
    ...
    it('should render without errors even when datetime is outside range (validation is centralized)', async () => {
        const fieldWithRange = {
            ...defaultField,
            min_date: '2025-01-10',
            max_date: '2025-01-20',
        };
        await renderComponent({field: fieldWithRange, value: '2025-01-01T14:30:00Z'});

        // Component should still render, validation is now handled centrally on form submission
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should not show error for valid datetime within range', async () => {
        const fieldWithRange = {
            ...defaultField,
            min_date: '2025-01-01',
            max_date: '2025-01-31',
        };
        await renderComponent({field: fieldWithRange, value: '2025-01-15T14:30:00Z'});
        expect(screen.queryByText(/error/i)).not.toBeInTheDocument();
    });

    it('should handle datetime range constraints', async () => {
        const fieldWithRange = {
            ...defaultField,
            min_date: '2025-01-01',
            max_date: '2025-01-31',
    ...
    it('should render readonly field', async () => {
        const readonlyField = {...defaultField, readonly: true};
        await renderComponent({field: readonlyField});
        expect(screen.getByTestId('datetime-input')).toBeInTheDocument();
    });

    it('should use default 60 minute interval when not specified', async () => {
        await renderComponent({value: '2025-01-15T14:30:00Z'});
    ...
    describe('allowPastDates logic', () => {
        it('should allow past dates by default (no min_date)', async () => {
            await renderComponent({value: '2025-01-15T14:30:00Z'});
        ...
        it('should restrict past dates when min_date is today or future', async () => {
            const fieldWithMinDate = {...defaultField, min_date: 'today'};
            await renderComponent({field: fieldWithMinDate, value: '2025-01-15T14:30:00Z'});
