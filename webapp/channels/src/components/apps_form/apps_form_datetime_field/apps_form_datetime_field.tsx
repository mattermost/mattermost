...
import {stringToMoment, momentToString, resolveRelativeDate, getRoundedTime, momentToLocalDate} from 'utils/date_utils';
...
const AppsFormDateTimeField: React.FC<Props> = ({
    ...
    // Compute minDate from field config (resolves relative dates like "today", "+5d")
    const fieldMinDate = useMemo((): Date | undefined => {
        if (!field.min_date) {
            return undefined;
        }
        const resolvedMinDate = resolveRelativeDate(field.min_date);
        const minMoment = stringToMoment(resolvedMinDate, timezone);
        return minMoment ? momentToLocalDate(minMoment) : undefined;
    ...
    // For the end field: use the start date as minDate (respecting allowSingleDayRange)
    const endFieldMinDate = useMemo((): Date | undefined => {
        if (!startMoment) {
            return fieldMinDate;
        }
        const startDate = momentToLocalDate(startMoment)!;
        const minFromStart = allowSingleDayRange
            ? startDate
            : new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate() + 1);

        // Use the later of fieldMinDate and start-derived min
        if (fieldMinDate && fieldMinDate > minFromStart) {
            return fieldMinDate;
        }
        return minFromStart;
    }, [startMoment, allowSingleDayRange, fieldMinDate]);
