...
import {renderWithContext, waitFor, userEvent, screen} from 'tests/vitest_react_testing_utils';
...
describe('AppsFormComponent', () => {
    ...
    const baseProps: Props = {
        ...
        isEmbedded: false,
        actions: {
            performLookupCall: vi.fn(),
            refreshOnSelect: vi.fn(),
    ...
    test('it should submit and close the modal', async () => {
        const submit = vi.fn().mockResolvedValue({data: {type: 'ok'}});
    ...
    describe('Refresh on Select Functionality', () => {
        test('should handle refresh on select success', async () => {
            const mockRefreshOnSelect = vi.fn().mockResolvedValue({
                data: {
                    type: 'form',
                    form: {
                        title: 'Updated Form',
