...
export interface Props {
    ...
    autoFocus?: boolean;
    listComponent?: React.ComponentProps<
        typeof AutocompleteSelector
    >["listComponent"];
    performLookup: (
        name: string,
        userInput: string,
...
export default class AppsFormField extends React.PureComponent<Props> {
    ...
    handleSelected = (selected: AppSelectOption | AppSelectOption[]) => {
        ...
        if (Array.isArray(selected)) {
            const options = selected.map((selectedItem) => ({
    ...
    render() {
        const { field, name, value, onChange, errorText } = this.props;

        const placeholder = field.hint || "";

        const displayName = (field.modal_label || field.label) as string;
        let displayNameContent: React.ReactNode = (field.modal_label ||
        ...
        switch (field.type) {
            ...
            case AppFieldTypes.MARKDOWN: {
                return <Markdown message={field.description} />;
            }
        }
