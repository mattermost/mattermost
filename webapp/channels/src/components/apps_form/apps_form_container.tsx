...
class AppsFormContainer extends React.PureComponent<Props, State> {
    ...
    submitForm = async (submission: {
        ...
        if (!form) {
            const errMsg = this.props.intl.formatMessage({
        ...
        if (!this.props.context) {
            return {
                error: makeCallErrorResponse("unreachable: empty context"),
            };
        }

        const creq = createCallRequest(
            form.submit,
            this.props.context,
            {},
            submission.values,
        ...
        const res = (await this.props.actions.doAppSubmit(
            creq,
            this.props.intl,
        )) as DoAppCallResult<FormResponseData>;
        if (res.error) {
            return res;
        }
    ...
    refreshOnSelect = async (
        field: AppField,
        values: AppFormValues,
    ): Promise<DoAppCallResult<FormResponseData>> => {
        const makeErrMsg = (message: string) =>
            this.props.intl.formatMessage(
                {
    ...
    performLookupCall = async (
        field: AppField,
        values: AppFormValues,
        userInput: string,
    ): Promise<DoAppCallResult<AppLookupResponse>> => {
        const intl = this.props.intl;
        const makeErrorMsg = (message: string) =>
            intl.formatMessage(
                {
