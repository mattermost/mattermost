...
const RenewalLink = (props: RenewalLinkProps) => {
    ...
    const [manualInterventionRequired, setManualInterventionRequired] =
        useState(false);

    const [openContactSales] = useOpenSalesLink();

    useEffect(() => {
        Client4.getRenewalLink()
            .then(({ renewal_link: renewalLinkParam }) => {
                try {
                    if (
                        renewalLinkParam &&
                        /^http[s]?:\/\//.test(renewalLinkParam)
                    ) {
    ...
    let btnText = props.customBtnText ? (
        props.customBtnText
    ) : (
        <FormattedMessage
