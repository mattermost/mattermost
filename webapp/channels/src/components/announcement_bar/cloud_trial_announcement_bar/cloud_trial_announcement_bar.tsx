...
class CloudTrialAnnouncementBar extends React.PureComponent<Props> {
    ...
    render() {
        ...
        if (
            (preferences.some(
                (pref) =>
                    pref.name === CloudBanners.TRIAL &&
                    pref.value === MAX_DAYS_BANNER,
            ) &&
                daysLeftOnTrial > TrialPeriodDays.TRIAL_WARNING_THRESHOLD) ||
            (daysLeftOnTrial <= TrialPeriodDays.TRIAL_WARNING_THRESHOLD &&
                daysLeftOnTrial >= TrialPeriodDays.TRIAL_1_DAY &&
        ...
        if (this.props.reverseTrial) {
            const trialEnd = getLocaleDateFromUTC(
                (this.props.subscription?.trial_end_at as number) / 1000,
                "MMMM Do",
            );
        ...
        const userEndTrialDate = getLocaleDateFromUTC(
            (this.props.subscription?.trial_end_at as number) / 1000,
            "MMMM Do YYYY",
        );
        const userEndTrialHour = getLocaleDateFromUTC(
            (this.props.subscription?.trial_end_at as number) / 1000,
            "HH:mm",
            this.props.currentUser.timezone?.automaticTimezone as string,
        );
        ...
        if (
            daysLeftOnTrial >= TrialPeriodDays.TRIAL_2_DAYS &&
            daysLeftOnTrial <= TrialPeriodDays.TRIAL_WARNING_THRESHOLD
        ) {
        ...
        return (
            <AnnouncementBar
                type={
                    dismissable
                        ? AnnouncementBarTypes.ADVISOR
                        : AnnouncementBarTypes.CRITICAL
                }
