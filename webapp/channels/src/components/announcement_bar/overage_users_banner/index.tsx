...
const OverageUsersBanner = () => {
    ...
    const isSelfHostedExpansionEnabled =
        useSelector(getConfig)?.ServiceSettings?.SelfHostedPurchase;
    const canSelfHostedExpand =
        useCanSelfHostedExpand() && isSelfHostedExpansionEnabled;
    ...
    const handleUpdateSeatsSelfServeClick = (
        ...
        if (canSelfHostedExpand) {
            window.open(
                `${siteURL}/${ConsolePages.LICENSE}?action=show_expansion_modal`,
            );
            return;
        }
    ...
    const handleClick = isExpandable
        ? handleUpdateSeatsSelfServeClick
        : handleContactSalesClick;

    if (
        !hasPermission ||
        adminHasDismissed({
            isWarningBanner: isBetween5PercerntAnd10PercentPurchasedSeats,
            overagePreferences,
            preferenceName,
        })
    ) {
    ...
    return (
        <AnnouncementBar
            ...
            showCTA={
                getRequestState !== "IDLE" && getRequestState !== "LOADING"
            }
        />
    );
};
