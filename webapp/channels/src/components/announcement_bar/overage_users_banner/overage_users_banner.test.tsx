...
const seatsMinimumFor5PercentageState =
    Math.ceil(seatsPurchased * OverActiveUserLimits.MIN) + seatsPurchased;

const seatsMinimumFor10PercentageState =
    Math.ceil(seatsPurchased * OverActiveUserLimits.MAX) + seatsPurchased;

const text5PercentageState = `(Only visible to admins) Your workspace user count has exceeded your paid license seat count by ${
    seatsMinimumFor5PercentageState - seatsPurchased
} seats. Purchase additional seats to remain compliant.`;
const text10PercentageState = `(Only visible to admins) Your workspace user count has exceeded your paid license seat count by ${
    seatsMinimumFor10PercentageState - seatsPurchased
} seats. Purchase additional seats to remain compliant.`;
...
describe("components/overage_users_banner", () => {
    ...
    const renderComponent = (
        { store }: RenderComponentArgs = { props: {}, store: initialState },
    ) => {
        return renderWithIntlAndStore(<OverageUsersBanner />, store);
    };
    ...
    it("should not render the banner because we are not on overage state", () => {
        ...
        expect(
            screen.queryByText(
                "(Only visible to admins) Your workspace user count has exceeded your paid license seat count by",
                { exact: false },
            ),
    ...
    it("should not render the banner because we are not admins", () => {
        ...
        expect(
            screen.queryByText(
                "Your workspace user count has exceeded your paid license seat count by",
                { exact: false },
            ),
        ).not.toBeInTheDocument();
        expect(getLicenseSelfServeStatus).not.toBeCalled();
    ...
    it("should not render the 5% banner because we have dissmised it", () => {
        ...
        store.entities.preferences.myPreferences =
        ...
        expect(
            screen.queryByText(text5PercentageState),
        ).not.toBeInTheDocument();
        expect(getLicenseSelfServeStatus).not.toBeCalled();
    });
    ...
    it("should track if the admin click Contact Sales CTA in a 10% overage state", () => {
        ...
        const salesLinkWithEncodedParams =
            "https://mattermost.com/contact-sales/?qk=&qp=&qw=&qx=dGVzdEBtYXR0ZXJtb3N0LmNvbQ==&utm_source=mattermost&utm_medium=in-product";
    ...
    it("should save the preferences for 5% banner if admin click on close", () => {
        ...
        expect(savePreferences).toBeCalledWith(
            store.entities.users.profiles.current_user.id,
            [
                {
                    category: Preferences.OVERAGE_USERS_BANNER,
                    name: `warn_overage_seats_${licenseId.substring(0, 8)}`,
