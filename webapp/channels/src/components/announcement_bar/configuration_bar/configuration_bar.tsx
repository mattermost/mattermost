...
const ConfigurationAnnouncementBar = (props: Props) => {
    ...
    const getNoticeForWarnMetric = (warnMetricStatus: any) => {
        ...
        if (warnMetricStatus.acked) {
            message = (
                <>
                    <img className="advisor-icon" src={ackIcon} />
                    <FormattedMessage
            ...
            if (
                ...
                isDismissed =
                    props.dismissedNumberOfActiveUsersWarnMetricStatusAck;
            } else if (
                warnMetricStatus.id ===
                WarnMetricTypes.SYSTEM_WARN_METRIC_NUMBER_OF_POSTS_2M
            ) {
            ...
            if (
                ...
                isDismissed =
                    props.dismissedNumberOfActiveUsersWarnMetricStatus;
            } else if (
                warnMetricStatus.id ===
                WarnMetricTypes.SYSTEM_WARN_METRIC_NUMBER_OF_POSTS_2M
            ) {
    ...
    if (props.canViewSystemErrors) {
        if (
            (isLicensePastGracePeriod(props.license) ||
                isLicenseExpired(props.license)) &&
            !props.dismissedExpiredLicense
        ) {
            const message = (
                <>
                    <img className="advisor-icon" src={warningIcon} />
                    <FormattedMessage
                        ...
                        values={{
                            licenseSku: getSkuDisplayName(
                                props.license.SkuShortName,
                                props.license.IsGovSku === "true",
                            ),
                        }}
                    />
                </>
        ...
        if (
            isTrialLicense(props.license) &&
            typeof daysUntilLicenseExpires !== "undefined" &&
            daysUntilLicenseExpires <= 14 &&
            !props.dismissedExpiringTrialLicense
        ) {
        ...
        if (
            !isTrialLicense(props.license) &&
            isLicenseExpiring(props.license) &&
            !props.dismissedExpiringLicense
        ) {
            const message = (
                <>
                    <img className="advisor-icon" src={alertIcon} />
                    <FormattedMessage
                        ...
                        values={{
                            date: new Date(
                                parseInt(props.license?.ExpiresAt, 10),
                            ),
                            licenseSku: getSkuDisplayName(
                                props.license.SkuShortName,
                                props.license.IsGovSku === "true",
        ...
        if (props.license?.IsLicensed === "false" && props.warnMetricsStatus) {
    ...
    if (
        props.config?.SendEmailNotifications !== "true" &&
    ...
    if (props.canViewSystemErrors && props.config?.SiteURL === "") {
        ...
        const values: Record<string, ReactNode> = {
            ...
            linkConsole: (msg: string) => (
                <Link to="/admin_console/environment/web_server">{msg}</Link>
            ),
        };
