...
import NotifyAdminDowngradeDeliquencyBar, {
    BannerPreferenceName,
} from "./index";
...
describe("components/announcement_bar/notify_admin_downgrade_delinquency_bar", () => {
    ...
    it("Should not show banner when the user has notify their admin", () => {
        ...
        state.entities.preferences.myPreferences =
            TestHelper.getPreferencesMock([
                {
                    category:
                        Preferences.NOTIFY_ADMIN_REVOKE_DOWNGRADED_WORKSPACE,
    ...
    it("Should not show banner when the user is an admin", () => {
        ...
        const state = JSON.parse(JSON.stringify(initialState));
        state.entities.users.profiles.current_user_id = {
            roles: "system_admin",
        };
    ...
    it("Should save the preferences if the user close the banner", async () => {
        ...
        expect(savePreferences).toBeCalledWith(
            initialState.entities.users.profiles.current_user_id.id,
            [
                {
                    category:
                        Preferences.NOTIFY_ADMIN_REVOKE_DOWNGRADED_WORKSPACE,
    ...
    it("Should save the preferences and track event after notify their admin", async () => {
        ...
        await waitFor(() => {
            ...
            expect(trackEvent).toHaveBeenNthCalledWith(
                ...
                "click_notify_admin_upgrade_workspace_banner",
            );
            expect(trackEvent).toHaveBeenNthCalledWith(
                2,
                TELEMETRY_CATEGORIES.CLOUD_DELINQUENCY,
                "notify_admin_downgrade_delinquency_bar",
                undefined,
            );
        });
    });
