...
import {renderWithContext, userEvent, screen} from 'tests/vitest_react_testing_utils';
...
describe('NotificationPermissionBar', () => {
    ...
    test('should call requestNotificationPermission and hide the bar when the button is clicked in NotificationPermissionNeverGrantedBar', async () => {
        vi.spyOn(utilsNotifications, 'isNotificationAPISupported').mockReturnValue(true);
        vi.spyOn(utilsNotifications, 'getNotificationPermission').mockReturnValue(utilsNotifications.NotificationPermissionNeverGranted);
        vi.spyOn(utilsNotifications, 'requestNotificationPermission').mockResolvedValue(utilsNotifications.NotificationPermissionGranted);
