...
const ShowThreeDaysLeftTrialModal = () => {
    ...
    const isUserAdmin = useSelector((state: GlobalState) =>
    ...
    const hadAdminDismissedModal =
        useSelector((state: GlobalState) =>
            getPreference(
                state,
                Preferences.CLOUD_TRIAL_BANNER,
                CloudBanners.THREE_DAYS_LEFT_TRIAL_MODAL_DISMISSED,
            ),
        ) === "true";
    ...
    const someLimitNeedsAttention = Boolean(
        useGetHighestThresholdCloudLimit(useGetUsage(), useGetLimits()[0]),
    );
    ...
    useEffect(() => {
        if (
            subscription?.trial_end_at === undefined ||
