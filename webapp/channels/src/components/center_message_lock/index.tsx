...
export default function CenterMessageLock(props: Props) {
    ...
    const oldestPostTime =
        useSelector((state: GlobalState) =>
            getOldestPostTimeInChannel(state, props.channelId || ""),
        ) || getNextDay(props.firstInaccessiblePostTime);
    const [notifyAdminBtnText, notifyAdmin, notifyRequestStatus] =
        useNotifyAdmin(
            {
    ...
    let title = intl.formatMessage(
        {
            ...
            defaultMessage:
                "Notify your admin to unlock messages prior to {date} in {team}",
        },
        titleValues,
    );
    let description: React.ReactNode = intl.formatMessage(
        {
            ...
            defaultMessage:
                "Some older messages may not be shown because your workspace has over {limit} messages. Select Notify Admin to send an automatic request to your System Admins to upgrade.",
        },
        {
            limit,
        },
    ...
    let cta = (
        <button
            ...
            onClick={(e) =>
                notifyAdmin(e, "center_channel_posts_over_limit_banner")
            }
        ...
        description = intl.formatMessage(
            {
                ...
                defaultMessage:
                    "To view and search all of the messages in your workspace’s history, rather than just the most recent {limit} messages, upgrade to one of our paid plans. <a>Review our plan options and pricing.</a>",
            },
            {
                limit,
                a: (chunks: React.ReactNode | React.ReactNodeArray) => (
                    <a
                        ...
                        onClick={(e: React.MouseEvent) => {
                            ...
                            openPricingModal({
                                trackingLocation:
                                    "center_channel_posts_over_limit_banner",
                            });
                        }}
                    >
                        {chunks}
    ...
    return (
        <div className="CenterMessageLock">
            ...
            <div className="CenterMessageLock__right">
                <div className="CenterMessageLock__title">{title}</div>
                <div className="CenterMessageLock__description">
                    {description}
                </div>
                <div className="CenterMessageLock__cta">{cta}</div>
            </div>
        </div>
    );
}
