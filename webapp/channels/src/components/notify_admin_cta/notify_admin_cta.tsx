...
import {
    NotifyStatus,
    useGetNotifyAdmin,
} from "components/common/hooks/useGetNotifyAdmin";
...
export function useNotifyAdmin<T = HTMLAnchorElement | HTMLButtonElement>(
    props: HookProps,
    reqData: NotifyAdminRequest,
): [
    React.ReactNode,
    (e: React.MouseEvent<T, MouseEvent>, callerInfo: string) => void,
    ValueOf<typeof NotifyStatus>,
] {
    const {
        btnText: btnFormaText,
        notifyAdmin,
        notifyStatus,
    } = useGetNotifyAdmin({});
    ...
    const btnText = (status: NotifyStatusValues): React.ReactNode => {
        switch (status) {
            ...
            case NotifyStatus.Success:
                return (
                    props.successText ||
                    formatMessage(btnFormaText(NotifyStatus.Success))
                );
            ...
            default:
                return (
                    props.ctaText ||
                    formatMessage(btnFormaText(NotifyStatus.NotStarted))
                );
        }
    };
...
function NotifyAdminCTA(props: Props) {
    const [btnText, notify, status] = useNotifyAdmin(
        props,
        props.notifyRequestData,
    );
    const { formatMessage } = useIntl();
    let title = formatMessage({
        id: "pricing_modal.wantToUpgrade",
        defaultMessage: "Want to upgrade? ",
    });
    if (props.preTrial) {
        title = formatMessage({
            id: "pricing_modal.wantToTry",
            defaultMessage: "Want to try? ",
        });
    }
