...
const InstallAndConfigureLink = (props: {
    pluginId: string;
    pluginName: string;
}) => {
    const installedListing = useSelector(
        getInstalledListing,
    ) as MarketplacePlugin[];
    const error = useSelector((state: GlobalState) =>
        getError(state, props.pluginId),
    );

    const isInstalled = installedListing.some(
        (plugin) => plugin.manifest.id === props.pluginId,
    );
    const installing = useSelector((state: GlobalState) =>
        getInstalling(state, props.pluginId),
    );
    if (installing) {
        return (
            <span
...
export default function OpenPluginInstallPost(props: { post: Post }) {
    ...
    const [pluginsByPluginIds, setPluginsByPluginIds] =
        useState<RequestedPlugins>({});

    const postProps = props.post.props as CustomPostProps;
    const requestedPluginsByPluginIds =
        postProps?.requested_plugins_by_plugin_ids;
    ...
    const getUserIdsForUsersThatRequestedFeature = (
        requests: PluginRequest[],
    ): string[] => requests.map((request: PluginRequest) => request.user_id);
    ...
    useEffect(() => {
        ...
        if (
            ...
            !Object.keys(pluginsByPluginIds).length
        ) {
            ...
            for (const pluginId of Object.keys(requestedPluginsByPluginIds)) {
                plugins[pluginId] = requestedPluginsByPluginIds[pluginId].map(
                    (currPlugin: PluginRequest) => {
                        return {
                            ...currPlugin,
                            plugin_name:
                                mPlugins[pluginId].manifest.name || pluginId,
                            plugin_id: pluginId,
                        };
                    },
                );
                dispatch(
                    getMissingProfilesByIds(
                        getUserIdsForUsersThatRequestedFeature(
                            requestedPluginsByPluginIds[pluginId],
                        ),
                    ),
                );
            }
    ...
    const createUsernameMessage = (requests: PluginRequest[]) => {
        ...
        if (users.length === 1) {
            ...
            users.push(
                formatMessage({
                    id: "postypes.custom_open_pricing_modal_post_renderer.and",
                    defaultMessage: "and",
                }) +
                    " " +
                    lastUser,
    ...
    const getUserNamesForUsersThatRequestedFeature = (
        requests: PluginRequest[],
    ): string[] => {
    ...
    const getUserNameForUser = (userId: string) => {
        const unknownName = formatMessage({
            id: "postypes.custom_open_pricing_modal_post_renderer.unknown",
        ...
        if (userIds.length === 1 && pluginIds.length === 1) {
            ...
            messageBuilder.push(
                " " +
                    formatMessage(
                        {
                            id: "postypes.custom_open_plugin_install_post_rendered.plugin_request",
                            defaultMessage:
                                "requested installing the {pluginRequests} app.",
                        },
                        { pluginRequests: pluginName },
                    ),
            );
            ...
            const message = formatList(messageBuilder, {
                style: "narrow",
                type: "unit",
            });
            post = (
                <>
                    <Markdown
                        ...
                        userIds={getUserIdsForUsersThatRequestedFeature(
                            requestedPluginsByUserIds[userIds[0]],
                        )}
                    />{" "}
                    {instructions}
                </>
            ...
            messageBuilder.push(
                formatMessage({
                    id: "postypes.custom_open_plugin_install_post_rendered.app_installation_request_text",
                    defaultMessage:
                        "You’ve received the following app installation requests:",
                }),
            );
            const pluginIds = Object.keys(pluginsByPluginIds);

            post = (
                <ul style={usersListStyle} key={pluginIds.join("")}>
                    {pluginIds.map((pluginId) => {
                        const plugins = pluginsByPluginIds[pluginId];
                        const uniqueUserRequestsForPlugins = uniqWith(
                            plugins,
                            (one, two) => one.user_id === two.user_id,
                        );
                        const installRequests = [];
                        installRequests.push(
                            createUsernameMessage(uniqueUserRequestsForPlugins),
                        );
                        installRequests.push(
                            " " +
                                formatMessage(
                                    ...
                                    {
                                        pluginRequests:
                                            uniqueUserRequestsForPlugins[0]
                                                .plugin_name,
                                    },
                                ),
                        );
                    ...
                    values={{
                        marketplaceLink: (text: string) => (
                            <ToggleModalButton
                                ...
                                dialogType={MarketplaceModal}
                                dialogProps={{
                                    openedFrom: "open_plugin_install_post",
                                }}
                            >
                                {text}
                            </ToggleModalButton>
    ...
    return <div>{customMessageBody}</div>;
}
