...
export class InvitationModal extends React.PureComponent<Props, State> {
    defaultState: State = deepFreeze({
        ...
        invite: initializeInviteState(
            this.props.initialValue || "",
            this.props.inviteAsGuest,
        ),
    ...
    constructor(props: Props) {
        ...
        const defaultStateChannels =
            this.defaultState.invite.inviteChannels.channels;
        ...
        this.state = {
            ...
            invite: {
                ...
                inviteType:
                    !props.canAddUsers && props.canInviteGuests
                        ? InviteType.GUEST
                        : this.defaultState.invite.inviteType,
                inviteChannels: {
                    ...this.defaultState.invite.inviteChannels,
                    channels: props.channelToInvite
                        ? [...defaultStateChannels, props.channelToInvite]
    ...
    invite = async () => {
        ...
        if (inviteAs === InviteType.MEMBER && this.props.isCloud) {
            trackEvent("cloud_invite_users", "click_send_invitations", {
        ...
        if (inviteAs === InviteType.MEMBER) {
            if (this.props.channelToInvite) {
                ...
                const result =
                    await this.props.actions.sendMembersInvitesToChannels(
                        this.props.currentTeam.id,
                        this.state.invite.inviteChannels.channels,
                        users,
                        emails,
                        this.state.invite.customMessage.open
                            ? this.state.invite.customMessage.message
                            : "",
                    );
                invites = result.data;
            } else {
                const result = await this.props.actions.sendMembersInvites(
                    this.props.currentTeam.id,
                    users,
                    emails,
                );
    ...
    debouncedSearchChannels = debounce(
        (term) =>
            this.props.actions.searchChannels(this.props.currentTeam.id, term),
        150,
    );
    ...
    channelsLoader = async (value: string) => {
        ...
        return this.props.invitableChannels.filter((channel) => {
            return (
                ...
                channel.name.toLowerCase().startsWith(value.toLowerCase())
            );
        });
    };
    ...
    debouncedSearchProfiles = debounce(
        (term: string, callback: (users: UserProfile[]) => void) => {
            this.props.actions
                .searchProfiles(term)
    ...
    getBackdrop = (): Backdrop => {
        ...
        // false means no backdrop
        if (
            this.state.view === View.RESULT ||
            (!this.props.canAddUsers && !this.props.canInviteGuests)
        ) {
            return true;
        }

        const emptyInvites =
            this.state.invite.usersEmails.length === 0 &&
            this.state.invite.usersEmailsSearch === "";
        if (!emptyInvites) {
            return "static";
        } else if (this.state.invite.inviteType === InviteType.GUEST) {
            if (
                this.state.invite.inviteChannels.channels.length !== 0 ||
                this.state.invite.inviteChannels.search !== ""
            ) {
                return "static";
            }
        ...
        let view = (
            <InviteView
                ...
                regenerateTeamInviteId={
                    this.props.actions.regenerateTeamInviteId
                }
