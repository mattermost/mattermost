...
import {
    getTrackFlowRole,
    getRoleForTrackFlow,
    getSourceForTrackFlow,
} from "utils/utils";

import AddToChannels, {
    defaultCustomMessage,
    defaultInviteChannels,
} from "./add_to_channels";
...
export const initializeInviteState = (
    initialSearchValue = "",
    inviteAsGuest = false,
): InviteState => {
    return deepFreeze({
...
export default function InviteView(props: Props) {
    useEffect(() => {
        ...
        props.regenerateTeamInviteId,
    ]);

    const { formatMessage } = useIntl();

    const inviteURL = useMemo(() => {
        return `${getSiteURL()}/signup_user_complete/?id=${
            props.currentTeam.invite_id
        }&md=link&sbr=${getTrackFlowRole()}`;
    }, [props.currentTeam.invite_id]);

    const copyText = useCopyText({
        trackCallback: () =>
            trackEvent(
                getAnalyticsCategory(props.isAdmin),
                "click_copy_invite_link",
                { ...getRoleForTrackFlow(), ...getSourceForTrackFlow() },
            ),
        text: inviteURL,
    });
    ...
    if (props.usersEmails.length > Constants.MAX_ADD_MEMBERS_BATCH) {
        ...
        errorProperties.errorMessageDefault =
            "No more than **{text}** people can be invited at once";
        errorProperties.errorMessageValues.text =
            Constants.MAX_ADD_MEMBERS_BATCH.toString();
    }
        ...
        <>
            ...
            <Modal.Body>
                ...
                <UsersEmailsInput
                    ...
                    validAddressMessageDefault={
                        props.inviteType === InviteType.MEMBER
                            ? "Invite **{email}** as a team member"
                            : "Invite **{email}** as a guest"
                    }
            ...
            <Modal.Footer
                className={classNames("InviteView__footer", props.footerClass, {
                    "InviteView__footer-guest":
                        props.inviteType === InviteType.GUEST,
                })}
            >
