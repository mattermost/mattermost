...
describe('AddToChannels', () => {
    describe('placeholder selection', () => {
        it('should use townSquareDisplayName when not in a channel', async () => {
            const props = {...baseProps, currentChannel: undefined};
            await renderWithContext(<AddToChannels {...props}/>);
            expect(screen.getByText(props.townSquareDisplayName, {exact: false})).toBeInTheDocument();
        });

        it('should use townSqureDisplayName when not in a public or private channel', async () => {
            const props = {...baseProps, currentChannel: {type: 'D', display_name: ''} as Channel};
            await renderWithContext(<AddToChannels {...props}/>);
            expect(screen.getByText(props.townSquareDisplayName, {exact: false})).toBeInTheDocument();
        });

        it('should use the currentChannel display_name when in a channel', async () => {
    ...
    describe('custom message', () => {
        it('UI to toggle custom message opens it when closed', async () => {
            ...
            const {container} = await renderWithContext(<AddToChannels {...props}/>);
        ...
        it('UI to write custom message calls the on change handler with its input', async () => {
            const props = {
                ...baseProps,
                setCustomMessage: jest.fn(),
                customMessage: {
