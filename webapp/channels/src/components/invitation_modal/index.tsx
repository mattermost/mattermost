...
const InvitationModal = makeAsyncComponent(
    "InvitationModal",
    React.lazy(() => import("./invitation_modal")),
);
...
export function mapStateToProps(state: GlobalState, props: OwnProps) {
    ...
    const townSquareDisplayName =
        channelsByName[Constants.DEFAULT_CHANNEL]?.display_name ||
        Constants.DEFAULT_CHANNEL_UI_NAME;

    const currentTeamId = getCurrentTeamId(state);
    const currentTeam =
        currentTeamId === "" && props.channelToInvite
            ? getTeam(state, props.channelToInvite.team_id)
    ...
    const canInviteGuests =
        ...
        haveICurrentTeamPermission(state, Permissions.INVITE_GUEST);
    const isCloud = license.Cloud === "true";

    const canAddUsers = haveICurrentTeamPermission(
        state,
...
function mapDispatchToProps(dispatch: Dispatch<GenericAction>) {
    return {
        actions: bindActionCreators<
            ActionCreatorsMapObject<ActionFunc | CloseModalType>,
            Actions
        >(
            {
                sendGuestsInvites,
