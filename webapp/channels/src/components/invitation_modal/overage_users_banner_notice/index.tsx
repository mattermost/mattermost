...
const adminHasDismissed = ({
    preferenceName,
    overagePreferences,
}: AdminHasDismissedArgs): boolean => {
    return (
        overagePreferences.find((value) => value.name === preferenceName) !==
        undefined
    );
};
...
const OverageUsersBannerNotice = () => {
    ...
    const stats =
        useSelector((state: GlobalState) => state.entities.admin.analytics) ||
        {};
    ...
    const overagePreferences = useSelector((state: GlobalState) =>
        getPreferencesCategory(state, Preferences.OVERAGE_USERS_BANNER),
    );
    ...
    const canSelfHostedExpand =
        useCanSelfHostedExpand() && isSelfHostedPurchaseEnabled;
    ...
    const { cta, expandableLink, trackEventFn, getRequestState, isExpandable } =
        useExpandOverageUsersCheck({
            shouldRequest:
                hasPermission &&
                !adminHasDismissed({ overagePreferences, preferenceName }),
    ...
    if (canSelfHostedExpand) {
        message = (
            <FormattedMessage
                ...
                defaultMessage={
                    "<a>Purchase additional seats</a> to remain compliant."
                }
                values={{
        ...
        message = (
            <FormattedMessage
                ...
                values={{
                    a: () => {
                        ...
                        const handleClick = () => {
                            trackEventFn(
                                isExpandable ? "Self Serve" : "Contact Sales",
                            );
                        };
            ...
            >
                {(text) => (
                    <p className="overage_users_banner__description">{text}</p>
                )}
            </FormattedMessage>
        );
    }
