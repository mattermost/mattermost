...
const text5PercentageState = `Your workspace user count has exceeded your paid license seat count by ${
    seatsMinimumFor5PercentageState - seatsPurchased
} seats`;
const text10PercentageState = `Your workspace user count has exceeded your paid license seat count by ${
    seatsMinimumFor10PercentageState - seatsPurchased
} seats`;
const notifyText =
    "Notify your Customer Success Manager on your next true-up check";
...
describe("components/invitation_modal/overage_users_banner_notice", () => {
    ...
    const renderComponent = (
        { store }: RenderComponentArgs = { store: initialState },
    ) => {
        return renderWithIntlAndStore(<OverageUsersBannerNotice />, store);
    };
    ...
    it("should not render the banner because we are not on overage state", () => {
        ...
        expect(
            screen.queryByText(notifyText, { exact: false }),
    ...
    it("should render the banner because we are over 5% and we don't have any preferences", () => {
        ...
        expect(
            screen.getByText(notifyText, { exact: false }),
        ).toBeInTheDocument();
    });
    ...
    it("should not render the banner because we are over 10% and we have preferences", () => {
        ...
        expect(
            screen.queryByText(text10PercentageState),
        ).not.toBeInTheDocument();
        expect(
            screen.queryByText(notifyText, { exact: false }),
        ).not.toBeInTheDocument();
