// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

import React from 'react';
import {MemoryRouter, Route} from 'react-router-dom';

import {renderWithContext, screen} from 'tests/vitest_react_testing_utils';

import ThreadPopout from './thread_popout';

vi.mock('components/threading/global_threads/thread_pane', () => ({
    __esModule: true,
    default: ({children, thread}: {children: React.ReactNode; thread: unknown}) => (
        <div
            data-testid='thread-pane'
            data-thread-id={(thread as {id?: string})?.id}
        >
            {children}
        </div>
    ),
}));

vi.mock('components/threading/thread_viewer', () => ({
    __esModule: true,
    default: ({rootPostId, useRelativeTimestamp, isThreadView}: {rootPostId: string; useRelativeTimestamp: boolean; isThreadView: boolean}) => (
        <div
            data-testid='thread-viewer'
            data-root-post-id={rootPostId}
            data-use-relative-timestamp={useRelativeTimestamp?.toString()}
            data-is-thread-view={isThreadView?.toString()}
        >
            {'Thread Viewer'}
        </div>
    ),
}));

vi.mock('components/unreads_status_handler', () => ({
    __esModule: true,
    default: () => <div data-testid='unreads-status-handler'>{'Unreads Status Handler'}</div>,
}));

vi.mock('mattermost-redux/actions/channels', () => ({
    fetchChannelsAndMembers: vi.fn().mockReturnValue(() => ({type: 'FETCH_CHANNELS_AND_MEMBERS'})),
    selectChannel: vi.fn().mockReturnValue(() => ({type: 'SELECT_CHANNEL'})),
}));

vi.mock('mattermost-redux/actions/teams', () => ({
    selectTeam: vi.fn().mockReturnValue(() => ({type: 'SELECT_TEAM'})),
}));

vi.mock('mattermost-redux/actions/threads', () => ({
    getThread: vi.fn().mockReturnValue(() => ({type: 'GET_THREAD'})),
}));

vi.mock('mattermost-redux/actions/users', () => ({
    getProfiles: vi.fn().mockReturnValue(() => ({type: 'GET_PROFILES'})),
}));

const mockMarkThreadAsRead = vi.fn().mockReturnValue(() => ({type: 'MARK_THREAD_AS_READ'}));
vi.mock('actions/views/threads', () => ({
    markThreadAsRead: vi.fn((...args: unknown[]) => mockMarkThreadAsRead(...args)),
}));

vi.mock('utils/popouts/popout_windows', () => ({
    isThreadPopoutWindow: vi.fn().mockReturnValue(true),
}));

describe('ThreadPopout', () => {
    const mockPost = {
        id: 'post-123',
        channel_id: 'channel-123',
        message: 'Test post',
    };

    const mockTeam = {
        id: 'team-123',
        name: 'test-team',
        display_name: 'Test Team',
    };

    const baseState = {
        entities: {
            channels: {
                currentChannelId: 'channel-123',
                channels: {
                    'channel-123': {
                        id: 'channel-123',
                        name: 'test-channel',
                        display_name: 'Test Channel',
                        type: 'O' as const,
                        delete_at: 0,
                    },
                },
            },
            teams: {
                currentTeamId: 'team-123',
                teams: {
                    'team-123': mockTeam,
                },
            },
            users: {
                currentUserId: 'user-123',
            },
            posts: {
                posts: {
                    'post-123': mockPost,
                },
            },
            threads: {
                threads: {
                    'post-123': {
                        id: 'post-123',
                        last_reply_at: 1000,
                    },
                },
            },
        },
        views: {
            threads: {
                manuallyUnread: {},
            },
        },
    };

    beforeEach(() => {
        vi.clearAllMocks();
        Object.defineProperty(window, 'isActive', {
            writable: true,
            value: false,
        });
        vi.spyOn(window, 'addEventListener').mockImplementation(vi.fn());
        vi.spyOn(window, 'removeEventListener').mockImplementation(vi.fn());
    });

    afterEach(() => {
        vi.restoreAllMocks();
    });

    it('should render nothing when thread is not available', () => {
        const stateWithoutPost = {
            ...baseState,
            entities: {
                ...baseState.entities,
                posts: {
                    posts: {},
                },
            },
        };

        const {container} = renderWithContext(
            <MemoryRouter initialEntries={['/_popout/thread/test-team/post-123']}>
                <ThreadPopout/>
            </MemoryRouter>,
            stateWithoutPost,
        );

        expect(container.firstChild).toBeNull();
    });

    it('should pass correct props to ThreadPane', () => {
        renderWithContext(
            <MemoryRouter initialEntries={['/_popout/thread/test-team/post-123']}>
                <Route
                    path='/_popout/thread/:team/:postId'
                    component={ThreadPopout}
                />
            </MemoryRouter>,
            baseState,
        );

        const threadPane = screen.getByTestId('thread-pane');
        expect(threadPane).toHaveAttribute('data-thread-id', 'post-123');
    });

    it('should pass correct props to ThreadViewer', () => {
        renderWithContext(
            <MemoryRouter initialEntries={['/_popout/thread/test-team/post-123']}>
                <Route
                    path='/_popout/thread/:team/:postId'
                    component={ThreadPopout}
                />
            </MemoryRouter>,
            baseState,
        );

        const threadViewer = screen.getByTestId('thread-viewer');
        expect(threadViewer).toHaveAttribute('data-root-post-id', 'post-123');
        expect(threadViewer).toHaveAttribute('data-use-relative-timestamp', 'true');
        expect(threadViewer).toHaveAttribute('data-is-thread-view', 'true');
    });

    it('should handle missing post gracefully', () => {
        const stateWithoutPost = {
            ...baseState,
            entities: {
                ...baseState.entities,
                posts: {
                    posts: {},
                },
            },
        };

        const {container} = renderWithContext(
            <MemoryRouter initialEntries={['/_popout/thread/test-team/post-123']}>
                <ThreadPopout/>
            </MemoryRouter>,
            stateWithoutPost,
        );

        expect(container.firstChild).toBeNull();
    });

    describe('window focus and blur events', () => {
        it('should set window.isActive to true and call markThreadAsRead on focus', () => {
            let focusHandler: (() => void) | undefined;
            (window.addEventListener as ReturnType<typeof vi.fn>).mockImplementation((event: string, handler: () => void) => {
                if (event === 'focus') {
                    focusHandler = handler;
                }
            });

            renderWithContext(
                <MemoryRouter initialEntries={['/_popout/thread/test-team/post-123']}>
                    <Route
                        path='/_popout/thread/:team/:postId'
                        component={ThreadPopout}
                    />
                </MemoryRouter>,
                baseState,
            );

            expect(screen.getByTestId('thread-pane')).toBeInTheDocument();
            expect(focusHandler).toBeDefined();

            if (focusHandler) {
                Object.defineProperty(window, 'isActive', {
                    writable: true,
                    value: false,
                });
                focusHandler();
            }

            expect((window as Window & {isActive?: boolean}).isActive).toBe(true);
            expect(mockMarkThreadAsRead).toHaveBeenCalledWith('post-123');
        });

        it('should set window.isActive to false on blur', () => {
            let blurHandler: (() => void) | undefined;
            (window.addEventListener as ReturnType<typeof vi.fn>).mockImplementation((event: string, handler: () => void) => {
                if (event === 'blur') {
                    blurHandler = handler;
                }
            });

            renderWithContext(
                <MemoryRouter initialEntries={['/_popout/thread/test-team/post-123']}>
                    <Route
                        path='/_popout/thread/:team/:postId'
                        component={ThreadPopout}
                    />
                </MemoryRouter>,
                baseState,
            );

            expect(screen.getByTestId('thread-pane')).toBeInTheDocument();
            expect(blurHandler).toBeDefined();

            if (blurHandler) {
                Object.defineProperty(window, 'isActive', {
                    writable: true,
                    value: true,
                });
                blurHandler();
            }

            expect((window as Window & {isActive?: boolean}).isActive).toBe(false);
        });
    });
});
