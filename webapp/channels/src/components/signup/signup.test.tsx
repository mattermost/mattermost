...
jest.mock("mattermost-redux/selectors/entities/general", () => ({
    ...
    ) as typeof import("mattermost-redux/selectors/entities/general")),
...
jest.mock("mattermost-redux/selectors/entities/users", () => ({
    ...
    ) as typeof import("mattermost-redux/selectors/entities/users")),
    getCurrentUserId: () => "",
}));
...
jest.mock("actions/team_actions", () => ({
    ...
    ) as typeof import("actions/team_actions")),
...
jest.mock("mattermost-redux/actions/users", () => ({
    ...
    ) as typeof import("mattermost-redux/actions/users")),
...
jest.mock("actions/views/login", () => ({
    ...(jest.requireActual(
        "actions/views/login",
    ) as typeof import("actions/views/login")),
...
describe("components/signup/Signup", () => {
    ...
    it("should create user, log in and redirect to invite teamname", async () => {
        ...
        expect(wrapper.find("#input_name").first().props().disabled).toEqual(
            true,
        );
        expect(wrapper.find(PasswordInput).first().props().disabled).toEqual(
            true,
        );

        expect(mockHistoryPush).toHaveBeenCalledWith(
            "/should_verify_email?email=jdoe%40mm.com&teamname=teamName",
        );
    });
    ...
    it("should handle failure adding user to team when team invite and logged in", () => {
        ...
        setTimeout(() => {
            ...
            expect(wrapper.find(".content-layout-column-title").text()).toEqual(
    ...
    it("should show newsletter check box opt-in for self-hosted non airgapped workspaces", async () => {
        jest.spyOn(useCWSAvailabilityCheckAll, "default").mockImplementation(
            () => true,
        );
        mockLicense = { IsLicensed: "true", Cloud: "false" };
        ...
        const checkInput = screen.getByTestId(
            "signup-body-card-form-check-newsletter",
        );
        expect(checkInput).toHaveAttribute("type", "checkbox");

        expect(signupContainer).toHaveTextContent(
            "I would like to receive Mattermost security updates via newsletter. By subscribing, I consent to receive emails from Mattermost with product updates, promotions, and company news. I have read the Privacy Policy and understand that I can unsubscribe at any time",
        );
    });
