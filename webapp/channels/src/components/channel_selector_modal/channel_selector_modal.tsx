...
> {
    ...
    componentDidMount() {
        this.props.actions
            .loadChannels(
                ...
                this.props.groupID,
                false,
                this.props.excludePolicyConstrained,
            )
            .then((response) => {
                this.setState({
                    channels: response.data.sort(compareChannels),
                });
        ...
        if (prevProps.searchTerm !== this.props.searchTerm) {
            ...
            if (searchTerm === "") {
                ...
                this.searchTimeoutId = window.setTimeout(async () => {
                    ...
                    const response = await this.props.actions.searchAllChannels(
                        searchTerm,
                        { not_associated_to_group: this.props.groupID },
                    );
        ...
        if (page > prevPage) {
            ...
            this.props.actions
                ...
                .then((response) => {
                    ...
                    const stateChannelIDs = this.state.channels.map(
                        (stateChannel) => stateChannel.id,
                    );
    ...
    search = (
        term: string,
        multiselectComponent: MultiSelect<ChannelWithTeamDataValue>,
    ) => {
    ...
    render() {
        ...
        let options = this.state.channels.map(
            (i): ChannelWithTeamDataValue => ({
                ...i,
        ...
        if (this.props.excludeTeamIds) {
            options = options.filter(
                (channel) =>
                    this.props.excludeTeamIds?.indexOf(channel.team_id) === -1,
            );
        }
        ...
        return (
            <Modal
                dialogClassName={
                    "a11y__modal more-modal more-direct-channels channel-selector-modal"
                }
