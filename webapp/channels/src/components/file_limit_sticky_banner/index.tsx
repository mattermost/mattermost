...
function FileLimitStickyBanner() {
    ...
    const snoozePreferenceVal = useSelector((state: GlobalState) =>
        selectPreference(
            state,
            Preferences.CLOUD_USER_EPHEMERAL_INFO,
            "file_limit_banner_snooze",
        ),
    );

    let shouldShowAgain = true;
    if (snoozePreferenceVal !== "") {
        const snoozeInfo = JSON.parse(
            snoozePreferenceVal,
        ) as FileLimitSnoozePreference;
    ...
    if (
        fileStorageLimit === undefined ||
        !(currentFileStorageUsage > fileStorageLimit)
    ) {
        return null;
    }
    ...
    const title = (
        <FormattedMessage
            ...
            defaultMessage={
                "Your free plan is limited to {storageGB} of files."
            }
            values={{
                ...
                a: (chunks: React.ReactNode) => (
                    <NotifyAdminCTA
                        ...
                        notifyRequestData={{
                            ...
                            required_feature:
                                MattermostFeatures.UNLIMITED_FILE_STORAGE,
    ...
    const tooltip = (
        <Tooltip id="file_limit_banner_snooze">
            {formatMessage(
                {
                    id: "create_post.file_limit_sticky_banner.snooze_tooltip",
                    defaultMessage: "Snooze for {snoozeDays} days",
                },
                { snoozeDays: snoozeCoolOffDays },
            )}
        </Tooltip>
    );
