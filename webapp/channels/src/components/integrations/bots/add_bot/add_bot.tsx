...
export default class AddBot extends React.PureComponent<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this.state = {
            ...
            role: UserUtils.isSystemAdmin(this.props.roles || "")
                ? roleOptionSystemAdmin
                : roleOptionMember,
            postAll: this.props.bot
                ? UserUtils.hasPostAllRole(this.props.roles || "")
                : false,
            postChannels: this.props.bot
                ? UserUtils.hasPostAllPublicRole(this.props.roles || "")
                : false,
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = (e2) => {
                const orientation = FileUtils.getExifOrientation(
                    e2.target?.result as ArrayBuffer,
                );
    ...
    handleSubmit = async (
        ...
        if (this.props.bot) {
            ...
            if (usernameError) {
                let errObj;
                if (
                    usernameError.id === ValidationErrors.INVALID_LAST_CHARACTER
                ) {
                    errObj = {
                        adding: false,
                        error: (
            ...
            if (!error && data) {
                ...
                const tokenResult =
                    await this.props.actions.createUserAccessToken(
                        data.user_id,
    ...
    render() {
        let subtitle = (
            <FormattedMessage id="bots.manage.add.add" defaultMessage="Add" />
        );
        let buttonText = (
            <FormattedMessage
        ...
        if (this.props.bot && !this.state.pictureFile) {
            if (this.props.user) {
                imageURL = Utils.imageURLForUser(
                    this.props.user.id,
