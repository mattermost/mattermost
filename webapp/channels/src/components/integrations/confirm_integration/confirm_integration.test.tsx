...
describe("components/integrations/ConfirmIntegration", () => {
    ...
    const getSearchString = (type: string, identifier = id) =>
    ...
    const oauthApps = {
        [id]: oauthApp,
    } as unknown as IDMappedObjects<OAuthApp>;
    const incomingHooks: IDMappedObjects<IncomingWebhook> = {
        [id]: TestHelper.getIncomingWebhookMock({ id }),
    };
    const outgoingHooks = {
        [id]: { id, token },
    } as unknown as IDMappedObjects<OutgoingWebhook>;
    ...
    test("should match snapshot, oauthApps case", () => {
        ...
        const wrapper = shallow(<ConfirmIntegration {...props} />);
    ...
    test("should match callback URLs of OAuth Apps", () => {
        ...
        expect(container.querySelector(".word-break--all")).toHaveTextContent(
            "URL(s): https://someCallback, https://anotherCallback",
        );
    });
