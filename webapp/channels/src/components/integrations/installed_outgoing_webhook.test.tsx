...
import InstalledOutgoingWebhook, {
    matchesFilter,
} from "components/integrations/installed_outgoing_webhook";
...
describe("components/integrations/InstalledOutgoingWebhook", () => {
    ...
    test("Should not display description as it is null", () => {
        const newOutgoingWebhook = TestHelper.getOutgoingWebhookMock({
            ...outgoingWebhook,
    ...
    test("Should match snapshot of makeDisplayName", () => {
        ...
        expect(
            instance.makeDisplayName(
                TestHelper.getOutgoingWebhookMock({
                    display_name: "hook display name",
                }),
    ...
    test("Should match result when matchesFilter is called", () => {
        ...
        expect(
            matchesFilter(
                TestHelper.getOutgoingWebhookMock({ display_name: undefined }),
                TestHelper.getChannelMock(),
                "word",
            ),
        ).toEqual(false);
        expect(
            matchesFilter(
                TestHelper.getOutgoingWebhookMock({ description: undefined }),
                TestHelper.getChannelMock(),
                "word",
            ),
        ).toEqual(false);
        expect(
            matchesFilter(
                TestHelper.getOutgoingWebhookMock({ trigger_words: undefined }),
                TestHelper.getChannelMock(),
                "word",
            ),
        ).toEqual(false);
        expect(
            matchesFilter(
                TestHelper.getOutgoingWebhookMock(),
                TestHelper.getChannelMock({ name: undefined }),
                "channel",
            ),
        ).toEqual(false);
        ...
        expect(
            matchesFilter(
                TestHelper.getOutgoingWebhookMock({ display_name: "Word" }),
                TestHelper.getChannelMock(),
                "word",
            ),
        ).toEqual(true);
        ...
        expect(
            matchesFilter(
                TestHelper.getOutgoingWebhookMock(),
                TestHelper.getChannelMock({ name: "channel_name" }),
                "channel",
            ),
        ).toEqual(true);
    });
