...
import {
    ...
    getChannelsMemberCount as getChannelsMemberCountSelector,
...
const getChannelsWithoutArchived = createSelector(
    ...
    (channels: Channel[]) =>
        channels &&
        channels.filter(
            (c) => c.delete_at === 0 && c.type !== Constants.PRIVATE_CHANNEL,
        ),
);
...
const getPrivateChannelsSelector = createSelector(
    ...
    (channels: Channel[]) =>
        channels &&
        channels.filter((c) => c.type === Constants.PRIVATE_CHANNEL),
);

function mapStateToProps(state: GlobalState) {
    const team = getCurrentTeam(state) || {};
    const getGlobalItem = makeGetGlobalItem(
    ...
    return {
        ...
        canShowArchivedChannels:
            getConfig(state).ExperimentalViewArchivedChannels === "true",
