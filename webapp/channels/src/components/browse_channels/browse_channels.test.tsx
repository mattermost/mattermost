...
import BrowseChannels, {
    Filter,
} from "components/browse_channels/browse_channels";
...
describe("components/BrowseChannels", () => {
    ...
    test("should match snapshot and state", () => {
        ...
        expect(
            wrapper.instance().props.actions.getChannels,
        ).toHaveBeenCalledTimes(1);
        expect(
            wrapper.instance().props.actions.getChannels,
        ).toHaveBeenCalledWith(wrapper.instance().props.teamId, 0, 100);
    });
    ...
    test("should match state on onChange", () => {
        ...
        wrapper.setState({
            searchedChannels: [
                TestHelper.getChannelMock({ id: "other_channel_id" }),
            ],
        });
    ...
    test("should call props.getChannels on nextPage", () => {
        ...
        expect(
            wrapper.instance().props.actions.getChannels,
        ).toHaveBeenCalledWith(wrapper.instance().props.teamId, 2, 50);
    });

    test("should have loading prop true when searching state is true", () => {
        const wrapper = shallow(<BrowseChannels {...baseProps} />);
    ...
    test("should attempt to join the channel and fail", (done) => {
        ...
        const wrapper = shallow<BrowseChannels>(<BrowseChannels {...props} />);

        const callback = jest.fn();
        wrapper.instance().handleJoin(baseProps.channels[0], callback);
        expect(
            wrapper.instance().props.actions.joinChannel,
        ).toHaveBeenCalledTimes(1);
        expect(
            wrapper.instance().props.actions.joinChannel,
        ).toHaveBeenCalledWith(
            wrapper.instance().props.currentUserId,
            wrapper.instance().props.teamId,
            baseProps.channels[0].id,
        );
        process.nextTick(() => {
    ...
    test("should handle a failed search", (done) => {
        ...
        expect(
            wrapper.instance().props.actions.searchAllChannels,
        ).toHaveBeenCalledTimes(1);
        expect(
            wrapper.instance().props.actions.searchAllChannels,
