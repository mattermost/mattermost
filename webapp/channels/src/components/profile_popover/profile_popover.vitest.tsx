...
vi.mock('@mattermost/client', async () => {
    const actual = await vi.importActual('@mattermost/client');
    return {
        ...actual,
        Client4: class MockClient4 extends (actual as any).Client4 {
            getCallsChannelState = vi.fn();
...
vi.mock('./profile_popover_other_user_row', async () => {
    const React = await import('react');
    const {default: RealProfilePopoverAddToChannel} = await import('./profile_popover_add_to_channel');
    const {default: RealProfilePopoverCallButtonWrapper} = await import('./profile_popover_call_button_wrapper');

    return {
        default: function MockProfilePopoverOtherUserRow(props: ComponentProps<any>) {
...
describe('components/ProfilePopover', () => {
    beforeEach(() => {
        vi.mocked(Client4.getCallsChannelState as any).mockImplementation(async () => ({enabled: true}));
    });
    ...
    test('should display attributes if attribute exists for user', async () => {
        const [props, initialState] = getBasePropsAndState();
        vi.mocked(Client4.getUserCustomProfileAttributesValues as any).mockImplementation(async () => {
            return {
                123: 'Private',
                456: 'Seargent York',
            };
