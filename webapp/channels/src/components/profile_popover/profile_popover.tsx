...
export interface ProfilePopoverProps
    extends Omit<React.ComponentProps<typeof Popover>, "id"> {
    /**
...
> {
        ...
        if (this.props.returnFocus) {
            ...
            this.returnFocus = () => {
                document.dispatchEvent(
                    new CustomEvent<A11yFocusEventDetail>(
        ...
        actions
            ...
            .then((result: { error: ServerError }) => {
                if (!result.error) {
                    ...
                    this.props.hide?.();
                    getHistory().push(
                        `${this.props.teamUrl}/messages/@${user.username}`,
                    );
                }
            });
    ...
    render() {
        ...
        const keysToBeRemoved: Array<keyof ProfilePopoverProps> = [
            "user",
            "userId",
            "channelId",
            "src",
        ...
        const popoverProps: React.ComponentProps<typeof Popover> =
            Utils.deleteKeysFromObject({ ...this.props }, keysToBeRemoved);
        const { formatMessage } = this.props.intl;
        const dataContent = [];
        const urlSrc = this.props.overwriteIcon
            ? this.props.overwriteIcon
            : this.props.src;
        dataContent.push(
            <div className="user-popover-image" key="user-popover-image">
                <Avatar
                    id="userAvatar"
                    size="xxl"
                ...
                <StatusIcon
                    ...
                    status={
                        this.props.hideStatus ? undefined : this.props.status
                    }
                    button={true}
                />
            </div>,
        ...
        if (
            this.props.enableLastActiveTime &&
        ...
        const fullname = this.props.overwriteName
            ? this.props.overwriteName
            : Utils.getFullName(this.props.user);
        ...
        const userNameClass = classNames("overflow--ellipsis pb-1", {
            "user-profile-popover__heading": fullname === "",
        });
        ...
        dataContent.push(
            <hr key="user-popover-hr" className="divider divider--expanded" />,
        );
        ...
        const customStatusAndExpiryContent =
            !haveOverrideProp && this.renderCustomStatus();
        if (customStatusAndExpiryContent) {
            const { customStatusContent, expiryContent } =
                customStatusAndExpiryContent;
            dataContent.push(
                <div
        ...
        const renderCallButton = () => {
            const {
                ...
                isCallsCanBeDisabledOnSpecificChannels,
                dMChannel,
            } = this.props;
            if (
                !isCallsEnabled ||
                ...
                (!isCallsDefaultEnabledOnAllChannels &&
                    !isCallsCanBeDisabledOnSpecificChannels &&
                    this.state.callsChannelState?.enabled === false)
            ) {
                return null;
            }

            const disabled =
                this.props.isUserInCall || this.props.isCurrentUserInCall;
            const startCallMessage = this.props.isUserInCall
                ? formatMessage(
                      {
            ...
            const iconButtonClassname = classNames("btn icon-btn", {
        ...
        if (this.props.user.is_bot) {
            roleTitle = <BotTag className="user-popover__role" size={"sm"} />;
        } else if (isGuest(this.props.user.roles)) {
            roleTitle = <GuestTag className="user-popover__role" size={"sm"} />;
        ...
        const title = (
            <span
                data-testid={`profilePopoverTitle_${this.props.user.username}`}
            >
                {roleTitle}
                <button
        ...
        const titleClassName = classNames("popover-title", {
            "popover-title_height": !roleTitle,
        });
        ...
        return (
            <Popover {...popoverProps} id="user-profile-popover">
                {tabCatcher}
                <div
                    role="dialog"
                    aria-label={formatMessage(
