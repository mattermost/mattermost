...
function makeMapStateToProps() {
    ...
    return (
        state: GlobalState,
        { userId, channelId = getDefaultChannelId(state) }: OwnProps,
    ) => {
        ...
        return {
            ...
            canManageAnyChannelMembersInCurrentTeam:
                canManageAnyChannelMembersInCurrentTeam(state),
            status,
            ...
            isUserInCall: callsEnabled
                ? checkUserInCall(state, userId)
                : undefined,
            isCurrentUserInCall: callsEnabled
                ? checkUserInCall(state, currentUserId)
                : undefined,
            isCallsDefaultEnabledOnAllChannels: callsConfig?.DefaultEnabled,
            isCallsCanBeDisabledOnSpecificChannels:
                callsConfig?.AllowEnableCalls,
            dMChannel: getChannelByName(
                state,
                getDirectChannelName(currentUserId, userId),
            ),
