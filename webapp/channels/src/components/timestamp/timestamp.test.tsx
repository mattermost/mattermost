...
describe("components/timestamp/Timestamp", () => {
    ...
    test("should be wrapped in SemanticTime and support passthrough className and label", () => {
        ...
        expect(wrapper.find(SemanticTime).prop("aria-label")).toBe(
            "test label",
        );
    });
    ...
    test("should not be wrapped in SemanticTime", () => {
        const wrapper = shallowWithIntl(
            <Timestamp useTime={false} useSemanticOutput={false} />,
        );
        expect(wrapper).toMatchSnapshot();
        expect(wrapper.find(SemanticTime).exists()).toBeFalsy();
    });

    test.each(moment.tz.names())(
        "should render supported timezone %p",
        (timeZone) => {
    ...
    test("should render title-case Today", () => {
        const wrapper = mountWithIntl(
            <Timestamp
                useTime={false}
                ranges={[RelativeRanges.TODAY_TITLE_CASE]}
            />,
        );
    ...
    test("should render title-case Yesterday", () => {
        const wrapper = mountWithIntl(
            <Timestamp
                value={daysFromNow(-1)}
                useTime={false}
                ranges={[RelativeRanges.YESTERDAY_TITLE_CASE]}
            />,
        );
    ...
    test("should render normal tomorrow", () => {
        const wrapper = mountWithIntl(
            <Timestamp value={daysFromNow(1)} useTime={false} unit="day" />,
        );
    ...
    test("should render 3 days ago", () => {
        const wrapper = mountWithIntl(
            <Timestamp value={daysFromNow(-3)} useTime={false} unit="day" />,
        );
    ...
    test("should render 3 days ago as weekday", () => {
        ...
        const wrapper = mountWithIntl(
            <Timestamp value={date} useTime={false} />,
        );
    ...
    test("should render 2 days ago as weekday in supported timezone", () => {
        ...
        const wrapper = mountWithIntl(
            <Timestamp value={date} timeZone="Asia/Manila" useTime={false} />,
        );

        expect(wrapper.text()).toEqual(
            moment.utc(date).tz("Asia/Manila").format("dddd"),
        );
    });
    ...
    test("should render time without timezone, in military time", () => {
        ...
        expect(wrapper.find("time").prop("dateTime")).toBe(
            "2018-01-12T15:15:13.000",
        );
