...
export const supportsHourCycle = Boolean(
    (
        new Intl.DateTimeFormat("en-US", {
            hour: "numeric",
        }).resolvedOptions() as DateTimeOptions
    ).hourCycle,
);
...
function is12HourTime(
    hourCycle: DateTimeOptions["hourCycle"],
    hour12?: DateTimeOptions["hour12"],
) {
...
function isRelative(
    format: ResolvedFormats["relative"],
): format is RelativeOptions {
...
const defaultRefreshIntervals = new Map<
...
function normalizeRangeDescriptor(
    unit: NonNullable<Props["units"]>[number],
): RangeDescriptor {
...
export type Props = FormatOptions & {
    ...
    useRelative?: Resolvable<
        ResolvedFormats["relative"],
        { value: Date },
        FormatOptions
    >;
    units?: Array<
        ...
        | UnitDescriptor
        | Intl.RelativeTimeFormatUnit
        | keyof typeof STANDARD_UNITS
    >;
    ranges?: Props["units"];
    useDate?: Resolvable<
        Exclude<ResolvedFormats["date"], "timeZone"> | false,
        { value: Date },
        FormatOptions
    >;
    useTime?: Resolvable<
        | Exclude<ResolvedFormats["time"], "timeZone" | "hourCycle" | "hour12">
        | false,
        { value: Date },
        FormatOptions
    >;
...
class Timestamp extends PureComponent<Props, State> {
    ...
    formatParts(
        ...
        try {
            ...
            if (timeFormat) {
                const {
                    hourCycle,
                    hour12 = supportsHourCycle
                        ? undefined
                        : is12HourTime(hourCycle),
                } = this.props;

                time = this.formatDateTime(value, {
                    hourCycle,
                    hour12,
                    ...timeFormat,
                });
            ...
            return {
                date:
                    dateFormat &&
                    Timestamp.momentDate(momentValue, { ...dateFormat }),
                time:
                    timeFormat &&
                    Timestamp.momentTime(momentValue, {
                        hourCycle,
                        hour12,
                        ...timeFormat,
                    }),
    ...
    formatRelative(
        value: Date,
        { unit, relNearest, truncateEndpoints, ...format }: RelativeOptions,
    ): string {
        let diff: number;
    ...
    formatDateTime(value: Date, format: DateTimeOptions): string {
        ...
        return new Intl.DateTimeFormat(locale, {
            timeZone,
            ...format,
        } as any).format(value); // TODO remove any when React-Intl is next updated
    }

    static momentTime(
        value: Moment,
        { hour, minute, hourCycle, hour12 }: DateTimeOptions,
    ): string | undefined {
        if (hour && minute) {
            return value.format(
                is12HourTime(hourCycle, hour12) ? "h:mm A" : "HH:mm",
            );
        }
        return undefined;
    }
    ...
    static momentDate(
        value: Moment,
        { weekday, day, month, year }: DateTimeOptions,
    ): string | undefined {
    ...
    autoRange(
        value: Date,
        units: Props["units"] = this.props.units || this.props.ranges,
    ): DisplayAs {
        return (
            units?.map(normalizeRangeDescriptor).find(({ equals, within }) => {
                if (equals != null) {
                    return isEqual(
                        value,
                        this.state.now,
    ...
    private getFormats(value: Date): ResolvedFormats {
        const {
            ...
            useRelative = (): ResolvedFormats["relative"] => {
                const {
                    display,
                    updateIntervalInSeconds = this.props
                ...
                if (display) {
                    ...
                    if (unit) {
                        return {
                            ...
                            updateIntervalInSeconds:
                                updateIntervalInSeconds ??
                                defaultRefreshIntervals.get(unit),
                            capitalize,
                        };
                    }
                }
    ...
    private maybeUpdate(
        relative: ResolvedFormats["relative"],
    ...
    static formatLabel(value: Date, timeZone?: string) {
        ...
        return (
            momentValue.toString() + (timeZone ? ` (${momentValue.tz()})` : "")
        );
    }
    ...
    render() {
        ...
        if (children) {
            return resolve(
                children,
                { value, timeZone, formatted, ...parts },
                formats,
            );
        }
