...
const mapStateToProps = (state: GlobalState, ownProps: OwnProps) => ({
    ...
    inGlobalThreads: Boolean(
        matchPath(ownProps.location.pathname, {
            path: "/:team/threads/:threadIdentifier?",
        }),
    ),
    inDrafts: Boolean(
        matchPath(ownProps.location.pathname, { path: "/:team/drafts" }),
    ),
});
...
export default withRouter(
    connect(mapStateToProps, mapDispatchToProps)(ChannelHeaderMobile),
);
