...
type DescriptionStatusKey =
    | ValueOf<typeof LimitTypes>
    | "noLimits"
    | "multipleLimits";
...
const DescriptionMessages: Record<DescriptionStatusKey, JSX.Element> = {
    noLimits: (
        ...
        >
            {(text) => (
                <p className="DelinquencyModal__body__limits-information">
                    {text}
                </p>
            )}
        </FormattedMessage>
    ...
    [LimitTypes.messageHistory]: (
        <FormattedMessage
            ...
            defaultMessage={
                "Some of your workspace's message history are no longer accessible. Upgrade to a paid plan and get unlimited access to your message history."
            }
        >
            {(text) => (
                <p className="DelinquencyModal__body__limits-information">
    ...
    [LimitTypes.fileStorage]: (
        <FormattedMessage
            ...
            defaultMessage={
                "Some of your workspace's files are no longer accessible. Upgrade to a paid plan and get unlimited access to your files."
            }
        >
            {(text) => (
                <p className="DelinquencyModal__body__limits-information">
...
const getDescriptionKey = (
    limits: Array<ValueOf<typeof LimitTypes>>,
): DescriptionStatusKey => {
    if (limits.length > 1) {
...
export const FreemiumModal = ({
    ...
    const handleReactivate = () => {
        ...
        trackEvent(
            ...
            "clicked_re_activate_plan",
        );
        openPurchaseModal({
            trackingLocation: "delinquency_modal_freemium_admin",
        });
    };
    ...
    const description = (
        <>
            ...
            >
                {(text) => (
                    <span className="DelinquencyModal__body__subheader">
                        {text}
                    </span>
                )}
            </FormattedMessage>
