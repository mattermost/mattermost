...
import {
    getShortcutReactToLastPostEmittedFrom,
...
function isFirstReply(post: Post, previousPost?: Post | null): boolean {
    if (post.root_id) {
        if (previousPost) {
            ...
            return (
                post.root_id !== previousPost.id &&
                post.root_id !== previousPost.root_id
            );
        }
...
function makeMapStateToProps() {
    ...
    return function mapStateToProps(state: GlobalState, ownProps: OwnProps) {
        ...
        if (post.root_id) {
            ...
            parentPostUser = parentPost
                ? getUser(state, parentPost.user_id)
                : null;
        }
        ...
        const shortcutReactToLastPostEmittedFrom =
            getShortcutReactToLastPostEmittedFrom(state);
        ...
        let emojis: Emoji[] = [];
        const oneClickReactionsEnabled =
            get(
                state,
        ...
        if (
            ...
            memberships &&
            Object.values(memberships).length > 1 // Not show if the user only belongs to one team
        ) {
        ...
        return {
            ...
            isFirstReply: previousPost
                ? isFirstReply(post, previousPost)
                : false,
            ...
            isFlagged:
                get(state, Preferences.CATEGORY_FLAGGED_POST, post.id, null) !==
                null,
            compactDisplay:
                get(
                    state,
            ...
            colorizeUsernames:
                get(
                    state,
                    Preferences.CATEGORY_DISPLAY_SETTINGS,
                    Preferences.COLORIZE_USERNAMES,
                    Preferences.COLORIZE_USERNAMES_DEFAULT,
                ) === "true",
            ...
            shouldHighlight:
                ownProps.shouldHighlight || highlightedPostId === post.id,
            ...
            isPostBeingEdited:
                ownProps.location === Locations.CENTER
                    ? !getIsPostBeingEditedInRHS(state, post.id) &&
                      getIsPostBeingEdited(state, post.id)
                    : getIsPostBeingEditedInRHS(state, post.id),
