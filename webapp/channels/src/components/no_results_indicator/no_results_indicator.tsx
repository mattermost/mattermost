...
const iconMap: { [key in NoResultsVariant]: React.ReactNode } = {
    [NoResultsVariant.ChannelSearch]: (
        <SearchIcon className="no-results__icon" />
    ),
    ...
    [NoResultsVariant.ChannelFiles]: (
        <i className="icon icon-file-text-outline no-results__icon" />
    ),
    [NoResultsVariant.ChannelFilesFiltered]: (
        <i className="icon icon-file-text-outline no-results__icon" />
    ),
    [NoResultsVariant.UserGroups]: (
        <i className="icon icon-account-multiple-outline no-results__icon" />
    ),
    [NoResultsVariant.UserGroupMembers]: (
        <i className="icon icon-account-outline no-results__icon" />
    ),
...
const NoResultsIndicator = ({
    ...
    variant,
    iconGraphic = variant ? (
        <div className="no-results__variant-wrapper">{iconMap[variant]}</div>
    ) : null,
    titleValues,
    title = variant ? (
        <FormattedMessage {...titleMap[variant]} values={titleValues} />
    ) : null,
    subtitleValues,
    subtitle = variant ? (
        <FormattedMessage {...subtitleMap[variant]} values={subtitleValues} />
    ) : null,
    ...
    let content = (
        <div
            className={classNames("no-results__wrapper", {
                "horizontal-layout": layout === NoResultsLayout.Horizontal,
            })}
            style={style}
        >
            {iconGraphic}
