...
const isFormEmpty = (formDataState: FormDataState) => {
    if (formDataState.other) {
        return (
            isEmptyInput(formDataState.otherPaymentOption) &&
            !formDataState.wire &&
            !formDataState.ach
        );
    }

    return Object.values(formDataState).every(
        (value) => value === false || value == null,
    );
};

export const GatherIntentModal = ({
    onClose,
    onSave,
    isSubmitting,
    showError,
    ...
    return (
        <>
            <Modal.Header className="AltPaymentsModal__header ">
                ...
                >
                    {(text) => <h3 className="Form-section-title">{text}</h3>}
                </FormattedMessage>
                <button
                    id="closeIcon"
            ...
            <Modal.Body>
                ...
                >
                    ...
                    >
                        {(text) => (
                            <p className="AltPaymentsModal__body__question">
                                {text}
                            </p>
                        )}
                    </FormattedMessage>
                        ...
                        <div className="AltPaymentsModal__body__option">
                            <textarea
                                ...
                                placeholder={intl.formatMessage({
                                    id: "payment_form.gather_wire_transfer_intent_modal.otherPaymentOptionPlaceholder",
                                    defaultMessage: "Enter payment option here",
                                })}
                                rows={2}
                                maxLength={400}
                            />
                    ...
                    {showError && (
                        <div className="AltPaymentsModal__body__error">
                            ...
                            >
                                {(text) => (
                                    <span className="AltPaymentsModal__body__error__text">
                                        {text}
                                    </span>
                                )}
                            </FormattedMessage>
