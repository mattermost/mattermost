...
export default class SidebarRight extends React.PureComponent<Props, State> {
    ...
    handleShortcut = (e: KeyboardEvent) => {
        ...
        if (cmdOrCtrlPressed(e, true)) {
            if (e.shiftKey && isKeyPressed(e, Constants.KeyCodes.PERIOD)) {
                ...
                isKeyPressed(e, Constants.KeyCodes.I) &&
                (channelInfoShortcutMac || channelInfoShortcut)
            ) {
                e.preventDefault();
    ...
    componentDidUpdate(prevProps: Props) {
        ...
        const { actions, isChannelFiles, isPinnedPosts, rhsChannel, channel } =
            this.props;
        if (
            isPinnedPosts &&
            prevProps.isPinnedPosts === isPinnedPosts &&
            rhsChannel.id !== prevProps.rhsChannel.id
        ) {
        ...
        if (
            isChannelFiles &&
            prevProps.isChannelFiles === isChannelFiles &&
            rhsChannel.id !== prevProps.rhsChannel.id
        ) {
    ...
    handleClickOutside = (e: MouseEvent) => {
        if (
            this.props.isOpen &&
            this.props.isExpanded && // can be collapsed
            e.target && // has target
            ...
            !document
                ...
                ?.contains(e.target as Element) && // not within Global Header
