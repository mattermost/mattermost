...
import AppBarPluginComponent, {
    isAppBarPluginComponent,
} from "./app_bar_plugin_component";

import "./app_bar.scss";

export default function AppBar() {
    const channelHeaderComponents = useSelector(
        getChannelHeaderPluginComponents,
    );
    ...
    const canOpenMarketplace = useSelector(
        (state: GlobalState) =>
    ...
    if (!enabled || (currentProduct && !currentProduct.showAppBar)) {
        return null;
    }

    const coreProductsPluginIds = [
        suitePluginIds.focalboard,
        suitePluginIds.playbooks,
    ];
    ...
    const items: ReactNode[] = [
        ...
        if (isAppBarPluginComponent(x)) {
            if (
                !inScope(
                    x.supportedProductIds ?? null,
                    currentProductId,
                    currentProduct?.pluginId,
                )
            ) {
                return null;
            }
            return <AppBarPluginComponent key={x.id} component={x} />;
        } else if (isAppBinding(x)) {
            if (
                !inScope(
                    x.supported_product_ids ?? null,
                    currentProductId,
                    currentProduct?.pluginId,
                )
            ) {
            ...
            return <AppBarBinding key={`${x.app_id}_${x.label}`} binding={x} />;
        }
        return x;
    });
    ...
    return (
        <div className={"app-bar"}>
            <div className={"app-bar__top"}>{items}</div>
            {canOpenMarketplace && (
...
const getDivider = (beforeCount: number, afterCount: number) =>
    beforeCount && afterCount ? (
        <hr key="divider" className="app-bar__divider" />
    ) : null;
