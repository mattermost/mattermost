...
export type Props = {
    ...
    actions: {
        getGroups: (opts: GetGroupsParams) => Promise<{ data: Group[] }>;
        setModalSearchTerm: (term: string) => void;
        getGroupsByUserIdPaginated: (
            opts: GetGroupsForUserParams,
        ) => Promise<{ data: Group[] }>;
        searchGroups: (params: GroupSearchParams) => Promise<{ data: Group[] }>;
    };
};
...
const UserGroupsModal = (props: Props) => {
    ...
    const getGroups = useCallback(
        async (page: number, groupType: string) => {
            ...
            if (groupType === "all") {
                ...
                data =
                    await actions.getGroupsByUserIdPaginated(groupsUserParams);
        ...
        [
            ...
            props.actions.getGroupsByUserIdPaginated,
            props.currentUserId,
        ],
    );
        ...
        >
            ...
            <Modal.Body>
                <div className="user-groups-search">
                    <Input
                        ...
                        placeholder={Utils.localizeMessage(
                            "user_groups_modal.searchGroups",
                            "Search Groups",
                        )}
                ...
                {groups.length === 0 && !props.searchTerm ? (
                    <>
                        <NoResultsIndicator variant={noResultsType} />
                        <ADLDAPUpsellBanner />
                    </>
                ) : (
                    <>
