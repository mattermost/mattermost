...
export default function ChannelMembersDropdown({
    ...
    const handleRemoveFromChannel = async () => {
        ...
        if (user.id === currentUserId) {
            ...
            setRemoving(true);
            const { error } = await actions.removeChannelMember(
                channel.id,
                user.id,
            );
            setRemoving(false);
    ...
    const updateChannelMemberSchemeRole = async (schemeAdmin: boolean) => {
        const { error } = await actions.updateChannelMemberSchemeRoles(
            channel.id,
            user.id,
            true,
            schemeAdmin,
    ...
    const isChannelAdmin =
        UserUtils.isChannelAdmin(channelMember.roles) ||
    ...
    const canRemoveUserFromChannel =
        canRemoveMember &&
        (!channel.group_constrained || user.is_bot) &&
        (!isDefaultChannel || isGuest);
    const removeFromChannelText =
    ...
    const removeFromChannelTestId =
        user.id === currentUserId ? "leaveChannel" : "removeFromChannel";

    if (
        canMakeUserChannelMember ||
        canMakeUserChannelAdmin ||
