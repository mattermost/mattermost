...
describe("components/threading/global_threads/thread_item", () => {
    ...
    test("should report total number of replies", () => {
        ...
        ).toHaveProperty("id", "threading.numReplies");
        expect(
            wrapper.find(".activity MemoizedFormattedMessage").props(),
        ).toHaveProperty("values.totalReplies", 9);
    });
    ...
    test("should report unread messages", () => {
        ...
        expect(
            wrapper.find(".activity MemoizedFormattedMessage").props(),
        ).toHaveProperty("id", "threading.numNewReplies");
        expect(
            wrapper.find(".activity MemoizedFormattedMessage").props(),
        ).toHaveProperty("values.newReplies", 2);
    });
    ...
    test("should report unread mentions", () => {
        ...
        expect(
            wrapper.find(".activity MemoizedFormattedMessage").props(),
        ).toHaveProperty("values.newReplies", 5);
    });
    ...
    test("should call Utils.handleFormattedTextClick on click", () => {
        const wrapper = shallow(<ThreadItem {...props} />);
        const spy = jest
            .spyOn(Utils, "handleFormattedTextClick")
