...
import {setThreadFollow, patchThread} from 'mattermost-redux/actions/threads';
...
const ThreadPane = ({
    ...
    // Custom thread name editing handlers
    ...
    const saveThreadName = useCallback(() => {
        if (threadId && editedName !== thread.props?.custom_name) {
            dispatch(patchThread(threadId, {props: {custom_name: editedName.trim()}}));
        }
        setIsEditingName(false);
    }, [dispatch, threadId, editedName, thread.props?.custom_name]);
    ...
    const autoGeneratedName = post ? cleanMessageForDisplay(post.message, 30) : '';
            ...
            return (
                <>
                    ...
                    <div className='ThreadPane__header-title'>
                        <h3 className={classNames('ThreadPane__header-name', {'ThreadPane__header-name--editable': isCustomThreadNamesEnabled && !isEditingName})}>
                            <span className='icon-discord-thread ThreadPane__header-icon'/>
                            {isEditingName ? (
                                <input
                                    type='text'
                                    className='ThreadPane__header-name-input'
                                    value={editedName}
                                ...
                                <span
                                    className='ThreadPane__header-text'
                                    onClick={isCustomThreadNamesEnabled ? startEditingName : undefined}
                                    role={isCustomThreadNamesEnabled ? 'button' : undefined}
