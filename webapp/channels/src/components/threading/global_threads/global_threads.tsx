...
const GlobalThreads = () => {
    ...
    } = useRouteMatch<{ threadIdentifier?: string }>();
    ...
    const selectedThread = useSelector((state: GlobalState) =>
        getThread(state, threadIdentifier),
    );
    const selectedThreadId = useSelector(getSelectedThreadIdInCurrentTeam);
    const selectedPost = useSelector((state: GlobalState) =>
        getPost(state, threadIdentifier!),
    );
    const threadIds = useSelector(
        (state: GlobalState) =>
            getThreadOrderInCurrentTeam(state, selectedThread?.id),
        shallowEqual,
    );
    const unreadThreadIds = useSelector(
        (state: GlobalState) =>
            getUnreadThreadOrderInCurrentTeam(state, selectedThread?.id),
        shallowEqual,
    );
    ...
    const isOnlySelectedThreadInList = (list: string[]) => {
        return (
            selectedThreadId &&
            list.length === 1 &&
            list[0] === selectedThreadId
        );
    };

    const shouldLoadThreads =
        isEmpty(threadIds) || isOnlySelectedThreadInList(threadIds);
    const shouldLoadUnreadThreads =
        isEmpty(unreadThreadIds) || isOnlySelectedThreadInList(unreadThreadIds);
        ...
        >
            <Header
                ...
                subtitle={formatMessage({
                    ...
                    defaultMessage:
                        "Threads you’re participating in will automatically show here",
                })}
            />
