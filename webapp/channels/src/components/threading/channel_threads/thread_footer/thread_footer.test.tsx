...
describe("components/threading/channel_threads/thread_footer", () => {
    ...
    test("should report total number of replies", () => {
        ...
        const wrapper = mount(<ThreadFooter {...props} />, mountOptions);
        expect(wrapper).toMatchSnapshot();
        expect(wrapper.exists(".dot-unreads")).toBe(false);
        expect(
            wrapper.exists('FormattedMessage[id="threading.numReplies"]'),
        ).toBe(true);
    });
    ...
    test("should show unread indicator", () => {
        ...
        expect(wrapper).toMatchSnapshot();
        expect(wrapper.find(SimpleTooltip).find(".dot-unreads").exists()).toBe(
            true,
        );
    });
    ...
    test("should have avatars", () => {
        const { mountOptions } = mockStore(state);
        const wrapper = mount(<ThreadFooter {...props} />, mountOptions);
        expect(wrapper.find(Avatars).props()).toHaveProperty("userIds", [
            "5",
            "4",
            "3",
            "2",
    ...
    test("should have a timestamp", () => {
        const { mountOptions } = mockStore(state);
        const wrapper = mount(<ThreadFooter {...props} />, mountOptions);
        expect(wrapper.find(Timestamp).props()).toHaveProperty(
            "value",
    ...
    test("should have a follow button", () => {
        ...
        expect(wrapper.find(FollowButton).props()).toHaveProperty(
            "isFollowing",
            thread.is_following,
        );
    ...
    test("should match snapshot when a single message is followed", () => {
        ...
        const wrapper = mount(
            <ThreadFooter threadId="singlemessageid" />,
            mountOptions,
        );
