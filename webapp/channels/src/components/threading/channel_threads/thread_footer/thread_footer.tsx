...
function ThreadFooter({ threadId, replyClick }: Props) {
    ...
    const thread = useSelector((state: GlobalState) =>
        getThreadOrSynthetic(state, post),
    );

    useEffect(() => {
        if (
            threadIsSynthetic(thread) &&
            thread.is_following &&
    ...
    const participantIds = useMemo(
        () => (participants || []).map(({ id }) => id).reverse(),
        [participants],
    );
    ...
    const handleReply = useCallback(
        (e) => {
            ...
            e.stopPropagation();
            dispatch(
                selectPost({ id: threadId, channel_id: channelId } as Post),
            );
        },
    ...
    return (
        <div className="ThreadFooter">
            ...
            {participantIds && participantIds.length > 0 ? (
                <Avatars userIds={participantIds} size="sm" />
            ) : null}
            ...
            {Boolean(lastReplyAt) && (
                <Timestamp value={lastReplyAt} {...THREADING_TIME}>
                    {({ formatted }) => (
