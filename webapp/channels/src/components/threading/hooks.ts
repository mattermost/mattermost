...
export function useThreadRouting() {
    const matchParams = useParams<{
        team: string;
        threadIdentifier?: UserThread["id"];
    }>();
    const params = useMemo(
        () => matchParams,
        [matchParams.threadIdentifier, matchParams.team],
    );
    ...
    const select = useCallback(
        (threadId?: UserThread["id"]) => {
            return history.push(
                `/${params.team}/threads${threadId ? "/" + threadId : ""}`,
            );
        },
        [params.team],
    );
    ...
    const clear = useCallback(
        () => history.replace(`/${params.team}/threads`),
        [params.team],
    );

    const goToInChannel = useCallback(
        (threadId?: UserThread["id"], teamName: Team["name"] = params.team) => {
            return history.push(
                `/${teamName}/pl/${threadId ?? params.threadIdentifier}`,
            );
        },
