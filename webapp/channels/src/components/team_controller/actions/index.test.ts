...
describe("components/team_controller/actions", () => {
    ...
    describe("joinTeam", () => {
        test("should not allow joining a deleted team", async () => {
            const getTeamByNameFn = () => () =>
                Promise.resolve({ data: { ...testTeam, delete_at: 154545 } });
            jest.spyOn(teamsActions, "getTeamByName").mockImplementation(
                getTeamByNameFn,
            );

            const testStore = mockStore(initialState);
            const result = await testStore.dispatch(
                joinTeam(testTeam.name, false),
            );

            expect(result).toEqual({
                error: Error("Team not found or deleted"),
            });
        });

        test("should not allow joining a team when user cannot be added to it", async () => {
            const getTeamByNameFn = () => () =>
                Promise.resolve({ data: testTeam });
            jest.spyOn(teamsActions, "getTeamByName").mockImplementation(
                getTeamByNameFn,
            );

            const addUserToTeamFn = () => () =>
                Promise.resolve({
                    error: { message: "cannot add user to team" },
                });
            jest.spyOn(teamActions, "addUserToTeam").mockImplementation(
                addUserToTeamFn,
            );
