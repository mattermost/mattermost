...
export function initializeTeam(team: Team): NewActionFuncAsync<Team> {
...
export function joinTeam(teamname: string, joinedOnFirstLoad: boolean): NewActionFuncAsync<Team> {
    return async (dispatch, getState) => {
        const state = getState();
        const currentUser = getCurrentUser(state);
