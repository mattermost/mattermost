...
describe("components/activity_log_modal/ActivityLog", () => {
    ...
    test("should match snapshot with mobile props", () => {
        const mobileDeviceIdProps = Object.assign({}, baseProps, {
            currentSession: { ...baseProps.currentSession, device_id: "apple" },
        });
        const wrapper = shallow(<ActivityLog {...mobileDeviceIdProps} />);

        expect(wrapper).toMatchSnapshot();
    });

    test("submitRevoke is called correctly", () => {
        const wrapper = shallow<ActivityLog>(<ActivityLog {...baseProps} />);
    ...
    test("should match when isMobileSession is called", () => {
        ...
        expect(
            isMobileSession(TestHelper.getSessionMock({ device_id: "apple" })),
        ).toEqual(true);
        expect(
            isMobileSession(
                TestHelper.getSessionMock({ device_id: "android" }),
            ),
        ).toEqual(true);
        expect(
            isMobileSession(TestHelper.getSessionMock({ device_id: "none" })),
        ).toEqual(false);
    });
    ...
    test("should match when mobileSessionInfo is called", () => {
        ...
        expect(
            mobileSessionInfo(
                TestHelper.getSessionMock({ device_id: "apple" }),
            ),
        ).toEqual(apple);
        ...
        expect(
            mobileSessionInfo(
                TestHelper.getSessionMock({ device_id: "apple_rn" }),
            ),
        ).toEqual(appleRN);
        ...
        expect(
            mobileSessionInfo(
                TestHelper.getSessionMock({ device_id: "android_rn" }),
            ),
        ).toEqual(androidRN);
    });
});
