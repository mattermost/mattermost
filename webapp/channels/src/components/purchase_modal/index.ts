...
const PurchaseModal = makeAsyncComponent(
    "PurchaseModal",
    React.lazy(() => import("./purchase_modal")),
);

function mapStateToProps(state: GlobalState) {
    const subscription = state.entities.cloud.subscription;

    const isDelinquencyModal = Boolean(
        state.entities.cloud.subscription?.delinquent_since,
    );
    ...
    const contactSalesLink = getCloudContactSalesLink(
        firstName,
        lastName,
        companyName,
        customerEmail,
    ...
    const contactSupportLink = getCloudSupportLink(
        customerEmail,
        "Cloud purchase",
        "",
    ...
    return {
        ...
        isCloudDelinquencyGreaterThan90Days:
            isCloudDelinquencyGreaterThan90Days(state),
...
export default connect(
    mapStateToProps,
    mapDispatchToProps,
)(withGetCloudSubscription(PurchaseModal));
