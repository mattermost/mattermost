...
import SeatsCalculator, {
    errorInvalidNumber,
} from "components/seats_calculator";
 ...
 * @returns Product
 */
function findProductInDictionary(
    products: Record<string, Product> | undefined,
    ...
    if (keys.length > 1) {
        ...
        keys.forEach((key) => {
            if (productId && products[key].id === productId) {
                ...
                productSku &&
                products[key].sku === productSku &&
                products[key].recurring_interval === productRecurringInterval
            ) {
                currentProduct = products[key];
            }
        });
...
function getSelectedProduct(
    ...
    if (isDelinquencyModal && !isCloudDelinquencyGreaterThan90Days) {
        const currentProduct = findProductInDictionary(
            products,
            currentProductId,
            undefined,
    ...
    return findProductInDictionary(
        yearlyProducts,
        null,
...
class PurchaseModal extends React.PureComponent<Props, State> {
        ...
        this.state = {
            ...
            selectedProductPrice:
                getSelectedProduct(
                    ...
                    props.isDelinquencyModal,
                    props.isCloudDelinquencyGreaterThan90Days,
                )?.price_per_seat.toString() || null,
            usersCount: this.props.usersCount,
            seats: {
                quantity: this.props.usersCount.toString(),
                error:
                    this.props.usersCount.toString() === "0"
                        ? errorInvalidNumber
                        : null,
            },
    ...
    async componentDidMount() {
        if (isEmpty(this.state.currentProduct || this.state.selectedProduct)) {
            ...
            this.setState({
                ...
                selectedProduct: getSelectedProduct(
                    this.props.products,
                    this.props.yearlyProducts,
                    this.props.productId,
                    this.props.isDelinquencyModal,
                    this.props.isCloudDelinquencyGreaterThan90Days,
                ),
                selectedProductPrice:
                    getSelectedProduct(
                        this.props.products,
    ...
    isFormComplete = () => {
        let paymentInfoIsValid =
            areBillingDetailsValid(this.state.billingDetails) &&
            this.state.cardInputComplete;
        if (!this.state.billingSameAsShipping) {
            paymentInfoIsValid =
                paymentInfoIsValid &&
                areShippingDetailsValid(this.state.shippingAddress);
        }
    ...
    onShippingInput = (address: Address) => {
        this.setState(
            { shippingAddress: { ...this.state.shippingAddress, ...address } },
            this.isFormComplete,
        );
    };
            ...
            dialogProps: {
                confirmSwitchToYearlyFunc: () => {
                    this.handleSubmitClick(
                        this.props.callerCTA +
                            "> purchase_modal > confirm_switch_to_annual_modal > confirm_click",
                    );
    ...
    paymentFooterText = () => {
        ...
        if (
            ...
            this.state.currentProduct?.billing_scheme ===
                ...
                >
                    <div className="tooltipTitle">
                        <FormattedMessage
                            ...
                            id={
                                "admin.billing.subscription.proratedPayment.tooltipTitle"
                            }
                        />
                    </div>
                    <div className="tooltipText">
                        <FormattedMessage
                            defaultMessage={
                                "If you upgrade to {selectedProductName} from {currentProductName} mid-month, you will be charged a prorated amount for both plans."
                            }
                            id={
                                "admin.billing.subscription.proratedPayment.tooltipText"
                            }
                            values={{
                                beginDate: getNextBillingDate(),
                                selectedProductName:
            ...
            const prorratedPaymentText = (
                <div className="prorrated-payment-text">
                    ...
                    <FormattedMessage
                        defaultMessage={
                            "Prorated payment begins: {beginDate}. "
                        }
    ...
    purchaseScreenCard = () => {
                    ...
                    <DelinquencyCard
                        ...
                        buttonDetails={{
                            action: () =>
                                this.handleSubmitClick(
                                    this.props.callerCTA +
                                        "> purchase_modal > upgrade_button_click",
                                ),
                            text: localizeMessage(
                                "cloud_delinquency.cc_modal.card.reactivate",
                                "Re-active",
                            ),
                            customClass: this.state.paymentInfoIsValid
                                ? ButtonCustomiserClasses.special
                                : ButtonCustomiserClasses.grayed,
        ...
        const showPlanLabel =
            this.state.selectedProduct?.sku === CloudProducts.PROFESSIONAL;
        const { formatMessage, formatNumber } = this.props.intl;

        const checkIsYearlyProfessionalProduct = (
        ...
        const yearlyProductMonthlyPrice = formatNumber(
            parseInt(this.state.selectedProductPrice || "0", 10) / 12,
            { maximumFractionDigits: 2 },
        );

        const currentProductMonthly =
            this.state.currentProduct?.recurring_interval ===
            RecurringIntervals.MONTH;
        const currentProductProfessional =
            this.state.currentProduct?.sku === CloudProducts.PROFESSIONAL;
        const currentProductMonthlyProfessional =
            currentProductMonthly && currentProductProfessional;

        const cardBtnText = currentProductMonthlyProfessional
            ? formatMessage({
                  id: "pricing_modal.btn.switch_to_annual",
                  defaultMessage: "Switch to annual billing",
              })
            : formatMessage({
                  id: "pricing_modal.btn.upgrade",
        ...
        return (
            <>
                ...
                <Card
                    ...
                    rate={formatMessage(
                        {
                            id: "pricing_modal.rate.seatPerMonth",
                            defaultMessage:
                                "USD per seat/month {br}<b>(billed annually)</b>",
                        },
                        {
                            br: <br />,
                            b: (
                    ...
                    buttonDetails={{
                        ...
                        customClass:
                            ...
                            checkIsYearlyProfessionalProduct(
    ...
    purchaseScreen = () => {
        ...
        if (
            this.props.customer?.billing_address &&
            this.props.customer?.payment_method
        ) {
        ...
        return (
            <div
                className={classnames("PurchaseModal__purchase-body", {
                            ...
                            {this.state.processing ? (
                                <div>
                                    <ProcessPaymentSetup
                                        ...
                                        shippingAddress={this.getShippingAddressForProcessing()}
                                        addPaymentMethod={
                                            this.props.actions
                                                .completeStripeAddPaymentMethod
                                        }
                                        ...
                                        isSwitchingToAnnual={
                                            this.state.isSwitchingToAnnual
                                        }
                                        telemetryProps={{
                                            callerInfo:
