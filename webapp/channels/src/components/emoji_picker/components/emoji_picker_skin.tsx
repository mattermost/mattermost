...
const skinToneEmojis = new Map(
    skinsList.map((pair) => [
        pair[1],
        Emoji.Emojis[Emoji.EmojiIndicesByAlias.get(pair[0])!],
    ]),
);
...
export class EmojiPickerSkin extends React.PureComponent<Props, State> {
    ...
    extended() {
        ...
        const choices = skinsList.map((skinPair) => {
            ...
            const spriteClassName = classNames(
                ...
                `emoji-${emoji.unified.toLowerCase()}`,
            );
            ...
            return (
                ...
                >
                    <img src={imgTrans} className={spriteClassName} />
                </button>
            );
        });
        return (
            <>
                ...
                <div className="skin-tones__icons">{choices}</div>
            </>
        );
    }
    collapsed() {
    ...
    render() {
        return (
            ...
            >
                <div
                    className={classNames("skin-tones", {
                        "skin-tones--active": this.state.pickerExtended,
                    })}
                >
                    <div
                        className={classNames("skin-tones__content", {
