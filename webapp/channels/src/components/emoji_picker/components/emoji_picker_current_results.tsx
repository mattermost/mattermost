...
const EmojiPickerCurrentResults = forwardRef<InfiniteLoader, Props>(
    (
        ...
        const getItemKey = (
            index: Parameters<ListItemKeySelector>[0],
            rowsData: Parameters<ListItemKeySelector<CategoryOrEmojiRow[]>>[1],
        ) => {
            ...
            const emojiNamesArray = emojisRow.map(
                (emoji) => `${emoji.categoryIndex}-${emoji.emojiId}`,
            );
            return emojiNamesArray.join("--");
        };

        const handleScroll = (
            scrollOffset: ListOnScrollProps["scrollOffset"],
            activeCategory: EmojiCategory,
            isFiltering: boolean,
            categoryOrEmojisRows: CategoryOrEmojiRow[],
        ) => {
            ...
            const closestCategory =
                categoryOrEmojisRows?.[approxRowsFromTop]?.items[0]
                    ?.categoryName;
