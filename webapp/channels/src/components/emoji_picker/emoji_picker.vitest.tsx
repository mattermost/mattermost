// Copyright (c) 2015-present Mattermost, Inc. All Rights Reserved.
// See LICENSE.txt for license information.

import React from 'react';

import type {SystemEmoji} from '@mattermost/types/emojis';

import {renderWithContext, screen} from 'tests/vitest_react_testing_utils';
import EmojiMap from 'utils/emoji_map';

import EmojiPicker from './emoji_picker';

vi.mock('components/emoji_picker/components/emoji_picker_skin', () => ({
    default: () => (
        <div/>
    ),
}));
vi.mock('components/emoji_picker/components/emoji_picker_preview', () => ({
    default: ({emoji}: {emoji?: SystemEmoji}) => (
        <div className='emoji-picker__preview'>{`Preview for ${emoji?.short_name} emoji`}</div>
    ),
}));

describe('components/emoji_picker/EmojiPicker', () => {
    const baseProps = {
        filter: '',
        visible: true,
        onEmojiClick: vi.fn(),
        handleFilterChange: vi.fn(),
        handleEmojiPickerClose: vi.fn(),
        customEmojisEnabled: false,
        customEmojiPage: 1,
        emojiMap: new EmojiMap(new Map()),
        recentEmojis: [],
        userSkinTone: 'default',
        currentTeamName: 'testTeam',
        actions: {
            getCustomEmojis: vi.fn(),
            incrementEmojiPickerPage: vi.fn(),
            searchCustomEmojis: vi.fn(),
            setUserSkinTone: vi.fn(),
        },
    };

    test('should match snapshot', () => {
        const {asFragment} = renderWithContext(
            <EmojiPicker {...baseProps}/>,
        );

        expect(asFragment()).toMatchSnapshot();
    });

    test('Recent category should not exist if there are no recent emojis', () => {
        renderWithContext(
            <EmojiPicker {...baseProps}/>,
        );

        expect(screen.queryByLabelText('Recent')).toBeNull();
    });

    test('Recent category should exist if there are recent emojis', () => {
        const props = {
            ...baseProps,
            recentEmojis: ['smile'],
        };

        renderWithContext(
            <EmojiPicker {...props}/>,
        );

        // Note: Vitest uses compiled translations where "emoji_picker.recent" = "Recently Used"
        // while Jest uses defaultMessage fallback "Recent". The actual production behavior uses
        // "Recently Used" so this test reflects real-world behavior more accurately.
        expect(screen.queryByLabelText('Recently Used')).not.toBeNull();
    });

    test('First emoji should be selected on search', () => {
        const props = {
            ...baseProps,
            filter: 'wave',
        };

        renderWithContext(
            <EmojiPicker {...props}/>,
        );

        expect(screen.queryByText('Preview for wave emoji')).not.toBeNull();
    });

    test('Categories should be hidden when filter has text', () => {
        const props = {
            ...baseProps,
            filter: 'smile',
        };

        renderWithContext(
            <EmojiPicker {...props}/>,
        );

        expect(screen.queryByTestId('emojiPickerCategories')).toBeNull();
    });

    test('Categories should be visible when filter is empty', () => {
        const props = {
            ...baseProps,
            filter: '',
        };

        renderWithContext(
            <EmojiPicker {...props}/>,
        );

        expect(screen.queryByTestId('emojiPickerCategories')).not.toBeNull();
    });
});
