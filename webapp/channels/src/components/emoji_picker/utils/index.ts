...
import {
    compareEmojis,
    convertEmojiSkinTone,
    emojiMatchesSkin,
    getSkin,
} from "utils/emoji_utils";

export function isCategoryHeaderRow(
    row: CategoryOrEmojiRow,
): row is CategoryHeaderRow {
...
function updateSkinTone(initialEmoji: SystemEmoji, skinTone: string): Emoji {
    ...
    if (initialEmojiSkin && initialEmojiSkin !== skinTone) {
        const emojiWithUpdatedSkinTone = convertEmojiSkinTone(
            initialEmoji,
            skinTone,
        );
...
function convertEmojisToUserSkinTone(
    emojiIds: string[],
    ...
    return emojiIds.map((emojiId) => {
        ...
        return isSystemEmoji(emoji)
            ? updateSkinTone(emoji, userSkinTone)
            : emoji;
    });
}

function isEmojiIdEqual(firstEmoji: Emoji, secondEmoji: Emoji): boolean {
    const firstEmojiId = isSystemEmoji(firstEmoji)
        ? firstEmoji.unified
        : firstEmoji.id;
    const secondEmojId = isSystemEmoji(secondEmoji)
        ? secondEmoji.unified
        : secondEmoji.id;
    return firstEmojiId === secondEmojId;
}

export function getFilteredEmojis(
    allEmojis: Record<string, Emoji>,
    filter: string,
    recentEmojisString: string[],
    userSkinTone: string,
): Emoji[] {
    const filteredEmojisWithRecent = Object.values(allEmojis).filter(
        (emoji) => {
            const aliases = isSystemEmoji(emoji)
                ? emoji.short_names
                : [emoji.name];
    ...
    const recentEmojis = convertEmojisToUserSkinTone(
        recentEmojisString,
        allEmojis,
        userSkinTone,
    );
    ...
    const filteredRecentEmojis = filteredEmojisWithRecent.filter((emoji) => {
        return recentEmojis.some((recentEmojis) =>
            isEmojiIdEqual(recentEmojis, emoji),
        );
    });

    const sortedRecentEmojis = filteredRecentEmojis.sort(
    ...
    const filtertedEmojisMinusRecent = filteredEmojisWithRecent.filter(
        (emoji) => {
            return !recentEmojis.some((recentEmojis) =>
                isEmojiIdEqual(recentEmojis, emoji),
            );
    ...
    const sortedFiltertedEmojisMinusRecent = filtertedEmojisMinusRecent.sort(
        (firstEmoji, secondEmoji) =>
            compareEmojis(firstEmoji, secondEmoji, filter),
    );
    ...
    const filteredEmojis = [
        ...sortedRecentEmojis,
        ...sortedFiltertedEmojisMinusRecent,
    ];
...
export function getUpdatedCategoriesAndAllEmojis(
    ...
    const categories: Categories = recentEmojis.length
    ...
    Object.keys(categories).forEach((categoryName) => {
        ...
        if (categoryName === "recent" && recentEmojis.length) {
            ...
            const indices =
                (
                    EmojiIndicesByCategory.get(userSkinTone) as Map<
                        string,
                        number[]
                    >
                ).get(categoryName) || [];
        ...
        categories[categoryName as EmojiCategory].emojiIds = categoryEmojis.map(
            (emoji: Emoji) =>
                isSystemEmoji(emoji) ? emoji.unified.toLowerCase() : emoji.id,
        );

        // populate allEmojis with emoji objects
        categoryEmojis.forEach((currentEmoji: Emoji) => {
            const currentEmojiId = isSystemEmoji(currentEmoji)
                ? currentEmoji.unified.toLowerCase()
                : currentEmoji.id;
            allEmojis[currentEmojiId] = {
                ...allEmojis[currentEmojiId],
                ...currentEmoji,
            };
...
export function calculateCategoryRowIndex(
    categories: Categories,
    categoryName: EmojiCategory,
) {
    const categoryIndex = Object.keys(categories).findIndex(
        (category) => category === categoryName,
    );

    const categoriesTillCurrentCategory = Object.values(categories).slice(
        0,
        categoryIndex,
    );

    const rowIndex = categoriesTillCurrentCategory.reduce(
        (previousIndexSum, currentCategory) => {
            const emojisInCurrentCategory =
                currentCategory?.emojiIds?.length ?? 0;

            const numberOfEmojiRowsInCurrentCategory = Math.ceil(
                emojisInCurrentCategory / EMOJI_PER_ROW,
            );
...
export function splitEmojisToRows(
    emojis: Emoji[],
...
export function createEmojisPositions(
    categoryOrEmojiRows: CategoryOrEmojiRow[],
): EmojiPosition[] {
    ...
    categoryOrEmojiRows.forEach((categoryOrEmojiRow) => {
        if (!isCategoryHeaderRow(categoryOrEmojiRow)) {
            ...
            const emojisOfARow: EmojiPosition[] = categoryOrEmojiRow.items.map(
                (emojiItem) => ({
                    rowIndex,
...
export function createCategoryAndEmojiRows(
    ...
    if (filter.length) {
        ...
        const [searchEmojisRows] = splitEmojisToRows(
            filteredEmojis,
            0,
            SEARCH_RESULTS,
            1,
        ...
        const searchEmojiRowsWithCategoryHeader: CategoryOrEmojiRow[] = [
            searchCategoryRow,
            ...searchEmojisRows,
        ];

        const emojisPositions = createEmojisPositions(
            searchEmojiRowsWithCategoryHeader,
        );
    ...
    Object.keys(categories).forEach((categoryName, categoryIndex) => {
        ...
        const [emojiRows, increasedRowIndexCounter] = splitEmojisToRows(
            emojis,
            categoryIndex,
...
export function getCursorProperties(
    cursorRowIndex: EmojiCursor["rowIndex"],
    cursorEmojiId: EmojiCursor["emojiId"],
    categoryOrEmojisRows: EmojiRow[],
): [string, number, number] {
    if (cursorEmojiId.length === 0 || cursorRowIndex === -1) {
        return ["", -1, -1];
    }

    const emojisRowOfCursor =
        categoryOrEmojisRows?.[cursorRowIndex]?.items ?? [];
    ...
    const cursorEmojiIndex =
        emojisRowOfCursor.find((emojiItem) => {
