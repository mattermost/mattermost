...
export const RECENT_EMOJI_CATEGORY: Pick<Categories, "recent"> = {
    recent: createCategory(RECENT),
};
export const SEARCH_EMOJI_CATEGORY: Pick<Categories, typeof SEARCH_RESULTS> = {
    searchResults: createCategory(SEARCH_RESULTS),
};

export const CATEGORIES: Categories = Emoji.CategoryNames.filter(
    (category) => !(category === "recent" || category === "searchResults"),
