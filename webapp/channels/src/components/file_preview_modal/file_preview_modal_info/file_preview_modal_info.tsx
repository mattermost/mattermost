...
const FilePreviewModalInfo: React.FC<Props> = (props: Props) => {
    const user = useSelector((state: GlobalState) =>
        selectUser(state, props.post?.user_id ?? ""),
    ...
    const name = useSelector((state: GlobalState) =>
        displayNameGetter(state, props.post?.user_id ?? "", true),
    );
    ...
    if (props.showFileName) {
        info = (
            <>
                ...
                <span className="file-preview-modal__file-details">
                    <span className="file-preview-modal__file-details-user-name">
                        {name}
                    </span>
                    <span className="file-preview-modal__channel">
                        {channelName}
                    </span>
                </span>
            </>
        ...
        info = (
            <>
                <h5 className="file-preview-modal__user-name">{name}</h5>
                <span className="file-preview-modal__channel">
                    {channelName}
                </span>
            </>
    ...
    return (
        <div className="file-preview-modal__info">
            ...
            <div className="file-preview-modal__info-details">{info}</div>
        </div>
    );
};
