...
const ProductMenu = (): JSX.Element => {
    ...
    const tutorialStep = useSelector((state: GlobalState) =>
        getInt(state, TutorialTourName.EXPLORE_OTHER_TOOLS, currentUserId, 0),
    );
    const triggerStep = useSelector((state: GlobalState) =>
        getInt(
            state,
            OnboardingTaskCategory,
    ...
    const showPlaybooksTour =
        enableTutorial &&
        tutorialStep === ExploreOtherToolsTourSteps.PLAYBOOKS_TOUR &&
        exploreToolsTourTriggered &&
        playbooksPlugin;
    ...
    const handleVisitConsoleClick = () => {
        ...
        handleOnBoardingTaskData(
            visitSystemConsoleTaskName,
            steps.FINISHED,
            true,
            "finish",
        );
    ...
    const productItems = products?.map((product) => {
        ...
        // playbooks
        if (
            product.pluginId === suitePluginIds.playbooks &&
            showPlaybooksTour
        ) {
                ...
                <ProductMenuContainer onClick={handleClick}>
                    <ProductMenuButton
                        ...
                        aria-label={formatMessage({
                            id: "global_header.productSwitchMenu",
                            defaultMessage: "Product switch menu",
                        })}
