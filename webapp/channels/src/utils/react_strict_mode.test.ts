...
describe('installReactStrictModeDiagnostics', () => {
    ...
    test('should detect React 19 legacy lifecycle warnings', () => {
        installReactStrictModeDiagnostics();

        console.warn('componentWillMount has been renamed, and is not recommended for use.');

        expect(flushPendingThrow).toThrow('React strict mode violation: componentWillMount has been renamed');
    });
    ...
    ])('should detect React 19 warning: %s', (warning) => {
        installReactStrictModeDiagnostics();

        console.error(warning);

        expect(flushPendingThrow).toThrow(`React strict mode violation: ${warning}`);
    });
