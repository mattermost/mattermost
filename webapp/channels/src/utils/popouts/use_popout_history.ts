...
export function usePopoutHistory() {
    const history = useHistory();
    useEffect(() => {
        const unblockHistory = history.block((blockState) => {
            // AAlow the popout to change to a different state
            if (blockState.pathname.includes('/_popout/')) {
                unblockHistory();
                history.replace(blockState.pathname);
                return false;
            }
