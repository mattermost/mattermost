...
describe("TextFormatting.Emails", () => {
    it("Valid email addresses", () => {
        expect(
            TextFormatting.formatText("email@domain.com", {}, emojiMap).trim(),
        ).toBe(
        ...
        expect(
            TextFormatting.formatText("email@domain.name", {}, emojiMap).trim(),
        ).toBe(
    ...
    it("Should be valid, but matching GitHub", () => {
        ...
        ).toBe("<p>email@123.123.123.123</p>");
    ...
    it("Should be valid, but broken due to Markdown parsing happening before email autolinking", () => {
        ...
        ).toBe("<p><strong>___</strong>@domain.com</p>");
    });

    it("Not valid emails", () => {
        expect(
            TextFormatting.formatText("plainaddress", {}, emojiMap).trim(),
        ).toBe("<p>plainaddress</p>");

        expect(
            TextFormatting.formatText("#@%^%#$@#$@#.com", {}, emojiMap).trim(),
        ).toBe("<p>#@%^%#$@#$@#.com</p>");

        expect(
            TextFormatting.formatText("@domain.com", {}, emojiMap).trim(),
        ).toBe("<p>@domain.com</p>");
