...
export const buildZendeskSupportForm = (
    form: ZendeskSupportForm,
    formFieldIDs: PrefillFieldFormFieldIDs[],
): string => {
    ...
    if (form === ZendeskSupportForm.SELF_HOSTED_SUPPORT_FORM) {
        formUrl = formUrl.concat(
            `&tf_${ZendeskFormFieldIDs.SELF_HOSTED_ENVIRONMENT}=production`,
        );
    }
...
export const goToCloudSupportForm = (
    email: string,
    subject: string,
    description: string,
    workspaceURL: string,
    ...
    url = url.concat(
        `&tf_${ZendeskFormFieldIDs.CLOUD_WORKSPACE_URL}=${workspaceURL}`,
    );
    window.open(url, "_blank");
};

export const getCloudSupportLink = (
    email: string,
    subject: string,
    description: string,
    workspaceURL: string,
...
export const buildMMURL = (
    ...
    const mmURL = `${baseURL}?qk=${encodeString(firstName)}&qp=${encodeString(
        lastName,
    )}&qw=${encodeString(companyName)}&qx=${encodeString(
        businessEmail,
    )}&utm_source=${source}&utm_medium=${medium}`;
    return mmURL;
};

export const goToMattermostContactSalesForm = (
    firstName: string,
    lastName: string,
    companyName: string,
    businessEmail: string,
...
export const getCloudContactSalesLink = (
    firstName: string,
    lastName: string,
    companyName: string,
    businessEmail: string,
