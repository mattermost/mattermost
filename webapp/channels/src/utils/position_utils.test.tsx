...
test("Should return placement position for overlay based on bounds, space required and innerHeight", () => {
    ...
    expect(popOverOverlayPosition(targetBounds as DOMRect, 1000, 300)).toEqual(
        "top",
    );
    expect(
        popOverOverlayPosition(targetBounds as DOMRect, 1000, 500, 300),
    ).toEqual("bottom");
    expect(popOverOverlayPosition(targetBounds as DOMRect, 1000, 450)).toEqual(
        "bottom",
    );
    expect(popOverOverlayPosition(targetBounds as DOMRect, 1000, 600)).toEqual(
        "left",
    );
});
