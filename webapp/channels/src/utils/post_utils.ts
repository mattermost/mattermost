...
export function shouldShowDotMenu(
    state: GlobalState,
    post: Post,
    channel: Channel,
): boolean {
...
export function containsAtChannel(
    text: string,
    options?: { checkAllMentions: boolean },
): boolean {
...
export function specialMentionsInText(text: string): {
    [key: string]: boolean;
} {
    const mentions = {
        all: false,
    ...
    mentions.all = new RegExp(Constants.ALL_MENTION_REGEX).test(
        mentionableText,
    );
    mentions.channel = new RegExp(Constants.CHANNEL_MENTION_REGEX).test(
        mentionableText,
    );
    mentions.here = new RegExp(Constants.HERE_MENTION_REGEX).test(
        mentionableText,
    );
...
export const groupsMentionedInText = (
    text: string,
...
export function shouldFocusMainTextbox(
    ...
    if (
        ...
        Keyboard.isKeyPressed(e, Constants.KeyCodes.V)
    ) {
        return true;
    }
    ...
    if (
        Keyboard.isKeyPressed(e, Constants.KeyCodes.SPACE) &&
        spaceKeepFocusTags.includes(activeElement.tagName)
    ) {
        return false;
    }
...
export function isWithinCodeBlock(
    message: string,
    caretPosition: number,
): boolean {
    const match = message
        .substring(0, caretPosition)
        .match(Constants.REGEX_CODE_BLOCK_OPTIONAL_LANGUAGE_TAG);
...
function sendOnCtrlEnter(
    message: string,
    ctrlOrMetaKeyPressed: boolean,
    isSendMessageOnCtrlEnter: boolean,
    caretPosition: number,
) {
...
export function postMessageOnKeyPress(
    ...
    if (
        !Keyboard.isKeyPressed(event, Constants.KeyCodes.ENTER) ||
        event.shiftKey ||
        event.altKey
    ) {
        return { allowSending: false };
...
export function getOldestPostId(postIds: string[]): string {
    for (let i = postIds.length - 1; i >= 0; i--) {
        ...
        if (PostListUtils.isCombinedUserActivityPost(item)) {
            ...
            const combinedIds =
                PostListUtils.getPostIdsForCombinedUserActivityPost(item);
...
export function getPreviousPostId(
    ...
    for (let i = startIndex + 1; i < postIds.length; i++) {
        ...
        if (PostListUtils.isCombinedUserActivityPost(itemId)) {
            ...
            const combinedIds =
                PostListUtils.getPostIdsForCombinedUserActivityPost(itemId);
...
export function makeGetMentionsFromMessage(): (
    state: GlobalState,
    post: Post,
    ...
    return createSelector(
        ...
        (post, users) => {
            ...
            const mentionsArray =
                post.message.match(Constants.MENTIONS_REGEX) || [];
            for (let i = 0; i < mentionsArray.length; i++) {
                const mention = mentionsArray[i];
                const user = getMentionDetails(users, mention.substring(1)) as
                    | UserProfile
                    | "";
...
export function createAriaLabelForPost(
    post: Post,
    author: string,
    isFlagged: boolean,
    ...
    intl: IntlShape,
    emojiMap: EmojiMap,
    mentions: Record<string, UserProfile>,
    ...
    for (const mention of Object.keys(mentions)) {
        ...
        if (user) {
            message = message.replace(
                mention,
                `@${displayUsername(user, teammateNameDisplaySetting)}`,
            );
        }
    }
        ...
        ariaLabel = formatMessage(
            {
                ...
                defaultMessage:
                    "At {time} {date}, {authorName} replied, {message}",
            },
            {
                authorName: author,
        ...
        ariaLabel = formatMessage(
            {
                ...
                defaultMessage:
                    "At {time} {date}, {authorName} wrote, {message}",
            },
            {
                authorName: author,
                time: formatTime(post.create_at),
...
export function splitMessageBasedOnCaretPosition(
    caretPosition: number,
    message: string,
): { firstPiece: string; lastPiece: string } {
...
export function splitMessageBasedOnTextSelection(
...
export function areConsecutivePostsBySameUser(
    ...
    ); // And neither is a system message
}
...
export function getPostURL(state: GlobalState, post: Post): string {
    ...
    const postURI =
        isCollapsedThreadsEnabled(state) && isComment(post)
            ? ""
            : `/${post.id}`;
...
export function matchUserMentionTriggersWithMessageMentions(
    userMentionKeys: UserMentionKey[],
    messageMentionKeys: RegExpMatchArray,
): boolean {
 ...
 */
export function makeGetUniqueReactionsToPost(): (
    state: GlobalState,
    postId: Post["id"],
) => Record<string, Reaction> | undefined | null {
    const getReactionsForPost = makeGetReactionsForPost();

    return createSelector(
        "makeGetUniqueReactionsToPost",
        (state: GlobalState, postId: string) =>
            getReactionsForPost(state, postId),
        getEmojiMap,
        (reactions, emojiMap) => {
            if (!reactions) {
                return null;
...
export function getMentionDetails(
    usersByUsername: Record<string, UserProfile | Group>,
    mentionName: string,
    ...
    while (mentionNameToLowerCase.length > 0) {
        ...
        if (/[._-]$/.test(mentionNameToLowerCase)) {
            mentionNameToLowerCase = mentionNameToLowerCase.substring(
                0,
                mentionNameToLowerCase.length - 1,
            );
        } else {
            break;
        }
...
export function makeGetUserOrGroupMentionCountFromMessage(): (
    state: GlobalState,
    ...
    return createSelector(
        ...
        (message, users, groups) => {
            let count = 0;
            const markdownCleanedText = formatWithRenderer(
                message,
                new MentionableRenderer(),
            );
            const mentions = new Set(
                markdownCleanedText.match(Constants.MENTIONS_REGEX) || [],
            );
            mentions.forEach((mention) => {
                const [user, group] = getUserOrGroupFromMentionName(
                    mention.substring(1),
                    users,
                    groups,
                );
...
export function hasRequestedPersistentNotifications(
