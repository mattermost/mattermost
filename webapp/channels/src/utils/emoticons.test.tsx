...
describe("Emoticons", () => {
    ...
    describe("matchEmoticons", () => {
        test("empty message", () => {
            expect(Emoticons.matchEmoticons("")).toEqual(null);
        });

        test("no emoticons", () => {
            expect(Emoticons.matchEmoticons("test")).toEqual(null);
        });

        describe("single", () => {
            test("shorthand forms", () => {
                expect(Emoticons.matchEmoticons(":+1:")).toEqual([":+1:"]);
            });

            test("named emoticons forms", () => {
                expect(Emoticons.matchEmoticons(":thumbs_up:")).toEqual([
                    ":thumbs_up:",
                ]);
            });
        });
        ...
        describe("multiple", () => {
            test("shorthand forms", () => {
                expect(Emoticons.matchEmoticons(":+1: :D")).toEqual([
                    ":+1:",
                    ":D",
                ]);
            });
            ...
            test("named emoticons forms", () => {
                expect(Emoticons.matchEmoticons(":thumbs_up: :smile:")).toEqual(
                    [":thumbs_up:", ":smile:"],
                );
            });
            ...
            test("mixed", () => {
                expect(
                    Emoticons.matchEmoticons(":thumbs_up: :smile: :+1: :D"),
        ...
        test("inline", () => {
            expect(
                Emoticons.matchEmoticons(
                    "I am feeling pretty :D -- you are: ok?",
                ),
            ).toEqual([":D"]);
        });

        test("shouldn't render emoticons in code blocks", () => {
            expect(Emoticons.matchEmoticons("`:goat:`")).toEqual(null);
        });
        ...
        test("shouldn't render emoticons in links", () => {
            expect(
                Emoticons.matchEmoticons("[link](www.google.com/:goat:)"),
            ).toEqual(null);
        });
