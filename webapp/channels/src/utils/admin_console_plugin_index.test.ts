...
describe("AdminConsolePluginsIndex.getPluginEntries", () => {
    ...
    it("should not return the markdown link texts", () => {
        ...
        expect(entries["plugin_plugin-with-markdown"]).toContain(
            "Markdown plugin label",
        );
        expect(entries["plugin_plugin-with-markdown"]).not.toContain(
            "localhost",
        );
    });
    ...
    it("should index the enable plugin setting even if other settings are not present", () => {
        ...
        expect(entries["plugin_plugin-without-settings"]).toContain(
            "PluginSettings.PluginStates.plugin-without-settings.Enable",
        );
    });
});
