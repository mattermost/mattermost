...
`;
        ...
        expect(
            messageHtmlToComponent(html, { postId: "randompostid" }),
        ).toMatchSnapshot();
    });
...
`;
    ...
    test("link with enabled a tooltip plugin", () => {
        ...
        expect(
            messageHtmlToComponent(html, { hasPluginTooltips: true }),
        ).toMatchSnapshot();
    });
    ...
    test("Inline markdown image", () => {
        ...
        expect(shallow(component).find(MarkdownImage).prop("imageIsLink")).toBe(
            false,
        );
    });
    ...
    test("At mention", () => {
        ...
        expect(component).toMatchSnapshot();
        expect(
            shallow(component).find(AtMention).prop("disableHighlight"),
        ).toBe(false);
    ...
    test("At mention with group highlight disabled", () => {
        const options: TextFormatting.TextFormattingOptions = {
            mentionHighlight: true,
            atMentions: true,
            mentionKeys: [{ key: "@joram" }],
        };
        let html = TextFormatting.formatText(
            "@developers",
            options,
            emptyEmojiMap,
        );
        ...
        expect(component).toMatchSnapshot();
        expect(
            shallow(component).find(AtMention).prop("disableGroupHighlight"),
        ).toBe(false);
