...
describe("Utils.isUnhandledLineBreakKeyCombo", () => {
    test("isUnhandledLineBreakKeyCombo returns true for alt + enter for Chrome UA", () => {
        ua.mockChrome();
        expect(
            Utils.isUnhandledLineBreakKeyCombo(
                lineBreakHelpers.getAltKeyEvent(),
            ),
        ).toBe(true);
    });
    ...
    test("isUnhandledLineBreakKeyCombo returns false for shift + enter", () => {
        expect(
            Utils.isUnhandledLineBreakKeyCombo(
                lineBreakHelpers.getShiftKeyEvent(),
            ),
        ).toBe(false);
    });
    ...
    test("isUnhandledLineBreakKeyCombo returns false for ctrl/command + enter", () => {
        expect(
            Utils.isUnhandledLineBreakKeyCombo(
                lineBreakHelpers.getCtrlKeyEvent(),
            ),
        ).toBe(false);
        expect(
            Utils.isUnhandledLineBreakKeyCombo(
                lineBreakHelpers.getMetaKeyEvent(),
            ),
        ).toBe(false);
    });
    ...
    test("isUnhandledLineBreakKeyCombo returns false for just enter", () => {
        expect(
            Utils.isUnhandledLineBreakKeyCombo(lineBreakHelpers.BASE_EVENT),
        ).toBe(false);
    });
...
describe("Utils.insertLineBreakFromKeyEvent", () => {
    test("insertLineBreakFromKeyEvent returns with line break appending (no selection range)", () => {
        expect(
            Utils.insertLineBreakFromKeyEvent(
                lineBreakHelpers.getAppendEvent(),
            ),
        ).toBe(lineBreakHelpers.OUTPUT_APPEND);
    });
    test("insertLineBreakFromKeyEvent returns with line break replacing (with selection range)", () => {
        expect(
            Utils.insertLineBreakFromKeyEvent(
                lineBreakHelpers.getReplaceEvent(),
            ),
        ).toBe(lineBreakHelpers.OUTPUT_REPLACE);
    });
});
...
describe("Utils.getSuggestionBoxAlgn", () => {
    ...
    const fixedToTheRight =
        textArea.offsetWidth - Constants.SUGGESTION_LIST_MODAL_WIDTH;
...
describe("Utils.numberToFixedDynamic", () => {
    ...
    tests.forEach((testCase) => {
        test(testCase.label, () => {
            const actual = Utils.numberToFixedDynamic(
                testCase.num,
                testCase.places,
            );
