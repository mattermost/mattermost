...
import {
    ...
    splitMessageBasedOnTextSelection,
...
export function hasHtmlLink(clipboardData: DataTransfer): boolean {
    return (
        ...
        /<a/i.test(clipboardData.getData("text/html"))
    );
}
...
): { formattedMessage: string; formattedMarkdown: string } {
    const html = clipboardData.getData("text/html");
...
export function formatGithubCodePaste({
    ...
    selectionStart,
    selectionEnd,
}: FormatGithubCodePasteParams): {
    formattedMessage: string;
    formattedCodeBlock: string;
} {
    const isTextSelected = selectionStart !== selectionEnd;
    const { firstPiece, lastPiece } = isTextSelected
        ? splitMessageBasedOnTextSelection(
 ...
 */
export function formatMarkdownLinkMessage({
    message,
    clipboardData,
    selectionStart,
    selectionEnd,
    ...
    if (selectedText === DEFAULT_PLACEHOLDER_URL) {
        if (message.length > DEFAULT_PLACEHOLDER_URL.length) {
            ...
            const textBefore = message.slice(
                selectionStart - FORMATTED_LINK_URL_PREFIX.length,
                selectionStart,
            );
            const textAfter = message.slice(
                selectionEnd,
                selectionEnd + FORMATTED_LINK_URL_SUFFIX.length,
            );

            // We check "](" "url" ")" to see if user is trying to paste inside of a markdown link
            // and selection is on "url"
            if (
                textBefore === FORMATTED_LINK_URL_PREFIX &&
                textAfter === FORMATTED_LINK_URL_SUFFIX
            ) {
                return clipboardUrl;
            }
        }
...
export function pasteHandler(
    ...
    const textboxId =
        location === Locations.RHS_COMMENT ? "reply_textbox" : "post_textbox";

    if (
        !clipboardData ||
        !clipboardData.items ||
        !target ||
        (target as TextboxElement)?.id !== textboxId
    ) {
        return;
    }
    ...
    const shouldApplyGithubCodeBlock =
        htmlTable && isGitHubCodeBlock(htmlTable.className);
...
export function createFileFromClipboardDataItem(
    item: DataTransferItem,
    fileNamePrefixIfNoName: string,
): File | null {
    ...
    if (file.name && file.name.includes(".")) {
        ...
        ext =
            "." + item.type.slice(item.type.lastIndexOf("/") + 1).toLowerCase();
    }
