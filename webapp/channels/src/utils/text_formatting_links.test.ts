...
describe("Markdown.Links", () => {
    it("Not links", () => {
        ...
        expect(Markdown.format("readme.md").trim()).toBe("<p>readme.md</p>");
    ...
    it("Links with text", () => {
        ...
        expect(
            Markdown.format(
                "[example.com/other_link](example.com/example)",
            ).trim(),
        ).toBe(
        ...
        expect(
            Markdown.format(
                "[link with spaces](example.com/ spaces in the url)",
            ).trim(),
        ).toBe(
    ...
    it("Relative link", () => {
        expect(
            Markdown.format(
                "[A Relative Link](/static/files/5b4a7904a3e041018526a00dba59ee48.png)",
            ).trim(),
        ).toBe(
    ...
    describe("autolinkedUrlSchemes", () => {
        test("all links are rendered when not provided", () => {
            ...
            expect(Markdown.format("ftp://ftp.example.com").trim()).toBe(
                `<p>${link("ftp://ftp.example.com")}</p>`,
            );

            expect(Markdown.format("tel:1-555-123-4567").trim()).toBe(
                `<p>${link("tel:1-555-123-4567")}</p>`,
            );

            expect(Markdown.format("mailto:test@example.com").trim()).toBe(
                `<p>${link("mailto:test@example.com")}</p>`,
            );

            expect(Markdown.format("git://git.example.com").trim()).toBe(
                `<p>${link("git://git.example.com")}</p>`,
            );
        ...
        test("no links are rendered when no schemes are provided", () => {
            ...
            expect(
                Markdown.format("ftp://ftp.example.com", options).trim(),
            ).toBe("<p>ftp://ftp.example.com</p>");
            ...
            expect(
                Markdown.format("mailto:test@example.com", options).trim(),
            ).toBe("<p>mailto:test@example.com</p>");

            expect(
                Markdown.format("git://git.example.com", options).trim(),
            ).toBe("<p>git://git.example.com</p>");
        ...
        test("only matching links are rendered when schemes are provided", () => {
            ...
            expect(
                Markdown.format("git://git.example.com", options).trim(),
            ).toBe(`<p>${link("git://git.example.com")}</p>`);
            ...
            expect(
                Markdown.format("taco+what://example.com", options).trim(),
            ).toBe(`<p>${link("taco+what://example.com")}</p>`);
        ...
        test("explicit links are not affected by this setting", () => {
            ...
            expect(Markdown.format("www.example.com", options).trim()).toBe(
                `<p>${link("http://www.example.com", "www.example.com")}</p>`,
            );
            ...
            ).toBe(`<p>${link("git://git.example.com", "link")}</p>`);

            expect(
                Markdown.format("<http://example.com>", options).trim(),
            ).toBe(`<p>${link("http://example.com")}</p>`);
        });
    });
});
