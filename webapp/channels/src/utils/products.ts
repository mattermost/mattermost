...
import type {
    ProductIdentifier,
    ProductScope,
} from "@mattermost/types/products";
...
export const getCurrentProduct = (
    ...
    return (
        products?.find(({ baseURL }) =>
            matchPath(pathname, { path: baseURL, exact: false, strict: false }),
        ) ?? null
    );
};
...
export const useCurrentProductId = () => {
    ...
    return useSelector((state: GlobalState) =>
        selectCurrentProductId(state, pathname),
    );
};
...
export const useCurrentProduct = () => {
    const { pathname } = useLocation();
    return useSelector((state: GlobalState) =>
        selectCurrentProduct(state, pathname),
    );
};
...
export const inScope = (
    ...
    if (Array.isArray(scope)) {
        return (
            scope.includes(productId) ||
            (pluginId !== undefined && scope.includes(pluginId))
        );
    }
    return (
        scope === productId || (pluginId !== undefined && scope === pluginId)
    );
};
...
export const findProductBySkuAndInterval = (
...
const filterProductsRecord = (
    data: Record<string, Product>,
    predicate: (product: Product) => boolean,
): Record<string, Product> => {
    return Object.keys(data).reduce(
        (acc: Record<string, Product>, current: string) => {
...
export const findOnlyYearlyProducts = (products: Record<string, Product>) => {
    return filterProductsRecord(
        products,
        (product: Product) =>
            product.recurring_interval === RecurringIntervals.YEAR,
    );
};
