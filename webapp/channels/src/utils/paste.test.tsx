...
describe("formatMarkdownMessage", () => {
    ...
    test("returns a markdown table when valid html table provided", () => {
        expect(formatMarkdownMessage(validClipboardData).formattedMessage).toBe(
            `${markdownTable}\n`,
        );
    });
    ...
    test("returns a markdown table when valid html table with headers provided", () => {
        ...
        expect(
            formatMarkdownMessage(tableHeadersClipboardData).formattedMessage,
        ).toBe(markdownTable);
    });
    ...
    test("removes style contents and additional whitespace around tables", () => {
        ...
        expect(formatMarkdownMessage(styleClipboardData).formattedMessage).toBe(
            markdownTable,
        );
    });
    ...
    test("returns a markdown table under a message when one is provided", () => {
        ...
        expect(
            formatMarkdownMessage(validClipboardData, testMessage)
                .formattedMessage,
        ).toBe(`${testMessage}\n\n${markdownTable}\n`);
    });
    ...
    test("returns a markdown formatted link when valid hyperlink provided", () => {
        ...
        expect(formatMarkdownMessage(linkClipboardData).formattedMessage).toBe(
            markdownLink,
        );
    });
});
...
describe("formatMarkdownLinkMessage", () => {
    ...
    test("Should return empty selection when no selection is made", () => {
        ...
        const formatttedMarkdownLinkMessage = formatMarkdownLinkMessage({
            selectionStart: 0,
            selectionEnd: 0,
            message,
            clipboardData,
        });
        expect(formatttedMarkdownLinkMessage).toEqual(
            "[](https://example.com/)",
        );
    });
    ...
    test("Should add link when pasting inside of an improper formatted markdown link", () => {
        const improperFormattedLinkMessages = [
            ...
            },
            {
                message: "[test](url)",
                selection: "[test](url",
                expected: "[[test](url](https://example.com/)",
            },
            {
                message: "[test](url)",
                selection: "test](url",
        ...
        for (const {
            ...
            selection,
            expected,
        } of improperFormattedLinkMessages) {
...
describe("pasteHandler", () => {
    const testCases = [
        ...
        {
            testName:
                "should be able to format a pasted markdown table without headers",
            clipboardData: {
                items: [1],
                types: ["text/html"],
                getData: () => {
        ...
        {
            testName:
                "should be able to format a github codeblock (pasted as a table)",
            clipboardData: {
                items: [1],
                types: ["text/plain", "text/html"],
                getData: (type: string) => {
