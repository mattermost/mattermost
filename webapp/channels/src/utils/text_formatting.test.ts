...
describe("autolinkAtMentions", () => {
    ...
    test("@channel, @all, @here should be treated as non special mentions with trailing period followed by a word", () => {
        runUnsuccessfulAtMentionTestsMatchingNonSpecialMentions(
            "Hello ",
            ".developers",
        );
    });
...
describe("autolink channel mentions", () => {
    test("link a channel mention", () => {
        ...
        expect(output).toBe(
            `${leadingText}$MM_CHANNELMENTION0$${trailingText}`,
        );
        expect(tokens.get("$MM_CHANNELMENTION0$").value).toBe(
            '<a class="mention-link" href="/ad-1/channels/test-channel" data-channel-mention-team="ad-1" data-channel-mention="test-channel">~Test Channel</a>',
        );
    });
});
...
describe("handleUnicodeEmoji", () => {
    ...
    for (const t of tests) {
        test(t.description, () => {
            const output = handleUnicodeEmoji(
                t.text,
                emojiMap,
                UNICODE_EMOJI_REGEX,
            );
    ...
    test("without emojiMap, should work as unsupported emoji", () => {
        const output = handleUnicodeEmoji(
            "👍",
            undefined as unknown as EmojiMap,
            UNICODE_EMOJI_REGEX,
        );
        expect(output).toBe(
...
describe("linkOnlyMarkdown", () => {
    ...
    test("link with header signs to skip", () => {
        const text =
            "#### Do you like [Mattermost](https://www.mattermost.com)?";
        const output = formatText(text, options, emptyEmojiMap);
        expect(output).toBe(
            'Do you like <a class="theme markdown__link" href="https://www.mattermost.com" target="_blank">' +
                "Mattermost</a>?",
...
describe("highlightCurrentMentions", () => {
    ...
    it("should find and match Korean, Japanese, latin and Russian words", () => {
        const text =
            "메터모스트, notinkeys, マッターモスト, маттермост!, Mattermost, notinkeys";
        const highlightedText = highlightCurrentMentions(
            text,
            tokens,
            mentionKeys,
        );
        ...
        const expectedOutput =
            "$MM_SELFMENTION0$, notinkeys, $MM_SELFMENTION1$, $MM_SELFMENTION2$!, $MM_SELFMENTION3$, notinkeys";
