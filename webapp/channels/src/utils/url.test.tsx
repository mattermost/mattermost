...
describe("Utils.URL", () => {
    ...
    describe("getSiteURL", () => {
        ...
        testCases.forEach((testCase) =>
            it(testCase.description, () => {
                ...
                expect(getSiteURLFromWindowObject(obj)).toEqual(
                    testCase.expectedSiteURL,
                );
            }),
        );
    });
    ...
    describe("validateChannelUrl", () => {
        const testCases = [
            ...
            {
                description:
                    "Called with a url starting and ending with an underscore",
                url: "_a_",
                expectedErrors: ["change_url.startAndEndWithLetter"],
            },
            {
                description:
                    "Called with a url starting and ending with an dash",
                url: "-a-",
                expectedErrors: ["change_url.startAndEndWithLetter"],
            },
            {
            ...
            {
                description:
                    "Called with a url resembling a direct message url",
        ...
        testCases.forEach((testCase) =>
            it(testCase.description, () => {
                const returnedErrors = validateChannelUrl(testCase.url).map(
                    (error) => (typeof error === "string" ? error : error.key),
                );
                returnedErrors.sort();
