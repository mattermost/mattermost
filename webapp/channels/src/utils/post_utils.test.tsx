...
describe("PostUtils.containsAtChannel", () => {
    test("should return correct @all (same for @channel)", () => {
        for (const data of [
            ...
            const containsAtChannel = PostUtils.containsAtChannel(
                data.text,
                data.options,
            );
    ...
    test("basic cases", () => {
        for (const data of [
            ...
            const shouldFocus = PostUtils.shouldFocusMainTextbox(
                data.event as unknown as KeyboardEvent,
                data.activeElement as unknown as Element,
            );
...
describe("PostUtils.getOldestPostId", () => {
    test("Should not return LOAD_OLDER_MESSAGES_TRIGGER", () => {
        const postId = PostUtils.getOldestPostId([
            "postId1",
            "postId2",
...
describe("PostUtils.getPreviousPostId", () => {
    test("Should skip dateline", () => {
        const postId = PostUtils.getPreviousPostId(
            ["postId1", "postId2", "date-1558290600000", "postId3"],
            1,
        );
...
describe("PostUtils.getLatestPostId", () => {
    test("Should not return LOAD_OLDER_MESSAGES_TRIGGER", () => {
        const postId = PostUtils.getLatestPostId([
            PostListRowListIds.LOAD_OLDER_MESSAGES_TRIGGER,
            "postId1",
            "postId2",
        ]);
...
describe("PostUtils.createAriaLabelForPost", () => {
    ...
    test("Should show username, timestamp, message, attachments, reactions, flagged and pinned", () => {
        ...
        const ariaLabel = PostUtils.createAriaLabelForPost(
            testPost,
            author,
            isFlagged,
            reactions,
    ...
    test("Should translate emoji into {emoji-name} emoji", () => {
        ...
        const testPost = TestHelper.getPostMock({
            message:
                "emoji_test :smile: :+1: :non-potable_water: :space emoji: :not_an_emoji:",
            create_at: new Date().getTime() / 1000 || 0,
        });
        const author = "test_author";
        const reactions = {};
    ...
    test("Generating aria label should not break if message is undefined", () => {
        ...
        expect(() =>
            PostUtils.createAriaLabelForPost(
                testPost,
                author,
                isFlagged,
                reactions,
...
describe("PostUtils.splitMessageBasedOnCaretPosition", () => {
    ...
    it("should return an object with two strings when given context and message", () => {
        const stringPieces = PostUtils.splitMessageBasedOnCaretPosition(
            state.caretPosition,
            message,
        );
...
describe("PostUtils.splitMessageBasedOnTextSelection", () => {
    const cases: Array<
        [number, number, string, { firstPiece: string; lastPiece: string }]
    > = [
        [
            0,
            0,
            "Test Replace Message",
            { firstPiece: "", lastPiece: "Test Replace Message" },
        ],
        [
            20,
            20,
            "Test Replace Message",
            { firstPiece: "Test Replace Message", lastPiece: "" },
        ],
        ...
        [
            ...
            10,
            "Test Replace Message",
            { firstPiece: "", lastPiece: "ce Message" },
        ],
        [
            5,
            12,
            "Test Replace Message",
            { firstPiece: "Test ", lastPiece: " Message" },
        ],
        [
            7,
            20,
            "Test Replace Message",
            { firstPiece: "Test Re", lastPiece: "" },
        ],
    ];

    test.each(cases)(
        "should return an object with two strings when given context and message",
        (start, end, message, expected) => {
            expect(
                PostUtils.splitMessageBasedOnTextSelection(start, end, message),
            ).toEqual(expected);
        },
    );
});
...
describe("PostUtils.getPostURL", () => {
    ...
    const dmChannel = TestHelper.getChannelMock({
        id: "dm_channel_id",
        name: "current_user_id__user_id_1",
        type: Constants.DM_CHANNEL as "D",
        team_id: "",
    });
    const gmChannel = TestHelper.getChannelMock({
        id: "gm_channel_id",
        name: "gm_channel_name",
        type: Constants.GM_CHANNEL as "G",
        team_id: "",
    });
    ...
    const dmPost = TestHelper.getPostMock({
        id: "dm_post_id",
        channel_id: dmChannel.id,
    });
    const gmPost = TestHelper.getPostMock({
        id: "gm_post_id",
    ...
    const dmReply = TestHelper.getPostMock({
        id: "dm_reply_id",
        root_id: "root_post_id_1",
        channel_id: dmChannel.id,
    });
    const gmReply = TestHelper.getPostMock({
        id: "gm_reply_id",
        root_id: "root_post_id_1",
        channel_id: gmChannel.id,
    });
    const reply = TestHelper.getPostMock({
        id: "reply_id",
        root_id: "root_post_id_1",
        channel_id: channel.id,
    });
    ...
    test.each([
        ...
        [
            "/current_team_name/messages/gm_channel_name/gm_post_id",
            gmPost,
            true,
        ],
        ...
        "root posts should return %s",
        (expected, postCase, collapsedThreads) => {
    ...
    test.each([
        ...
        [
            "/current_team_name/messages/@jessicahyde/dm_reply_id",
            dmReply,
            false,
        ],
        [
            "/current_team_name/messages/gm_channel_name/gm_reply_id",
            gmReply,
            false,
        ],
...
describe("PostUtils.isWithinCodeBlock", () => {
    ...
    it("should produce consistent results when called multiple times", () => {
        ...
        const results = Array.from({ length: 10 }, () =>
            PostUtils.isWithinCodeBlock(message, caretPosition),
        );
...
describe("PostUtils.getUserOrGroupFromMentionName", () => {
    ...
    test.each([
        ...
        [
            "nothing when not matching user or group",
            "user3",
            false,
            [undefined, undefined],
        ],
        ...
        [
            "no group when groups highlights are disabled",
            groupMention,
            true,
            [undefined, undefined],
        ],
        [
            "user when there is a matching user and group mention",
            userAndGroupMention,
            false,
            [user2, undefined],
        ],
