...
export function getSiteURL(): string {
    return (
        getModule<() => string>("utils/url/getSiteURL")?.() ??
        getSiteURLFromWindowObject(window)
    );
}
...
export function getRelativeChannelURL(
    teamName: string,
    channelName: string,
): string {
...
export function validateChannelUrl(
    ...
    const directMessageRegex = new RegExp(
        `^.{${USER_ID_LENGTH}}__.{${USER_ID_LENGTH}}$`,
    );
    ...
    if (
        ...
        if (urlLonger) {
            errors.push(
                formattedError(
                    t("change_url.longer"),
                    "URLs must have at least 2 characters.",
                    intl,
                ),
            );
        }
        ...
        if (urlShorter) {
            errors.push(
                formattedError(
                    t("change_url.shorter"),
                    "URLs must have maximum 64 characters.",
                    intl,
                ),
            );
        }
        ...
        if (url.match(/[^A-Za-z0-9-_]/)) {
            errors.push(
                formattedError(
                    t("change_url.noSpecialChars"),
                    "URLs cannot use special characters.",
                    intl,
                ),
            );
        }
        ...
        if (isDirectMessageFormat) {
            errors.push(
                formattedError(
                    t("change_url.invalidDirectMessage"),
                    "User IDs are not allowed in channel URLs.",
                    intl,
                ),
            );
        }

        const startsWithoutLetter =
            url.charAt(0) === "-" || url.charAt(0) === "_";
        ...
        if (startsWithoutLetter && endsWithoutLetter) {
            ...
            errors.push(
                formattedError(
                    ...
                    "URLs must start with a lowercase letter or number.",
                    intl,
                ),
            );
            ...
            errors.push(
                formattedError(
                    ...
                    "URLs must end with a lowercase letter or number.",
                    intl,
                ),
            );
        }
        ...
        if (errors.length === 0) {
            errors.push(
                formattedError(t("change_url.invalidUrl"), "Invalid URL", intl),
            );
        }
    }
...
export function shouldOpenInNewTab(
    ...
    const path = url.startsWith("/")
        ? url
        : url.substring(siteURL?.length || 0);
    ...
    return unhandledPaths.some((unhandledPath) =>
        new RegExp("^/" + unhandledPath + "\\b").test(path),
    );
}
...
export function isPermalinkURL(url: string): boolean {
    ...
    const regexp = new RegExp(
        `^(${siteURL})?/[a-z0-9]+([a-z\\-0-9]+|(__)?)[a-z0-9]+/pl/\\w+`,
        "gu",
    );
...
export function teamNameToUrl(teamName: string): UrlValidationCheck {
    ...
    if (
        url.length < Constants.MIN_TEAMNAME_LENGTH ||
        url.length > Constants.MAX_TEAMNAME_LENGTH
    ) {
        return { url, error: BadUrlReasons.Length };
    }

    if (
        Constants.RESERVED_TEAM_NAMES.some((reservedName) =>
            url.startsWith(reservedName),
        )
    ) {
...
export function channelNameToUrl(channelName: string): UrlValidationCheck {
    ...
    if (
        url.length < Constants.MIN_CHANNELNAME_LENGTH ||
        url.length > Constants.MAX_CHANNELNAME_LENGTH
    ) {
        return { url, error: BadUrlReasons.Length };
    }
