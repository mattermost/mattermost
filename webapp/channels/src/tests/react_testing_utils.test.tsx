...
describe("renderWithFullContext", () => {
    ...
    test("should be able to render react-router components", () => {
        ...
        expect(
            screen.getByText("this is the route component"),
        ).toBeInTheDocument();
        expect(
    ...
    test("should be able to render components that use connect to access the Redux store", () => {
        ...
        expect(
            screen.getByText("There are 0 users loaded"),
        ).toBeInTheDocument();
    });

    test("should be able to render components that use hooks to access the Redux store", () => {
        const TestComponent = () => {
            const numProfiles = useSelector(
                (state: GlobalState) =>
                    Object.keys(state.entities.users.profiles).length,
            );
    ...
    test("should be able to rerender components without losing context", () => {
        ...
        rerender(<TestComponent appTitle="Mattermots" />);
    ...
    test("should be able to inject store state and replace it later", () => {
        ...
        const TestComponent = () => {
            const user1 = useSelector((state: GlobalState) =>
                getUser(state, "user1"),
            );
            const user2 = useSelector((state: GlobalState) =>
                getUser(state, "user2"),
            );

            return (
                <div>{`User1 is ${user1.username} and User2 is ${user2.username}!`}</div>
            );
        };
        ...
        expect(
            screen.getByText("User1 is Alpha and User2 is Bravo!"),
        ).toBeInTheDocument();
        ...
        expect(
            screen.getByText("User1 is Charlie and User2 is Bravo!"),
        ).toBeInTheDocument();
        ...
        expect(
            screen.getByText("User1 is Alpha and User2 is Delta!"),
        ).toBeInTheDocument();
    });
    ...
    test("should be able to update store state", () => {
        ...
        expect(
            screen.getByText("User1 is Echo and User2 is Foxtrot!"),
        ).toBeInTheDocument();
        ...
        expect(
            screen.getByText("User1 is Golf and User2 is Foxtrot!"),
        ).toBeInTheDocument();
        ...
        expect(
            screen.getByText("User1 is Golf and User2 is Hotel!"),
        ).toBeInTheDocument();
    });
});
