...
export function showDraftsPulsatingDotAndTourTip(state: GlobalState): boolean {
    ...
    const draftsTourTipShowed = get(
        state,
        Preferences.CATEGORY_DRAFTS,
        Preferences.DRAFTS_TOUR_TIP_SHOWED,
        "",
    );
    const draftsAlreadyViewed =
        draftsTourTipShowed &&
        JSON.parse(draftsTourTipShowed)[Preferences.DRAFTS_TOUR_TIP_SHOWED];
    ...
    return createSelector(
        ...
        (channelDrafts, rhsDrafts, myChannels) =>
            [...channelDrafts, ...rhsDrafts]
                .filter((draft) =>
                    excludeInactive
                        ? myChannels.indexOf(draft.value.channelId) !== -1
                        : true,
                )
...
export function makeGetDraftsCount(): DraftCountSelector {
    ...
    return createSelector(
        ...
        getMyActiveChannelIds,
        (channelDrafts, rhsDrafts, myChannels) =>
            [...channelDrafts, ...rhsDrafts].filter(
                (draft) => myChannels.indexOf(draft.value.channelId) !== -1,
            ).length,
    );
}
