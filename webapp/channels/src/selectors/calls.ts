...
export function isCallsEnabled(state: GlobalState, minVersion = "0.4.2") {
    return Boolean(
        state.plugins.plugins[suitePluginIds.calls] &&
            semver.gte(
                String(
                    semver.clean(
                        state.plugins.plugins[suitePluginIds.calls].version ||
                            "0.0.0",
                    ),
                ),
                minVersion,
...
export function isCallsRingingEnabledOnServer(state: GlobalState) {
    ...
    // @ts-ignore
    return Boolean(
        state[`plugins-${suitePluginIds.calls}`]?.callsConfig?.EnableRinging,
    );
}
