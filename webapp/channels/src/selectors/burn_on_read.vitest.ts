...
describe('selectors/burn_on_read', () => {
    ...
    describe('isBurnOnReadEnabled', () => {
        test('should return true when config.EnableBurnOnRead is true', () => {
            state.entities.general.config.EnableBurnOnRead = 'true';
            const result = isBurnOnReadEnabled(state);
            expect(result).toBe(true);
        });

        test('should return false when config.EnableBurnOnRead is false', () => {
            state.entities.general.config.EnableBurnOnRead = 'false';
            const result = isBurnOnReadEnabled(state);
            expect(result).toBe(false);
        });

        test('should return false when config.EnableBurnOnRead is not set', () => {
            const result = isBurnOnReadEnabled(state);
            expect(result).toBe(false);
        });
    });
    ...
    describe('getBurnOnReadDurationMinutes', () => {
        test('should return default 10 minutes when not configured', () => {
            const result = getBurnOnReadDurationMinutes(state);
            expect(result).toBe(10);
        });

        test('should return configured duration', () => {
            state.entities.general.config.BurnOnReadDurationMinutes = '15';
            const result = getBurnOnReadDurationMinutes(state);
            expect(result).toBe(15);
        });

        test('should parse different duration values', () => {
    ...
    describe('canUserSendBurnOnRead', () => {
        ...
        test('should return false when feature is not configured', () => {
            const result = canUserSendBurnOnRead(state);
            expect(result).toBe(false);
        });
    });
    ...
    describe('hasSeenBurnOnReadTourTip', () => {
        test('should return false when user has not seen tour tip', () => {
            const result = hasSeenBurnOnReadTourTip(state);
            expect(result).toBe(false);
        });

        test('should return true when user has seen tour tip', () => {
            const pref = {
        ...
        test('should return false when preference value is 0', () => {
            const pref = {
                category: BURN_ON_READ_TOUR_TIP_PREFERENCE,
                name: user.id,
                user_id: user.id,
