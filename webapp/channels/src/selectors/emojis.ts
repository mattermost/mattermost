...
export function normalizeRecentEmojisData(
    data: RecentEmojiData[],
    userSkinTone: string,
) {
    ...
    for (const recentEmoji of data) {
        ...
        if (systemEmoji) {
            ...
            normalizedName = convertEmojiSkinTone(
                systemEmoji,
                userSkinTone,
            ).short_name;
        } else {
    ...
    normalizedData.sort(
        (emojiA: RecentEmojiData, emojiB: RecentEmojiData) =>
            emojiA.usageCount - emojiB.usageCount,
    );
