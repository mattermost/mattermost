...
import {
    ResourceToSysConsolePermissionsTable,
    RESOURCE_KEYS,
} from "mattermost-redux/constants/permissions_sysconsole";
...
export const getConsoleAccess = createSelector(
    ...
    (adminDefinition, mySystemPermissions) => {
        ...
        const addEntriesForKey = (entryKey) => {
            const permissions = ResourceToSysConsolePermissionsTable[
                entryKey
            ].filter((x) => mySystemPermissions.has(x));
        ...
        const mapAccessValuesForKey = ([key]) => {
            if (typeof RESOURCE_KEYS[key.toUpperCase()] === "object") {
                Object.values(RESOURCE_KEYS[key.toUpperCase()]).forEach(
                    (entry) => {
