...
export const getABTestPreferences = (() => {
    return (state: GlobalState) =>
        getCategory(state, Preferences.AB_TEST_PREFERENCE_VALUE);
})();
...
const getFirstChannelNamePref = createSelector(
    ...
    (preferences) => {
        return preferences.find(
            (pref) =>
                pref.name === RecommendedNextStepsLegacy.CREATE_FIRST_CHANNEL,
        );
    },
);
...
export function getFirstChannelName(state: GlobalState) {
    return (
        getFirstChannelNameViews(state) ||
        getFirstChannelNamePref(state)?.value ||
        ""
    );
}
...
const getSteps = createSelector(
    ...
    (currentUser, firstAdmin) => {
        const roles = firstAdmin
            ? `first_admin ${currentUser?.roles}`
            : currentUser?.roles;
...
export const legacyNextStepsNotFinished = createSelector(
    ...
    (state: GlobalState) =>
        getCategory(state, Preferences.RECOMMENDED_NEXT_STEPS),
    (state: GlobalState) => getCurrentUser(state),
    (state: GlobalState) => isFirstAdmin(state),
    ...
    (stepPreferences, currentUser, firstAdmin, mySteps) => {
        const roles = firstAdmin
            ? `first_admin ${currentUser?.roles}`
            : currentUser?.roles;
        const checkPref = (step: StepType) =>
            stepPreferences.some(
                (pref) =>
                    (pref.name === step.id && pref.value === "true") ||
...
export const hasLegacyNextStepsPreferences = createSelector(
    ...
    (stepPreferences, mySteps) => {
        const checkPref = (step: StepType) =>
            stepPreferences.some((pref) => pref.name === step.id);
...
export const getShowTaskListBool = createSelector(
    ...
    (state, onboardingPreferences, legacyStepsPreferences, isMobileView) => {
        ...
        const hasUserStartedOnboardingTaskListProcess =
            onboardingPreferences?.some(
                (pref) =>
                    pref.name ===
                        OnboardingTaskList.ONBOARDING_TASK_LIST_SHOW ||
                    pref.name === OnboardingTaskList.ONBOARDING_TASK_LIST_OPEN,
            );

        const taskListStatus = getBool(
            state,
            OnboardingTaskCategory,
            OnboardingTaskList.ONBOARDING_TASK_LIST_SHOW,
        );
        ...
        const hasSkipLegacyStepsPreference = legacyStepsPreferences.some(
            (pref) => pref.name === RecommendedNextStepsLegacy.SKIP,
        );
        const hideLegacyStepsSetToFalse = legacyStepsPreferences.some(
            (pref) =>
                pref.name === RecommendedNextStepsLegacy.HIDE &&
                pref.value === "false",
        );
        const hasAnyOfTheLegacyStepsPreferences =
            hasLegacyNextStepsPreferences(state);
        const areFirstUserPrefs =
            !hasSkipLegacyStepsPreference &&
            hideLegacyStepsSetToFalse &&
            !hasAnyOfTheLegacyStepsPreferences;

        const completelyNewUserForOnboarding =
            !hasUserStartedOnboardingTaskListProcess && areFirstUserPrefs;
        ...
        const hasSkippedLegacySteps = legacyStepsPreferences.some(
            (pref) =>
                pref.name === RecommendedNextStepsLegacy.SKIP &&
                pref.value === "true",
        );
        const hasCompletedLegacySteps = legacyStepsPreferences.some(
            (pref) =>
                pref.name === RecommendedNextStepsLegacy.HIDE &&
                pref.value === "true",
        );

        const existingUserHasntFinishedNorSkippedLegacyNextSteps =
            !hasSkippedLegacySteps && !hasCompletedLegacySteps;

        const showTaskList =
            existingUserHasntFinishedNorSkippedLegacyNextSteps && !isMobileView;
        const firstTimeOnboarding =
            existingUserHasntFinishedNorSkippedLegacyNextSteps;
