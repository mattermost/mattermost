...
export const getExpandSeatsLink: (
    state: GlobalState,
) => (licenseId: string) => string = createSelector(
    "getExpandSeatsLink",
    getConfig,
    (config) => {
...
export const getCloudDelinquentInvoices = createSelector(
    ...
    (state: GlobalState) =>
        state.entities.cloud.invoices as Record<string, Invoice>,
    (invoices: Record<string, Invoice>) => {
        ...
        return Object.values(invoices || []).filter(
            (invoice) => invoice.status !== "paid" && invoice.total > 0,
        );
    },
);
...
export const isCloudDelinquencyGreaterThan90Days = createSelector(
    ...
    (subscription: Subscription) => {
        ...
        return (
            Math.floor(
                (now.getTime() - delinquentDate.getTime()) /
                    (1000 * 60 * 60 * 24),
            ) >= 90
        );
    },
...
export const isCwsMockMode = (state: GlobalState) =>
    getConfig(state)?.CWSMock === "true";
