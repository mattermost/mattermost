...
export function getIsPostBeingEdited(state: GlobalState, postId: string) {
    return (
        state.views.posts.editingPost.postId === postId &&
        state.views.posts.editingPost.show
    );
}
...
export const getEditingPost = createSelector(
    ...
    (state: GlobalState) =>
        getPost(state, state.views.posts.editingPost.postId),
    (editingPost, post) => {
        return {
            ...editingPost,
            post,
...
export function isInlineImageVisible(
    state: GlobalState,
    postId: string,
    imageKey: string,
) {
