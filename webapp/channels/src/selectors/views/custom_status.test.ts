...
describe("showStatusDropdownPulsatingDot and showPostHeaderUpdateStatusButton", () => {
    ...
    it("should return false if user has opened the custom status modal before", async () => {
        const store = await configureStore();
        preference.myPreference.value = JSON.stringify({
            [Preferences.CUSTOM_STATUS_MODAL_VIEWED]: true,
        });
        (PreferenceSelectors.get as jest.Mock).mockReturnValue(
            preference.myPreference.value,
        );
    ...
    it("should return false if user was created less than seven days before from today", async () => {
        ...
        const todayMinusSixDays = addTimeToTimestamp(
            todayTimestamp,
            TimeInformation.DAYS,
            6,
            TimeInformation.PAST,
        );
    ...
    it("should return true if user was created more than seven days before from today", async () => {
        const store = await configureStore();
        preference.myPreference.value = JSON.stringify({
            [Preferences.CUSTOM_STATUS_MODAL_VIEWED]: false,
        });
        (PreferenceSelectors.get as jest.Mock).mockReturnValue(
            preference.myPreference.value,
        );
        ...
        const todayMinusEightDays = addTimeToTimestamp(
            todayTimestamp,
            TimeInformation.DAYS,
            8,
            TimeInformation.PAST,
