...
describe("isUnreadFilterEnabled", () => {
    const preferenceKey = getPreferenceKey(
        ...
        Preferences.SHOW_UNREAD_SECTION,
    );
...
describe("getUnreadChannels", () => {
    ...
    const unreadChannel1 = {
        id: "unreadChannel1",
        delete_at: 0,
        last_post_at: 100,
    };
    const unreadChannel2 = {
        id: "unreadChannel2",
        delete_at: 0,
        last_post_at: 200,
    };
    ...
    test("should return channels sorted by recency", () => {
        expect(Selectors.getUnreadChannels(baseState)).toEqual([
            unreadChannel2,
            unreadChannel1,
            currentChannel,
        ]);
    ...
    test("should return channels with mentions before those without", () => {
        ...
        expect(Selectors.getUnreadChannels(state)).toEqual([
            unreadChannel1,
            unreadChannel2,
            currentChannel,
        ]);
...
describe("getDisplayedChannels", () => {
    ...
    const category1 = {
        id: "category1",
        team_id: "team1",
        channel_ids: [currentChannel.id, unreadChannel1.id],
    };
    const category2 = {
        id: "category2",
        team_id: "team1",
        channel_ids: [readChannel.id, unreadChannel2.id],
    };
    ...
    describe("memoization", () => {
        test("should return the same result when called with the same state", () => {
            expect(Selectors.getDisplayedChannels(baseState)).toBe(
                Selectors.getDisplayedChannels(baseState),
            );
        });
        ...
        test("should return the same result when called with identical state", () => {
            ...
            expect(Selectors.getDisplayedChannels(baseState)).toBe(
                Selectors.getDisplayedChannels(modifiedState),
            );
        });
        ...
        test("should return a new result when the unreads section is enabled", () => {
            ...
            expect(Selectors.getDisplayedChannels(baseState)).not.toBe(
                Selectors.getDisplayedChannels(modifiedState),
            );
            expect(Selectors.getDisplayedChannels(modifiedState)).toBe(
                Selectors.getDisplayedChannels(modifiedState),
            );
        });
    });
...
describe("makeGetFilteredChannelIdsForCategory", () => {
    ...
    test("should only include channels in the given category", () => {
        ...
        const getFilteredChannelIdsForCategory =
            Selectors.makeGetFilteredChannelIdsForCategory();

        expect(getFilteredChannelIdsForCategory(state, category1)).toEqual([
            currentChannel.id,
            unreadChannel1.id,
        ]);
        expect(getFilteredChannelIdsForCategory(state, category2)).toEqual([
            readChannel.id,
            unreadChannel2.id,
        ]);
    });
