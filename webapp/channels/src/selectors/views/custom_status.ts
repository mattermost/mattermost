...
export function makeGetCustomStatus(): (
    ...
    return createSelector(
        ...
        (state: GlobalState, userID?: string) =>
            userID ? getUser(state, userID) : getCurrentUser(state),
        (user) => {
            const userProps = user?.props || {};
            return userProps.customStatus
...
export function isCustomStatusExpired(
    state: GlobalState,
...
export const getRecentCustomStatuses = createSelector(
    ...
    (state: GlobalState) =>
        get(
            ...
            Preferences.NAME_RECENT_CUSTOM_STATUSES,
        ),
    (value) => {
...
function showCustomStatusPulsatingDotAndPostHeader(state: GlobalState) {
    ...
    const hasUserCreationMoreThanSevenDays = isDateWithinDaysRange(
        currentUser?.create_at,
        7,
        TimeInformation.FUTURE,
    );
    ...
    const modalAlreadyViewed =
        customStatusTutorialState &&
        JSON.parse(customStatusTutorialState)[
            Preferences.CUSTOM_STATUS_MODAL_VIEWED
        ];
