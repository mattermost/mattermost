...
describe('handleThreadUpdated', () => {
    const makeMsg = (postType) => ({
    ...
    // store.dispatch is a bare jest.fn() that does not execute thunks, so handleEvent only
    // records the thunk. Pull it back out and run it to exercise the handler itself.
    const runHandler = (postType) => {
        store.dispatch.mockClear();
        handleEvent(makeMsg(postType));

        const thunk = store.dispatch.mock.calls[0][0];
        const innerDispatch = jest.fn();
        const state = mergeObjects(mockState, {
            views: {threads: {selectedThreadIdInTeam: {}, manuallyUnread: {}}},
            entities: {threads: {threads: {}, threadsInTeam: {}, counts: {}}},
        });
        thunk(innerDispatch, () => state);
        return innerDispatch;
    };

    test('should not update the Threads store for a Docs page-comment thread', () => {
        // This is the one leak group 15's server-side thread_store exclusion cannot reach,
        // because the event is published per follower rather than read back through the
        // store. If this regresses, Docs comment threads appear in the global Threads list
        // indistinguishable from chat threads.
        expect(runHandler('custom_page_comment')).not.toHaveBeenCalled();
    });

    test('should still update the Threads store for an ordinary chat thread', () => {
