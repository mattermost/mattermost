...
jest.mock("mattermost-redux/actions/teams", () => ({
    ...
    sendEmailInvitesToTeamGracefully: (team: string, emails: string[]) => {
        if (team === "incorrect-default-smtp") {
            return {
                ...
                data: emails.map((email) => ({
                    email,
                    error: {
                        message: "SMTP is not configured in System Console.",
            ...
            return {
                ...
                data: emails.map((email) => ({
                    email,
                    error: { message: "Invite emails rate limit exceeded." },
                })),
            };
        } else if (team === "error") {
            return {
                ...
                data: emails.map((email) => ({
                    email,
                    error: { message: "Unable to add the user to the team." },
                })),
            };
        }
        ...
        return {
            type: "MOCK_RECEIVED_ME",
            data: emails.map((email) => ({ email, error: undefined })),
        };
    },
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
        ...
        if (teamId === "incorrect-default-smtp") {
            ...
            return {
                ...
                data: emails.map((email) => ({
                    email,
                    error: {
                        message: "Unable to add the guest to the channels.",
                    },
                })),
            };
        }
...
describe("actions/invite_actions", () => {
    ...
    describe("sendMembersInvites", () => {
        it("should generate and empty list if nothing is passed", async () => {
            const response = await sendMembersInvites(
                "correct",
                [],
                [],
            )(store.dispatch as DispatchFunc, store.getState as GetStateFunc);
        ...
        it("should generate a failure for problems adding a user", async () => {
            ...
            expect(response).toEqual({
                data: {
                    sent: [
                        {
                            user: { id: "other-user", roles: "system_user" },
                            reason: "This member has been added to the team.",
                        },
                    ],
                    notSent: [
    ...
    describe("sendGuestsInvites", () => {
        ...
        it("should generate a failure for users that are part of all or some of the channels", async () => {
            ...
            const response = await sendGuestsInvites(
                "correct",
                [{ id: "correct" }, { id: "correct2" }] as Channel[],
                users,
                [],
                "message",
            )(store.dispatch as DispatchFunc, store.getState as GetStateFunc);
