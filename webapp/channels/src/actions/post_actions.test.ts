...
jest.mock("mattermost-redux/actions/posts", () => ({
    ...
    createPostImmediately: (...args: any[]) => ({
    ...
    receivedNewPost: (...args: any[]) => ({
        type: "MOCK_RECEIVED_NEW_POST",
        args,
    }),
}));

jest.mock("actions/emoji_actions", () => ({
    addRecentEmoji: (...args: any[]) => ({
        type: "MOCK_ADD_RECENT_EMOJI",
        args,
    }),
    addRecentEmojis: (...args: any[]) => ({
...
jest.mock("actions/notification_actions", () => ({
    sendDesktopNotification: jest
        .fn()
        .mockReturnValue({ type: "MOCK_SEND_DESKTOP_NOTIFICATION" }),
}));
...
describe("Actions.Posts", () => {
    ...
    test("handleNewPost", async () => {
        ...
        const msg = {
            data: { team_a: "team_a", mentions: ["current_user_id"] },
        };
    ...
    test("handleNewPostOtherChannel", async () => {
        ...
        const msg = {
            data: { team_b: "team_b", mentions: ["current_user_id"] },
        };
    ...
    test("unsetEditingPost", async () => {
        ...
        const testStore = mockStore({ ...initialState });
        const { data: dataSet } = await testStore.dispatch(
            (Actions.setEditingPost as any)("latest_post_id", "test", "title"),
        );
        ...
        const { data: dataUnset } = testStore.dispatch(
    ...
    test("setEditingPost", async () => {
        ...
        const { data } = await testStore.dispatch(
            Actions.setEditingPost("latest_post_id", "test", "title"),
        );
        ...
        const newLatestPost = {
            ...latestPost,
            pending_post_id: latestPost.id,
        } as Post;
        const withPendingPostState = { ...initialState };
        withPendingPostState.entities.posts.posts[latestPost.id] =
            newLatestPost;
    ...
    test("flagPost", async () => {
        ...
        } as unknown as GlobalState["views"]["rhs"];
