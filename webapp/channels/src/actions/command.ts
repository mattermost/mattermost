...
export function executeCommand(message: string, args: CommandArgs): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        switch (cmd) {
            case "/search":
                dispatch(
                    PostActions.searchForTerm(
                        msg.substring(cmdLength + 1, msg.length),
                    ),
                );
                return { data: true };
            case "/shortcuts":
            ...
            case "/leave": {
                ...
                if (channel.type === Constants.PRIVATE_CHANNEL) {
                    dispatch(
                        openModal({
                            modalId:
                                ModalIdentifiers.LEAVE_PRIVATE_CHANNEL_MODAL,
            ...
            case "/marketplace":
                ...
                if (
                    ...
                    return {
                        error: {
                            message: localizeMessage(
                                "marketplace_command.no_permission",
                                "You do not have the appropriate permissions to access the marketplace.",
                            ),
                        },
                    };
                }
                ...
                dispatch(
                    openModal({
                        ...
                        dialogProps: { openedFrom: "command" },
                    }),
                );
                return { data: true };
            case "/collapse":
        ...
        if (appsEnabled(state)) {
            ...
            const parser = new AppCommandParser(
                { dispatch, getState: getGlobalState } as any,
                intlShim,
                args.channel_id,
                args.team_id,
                args.root_id,
            ...
            if (parser.isAppCommand(msg)) {
                try {
                    const { creq, errorMessage } =
                        await parser.composeCommandSubmitCall(msg);
                    if (!creq) {
                        return createErrorMessage(errorMessage!);
                    }

                    const res = (await dispatch(
                        doAppSubmit(creq, intlShim),
                    )) as DoAppCallResult;
                    ...
                    switch (callResp.type) {
                        ...
                        case AppCallResponseTypes.FORM:
                            if (callResp.form) {
                                dispatch(
                                    openAppsModal(callResp.form, creq.context),
                                );
                            }
                            return { data: true };
        ...
        const hasGotoLocation =
            data.goto_location && isUrlSafe(data.goto_location);

        if (msg.trim() === "/logout") {
            GlobalActions.emitUserLoggedOutEvent(
                hasGotoLocation ? data.goto_location : "/",
            );
            return { data: true };
        }
        ...
        if (hasGotoLocation) {
            if (data.goto_location.startsWith("/")) {
                ...
                getHistory().push(
                    data.goto_location.substr(getSiteURL().length),
                );
            } else {
