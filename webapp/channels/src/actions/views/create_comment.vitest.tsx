...
vi.mock('actions/command', async () => ({
    executeCommand: vi.fn((...args) => ({type: 'MOCK_ACTIONS_COMMAND_EXECUTE', args})),
}));

vi.mock('actions/global_actions', async () => ({
    emitUserCommentedEvent: vi.fn(),
}));

vi.mock('actions/hooks', async () => ({
    runMessageWillBePostedHooks: vi.fn((post) => () => ({data: post})),
    runSlashCommandWillBePostedHooks: vi.fn((message, args) => () => ({data: {message, args}})),
}));
...
vi.mock('actions/post_actions', async () => ({
    ...
    createPost: vi.fn(() => ({type: 'MOCK_CREATE_POST'})),
    setEditingPost: (...args) => ({type: 'MOCK_SET_EDITING_POST', args}),
}));

vi.mock('actions/storage', async () => {
    const original = await vi.importActual('actions/storage');
    return {
        ...original,
...
describe('rhs view actions', async () => {
    const initialState = {
        entities: {
            posts: {
                posts: {
    ...
    describe('onSubmit', async () => {
        ...
        test('it submits a regular post when options.ignoreSlash is true', async () => {
            store.dispatch(onSubmit({
                message: '/fakecommand',
                fileInfos: [],
                uploadsInProgress: [],
