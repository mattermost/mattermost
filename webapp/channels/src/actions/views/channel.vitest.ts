...
vi.mock('utils/channel_utils.tsx', async () => {
    const original = await vi.importActual('utils/channel_utils.tsx');
...
vi.mock('mattermost-redux/actions/channels', async () => ({
    ...await vi.importActual('mattermost-redux/actions/channels'),
    markChannelAsRead: vi.fn(() => ({type: ''})),
    leaveChannel: vi.fn(() => ({type: ''})),
    getChannel: vi.fn(() => ({
...
vi.mock('actions/views/rhs', async () => ({
    ...await vi.importActual('actions/views/rhs'),
    closeRightHandSide: vi.fn(() => ({type: ''})),
}));

vi.mock('actions/views/threads', async () => ({
    ...await vi.importActual('actions/views/threads'),
    markThreadAsRead: vi.fn(() => ({type: ''})),
}));

vi.mock('mattermost-redux/actions/posts');

vi.mock('selectors/local_storage', () => ({
    getLastViewedChannelName: () => 'channel1',
}));

vi.mock('mattermost-redux/selectors/entities/utils', () => ({
    makeAddLastViewAtToProfiles: () => vi.fn().mockReturnValue([]),
}));
...
describe('channel view actions', () => {
    ...
    describe('loadLatestPosts', () => {
        ...
        test('when latest posts are received', async () => {
            Date.now = vi.fn().mockReturnValue(12344);
