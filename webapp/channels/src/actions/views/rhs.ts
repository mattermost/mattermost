...
export function performSearch(terms: string, teamId: string, isMentionSearch?: boolean): ThunkActionFunc<unknown> {
    return (dispatch, getState) => {
        ...
        const filesPromise = dispatch(searchFilesWithParams(teamId, {terms: termsWithExtensionsFilters, is_or_search: false, include_deleted_channels: true, time_zone_offset: timezoneOffset, page: 0, per_page: 20}));
