...
import type {FileSearchResults} from '@mattermost/types/files';
...
import type {ActionResult, NewActionFunc, NewActionFuncAsync} from 'mattermost-redux/types/actions';
...
export function updateRhsState(rhsState: string, channelId?: string, previousRhsState?: RhsState): NewActionFunc<boolean, GlobalState> {
...
export function updateSearchTermsForShortcut(): NewActionFunc {
    return (dispatch, getState) => {
...
export function performSearch(terms: string, isMentionSearch?: boolean): ThunkAction<Promise<[ActionResult<FileSearchResults>, ActionResult<PostSearchResults>]>, GlobalState, unknown, Action> {
    return (dispatch, getState) => {
...
export function showSearchResults(isMentionSearch = false): ReturnType<typeof performSearch> {
    return (dispatch, getState) => {
        const state = getState();
...
export function openRHSSearch(): NewActionFunc<boolean, GlobalState> {
    return (dispatch) => {
...
export function openAtPrevious(previous: any): ThunkAction<unknown, GlobalState, unknown, AnyAction> {
    return (dispatch, getState) => {
        if (!previous) {
