...
vi.mock('mattermost-redux/selectors/entities/users', async (importOriginal) => {
    const actual = await importOriginal() as object;
    return {
        ...actual,
        getCurrentUserMentionKeys: vi.fn(() => [{key: '@here'}, {key: '@mattermost'}, {key: '@channel'}, {key: '@all'}]),
    };
});
...
vi.mock('mattermost-redux/actions/search', () => ({
...
describe('rhs view actions', () => {
    ...
    describe('showFlaggedPosts', () => {
        test('it dispatches the right actions', async () => {
            (SearchActions.getFlaggedPosts as any).mockReturnValue((dispatch: Dispatch) => {
    ...
    describe('showPinnedPosts', () => {
        test('it dispatches the right actions for the current channel', async () => {
            (SearchActions.getPinnedPosts as any).mockReturnValue((dispatch: Dispatch) => {
    ...
    describe('selectPostAndHighlight', () => {
        ...
        beforeEach(() => {
            vi.useFakeTimers();
            vi.setSystemTime(POST_CREATED_TIME);
        });
        ...
        it('should select, and highlight a post, and after a delay clear the highlight', () => {
            ...
            vi.advanceTimersByTime(Constants.PERMALINK_FADEOUT);
        ...
        it('should clear highlight only once after last call no matter how many times called', () => {
            ...
            vi.advanceTimersByTime(Constants.PERMALINK_FADEOUT - 2000);
