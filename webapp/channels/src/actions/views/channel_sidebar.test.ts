    ...
    describe("should place newly added channels correctly in the category", () => {
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            test("at " + i, async () => {
                ...
                const actualChannelIds = insertWithoutDuplicates(
                    channelIds,
                    "new",
                    newIndex,
                );
...
describe("multiSelectChannelTo", () => {
    ...
    const initialState = {
        entities: {
            ...
            channels: {
                ...
                channels: {
                    ...channelIds.reduce(
                        (init: { [key: string]: Partial<Channel> }, val) => {
                            init[`category1_${val}`] = {
                                id: `category1_${val}`,
                                delete_at: 0,
                            };
                ...
                myMembers: {
                    ...channelIds.reduce(
                        (
                            init: { [key: string]: Partial<ChannelMembership> },
                            val,
                        ) => {
                            init[`category1_${val}`] = {
                                channel_id: `category1_${val}`,
                                user_id: "user",
                            };
                            return init;
                        },
                ...
                channelsInTeam: {
                    team1: channelIds
                        .map((id) => `category1_${id}`)
                        .concat(channelIds.map((id) => `category2_${id}`)),
                },
            },
            teams: {
                currentTeamId: "team1",
    ...
    test("should only select current channel if is selected ID and none other are selected", () => {
        ...
        expect(
            store.getState().views.channelSidebar.multiSelectedChannelIds,
        ).toEqual(["category1_one"]);
    });
