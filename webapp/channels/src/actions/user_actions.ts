...
import {
    Preferences as PreferencesRedux,
    General,
...
export function loadProfilesAndReloadTeamMembers(
    page: number,
    perPage: number,
    teamId: string,
    options = {},
    ...
    return async (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        ...
        if (data) {
            await Promise.all([
                doDispatch(
                    loadTeamMembersForProfilesList(data, newTeamId, true),
                ),
                doDispatch(loadStatusesForProfilesList(data)),
            ]);
        }
...
export function loadProfilesAndReloadChannelMembers(
    ...
    return async (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        ...
        if (data) {
            await Promise.all([
                doDispatch(
                    loadChannelMembersForProfilesList(data, newChannelId, true),
                ),
                doDispatch(loadStatusesForProfilesList(data)),
            ]);
        }
...
export function loadProfilesAndTeamMembers(
    page: number,
    perPage: number,
    teamId: string,
...
export function searchProfilesAndTeamMembers(
    term = "",
    options: Record<string, any> = {},
) {
    return async (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        ...
        const { data } = await doDispatch(
            UserActions.searchProfiles(term, options),
        );
        if (data) {
            await Promise.all([
                doDispatch(loadTeamMembersForProfilesList(data, newTeamId)),
...
export function searchProfilesAndChannelMembers(
    term: string,
    options: Record<string, any> = {},
) {
    return async (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        const newChannelId =
            options.in_channel_id || getCurrentChannelId(doGetState());
        const { data } = await doDispatch(
            UserActions.searchProfiles(term, options),
        );
        if (data) {
            await Promise.all([
                doDispatch(
                    loadChannelMembersForProfilesList(data, newChannelId),
                ),
                doDispatch(loadStatusesForProfilesList(data)),
            ]);
        }
...
export function loadTeamMembersForProfilesList(
    ...
    return async (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        ...
        for (let i = 0; i < profiles.length; i++) {
            ...
            if (
                reloadAllMembers === true ||
                !getTeamMember(state, teamIdParam, pid)
            ) {
...
export function loadProfilesWithoutTeam(
    page: number,
    perPage: number,
    options?: Record<string, any>,
) {
    return async (doDispatch: DispatchFunc) => {
        const { data } = await doDispatch(
            UserActions.getProfilesWithoutTeam(page, perPage, options),
        );
...
export function loadChannelMembersForProfilesList(
    profiles: UserProfile[],
    channelId: string,
    reloadAllMembers = false,
) {
...
export function loadNewDMIfNeeded(channelId: string) {
    return async (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        ...
        if (channel) {
            ...
            result = (await getChannelAndMyMember(channelId)(
                doDispatch,
                doGetState,
            )) as ActionResult;
            if (result.data) {
...
export function loadProfilesForGroupChannels(groupChannels: Channel[]) {
    return (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        ...
        if (groupChannelsToFetch.length > 0) {
            doDispatch(
                UserActions.getProfilesInGroupChannels(groupChannelsToFetch),
            );
            return { data: true };
        }
...
export const getGMsForLoading = (state: GlobalState) => {
    ...
    channels = channels.filter(
        (channel) => channel.type === General.GM_CHANNEL,
    );
...
export async function loadProfilesForDM() {
    ...
    await loadCustomEmojisForCustomStatusesByUserIds(profileIds)(
        dispatch,
        getState,
    );
}
...
export function autocompleteUsersInTeam(username: string) {
    return async (doDispatch: DispatchFunc, doGetState: GetStateFunc) => {
        ...
        const { data } = await doDispatch(
            UserActions.autocompleteUsers(username, currentTeamId),
        );
        return data;
    };
}
...
export function autocompleteUsers(username: string) {
    return async (doDispatch: DispatchFunc) => {
        const { data } = await doDispatch(
            UserActions.autocompleteUsers(username),
        );
        return data;
    };
}
...
export function autoResetStatus() {
    return async (
        doDispatch: DispatchFunc,
        doGetState: GetStateFunc,
    ): Promise<{ data: UserStatus }> => {
        const { currentUserId } = getState().entities.users;
        const { data: userStatus } = await (UserActions.getStatus(
            currentUserId,
        )(doDispatch, doGetState) as Promise<{ data: UserStatus }>);
