...
export function handleBindingClick<Res = unknown>(
    binding: AppBinding,
    context: AppContext,
    intl: any,
): ActionFunc {
    return async (dispatch: DispatchFunc) => {
        ...
        if (form) {
            ...
            if (!form?.submit) {
                const errMsg = intl.formatMessage({
                    ...
                    defaultMessage:
                        "This binding is not properly formed. Contact the App developer.",
                });
        ...
        const callRequest = createCallRequest(binding.submit, context);
...
export function doAppSubmit<Res = unknown>(
    inCall: AppCallRequest,
    intl: any,
): ActionFunc {
    return async () => {
        try {
            ...
            const res = (await Client4.executeAppCall(
                call,
                true,
            )) as AppCallResponse<Res>;
                ...
                case AppCallResponseTypes.FORM:
                    if (!res.form?.submit) {
                        const errMsg = intl.formatMessage({
                            ...
                            defaultMessage:
                                "Response type is `form`, but no valid form was included in response.",
                        });
                        return { error: makeCallErrorResponse(errMsg) };
                    }
                ...
                case AppCallResponseTypes.NAVIGATE: {
                    if (!res.navigate_to_url) {
                        const errMsg = intl.formatMessage({
                            ...
                            defaultMessage:
                                "Response type is `navigate`, but no url was included in response.",
                        });
                        return { error: makeCallErrorResponse(errMsg) };
                    }
                    ...
                    const navigateURL = res.navigate_to_url.startsWith(
                        getSiteURL(),
                    )
...
export function doAppFetchForm<Res = unknown>(
    call: AppCallRequest,
    intl: any,
): ActionFunc {
    return async () => {
...
export function doAppLookup<Res = unknown>(
    call: AppCallRequest,
    intl: any,
): ActionFunc {
    return async () => {
...
export function makeFetchBindings(
    location: string,
): (channelId: string, teamId: string) => ActionFunc {
    return (channelId: string, teamId: string): ActionFunc => {
        return async () => {
            try {
                const allBindings = await Client4.getAppsBindings(
                    channelId,
                    teamId,
                );
                const headerBindings = allBindings.filter(
                    (b) => b.location === location,
                );
                const bindings = headerBindings.reduce(
...
export function postEphemeralCallResponseForPost(
    response: AppCallResponse,
    message: string,
    post: Post,
): ActionFunc {
...
export function postEphemeralCallResponseForChannel(
    response: AppCallResponse,
    message: string,
    channelID: string,
): ActionFunc {
...
export function postEphemeralCallResponseForContext(
    response: AppCallResponse,
    message: string,
    context: AppContext,
): ActionFunc {
...
export function postEphemeralCallResponseForCommandArgs(
    response: AppCallResponse,
    message: string,
    args: CommandArgs,
): ActionFunc {
