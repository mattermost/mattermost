...
jest.mock("mattermost-redux/actions/users", () => {
    ...
    return {
        ...original,
        searchProfiles: (...args: any[]) => ({
            type: "MOCK_SEARCH_PROFILES",
            args,
        }),
        getProfilesInTeam: (...args: any[]) => ({
            type: "MOCK_GET_PROFILES_IN_TEAM",
            args,
        }),
        getProfilesInChannel: (...args: any[]) => ({
            type: "MOCK_GET_PROFILES_IN_CHANNEL",
            args,
            data: [{ id: "user_1" }],
        }),
        getProfilesInGroupChannels: (...args: any[]) => ({
            type: "MOCK_GET_PROFILES_IN_GROUP_CHANNELS",
            args,
        }),
        getStatusesByIds: (...args: any[]) => ({
            type: "MOCK_GET_STATUSES_BY_ID",
            args,
        }),
    };
});
...
jest.mock("mattermost-redux/actions/teams", () => {
    ...
    return {
        ...original,
        getTeamMembersByIds: (...args: any[]) => ({
            type: "MOCK_GET_TEAM_MEMBERS_BY_IDS",
            args,
        }),
    };
});
...
jest.mock("mattermost-redux/actions/channels", () => {
    ...
    return {
        ...original,
        getChannelMembersByIds: (...args: any[]) => ({
...
jest.mock("mattermost-redux/actions/preferences", () => {
    ...
    return {
        ...original,
        deletePreferences: (...args: any[]) => ({
            type: "MOCK_DELETE_PREFERENCES",
            args,
        }),
        savePreferences: (...args: any[]) => ({
...
describe("Actions.User", () => {
    const initialState: GlobalState = {
        ...
        views: {
            channel: {} as GlobalState["views"]["channel"],
            channelSidebar: {
    ...
    test("loadProfilesAndStatusesInChannel", async () => {
        ...
        await testStore.dispatch(
            UserActions.loadProfilesAndStatusesInChannel(
                "channel_1",
                0,
                60,
                "status",
        ...
        expect(actualActions[0].args).toEqual([
            "channel_1",
            0,
            60,
            "status",
    ...
    test("loadProfilesAndTeamMembers", async () => {
        ...
        await testStore.dispatch(
            UserActions.loadProfilesAndTeamMembers(0, 60, "team_1", {}),
        );
        ...
        await testStore.dispatch(
            UserActions.loadProfilesAndTeamMembers(0, 60, "", {}),
        );
    ...
    test("loadProfilesAndReloadChannelMembers", async () => {
        const expectedActions = [
            {
                type: "MOCK_GET_PROFILES_IN_CHANNEL",
                args: ["current_channel_id", 0, 60, "sort", {}],
            },
        ];

        const testStore = mockStore(initialState);
        await testStore.dispatch(
            UserActions.loadProfilesAndReloadChannelMembers(
                0,
                60,
                "current_channel_id",
                "sort",
    ...
    test("loadTeamMembersForProfilesList", async () => {
        ...
        testStore = mockStore(initialState);
        await testStore.dispatch(
            UserActions.loadTeamMembersForProfilesList([], "team_1"),
        );
        expect(testStore.getActions()).toEqual([]);
    });
    ...
    test("loadProfilesForGroupChannels", async () => {
        ...
        const expectedActions = [
            {
                args: [["group_channel_1"]],
                type: "MOCK_GET_PROFILES_IN_GROUP_CHANNELS",
            },
        ];
        const testStore = mockStore(initialState);
        await testStore.dispatch(
            UserActions.loadProfilesForGroupChannels(mockedGroupChannels),
        );
        expect(testStore.getActions()).toEqual(expectedActions);
    });
    ...
    test("searchProfilesAndChannelMembers", async () => {
        const expectedActions = [
            { type: "MOCK_SEARCH_PROFILES", args: ["term", {}] },
        ];

        const testStore = mockStore(initialState);
        await testStore.dispatch(
            UserActions.searchProfilesAndChannelMembers("term"),
        );
        const actualActions = testStore.getActions();
        expect(actualActions[0].args).toEqual(expectedActions[0].args);
        expect(actualActions[0].type).toEqual(expectedActions[0].type);
    ...
    describe("getGMsForLoading", () => {
        ...
        const dmsCategory = {
            ...
            channel_ids: [gmChannel1.id, gmChannel2.id],
        };
        ...
        const baseState = {
            ...
            entities: {
                ...
                channelCategories: {
                    ...
                    orderByTeam: {
                        [initialState.entities.teams.currentTeamId]: [
                            dmsCategory.id,
                        ],
                    },
                },
        ...
        test("should not return autoclosed GMs", () => {
            ...
            expect(
                UserActions.getGMsForLoading(state as unknown as GlobalState),
            ).toEqual([gmChannel1, gmChannel2]);
        ...
        test("should return GMs that are in custom categories, even if they would be automatically hidden in the DMs category", () => {
                ...
                entities: {
                    ...
                    channelCategories: {
                        ...
                        orderByTeam: {
                            ...
                            [baseState.entities.teams.currentTeamId]: [
                                customCategory.id,
                                dmsCategory.id,
                            ],
                        },
            ...
            expect(
                UserActions.getGMsForLoading(state as unknown as GlobalState),
            ).toEqual([gmChannel3, gmChannel1, gmChannel2]);
