...
describe('executeCommand', () => {
    ...
    describe('collapse', () => {
        test('call executeCommand with right params', async () => {
            Client4.executeCommand = vi.fn().mockResolvedValue({});
    ...
    describe('leave', () => {
        test('should send message when command typed in reply threads', async () => {
            GlobalActions.sendEphemeralPost = vi.fn().mockReturnValue({type: 'someaction'});
        ...
        test('should show private modal if channel is private', async () => {
            vi.spyOn(Channels, 'getCurrentChannel').mockReturnValue({type: Constants.PRIVATE_CHANNEL});
        ...
        test('should use user id as name if channel is dm', async () => {
            vi.spyOn(Utils, 'getUserIdFromChannelName').mockReturnValue('userId');
            vi.spyOn(Channels, 'getRedirectChannelNameForTeam').mockReturnValue('channel1');
            vi.spyOn(Teams, 'getCurrentRelativeTeamUrl').mockReturnValue('/team1');
            vi.spyOn(Channels, 'getCurrentChannel').mockReturnValue({type: Constants.DM_CHANNEL});
        ...
        test('should use channel id as name if channel is gm', async () => {
            ...
            vi.spyOn(Channels, 'getRedirectChannelNameForTeam').mockReturnValue('channel1');
            vi.spyOn(Teams, 'getCurrentRelativeTeamUrl').mockReturnValue('/team1');
            vi.spyOn(Channels, 'getCurrentChannel').mockReturnValue({type: Constants.GM_CHANNEL, id: 'channelId'});
    ...
    describe('app command', () => {
        test('should call executeAppCall', async () => {
            ...
            const mocked = vi.fn().mockResolvedValue(Promise.resolve({
