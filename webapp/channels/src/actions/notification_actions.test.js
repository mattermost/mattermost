...
describe("notification_actions", () => {
    describe("sendDesktopNotification", () => {
        ...
        test("should notify user", async () => {
            ...
            return store
                .dispatch(sendDesktopNotification(post, msgProps))
                .then(() => {
        ...
        test.each([UserStatuses.DND, UserStatuses.OUT_OF_OFFICE])(
            "should not notify user on user status %s",
            (status) => {
        ...
        test("should default sound when no sound is specified", () => {
            ...
            baseState.entities.users.profiles.current_user_id.notify_props.desktop_sound =
                "true";
            const store = testConfigureStore(baseState);
            return store
                .dispatch(sendDesktopNotification(post, msgProps))
        ...
        test("should use specified sound when specified", () => {
            ...
            baseState.entities.users.profiles.current_user_id.notify_props.desktop_sound =
                "true";
            baseState.entities.users.profiles.current_user_id.notify_props.desktop_notification_sound =
                "Crackle";
            const store = testConfigureStore(baseState);
            return store
                .dispatch(sendDesktopNotification(post, msgProps))
        ...
        describe("CollapsedThreads: true", () => {
            ...
            test("should not notify user on crt reply when the tab is active and the thread is open", () => {
                baseState.views.threads.selectedThreadIdInTeam.team_id =
                    "root_id";
                baseState.views.browser.focused = true;
                msgProps.mentions = JSON.stringify(["current_user_id"]);
