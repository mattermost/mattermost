...
vi.mock('actions/notification_actions', () => ({
    sendDesktopNotification: vi.fn().mockReturnValue(() => ({data: {}})),
}));
...
vi.mock('utils/user_agent', () => ({
    isIosClassic: vi.fn().mockReturnValueOnce(true).mockReturnValue(false),
    isDesktopApp: vi.fn().mockReturnValue(false),
}));

vi.mock('utils/post_utils', () => ({
    makeGetUniqueEmojiNameReactionsForPost: vi.fn(),
    makeGetIsReactionAlreadyAddedToPost: vi.fn(),
}));

const mockMakeGetIsReactionAlreadyAddedToPost = PostUtils.makeGetIsReactionAlreadyAddedToPost as Mock;
const mockMakeGetUniqueEmojiNameReactionsForPost = PostUtils.makeGetUniqueEmojiNameReactionsForPost as Mock;
