    ...
    test("should wait for async hooks", async () => {
        ...
        const hook = jest.fn((post) => {
            return new Promise((resolve) => {
                setTimeout(() => {
                    resolve({
                        post: { ...post, message: post.message + "async" },
                    });
                }, 100);
...
describe("runSlashCommandWillBePostedHooks", () => {
    ...
    test("should pass the result of each hook to the next", async () => {
        const hook1 = jest.fn((message, args) => ({
            message: message + "a",
            args,
        }));
        const hook2 = jest.fn((message, args) => ({
            message: message + "b",
            args,
        }));
        const hook3 = jest.fn((message, args) => ({
            message: message + "c",
            args,
        }));
...
describe("runMessageWillBeUpdatedHooks", () => {
    test("should do nothing when no hooks are registered", async () => {
        ...
        const result = await store.dispatch(
            runMessageWillBeUpdatedHooks(newPost, oldPost),
        );
    ...
    test("should assume post is unchanged if a hook returns undefined", async () => {
        ...
        const store = mockStore({
            plugins: {
                components: {
                    MessageWillBeUpdated: [{ hook: hook1 }, { hook: hook2 }],
                },
            },
        });
        const oldPost = { message: "test" };
...
describe("runDesktopNotificationHooks", () => {
    ...
    test("should pass the args through every hook", async () => {
        const hook1 = jest.fn((post, msgProps, channel, teamId, args) => ({
            args,
        }));
    ...
    test("should pass the result of each hook to the next", async () => {
        ...
        expect(hook2).toHaveBeenCalledWith(post, msgProps, channel, teamId, {
            ...args,
            title: "Notification titlea",
        });
        expect(hook3).toHaveBeenCalledWith(post, msgProps, channel, teamId, {
            ...args,
            title: "Notification titleab",
            notify: false,
        });
    });
    ...
    test("should wait for async hooks", async () => {
        ...
        const hook = jest.fn((post, msgProps, channel, teamId, args) => {
            return new Promise((resolve) => {
                setTimeout(() => {
                    resolve({
                        args: { ...args, title: args.title + " async" },
                    });
                }, 100);
        ...
        expect(result.args).toEqual({
            ...args,
            title: "Notification title async",
        });
