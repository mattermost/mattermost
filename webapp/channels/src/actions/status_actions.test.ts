...
describe("actions/status_actions", () => {
    const initialState = {
        ...
        entities: {
            ...
            users: {
                ...
                profiles: {
                    user_id2: { id: "user_id2", username: "user2" },
            ...
            posts: {
                posts: {
                    post_id1: { id: "post_id1", user_id: "current_user_id" },
                    post_id2: { id: "post_id2", user_id: "user_id2" },
                },
                postsInChannel: {
                    channel_id1: [
            ...
            preferences: {
                myPreferences: {
                    [Preferences.CATEGORY_DIRECT_CHANNEL_SHOW + "--user_id3"]: {
                        category: Preferences.CATEGORY_DIRECT_CHANNEL_SHOW,
                        name: "user_id3",
                        value: "true",
                        user_id: "current_user_id",
                    },
                    [Preferences.CATEGORY_DIRECT_CHANNEL_SHOW + "--user_id1"]: {
                        category: Preferences.CATEGORY_DIRECT_CHANNEL_SHOW,
                        name: "user_id1",
                        value: "false",
                        user_id: "current_user_id",
    ...
    describe("loadStatusesForChannelAndSidebar", () => {
        test("load statuses with posts in channel and user in sidebar", () => {
            ...
            expect(getStatusesByIds).toHaveBeenCalledWith(
                (expect as GreatExpectations).arrayContainingExactly([
                    "current_user_id",
                    "user_id2",
                    "user_id3",
                ]),
    ...
    describe("loadStatusesForProfilesList", () => {
        test("load statuses for users array", () => {
            ...
            const testStore = mockStore(state);
            testStore.dispatch(
                Actions.loadStatusesForProfilesList([
                    { id: "user_id2", username: "user2" } as UserProfile,
                    { id: "user_id4", username: "user4" } as UserProfile,
                ]),
            );
            expect(getStatusesByIds).toHaveBeenCalledWith(
                (expect as GreatExpectations).arrayContainingExactly([
    ...
    describe("loadStatusesForProfilesMap", () => {
        test("load statuses for users map", () => {
            ...
            const testStore = mockStore(state);
            testStore.dispatch(
                Actions.loadStatusesForProfilesMap({
                    user_id2: {
                        id: "user_id2",
                        username: "user2",
                    } as UserProfile,
