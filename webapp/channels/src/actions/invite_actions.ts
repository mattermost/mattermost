...
export function sendMembersInvites(
    teamId: string,
    users: UserProfile[],
    emails: string[],
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        if (users.length > 0) {
            await dispatch(
                TeamActions.getTeamMembersByIds(
                    teamId,
                    users.map((u) => u.id),
                ),
            );
        ...
        for (const user of users) {
            ...
            if (isGuest(user.roles)) {
                notSent.push({
                    user,
                    reason: localizeMessage(
                        "invite.members.user-is-guest",
                        "Contact your admin to make this guest a full member.",
                    ),
                });
            } else if (member) {
                notSent.push({
        ...
        if (usersToAdd.length > 0) {
            ...
            if (response.error) {
                ...
                for (const userToAdd of usersToAdd) {
                    const memberWithError = members.find(
                        (m: TeamMemberWithError) =>
                            m.user_id === userToAdd.id && m.error,
                    );
                    if (memberWithError) {
                        notSent.push({
                            user: userToAdd,
        ...
        if (emails.length > 0) {
            ...
            try {
                response = await dispatch(
                    TeamActions.sendEmailInvitesToTeamGracefully(
                        teamId,
                        emails,
                    ),
                );
            ...
            if (response.error) {
                if (
                    response.error.server_error_id ===
                    "app.email.rate_limit_exceeded.app_error"
                ) {
                    response.error.message = localizeMessage(
                        "invite.rate-limit-exceeded",
                        "Invite emails rate limit exceeded.",
                    );
                }
                ...
                for (const email of emails) {
                    const inviteWithError = invitesWithErrors.find(
                        (i: TeamInviteWithError) =>
                            email.toLowerCase() === i.email && i.error,
                    );
                    if (
                        inviteWithError &&
                        inviteWithError.error.id ===
                            "api.team.invite_members.unable_to_send_email_with_defaults.app_error" &&
                        ...
                        notSent.push({
                            ...
                            reason: {
                                ...
                                message:
                                    "SMTP is not configured in System Console. Can be configured <a>here</a>.",
                            },
...
export async function sendGuestInviteForUser(
    ...
    for (const channel of channels) {
        const member =
            members && members[channel.id] && members[channel.id][user.id];
        if (member) {
            memberOfAny = true;
        } else {
            memberOfAll = false;
    ...
    if (memberOfAll) {
        return {
            notSent: {
                user,
                reason: localizeMessage(
                    "invite.guests.already-all-channels-member",
    ...
    try {
        ...
        for (const channel of channels) {
            ...
            if (!member) {
                await dispatch(
                    joinChannel(user.id, teamId, channel.id, channel.name),
        ...
        return {
            notSent: {
                ...
                reason: localizeMessage(
                    ...
                    "Unable to add the guest to the channels.",
                ),
            },
        };
    }
    ...
    if (memberOfAny) {
        return {
            notSent: {
                user,
                reason: localizeMessage(
                    "invite.guests.already-some-channels-member",
                    "This person is already a member of some of the channels.",
                ),
            },
        };
    }
...
export function sendGuestsInvites(
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (emails.length > 0) {
            ...
            try {
                response = await dispatch(
                    TeamActions.sendEmailGuestInvitesToChannelsGracefully(
                        teamId,
                        channels.map((x) => x.id),
                        emails,
                        message,
