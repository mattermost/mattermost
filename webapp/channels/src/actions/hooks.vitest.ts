...
describe('runMessageWillBePostedHooks', () => {
    ...
    test('should pass the post through every hook', async () => {
        const hook1 = vi.fn((post) => ({post}));
        const hook2 = vi.fn((post) => ({post}));
        const hook3 = vi.fn((post) => ({post}));
    ...
    test('should return an error when a hook rejects the post', async () => {
        const hook1 = vi.fn((post) => ({post}));
        const hook2 = vi.fn(() => ({error: {message: 'an error occurred'}}));
        const hook3 = vi.fn((post) => ({post}));
    ...
    test('should pass the result of each hook to the next', async () => {
        const hook1 = vi.fn((post) => ({post: {...post, message: post.message + 'a'}}));
        const hook2 = vi.fn((post) => ({post: {...post, message: post.message + 'b'}}));
        const hook3 = vi.fn((post) => ({post: {...post, message: post.message + 'c'}}));
...
describe('runSlashCommandWillBePostedHooks', () => {
    ...
    test('should pass the command through every hook', async () => {
        const hook1 = vi.fn((message, args) => ({message, args}));
        const hook2 = vi.fn((message, args) => ({message, args}));
        const hook3 = vi.fn((message, args) => ({message, args}));
    ...
    test('should pass the result of each hook to the next', async () => {
        const hook1 = vi.fn((message, args) => ({message: message + 'a', args}));
        const hook2 = vi.fn((message, args) => ({message: message + 'b', args}));
        const hook3 = vi.fn((message, args) => ({message: message + 'c', args}));
...
describe('runDesktopNotificationHooks', () => {
    ...
    test('should pass the args through every hook', async () => {
        const hook1 = vi.fn((post, msgProps, channel, teamId, args) => ({args}));
        const hook2 = vi.fn((post, msgProps, channel, teamId, args) => ({args}));
        const hook3 = vi.fn((post, msgProps, channel, teamId, args) => ({args}));
    ...
    test('should return an error when a hook returns an error', async () => {
        const hook1 = vi.fn((post, msgProps, channel, teamId, args) => ({args}));
        const hook2 = vi.fn(() => ({error: 'an error occurred'}));
        const hook3 = vi.fn((post, msgProps, channel, teamId, args) => ({args}));
    ...
    test('should pass the result of each hook to the next', async () => {
        const hook1 = vi.fn((post, msgProps, channel, teamId, args) => ({args: {...args, title: args.title + 'a'}}));
        const hook2 = vi.fn((post, msgProps, channel, teamId, args) => ({
            args: {
                ...args,
                title: args.title + 'b',
                notify: false,
        ...
        const hook3 = vi.fn((post, msgProps, channel, teamId, args) => ({
            args: {
                ...args,
                title: args.title + 'c',
                notify: true,
    ...
    test('should wait for async hooks', async () => {
        ...
        const hook = vi.fn((post, msgProps, channel, teamId, args) => {
            return new Promise((resolve) => {
                setTimeout(() => {
