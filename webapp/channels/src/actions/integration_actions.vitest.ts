...
vi.mock('mattermost-redux/actions/integrations', () => ({
    ...
    getIncomingHooks: vi.fn(() => {
        return {type: 'MOCK_GET_INCOMING_HOOKS', data: []};
    }),
    getOutgoingHooks: vi.fn(() => {
    ...
    getOutgoingOAuthConnections: vi.fn(() => {
        return {type: 'MOCK_GET_OUTGOING_OAUTH_CONNECTIONS', data: []};
    }),
    getAppsOAuthAppIDs: vi.fn(() => {
        return {type: 'MOCK_GET_APPS_OAUTH_APP_IDS'};
    }),
    isIncomingWebhooksWithCount: vi.fn(() => false),
}));

vi.mock('mattermost-redux/selectors/entities/apps', () => ({
    appsEnabled: vi.fn(() => true),
}));

vi.mock('mattermost-redux/selectors/entities/integrations', () => ({
    getDialogArguments: vi.fn(() => undefined),
}));
...
describe('actions/integration_actions', () => {
    ...
    describe('submitInteractiveDialog', () => {
        test('submitInteractiveDialog with current channel', async () => {
            const {getDialogArguments} = await import('mattermost-redux/selectors/entities/integrations');
            const testState = {
                ...initialState,
                entities: {
            ...
            vi.mocked(getDialogArguments).mockReturnValue({channel_id: 'dialog_channel_id'});
            const submission = {
        ...
        test('submitInteractiveDialog with currentChannel context', async () => {
            const {getDialogArguments} = await import('mattermost-redux/selectors/entities/integrations');
            vi.mocked(getDialogArguments).mockReturnValue(undefined);
            const testStore = mockStore(initialState);
