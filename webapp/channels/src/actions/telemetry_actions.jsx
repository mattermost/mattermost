...
export function measure(name1, name2) {
    ...
    const requestCount =
        getRequestCountAtMark(name2) - getRequestCountAtMark(name1);
...
export function measurePageLoadTelemetry() {
    ...
    setTimeout(() => {
        ...
        performance
            ...
            .forEach((resourceTimingEntry) => {
                if (
                    resourceTimingEntry.initiatorType === "xmlhttprequest" ||
                    resourceTimingEntry.initiatorType === "fetch"
                ) {
                    numOfRequest++;
                    maxAPIResourceSize = Math.max(
                        maxAPIResourceSize,
                        resourceTimingEntry.encodedBodySize,
                    );

                    if (
                        resourceTimingEntry.responseEnd -
                            resourceTimingEntry.startTime >
                        longestAPIResourceDuration
                    ) {
                        longestAPIResourceDuration =
                            resourceTimingEntry.responseEnd -
                            resourceTimingEntry.startTime;
                        longestAPIResource =
                            resourceTimingEntry.name?.split("/api/")?.[1] ?? "";
                    }
                }
            });
...
export function trackPluginInitialization(plugins) {
    ...
    for (const plugin of plugins) {
        const filename = plugin.webapp.bundle_path.substring(
            plugin.webapp.bundle_path.lastIndexOf("/"),
        );
