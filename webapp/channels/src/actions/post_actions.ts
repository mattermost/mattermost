...
export function handleNewPost(
    post: Post,
    msg?: { data?: NewPostMessageProps & GroupChannel },
) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        dispatch(
            completePostReceive(
                post,
                websocketMessageProps as NewPostMessageProps,
                myChannelMemberDoesntExist,
            ),
        );
...
export function createPost(post: Post, files: FileInfo[]) {
    return async (dispatch: DispatchFunc) => {
        ...
        if (emojis) {
            const trimmedEmojis = emojis.map((emoji) =>
...
function addPostToSearchResults(postId: string) {
    return (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (index === -1) {
            ...
            const posts = getPostsForIds(state, results).reduce(
                (acc, post) => {
                    acc[post.id] = post;
                    return acc;
                },
...
function removePostFromSearchResults(
    postId: string,
    state: GlobalState,
...
export function markPostAsUnread(post: Post, location: string) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (
            ...
            (location === "RHS_ROOT" || location === "RHS_COMMENT")
        ) {
            const threadId = post.root_id || post.id;
            ThreadActions.handleFollowChanged(
                dispatch,
                threadId,
                currentTeamId,
                true,
            ...
            await dispatch(
                ThreadActions.markThreadAsUnread(
                    userId,
                    currentTeamId,
                    threadId,
                    post.id,
...
export function markMostRecentPostInChannelAsUnread(channelId: string) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        let state = getState();
        let postId = PostSelectors.getMostRecentPostIdInChannel(
            state,
            channelId,
        );
        if (!postId) {
            ...
            state = getState();
            postId = PostSelectors.getMostRecentPostIdInChannel(
                state,
                channelId,
            );
        }
...
export function toggleInlineImageVisibility(postId: string, imageKey: string) {
    return (dispatch: DispatchFunc, getState: GetStateFunc) => {
        const state = getState();
        const currentUserId = getCurrentUserId(state);
        const visible = isInlineImageVisible(
        ...
        dispatch(
            StorageActions.setGlobalItem(
                StoragePrefixes.INLINE_IMAGE_VISIBLE +
                    currentUserId +
                    "_" +
                    postId +
                    "_" +
