...
class ChannelHeaderPlug extends React.PureComponent<
    ChannelHeaderPlugProps,
    ChannelHeaderPlugState
> {
    ...
    fireAction = (
        action: (channel: Channel, channelMember?: ChannelMembership) => void,
    ) => {
    ...
    createAppBindingButton = (binding: AppBinding) => {
        return (
            <HeaderIconWrapper
                ...
                iconComponent={
                    <img src={binding.icon} width="24" height="24" />
                }
    ...
    createDropdown = (plugs: PluginComponent[], appBindings: AppBinding[]) => {
        const componentItems = plugs
            .filter((plug) => plug.action)
            .map((plug) => {
                return (
                    <li key={"channelHeaderPlug" + plug.id}>
                        <a
                            ...
                            onClick={() =>
                                this.fireActionAndClose(plug.action!)
                            }
                        >
                            <span className="d-flex align-items-center overflow--ellipsis">
                                {plug.icon}
                            </span>
                            ...
                            <a
                                ...
                                onClick={() =>
                                    this.fireActionAndClose(() =>
                                        this.onBindingClick(binding),
                                    )
                                }
                            >
                                <span className="d-flex align-items-center overflow--ellipsis icon">
                        ...
                        >
                            <React.Fragment>
                                <PluginChannelHeaderIcon
                                    ...
                                    aria-hidden="true"
                                />
                                <span id="pluginCount" className="icon__text">
                                    {items.length}
                                </span>
                            </React.Fragment>
                        </OverlayTrigger>
    ...
    render() {
        ...
        const appBindings = this.props.appsEnabled
            ? this.props.appBindings || []
            : [];
        if (
            this.props.shouldShowAppBar ||
            (components.length === 0 && appBindings.length === 0)
        ) {
            return null;
        } else if (components.length + appBindings.length <= 15) {
            let componentButtons = components
                .filter((plug) => plug.icon && plug.action)
                .map(this.createComponentButton);
            if (this.props.appsEnabled) {
                componentButtons = componentButtons.concat(
                    appBindings.map(this.createAppBindingButton),
                );
            }
