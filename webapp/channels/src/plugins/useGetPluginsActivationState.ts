...
export const useGetPluginsActivationState = () => {
    ...
    if (pluginProducts) {
        playbooksProductEnabled = pluginProducts.some(
            (product) => product.pluginId === suitePluginIds.playbooks,
        );
    }
    ...
    return {
        boardsPlugin: Boolean(boardsPlugin),
        playbooksPlugin: Boolean(playbooksPlugin),
        playbooksProductEnabled,
    };
};
