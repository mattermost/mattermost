...
export function getUserIdsInChannels(
    state: GlobalState,
): RelationOneToManyUnique<Channel, UserProfile> {
...
export function getUserIdsInGroups(
    state: GlobalState,
): RelationOneToMany<Group, UserProfile> {
    return state.entities.users.profilesInGroup;
}

export function getUserIdsNotInGroups(
    state: GlobalState,
): RelationOneToMany<Group, UserProfile> {
...
export const getUsersByUsername: (
    a: GlobalState,
) => Record<string, UserProfile> = createSelector(
    "getUsersByUsername",
    getUsers,
    (users) => {
...
export const getUsersByEmail: (a: GlobalState) => Record<string, UserProfile> =
    createSelector("getUsersByEmail", getUsers, (users) => {
...
export const isCurrentUserSystemAdmin: (state: GlobalState) => boolean =
    createSelector("isCurrentUserSystemAdmin", getCurrentUser, (user) => {
        const roles = user?.roles || "";
        return isSystemAdmin(roles);
    });

export const isCurrentUserGuestUser: (state: GlobalState) => boolean =
    createSelector("isCurrentUserGuestUser", getCurrentUser, (user) => {
        const roles = user?.roles || "";
        return isGuest(roles);
    });

export const currentUserHasAnAdminRole: (state: GlobalState) => boolean =
    createSelector("currentUserHasAnAdminRole", getCurrentUser, (user) => {
        const roles = user.roles || "";
...
export const getCurrentUserRoles: (a: GlobalState) => UserProfile["roles"] =
    createSelector(
        ...
        getMyCurrentChannelMembership,
        (state) =>
            state.entities.teams.myMembers[state.entities.teams.currentTeamId],
        getCurrentUser,
...
export const getProfileSetInCurrentChannel: (
    state: GlobalState,
) => Set<UserProfile["id"]> = createSelector(
...
export const getProfileSetInCurrentTeam: (
    state: GlobalState,
) => Array<UserProfile["id"]> = createSelector(
...
function sortAndInjectProfiles(
    ...
    profileSet?: "all" | Array<UserProfile["id"]> | Set<UserProfile["id"]>,
): UserProfile[] {
...
export function getIsManualStatusForUserId(
    state: GlobalState,
...
export const getActiveProfilesInCurrentChannel: (
    state: GlobalState,
) => UserProfile[] = createSelector(
    "getProfilesInCurrentChannel",
    getUsers,
    getProfileSetInCurrentChannel,
    (profiles, currentChannelProfileSet) => {
        return sortAndInjectProfiles(profiles, currentChannelProfileSet).filter(
            (user) => user.delete_at === 0,
        );
    },
);

export const getActiveProfilesInCurrentChannelWithoutSorting: (
    state: GlobalState,
) => UserProfile[] = createSelector(
    "getProfilesInCurrentChannel",
    getUsers,
    getProfileSetInCurrentChannel,
    (profiles, currentChannelProfileSet) => {
        return injectProfiles(profiles, currentChannelProfileSet).filter(
            (user) => user.delete_at === 0,
        );
    },
);
...
export const getProfilesInCurrentTeam: (state: GlobalState) => UserProfile[] =
    createSelector(
...
export const getProfilesInTeam: (
    ...
    (profiles, usersInTeams, memberships, teamId, filters) => {
        return sortAndInjectProfiles(
            filterProfiles(profiles, filters, memberships),
            usersInTeams[teamId] || new Set(),
        );
    },
);

export const getProfilesNotInTeam: (
    state: GlobalState,
    teamId: Team["id"],
    filters?: Filters,
) => UserProfile[] = createSelector(
    ...
    (profiles, usersNotInTeams, teamId, filters) => {
        return sortAndInjectProfiles(
            filterProfiles(profiles, filters),
            usersNotInTeams[teamId] || new Set(),
        );
    },
);
...
export function makeSearchProfilesStartingWithTerm(): (
    ...
    return createSelector(
        ...
        (state: GlobalState, term: string, skipCurrent?: boolean) =>
            skipCurrent || false,
        (
            stateGlobalState,
            term: string,
        ...
        (users, currentUserId, term, skipCurrent, filters) => {
            const profiles = filterProfilesMatchingWithTerm(
                Object.values(users),
                term,
            );
            return filterFromProfiles(
                currentUserId,
                profiles,
                skipCurrent,
                filters,
...
export function makeSearchProfilesInChannel() {
    ...
    return (
        ...
        const profiles = filterProfilesStartingWithTerm(
            doGetProfilesInChannel(state, channelId, filters),
            term,
        );
...
export function searchActiveProfilesInCurrentChannel(
    state: GlobalState,
    term: string,
    skipCurrent = false,
): UserProfile[] {
    return searchProfilesInCurrentChannel(state, term, skipCurrent).filter(
        (user) => user.delete_at === 0,
    );
}
...
export function searchProfilesInTeam(
    ...
    const profiles = filterProfilesMatchingWithTerm(
        getProfilesInTeam(state, teamId, filters),
        term,
    );
    if (skipCurrent) {
        removeCurrentUserFromList(profiles, getCurrentUserId(state));
...
export function searchProfilesWithoutTeam(
    ...
    const filteredProfiles = filterProfilesStartingWithTerm(
...
export const shouldShowTermsOfService: (state: GlobalState) => boolean =
    createSelector(
        ...
        (config, user, license) => {
            ...
            const reacceptanceTime =
                parseInt(config.CustomTermsOfServiceReAcceptancePeriod!, 10) *
                1000 *
                60 *
                60 *
                24;
...
export const getUsersInVisibleDMs: (state: GlobalState) => UserProfile[] =
    createSelector(
...
export function makeGetProfilesInChannel(): (
    ...
    return createSelector(
        ...
        (users, userIds, membersInChannel, channelId, filters = {}) => {
            ...
            return sortAndInjectProfiles(
                filterProfiles(users, filters, membersInChannel),
                userIdsInChannel,
            );
        },
    );
...
export function makeGetProfilesByIdsAndUsernames(): (
    ...
    return createSelector(
        ...
        (
            allProfilesById: Record<string, UserProfile>,
            allProfilesByUsername: Record<string, UserProfile>,
            allUserIds: string[],
            allUsernames: string[],
        ) => {
...
export function makeDisplayNameGetter(): (
    state: GlobalState,
    useFallbackUsername: boolean,
) => (user: UserProfile | null | undefined) => string {
    return createSelector(
        "makeDisplayNameGetter",
        getTeammateNameDisplaySetting,
        (_, useFallbackUsername = true) => useFallbackUsername,
        (teammateNameDisplaySetting, useFallbackUsername) => {
            return (user: UserProfile | null | undefined) =>
                displayUsername(
                    user,
                    teammateNameDisplaySetting!,
                    useFallbackUsername,
...
export const getProfilesInGroup: (
    state: GlobalState,
    groupId: Group["id"],
    filters?: Filters,
) => UserProfile[] = createSelector(
    ...
    (profiles, usersInGroups, groupId, filters) => {
        return sortAndInjectProfiles(
            filterProfiles(profiles, filters),
            usersInGroups[groupId] || new Set(),
        );
    },
);

export const getProfilesInGroupWithoutSorting: (
    state: GlobalState,
    groupId: Group["id"],
    filters?: Filters,
) => UserProfile[] = createSelector(
...
export const getProfilesNotInCurrentGroup: (
    state: GlobalState,
    groupId: Group["id"],
    filters?: Filters,
) => UserProfile[] = createSelector(
    ...
    (profiles, usersNotInGroups, groupId, filters) => {
        return sortAndInjectProfiles(
            filterProfiles(profiles, filters),
            usersNotInGroups[groupId] || new Set(),
        );
    },
);

export function searchProfilesInGroup(
    state: GlobalState,
    groupId: Group["id"],
    term: string,
    skipCurrent = false,
    ...
    const profiles = filterProfilesStartingWithTerm(
        getProfilesInGroup(state, groupId, filters),
        term,
    );
    if (skipCurrent) {
        removeCurrentUserFromList(profiles, getCurrentUserId(state));
...
export function searchProfilesInGroupWithoutSorting(
    state: GlobalState,
    groupId: Group["id"],
    term: string,
    skipCurrent = false,
    ...
    const profiles = filterProfilesStartingWithTerm(
        getProfilesInGroupWithoutSorting(state, groupId, filters),
        term,
    );
    if (skipCurrent) {
        removeCurrentUserFromList(profiles, getCurrentUserId(state));
...
export function getUserLastActivities(
    state: GlobalState,
): RelationOneToOne<UserProfile, number> {
    return state.entities.users.lastActivity;
}

export function getLastActivityForUserId(
    state: GlobalState,
    userId: UserProfile["id"],
): number {
...
export function checkIsFirstAdmin(
    ...
    for (const user of Object.values(users)) {
        if (
            ...
            user.create_at < currentUser.create_at
        ) {
...
export const displayLastActiveLabel: (
    state: GlobalState,
    userId: string,
) => boolean = createSelector(
    "displayLastActiveLabel",
    (state: GlobalState, userId: string) => getStatusForUserId(state, userId),
    (state: GlobalState, userId: string) =>
        getLastActivityForUserId(state, userId),
    (state: GlobalState, userId: string) => getUser(state, userId),
    getConfig,
    (userStatus, timestamp, user, config) => {
        ...
        if (
            !userStatus ||
            userStatus === General.ONLINE ||
            (timestamp &&
                currentTime.valueOf() - new Date(timestamp).valueOf() <=
                    oneMin) ||
...
export const getLastActiveTimestampUnits: (
    state: GlobalState,
    userId: string,
    ...
    (state: GlobalState, userId: string) =>
        getLastActivityForUserId(state, userId),
    (timestamp) => {
        const timestampUnits = ["now", "minute", "hour"];
        const currentTime = new Date();
