...
describe("Selectors.Roles", () => {
    ...
    myTeamMembers[team1.id] = {
        roles: "test_team1_role1 test_team1_role2",
    } as TeamMembership;
    myTeamMembers[team2.id] = {
        roles: "test_team2_role1 test_team2_role2",
    } as TeamMembership;
    ...
    channelsRoles[channel1.id] = new Set([
        "test_channel_a_role1",
        "test_channel_a_role2",
    ]);
    channelsRoles[channel2.id] = new Set([
        "test_channel_a_role1",
        "test_channel_a_role2",
    ]);
    channelsRoles[channel3.id] = new Set([
        "test_channel_a_role1",
        "test_channel_a_role2",
    ]);
    channelsRoles[channel4.id] = new Set([
        "test_channel_a_role1",
        "test_channel_a_role2",
    ]);
    channelsRoles[channel5.id] = new Set([
        "test_channel_a_role1",
        "test_channel_a_role2",
    ]);
    channelsRoles[channel7.id] = new Set([
        "test_channel_b_role1",
        "test_channel_b_role2",
    ]);
    channelsRoles[channel8.id] = new Set([
        "test_channel_b_role1",
        "test_channel_b_role2",
    ]);
    channelsRoles[channel9.id] = new Set([
        "test_channel_b_role1",
        "test_channel_b_role2",
    ]);
    channelsRoles[channel10.id] = new Set([
        "test_channel_c_role1",
        "test_channel_c_role2",
    ]);
    channelsRoles[channel11.id] = new Set([
        "test_channel_c_role1",
        "test_channel_c_role2",
    ]);
    const roles = {
    ...
    it("should return my roles by scope on getMySystemRoles/getMyTeamRoles/getMyChannelRoles/getMyGroupRoles", () => {
        ...
        teamsRoles[team1.id] = new Set([
            "test_team1_role1",
            "test_team1_role2",
        ]);
        teamsRoles[team2.id] = new Set([
            "test_team2_role1",
            "test_team2_role2",
        ]);
    ...
    it("should return my system permission on getMySystemPermissions", () => {
        expect(getMySystemPermissions(testState)).toEqual(
            new Set([
                "user_role2",
                Permissions.EDIT_CUSTOM_GROUP,
                Permissions.CREATE_CUSTOM_GROUP,
    ...
    it("should return if i have a system permission on haveISystemPermission", () => {
        expect(
            Selectors.haveISystemPermission(testState, {
                permission: "user_role2",
            }),
        ).toEqual(true);
        expect(
            Selectors.haveISystemPermission(testState, {
    ...
    it("should return if i have a team permission on haveITeamPermission", () => {
        expect(
            Selectors.haveITeamPermission(testState, team1.id, "user_role2"),
        ).toEqual(true);
        expect(
            Selectors.haveITeamPermission(testState, team1.id, "team1_role1"),
        ).toEqual(true);
        expect(
            Selectors.haveITeamPermission(testState, team1.id, "team2_role2"),
        ).toEqual(false);
        expect(
            Selectors.haveITeamPermission(
                testState,
                team1.id,
                "invalid_permission",
            ),
    ...
    it("should return if i have a team permission on haveICurrentTeamPermission", () => {
        expect(
            Selectors.haveICurrentTeamPermission(testState, "user_role2"),
        ).toEqual(true);
        expect(
            Selectors.haveICurrentTeamPermission(testState, "team1_role1"),
        ).toEqual(true);
        expect(
            Selectors.haveICurrentTeamPermission(testState, "team2_role2"),
        ).toEqual(false);
        expect(
            Selectors.haveICurrentTeamPermission(
                testState,
                "invalid_permission",
            ),
        ).toEqual(false);
    ...
    it("should return if i have a channel permission on haveIChannelPermission", () => {
        expect(
            Selectors.haveIChannelPermission(
                testState,
                team1.id,
                channel1.id,
                "user_role2",
    ...
    it("should return if i have a channel permission on haveICurrentChannelPermission", () => {
        expect(
            Selectors.haveICurrentChannelPermission(testState, "user_role2"),
        ).toEqual(true);
        expect(
            Selectors.haveICurrentChannelPermission(testState, "team1_role1"),
        ).toEqual(true);
        expect(
            Selectors.haveICurrentChannelPermission(testState, "team2_role2"),
        ).toEqual(false);
        expect(
            Selectors.haveICurrentChannelPermission(
                testState,
                "channel_a_role1",
            ),
        ).toEqual(true);
    ...
    it("should return group memberships on getGroupMemberships", () => {
        expect(Selectors.getGroupMemberships(testState)).toEqual({
            [group1.id]: { user_id: user.id, roles: "custom_group_user" },
        });
    });

    it("should return if i have a group permission on haveIGroupPermission", () => {
        expect(
            Selectors.haveIGroupPermission(
                testState,
                group1.id,
                Permissions.EDIT_CUSTOM_GROUP,
            ),
