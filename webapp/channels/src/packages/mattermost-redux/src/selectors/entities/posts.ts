...
export function getPostRepliesCount(
    state: GlobalState,
    postId: Post["id"],
): number {
...
export function getPostsInThread(
    state: GlobalState,
): RelationOneToMany<Post, Post> {
    return state.entities.posts.postsInThread;
}

export function getReactionsForPosts(state: GlobalState): RelationOneToOne<
    Post,
    {
        [x: string]: Reaction;
    }
...
export function getHasReactions(
    state: GlobalState,
    postId: Post["id"],
): boolean {
...
export function getOpenGraphMetadata(
    state: GlobalState,
): RelationOneToOne<Post, Record<string, OpenGraphMetadata>> {
    return state.entities.posts.openGraph;
}

export function getOpenGraphMetadataForUrl(
    state: GlobalState,
    postId: string,
    url: string,
) {
...
export function getPostIdsInCurrentChannel(
    state: GlobalState,
...
export const getPostsInCurrentChannel: (
    state: GlobalState,
...
export function makeGetPostIdsForThread(): (
    ...
    return createIdsSelector(
        ...
        (state: GlobalState, rootId: Post["id"]) =>
            getPostsForThread(state, rootId),
        (posts) => {
...
export function makeGetPostsChunkAroundPost(): (
    ...
    return createIdsSelector(
        ...
        (state: GlobalState, postId: string, channelId: string) =>
...
export function makeGetPostIdsAroundPost(): (
    ...
    return createIdsSelector(
        ...
        (state: GlobalState, postId: string, channelId: string) =>
            getPostsChunkAroundPost(state, postId, channelId),
        (state: GlobalState, postId) => postId,
        (state: GlobalState, postId, channelId, options) =>
            options && options.postsBeforeCount,
        (state: GlobalState, postId, channelId, options) =>
            options && options.postsAfterCount,
        (
            postsChunk,
            postId,
            postsBeforeCount = Posts.POST_CHUNK_SIZE / 2,
            ...
            const minPostIndex =
                postsAfterCount === -1
                    ? 0
                    : Math.max(index - postsAfterCount, 0);
            const maxPostIndex =
                postsBeforeCount === -1
                    ? postIds.length
                    : Math.min(index + postsBeforeCount + 1, postIds.length); // Needs the extra 1 to include the focused post
...
function formatPostInChannel(
    ...
    if (post.root_id) {
        ...
        if (
            index - 1 < 0 ||
            allPosts[postIds[index - 1]].root_id !== post.root_id
        ) {
    ...
    const prevPostFromWebhook = Boolean(
        previousPost && previousPost.props && previousPost.props.from_webhook,
    );
...
export function makeGetPostsInChannel(): (
    ...
    return createSelector(
        ...
        getAllPosts,
        getPostsInThread,
        (state: GlobalState, channelId: Channel["id"]) =>
            getPostIdsInChannel(state, channelId),
        getCurrentUser,
        getMyPreferences,
        (state: GlobalState, channelId: Channel["id"], numPosts: number) =>
            numPosts || Posts.POST_CHUNK_SIZE,
        (
            allPosts,
            postsInThread,
            allPostIds,
            ...
            const showJoinLeave = joinLeavePref
                ? joinLeavePref.value !== "false"
                : true;

            const postIds =
                numPosts === -1 ? allPostIds : allPostIds.slice(0, numPosts);
...
export function makeGetPostsForThread(): (
    ...
    return createIdsSelector(
        ...
        (state: GlobalState, rootId: string) =>
            state.entities.posts.postsInThread[rootId],
        (state: GlobalState, rootId: string) =>
            state.entities.posts.posts[rootId],
        ...
        (posts, currentUser, postsForThread, rootPost, showJoinLeave) => {
            ...
            postsForThread?.forEach((id) => {
                ...
                const skip = shouldFilterJoinLeavePost(
                    post,
                    showJoinLeave,
                    currentUser ? currentUser.username : "",
                );
...
export function makeGetProfilesForThread(): (
    state: GlobalState,
    rootId: string,
) => UserProfile[] {
    const getPostsForThread = makeGetPostsForThread();
...
export function makeGetCommentCountForPost(): (
    ...
    return createSelector(
        ...
        getAllPosts,
        (state: GlobalState, post: Post) =>
            state.entities.posts.postsInThread[
                post ? post.root_id || post.id : ""
            ] || null,
...
export function makeGetMessageInHistoryItem(
    type: "post" | "comment",
...
export const getMostRecentPostIdInChannel: (
    state: GlobalState,
    channelId: Channel["id"],
) => Post["id"] | undefined | null = createSelector(
    ...
    (posts, postIdsInChannel, preferences) => {
        ...
        const allowSystemMessages = preferences[key]
        ...
        if (!allowSystemMessages) {
            ...
            for (let i = 0; i < postIdsInChannel.length; i++) {
                ...
                if (
                    !p.type ||
                    !p.type.startsWith(Posts.SYSTEM_MESSAGE_PREFIX)
                ) {
                    postId = p.id;
                    break;
                }
...
export const getLatestReplyablePostId: (state: GlobalState) => Post["id"] =
    createSelector(
...
export function getOldestPostTimeInChannel(
    state: GlobalState,
    channelId: Channel["id"],
): number {
    const postsForChannel = state.entities.posts.postsInChannel[channelId];
    ...
    const oldestPostTime = postsForChannel.reduce((acc: number, postBlock) => {
        if (postBlock.order.length > 0) {
            const oldestPostIdInBlock =
                postBlock.order[postBlock.order.length - 1];
            const blockOldestPostTime =
                allPosts[oldestPostIdInBlock]?.create_at;
            if (
                typeof blockOldestPostTime === "number" &&
                blockOldestPostTime < acc
            ) {
...
export function getPostsChunkInChannelAroundTime(
    ...
    const blockAroundTimestamp = postsForChannel.find((block) => {
        ...
        if (recentPostInBlock && oldestPostInBlock) {
            return (
                recentPostInBlock.create_at >= timeStamp &&
                oldestPostInBlock.create_at <= timeStamp
            );
        }
        return false;
    });
...
export const isPostsChunkIncludingUnreadsPosts = (
    state: GlobalState,
    chunk: PostOrderBlock,
    timeStamp: number,
): boolean => {
    const postsEntity = state.entities.posts;
    const posts = postsEntity.posts;
...
export const isPostIdSending = (
    state: GlobalState,
    postId: Post["id"],
): boolean => {
    return state.entities.posts.pendingPostIds.some(
        (sendingPostId) => sendingPostId === postId,
    );
};
...
export function getLimitedViews(
    state: GlobalState,
): GlobalState["entities"]["posts"]["limitedViews"] {
...
export function getPostAcknowledgements(
    state: GlobalState,
    postId: Post["id"],
): Record<UserProfile["id"], PostAcknowledgement["acknowledged_at"]> {
...
export const isPersistentNotificationsEnabled = createSelector(
    ...
    (user, forAll, forGuests) =>
        isGuest(user.roles) ? forAll && forGuests : forAll,
);

export function makeGetPostAcknowledgementsWithProfiles(): (
    state: GlobalState,
    postId: Post["id"],
) => Array<{
    user: UserProfile;
    ...
    return createSelector(
        ...
        (users, acknowledgements) => {
            ...
            return Object.keys(acknowledgements)
                .flatMap((userId) => {
                    if (!users[userId]) {
                        return [];
                    }
...
export function getTeamIdFromPost(
    state: GlobalState,
    post: Post,
): Team["id"] | undefined {
