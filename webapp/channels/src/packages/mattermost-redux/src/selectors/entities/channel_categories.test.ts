...
describe("getCategoryInTeamByType", () => {
    const favoritesCategory1 = {
        id: "favoritesCategory1",
        team_id: "team1",
        type: CategoryTypes.FAVORITES,
    };
    const channelsCategory1 = {
        id: "channelsCategory1",
        team_id: "team1",
        type: CategoryTypes.CHANNELS,
    };
    const directMessagesCategory1 = {
        id: "directMessagesCategory1",
        team_id: "team1",
        type: CategoryTypes.DIRECT_MESSAGES,
    };
    const channelsCategory2 = {
        id: "channelsCategory2",
        team_id: "team2",
        type: CategoryTypes.CHANNELS,
    };
    ...
    test("should return categories from each team", () => {
        expect(
            Selectors.getCategoryInTeamByType(
                state,
                "team1",
                CategoryTypes.FAVORITES,
            ),
        ).toBe(favoritesCategory1);
        expect(
            Selectors.getCategoryInTeamByType(
                state,
                "team1",
...
describe("getCategoryInTeamWithChannel", () => {
    ...
    test("should return the category containing a given channel", () => {
        expect(
            Selectors.getCategoryInTeamWithChannel(state, "team1", "channel1"),
        ).toBe(category1);
        expect(
            Selectors.getCategoryInTeamWithChannel(state, "team1", "channel2"),
        ).toBe(category1);
    });

    test("should return the category on the correct team for a cross-team channel", () => {
        expect(
            Selectors.getCategoryInTeamWithChannel(
                state,
                "team1",
                "dmChannel1",
            ),
...
describe("makeGetCategoriesForTeam", () => {
    ...
    test("should memoize properly", () => {
        ...
        expect(result).not.toBe(
            Selectors.makeGetCategoriesForTeam()(state, "team1"),
        );
...
describe("makeFilterAutoclosedDMs", () => {
            ...
            entities: {
                channels: {
                    ...
                    myMembers: {
                        gmChannel1: {
                            msg_count: 1,
                            notify_props: { mark_unread: MarkUnread.ALL },
                        },
                        gmChannel2: {
                            msg_count: 0,
                            notify_props: { mark_unread: MarkUnread.ALL },
    ...
    test("Should always show the current channel", () => {
        ...
        const dmChannel1 = ch({
            ...
            name: `${currentUser.id}__${jeffWinger.id}`,
        });
        const gmChannel1 = ch({ id: "gmChannel1", type: General.GM_CHANNEL });
    ...
    describe("Should always show the exact number of channels specified by the user", () => {
        ...
        const dmChannel1 = ch({
            id: "dmChannel1",
            type: General.DM_CHANNEL,
            name: `${currentUser.id}__${tigerKing.id}`,
        });
        const gmChannel1 = ch({
            id: "gmChannel1",
        ...
        const dmChannel2 = ch({
            id: "dmChannel2",
            type: General.DM_CHANNEL,
            name: `${currentUser.id}__${bojackHorseman.id}`,
        });
        const dmChannel3 = ch({
            id: "dmChannel3",
            type: General.DM_CHANNEL,
    ...
    test("should consider approximate view time and open time preferences for most recently viewed channel", () => {
        ...
        expect(
            filterAutoclosedDMs(
                state,
                [dmChannel1, dmChannel2, dmChannel3],
                CategoryTypes.DIRECT_MESSAGES,
            ),
        ).toEqual([dmChannel1, dmChannel2]);
        ...
        state = mergeObjects(state, {
            entities: {
                preferences: {
                    myPreferences: {
                        [getPreferenceKey(
                            Preferences.CATEGORY_CHANNEL_OPEN_TIME,
                            dmChannel3.id,
                        )]: { value: "3000" },
                    },
                },
...
describe("makeFilterManuallyClosedDMs", () => {
    ...
    test("should filter DMs based on preferences", () => {
        ...
        const dmChannel1 = ch({
            id: "dmChannel1",
            type: General.DM_CHANNEL,
            name: `${currentUser.id}__${otherUser1.id}`,
        });
        const dmChannel2 = ch({
            id: "dmChannel2",
            type: General.DM_CHANNEL,
            name: `${currentUser.id}__${otherUser2.id}`,
        });
        ...
        expect(
            filterManuallyClosedDMs(state, [dmChannel1, dmChannel2]),
        ).toMatchObject([dmChannel2]);
    });
    ...
    test("should filter GMs based on preferences", () => {
        ...
        expect(
            filterManuallyClosedDMs(state, [gmChannel1, gmChannel2]),
        ).toMatchObject([gmChannel1]);
    });
    ...
    test("should show unread DMs and GMs, regardless of preferences", () => {
        ...
        expect(
            filterManuallyClosedDMs(state, [
                dmChannel1,
                dmChannel2,
                gmChannel1,
                gmChannel2,
            ]),
        ).toEqual([dmChannel1, gmChannel1]);
    });
    ...
    test("should show the current channel, regardless of preferences", () => {
        ...
        expect(
            filterManuallyClosedDMs(state, [dmChannel1, gmChannel1]),
        ).toEqual([]);
...
describe("makeSortChannelsByName", () => {
    ...
    test("should sort channels by display name", () => {
        ...
        expect(sortChannelsByName(baseState, channels)).toEqual([
            channel2,
            channel3,
            channel1,
        ]);
    ...
    test("should sort muted channels last", () => {
        ...
        const state = mergeObjects(baseState, {
            entities: {
                channels: {
                    myMembers: {
                        channel1: {
                            notify_props: { mark_unread: MarkUnread.MENTION },
                        },
                        channel3: {
                            notify_props: { mark_unread: MarkUnread.MENTION },
                        },
        ...
        expect(sortChannelsByName(state, channels)).toEqual([
            channel2,
            channel4,
            channel3,
            channel1,
...
describe("makeSortChannelsByNameWithDMs", () => {
    ...
    const dmChannel2 = ch({
        ...
        type: General.DM_CHANNEL,
        display_name: "",
        name: `${otherUser2.id}__${currentUser.id}`,
    });
    const gmChannel1 = ch({
        id: "gmChannel1",
        type: General.GM_CHANNEL,
        display_name: `${currentUser.username}, ${otherUser1.username}, ${otherUser2.username}`,
        name: "gmChannel1",
    });
    ...
    test("should sort regular channels by display name", () => {
        const sortChannelsByNameWithDMs =
            Selectors.makeSortChannelsByNameWithDMs();

        expect(
            sortChannelsByNameWithDMs(baseState, [
                channel1,
                channel2,
                channel3,
            ]),
    ...
    test("should sort GM channels by the display name of the other users", () => {
        ...
        expect(
            sortChannelsByNameWithDMs(state, [
                channel1,
                channel2,
                channel3,
                gmChannel1,
...
describe("makeSortChannelsByRecency", () => {
    ...
    test("should sort channels by their last_post_at when no posts are loaded", () => {
        ...
        expect(
            sortChannelsByRecency(state, [channel1, channel2, channel3]),
        ).toMatchObject([channel3, channel2, channel1]);
        expect(
            sortChannelsByRecency(state, [channel3, channel2, channel1]),
        ).toMatchObject([channel3, channel2, channel1]);
    });
    ...
    test("should sort channels by their last_post_at when no posts are loaded and CRT in enabled", () => {
        ...
        expect(
            sortChannelsByRecency(state, [channel3, channel2, channel1]),
        ).toMatchObject([channel1, channel3, channel2]);
        expect(
            sortChannelsByRecency(state, [channel1, channel3, channel2]),
        ).toMatchObject([channel1, channel3, channel2]);
    });
});
...
describe("makeGetChannelIdsForCategory", () => {
    ...
    test("should return sorted and filtered channels for favorites category", () => {
        const getChannelIdsForCategory =
            Selectors.makeGetChannelIdsForCategory();
        ...
        expect(
            getChannelIdsForCategory(baseState, favoritesCategory),
        ).toMatchObject([dmChannel2.id, channel1.id]);
    });
    ...
    test("should return sorted and filtered channels for channels category with manual sorting", () => {
        ...
        expect(
            getChannelIdsForCategory(baseState, publicCategory),
        ).toMatchObject([channel3.id, channel2.id]);
    });
    ...
    test("should return sorted and filtered channels for channels category with alphabetical sorting", () => {
        ...
        expect(
            getChannelIdsForCategory(baseState, publicCategory),
        ).toMatchObject([channel2.id, channel3.id]);
    });
    ...
    test("should return sorted and filtered channels for channels category with alphabetical sorting and a muted channel", () => {
        ...
        expect(getChannelIdsForCategory(state, publicCategory)).toMatchObject([
            channel3.id,
            channel2.id,
        ]);
    });
    ...
    test("should return sorted and filtered channels for direct messages category with alphabetical sorting", () => {
        ...
        expect(
            getChannelIdsForCategory(state, directMessagesCategory),
        ).toMatchObject([gmChannel1.id, dmChannel1.id]);
    });
    ...
    test("should return sorted and filtered channels for direct messages category with recency sorting", () => {
        ...
        const gmChannel2 = {
            ...
            type: General.GM_CHANNEL,
            team_id: "",
            display_name: `${currentUser.username}, ${otherUser1.username}, ${otherUser3.username}`,
            name: "gmChannel2",
            delete_at: 0,
            last_post_at: 2000,
            create_at: 0,
        ...
        expect(
            getChannelIdsForCategory(state, directMessagesCategory),
        ).toMatchObject(["dmChannel1", "gmChannel2", "gmChannel1"]);
    });
    ...
    describe("memoization", () => {
        test("should return the same result when called twice with the same category", () => {
            ...
            const originalResult = getChannelIdsForCategory(
                baseState,
                favoritesCategory,
            );

            expect(getChannelIdsForCategory(baseState, favoritesCategory)).toBe(
                originalResult,
            );
        });
        ...
        test("should return a different result when called with a different sorting method", () => {
            ...
            expect(
                getChannelIdsForCategory(baseState, {
                    ...favoritesCategory,
                    sorting: CategorySorting.Recency,
                }),
            ).not.toBe(originalResult);
        ...
        test("should return a different result for alphabetically sorted DMs when the display name setting causes an order change", () => {
                    ...
                    preferences: {
                        myPreferences: {
                            [getPreferenceKey(
                                Preferences.CATEGORY_DISPLAY_SETTINGS,
                                Preferences.NAME_NAME_FORMAT,
                            )]: { value: Preferences.DISPLAY_PREFER_USERNAME },
                        },
                    },
                },
            });
            ...
            const result = getChannelIdsForCategory(
                state,
                directMessagesCategory,
            );
            expect(result).toEqual([dmChannel1.id, dmChannel2.id]);
...
describe("makeGetChannelsByCategory", () => {
    ...
    describe("memoization", () => {
        test("should return the same object when called with the same state", () => {
            ...
            expect(getChannelsByCategory(baseState, "team1")).toBe(
                getChannelsByCategory(baseState, "team1"),
            );
        });
        ...
        test("should return a new object when user profiles change", () => {
            ...
            expect(result.favoritesCategory).not.toBe(
                previousResult.favoritesCategory,
            );
            expect(result.favoritesCategory).toEqual(
                previousResult.favoritesCategory,
            );
            expect(result.channelsCategory).toBe(
                previousResult.channelsCategory,
            );
            expect(result.directMessagesCategory).toEqual(
        ...
        test("should return a new object when a DM is closed", () => {
            ...
            expect(result.channelsCategory).toEqual(
                previousResult.channelsCategory,
            );
            expect(result.directMessagesCategory).not.toEqual(
                previousResult.directMessagesCategory,
            );
        });
    });
