...
describe("Selectors.Search", () => {
    ...
    it("should return current search for current team", () => {
        expect(Selectors.getCurrentSearchForCurrentTeam(testState)).toEqual(
            team1CurrentSearch,
        );
    });
        ...
        const state = {
            entities: {
                ...
                groups: {
                    ...
                    myGroups: ["test1", "test2", "test3"],
                },
            },
        } as unknown as GlobalState;

        expect(Selectors.getAllUserMentionKeys(state)).toEqual([
            { key: "First", caseSensitive: true },
            { key: "@user" },
            { key: "@Do-you-love-me?" },
            { key: "@I-AM-THE-BEST!" },
        ]);
