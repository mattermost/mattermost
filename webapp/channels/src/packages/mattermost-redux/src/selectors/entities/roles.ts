...
export const getGroupMemberships: (
    state: GlobalState,
) => Record<string, GroupMembership> = createSelector(
    "getGroupMemberships",
    ...
    (myGroupIDs: string[], currentUserID: string) => {
        ...
        myGroupIDs.forEach((groupID) => {
            groupMemberships[groupID] = {
                ...
                roles: General.CUSTOM_GROUP_USER_ROLE,
            };
        });
...
export const getMyGroupRoles: (
    state: GlobalState,
) => Record<string, Set<string>> = createSelector(
    "getMyGroupRoles",
    getGroupMemberships,
    (groupMemberships) => {
        ...
        if (groupMemberships) {
            for (const key in groupMemberships) {
                if (
                    groupMemberships.hasOwnProperty(key) &&
                    groupMemberships[key].roles
                ) {
                    roles[key] = new Set<string>(
                        groupMemberships[key].roles.split(" "),
                    );
                }
            }
        }
 ...
 */
export const getGroupListPermissions: (
    state: GlobalState,
) => Record<string, GroupPermissions> = createSelector(
    ...
    (myGroupRoles, roles, systemPermissions, allGroups) => {
        ...
        groups.forEach((g) => {
            groupPermissionsMap[g.id] = {
                can_delete:
                    permissions.has(Permissions.DELETE_CUSTOM_GROUP) &&
                    g.source.toLowerCase() !== "ldap" &&
                    g.delete_at === 0,
                can_manage_members:
                    permissions.has(Permissions.MANAGE_CUSTOM_GROUP_MEMBERS) &&
                    g.source.toLowerCase() !== "ldap" &&
                    g.delete_at === 0,
                can_restore:
                    permissions.has(Permissions.RESTORE_CUSTOM_GROUP) &&
                    g.source.toLowerCase() !== "ldap" &&
                    g.delete_at !== 0,
            };
        });
    ...
    (teamsMemberships) => {
        ...
        if (teamsMemberships) {
            for (const key in teamsMemberships) {
                if (
                    teamsMemberships.hasOwnProperty(key) &&
                    teamsMemberships[key].roles
                ) {
                    roles[key] = new Set<string>(
                        teamsMemberships[key].roles.split(" "),
                    );
                }
            }
        }
...
export function getMyChannelRoles(
    state: GlobalState,
): Record<string, Set<string>> {
    return state.entities.channels.roles;
}

export const getRolesById: (state: GlobalState) => Record<string, Role> =
    createSelector("getRolesById", getRoles, (rolesByName) => {
...
const getMyPermissionsByGroup = createSelector(
    ...
    (myGroupRoles, allRoles) => {
        ...
        for (const [groupId, roles] of Object.entries(myGroupRoles)) {
            permissionsByGroup[groupId] = getPermissionsForRoles(
                allRoles,
                roles,
            );
        }
...
const getMyPermissionsByChannel = createSelector(
    ...
    (myChannelRoles, allRoles) => {
        ...
        for (const [channelId, roles] of Object.entries(myChannelRoles)) {
            permissionsByChannel[channelId] = getPermissionsForRoles(
                allRoles,
                roles,
            );
        }
...
export function haveITeamPermission(
    state: GlobalState,
    teamId: string,
    permission: string,
) {
...
export const haveIGroupPermission: (
    state: GlobalState,
    groupID: string,
    permission: string,
    ...
    getMySystemPermissions,
    getMyPermissionsByGroup,
    (state: GlobalState, groupID: string) =>
        state.entities.groups.groups[groupID],
    ...
    (systemPermissions, permissionGroups, group, permission) => {
        if (permission === Permissions.RESTORE_CUSTOM_GROUP) {
            if (
                group.source !== "ldap" &&
                group.delete_at !== 0 &&
                (systemPermissions.has(permission) ||
                    (permissionGroups[group.id] &&
                        permissionGroups[group.id].has(permission)))
            ) {
                return true;
            }
            return false;
...
export function haveIChannelPermission(
    state: GlobalState,
    teamId: string,
    channelId: string,
    permission: string,
...
export function haveICurrentTeamPermission(
    state: GlobalState,
    permission: string,
): boolean {
...
export function haveICurrentChannelPermission(
    state: GlobalState,
    permission: string,
): boolean {
