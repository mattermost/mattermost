...
export function getCategoryInTeamByType(
    state: GlobalState,
    teamId: string,
    categoryType: ChannelCategoryType,
) {
    return getCategoryWhere(
        state,
        (category) =>
            category.type === categoryType && category.team_id === teamId,
    );
}

// getCategoryInTeamWithChannel returns the category on a given team containing the given channel ID.
export function getCategoryInTeamWithChannel(
    state: GlobalState,
    teamId: string,
    channelId: string,
) {
    return getCategoryWhere(
        ...
        (category) =>
            ...
            category.channel_ids.includes(channelId),
    );
}
...
export function getCategoryWhere(
    state: GlobalState,
    condition: (category: ChannelCategory) => boolean,
) {
...
export function getCategoryIdsForTeam(
    state: GlobalState,
    teamId: string,
): string[] {
...
export function makeFilterArchivedChannels(): (
    state: GlobalState,
    channels: Channel[],
) => Channel[] {
    return createSelector(
...
export function makeFilterAutoclosedDMs(): (
    ...
    return createSelector(
        ...
        (state: GlobalState, channels: Channel[]) => channels,
        (state: GlobalState, channels: Channel[], categoryType: string) =>
            categoryType,
        getCurrentChannelId,
        ...
        (
            ...
            const filteredChannels = channels.filter((channel) =>
                visibleChannelsSet.has(channel),
            );

            return filteredChannels.length === channels.length
                ? channels
                : filteredChannels;
        },
    );
...
export function makeCompareChannels(
    ...
    return (a: Channel, b: Channel) => {
        ...
        return getDisplayName(a).localeCompare(getDisplayName(b), locale, {
            numeric: true,
        });
    };
}

export function makeSortChannelsByName(): (
    state: GlobalState,
    channels: Channel[],
) => Channel[] {
    return createSelector(
        ...
        (
            ...
            return [...channels].sort(
                makeCompareChannels(getDisplayName, locale, myMembers),
            );
        },
    );
}

export function makeSortChannelsByNameWithDMs(): (
    state: GlobalState,
    channels: Channel[],
) => Channel[] {
    return createSelector(
                ...
                if (channel.type === General.DM_CHANNEL) {
                    ...
                    for (const username of usernames) {
                        ...
                        userDisplayNames.push(
                            displayUsername(user, teammateNameDisplay, false),
                        );
                    }

                    displayName = userDisplayNames
                        .sort((a, b) =>
...
export function makeSortChannelsByRecency(): (
    state: GlobalState,
    channels: Channel[],
) => Channel[] {
    return createSelector(
        ...
        (channels, crtEnabled) => {
            return [...channels].sort((a, b) => {
                const aLastPostAt = Math.max(
                    crtEnabled
                        ? a.last_root_post_at || a.last_post_at
                        : a.last_post_at,
                    a.create_at,
                );
                const bLastPostAt = Math.max(
                    crtEnabled
                        ? b.last_root_post_at || b.last_post_at
                        : b.last_post_at,
                    b.create_at,
                );
...
export function makeSortChannels() {
    ...
    return (
        ...
        if (category.sorting === CategorySorting.Recency) {
            ...
            category.sorting === CategorySorting.Default
        ) {
            if (
                channels.some(
                    (channel) =>
...
export function makeGetChannelIdsForCategory() {
    ...
    const filterAndSortChannelsForCategory =
        makeFilterAndSortChannelsForCategory();
    ...
    return (state: GlobalState, category: ChannelCategory) => {
        ...
        const filteredChannelIds = filterAndSortChannelsForCategory(
            state,
            channels,
            category,
...
export function makeGetChannelsByCategory() {
    ...
    let getChannels: RelationOneToOne<
        ChannelCategory,
        ReturnType<typeof makeGetChannelsForIds>
    >;
    let filterAndSortChannels: RelationOneToOne<
        ChannelCategory,
        ReturnType<typeof makeFilterAndSortChannelsForCategory>
    >;
    ...
    return (state: GlobalState, teamId: string) => {
        ...
        if (categoryIds !== lastCategoryIds) {
            ...
            if (categoryIds) {
                for (const categoryId of categoryIds) {
                    ...
                    filterAndSortChannels[categoryId] =
                        makeFilterAndSortChannelsForCategory();
                }
            }
        }
        ...
        for (const category of categories) {
            const channels = getChannels[category.id](
                state,
                category.channel_ids,
            );
            channelsByCategory[category.id] = filterAndSortChannels[
                category.id
