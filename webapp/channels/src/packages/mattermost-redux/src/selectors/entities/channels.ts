...
export function getChannelsInPolicy() {
    return createSelector(
        ...
        (getAllChannels, policy) => {
            ...
            Object.entries(getAllChannels).forEach(
                (channelEntry: [string, Channel]) => {
...
export const getDirectChannelsSet: (state: GlobalState) => Set<string> =
    createSelector(
...
export function makeGetChannelsForIds(): (
    state: GlobalState,
    ids: string[],
) => Channel[] {
    return createSelector(
...
export const getCurrentChannel: (state: GlobalState) => Channel =
    createSelector(
        "getCurrentChannel",
        getAllChannels,
        getCurrentChannelId,
        ...
        (
            ...
            if (channel) {
                return completeDirectChannelInfo(
                    users,
                    teammateNameDisplay,
                    channel,
                );
...
export const getCurrentChannelNameForSearchShortcut: (
    state: GlobalState,
    ...
    (
        ...
        if (channel?.type === General.DM_CHANNEL) {
            const dmChannelWithInfo = completeDirectChannelInfo(
                users,
...
export const isCurrentChannelMuted: (state: GlobalState) => boolean =
    createSelector(
...
export const isMutedChannel: (
    state: GlobalState,
    channelId: string,
) => boolean = createSelector(
    "isMutedChannel",
    (state: GlobalState, channelId: string) =>
        getMyChannelMembership(state, channelId),
    (membership?: ChannelMembership): boolean => {
        if (!membership) {
            return false;
        }
...
export const isCurrentChannelArchived: (state: GlobalState) => boolean =
    createSelector(
...
export const isCurrentChannelDefault: (state: GlobalState) => boolean =
    createSelector("isCurrentChannelDefault", getCurrentChannel, (channel) =>
        isDefault(channel),
    );
...
export function isChannelReadOnlyById(
    state: GlobalState,
    channelId: string,
): boolean {
...
export function getChannelMessageCounts(
    state: GlobalState,
...
export const countCurrentChannelUnreadMessages: (state: GlobalState) => number =
    createSelector(
        ...
        (
            ...
            return crtEnabled
                ? messageCount.root - membership.msg_count_root
                : messageCount.total - membership.msg_count;
        },
    );
...
export function makeGetChannelUnreadCount(): (
    ...
    return createSelector(
        ...
        (state: GlobalState, channelId: string) =>
            getChannelMessageCount(state, channelId),
        (state: GlobalState, channelId: string) =>
            getMyChannelMembership(state, channelId),
        isCollapsedThreadsEnabled,
        (
...
export function getChannelByTeamIdAndChannelName(
    state: GlobalState,
    teamId: string,
    channelName: string,
): Channel | undefined | null {
    return Object.values(getAllChannels(state)).find(
        (channel) => channel.team_id === teamId && channel.name === channelName,
    );
}

export const getChannelSetInCurrentTeam: (state: GlobalState) => string[] =
    createSelector(
...
export const getChannelSetForAllTeams: (state: GlobalState) => string[] =
    createSelector(
...
function sortAndInjectChannels(
...
export const getChannelsInCurrentTeam: (state: GlobalState) => Channel[] =
    createSelector(
...
export const getChannelsInAllTeams: (state: GlobalState) => Channel[] =
    createSelector(
        ...
        (
            channels: IDMappedObjects<Channel>,
            getChannelSetForAllTeams: string[],
            currentUser: UserProfile,
        ): Channel[] => {
...
export const getChannelsNameMapInTeam: (
    state: GlobalState,
    teamId: string,
) => Record<string, Channel> = createSelector(
...
export const getChannelsNameMapInCurrentTeam: (
    state: GlobalState,
) => Record<string, Channel> = createSelector(
...
export const getChannelNameToDisplayNameMap: (
    state: GlobalState,
) => Record<string, string> = createIdsSelector(
...
export const getAllDirectChannels: (state: GlobalState) => Channel[] =
    createSelector(
...
export const getAllDirectChannelsNameMapInCurrentTeam: (
    ...
    (
        ...
        channelSet.forEach((id) => {
            const channel = channels[id];
            channelMap[channel.name] = completeDirectChannelInfo(
                users,
                teammateNameDisplay,
                channel,
            );
...
// Returns only GMs
export const getGroupChannels: (state: GlobalState) => Channel[] =
    createSelector(
        "getGroupChannels",
        getAllChannels,
        getDirectChannelsSet,
...
export const getMyChannels: (state: GlobalState) => Channel[] = createSelector(
    ...
    (
        ...
        return [...channels, ...directChannels].filter((c) =>
            myMembers.hasOwnProperty(c.id),
        );
    },
);

export const getOtherChannels: (
    state: GlobalState,
    ...
    getChannelsInCurrentTeam,
    getMyChannelMemberships,
    (state: GlobalState, archived: boolean | undefined | null = true) =>
        archived,
    (
        channels: Channel[],
        myMembers: RelationOneToOne<Channel, ChannelMembership>,
...
export const getMembersInCurrentChannel: (
    state: GlobalState,
) => Record<string, ChannelMembership> = createSelector(
...
export function basicUnreadMeta(
    unreadStatus: BasicUnreadStatus,
): BasicUnreadMeta {
    return {
        isUnread: Boolean(unreadStatus),
        unreadMentionCount:
            (typeof unreadStatus === "number" && unreadStatus) || 0,
    };
}

// muted channel will not be counted towards unreads
export const getUnreadStatus: (state: GlobalState) => BasicUnreadStatus =
    createSelector(
        "getUnreadStatus",
        getAllChannels,
        getMyChannelMemberships,
        ...
        (
            ...
            } = Object.entries(myMembers).reduce(
                (counts, [channelId, membership]) => {
                    ...
                    const mentions = collapsedThreads
            ...
            const {
                ...
                (acc, team) => {
                    if (currentTeamId !== team.id) {
                        ...
                        acc.messages += collapsedThreads
                            ? member.msg_count_root
                            : member.msg_count;
                        acc.mentions += collapsedThreads
            ...
            const totalUnreadMessages =
                currentTeamUnreadMessages + otherTeamsUnreadMessages;
            let totalUnreadMentions =
                currentTeamUnreadMentions + otherTeamsUnreadMentions;
            ...
            if (collapsedThreads) {
                Object.keys(threadCounts).forEach((teamId) => {
                    ...
                    if (teamId === currentTeamId) {
                        const currentTeamDirectCounts =
                            threadCountsIncludingDirect[currentTeamId] || 0;
                        anyUnreadThreads = Boolean(
                            currentTeamDirectCounts.total_unread_threads,
                        );
                        totalUnreadMentions +=
                            currentTeamDirectCounts.total_unread_mentions;
                    } else {
                        anyUnreadThreads =
                            anyUnreadThreads || Boolean(c.total_unread_threads);
 ...
 */
export const getTeamsUnreadStatuses: (
    state: GlobalState,
) => [Set<Team["id"]>, Map<Team["id"], number>, Map<Team["id"], boolean>] =
    createSelector(
        ...
        (
            ...
            for (const [channelId, channelMembership] of Object.entries(
                channelMemberships,
            )) {
                const channel = channels[channelId];
                ...
                const unreadCountObjectForChannel = calculateUnreadCount(
                ...
                if (unreadCountObjectForChannel.mentions > 0) {
                    const previousMentionsInTeam = teamMentionsMap.has(
                        channel.team_id,
                    )
                        ? (teamMentionsMap.get(channel.team_id) as number)
                        : 0;
                    if (previousMentionsInTeam === 0) {
                        teamMentionsMap.set(
                            channel.team_id,
                            unreadCountObjectForChannel.mentions,
                        );
                    } else {
                        teamMentionsMap.set(
                            channel.team_id,
                            unreadCountObjectForChannel.mentions +
                                previousMentionsInTeam,
                        );
                    }
                }
                ...
                if (unreadCountObjectForChannel.hasUrgent) {
                    const previousHasUrgetInTeam = teamHasUrgentMap.has(
                        channel.team_id,
                    )
                        ? (teamHasUrgentMap.get(channel.team_id) as boolean)
                        : false;
                    if (!previousHasUrgetInTeam) {
                        teamHasUrgentMap.set(
                            channel.team_id,
                            unreadCountObjectForChannel.hasUrgent,
                        );
                    }
                }
            }
                ...
                for (const teamId of Object.keys(teamThreadCounts)) {
                    ...
                    if (threadCountsObjectForTeam.total_unread_mentions > 0) {
                        const previousMentionsInTeam = teamMentionsMap.has(
                            ...
                            ? (teamMentionsMap.get(teamId) as number)
                            : 0;
                        if (previousMentionsInTeam === 0) {
                            teamMentionsMap.set(
                                teamId,
                                threadCountsObjectForTeam.total_unread_mentions,
                            );
                        } else {
                            teamMentionsMap.set(
                                teamId,
                    ...
                    // Add mentions count from global threads view for team
                    if (
                        threadCountsObjectForTeam.total_unread_urgent_mentions
                    ) {
                        const previousHasUrgetInTeam = teamHasUrgentMap.has(
                            teamId,
                        )
                            ? (teamHasUrgentMap.get(teamId) as boolean)
                            : false;
                        if (!previousHasUrgetInTeam) {
                            teamHasUrgentMap.set(
                                teamId,
...
export const getUnreadStatusInCurrentTeam: (
    state: GlobalState,
) => BasicUnreadStatus = createSelector(
    ...
    (
        ...
        const {
            ...
            (counts, channel) => {
                ...
                const mentions = collapsedThreadsEnabled
                    ? m.mention_count_root
                    : m.mention_count;
                if (mentions) {
                    counts.mentions += mentions;
                }
        ...
        return (
            totalUnreadMentions ||
            anyUnreadThreads ||
            Boolean(currentTeamUnreadMessages)
        );
    },
);

export const canManageChannelMembers: (state: GlobalState) => boolean =
    createSelector(
...
export function canManageAnyChannelMembersInCurrentTeam(
    state: GlobalState,
): boolean {
...
export const getAllDirectChannelIds: (state: GlobalState) => string[] =
    createIdsSelector(
...
export const getChannelIdsInCurrentTeam: (state: GlobalState) => string[] =
    createIdsSelector(
...
export const getChannelIdsForCurrentTeam: (state: GlobalState) => string[] =
    createIdsSelector(
...
export const getChannelIdsInAllTeams: (state: GlobalState) => string[] =
    createIdsSelector(
...
export const getChannelIdsForAllTeams: (state: GlobalState) => string[] =
    createIdsSelector(
...
export const getUnreadChannelIds: (
    ...
    (
        state: GlobalState,
        lastUnreadChannel: Channel | undefined | null = null,
    ): Channel | undefined | null => lastUnreadChannel,
    (
        collapsedThreads,
        ...
        if (
            lastUnreadChannel &&
            members[lastUnreadChannel.id] &&
            !unreadIds.includes(lastUnreadChannel.id)
        ) {
...
export const getAllTeamsUnreadChannelIds: (state: GlobalState) => string[] =
    createIdsSelector(
    ...
    (
        ...
        const allUnreadChannels = unreadIds
            ...
            .map((id) => {
                ...
                if (
                    c.type === General.DM_CHANNEL ||
                    c.type === General.GM_CHANNEL
                ) {
...
export const getUnsortedAllTeamsUnreadChannels: (
    state: GlobalState,
) => Channel[] = createSelector(
    ...
    (
        ...
        return allTeamsUnreadChannelIds
            .filter((id) => channels[id] && channels[id].delete_at === 0)
            .map((id) => {
                const c = channels[id];
...
export const sortUnreadChannels = (
    ...
    function hasMentions(channel: Channel) {
        if (
            lastUnreadChannel &&
            channel.id === lastUnreadChannel.id &&
            lastUnreadChannel.hadMentions
        ) {
            return true;
        }
    ...
    return [...channels].sort((a, b) => {
        ...
        const aLastPostAt =
            max([
                crtEnabled ? a.last_root_post_at : a.last_post_at,
                a.create_at,
            ]) || 0;
        const bLastPostAt =
            max([
                crtEnabled ? b.last_root_post_at : b.last_post_at,
                b.create_at,
            ]) || 0;
...
export const getDirectAndGroupChannels: (a: GlobalState) => Channel[] =
    createSelector(
        ...
        return channels.map(
            (
                ...
                const profiles = getProfiles(
                    currentUserId,
                    channelUserMap[channel.id] || new Set(),
                    users,
                );
                return {
                    ...channel,
...
export const getDefaultChannelForTeams: (
    state: GlobalState,
) => RelationOneToOne<Team, Channel> = createSelector(
    ...
    (channels: IDMappedObjects<Channel>): RelationOneToOne<Team, Channel> => {
        ...
        for (const channel of Object.keys(channels).map(
            (key) => channels[key],
        )) {
...
export const getMyFirstChannelForTeams: (
    state: GlobalState,
) => RelationOneToOne<Team, Channel> = createSelector(
            ...
            const teamChannels = Object.values(allChannels)
                .filter(
                    (channel: Channel) =>
                        ...
                        Boolean(myChannelMemberships[channel.id]),
                )
...
export const getRedirectChannelNameForTeam = (
    state: GlobalState,
    teamId: string,
): string => {
    const defaultChannelForTeam = getDefaultChannelForTeams(state)[teamId];
    const canIJoinPublicChannelsInTeam = haveITeamPermission(
        state,
        teamId,
    ...
    return (
        (myFirstChannelForTeam && myFirstChannelForTeam.name) ||
        General.DEFAULT_CHANNEL
    );
};

// isManually unread looks into state if the provided channelId is marked as unread by the user.
export function isManuallyUnread(
    state: GlobalState,
    channelId?: string,
): boolean {
    if (!channelId) {
...
export function getChannelModerations(
    state: GlobalState,
    channelId: string,
): ChannelModeration[] {
...
const EMPTY_OBJECT = {};
export function getChannelMemberCountsByGroup(
    state: GlobalState,
    channelId: string,
): ChannelMemberCountsByGroup {
    return (
        state.entities.channels.channelMemberCountsByGroup[channelId] ||
        EMPTY_OBJECT
    );
}
...
export function getDirectTeammate(
    state: GlobalState,
    channelId: string,
...
export function makeGetGmChannelMemberCount(): (
    state: GlobalState,
    channel: Channel,
) => number {
    return createSelector(
        ...
        (memberIds, userId, channel) => {
            ...
            if (memberIds && memberIds[channel.id]) {
                const groupMemberIds: Set<string> = memberIds[
                    channel.id
...
export const getRecentProfilesFromDMs: (state: GlobalState) => UserProfile[] =
    createSelector(
        ...
        (
            ...
            const recentChannelIds = Object.values(memberships)
                .sort((aMembership, bMembership) => {
                    return (
                        bMembership.last_viewed_at - aMembership.last_viewed_at
                    );
                })
                .map((membership) => membership.channel_id);
            const groupChannels = Object.values(allChannels).filter(
                (channel: Channel) => channel.type === General.GM_CHANNEL,
            );
            const dmChannels = Object.values(allChannels).filter(
                (channel: Channel) => channel.type === General.DM_CHANNEL,
            );
            ...
            groupChannels.forEach((channel) => {
                if (channel.display_name) {
                    const memberUsernames = channel.display_name
                        .split(",")
                    ...
                    if (memberProfiles) {
                        userProfilesByChannel[channel.id] =
                            memberProfiles as UserProfile[];
                    }
                }
            });
            ...
            recentChannelIds.forEach((cid: string) => {
                if (userProfilesByChannel[cid]) {
                    userProfilesByChannel[cid].forEach((user) =>
                        sortedUserProfiles.add(user),
                    );
                }
            });
