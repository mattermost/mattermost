...
const searchProfilesMatchingWithTerm =
    Selectors.makeSearchProfilesMatchingWithTerm();
const searchProfilesStartingWithTerm =
    Selectors.makeSearchProfilesStartingWithTerm();
...
describe("Selectors.Users", () => {
    ...
    user1.notify_props = {
        mention_keys: "testkey1,testkey2",
    ...
    const membersInTeam: Record<
        Team["id"],
        Record<UserProfile["id"], TeamMembership>
    > = {};
    ...
    const membersInChannel: Record<
        Channel["id"],
        Record<UserProfile["id"], ChannelMembership>
    > = {};
    ...
    const profilesNotInChannel: Record<
        Channel["id"],
        Set<UserProfile["id"]>
    > = {};
    ...
    it("getUserIdsInChannels", () => {
        expect(Selectors.getUserIdsInChannels(testState)).toEqual(
            profilesInChannel,
        );
    });

    it("getUserIdsNotInChannels", () => {
        expect(Selectors.getUserIdsNotInChannels(testState)).toEqual(
            profilesNotInChannel,
        );
    });
    ...
    it("getUserIdsNotInTeams", () => {
        expect(Selectors.getUserIdsNotInTeams(testState)).toEqual(
            profilesNotInTeam,
        );
    });

    it("getUserIdsWithoutTeam", () => {
        expect(Selectors.getUserIdsWithoutTeam(testState)).toEqual(
            profilesWithoutTeam,
        );
    });
    ...
    describe("getCurrentUserMentionKeys", () => {
        it("at mention", () => {
            ...
            expect(Selectors.getCurrentUserMentionKeys(state)).toEqual([
                { key: "@user" },
            ]);
        });
    ...
    describe("getProfiles", () => {
        ...
        it("getProfiles with role filter", () => {
            ...
            expect(
                Selectors.getProfiles(testState, { role: "system_admin" }),
            ).toEqual(users);
        });
        it("getProfiles with inactive", () => {
            const users = [user2, user7].sort(sortByUsername);
            expect(
                Selectors.getProfiles(testState, { inactive: true }),
            ).toEqual(users);
        });
        it("getProfiles with active", () => {
            const users = [user1, user3, user4, user5, user6].sort(
                sortByUsername,
            );
            expect(Selectors.getProfiles(testState, { active: true })).toEqual(
                users,
            );
        });
        it("getProfiles with multiple filters", () => {
            const users = [user7];
            expect(
                Selectors.getProfiles(testState, {
                    role: "system_admin",
                    inactive: true,
                }),
            ).toEqual(users);
    ...
    describe("getProfilesInTeam", () => {
        it("getProfilesInTeam without filter", () => {
            const users = [user1, user2, user7].sort(sortByUsername);
            expect(Selectors.getProfilesInTeam(testState, team1.id)).toEqual(
                users,
            );
        ...
        it("getProfilesInTeam with role filter", () => {
            ...
            expect(
                Selectors.getProfilesInTeam(testState, team1.id, {
                    role: "system_admin",
                }),
            ).toEqual(users);
            expect(
                Selectors.getProfilesInTeam(testState, "junk", {
                    role: "system_admin",
                }),
            ).toEqual([]);
        });
        ...
        it("getProfilesInTeam with role filters", () => {
            ...
            ).toEqual([user1, user7].sort(sortByUsername));
            expect(
                Selectors.getProfilesInTeam(testState, team1.id, {
    ...
    describe("getProfilesNotInTeam", () => {
        ...
        expect(Selectors.getProfilesNotInTeam(testState, team1.id)).toEqual(
            users,
        );
        expect(
            Selectors.getProfilesNotInTeam(testState, team1.id, {
                role: "system_user",
            }),
        ).toEqual(users);
        expect(
    ...
    describe("getProfilesWithoutTeam", () => {
        it("getProfilesWithoutTeam", () => {
            ...
            expect(
                Selectors.getProfilesWithoutTeam(testState, {} as any),
            ).toEqual(users);
        });
        it("getProfilesWithoutTeam with filter", () => {
            expect(
                Selectors.getProfilesWithoutTeam(testState, {
                    role: "system_admin",
                }),
            ).toEqual([user6]);
        });
    ...
    it("getProfilesInGroup", () => {
        expect(Selectors.getProfilesInGroup(testState, group1.id)).toEqual([
            user1,
        ]);
        const users = [user2, user3].sort(sortByUsername);
        expect(Selectors.getProfilesInGroup(testState, group2.id)).toEqual(
            users,
        );
    });
    ...
    describe("searchProfilesStartingWithTerm", () => {
        it("searchProfiles without filter", () => {
            expect(
                searchProfilesStartingWithTerm(testState, user1.username),
            ).toEqual([user1]);
            expect(
                searchProfilesStartingWithTerm(
                    testState,
                    user2.first_name + " " + user2.last_name,
                ),
            ).toEqual([user2]);
            expect(
                searchProfilesStartingWithTerm(testState, user1.username, true),
            ).toEqual([]);
        });
    ...
    describe("searchProfilesMatchingWithTerm", () => {
        it("searchProfiles without filter", () => {
            expect(
                searchProfilesMatchingWithTerm(
                    testState,
                    user1.username.slice(1, user1.username.length),
                ),
            ).toEqual([user1]);
            expect(
                searchProfilesMatchingWithTerm(
        ...
        it("searchProfiles with filters", () => {
            expect(
                searchProfilesMatchingWithTerm(
                    testState,
                    user1.username.slice(2, user1.username.length),
                    false,
                    { role: "system_admin" },
                ),
            ).toEqual([user1]);
            expect(
                searchProfilesMatchingWithTerm(
                    testState,
                    user3.username.slice(3, user3.username.length),
                    false,
                    { role: "system_admin" },
                ),
            ).toEqual([]);
            expect(
                searchProfilesMatchingWithTerm(
                    testState,
                    user1.username.slice(0, user1.username.length),
                    false,
                    { roles: ["system_user"] },
                ),
            ).toEqual([]);
    ...
    it("searchProfilesInChannel", () => {
        const doSearchProfilesInChannel =
            Selectors.makeSearchProfilesInChannel();
        expect(
            doSearchProfilesInChannel(testState, channel1.id, user1.username),
        ).toEqual([user1]);
        expect(
        ...
        expect(
            doSearchProfilesInChannel(testState, channel2.id, user2.username),
        ).toEqual([user2]);
        expect(
            doSearchProfilesInChannel(
                testState,
    ...
    it("searchProfilesInCurrentChannel", () => {
        expect(
            Selectors.searchProfilesInCurrentChannel(testState, user1.username),
        ).toEqual([user1]);
        expect(
            Selectors.searchProfilesInCurrentChannel(
                testState,
                user1.username,
                true,
            ),
    ...
    it("searchProfilesNotInCurrentChannel", () => {
        expect(
            Selectors.searchProfilesNotInCurrentChannel(
                testState,
                user2.username,
            ),
        ).toEqual([user2]);
    ...
    it("searchProfilesInCurrentTeam", () => {
        expect(
            Selectors.searchProfilesInCurrentTeam(testState, user1.username),
        ).toEqual([user1]);
        expect(
            Selectors.searchProfilesInCurrentTeam(
                testState,
                user1.username,
                true,
            ),
    ...
    describe("searchProfilesInTeam", () => {
        it("searchProfilesInTeam without filter", () => {
            expect(
                Selectors.searchProfilesInTeam(
                    testState,
                    team1.id,
                    user1.username,
                ),
        ...
        it("getProfiles with multiple filters", () => {
            ...
            expect(
                Selectors.searchProfilesInTeam(
                    ...
                    user7.username,
                    false,
                    { role: "system_admin", inactive: true },
                ),
            ).toEqual(users);
        });
    });
    ...
    it("searchProfilesNotInCurrentTeam", () => {
        expect(
            Selectors.searchProfilesNotInCurrentTeam(testState, user3.username),
        ).toEqual([user3]);
        expect(
            Selectors.searchProfilesNotInCurrentTeam(
                testState,
                user3.username,
                true,
            ),
    ...
    describe("searchProfilesWithoutTeam", () => {
        it("searchProfilesWithoutTeam without filter", () => {
            expect(
                Selectors.searchProfilesWithoutTeam(
                    testState,
                    user5.username,
                    false,
                    {},
    ...
    it("searchProfilesInGroup", () => {
        expect(
            Selectors.searchProfilesInGroup(
                testState,
                group1.id,
                user5.username,
            ),
    ...
    it("getUserByUsername", () => {
        expect(Selectors.getUserByUsername(testState, user1.username)).toEqual(
            user1,
        );
    });
    ...
    it("makeGetProfilesInChannel", () => {
        ...
        expect(
            getProfilesInChannel(testState, channel2.id, { active: true }),
        ).toEqual([user1]);
        expect(
            getProfilesInChannel(testState, channel2.id, {
                channel_roles: ["channel_admin"],
            }),
        ).toEqual([]);
        expect(
            getProfilesInChannel(testState, channel2.id, {
        ...
        ).toEqual([user1, user2].sort(sortByUsername));
    ...
    it("makeGetProfilesNotInChannel", () => {
        ...
        expect(getProfilesNotInChannel(testState, channel1.id)).toEqual(
            [user2, user3].sort(sortByUsername),
        );

        expect(getProfilesNotInChannel(testState, channel2.id)).toEqual(
    ...
    it("makeGetProfilesByIdsAndUsernames", () => {
        const getProfilesByIdsAndUsernames =
            Selectors.makeGetProfilesByIdsAndUsernames();

        const testCases = [
            { input: { allUserIds: [], allUsernames: [] }, output: [] },
            {
                input: {
                    allUserIds: ["nonexistentid"],
                    allUsernames: ["nonexistentid"],
                },
                output: [],
            },
            {
                input: { allUserIds: [user1.id], allUsernames: [] },
                output: [user1],
            },
            { input: { allUserIds: [user1.id] }, output: [user1] },
            {
                input: { allUserIds: [user1.id, "nonexistentid"] },
                output: [user1],
            },
            {
                input: { allUserIds: [user1.id, user2.id] },
                output: [user1, user2],
            },
            {
                input: { allUserIds: ["nonexistentid", user1.id, user2.id] },
                output: [user1, user2],
            },
            {
                input: { allUserIds: [], allUsernames: [user1.username] },
                output: [user1],
            },
            { input: { allUsernames: [user1.username] }, output: [user1] },
            {
                input: { allUsernames: [user1.username, "nonexistentid"] },
                output: [user1],
            },
            {
                input: { allUsernames: [user1.username, user2.username] },
                output: [user1, user2],
            },
            {
                input: {
            ...
            },
            {
                input: {
                    allUserIds: [user1.id],
                    allUsernames: [user2.username],
                },
                output: [user1, user2],
            },
            {
                input: {
                    allUserIds: [user1.id, user2.id],
                    allUsernames: [user3.username, user4.username],
                },
                output: [user1, user2, user3, user4],
            },
            {
                input: {
                    allUserIds: [user1.username, user2.username],
                    allUsernames: [user3.id, user4.id],
                },
                output: [],
            },
        ];
        ...
        testCases.forEach((testCase) => {
            expect(
                getProfilesByIdsAndUsernames(
                    testState,
                    testCase.input as Parameters<
                        typeof getProfilesByIdsAndUsernames
                    >[1],
                ),
    ...
    describe("makeGetDisplayName and makeDisplayNameGetter", () => {
        ...
        it("Should show full name since preferences is being used and LockTeammateNameDisplay is false", () => {
                    ...
                    preferences: {
                        myPreferences: {
                            [`${Preferences.CATEGORY_DISPLAY_SETTINGS}--${Preferences.NAME_NAME_FORMAT}`]:
                                {
                                    value: General.TEAMMATE_NAME_DISPLAY
                                        .SHOW_FULLNAME,
                                },
                        },
                    },
            ...
            expect(
                Selectors.makeGetDisplayName()(newTestState, testUser1.id),
            ).toEqual("First Last");
            expect(
                Selectors.makeDisplayNameGetter()(newTestState, false)(
                    testUser1,
                ),
            ).toEqual("First Last");
        });
    ...
    describe("currentUserHasAnAdminRole", () => {
        it("returns the expected result", () => {
            expect(Selectors.currentUserHasAnAdminRole(testState)).toEqual(
                true,
            );
            const state = {
                ...testState,
    ...
    describe("filterProfiles", () => {
        it("no filter, return all users", () => {
            expect(
                Object.keys(Selectors.filterProfiles(profiles)).length,
            ).toEqual(7);
        });
        ...
        it("filter roles", () => {
            ...
            expect(
                Object.keys(
                    Selectors.filterProfiles(profiles, filter, memberships),
                ).length,
            ).toEqual(4);
        });
        ...
        it("exclude bots", () => {
            ...
            expect(
                Object.keys(Selectors.filterProfiles(newProfiles, filter))
                    .length,
            ).toEqual(7);
        });
