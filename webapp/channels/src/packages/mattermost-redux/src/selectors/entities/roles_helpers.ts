...
export const getMySystemRoles: (state: GlobalState) => Set<string> =
    createSelector("getMySystemRoles", getCurrentUser, (user: UserProfile) => {
        if (user) {
...
export const getMySystemPermissions: (state: GlobalState) => Set<string> =
    createSelector(
...
export function haveISystemPermission(
    state: GlobalState,
