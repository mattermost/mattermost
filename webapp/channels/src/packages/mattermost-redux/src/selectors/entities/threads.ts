...
export function getThreadsInTeam(
    state: GlobalState,
): RelationOneToMany<Team, UserThread> {
    return state.entities.threads.threadsInTeam;
}

export function getUnreadThreadsInTeam(
    state: GlobalState,
): RelationOneToMany<Team, UserThread> {
    return state.entities.threads.unreadThreadsInTeam;
}

export const getThreadsInCurrentTeam: (
    state: GlobalState,
...
export const getUnreadThreadsInCurrentTeam: (
    state: GlobalState,
) => Array<UserThread["id"]> = createSelector(
...
export function getThreadCountsIncludingDirect(
    state: GlobalState,
): ThreadsState["counts"] {
    return state.entities.threads.countsIncludingDirect;
}

export const getThreadCountsInCurrentTeam: (
    state: GlobalState,
) => ThreadsState["counts"][Team["id"]] = createSelector(
...
export function makeGetThreadOrSynthetic(): (
    state: GlobalState,
    rootPost: Post,
) => UserThread | UserThreadSynthetic {
    return createSelector(
        ...
        (rootPost, threads) => {
            ...
            return {
                ...
                is_following:
                    thread?.is_following ?? rootPost.is_following ?? null,
                post: {
...
export const getThreadOrderInCurrentTeam: (
    state: GlobalState,
    ...
    getThreadsInCurrentTeam,
    getThreads,
    (state: GlobalState, selectedThreadIdInTeam?: UserThread["id"]) =>
        selectedThreadIdInTeam,
    (threadsInTeam, threads, selectedThreadIdInTeam) => {
...
export const getNewestThreadInTeam: (
    state: GlobalState,
    teamID: string,
    ...
    (threadsInTeam, threads, teamID: string) => {
        const threadsInGivenTeam = threadsInTeam?.[teamID] ?? [];
        if (!threadsInGivenTeam) {
            return null;
        }
        const ids = [
            ...threadsInGivenTeam.filter((id) => threads[id].is_following),
        ];
...
function sortByLastReply(
    ids: Array<UserThread["id"]>,
    threads: ReturnType<typeof getThreads>,
) {
    return ids
        .filter((id) => threads[id].last_reply_at !== 0)
        .sort((a, b) => threads[b].last_reply_at - threads[a].last_reply_at);
}
...
export const getThreadsInChannel: (
    ...
    (allThreads: IDMappedObjects<UserThread>, channelID: string) => {
        return Object.keys(allThreads).filter(
            (id) => allThreads[id].post.channel_id === channelID,
        );
    },
);
