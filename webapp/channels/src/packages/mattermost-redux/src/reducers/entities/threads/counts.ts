...
function isDmGmChannel(channelType: Channel["type"]) {
    return (
        channelType === General.DM_CHANNEL || channelType === General.GM_CHANNEL
    );
}

function handleAllTeamThreadsRead(
    state: ThreadsState["counts"],
...
function handleReadChangedThread(
    state: ThreadsState["counts"],
    action: GenericAction,
    teamId: string,
    isUrgent: boolean,
    ...
    counts.total_unread_mentions = Math.max(
        counts.total_unread_mentions + unreadMentionDiff,
        0,
    );
    if (isUrgent) {
        counts.total_unread_urgent_mentions = Math.max(
            counts.total_unread_urgent_mentions + unreadMentionDiff,
            0,
        );
    }
    ...
    if (newUnreadReplies > 0 && prevUnreadReplies === 0) {
        ...
        counts.total_unread_threads = Math.max(
            counts.total_unread_threads - 1,
            0,
        );
    }
...
function handleLeaveChannel(
    ...
    const { unreadMentions, unreadThreads, unreadUrgentMentions } =
        extra.threadsToDelete.reduce(
            (curr, item: UserThread) => {
                curr.unreadMentions += item.unread_mentions;
                curr.unreadThreads =
                    item.unread_replies > 0
                        ? curr.unreadThreads + 1
                        : curr.unreadThreads;
                curr.unreadUrgentMentions = item.is_urgent
                    ? curr.unreadUrgentMentions + item.unread_mentions
                    : curr.unreadUrgentMentions;
                return curr;
            },
    ...
    return {
        ...
        [teamId]: {
            ...
            total_unread_mentions: Math.max(
                totalUnreadMentions - unreadMentions,
                0,
            ),
            total_unread_threads: Math.max(
                totalUnreadThreads - unreadThreads,
                0,
            ),
            total_unread_urgent_mentions: Math.max(
                (totalUnreadUrgentMentions || 0) - unreadUrgentMentions,
                0,
            ),
        },
    };
...
function handleDecrementThreadCounts(
    state: ThreadsState["counts"],
    action: GenericAction,
) {
    ...
    return {
        ...
        [teamId]: {
            ...
            total_unread_mentions: Math.max(
                counts.total_unread_mentions - mentions,
                0,
            ),
            total_unread_threads: Math.max(
                counts.total_unread_threads - replies,
                0,
            ),
        },
    };
...
export function countsIncludingDirectReducer(
    state: ThreadsState["counts"] = {},
    action: GenericAction,
    extra: ExtraData,
) {
    switch (action.type) {
        ...
        case ThreadTypes.READ_CHANGED_THREAD: {
            ...
            if (isDmGmChannel(channelType)) {
                ...
                let newState = { ...state };
                teamIds.forEach((id) => {
                    newState = handleReadChangedThread(
                        newState,
                        action,
                        id,
                        isUrgent,
            ...
            return {
                ...
                [action.data.team_id]: {
                    ...
                    total_unread_urgent_mentions:
                        action.data.total_unread_urgent_mentions,
                },
            };
    ...
    switch (action.type) {
        ...
        case TeamTypes.RECEIVED_MY_TEAM_UNREADS: {
            ...
            return {
                ...state,
                ...members.reduce(
                    (result: ThreadsState["counts"], member: TeamUnread) => {
                        result[member.team_id] = {
                            ...state[member.team_id],
                            total_unread_threads: member.thread_count || 0,
                            total_unread_mentions:
                                member.thread_mention_count || 0,
                            total_unread_urgent_mentions:
                                member.thread_urgent_mention_count || 0,
                        };
