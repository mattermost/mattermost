...
function makeValue(
    overrides: Partial<PropertyValue<unknown>> = {},
...
describe('Reducers.ContentFlagging', () => {
    ...
    test('CONTENT_FLAGGING_REPORT_VALUE_UPDATED merges updated values with cached values', () => {
        const existingValue = makeValue({
            field_id: 'review_status',
            value: 'pending',
        });
        const retainedValue = makeValue({
            id: 'value-2',
            field_id: 'reason',
            value: 'data_spillage',
        });
        const updatedValue = makeValue({
            field_id: 'review_status',
            value: 'removed',
        });
    ...
    test('CONTENT_FLAGGING_REPORT_VALUE_UPDATED merges multiple values and preserves other posts', () => {
        ...
        const otherPostValue = makeValue({
            id: 'value-3',
            target_id: 'post-2',
            field_id: 'review_status',
            value: 'pending',
    ...
    test('CONTENT_FLAGGING_REPORT_VALUE_UPDATED ignores malformed property values', () => {
        const stateWithValues = contentFlaggingReducer(undefined, {
            ...
            data: {
                postId: 'post-1',
                values: [
                    makeValue({field_id: 'review_status', value: 'pending'}),
                ],
            },
        });

        expect(() =>
            contentFlaggingReducer(stateWithValues, {
                type: ContentFlaggingTypes.CONTENT_FLAGGING_REPORT_VALUE_UPDATED,
                data: {
                    target_id: 'post-1',
                    property_values: '{',
    ...
    test('CONTENT_FLAGGING_REPORT_VALUE_UPDATED tolerates non-array cached values', () => {
        ...
        const state = contentFlaggingReducer(
            invalidState as unknown as Parameters<
                typeof contentFlaggingReducer
            >[0],
            {
                type: ContentFlaggingTypes.CONTENT_FLAGGING_REPORT_VALUE_UPDATED,
                data: {
    ...
    describe('FLAGGED_POST_REMOVED', () => {
        test('clears the flagged post and its property values for the given post id', () => {
            ...
            const stateWithOtherValues = contentFlaggingReducer(
                stateWithValues,
                {
                    type: ContentFlaggingTypes.RECEIVED_POST_CONTENT_FLAGGING_VALUES,
                    data: {
            ...
            const stateWithFlaggedPost = contentFlaggingReducer(
                stateWithOtherValues,
                {
                    type: ContentFlaggingTypes.RECEIVED_FLAGGED_POST,
                    data: flaggedPost,
                },
            );
            const stateWithBothFlaggedPosts = contentFlaggingReducer(
                stateWithFlaggedPost,
                {
                    type: ContentFlaggingTypes.RECEIVED_FLAGGED_POST,
