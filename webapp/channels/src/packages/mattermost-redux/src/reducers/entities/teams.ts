...
function myMembers(
    state: RelationOneToOne<Team, TeamMembership> = {},
    ...
    switch (action.type) {
        ...
        case TeamTypes.RECEIVED_MY_TEAM_UNREADS: {
            ...
            for (const u of unreads) {
                ...
                const msgCountRoot =
                    u.msg_count_root < 0 ? 0 : u.msg_count_root;
                const mentionCountRoot =
                    u.mention_count_root < 0 ? 0 : u.mention_count_root;
                const m = {
                    ...state[u.team_id],
            ...
            return {
                ...
                [teamId]: {
                    ...
                    msg_count_root: Math.max(
                        member.msg_count_root - Math.abs(amountRoot),
                        0,
                    ),
                },
            };
        ...
        case ChannelTypes.POST_UNREAD_SUCCESS: {
            ...
            const newTeamState = {
                ...teamState,
                msg_count:
                    typeof teamState.msg_count === "undefined"
                        ? msgCount
                        : teamState.msg_count - deltaMsgs,
                mention_count:
                    typeof teamState.mention_count === "undefined"
                        ? mentionCount
                        : teamState.mention_count + mentionCount,
            };
        ...
        case UserTypes.LOGIN: {
            ...
            for (const m of teamMembers) {
                if (m.delete_at == null || m.delete_at === 0) {
                    const unread = teamUnreads.find(
                        (u: TeamUnread) => u.team_id === m.team_id,
                    );
                    if (unread) {
...
function updateMyTeamMemberSchemeRoles(
    state: RelationOneToOne<Team, TeamMembership>,
    action: GenericAction,
) {
