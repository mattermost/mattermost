...
import type {
    TrueUpReviewProfileReducer,
    TrueUpReviewStatusReducer,
...
function signupProgress(
    state = SelfHostedSignupProgress.START,
    action: GenericAction,
): SignupProgress {
    switch (action.type) {
        case HostedCustomerTypes.RECEIVED_SELF_HOSTED_SIGNUP_PROGRESS: {
            if (!action.data) {
                throw new Error(
                    `uh ohh, expect action to have data but it dit not. Action: ${JSON.stringify(
                        action,
                        null,
                        2,
                    )}`,
...
export function errors(
    state: ErrorsReducer = emptyErrors,
    action: GenericAction,
) {
    switch (action.type) {
...
function trueUpReviewProfile(
    state: TrueUpReviewProfileReducer | null = null,
    action: GenericAction,
) {
    switch (action.type) {
...
function trueUpReviewStatus(
    state: TrueUpReviewStatusReducer | null = null,
    action: GenericAction,
) {
    switch (action.type) {
