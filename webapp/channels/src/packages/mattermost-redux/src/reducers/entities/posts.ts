...
export function nextPostsReplies(
    state: { [x in Post["id"]]: number } = {},
...
export function handlePosts(
    state: RelationOneToOne<Post, Post> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case PostTypes.RECEIVED_POST:
        ...
        case PostTypes.POST_DELETED: {
            ...
            for (const otherPost of Object.values(state)) {
                ...
                if (
                    otherPost.metadata &&
                    otherPost.metadata.embeds &&
                    otherPost.metadata.embeds.length > 0
                ) {
                    ...
                    for (const embed of otherPost.metadata.embeds) {
                        if (
                            ...
                            embed.data &&
                            (embed.data as PostPreviewMetadata).post_id ===
                                post.id
                        ) {
...
function handlePostReceived(
    nextState: any,
    post: Post,
    nestedPermalinkLevel?: number,
) {
    let currentState = nextState;

    // Check if post already exists in state or if nested permalink
    if (
        !shouldUpdatePost(post, currentState[post.id]) ||
        (nestedPermalinkLevel && nestedPermalinkLevel > 1)
    ) {
        return currentState;
    }
    ...
    if (
        !nestedPermalinkLevel &&
        isPermalink(post) &&
        currentState[post.id] &&
        !currentState[post.id].metadata &&
        post.metadata
    ) {
        ...
        post.metadata.embeds.forEach((embed) => {
            if (embed.type === "permalink") {
                if (embed.data && "post_id" in embed.data && embed.data.post) {
                    currentState = handlePostReceived(
                        currentState,
                        embed.data.post,
                        nestedPermalinkLevel ? nestedPermalinkLevel + 1 : 1,
                    );
    ...
    if (
        ...
        post.id !== post.pending_post_id &&
        currentState[post.pending_post_id]
    ) {
    ...
    if (post.root_id && rootPost) {
        ...
        if (
            !participants.find((user: UserProfile) => user.id === post.user_id)
        ) {
...
export function handlePendingPosts(
    state: string[] = [],
    action: GenericAction,
) {
    switch (action.type) {
...
export function postsInChannel(
    state: Record<string, PostOrderBlock[]> = {},
    action: GenericAction,
    prevPosts: IDMappedObjects<Post>,
    nextPosts: Record<string, Post>,
) {
    switch (action.type) {
        ...
        case PostTypes.RECEIVED_NEW_POST: {
            ...
            if (
                action.features?.crtEnabled &&
                post.root_id &&
                post.type !== PostConstant.EPHEMERAL
            ) {
                return state;
            }
            ...
            const recentBlockIndex = postsForChannel.findIndex(
                (block: PostOrderBlock) => block.recent,
            );
            ...
            if (post.pending_post_id && post.id !== post.pending_post_id) {
                const index = nextRecentBlock.order.indexOf(
                    post.pending_post_id,
                );
        ...
        case PostTypes.RECEIVED_POSTS_IN_CHANNEL: {
            ...
            if (recent) {
                ...
                if (recentBlockIndex !== -1) {
                    ...
                    if (
                        ...
                        recentBlock.order[recentBlock.order.length - 1] ===
            ...
            nextPostsForChannel = mergePostBlocks(
                nextPostsForChannel,
                nextPosts,
            );
        ...
        case PostTypes.RECEIVED_POSTS_SINCE: {
            ...
            const mostOldestCreateAt =
                nextPosts[recentBlock.order[recentBlock.order.length - 1]]
                    .create_at;
        ...
        case PostTypes.POST_DELETED: {
            ...
            for (let i = 0; i < nextPostsForChannel.length; i++) {
                ...
                const nextOrder = block.order.filter(
                    (postId: string) => prevPosts[postId].root_id !== post.id,
                );
            ...
            nextPostsForChannel =
                removeNonRecentEmptyPostBlocks(nextPostsForChannel);
        ...
        case PostTypes.POST_REMOVED: {
            ...
            for (let i = 0; i < nextPostsForChannel.length; i++) {
                ...
                const nextOrder = block.order.filter(
                    (postId: string) =>
                        postId !== post.id &&
                        prevPosts[postId].root_id !== post.id,
                );
...
export function removeNonRecentEmptyPostBlocks(blocks: PostOrderBlock[]) {
    return blocks.filter(
        (block: PostOrderBlock) => block.order.length !== 0 || block.recent,
    );
}
...
export function mergePostOrder(
    left: string[],
    right: string[],
    posts: Record<string, Post>,
) {
    ...
    switch (action.type) {
        ...
        case PostTypes.RECEIVED_POSTS_SINCE: {
            ...
            for (const post of newPosts) {
                ...
                const postsForThread = state[post.root_id] || [];
                const nextPostsForThread = nextState[post.root_id] || [
                    ...postsForThread,
                ];
        ...
        case PostTypes.POST_REMOVED: {
            ...
            if (post.root_id) {
                ...
                const index = postsForThread.findIndex(
        ...
        case ChannelTypes.LEAVE_CHANNEL: {
            ...
            for (const rootId of Object.keys(state)) {
                if (
                    prevPosts[rootId] &&
                    prevPosts[rootId].channel_id === channelId
                ) {
...
export function reactions(
    state: RelationOneToOne<Post, Record<string, Reaction>> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case PostTypes.RECEIVED_REACTIONS: {
            ...
            reactionsList.forEach((reaction: Reaction) => {
                nextReactions[reaction.user_id + "-" + reaction.emoji_name] =
                    reaction;
            });
        ...
        case PostTypes.REACTION_DELETED: {
            ...
            Reflect.deleteProperty(
                nextReactions,
                reaction.user_id + "-" + reaction.emoji_name,
            );
...
export function acknowledgements(
    state: RelationOneToOne<Post, Record<UserProfile["id"], number>> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
function storeReactionsForPost(state: any, post: Post) {
    ...
    if (post.metadata.reactions && post.metadata.reactions.length > 0) {
        for (const reaction of post.metadata.reactions) {
            reactionsForPost[reaction.user_id + "-" + reaction.emoji_name] =
                reaction;
        }
    }
...
function storeAcknowledgementsForPost(state: any, post: Post) {
    ...
    if (
        post?.metadata?.acknowledgements &&
        post.metadata.acknowledgements.length > 0
    ) {
    ...
    return post.metadata.embeds.reduce((nextState, embed) => {
        ...
        if (
            ...
            if (previewPost.metadata && previewPost.metadata.embeds) {
                return previewPost.metadata.embeds.reduce(
                    (nextState, embed) => {
                        if (
                            embed.type !== "opengraph" ||
                            !embed.data ||
                            nextState[previewPost.id]
                        ) {
                            return nextState;
                        }
...
export function limitedViews(
    ...
    switch (action.type) {
            ...
            if (action.data.first_inaccessible_post_time && action.channelId) {
                return {
                    ...
                    channels: {
                        ...state.channels,
                        [action.channelId]:
                            action.data.first_inaccessible_post_time || 0,
                    },
                };
            }
            return state;
        ...
        case CloudTypes.RECEIVED_CLOUD_LIMITS: {
            ...
            if (
                !limits?.messages ||
                (!limits?.messages?.history && limits?.messages?.history !== 0)
            ) {
...
export default function reducer(
    ...
    const nextState = {
        ...
        currentFocusedPostId: currentFocusedPostId(
            state.currentFocusedPostId,
            action,
        ),
