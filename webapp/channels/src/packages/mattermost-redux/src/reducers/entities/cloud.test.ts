...
describe("limits reducer", () => {
    test("returns empty limits by default", () => {
        expect(
            limits(undefined, { type: "some action", data: minimalLimits }),
        ).toEqual({
            limits: {},
            limitsLoaded: false,
        });
    ...
    test("returns prior limits on unmatched action", () => {
        const unchangedLimits = limits(minimalLimits, {
            type: "some action",
            data: {
                ...minimalLimits,
                integrations: {
    ...
    test("returns new limits on RECEIVED_CLOUD_LIMITS", () => {
        ...
        expect(unchangedLimits).toEqual({
    ...
    test("clears limits when new subscription received", () => {
        const emptyLimits = limits(minimalLimits, {
