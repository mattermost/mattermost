...
describe("Reducers.users", () => {
    describe("profilesInChannel", () => {
        it("initial state", () => {
            ...
            expect(newState.profilesInChannel).toEqual(
                expectedState.profilesInChannel,
            );
        });
    ...
    describe("profilesNotInChannel", () => {
        it("initial state", () => {
            ...
            const newState = reducer(state, action as GenericAction);
            expect(newState.profilesNotInChannel).toEqual(
    ...
    describe("profilesNotInGroup", () => {
        it("initial state", () => {
            ...
            const newState = reducer(state, action as GenericAction);
            expect(newState.profilesNotInGroup).toEqual(
                expectedState.profilesNotInGroup,
            );
        });
    ...
    describe("profiles", () => {
        ...
        for (const actionType of [
            UserTypes.RECEIVED_ME,
            UserTypes.RECEIVED_PROFILE,
        ]) {
            ...
            test(`should not overwrite unsanitized data with sanitized data (${actionType})`, () => {
                ...
                expect(nextState.profiles[user1.id].auth_service).toBe(
                    user1.auth_service,
                );
            });
        ...
        for (const actionType of [
            UserTypes.RECEIVED_PROFILES,
            UserTypes.RECEIVED_PROFILES_LIST,
        ]) {
        ...
        test("UserTypes.RECEIVED_PROFILES_LIST, should merge existing users with new ones", () => {
            ...
            const action = {
                ...
                data: [partialUpdatedFirstUser, thirdUser],
            };
            const { profiles: newProfiles } = reducer(
                state as unknown as ReducerState,
                action,
            );

            expect(newProfiles.first_user_id).toEqual({
                ...firstUser,
