...
export function byId(
    state: IDMappedObjects<ChannelCategory> = {},
    ...
    switch (action.type) {
        ...
        case ChannelTypes.GM_CONVERTED_TO_CHANNEL: {
            ...
            categoryIDs.forEach((categoryID) => {
                if (
                    categoryID.startsWith("channels_") &&
                    state[categoryID].team_id === receivedChannel.team_id &&
                    state[categoryID].channel_ids.indexOf(receivedChannel.id) <
                        0
                ) {
                    ...
                    newState[categoryID] = {
                        ...state[categoryID],
                        channel_ids: state[categoryID].channel_ids.filter(
                            (channelID) => channelID !== receivedChannel.id,
                        ),
                    };
                }
            });
...
export function orderByTeam(
    state: RelationOneToOne<Team, Array<ChannelCategory["id"]>> = {},
    action: GenericAction,
) {
    switch (action.type) {
