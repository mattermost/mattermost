...
function removeMemberFromChannels(
    state: RelationOneToOne<Channel, Record<string, ChannelMembership>>,
    ...
    switch (action.type) {
        case ChannelTypes.RECEIVED_CHANNEL: {
            ...
            if (state[channel.id] && channel.type === General.DM_CHANNEL) {
                channel.display_name =
                    channel.display_name || state[channel.id].display_name;
            }
        ...
        case AdminTypes.RECEIVED_DATA_RETENTION_CUSTOM_POLICY_CHANNELS: {
            const channels: Channel[] =
                action.data.channels.map(toClientChannel);
        ...
        case SchemeTypes.RECEIVED_SCHEME_CHANNELS: {
            ...
            for (let channel of channels) {
                if (
                    ...
                    channel = {
                        ...channel,
                        display_name: state[channel.id].display_name,
                    };
                }
        ...
        case PostTypes.RECEIVED_NEW_POST: {
            ...
            const lastRootPostAt = isCrtReply
                ? channel.last_root_post_at
                : Math.max(createAt, channel.last_root_post_at);
        ...
        case ChannelTypes.UPDATED_CHANNEL_SCHEME: {
            ...
            return {
                ...state,
                [channelId]: { ...channel, scheme_id: schemeId },
            };
        }
...
function channelsInTeam(
    state: RelationOneToMany<Team, Channel> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case ChannelTypes.RECEIVED_CHANNEL: {
...
export function myMembers(
    state: RelationOneToOne<Channel, ChannelMembership> = {},
    action: GenericAction,
) {
    switch (action.type) {
            ...
            return {
                ...
                [channelId]: {
                    ...
                    urgent_mention_count:
                        member.urgent_mention_count + amountUrgent,
                },
            };
        }
        case ChannelTypes.DECREMENT_UNREAD_MENTION_COUNT: {
            ...
            return {
                ...
                [channelId]: {
                    ...
                    mention_count_root: Math.max(
                        member.mention_count_root - amountRoot,
                        0,
                    ),
                    urgent_mention_count: Math.max(
                        member.urgent_mention_count - amountUrgent,
                        0,
                    ),
                },
...
function receiveChannelMember(
    ...
    if (
        existingChannelMember &&
        updatedChannelMember.last_viewed_at <
            existingChannelMember.last_viewed_at &&
        updatedChannelMember.last_update_at <=
            existingChannelMember.last_update_at
    ) {
        ...
        updatedChannelMember = {
            ...
            last_viewed_at: Math.max(
                existingChannelMember.last_viewed_at,
                received.last_viewed_at,
            ),
            last_update_at: Math.max(
                existingChannelMember.last_update_at,
                received.last_update_at,
            ),
            msg_count: Math.max(
                existingChannelMember.msg_count,
                received.msg_count,
            ),
            msg_count_root: Math.max(
                existingChannelMember.msg_count_root,
                received.msg_count_root,
            ),
            mention_count: Math.min(
                existingChannelMember.mention_count,
                received.mention_count,
            ),
            urgent_mention_count: Math.min(
                existingChannelMember.urgent_mention_count,
                received.urgent_mention_count,
            ),
            mention_count_root: Math.min(
                existingChannelMember.mention_count_root,
                received.mention_count_root,
            ),
        };
    }
        ...
        case ChannelTypes.RECEIVED_CHANNEL_MEMBER: {
            ...
            if (
                ...
                member.last_update_at >
                    members[member.user_id]?.last_update_at ||
...
function stats(
    state: RelationOneToOne<Channel, ChannelStats> = {},
    action: GenericAction,
) {
    switch (action.type) {
    ...
    switch (action.type) {
        ...
        case GroupTypes.RECEIVED_GROUP_ASSOCIATED_TO_CHANNEL: {
            ...
            const associatedGroupIDs = new Set(
                state[channelID] ? state[channelID].ids : [],
            );
            ...
            nextState[channelID] = {
                ...
                totalCount: associatedGroupIDs.size,
            };
...
export function manuallyUnread(
    state: RelationOneToOne<Channel, boolean> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
export function channelMemberCountsByGroup(
    state: any = {},
    action: GenericAction,
) {
    switch (action.type) {
        case ChannelTypes.RECEIVED_CHANNEL_MEMBER_COUNTS_BY_GROUP: {
            ...
            memberCounts.forEach(
                (channelMemberCount: ChannelMemberCountByGroup) => {
                    memberCountsByGroup[channelMemberCount.group_id] =
                        channelMemberCount;
                },
            );
...
function roles(
    state: RelationOneToOne<Channel, Set<string>> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case ChannelTypes.RECEIVED_MY_CHANNEL_MEMBER: {
