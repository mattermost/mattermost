...
type State =
    | ThreadsState["threadsInTeam"]
    | ThreadsState["unreadThreadsInTeam"];
...
function handlePostRemoved(state: State, action: GenericAction): State {
    ...
    const teams = Object.keys(state).filter(
        (id) => state[id].indexOf(post.id) !== -1,
    );
...
function handleAllTeamsReceivedThread(
    state: State,
    thread: UserThread,
    teamId: Team["id"],
    extra: ExtraData,
    ...
    for (const teamId of teamIds) {
        newState = handleSingleTeamReceivedThread(
            newState,
            thread,
            teamId,
            extra,
...
function handleSingleTeamReceivedThread(
    state: State,
    thread: UserThread,
    teamId: Team["id"],
    extra: ExtraData,
...
export function handleReceivedThread(
    state: State,
...
function handleLeaveChannel(
    ...
    for (const thread of extra.threadsToDelete) {
        if (nextState[teamId]) {
            ...
            if (index !== -1) {
                nextState[teamId] = [
                    ...nextState[teamId].slice(0, index),
                    ...nextState[teamId].slice(index + 1),
                ];
                threadDeleted = true;
            }
        }
...
function handleSingleTeamThreadRead(
    ...
    return {
        ...state,
        [teamId]: [...team.slice(0, index), ...team.slice(index + 1)],
    };
}

export const threadsInTeamReducer = (
    state: ThreadsState["threadsInTeam"] = {},
    action: GenericAction,
    extra: ExtraData,
) => {
    switch (action.type) {
    ...
    switch (action.type) {
        case ThreadTypes.READ_CHANGED_THREAD: {
            ...
            if (teamId === "") {
                ...
                let newState = { ...state };
                for (const teamId of teamIds) {
                    newState = handleSingleTeamThreadRead(
                        newState,
                        action,
                        teamId,
                        extra,
        ...
        case ThreadTypes.RECEIVED_THREAD:
            if (
                action.data.thread.unread_replies > 0 ||
                action.data.thread.unread_mentions > 0
            ) {
                return handleReceivedThread(state, action, extra);
            }
            return state;
