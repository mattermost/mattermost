...
function prevTrialLicense(
    state: Partial<AdminConfig> = {},
...
function environmentConfig(
    state: Partial<EnvironmentConfig> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
function complianceReports(
    state: Record<string, Compliance> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
function samlCertStatus(
    state: Partial<SamlCertificateStatus> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
export function convertAnalyticsRowsToStats(
    data: AnalyticsRow[],
    name: string,
...
function analytics(
    state: Record<string, number | AnalyticsRow[]> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
function teamAnalytics(
    state: RelationOneToOne<Team, Record<string, number | AnalyticsRow[]>> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case AdminTypes.RECEIVED_TEAM_ANALYTICS: {
            const nextState = { ...state };
            const stats = convertAnalyticsRowsToStats(action.data, action.name);
            const analyticsForTeam = {
                ...(nextState[action.teamId] || {}),
                ...stats,
            };
...
function userAccessTokensByUser(
    state: RelationOneToOne<UserProfile, Record<string, UserAccessToken>> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case AdminTypes.RECEIVED_USER_ACCESS_TOKEN: {
...
function plugins(
    state: Record<string, PluginRedux> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
function pluginStatuses(
    state: Record<string, PluginStatusRedux> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case AdminTypes.RECEIVED_PLUGIN_STATUSES: {
            ...
            for (const plugin of action.data || []) {
                ...
                // surface an error.
                const pluginState = Math.max(
                    (nextState[id] && nextState[id].state) || 0,
                    plugin.state,
                );
                ...
                nextState[id] = {
                    ...
                    description:
                        (nextState[id] && nextState[id].description) ||
                        plugin.description,
                    version:
                        (nextState[id] && nextState[id].version) ||
                        plugin.version,
...
function ldapGroups(
    state: Record<string, MixedUnlinkedGroupRedux> = {},
    action: GenericAction,
) {
    switch (action.type) {
        case AdminTypes.RECEIVED_LDAP_GROUPS: {
...
function samlMetadataResponse(
    state: Partial<SamlMetadataResponse> = {},
    action: GenericAction,
) {
    switch (action.type) {
...
function dataRetentionCustomPolicies(
    state: IDMappedObjects<DataRetentionCustomPolicy> = {},
    action: GenericAction,
): IDMappedObjects<DataRetentionCustomPolicy> {
    switch (action.type) {
