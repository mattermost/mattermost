    ...
    switch (action.type) {
        ...
        case ThreadTypes.RECEIVED_THREADS: {
            ...
            return {
                ...
                ...threads.reduce(
                    (
                        results: IDMappedObjects<UserThread>,
                        thread: UserThread,
                    ) => {
        ...
        case ThreadTypes.READ_CHANGED_THREAD: {
            const { id, newUnreadMentions, newUnreadReplies, lastViewedAt } =
                action.data;
        ...
        case ThreadTypes.ALL_TEAM_THREADS_READ: {
            return Object.entries(state).reduce<ThreadsState["threads"]>(
                (newState, [id, thread]) => {
                    newState[id] = {
                        ...thread,
                        unread_mentions: 0,
...
function reducer(
    ...
    if (
        ...
        if (!action.data.viewArchivedChannels) {
            extra.threadsToDelete = getThreadsOfChannel(
                state.threads,
                action.data.id,
            );
        }
    ...
    const nextState = {
        ...
        unreadThreadsInTeam: unreadThreadsInTeamReducer(
            state.unreadThreadsInTeam,
            action,
            extra,
        ),
        ...
        countsIncludingDirect: countsIncludingDirectReducer(
            state.countsIncludingDirect,
            action,
            extra,
        ),
    };
