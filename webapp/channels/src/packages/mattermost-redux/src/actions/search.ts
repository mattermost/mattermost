...
export function getMissingChannelsFromPosts(
    posts: PostList["posts"],
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        const { channels, membersInChannel, myMembers } =
...
export function getMissingChannelsFromFiles(
...
export function searchPostsWithParams(
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        try {
            ...
            const profilesAndStatuses = getMentionsAndStatusesForPosts(
                posts.posts,
                dispatch,
                getState,
            );
            const missingChannels = dispatch(
                getMissingChannelsFromPosts(posts.posts),
            );
            const arr: [Promise<any>, Promise<any>] = [
                profilesAndStatuses,
                missingChannels,
            ];
...
export function searchFilesWithParams(
    teamId: string,
    params: SearchParameter,
): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
...
export function getPinnedPosts(channelId: string): ActionFunc {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        try {
            ...
            const missingChannels = dispatch(
                getMissingChannelsFromPosts(result.posts),
            );
            const arr: [Promise<any>, Promise<any>] = [
                profilesAndStatuses,
                missingChannels,
