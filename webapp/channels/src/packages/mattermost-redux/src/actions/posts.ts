...
export function receivedPostsAfter(
    posts: PostList,
    channelId: string,
    afterPostId: string,
    recent = false,
...
export function receivedPostsBefore(
    posts: PostList,
    channelId: string,
    beforePostId: string,
    oldest = false,
...
export function receivedPostsInChannel(
    posts: PostList,
    channelId: string,
    recent = false,
    oldest = false,
...
export function createPost(post: Post, files: any[] = []) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        const pendingPostId =
            post.pending_post_id || `${currentUserId}:${timestamp}`;
        ...
        if (post.root_id) {
            newPost.reply_count =
                PostSelectors.getPostRepliesCount(state, post.root_id) + 1;
        }

        // We are retrying a pending post that had files
        if (newPost.file_ids && !files.length) {
            // eslint-disable-next-line no-param-reassign
            files = newPost.file_ids.map(
                (id) => state.entities.files.files[id],
            );
        }
        ...
        (async function createPostWrapper() {
            try {
                const created = await Client4.createPost({
                    ...newPost,
                    create_at: 0,
                });
                ...
                if (
                    ...
                    error.server_error_id ===
                        "api.post.create_post.town_square_read_only" ||
                    error.server_error_id ===
...
export function deletePost(post: ExtendedPost) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (
            delPost.type === Posts.POST_TYPES.COMBINED_USER_ACTIVITY &&
            delPost.system_post_ids
        ) {
...
function getUnreadPostData(unreadChan: ChannelUnread, state: GlobalState) {
    ...
    const delta = member
        ? member.msg_count - unreadChan.msg_count
        : unreadChan.msg_count;
    const deltaRoot = member
        ? member.msg_count_root - unreadChan.msg_count_root
        : unreadChan.msg_count_root;
...
export function addReaction(postId: string, emojiName: string) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        let reaction;
        try {
            reaction = await Client4.addReaction(
                currentUserId,
                postId,
                emojiName,
            );
...
export function getReactionsForPost(postId: string) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        if (reactions && reactions.length > 0) {
            const nonExistentEmoji =
                getState().entities.emojis.nonExistentEmoji;
            const customEmojisByName = selectCustomEmojisByName(getState());
...
async function getPaginatedPostThread(
    rootId: string,
    options: FetchPaginatedThreadOptions,
    prevList?: PostList,
): Promise<PostList> {
    ...
    const list: PostList = prevList
        ? JSON.parse(JSON.stringify(prevList))
        : {
              order: [rootId],
              posts: {},
              prev_post_id: "",
    ...
    if (result.first_inaccessible_post_time) {
        list.first_inaccessible_post_time = list.first_inaccessible_post_time
            ? Math.min(
                  result.first_inaccessible_post_time,
                  list.first_inaccessible_post_time,
              )
            : result.first_inaccessible_post_time;
    }
...
export function getPostsUnread(
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        const shouldLoadRecent =
            getUnreadScrollPositionPreference(getState()) ===
        ...
        let recentPosts;
        try {
            posts = await Client4.getPostsUnread(
                channelId,
                userId,
                DEFAULT_LIMIT_BEFORE,
                DEFAULT_LIMIT_AFTER,
            ...
            if (posts.next_post_id && shouldLoadRecent) {
                recentPosts = await Client4.getPosts(
                    channelId,
                    0,
        ...
        const recentPostsActions = recentPosts
            ? [
                  ...
                  receivedPostsInChannel(
                      recentPosts,
                      channelId,
                      recentPosts.next_post_id === "",
                      recentPosts.prev_post_id === "",
                  ),
              ]
            : [];
...
export function getPostsSince(
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        let posts;
        try {
            const collapsedThreadsEnabled = isCollapsedThreadsEnabled(
                getState(),
            );
            posts = await Client4.getPostsSince(
                channelId,
                since,
                fetchThreads,
                collapsedThreadsEnabled,
...
export function getPostsBefore(
    channelId: string,
    postId: string,
    page = 0,
    perPage = Posts.POST_CHUNK_SIZE,
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        try {
            ...
            posts = await Client4.getPostsBefore(
                channelId,
                postId,
                page,
                perPage,
...
export function getPostsAfter(
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        try {
            ...
            posts = await Client4.getPostsAfter(
                channelId,
                postId,
                page,
                perPage,
...
export function getPostsAround(
    channelId: string,
    postId: string,
    perPage = Posts.POST_CHUNK_SIZE / 2,
    fetchThreads = true,
    collapsedThreadsExtended = false,
) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        try {
            ...
            [after, thread, before] = await Promise.all([
                ...
                Client4.getPostThread(
                    postId,
                    fetchThreads,
                    collapsedThreadsEnabled,
                    collapsedThreadsExtended,
                ),
                Client4.getPostsBefore(
                    channelId,
                    postId,
                    0,
                    perPage,
        ...
        const posts: PostList = {
            ...
            order: [
                // Remember that the order is newest posts first
                ...after.order,
                postId,
                ...before.order,
            ],
            ...
            first_inaccessible_post_time:
                Math.max(
                    before.first_inaccessible_post_time,
                    after.first_inaccessible_post_time,
                    thread.first_inaccessible_post_time,
                ) || 0,
        };
...
export async function getMentionsAndStatusesForPosts(
    postsArrayOrMap: Post[] | PostList["posts"],
    ...
    const postsArray: Post[] = Array.isArray(postsArrayOrMap)
        ? postsArrayOrMap
        : Object.values(postsArrayOrMap);
            ...
            if (post.metadata.embeds) {
                post.metadata.embeds.forEach((embed: any) => {
                    if (embed.type === "permalink" && embed.data) {
                        if (
                            embed.data.post?.user_id &&
                            !profiles[embed.data.post.user_id] &&
                            embed.data.post.user_id !== currentUserId
                        ) {
                            userIdsToLoad.add(embed.data.post.user_id);
                        }
                        if (
                            embed.data.post?.user_id &&
                            !statuses[embed.data.post.user_id]
                        ) {
    ...
    if (userIdsToLoad.size > 0) {
        promises.push(
            getProfilesByIds(Array.from(userIdsToLoad))(dispatch, getState),
        );
    }

    if (statusesToLoad.size > 0) {
        promises.push(
            getStatusesByIds(Array.from(statusesToLoad))(dispatch, getState),
        );
    }

    // Profiles of users mentioned in the posts
    const usernamesAndGroupsToLoad = getNeededAtMentionedUsernamesAndGroups(
        state,
        postsArray,
    );
    ...
    if (usernamesAndGroupsToLoad.size > 0) {
        ...
        const getProfilesPromise = getProfilesByUsernames(
            Array.from(usernamesAndGroupsToLoad),
        )(dispatch, getState);
        ...
        const groupsToCheck = Array.from(usernamesAndGroupsToLoad).filter(
            (name) => !loadedProfiles.has(name),
        );
...
export function getNeededAtMentionedUsernamesAndGroups(
    state: GlobalState,
    posts: Post[],
): Set<string> {
...
export function unflagPost(postId: string) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        return deletePreferences(currentUserId, [preference])(
            dispatch,
            getState,
        );
    };
...
export function addPostReminder(
    userId: string,
    postId: string,
    timestamp: number,
) {
...
export function doPostActionWithCookie(
    postId: string,
    actionId: string,
    actionCookie: string,
    selectedOption = "",
    ...
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        let data;
        try {
            data = await Client4.doPostActionWithCookie(
                postId,
                actionId,
                actionCookie,
                selectedOption,
