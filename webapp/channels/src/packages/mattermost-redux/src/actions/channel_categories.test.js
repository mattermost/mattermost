...
describe("patchCategory", () => {
    ...
    test("should only modify the provided fields", async () => {
        ...
        nock(Client4.getBaseRoute())
            .put(
                `/users/${currentUserId}/teams/${teamId}/channels/categories/${category1.id}`,
            )
            .reply(200, { ...category1, display_name: "Category A" });

        await store.dispatch(
            Actions.patchCategory(category1.id, {
...
describe("setCategorySorting", () => {
    test("should call the correct API", async () => {
        ...
        await store.dispatch(
            Actions.setCategorySorting("category1", CategorySorting.Recency),
        );
...
describe("setCategoryMuted", () => {
    ...
    test("should mute the category and all of its channels", async () => {
        ...
        expect(state.entities.channelCategories.byId.category1.muted).toBe(
            true,
        );
        expect(
            state.entities.channels.myMembers.channel1.notify_props.mark_unread,
        ).toBe(MarkUnread.MENTION);
        expect(
            state.entities.channels.myMembers.channel2.notify_props.mark_unread,
        ).toBe(MarkUnread.MENTION);
    });
...
describe("fetchMyCategories", () => {
    test("should populate state correctly", async () => {
        ...
        nock(Client4.getBaseRoute())
            ...
            .reply(200, {
                categories,
                order: categories.map((category) => category.id),
            });

        await store.dispatch(Actions.fetchMyCategories(teamId));

        const state = store.getState();
        expect(state.entities.channelCategories.byId.category1).toEqual(
            categories[0],
        );
        expect(state.entities.channelCategories.byId.category2).toEqual(
            categories[1],
        );
        expect(state.entities.channelCategories.orderByTeam[teamId]).toEqual([
            "category1",
            "category2",
        ]);
    });
...
describe("addChannelToInitialCategory", () => {
    test("should add new DM channel to Direct Messages categories on all teams", () => {
        ...
        const categoriesById = getAllCategoriesByIds(store.getState());
        expect(categoriesById.dmCategory1.channel_ids).toEqual([
            "newDmChannel",
            "dmChannel1",
            "dmChannel2",
        ]);
        expect(categoriesById.dmCategory2.channel_ids).toEqual([
            "newDmChannel",
            "dmChannel1",
            "dmChannel2",
        ]);
        expect(categoriesById.channelsCategory1.channel_ids).not.toContain(
            "newDmChannel",
        );
    });
    ...
    test("should do nothing if categories have not been loaded yet for the given team", () => {
        ...
        const categoriesById = getAllCategoriesByIds(store.getState());
        expect(categoriesById.channelsCategory1.channel_ids).toEqual([
            "publicChannel1",
            "privateChannel1",
        ]);
    });
    ...
    test("should add new channel to Channels category", () => {
        ...
        expect(categoriesById.dmCategory1.channel_ids).not.toContain(
            "newChannel",
        );
        expect(categoriesById.channelsCategory2.channel_ids).not.toContain(
            "newChannel",
        );
    });
    ...
    test("should add the channel to the given category", async () => {
        ...
        nock(Client4.getBaseRoute())
            ...
            .reply(200, [
                {
                    ...category1,
                    channel_ids: ["channel5", "channel1", "channel2"],
                },
            ]);

        await store.dispatch(
            Actions.addChannelToCategory("category1", "channel5"),
        );

        const state = store.getState();

        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel5", "channel1", "channel2"]);
        ...
        expect(state.entities.channelCategories.byId.category1.sorting).toBe(
    ...
    test("should remove the channel from its previous category", async () => {
        ...
        nock(Client4.getBaseRoute())
            ...
            .reply(200, [
                {
                    ...category1,
                    channel_ids: ["channel3", "channel1", "channel2"],
                },
                { ...category2, channel_ids: ["channel4"] },
            ]);

        await store.dispatch(
            Actions.addChannelToCategory("category1", "channel3"),
        );
        ...
        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel3", "channel1", "channel2"]);
        expect(
            state.entities.channelCategories.byId.category2.channel_ids,
        ).toEqual(["channel4"]);
        ...
        expect(state.entities.channelCategories.byId.category2.sorting).toBe(
            CategorySorting.Default,
        );
    });
});
...
describe("moveChannelToCategory", () => {
    ...
    test("should add the channel to the given category at the correct index", async () => {
        ...
        const otherTeamCategory = {
            id: "otherTeamCategory",
            team_id: "team2",
            channel_ids: ["channel1", "channel2"],
        };
        ...
        nock(Client4.getBaseRoute())
            ...
            .reply(200, [
                {
                    ...category1,
                    channel_ids: ["channel1", "channel5", "channel2"],
                },
            ]);

        await store.dispatch(
            Actions.moveChannelToCategory("category1", "channel5", 1),
        );
        ...
        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel1", "channel5", "channel2"]);
        expect(state.entities.channelCategories.byId.category2).toBe(category2);
        expect(state.entities.channelCategories.byId.otherTeamCategory).toBe(
            otherTeamCategory,
        );
        ...
        await store.dispatch(
            Actions.moveChannelToCategory("category1", "channel6", 2),
        );
        ...
        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel1", "channel5", "channel6", "channel2"]);
        expect(state.entities.channelCategories.byId.category2).toBe(category2);
        expect(state.entities.channelCategories.byId.otherTeamCategory).toBe(
            otherTeamCategory,
        );
    ...
    test("should remove the channel from its previous category", async () => {
        ...
        nock(Client4.getBaseRoute())
            ...
            .reply(200, [
                ...
                {
                    ...category2,
                    channel_ids: ["channel3", "channel4", "channel1"],
                },
            ]);

        await store.dispatch(
            Actions.moveChannelToCategory("category2", "channel1", 2),
        );
        ...
        expect(
            state.entities.channelCategories.byId.category2.channel_ids,
        ).toEqual(["channel3", "channel4", "channel1"]);
        expect(state.entities.channelCategories.byId.otherTeamCategory).toBe(
            otherTeamCategory,
        );
    });
    ...
    test("should move channel within its current category", async () => {
        ...
        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel1", "channel5", "channel2", "channel3", "channel4"]);
        ...
        await store.dispatch(
            Actions.moveChannelToCategory("category1", "channel1", 3),
        );
        ...
        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel5", "channel2", "channel3", "channel1", "channel4"]);
    });
    ...
    test("moving a channel to the favorites category should also favorite the channel in preferences", async () => {
        ...
        await store.dispatch(
            Actions.moveChannelToCategory("favoritesCategory", "channel1", 0),
        );
        ...
        expect(
            state.entities.channelCategories.byId.otherCategory.channel_ids,
        ).toEqual([]);
        ...
        // And back out
        await store.dispatch(
            Actions.moveChannelToCategory("otherCategory", "channel1", 0),
        );
    ...
    describe("changes to sorting method", () => {
        test("if the category was sorted by default, should set the destination category to manual sorting", async () => {
            ...
            nock(Client4.getBaseRoute())
                .put(
                    `/users/${currentUserId}/teams/${teamId}/channels/categories`,
                    (body) => {
                        return (
                            body.find((category) => category.id === "category1")
                                .sorting !== CategorySorting.Manual &&
                            body.find((category) => category.id === "category2")
                                .sorting === CategorySorting.Manual
                        );
                    },
                )
                .reply(200, [
                    ...
                    {
                        ...category2,
                        channel_ids: ["channel1", "channel3", "channel4"],
                        sorting: CategorySorting.Manual,
                    },
                ]);

            await store.dispatch(
                Actions.moveChannelToCategory(category2.id, "channel1", 0),
            );

            let state = store.getState();
            expect(
                state.entities.channelCategories.byId.category1.sorting,
            ).toBe(CategorySorting.Default);
            expect(
                state.entities.channelCategories.byId.category2.sorting,
            ).toBe(CategorySorting.Manual);
            ...
            await store.dispatch(
                Actions.moveChannelToCategory(category1.id, "channel1", 2),
            );
        ...
        test("if the category was sorted automatically, should not change the sorting method", async () => {
            ...
            expect(
                state.entities.channelCategories.byId.category1.sorting,
            ).toBe(CategorySorting.Alphabetical);
            expect(
                state.entities.channelCategories.byId.category2.sorting,
            ).toBe(CategorySorting.Recency);
    ...
    test("should optimistically update the modified categories", async () => {
        ...
        const moveRequest = store.dispatch(
            Actions.moveChannelToCategory(favoritesCategory.id, "channel1", 0),
        );
...
describe("moveCategory", () => {
    ...
    test("should call the correct API", async () => {
        ...
        const mock = nock(Client4.getBaseRoute())
            .put(
                `/users/${currentUserId}/teams/team1/channels/categories/order`,
            )
...
describe("moveChannelsToCategory", () => {
    ...
    test("should add channels to the given category at the correct index", async () => {
        ...
        await store.dispatch(
            Actions.moveChannelsToCategory(
                "category1",
                ["channel5", "channel6"],
                1,
            ),
    ...
    test("should remove the channels from their previous category", async () => {
        ...
        const category2 = {
            id: "category2",
            team_id: teamId,
            channel_ids: ["channel4", "channel5", "channel6"],
        };
        const otherTeamCategory = {
            id: "otherTeamCategory",
            team_id: "team2",
        ...
        expect(
            state.entities.channelCategories.byId.category2.channel_ids,
        ).toEqual(["channel4", "channel5", "channel6", "channel1", "channel3"]);
        expect(state.entities.channelCategories.byId.otherTeamCategory).toBe(
            otherTeamCategory,
        );
    });
    ...
    test("should move channel within its current category", async () => {
        ...
        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel1", "channel5", "channel3", "channel2", "channel4"]);
        ...
        expect(
            state.entities.channelCategories.byId.category1.channel_ids,
        ).toEqual(["channel5", "channel3", "channel2", "channel1", "channel4"]);
    });
    ...
    describe("changes to sorting method", () => {
        test("if the category was sorted by default, should set the destination category to manual sorting", async () => {
            ...
            await store.dispatch(
                Actions.moveChannelsToCategory(category2.id, ["channel1"], 0),
            );
            ...
            await store.dispatch(
                Actions.moveChannelsToCategory(category1.id, ["channel1"], 2),
            );
...
describe("renameCategory", () => {
    ...
    test("should call the correct API", async () => {
        ...
        const mock = nock(Client4.getBaseRoute())
            .put(
                `/users/${currentUserId}/teams/${teamId}/channels/categories/${category.id}`,
            )
...
describe("deleteCategory", () => {
    ...
    test("should call the correct API", async () => {
        ...
        const mock = nock(Client4.getBaseRoute())
            .delete(
                `/users/${currentUserId}/teams/${teamId}/channels/categories/category3`,
            )
