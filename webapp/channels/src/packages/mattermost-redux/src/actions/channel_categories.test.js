...
describe('addChannelToDefaultCategory', () => {
    ...
    test('should add channel to default category when category exists', async () => {
        const channel = {
            id: 'channel1',
            team_id: teamId,
            default_category_name: 'Test Category',
        };
        ...
        const result = await store.dispatch(Actions.addChannelToDefaultCategory(channel));
        expect(result.data).toBe(true);

        const state = store.getState();
        expect(state.entities.channelCategories.byId.defaultCategory.channel_ids).toEqual([channel.id]);
        expect(mock.isDone());
    });

    test('should create new category if default category does not exist', async () => {
        const channel = {
            id: 'channel1',
            team_id: teamId,
        ...
        const mock = nock(Client4.getBaseRoute()).
            ...
            reply(200, {
                id: 'newCategory',
                team_id: teamId,
                display_name: channel.default_category_name,
                channel_ids: [channel.id],
            });

        await store.dispatch(Actions.addChannelToDefaultCategory(channel));

        const state = store.getState();
        expect(state.entities.channelCategories.byId.newCategory.channel_ids).toEqual([channel.id]);
        expect(mock.isDone());
    });
});
