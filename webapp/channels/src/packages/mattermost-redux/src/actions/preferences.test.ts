...
describe("Actions.Preferences", () => {
    ...
    it("makeDirectChannelVisibleIfNecessary", async () => {
        ...
        const user2 = await TestHelper.createClient4().createUser(
            TestHelper.fakeUser(),
            "",
            "",
        );
        ...
        nock(Client4.getUsersRoute())
            .put(`/${TestHelper.basicUser!.id}/preferences`)
            .reply(200, OK_RESPONSE);
        await Actions.makeDirectChannelVisibleIfNecessary(user2.id)(
            store.dispatch,
            store.getState,
        );
        ...
        let preference =
            myPreferences[
                `${Preferences.CATEGORY_DIRECT_CHANNEL_SHOW}--${user2.id}`
            ];
    ...
    it("saveTheme", async () => {
        ...
        nock(Client4.getUsersRoute())
            .put(`/${TestHelper.basicUser!.id}/preferences`)
            .reply(200, OK_RESPONSE);
        await Actions.saveTheme(team.id, newTheme)(
            store.dispatch,
            store.getState,
        );
        ...
        expect(myPreferences[`theme--${team.id}`].value).toEqual(
    ...
    it("deleteTeamSpecificThemes", async () => {
        ...
        nock(Client4.getUsersRoute())
            ...
            .reply(200, OK_RESPONSE);
        await Actions.deleteTeamSpecificThemes()(
            store.dispatch,
            store.getState,
        );
