...
describe("Actions.Schemes", () => {
    ...
    it("createScheme", async () => {
        ...
        nock(Client4.getBaseRoute()).post("/schemes").reply(201, mockScheme!);
        await Actions.createScheme(TestHelper.mockScheme())(
            store.dispatch,
            store.getState,
        );
    ...
    it("getScheme", async () => {
        ...
        await Actions.getScheme(TestHelper.basicScheme!.id)(
            store.dispatch,
            store.getState,
        );
        ...
        expect(schemes[TestHelper.basicScheme!.id].name).toEqual(
            TestHelper.basicScheme!.name,
        );
    });

    it("patchScheme", async () => {
        const patchData = {
            name: "The Updated Scheme",
            description: "This is a scheme created by unit tests",
        };
        const scheme = {
        ...
        await Actions.patchScheme(TestHelper.basicScheme!.id, scheme)(
            store.dispatch,
            store.getState,
        );
    ...
    it("deleteScheme", async () => {
        ...
        await Actions.deleteScheme(TestHelper.basicScheme!.id)(
            store.dispatch,
            store.getState,
        );
