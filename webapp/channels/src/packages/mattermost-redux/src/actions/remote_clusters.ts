...
import type {RemoteCluster, RemoteClusterInvite} from '@mattermost/types/remote_clusters';

import {RemoteClusterTypes} from 'mattermost-redux/action_types';
...
export function deleteRemoteCluster(channelId: string, id: string, connectionId: string) {
    return async (dispatch: DispatchFunc, getState: GetStateFunc) => {
        const state = getState();
        const bookmark = getChannelBookmark(state, channelId, id);

        try {
            await Client4.getRemoteCluster(channelId, id, connectionId);
