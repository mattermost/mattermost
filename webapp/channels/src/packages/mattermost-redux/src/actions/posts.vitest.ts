...
vi.mock('mattermost-redux/actions/status_profile_polling', () => ({
    batchFetchStatusesProfilesGroupsFromPosts: vi.fn(() => {
        return {type: ''};
    }),
}));
...
describe('Actions.Posts', () => {
    ...
    test('resetCreatePostRequest', async () => {
        const channelId = TestHelper.basicChannel!.id;
        const post = TestHelper.fakePost(channelId);
    ...
    test('createPost with file attachments', async () => {
        const channelId = TestHelper.basicChannel!.id;
        const post = TestHelper.fakePost(channelId);
    ...
    test('deletePostWithReaction', async () => {
    ...
    test('removePostWithReaction', async () => {
        TestHelper.mockLogin();
        store.dispatch({
            type: UserTypes.LOGIN_SUCCESS,
        });
    ...
    test('getPostsUnread should load recent posts when unreadScrollPosition is startFromNewest and unread posts are not the latestPosts', async () => {
    ...
    test('getPostsAfter with empty next_post_id', async () => {
        const channelId = 'channel1';
    ...
    test('getCustomEmojiForReaction', async () => {
    ...
    test('doPostActionWithCookie with trigger_id', async () => {
    ...
    test('addMessageIntoHistory', async () => {
        const {dispatch, getState} = store;
    ...
    test('moveHistoryIndexBack', async () => {
        const {dispatch, getState} = store;
    ...
    test('moveHistoryIndexForward', async () => {
        const {dispatch, getState} = store;
    ...
    describe('receivedPostsBefore', () => {
        test('Should return default false for oldest key if param does not exist', () => {
        ...
        test('Should return true for oldest key', () => {
            const posts = {} as PostList;
    ...
    describe('receivedPostsInChannel', () => {
        test('Should return default false for both recent and oldest keys if params dont exist', () => {
            const posts = {} as PostList;
        ...
        test('Should return true for oldest and recent keys', () => {
            const posts = {} as PostList;
...
describe('getPostThreads', () => {
    ...
    test('pulls up the thread of missing root post in the same channel', async () => {
        const testStore = await mockStore(initialState);
        nock(Client4.getBaseRoute()).
