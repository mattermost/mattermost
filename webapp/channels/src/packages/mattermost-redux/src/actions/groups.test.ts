...
describe("Actions.Groups", () => {
    ...
    it("getGroupSyncables", async () => {
        ...
        await Actions.getGroupSyncables(groupID, SyncableType.Team)(
            store.dispatch,
            store.getState,
        );
        await Actions.getGroupSyncables(groupID, SyncableType.Channel)(
            store.dispatch,
            store.getState,
        );
        ...
        for (let i = 0; i < 2; i++) {
            expect(
                JSON.stringify(groupSyncables.teams[i]) ===
                    JSON.stringify(groupTeams[i]),
            ).toBeTruthy();
            expect(
                JSON.stringify(groupSyncables.channels[i]) ===
                    JSON.stringify(groupChannels[i]),
            ).toBeTruthy();
        }
    });
    ...
    it("getGroup", async () => {
        ...
        const response = {
            ...
            type: "ldap",
            remote_id:
                "\\eb\\80\\94\\cd\\d4\\32\\7c\\45\\87\\79\\1b\\fe\\45\\d9\\ac\\7b",
        ...
        expect(
            JSON.stringify(response) === JSON.stringify(groups[groupID]),
        ).toBeTruthy();
    });
    ...
    it("linkGroupSyncable", async () => {
        ...
        await (Actions.linkGroupSyncable as any)(
            groupID,
            teamID,
            SyncableType.Team,
        ...
        expect(
            JSON.stringify(syncables[groupID].teams[0]) ===
                JSON.stringify(groupTeamResponse),
        ).toBeTruthy();
        expect(
            JSON.stringify(syncables[groupID].channels[0]) ===
                JSON.stringify(groupChannelResponse),
        ).toBeTruthy();
    });
    ...
    it("unlinkGroupSyncable", async () => {
        ...
        await Actions.unlinkGroupSyncable(
            groupID,
            teamID,
            SyncableType.Team,
        )(store.dispatch, store.getState);
        ...
        expect(syncables[groupID]).toBeTruthy();
        expect(
            syncables[groupID].teams.length === beforeTeamsLength - 1,
        ).toBeTruthy();
        expect(
            syncables[groupID].channels.length === beforeChannelsLength - 1,
        ).toBeTruthy();
    });
        ...
        for (const id of Object.keys(groups)) {
            ...
            expect(
                JSON.stringify(groups[id]) ===
                    JSON.stringify(response1.groups[index]),
            ).toBeTruthy();
        }
    });
    ...
    it("getAllGroupsAssociatedToTeam", async () => {
        ...
        nock(Client4.getBaseRoute())
            .get(
                `/teams/${teamID}/groups?paginate=false&filter_allow_reference=false&include_member_count=true`,
            )
            .reply(200, response);

        await Actions.getAllGroupsAssociatedToTeam(
            teamID,
            false,
            true,
        )(store.dispatch, store.getState);
        ...
        groupIDs.forEach((id: string) => {
            expect(
                response.groups.map((group) => group.id).includes(id),
            ).toBeTruthy();
        });
    });
    ...
    it("getGroupsAssociatedToTeam", async () => {
        ...
        nock(Client4.getBaseRoute())
            .get(
                `/teams/${teamID}/groups?page=100&per_page=60&q=0&include_member_count=true&filter_allow_reference=false`,
            )
            .reply(200, response);

        await Actions.getGroupsAssociatedToTeam(
            teamID,
            "0",
            100,
        )(store.dispatch, store.getState);
        ...
        const count =
            state.entities.teams.groupsAssociatedToTeam[teamID].totalCount;
    ...
    it("getGroupsNotAssociatedToTeam", async () => {
        ...
        nock(Client4.getBaseRoute())
            .get(
                `/groups?not_associated_to_team=${teamID}&page=100&per_page=60&q=0&include_member_count=true`,
            )
            .reply(200, response);

        await Actions.getGroupsNotAssociatedToTeam(
            teamID,
            "0",
            100,
        )(store.dispatch, store.getState);
        ...
        const groupIDs =
            state.entities.teams.groupsAssociatedToTeam[teamID].ids;
        const expectedIDs = ["existing2"].concat(
    ...
    it("getAllGroupsAssociatedToChannel", async () => {
        ...
        nock(Client4.getBaseRoute())
            .get(
                `/channels/${channelID}/groups?paginate=false&filter_allow_reference=false&include_member_count=true`,
            )
            .reply(200, response);

        await Actions.getAllGroupsAssociatedToChannel(
            channelID,
            false,
            true,
        )(store.dispatch, store.getState);
    ...
    it("getAllGroupsAssociatedToChannelsInTeam", async () => {
        ...
        nock(Client4.getBaseRoute())
            .get(
                `/teams/${teamID}/groups_by_channels?paginate=false&filter_allow_reference=false`,
            )
            .reply(200, response1);

        nock(Client4.getBaseRoute())
            .get(
                `/teams/${teamID}/groups_by_channels?paginate=false&filter_allow_reference=true`,
            )
            .reply(200, response2);

        await Actions.getAllGroupsAssociatedToChannelsInTeam(teamID, false)(
            store.dispatch,
            store.getState,
        );
        ...
        let groupIDs =
            state.entities.channels.groupsAssociatedToChannel[channelID1].ids;
        expect(groupIDs.length).toEqual(response1.groups[channelID1].length);
        groupIDs.forEach((id: string) => {
            expect(
                response1.groups[channelID1]
                    .map((group) => group.id)
                    .includes(id),
            ).toBeTruthy();
        });

        await Actions.getAllGroupsAssociatedToChannelsInTeam(teamID, true)(
            store.dispatch,
            store.getState,
        );
    ...
    it("getGroupsAssociatedToChannel", async () => {
        ...
        await Actions.getGroupsAssociatedToChannel(
            channelID,
            "0",
            100,
        )(store.dispatch, store.getState);
        ...
        const count =
            state.entities.channels.groupsAssociatedToChannel[channelID]
                .totalCount;
        expect(count).toEqual(response.total_group_count);
    });
    ...
    it("getGroupsNotAssociatedToChannel", async () => {
        ...
        nock(Client4.getBaseRoute())
            .get(
                `/groups?not_associated_to_channel=${channelID}&page=100&per_page=60&q=0&include_member_count=true`,
            )
            .reply(200, response);

        await Actions.getGroupsNotAssociatedToChannel(
            channelID,
            "0",
            100,
        )(store.dispatch, store.getState);
    ...
    it("patchGroupSyncable", async () => {
        ...
        expect(
            groupSyncables.teams[0].auto_add === groupSyncablePatch.auto_add,
        ).toBeTruthy();
        expect(
            groupSyncables.channels[0].auto_add === groupSyncablePatch.auto_add,
        ).toBeTruthy();

        expect(
            groupSyncables.teams[0].scheme_admin ===
                groupSyncablePatch.scheme_admin,
        ).toBeTruthy();
        expect(
            groupSyncables.channels[0].scheme_admin ===
                groupSyncablePatch.scheme_admin,
        ).toBeTruthy();
    });
    ...
    it("patchGroup", async () => {
        ...
        await Actions.patchGroup(groupID, groupPatch)(
            store.dispatch,
            store.getState,
        );
        ...
        expect(groups[groupID]).toBeTruthy();
        expect(
            groups[groupID].allow_reference === groupPatch.allow_reference,
        ).toBeTruthy();
        expect(
            JSON.stringify(response) === JSON.stringify(groups[groupID]),
        ).toBeTruthy();
    ...
    it("getGroupStats", async () => {
        ...
        expect(
            JSON.stringify(response) === JSON.stringify(stats[groupID]),
        ).toBeTruthy();
    });
});
