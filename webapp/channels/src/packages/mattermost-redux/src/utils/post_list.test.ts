...
describe("makeFilterPostsAndAddSeparators", () => {
    it("filter join/leave posts", () => {
        ...
        let now = filterPostsAndAddSeparators(state, {
            postIds,
            lastViewedAt,
            indicateNewMessages,
        });
        expect(now).toEqual(["1002", "1001", "date-" + today.getTime()]);
        ...
        now = filterPostsAndAddSeparators(state, {
            postIds,
            lastViewedAt,
            indicateNewMessages,
        });
        ...
        expect(now).toEqual(["1001", "date-" + today.getTime()]);
            ...
            entities: {
                ...
                posts: {
                    ...
                    posts: {
                        ...state.entities.posts.posts,
                        1006: {
                            ...state.entities.posts.posts["1006"],
                            message: "abcd",
                        },
                    },
                },
...
describe("makeCombineUserActivityPosts", () => {
    ...
    test("should combine adjacent user activity posts", () => {
        ...
        const postIds = deepFreeze(["post1", "post2", "post3"]);
        const state = {
            entities: {
                posts: {
                    posts: {
                        ...
                        post2: {
                            id: "post2",
                            type: Posts.POST_TYPES.LEAVE_CHANNEL,
                        },
                        post3: {
                            id: "post3",
        ...
        expect(result).toEqual([COMBINED_USER_ACTIVITY + "post1_post2_post3"]);
    });
    ...
    describe("memoization", () => {
        ...
        test("should not recalculate if an unrelated post changes", () => {
            ...
            let state = initialState;
            const initialResult = combineUserActivityPosts(
                state,
                initialPostIds,
            );
...
describe("isCombinedUserActivityPost", () => {
    test("should correctly identify combined user activity posts", () => {
        ...
        expect(isCombinedUserActivityPost("user-activity-post1_post2")).toBe(
            true,
        );
        expect(
            isCombinedUserActivityPost("user-activity-post1_post2_post4"),
        ).toBe(true);
    });
});
...
describe("getFirstPostId", () => {
    ...
    test("should return the first ID from a combined post", () => {
        expect(
            getFirstPostId([
                "user-activity-post2_post3",
                "post4",
                "user-activity-post5_post6",
            ]),
        ).toBe("post2");
    });
    ...
    test("should skip the new message line", () => {
        expect(
            getFirstPostId([START_OF_NEW_MESSAGES, "post2", "post3", "post4"]),
        ).toBe("post2");
    });
});
...
describe("getLastPostId", () => {
    ...
    test("should skip the new message line", () => {
        expect(
            getLastPostId(["post2", "post3", "post4", START_OF_NEW_MESSAGES]),
        ).toBe("post4");
    });
});
...
describe("extractUserActivityData", () => {
    ...
    it("should return empty activity when empty ", () => {
        expect(extractUserActivityData([])).toEqual({
            allUserIds: [],
            allUsernames: [],
            messageData: [],
        });
    ...
    it("should match return for JOIN_CHANNEL", () => {
        ...
        expect(extractUserActivityData([postJoinChannel2])).toEqual(
            expectedOutput2,
        );
    });
    ...
    it("should return expected data for ADD_TO_CHANNEL", () => {
        ...
        expect(extractUserActivityData(userActivities2)).toEqual(
            expectedOutput2,
        );
    });
...
describe("combineUserActivityData", () => {
    ...
    it("correctly combine different post types and actorIds by order", () => {
        const postAddToChannel1 = TestHelper.getPostMock({
            type: PostTypes.ADD_TO_CHANNEL,
            user_id: "user_id_1",
            props: {
                addedUserId: "added_user_id_1",
                addedUsername: "added_username_1",
            },
        });
        const postAddToTeam1 = TestHelper.getPostMock({
            type: PostTypes.ADD_TO_TEAM,
            user_id: "user_id_1",
            props: { addedUserId: "added_user_id_1" },
        });
        const postJoinChannel1 = TestHelper.getPostMock({
            type: PostTypes.JOIN_CHANNEL,
            user_id: "user_id_1",
        });
        const postJoinTeam1 = TestHelper.getPostMock({
            type: PostTypes.JOIN_TEAM,
            user_id: "user_id_1",
        });
        const postLeaveChannel1 = TestHelper.getPostMock({
            type: PostTypes.LEAVE_CHANNEL,
            user_id: "user_id_1",
        });
        const postLeaveTeam1 = TestHelper.getPostMock({
            type: PostTypes.LEAVE_TEAM,
            user_id: "user_id_1",
        });
        const postRemoveFromChannel1 = TestHelper.getPostMock({
            type: PostTypes.REMOVE_FROM_CHANNEL,
            user_id: "user_id_1",
            props: {
                removedUserId: "removed_user_id_1",
                removedUsername: "removed_username_1",
            },
        });
        const postRemoveFromTeam1 = TestHelper.getPostMock({
    ...
    it("correctly combine same post types", () => {
        ...
        const postAddToChannel2 = TestHelper.getPostMock({
            type: PostTypes.ADD_TO_CHANNEL,
            user_id: "user_id_2",
            props: {
                addedUserId: "added_user_id_2",
                addedUsername: "added_username_2",
            },
        });
        const postAddToChannel3 = TestHelper.getPostMock({
            type: PostTypes.ADD_TO_CHANNEL,
            user_id: "user_id_3",
            props: {
                addedUserId: "added_user_id_3",
                addedUsername: "added_username_3",
            },
        });
    ...
    it("correctly combine Join and Leave Posts", () => {
        ...
        const postJoinChannel2 = TestHelper.getPostMock({
            type: PostTypes.JOIN_CHANNEL,
            user_id: "user_id_2",
        });
        const postLeaveChannel2 = TestHelper.getPostMock({
            type: PostTypes.LEAVE_CHANNEL,
            user_id: "user_id_2",
        });
        const postJoinChannel3 = TestHelper.getPostMock({
            type: PostTypes.JOIN_CHANNEL,
            user_id: "user_id_3",
        });
        const postLeaveChannel3 = TestHelper.getPostMock({
            type: PostTypes.LEAVE_CHANNEL,
            user_id: "user_id_3",
        });
        ...
        const expectedOutput = {
            ...
            messageData: [
                {
                    postType: PostTypes.JOIN_LEAVE_CHANNEL,
                    userIds: ["user_id_1"],
                },
            ],
        };
    ...
    it("should not combine join and leave posts with other actions in between", () => {
        ...
        const postAddToTeam2 = TestHelper.getPostMock({
            type: PostTypes.ADD_TO_TEAM,
            user_id: "user_id_2",
            props: { addedUserId: "added_user_id_1" },
        });
        const postJoinTeam2 = TestHelper.getPostMock({
            type: PostTypes.JOIN_TEAM,
            user_id: "user_id_2",
        });
        const postLeaveTeam2 = TestHelper.getPostMock({
            type: PostTypes.LEAVE_TEAM,
            user_id: "user_id_2",
        });
        const postRemoveFromChannel2 = TestHelper.getPostMock({
            type: PostTypes.REMOVE_FROM_CHANNEL,
            user_id: "user_id_2",
            props: {
                removedUserId: "removed_user_id_1",
        ...
        const postRemoveFromTeam2 = TestHelper.getPostMock({
            type: PostTypes.REMOVE_FROM_TEAM,
            user_id: "removed_user_id_1",
        });
