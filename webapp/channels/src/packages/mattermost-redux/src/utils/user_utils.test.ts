...
describe("user utils", () => {
    describe("displayUsername", () => {
        ...
        it("should return username", () => {
            expect(displayUsername(userObj, "UNKNOWN_PREFERENCE")).toBe(
                "testUser",
            );
        });
        ...
        it("should return nickname", () => {
            expect(
                displayUsername(userObj, Preferences.DISPLAY_PREFER_NICKNAME),
            ).toBe("nick");
        });
        ...
        it("should return fullname", () => {
            expect(
                displayUsername(userObj, Preferences.DISPLAY_PREFER_FULL_NAME),
            ).toBe("test user");
        });
        ...
        it("should return default username string", () => {
            let noUserObj;
            expect(displayUsername(noUserObj, "UNKNOWN_PREFERENCE")).toBe(
                "Someone",
            );
        });
        ...
        it("should return empty string when user does not exist and useDefaultUserName param is false", () => {
            let noUserObj;
            expect(
                displayUsername(noUserObj, "UNKNOWN_PREFERENCE", false),
            ).toBe("");
        });
    });
    ...
    describe("filterProfilesStartingWithTerm", () => {
        ...
        it("should match all for empty filter", () => {
            expect(filterProfilesStartingWithTerm(users, "")).toEqual([
                userA,
                userB,
            ]);
        });
        ...
        it("should match by username", () => {
            expect(filterProfilesStartingWithTerm(users, "testUser")).toEqual([
                userA,
            ]);
        });

        it("should match by split part of the username", () => {
            expect(filterProfilesStartingWithTerm(users, "split")).toEqual([
                userA,
                userB,
            ]);
            expect(filterProfilesStartingWithTerm(users, "10")).toEqual([
                userA,
            ]);
        });

        it("should match by firstname", () => {
            expect(filterProfilesStartingWithTerm(users, "First")).toEqual([
                userA,
                userB,
            ]);
        });
        ...
        it("should match by lastname prefix", () => {
            expect(filterProfilesStartingWithTerm(users, "Last")).toEqual([
                userA,
                userB,
            ]);
        });
        ...
        it("should match by lastname fully", () => {
            expect(filterProfilesStartingWithTerm(users, "Last2")).toEqual([
                userB,
            ]);
        });

        it("should match by fullname prefix", () => {
            expect(filterProfilesStartingWithTerm(users, "First Last")).toEqual(
                [userA, userB],
            );
        });
        ...
        it("should match by fullname fully", () => {
            expect(
                filterProfilesStartingWithTerm(users, "First Last1"),
            ).toEqual([userA]);
        });
        ...
        it("should match by nickname", () => {
            expect(filterProfilesStartingWithTerm(users, "some")).toEqual([
                userB,
            ]);
        });
        ...
        it("should match by email prefix", () => {
            expect(filterProfilesStartingWithTerm(users, "left")).toEqual([
                userB,
            ]);
        });

        it("should match by email domain", () => {
            expect(filterProfilesStartingWithTerm(users, "right")).toEqual([
                userB,
            ]);
        });
        ...
        it("should match by full email", () => {
            expect(
                filterProfilesStartingWithTerm(users, "left@right.com"),
            ).toEqual([userB]);
        });
    ...
    describe("filterProfilesMatchingWithTerm", () => {
        ...
        it("should match all for empty filter", () => {
            expect(filterProfilesMatchingWithTerm(users, "")).toEqual([
                userA,
                userB,
            ]);
        });
        ...
        it("should match by username", () => {
            expect(filterProfilesMatchingWithTerm(users, "estUser")).toEqual([
                userA,
            ]);
        });

        it("should match by split part of the username", () => {
            expect(filterProfilesMatchingWithTerm(users, "split")).toEqual([
                userA,
                userB,
            ]);
            expect(filterProfilesMatchingWithTerm(users, "10")).toEqual([
                userA,
            ]);
        });

        it("should match by firstname substring", () => {
            expect(filterProfilesMatchingWithTerm(users, "rst")).toEqual([
                userA,
                userB,
            ]);
        });
        ...
        it("should match by lastname substring", () => {
            expect(filterProfilesMatchingWithTerm(users, "as")).toEqual([
                userA,
                userB,
            ]);
            expect(filterProfilesMatchingWithTerm(users, "st2")).toEqual([
                userB,
            ]);
        });

        it("should match by fullname substring", () => {
            expect(filterProfilesMatchingWithTerm(users, "rst Last")).toEqual([
                userA,
                userB,
            ]);
        });
        ...
        it("should match by fullname fully", () => {
            expect(
                filterProfilesMatchingWithTerm(users, "First Last1"),
            ).toEqual([userA]);
        });

        it("should match by fullname case-insensitive", () => {
            expect(filterProfilesMatchingWithTerm(users, "first LAST")).toEqual(
                [userA, userB],
            );
        });

        it("should match by nickname substring", () => {
            expect(filterProfilesMatchingWithTerm(users, "ome")).toEqual([
                userB,
            ]);
            expect(filterProfilesMatchingWithTerm(users, "body")).toEqual([
                userB,
            ]);
        });

        it("should match by email prefix", () => {
            expect(filterProfilesMatchingWithTerm(users, "left")).toEqual([
                userB,
            ]);
        });

        it("should match by email domain", () => {
            expect(filterProfilesMatchingWithTerm(users, "right")).toEqual([
                userB,
            ]);
        });
        ...
        it("should match by full email", () => {
            expect(
                filterProfilesMatchingWithTerm(users, "left@right.com"),
            ).toEqual([userB]);
        });

        it("should ignore leading @ for username", () => {
            expect(filterProfilesMatchingWithTerm(users, "@testUser")).toEqual([
                userA,
            ]);
        });

        it("should ignore leading @ for firstname", () => {
            expect(filterProfilesMatchingWithTerm(users, "@first")).toEqual([
                userA,
                userB,
            ]);
        });
    ...
    describe("Utils.getSuggestionsSplitBy", () => {
        test("correct suggestions when splitting by a character", () => {
            ...
            expect(getSuggestionsSplitBy(term, ".")).toEqual(
                expectedSuggestions,
            );
        });
    });
    ...
    describe("Utils.getSuggestionsSplitByMultiple", () => {
        test("correct suggestions when splitting by multiple characters", () => {
            ...
            expect(getSuggestionsSplitByMultiple(term, [".", "-"])).toEqual(
                expectedSuggestions,
            );
        });
    });
    ...
    describe("Utils.applyRolesFilters", () => {
        ...
        const guestUser = {
            ...TestHelper.fakeUserWithId(),
            roles: `${General.SYSTEM_GUEST_ROLE}`,
        };
