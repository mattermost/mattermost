...
describe("EmojiUtils", () => {
    describe("parseEmojiNamesFromText", () => {
        test("should return empty array for text without emojis", () => {
            const actual =
                EmojiUtils.parseEmojiNamesFromText("This has no emojis");
    ...
    describe("getEmojiImageUrl", () => {
        test("returns correct url for system emojis", () => {
            expect(
                EmojiUtils.getEmojiImageUrl(
                    TestHelper.getSystemEmojiMock({ unified: "system_emoji" }),
                ),
            ).toBe("/static/emoji/system_emoji.png");

            expect(
                EmojiUtils.getEmojiImageUrl(
                    TestHelper.getSystemEmojiMock({
                        short_names: ["system_emoji_short_names"],
                    }),
                ),
            ).toBe("/static/emoji/system_emoji_short_names.png");
        });
        ...
        test("return correct url for mattermost emoji", () => {
            ...
            ).toBe("/static/emoji/mattermost.png");

            expect(
                EmojiUtils.getEmojiImageUrl(
                    TestHelper.getCustomEmojiMock({ id: "mattermost" }),
                ),
            ).toBe("/static/emoji/mattermost.png");
        });
        ...
        test("return correct url for custom emojis", () => {
            ...
            ).toBe("/api/v4/emoji/custom_emoji/image");

            expect(
                EmojiUtils.getEmojiImageUrl(
                    TestHelper.getCustomEmojiMock({ id: "custom_emoji" }),
                ),
            ).toBe("/api/v4/emoji/custom_emoji/image");
        });
    });
