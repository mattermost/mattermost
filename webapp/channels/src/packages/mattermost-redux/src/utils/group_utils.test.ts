...
describe("group utils", () => {
    describe("filterGroupsMatchingTerm", () => {
        ...
        it("should match all for empty filter", () => {
            expect(filterGroupsMatchingTerm(groups, "")).toEqual([
                groupA,
                groupB,
                groupC,
            ]);
        ...
        it("should match by name", () => {
            expect(
                filterGroupsMatchingTerm(groups, "software-engineers"),
            ).toEqual([groupC]);
        });

        it("should match by split part of the name", () => {
            expect(filterGroupsMatchingTerm(groups, "group")).toEqual([
                groupA,
                groupB,
            ]);
        ...
        it("should ignore leading @ for name", () => {
            expect(filterGroupsMatchingTerm(groups, "@developers")).toEqual([
                groupB,
            ]);
        });

        it("should ignore leading @ for display_name", () => {
            expect(filterGroupsMatchingTerm(groups, "@software")).toEqual([
                groupC,
            ]);
        });
    });
