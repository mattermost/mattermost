...
describe("ChannelUtils", () => {
    it("areChannelMentionsIgnored", () => {
        const currentUserNotifyProps1 = TestHelper.fakeUserNotifyProps({
            channel: "true",
        });
        const channelMemberNotifyProps1 = TestHelper.fakeChannelNotifyProps({
            ignore_channel_mentions:
                Users.IGNORE_CHANNEL_MENTIONS_DEFAULT as "default",
        });
        expect(
            areChannelMentionsIgnored(
                channelMemberNotifyProps1,
                currentUserNotifyProps1,
            ),
        ).toBe(false);

        const currentUserNotifyProps2 = TestHelper.fakeUserNotifyProps({
            channel: "true",
        });
        const channelMemberNotifyProps2 = TestHelper.fakeChannelNotifyProps({
            ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_OFF as "off",
        });
        expect(
            areChannelMentionsIgnored(
                channelMemberNotifyProps2,
                currentUserNotifyProps2,
            ),
        ).toBe(false);

        const currentUserNotifyProps3 = TestHelper.fakeUserNotifyProps({
            channel: "true",
        });
        const channelMemberNotifyProps3 = TestHelper.fakeChannelNotifyProps({
            ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_ON as "on",
        });
        expect(
            areChannelMentionsIgnored(
                channelMemberNotifyProps3,
                currentUserNotifyProps3,
            ),
        ).toBe(true);

        const currentUserNotifyProps4 = TestHelper.fakeUserNotifyProps({
            channel: "false",
        });
        const channelMemberNotifyProps4 = TestHelper.fakeChannelNotifyProps({
            ignore_channel_mentions:
                Users.IGNORE_CHANNEL_MENTIONS_DEFAULT as "default",
        });
        expect(
            areChannelMentionsIgnored(
                channelMemberNotifyProps4,
                currentUserNotifyProps4,
            ),
        ).toBe(true);

        const currentUserNotifyProps5 = TestHelper.fakeUserNotifyProps({
            channel: "false",
        });
        const channelMemberNotifyProps5 = TestHelper.fakeChannelNotifyProps({
            ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_OFF as "off",
        });
        expect(
            areChannelMentionsIgnored(
                channelMemberNotifyProps5,
                currentUserNotifyProps5,
            ),
        ).toBe(false);

        const currentUserNotifyProps6 = TestHelper.fakeUserNotifyProps({
            channel: "false",
        });
        const channelMemberNotifyProps6 = TestHelper.fakeChannelNotifyProps({
            ignore_channel_mentions: Users.IGNORE_CHANNEL_MENTIONS_ON as "on",
        });
        expect(
            areChannelMentionsIgnored(
        ...
        const currentUserNotifyProps7 = TestHelper.fakeUserNotifyProps({
            channel: "true",
        });
        const channelMemberNotifyProps7 = null as unknown as ChannelNotifyProps;
        expect(
            areChannelMentionsIgnored(
                channelMemberNotifyProps7,
                currentUserNotifyProps7,
            ),
        ).toBe(false);

        const currentUserNotifyProps8 = TestHelper.fakeUserNotifyProps({
            channel: false as unknown as "false",
        });
        const channelMemberNotifyProps8 = null as unknown as ChannelNotifyProps;
        expect(
            areChannelMentionsIgnored(
                channelMemberNotifyProps8,
                currentUserNotifyProps8,
            ),
        ).toBe(false);
    });
    ...
    it("filterChannelsMatchingTerm", () => {
        ...
        expect(filterChannelsMatchingTerm(channels, "CHAN")).toEqual(channels);
        expect(filterChannelsMatchingTerm(channels, "blargh")).toEqual(
            channels,
        );
        expect(filterChannelsMatchingTerm(channels, "channel1")).toEqual([
            channel1,
        ]);
    ...
    it("sortChannelsByRecency", () => {
        ...
        const lastPosts: Record<string, Post> = {
            channel_a: TestHelper.fakePostOverride({
                id: "post_id_1",
                create_at: 5,
                update_at: 5,
            }),
            channel_b: TestHelper.fakePostOverride({
                id: "post_id_2",
                create_at: 7,
                update_at: 7,
            }),
    ...
    it("sortChannelsByTypeListAndDisplayName", () => {
        ...
        let sortfn = sortChannelsByTypeListAndDisplayName.bind(null, "en", [
        ...
        sortfn = sortChannelsByTypeListAndDisplayName.bind(null, "en", [
            General.DM_CHANNEL,
            General.GM_CHANNEL,
            General.PRIVATE_CHANNEL,
        ]);
